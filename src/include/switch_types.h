...
#define SWITCH_SEQ_DEFAULT_COLOR \
  SWITCH_SEQ_ESC SWITCH_SEQ_END_COLOR /* Reset to Default fg/bg color */
...
#define SWITCH_SEQ_FYELLOW \
  SWITCH_SEQ_ESC SWITCH_SEQ_F_YELLOW SWITCH_SEQ_END_COLOR
...
#define SWITCH_SEQ_BYELLOW \
  SWITCH_SEQ_ESC SWITCH_SEQ_B_YELLOW SWITCH_SEQ_END_COLOR
...
#define SWITCH_SEQ_CLEARSCR \
  SWITCH_SEQ_ESC SWITCH_SEQ_CLEARSCR_CHAR SWITCH_SEQ_HOME
#endif
...
#define SWITCH_DEFAULT_DIR_PERMS                                     \
  SWITCH_FPROT_UREAD | SWITCH_FPROT_UWRITE | SWITCH_FPROT_UEXECUTE | \
      SWITCH_FPROT_GREAD | SWITCH_FPROT_GEXECUTE
#ifdef WIN32
...
typedef enum {
  ...
  SCF_NO_NEW_SESSIONS =
      (SCF_NO_NEW_OUTBOUND_SESSIONS | SCF_NO_NEW_INBOUND_SESSIONS),
...
<pre>
SWITCH_DATABASE_FLAG_ROW_SIZE_LIMIT = (1 <<  0) - Indicates that database has
got row-size limit for the combined sizes of all columns.
</pre>
 */
typedef enum {
...
/* WARNING, Do not forget to update *SAY_METHOD_NAMES[] in
...
typedef enum { SMA_NONE, SMA_GET, SMA_SET } switch_management_action_t;
...
typedef enum {
  SWITCH_ABC_TYPE_INIT,   // bug 初始化时调用
  SWITCH_ABC_TYPE_READ,   // 读取音频流（入站音频，对方说话）应用场景:
                          // 录音、语音识别、音频分析等
  SWITCH_ABC_TYPE_WRITE,  // 写入音频流（出站音频,本方说话）
                          // 处理要发送给对方的音频数据（本方说话的声音）
  SWITCH_ABC_TYPE_WRITE_REPLACE,     // 替换写入流（修改发送给对方的音频）
  SWITCH_ABC_TYPE_READ_REPLACE,      // 替换读取流（修改从对方接收的音频）
  SWITCH_ABC_TYPE_READ_PING,         // 入站音频到达（对方说话），仅监听
                                     // 监听音频流但不修改数据
  SWITCH_ABC_TYPE_TAP_NATIVE_READ,   // 获取原生编码格式的读取流
  SWITCH_ABC_TYPE_TAP_NATIVE_WRITE,  // 获取原生编码格式的写入流
  SWITCH_ABC_TYPE_CLOSE,             // Bug 关闭时调用（销毁前）
  SWITCH_ABC_TYPE_READ_VIDEO_PING,  // 入站视频帧到达，视频活动检测、视频质量监控
  SWITCH_ABC_TYPE_READ_VIDEO_RAW,   // 入站视频原始RTP到达（在解码前） 新增
  SWITCH_ABC_TYPE_WRITE_VIDEO_PING,   // 写入视频流 ping 视频输出状态监控
  SWITCH_ABC_TYPE_STREAM_VIDEO_PING,  // 视频流 ping（用于监控）
  SWITCH_ABC_TYPE_VIDEO_PATCH,        // 视频补丁（修改视频帧）
  SWITCH_ABC_TYPE_READ_TEXT           // 读取文本流（如 RTT/实时文本）
} switch_abc_type_t;
...
#define SWITCH_ACCEPTABLE_INTERVAL(_i) \
  (_i && _i <= SWITCH_MAX_INTERVAL && (_i % 10) == 0)
...
typedef enum { SWITCH_RW_READ, SWITCH_RW_WRITE } switch_rw_t;
...
<pre>
    ...
    SWITCH_VAD_FLAG_EVENTS_TALK     - Fire events when talking is detected
        SWITCH_VAD_FLAG_EVENTS_NOTALK   - Fire events when not talking is
detected SWITCH_VAD_FLAG_CNG				- Send CNG
</pre>
 */
typedef enum {
...
typedef struct {
  ...
  uint32_t peer_ssrc;
  uint32_t last_rpt_ts; /* RTP timestamp at which the last report was generated
                           and sent */
  uint32_t ssrc;        /* identifier of the source */
  uint32_t csrc;        /* contributing source 0-15 32bit each */
  uint32_t last_pkt_tsdiff; /* Jitter calculation, timestamp difference between
                               the two last received packet */
  double inter_jitter;      /* Jitter calculation, Interarrival jitter */
  uint32_t
      last_rpt_ext_seq; /* Packet loss calculation, extended sequence number at
                           the begining of this RTCP report interval */
  uint16_t
      last_rpt_cycle; /* Packet loss calculation, sequence number cycle at the
                         begining of the current RTCP report interval */
  uint16_t period_pkt_count; /* Packet loss calculation, packet count received
                                during this RTCP report interval */
  uint16_t pkt_count; /* Packet loss calculation, packet count received during
                         this session */
  uint16_t sent_pkt_count;
  uint32_t rtcp_rtp_count; /* RTCP report generated count */
  uint32_t
      high_ext_seq_recv; /* Packet loss calculation, highest extended sequence
                            number received and processed for stats */
  uint16_t cycle;   /* Packet loss calculation, sequence number cycle of the
                       current RTCP report interval */
  uint32_t bad_seq; /* Bad SEQ found, used to detect reset on the other side */
  uint16_t
      base_seq; /* Packet loss calculation, first sequence number received */
  uint32_t
      cum_lost; /* Packet loss calculation, cumulative number of packet lost */
  uint32_t
      last_recv_lsr_local; /* RTT calculation, When receiving an SR we save our
                              local timestamp in fraction of 65536 seconds */
  uint32_t last_recv_lsr_peer; /* RTT calculation, When receiving an SR we
                                  extract the middle 32bits of the remote NTP
                                  timestamp to include it in the next SR LSR */
  uint32_t init;
...
<pre>
    ...
    SWITCH_RTP_FLAG_IO            - IO is ready
        SWITCH_RTP_FLAG_USE_TIMER     - Timeout Reads and replace with a CNG
Frame SWITCH_RTP_FLAG_SECURE        - Secure RTP SWITCH_RTP_FLAG_AUTOADJ       -
Auto-Adjust the dest based on the source SWITCH_RTP_FLAG_RAW_WRITE     - Try to
forward packets unscathed SWITCH_RTP_FLAG_GOOGLEHACK    - Convert payload from
102 to 97 SWITCH_RTP_FLAG_VAD           - Enable VAD SWITCH_RTP_FLAG_BREAK
- Stop what you are doing and return SWITCH_STATUS_BREAK
        SWITCH_RTP_FLAG_DATAWAIT	  - Do not return from reads unless
there is data even when non blocking SWITCH_RTP_FLAG_BUGGY_2833    - Emulate the
bug in cisco equipment to allow interop SWITCH_RTP_FLAG_PASS_RFC2833  - Pass
2833 (ignore it) SWITCH_RTP_FLAG_AUTO_CNG      - Generate outbound CNG frames
when idle
</pre>
 */
typedef enum {
...
typedef enum {
  ...
  /* Some Cisco devices get mad when you send the mark bit on new 2833 because
     it makes them flush their jitterbuffer and the dtmf along with it.

     This flag will disable the sending of the mark bit on the first DTMF
     packet.
   */
  ...
  /*
     ...
     The sender should increment the RTP timestamp in each packet when, in
     reality, the sender should send the same exact timestamp and increment the
     duration field in the 2833 payload. This allows a reconstruction of the
     duration if any of the packets are lost.

     final_duration - initial_timestamp = total_samples

     However, if the duration value exceeds the space allocated (16 bits), The
     sender should increment the timestamp one unit and reset the duration to 0.

     Always sending a duration of 0 with a new timestamp should be tolerated but
     is rarely intentional and is mistakenly done by many devices. The issue is
     that the Sonus expects everyone to do it this way instead of tolerating
     either way. Sonus will actually ignore every packet with the same timestamp
  ...
  /*
    A Huawei SBC has been discovered that sends the mark bit on every single RTP
    packet. Since this causes the RTP stack to flush it's buffers, it horribly
  ...
  /*
    Our friends at Sonus get real mad when the timestamps are not in perfect
    sequence even during periods of silence. With this flag, we will only
    increment the timestamp when write packets even if they are eons apart.
  ...
  /*
    Our friends at Sonus also get real mad if the sequence number does not start
  ...
  /*
    Our friends at Sonus are on a roll, They also get easily dumbfounded by
    marker bits. This flag will never send any. Sheesh....
   */

  RTP_BUG_IGNORE_DTMF_DURATION = (1 << 6),

  /*
    Guess Who? ... Yep, Sonus (and who know's who else) likes to interweave DTMF
    with the audio stream making it take 2X as long as it should and sending an
    incorrect duration making the DTMF very delayed. This flag will treat every
    dtmf as if it were 50ms and queue it on recipt of the leading packet rather
    than at the end.
   */
  ...
  /*
    Oracle's Contact Center Anywhere (CCA) likes to use a single RTP socket to
    send all its outbound audio. This messes up our ability to auto adjust to
    NATTED RTP and causes us to ignore its audio packets. This flag will allow
    compatibility with this dying product.
  */

  RTP_BUG_GEN_ONE_GEN_ALL = (1 << 8),

  /*
    Some RTP endpoints (and by some we mean *cough* _SONUS_!) do not like it
    when the timestamps jump forward or backwards in time. So say you are
    generating a file that says "please wait for me to complete your call, or
    generating ringback" Now you place and outbound call and you are bridging.
    Well, while you were playing the file, you were generating your own RTP
    timestamps. But, now that you have a remote RTP stream, you'd rather send
    those timestamps as-is in case they will be fed to a remote jitter
    buffer...... Ok, so this causes the audio to completely fade out despite the
    fact that we send the mark bit which should give them heads up its
    happening.

    Sigh, This flag will tell FreeSWITCH that if it ever generates even one RTP
    packet itself, to continue to generate all of them and ignore the actual
    timestamps in the frames.
  ...
  /*
    By default FS will change the SSRC when the marker is set and it detects a
    timestamp reset. If this setting is enabled it will NOT do this (old
    behaviour).
   */
  ...
  /*
    Make FS accept any payload type instead of dropping and returning CNG frame.
    Workaround while FS only supports a single payload per rtp session. This can
    be used by endpoint modules to detect payload changes and act appropriately
    (ex: sofia could send a reINVITE with single codec). This should probably be
    a flag, but flag enum is already full!
  */

  RTP_BUG_ALWAYS_AUTO_ADJUST = (1 << 12)

  /*
    Leave the auto-adjust behavior enableed permenantly rather than only at
    appropriate times.  (IMPLICITLY sets RTP_BUG_ACCEPT_ANY_PACKETS)
...
<pre>
    ...
    SWITCH_IVR_OPTION_ASYNC - Asynchronous (do things in the background when
applicable) SWITCH_IVR_OPTION_FILE  - string argument implies a filename
</pre>
 */
typedef enum {
...
<pre>
        SWITCH_MESSAGE_REDIRECT_AUDIO     - Indication to redirect audio to
another location if possible SWITCH_MESSAGE_TRANSMIT_TEXT      - A text message
        ...
        SWITCH_MESSAGE_INDICATE_VIDEO_REFRESH_REQ - indicate video refresh
request
</pre>
 */
typedef enum {
...
<pre>
    SWITCH_STATUS_SUCCESS	- General Success (common return value for most
functions) SWITCH_STATUS_FALSE		- General Falsehood
    SWITCH_STATUS_TIMEOUT	- A Timeout has occured
    SWITCH_STATUS_RESTART	- An indication to restart the previous
operation SWITCH_STATUS_TERM		- An indication to terminate
    SWITCH_STATUS_NOTIMPL	- An indication that requested resource is not
impelemented SWITCH_STATUS_MEMERR	- General memory error
    ...
    SWITCH_STATUS_INUSE		- An indication that requested resource is in
use SWITCH_STATUS_BREAK     - A non-fatal break of an operation
...
SWITCH_CHANNEL_ID_LOG_CLEAN		- Write to the currently defined log
with no extra file/line/date information SWITCH_CHANNEL_ID_EVENT
- Write to the event engine as a LOG event
</pre>
 */
typedef enum {
...
static inline switch_core_session_t* switch_core_session_type_check(
    switch_core_session_t* session) {
  return session;
}
static inline const char* switch_const_char_type_check(const char* str) {
  return str;
}
...
#define SWITCH_CHANNEL_LOG_CLEAN \
  SWITCH_CHANNEL_ID_LOG_CLEAN, __FILE__, __SWITCH_FUNC__, __LINE__, NULL
#define SWITCH_CHANNEL_SESSION_LOG_CLEAN(x)                         \
  SWITCH_CHANNEL_ID_LOG_CLEAN, __FILE__, __SWITCH_FUNC__, __LINE__, \
      switch_core_session_get_uuid((x))
#define SWITCH_CHANNEL_EVENT \
  SWITCH_CHANNEL_ID_EVENT, __FILE__, __SWITCH_FUNC__, __LINE__, NULL
#define SWITCH_CHANNEL_SESSION_LOG(x)                             \
  SWITCH_CHANNEL_ID_SESSION, __FILE__, __SWITCH_FUNC__, __LINE__, \
      (const char*)switch_core_session_type_check(x)
#define SWITCH_CHANNEL_CHANNEL_LOG(x)                             \
  SWITCH_CHANNEL_ID_SESSION, __FILE__, __SWITCH_FUNC__, __LINE__, \
      (const char*)switch_channel_get_session(x)
#define SWITCH_CHANNEL_UUID_LOG(x)                            \
  SWITCH_CHANNEL_ID_LOG, __FILE__, __SWITCH_FUNC__, __LINE__, \
      switch_const_char_type_check(x)
...
/*!
  ...
  \brief Channel States (these are the defaults, CS_SOFT_EXECUTE,
CS_EXCHANGE_MEDIA, and CS_CONSUME_MEDIA are often overridden by specific apps)
<pre>
...
CS_CONSUME_MEDIA		 - Channel is consuming all media and dropping
it. CS_HIBERNATE - Channel is in a sleep state. CS_RESET 	 - Channel is in
a reset state. CS_HANGUP    - Channel is flagged for hangup and ready to end.
...
typedef enum {
  ...
  CF_DEVICES_CHANGED,
  /* WARNING: DO NOT ADD ANY FLAGS BELOW THIS LINE */
  /* IF YOU ADD NEW ONES CHECK IF THEY SHOULD PERSIST OR ZERO THEM IN
     switch_core_session.c switch_core_session_request_xml() */
  CF_FLAG_MAX
...
SFF_WAIT_KEY_FRAME = (1 << 12) - Need a key from before could decode, or force
generate a key frame on encode
</pre>
 */
typedef enum {
  SFF_NONE = 0,               // 0(0x0000) = 0x00000 = 0
  SFF_CNG = (1 << 0),         // 1(0x0001) 舒适噪音 用途：在混音、转发、文本处理等场景中跳过或特殊处理
  SFF_RAW_RTP = (1 << 1),     // 2(0x0002) 包含原始的rtp负载
  SFF_RTP_HEADER = (1 << 2),  // 4(0x0004)包含rtp头
  SFF_PLC = (1 << 3),  //  8(0x0008) 这个音频帧是PLC生成的，不是从网络传过来的，而是合成出来用来掩盖丢包的。编码器会检测并清除它；有些处理（比如DTMF提取）可能会对它有不同的处理方式。
  SFF_RFC2833 = (1 << 4),       // 16(0x0010) Frame carries RFC2833 DTMF data
  SFF_PROXY_PACKET = (1 << 5),  // 32(0x0020) 帧应以原样转发（proxy），绕过常规处理（例如直接转发 RTP、UDPTL）
  SFF_DYNAMIC = (1 << 6), // 64(0x0040) 表示帧数据是动态分配的，需要由帧析构时进行释放/管理。
  SFF_UDPTL_PACKET = (1 << 7), // 128(0x0080) 帧包含 UDPTL（T.38）数据／传真包。
  SFF_NOT_AUDIO = (1 << 8), // 256(0x0100) 表示该帧不是音频帧（即便处在音频流中），用于区分控制包、传真等非音频类型
  SFF_RTCP = (1 << 9), // 512 (0x0200) 表示帧是 RTCP 包
  SFF_MARKER = (1 << 10), // 1024 (0x0400) 表示 RTP 的 Marker 标志被置位（通常由编码器设置）
  SFF_WAIT_KEY_FRAME = (1 << 11), // 2048 (0x0800) 表示需要等待关键帧才能解码/处理（通常用于切换编码器或从冻结状态恢复）。
  SFF_RAW_RTP_PARSE_FRAME = (1 << 12), // 4096 (0x1000) 表示这是要被解析的原始 RTP 帧（例如需要解析 payload 结构或者按 RTP 拆分）。
  SFF_PICTURE_RESET = (1 << 13), // 8192 (0x2000) 表示需要重置解码/视频渲染状态（可能要求发送关键帧或重新初始化）。
  SFF_SAME_IMAGE = (1 << 14),  // 16384 (0x4000) 视频帧与上一帧图像相同（没有视觉变化），以优化避免重复编码/发送
  SFF_USE_VIDEO_TIMESTAMP = (1 << 15), // 32768 (0x8000) 使用视频帧内的时间戳进行同步/处理（而不是系统时间）。
  SFF_ENCODED = (1 << 16), // 65536 (0x10000) 帧负载已经是编码后的，管道无需再做重新编码（例如 H.264 NALs）
  SFF_TEXT_LINE_BREAK = (1 << 17), // 131072 (0x20000) 文本帧表示换行（用于聊天 / T.140 文本分段）。
  SFF_IS_KEYFRAME = (1 << 18), // 262144 (0x40000) 帧是关键帧（I-frame），用于判定解码边界或转发决策。
  SFF_EXTERNAL = (1 << 19) // 524288 (0x80000) 帧由外部注入（external source）创建的标记，通常表示源不是内部生成。
...
typedef enum { SCAF_NONE = 0 } switch_chat_application_flag_enum_t;
...
SWITCH_CODEC_FLAG_FREE_POOL =		(1 <<  5) - Free codec's pool on
destruction SWITCH_CODEC_FLAG_AAL2 =			(1 <<  6) - USE AAL2
Bitpacking SWITCH_CODEC_FLAG_PASSTHROUGH =		(1 <<  7) - Passthrough
only
</pre>
*/
typedef enum {
...
<pre>
SWITCH_SPEECH_FLAG_HASTEXT =		(1 <<  0) - Interface is has text to
read. SWITCH_SPEECH_FLAG_PEEK =			(1 <<  1) - Read data but do not
erase it. SWITCH_SPEECH_FLAG_FREE_POOL =		(1 <<  2) - Free
interface's pool on destruction. SWITCH_SPEECH_FLAG_BLOCKING =       (1 <<  3) -
Indicate that a blocking call is desired SWITCH_SPEECH_FLAG_PAUSE =
(1 <<  4) - Pause toggle for playback
</pre>
*/
typedef enum {
...
<pre>
SWITCH_DIRECTORY_FLAG_FREE_POOL =		(1 <<  0) - Free interface's
pool on destruction.
</pre>
*/
typedef enum {
...
<pre>
SWITCH_TIMER_FLAG_FREE_POOL =		(1 <<  0) - Free timer's pool on
destruction
</pre>
*/
typedef enum {
...
SMBF_PRUNE -
SMBF_NO_PAUSE -
SMBF_STEREO_SWAP - Record in stereo: Write Stream - left channel, Read Stream -
right channel
</pre>
*/
typedef enum {
  SMBF_BOTH = 0,                  // 默认值，同时处理读和写流
  SMBF_READ_STREAM = (1 << 0),    // 拦截读取流（接收到的音频数据）
  SMBF_WRITE_STREAM = (1 << 1),   // 拦截写入流（发送的音频数据）
  SMBF_WRITE_REPLACE = (1 << 2),  // 替换写入流数据，可以修改发送的音频
  SMBF_READ_REPLACE = (1 << 3),   // 替换读取流数据，可以修改接收到的音频
  SMBF_READ_PING = (1 << 4),      // 仅拦截读取流的ping帧
  SMBF_STEREO = (1 << 5),         // 以立体声格式处理音频流
  SMBF_ANSWER_REQ = (1 << 6),     // 仅在通道被应答后才开始处理音频流
  SMBF_BRIDGE_REQ = (1 << 7),     // 仅在通道被桥接后才开始处理音频流
  SMBF_THREAD_LOCK = (1 << 8),    // 仅允许创建该bug的线程移除它
  SMBF_PRUNE = (1 << 9),          // 修剪/清理标志
  SMBF_NO_PAUSE = (1 << 10),      // 不允许暂停处理
  SMBF_STEREO_SWAP =
      (1 << 11),  // 以立体声格式处理音频流：写入流-左声道，读取流-右声道
  SMBF_LOCK = (1 << 12),              // 锁定标志，防止并发访问
  SMBF_TAP_NATIVE_READ = (1 << 13),   // 以原生格式拦截读取流
  SMBF_TAP_NATIVE_WRITE = (1 << 14),  // 以原生格式拦截写入流
  SMBF_ONE_ONLY = (1 << 15),          // 仅处理一次的标志
  SMBF_MASK = (1 << 16),              // 掩码标志
  SMBF_READ_VIDEO_PING = (1 << 17),   // 仅拦截读取流的视频ping帧
  SMBF_READ_VIDEO_RAW = (1 << 28),  // 在解码前拷贝原始RTP并发送给media_bug 新增
  SMBF_WRITE_VIDEO_PING = (1 << 18),       // 仅拦截写入流的视频ping帧
  SMBF_READ_VIDEO_STREAM = (1 << 19),      // 仅拦截读取流的视频流
  SMBF_WRITE_VIDEO_STREAM = (1 << 20),     // 仅拦截写入流的视频流
  SMBF_VIDEO_PATCH = (1 << 21),            // 视频修补标志
  SMBF_SPY_VIDEO_STREAM = (1 << 22),       // 监视视频流
  SMBF_SPY_VIDEO_STREAM_BLEG = (1 << 23),  // 监视B腿的视频流
  SMBF_READ_VIDEO_PATCH = (1 << 24),       // 仅拦截读取流的视频修补帧
  SMBF_READ_TEXT_STREAM = (1 << 25),       // 仅拦截读取流的文本流
  SMBF_FIRST = (1 << 26),                  // 仅处理第一个数据包
  SMBF_PAUSE = (1 << 27)                   // 暂停处理标志
...
SWITCH_FILE_FLAG_FREE_POOL =    (1 <<  2) - Free file handle's pool on
destruction SWITCH_FILE_DATA_SHORT =        (1 <<  3) - Read data in shorts
...
SWITCH_FILE_NATIVE =            (1 <<  9) - File is in native format (no
transcoding) SWITCH_FILE_SEEK = 				(1 << 10) - File
has done a seek SWITCH_FILE_OPEN =              (1 << 11) - File is open
</pre>
 */
typedef enum {
...
<pre>
    ...
    SWITCH_EVENT_CHANNEL_HANGUP_COMPLETE	- A channel has completed the
hangup SWITCH_EVENT_CHANNEL_EXECUTE	- A channel has executed a module's
application SWITCH_EVENT_CHANNEL_EXECUTE_COMPLETE	- A channel has finshed
executing a module's application SWITCH_EVENT_CHANNEL_HOLD		- A
channel has been put on hold SWITCH_EVENT_CHANNEL_UNHOLD		- A
channel has been unheld SWITCH_EVENT_CHANNEL_BRIDGE     - A channel has bridged
to another channel SWITCH_EVENT_CHANNEL_UNBRIDGE   - A channel has unbridged
from another channel SWITCH_EVENT_CHANNEL_PROGRESS	- A channel has started
ringing SWITCH_EVENT_CHANNEL_PROGRESS_MEDIA	- A channel has started early
media SWITCH_EVENT_CHANNEL_OUTGOING	- A channel has been unparked
    SWITCH_EVENT_CHANNEL_PARK 		- A channel has been parked
    SWITCH_EVENT_CHANNEL_UNPARK 	- A channel has been unparked
    SWITCH_EVENT_CHANNEL_APPLICATION- A channel has called and event from an
application SWITCH_EVENT_CHANNEL_ORIGINATE  - A channel has been originated
    ...
    SWITCH_EVENT_OUTBOUND_CHAN		- A new outbound channel has been
created SWITCH_EVENT_STARTUP			- The system has been started
    ...
    SWITCH_EVENT_NOTIFY_IN			- Received incoming NOTIFY from
gateway subscription SWITCH_EVENT_PRESENCE_OUT		- Presence out
    ...
    SWITCH_EVENT_RE_SCHEDULE		- Something scheduled has been
rescheduled SWITCH_EVENT_RELOADXML			- XML registry has been
reloaded SWITCH_EVENT_NOTIFY				- Notification
    ...
    SWITCH_EVENT_FAILURE            - A failure occurred which might impact the
normal functioning of the switch SWITCH_EVENT_SOCKET_DATA
    ...
    SWITCH_EVENT_SHUTDOWN_REQUESTED		- Shutdown of the system has
been requested SWITCH_EVENT_ALL				- All events at once
</pre>
...
typedef enum {
  SWITCH_EVENT_CUSTOM, 
  SWITCH_EVENT_CLONE, 
  SWITCH_EVENT_CHANNEL_CREATE, // 通道创建事件
  SWITCH_EVENT_CHANNEL_DESTROY, // 通道销毁事件
  SWITCH_EVENT_CHANNEL_STATE, // 通道状态改变事件
  SWITCH_EVENT_CHANNEL_CALLSTATE, // 通话状态变化，比如 ringing、active、held
  SWITCH_EVENT_CHANNEL_ANSWER, // 通道被应答事件
  SWITCH_EVENT_CHANNEL_HANGUP, // 通道挂断事件
  SWITCH_EVENT_CHANNEL_HANGUP_COMPLETE, // 挂断流程完成
  SWITCH_EVENT_CHANNEL_EXECUTE, // 开始执行某个应用
  SWITCH_EVENT_CHANNEL_EXECUTE_COMPLETE, // 通道完成执行模块应用事件
  SWITCH_EVENT_CHANNEL_HOLD, // 进入保持状态事件
  SWITCH_EVENT_CHANNEL_UNHOLD, // 取消保持事件
  SWITCH_EVENT_CHANNEL_BRIDGE, // 通道进入桥接，和另一通道建立桥
  SWITCH_EVENT_CHANNEL_UNBRIDGE, // 解除桥接
  SWITCH_EVENT_CHANNEL_PROGRESS, // 会议中 进入振铃/进展阶段，通常对应你理解的 ringing
  SWITCH_EVENT_CHANNEL_PROGRESS_MEDIA, //  进入早期媒体，已经能收到媒体但还没正式应答
  SWITCH_EVENT_CHANNEL_OUTGOING, // 通道被 unpark 或开始向外发送
  SWITCH_EVENT_CHANNEL_PARK, // 通道被 park 事件
  SWITCH_EVENT_CHANNEL_UNPARK, // 通道被 unpark 事件
  SWITCH_EVENT_CHANNEL_APPLICATION, // 通道触发了应用相关事件
  SWITCH_EVENT_CHANNEL_ORIGINATE, // 通道发起外呼
  SWITCH_EVENT_CHANNEL_UUID, // 通道 UUID 发生变化
  // 以下为系统级事件
  SWITCH_EVENT_API, // API 调用事件
  SWITCH_EVENT_LOG, // 日志事件
  SWITCH_EVENT_INBOUND_CHAN, // 新的入站通道创建事件
  SWITCH_EVENT_OUTBOUND_CHAN, // 新的出站通道创建事件
  SWITCH_EVENT_STARTUP,
  SWITCH_EVENT_SHUTDOWN, //
  SWITCH_EVENT_PUBLISH, // 发布事件
  SWITCH_EVENT_UNPUBLISH, // 取消发布事件
  SWITCH_EVENT_TALK, // 说话检测事件
  SWITCH_EVENT_NOTALK, // 未说话检测事件
  SWITCH_EVENT_SESSION_CRASH, // 会话崩溃事件
  SWITCH_EVENT_MODULE_LOAD,
  SWITCH_EVENT_MODULE_UNLOAD,
  SWITCH_EVENT_DTMF,
  SWITCH_EVENT_MESSAGE, // 基本消息事件
  SWITCH_EVENT_PRESENCE_IN, // presence 进
  SWITCH_EVENT_NOTIFY_IN, // 接收到来自网关的 NOTIFY 消息
  SWITCH_EVENT_PRESENCE_OUT, // presence 出
  SWITCH_EVENT_PRESENCE_PROBE, //presence 探测
  SWITCH_EVENT_MESSAGE_WAITING,
  SWITCH_EVENT_MESSAGE_QUERY,
  SWITCH_EVENT_ROSTER,
  SWITCH_EVENT_CODEC, // 编解码器变化事件
  SWITCH_EVENT_BACKGROUND_JOB,
  SWITCH_EVENT_DETECTED_SPEECH,
  SWITCH_EVENT_DETECTED_TONE,
  SWITCH_EVENT_PRIVATE_COMMAND,
  SWITCH_EVENT_HEARTBEAT, // 心跳事件
  SWITCH_EVENT_TRAP, // 错误捕获事件
  SWITCH_EVENT_ADD_SCHEDULE,
  SWITCH_EVENT_DEL_SCHEDULE,
  SWITCH_EVENT_EXE_SCHEDULE,
  SWITCH_EVENT_RE_SCHEDULE,
  SWITCH_EVENT_RELOADXML, // XML 重新加载事件
  SWITCH_EVENT_NOTIFY, // 通知事件
  SWITCH_EVENT_PHONE_FEATURE, // 电话功能事件（DND/CFWD 等）
  SWITCH_EVENT_PHONE_FEATURE_SUBSCRIBE, // 电话功能订阅事件
  SWITCH_EVENT_SEND_MESSAGE, // 消息发送事件
  SWITCH_EVENT_RECV_MESSAGE, // 消息接收事件
  SWITCH_EVENT_REQUEST_PARAMS, // 请求参数事件
  SWITCH_EVENT_CHANNEL_DATA,
  SWITCH_EVENT_GENERAL, // 一般事件
  SWITCH_EVENT_COMMAND, // 命令事件
  SWITCH_EVENT_SESSION_HEARTBEAT,
  SWITCH_EVENT_CLIENT_DISCONNECTED,
  SWITCH_EVENT_SERVER_DISCONNECTED,
  SWITCH_EVENT_SEND_INFO,
  SWITCH_EVENT_RECV_INFO,
  SWITCH_EVENT_RECV_RTCP_MESSAGE, // RTCP消息接收事件
  SWITCH_EVENT_SEND_RTCP_MESSAGE, // RTCP消息发送事件
  SWITCH_EVENT_CALL_SECURE,
  SWITCH_EVENT_NAT, // NAT 管理事件（new/del/status）
  SWITCH_EVENT_RECORD_START, // 录音开始事件
  SWITCH_EVENT_RECORD_STOP, // 录音停止事件
  SWITCH_EVENT_PLAYBACK_START, // 播放开始事件
  SWITCH_EVENT_PLAYBACK_STOP, // 播放停止事件
  SWITCH_EVENT_CALL_UPDATE, // 通话更新事件
  SWITCH_EVENT_FAILURE, // 发生可能影响系统正常运行的故障事件
  SWITCH_EVENT_SOCKET_DATA, // 套接字数据事件
  SWITCH_EVENT_MEDIA_BUG_START,
  SWITCH_EVENT_MEDIA_BUG_STOP,
  SWITCH_EVENT_CONFERENCE_DATA_QUERY,
  SWITCH_EVENT_CONFERENCE_DATA,
  ...
  SWITCH_EVENT_CALL_SETUP_RESULT,
  SWITCH_EVENT_CALL_DETAIL,
  SWITCH_EVENT_DEVICE_STATE, // 设备状态事件
  SWITCH_EVENT_TEXT, // 文本事件
  SWITCH_EVENT_SHUTDOWN_REQUESTED, // 系统关闭请求事件
  SWITCH_EVENT_ALL // 代表所有事件
...
typedef struct switch_loadable_module_interface
...
typedef struct switch_chat_application_interface
...
typedef struct switch_console_callback_match_node
...
typedef void (*switch_media_bug_exec_cb_t)(switch_media_bug_t* bug,
                                           void* user_data);

typedef switch_status_t (*switch_core_video_thread_callback_func_t)(
    switch_core_session_t* session, switch_frame_t* frame, void* user_data);
typedef switch_status_t (*switch_core_text_thread_callback_func_t)(
    switch_core_session_t* session, switch_frame_t* frame, void* user_data);
typedef void (*switch_cap_callback_t)(const char* var, const char* val,
                                      void* user_data);
typedef switch_status_t (*switch_console_complete_callback_t)(
    const char*, const char*, switch_console_callback_match_t** matches);
typedef switch_bool_t (*switch_media_bug_callback_t)(switch_media_bug_t*, void*,
                                                     switch_abc_type_t);
typedef switch_bool_t (*switch_tone_detect_callback_t)(switch_core_session_t*,
                                                       const char*,
                                                       const char*);
typedef struct switch_xml_binding switch_xml_binding_t;

typedef void (*switch_engine_function_t)(switch_core_session_t* session,
                                         void* user_data);

typedef switch_status_t (*switch_core_codec_encode_func_t)(
    switch_codec_t* codec, switch_codec_t* other_codec, void* decoded_data,
    uint32_t decoded_data_len, uint32_t decoded_rate, void* encoded_data,
    uint32_t* encoded_data_len, uint32_t* encoded_rate, unsigned int* flag);

typedef switch_status_t (*switch_core_codec_decode_func_t)(
    switch_codec_t* codec, switch_codec_t* other_codec, void* encoded_data,
...
typedef switch_status_t (*switch_core_codec_video_encode_func_t)(
    switch_codec_t* codec, switch_frame_t* frame);

typedef switch_status_t (*switch_core_codec_video_decode_func_t)(
    switch_codec_t* codec, switch_frame_t* frame);
...
typedef enum { SWITCH_IO_READ, SWITCH_IO_WRITE } switch_io_type_t;

typedef switch_status_t (*switch_core_codec_control_func_t)(
    switch_codec_t* codec, switch_codec_control_command_t cmd,
...
typedef switch_status_t (*switch_core_codec_init_func_t)(
    switch_codec_t*, switch_codec_flag_t,
    const switch_codec_settings_t* codec_settings);
typedef switch_status_t (*switch_core_codec_fmtp_parse_func_t)(
    const char* fmtp, switch_codec_fmtp_t* codec_fmtp);
typedef switch_status_t (*switch_core_codec_destroy_func_t)(switch_codec_t*);

typedef switch_status_t (*switch_chat_application_function_t)(switch_event_t*,
                                                              const char*);
#define SWITCH_STANDARD_CHAT_APP(name) \
  static switch_status_t name(switch_event_t* message, const char* data)

typedef void (*switch_application_function_t)(switch_core_session_t*,
                                              const char*);
...
typedef switch_caller_extension_t* (*switch_dialplan_hunt_function_t)(
    switch_core_session_t*, void*, switch_caller_profile_t*);
#define SWITCH_STANDARD_DIALPLAN(name)           \
  static switch_caller_extension_t* name(        \
      switch_core_session_t* session, void* arg, \
      switch_caller_profile_t* caller_profile)

typedef switch_bool_t (*switch_hash_delete_callback_t)(_In_ const void* key,
                                                       _In_ const void* val,
                                                       _In_opt_ void* pData);
#define SWITCH_HASH_DELETE_FUNC(name) \
  static switch_bool_t name(const void* key, const void* val, void* pData)
...
#define SWITCH_STANDARD_SCHED_FUNC(name) \
  static void name(switch_scheduler_task_t* task)

typedef switch_status_t (*switch_state_handler_t)(switch_core_session_t*);
typedef struct switch_stream_handle switch_stream_handle_t;
typedef uint8_t* (*switch_stream_handle_read_function_t)(
    switch_stream_handle_t* handle, int* len);
...
typedef switch_status_t (*switch_stream_handle_raw_write_function_t)(
    switch_stream_handle_t* handle, uint8_t* data, switch_size_t datalen);
...
#define SWITCH_STANDARD_API(name)                                      \
  static switch_status_t name(_In_opt_z_ const char* cmd,              \
                              _In_opt_ switch_core_session_t* session, \
                              _In_ switch_stream_handle_t* stream)

typedef switch_status_t (*switch_json_api_function_t)(
    const cJSON* json, _In_opt_ switch_core_session_t* session,
    cJSON** json_reply);

#define SWITCH_STANDARD_JSON_API(name)                                 \
  static switch_status_t name(const cJSON* json,                       \
                              _In_opt_ switch_core_session_t* session, \
                              cJSON** json_reply)

typedef switch_status_t (*switch_input_callback_function_t)(
    switch_core_session_t* session, void* input, switch_input_type_t input_type,
    void* buf, unsigned int buflen);
typedef switch_status_t (*switch_read_frame_callback_function_t)(
    switch_core_session_t* session, switch_frame_t* frame, void* user_data);
typedef struct switch_say_interface switch_say_interface_t;

#define DMACHINE_MAX_DIGIT_LEN 512

typedef enum { DM_MATCH_POSITIVE, DM_MATCH_NEGATIVE } dm_match_type_t;
...
typedef switch_status_t (*switch_ivr_dmachine_callback_t)(
  ...
  if (_args) {                                                            \
    if (_args->loops >= MAX_ARG_RECURSION) {                              \
      switch_log_printf(                                                  \
          ...
          "RECURSION ERROR!  It's not the best idea to call things that " \
          "collect input recursively from an input callback.\n");         \
...
typedef switch_status_t (*switch_say_string_callback_t)(
    switch_core_session_t* session, char* tosay, switch_say_args_t* say_args,
    char** rstr);

struct switch_say_file_handle;
typedef struct switch_say_file_handle switch_say_file_handle_t;

typedef switch_status_t (*switch_new_say_callback_t)(
    switch_say_file_handle_t* sh, char* tosay, switch_say_args_t* say_args);

typedef struct switch_xml* switch_xml_t;
typedef struct switch_core_time_duration switch_core_time_duration_t;
typedef switch_xml_t (*switch_xml_open_root_function_t)(uint8_t reload,
                                                        const char** err,
                                                        void* user_data);
typedef switch_xml_t (*switch_xml_search_function_t)(
    const char* section, const char* tag_name, const char* key_name,
...
/* 模块API版本号，用于确保模块与核心兼容 */
#define SWITCH_API_VERSION 5

/* 模块加载函数的参数列表：(模块接口指针的地址，内存池指针) */
...
/* 线程优先级枚举 */
typedef enum {
  SWITCH_PRI_LOW = 1,       /* 低优先级 */
  SWITCH_PRI_NORMAL = 10,   /* 普通优先级 */
  SWITCH_PRI_IMPORTANT = 50,/* 重要优先级 */
  SWITCH_PRI_REALTIME = 99, /* 实时优先级 */
} switch_thread_priority_t;

/* 模块标志位枚举 */
typedef enum {
  SMODF_NONE = 0,           /* 无标志 */
  SMODF_GLOBAL_SYMBOLS = (1 << 0)  /* 导出符号为全局可见 */
} switch_module_flag_enum_t;
typedef uint32_t switch_module_flag_t;

/* 可加载模块函数表结构体，包含模块的核心回调函数 */
typedef struct switch_loadable_module_function_table {
  int switch_api_version;      /* API版本号，用于兼容性检查 */
  switch_module_load_t load;   /* 模块加载函数指针 */
  switch_module_shutdown_t shutdown; /* 模块关闭函数指针 */
  switch_module_runtime_t runtime;   /* 模块运行时函数指针 */
  switch_module_flag_t flags;  /* 模块标志位 */
} switch_loadable_module_function_table_t;

typedef int (*switch_modulename_callback_func_t)(void* user_data,
 ...
 * 模块定义宏（扩展版）
 * 参数:
 *   name     - 模块名称（如 mod_sofia）
 *   load     - 模块加载函数指针
 *   shutdown - 模块关闭函数指针
 *   runtime  - 模块运行时函数指针（可为NULL）
 *   flags    - 模块标志位
 * 作用:
 *   1. 定义静态字符串 modname[] = "模块名"
 *   2. 声明并初始化模块接口结构体 name##_module_interface
 */
#define SWITCH_MODULE_DEFINITION_EX(name, load, shutdown, runtime, flags)     \
  static const char modname[] = #name;                                        \
  SWITCH_MOD_DECLARE_DATA switch_loadable_module_function_table_t             \
      name##_module_interface = {SWITCH_API_VERSION, load, shutdown, runtime, \
                                 flags}
 ...
 *   runtime  - 模块运行时函数指针（可为NULL）
 * 说明:
 *   内部调用 SWITCH_MODULE_DEFINITION_EX，flags 默认为 SMODF_NONE
 */
...
typedef void (*switch_event_channel_func_t)(const char* event_channel,
...
/* SDP会话描述类型：OFFER(提议)或ANSWER(应答) */
typedef enum { SDP_OFFER, SDP_ANSWER } switch_sdp_type_t;
...
typedef enum {
  CRYPTO_KEY_PARAM_METHOD_INLINE, /* identified by "inline" chars in SRTP key
                                     parameter */
...
/* If MKI is used, then one or more key-materials are present in the
 * <key-params> section of the crypto attribute. This struct describes the
 * single MKI entry (key-material) within <key-params> section of crypto
 * attribute. Key-material follows the format: "inline:" <key||salt> ["|"
 * lifetime] ["|" MKI ":" length] which translates to "inline:
 * KEYSALT|MKI_ID:MKI_SZ" or "inline: KEYSALT|LIFETIME|MKI_ID:MKI_SZ" */
typedef struct switch_crypto_key_material_s {
  switch_rtp_crypto_key_param_method_type_t method;
  unsigned char
      raw_key[SWITCH_RTP_MAX_CRYPTO_LEN]; /* Key-salt. Master key appended with
                                             salt. Sizes determined by crypto
                                             suite. */
  char* crypto_key;                       /* Complete key material string
                                             ("method:keysalt[|lifetime]|mki"). */
  uint64_t lifetime;     /* OPTIONAL. The lifetime value MAY be written as a
                            non-zero, positive decimal integer or as a power of 2.
                            Must be less than max lifetime of RTP and RTCP packets
                            in given crypto suite. */
  unsigned int mki_id;   /* OPTIONAL. */
  unsigned int mki_size; /* OPTIONAL. Byte length of the master key field in the
                            RTP packet. */
  struct switch_crypto_key_material_s*
      next; /* NULL if this is the last master key in crypto attribute set. */
...
typedef struct secure_settings_s {
   ...
   * Normally single crypto attribute contains one key-material in a
   * <key-params> section, e.g. "inline: KEYSALT" or "inline:
   * KEYSALT|2^LIFETIME_BITS". But if MKI is used, then one or more
   * key-materials are present in the <key-params> section of the crypto
   * attribute. Each key-material follows the format:
   *
   ...
   *
   * This points to singly linked list of key-material descriptions if there is
   * more than one key-material present in this crypto attribute (this key is
   * inserted as the head of the list in that case), or to NULL otherwise.
   */
  struct switch_crypto_key_material_s*
      local_key_material_next;        /* NULL if MKI not used for crypto set on
                                         outbound SRTP. */
  unsigned long local_key_material_n; /* number of key_materials in the linked
                                         list for outbound SRTP */
  struct switch_crypto_key_material_s*
      remote_key_material_next;        /* NULL if MKI not used for crypto set on
                                          inbound SRTP. */
  unsigned long remote_key_material_n; /* number of key_materials in the linked
                                          list for inbound SRTP */
} switch_secure_settings_t;

/* Default MKI index used for packets send from FS. We always use first key if
 * multiple master keys are present in the crypto attribute. */
