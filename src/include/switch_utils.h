...
static inline char* switch_get_hex_bytes(switch_byte_t* buf, switch_size_t datalen, char* new_buf,
...
#define switch_goto_status(_status, _label) \
    status = _status;                       \
    goto _label
#define switch_goto_int(_n, _i, _label) \
    _n = _i;                            \
    goto _label
...
#define switch_normalize_to_16bit(n) \
    if (n > SWITCH_SMAX)             \
        n = SWITCH_SMAX;             \
    else if (n < SWITCH_SMIN)        \
        n = SWITCH_SMIN;
#define switch_codec2str(codec, buf, len)                             \
    snprintf(buf, len, "%s@%uh@%ui", codec->implementation->iananame, \
...
/* find a character (find) in a string (in) and return a pointer to that point in the string where
   the character was found using the array (allowed) as allowed non-matching characters, when
   (allowed) is NULL, behaviour should be identical to strchr()
 */
...
#define switch_is_valid_rate(_tmp)                                                       \
    (_tmp == 8000 || _tmp == 12000 || _tmp == 16000 || _tmp == 24000 || _tmp == 32000 || \
     _tmp == 11025 || _tmp == 22050 || _tmp == 44100 || _tmp == 48000)
...
switch_is_uint_in_range(const char* str, unsigned int from, unsigned int to);
SWITCH_DECLARE(switch_bool_t) switch_is_number(const char* str);
SWITCH_DECLARE(switch_bool_t) switch_is_leading_number(const char* str);
SWITCH_DECLARE(char*)
switch_find_parameter(const char* str, const char* param, switch_memory_pool_t* pool);
...
static inline switch_bool_t switch_true(const char* expr) {
    return ((expr && (!strcasecmp(expr, "yes") || !strcasecmp(expr, "on") ||
                      !strcasecmp(expr, "true") || !strcasecmp(expr, "t") ||
                      !strcasecmp(expr, "enabled") || !strcasecmp(expr, "active") ||
                      !strcasecmp(expr, "allow") || (switch_is_number(expr) && atoi(expr))))
                ? SWITCH_TRUE
                : SWITCH_FALSE);
}
...
#define switch_true_buf(expr)                                                                   \
    (((!strcasecmp(expr, "yes") || !strcasecmp(expr, "on") || !strcasecmp(expr, "true") ||      \
       !strcasecmp(expr, "t") || !strcasecmp(expr, "enabled") || !strcasecmp(expr, "active") || \
       !strcasecmp(expr, "allow") || (switch_is_number(expr) && atoi(expr))))                   \
         ? SWITCH_TRUE                                                                          \
         : SWITCH_FALSE)
...
static inline int switch_false(const char* expr) {
    return ((expr && (!strcasecmp(expr, "no") || !strcasecmp(expr, "off") ||
                      !strcasecmp(expr, "false") || !strcasecmp(expr, "f") ||
                      !strcasecmp(expr, "disabled") || !strcasecmp(expr, "inactive") ||
                      !strcasecmp(expr, "disallow") || (switch_is_number(expr) && !atoi(expr))))
                ? SWITCH_TRUE
                : SWITCH_FALSE);
}
...
*/
SWITCH_DECLARE(switch_status_t)
switch_find_local_ip(_Out_opt_bytecapcount_(len) char* buf, _In_ int len, _In_opt_ int* mask,
                     _In_ int family);
...
*/
SWITCH_DECLARE(switch_status_t)
switch_find_interface_ip(_Out_opt_bytecapcount_(len) char* buf, _In_ int len, _In_opt_ int* mask,
                         _In_ const char* ifname, _In_ int family);
...
SWITCH_DECLARE(char*)
get_addr6(char* buf, switch_size_t len, struct sockaddr_in6* sa, socklen_t salen);

SWITCH_DECLARE(int) get_addr_int(switch_sockaddr_t* sa);
SWITCH_DECLARE(int)
switch_cmp_addr(switch_sockaddr_t* sa1, switch_sockaddr_t* sa2, switch_bool_t ip_only);
 ...
 */
SWITCH_DECLARE(int)
switch_build_uri(char* uri, switch_size_t size, const char* scheme, const char* user,
                 const switch_sockaddr_t* sa, int flags);

#define SWITCH_STATUS_IS_BREAK(x) \
    (x == SWITCH_STATUS_BREAK || x == 730035 || x == 35 || x == SWITCH_STATUS_INTR)
 ...
 */
#define is_dtmf(key)                                                                             \
    ((key > 47 && key < 58) || (key > 64 && key < 69) || (key > 96 && key < 101) || key == 35 || \
     key == 42 || key == 87 || key == 119 || key == 70 || key == 102)
...
*/
#define switch_copy_flags(dest, src, flags) \
    (dest)->flags &= ~(flags);              \
...
*/
static inline int switch_safe_atoi(const char* nptr, int dft) { return nptr ? atoi(nptr) : dft; }
...
switch_separate_string(_In_ char* buf, char delim, _Post_count_(return) char** array,
                       unsigned int arraylen);
SWITCH_DECLARE(unsigned int)
switch_separate_string_string(char* buf, char* delim, _Post_count_(return) char** array,
                              unsigned int arraylen);
...
/*!
  ...
  \param string the string to quote (example: a ' b"' c)
  \param pool a memory pool to use
  \return the quoted string (gives: 'a '\'' b"'\'' c' for unices, "a ' b ' c" for MS Windows), if
  pool not provided, returned value should be freed
*/
SWITCH_DECLARE(char*)
...
static inline void switch_calc_fps(switch_fps_t* fpsP, float fps, int samplerate) {
    /*
      implicit/truncf() - leave us with equal-or-smaller ms and thus equal-or-bigger fps, which is
      better for quality (than roundf()). also equal-or-bigger fps is better for things like
      (int)fps
    */
    fpsP->ms = (int)(1000.0f / fps);
    fpsP->fps = 1000.0f / fpsP->ms;
    fpsP->samples =
        (int)(samplerate / 1000 * fpsP->ms);  // samplerate 99.99% is a factor of 1000, so we safe
                                              // here with integer div by 1000
    return;
}
...
SWITCH_DECLARE(char*)
switch_core_url_encode_opt(switch_memory_pool_t* pool, const char* url,
                           switch_bool_t double_encode);
SWITCH_DECLARE(char*) switch_core_url_encode(switch_memory_pool_t* pool, const char* url);
SWITCH_DECLARE(char*)
switch_core_session_url_encode_opt(switch_core_session_t* session, const char* url,
                                   switch_bool_t double_encode);
SWITCH_DECLARE(char*)
switch_core_session_url_encode(switch_core_session_t* session, const char* url);

SWITCH_DECLARE(switch_bool_t)
switch_simple_email(const char* to, const char* from, const char* headers, const char* body,
                    const char* file, const char* convert_cmd, const char* convert_ext);
SWITCH_DECLARE(char*) switch_find_end_paren(const char* s, char open, char close);

static inline void switch_separate_file_params(const char* file, char** file_portion,
                                               char** params_portion) {
    char* e = NULL;
...
SWITCH_DECLARE(switch_status_t)
switch_network_list_create(switch_network_list_t** list, const char* name,
                           switch_bool_t default_type, switch_memory_pool_t* pool);
SWITCH_DECLARE(switch_status_t)
switch_network_list_add_cidr_token(switch_network_list_t* list, const char* cidr_str,
                                   switch_bool_t ok, const char* token);
...
SWITCH_DECLARE(switch_status_t)
switch_network_list_add_host_mask(switch_network_list_t* list, const char* host,
                                  const char* mask_str, switch_bool_t ok);

SWITCH_DECLARE(switch_status_t)
switch_network_list_add_cidr_port_token(switch_network_list_t* list, const char* cidr_str,
                                        switch_bool_t ok, const char* token,
                                        switch_network_port_range_p port);
SWITCH_DECLARE(switch_status_t)
switch_network_list_add_host_port_mask(switch_network_list_t* list, const char* host,
                                       const char* mask_str, switch_bool_t ok,
                                       switch_network_port_range_p port);

SWITCH_DECLARE(switch_bool_t)
switch_network_list_validate_ip_port_token(switch_network_list_t* list, uint32_t ip, int port,
                                           const char** token);
SWITCH_DECLARE(switch_bool_t)
switch_network_list_validate_ip6_port_token(switch_network_list_t* list, ip_t ip, int port,
                                            const char** token);
SWITCH_DECLARE(switch_bool_t)
switch_network_list_validate_ip_token(switch_network_list_t* list, uint32_t ip, const char** token);
SWITCH_DECLARE(switch_bool_t)
switch_network_list_validate_ip6_token(switch_network_list_t* list, ip_t ip, const char** token);
#define switch_network_list_validate_ip(_list, _ip) \
...
#define switch_malloc(ptr, len)                                                                  \
    (void)((!!(ptr = malloc(len))) ||                                                            \
               (fprintf(stderr, "ABORT! Malloc failure at: %s:%d", __FILE__, __LINE__), abort(), \
                0),                                                                              \
           ptr)
...
#define switch_malloc(ptr, len)                   \
    ...
    __analysis_assume(ptr)
#define switch_zmalloc(ptr, len)                   \
    (void)(assert((ptr = calloc(1, (len)))), ptr); \
    __analysis_assume(ptr)
#define switch_strdup(ptr, s)                  \
    (void)(assert(((ptr) = _strdup(s))), ptr); \
    __analysis_assume(ptr)
#else
...
#define DUMP_EVENT(_e)                                                                   \
    {                                                                                    \
        char* event_str;                                                                 \
        switch_event_serialize(_e, &event_str, SWITCH_FALSE);                            \
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_CRIT, "DUMP\n%s\n", event_str); \
        free(event_str);                                                                 \
    }
 ...
 * \return	Pointer to message buffer, returning error message or "Unknown error xxx" if none
 * found
 */
 ...
 *
 * qs buffer will be modified, so be sure to dup the qs before passing into this function if you
 * want to keep the original string untouched if qs is NULL, the it will parse request->qs,
 * request->qs will be duplicated before parse to avoid being modified
 */

SWITCH_DECLARE(void) switch_http_parse_qs(switch_http_request_t* request, char* qs);

SWITCH_DECLARE(switch_status_t)
switch_frame_buffer_free(switch_frame_buffer_t* fb, switch_frame_t** frameP);
SWITCH_DECLARE(switch_status_t)
switch_frame_buffer_dup(switch_frame_buffer_t* fb, switch_frame_t* orig, switch_frame_t** clone);
SWITCH_DECLARE(switch_status_t) switch_frame_buffer_destroy(switch_frame_buffer_t** fbP);
SWITCH_DECLARE(switch_status_t)
switch_frame_buffer_create(switch_frame_buffer_t** fbP, switch_size_t qlen);
...
SWITCH_DECLARE(switch_status_t)
switch_digest(const char* digest_name, unsigned char** digest, const void* input,
              switch_size_t inputLen, unsigned int* outputlen);
SWITCH_DECLARE(switch_status_t)
switch_digest_string(const char* digest_name, char** digest_str, const void* input,
                     switch_size_t inputLen, unsigned int* outputlen);
