...
static const char* get_recording_var(switch_channel_t* channel, switch_event_t* vars,
                                     const char* name);
static int recording_var_true(switch_channel_t* channel, switch_event_t* vars, const char* name);
static switch_status_t speech_on_dtmf(switch_core_session_t* session, const switch_dtmf_t* dtmf,
...
switch_ivr_dmachine_get_target(switch_ivr_dmachine_t* dmachine) {
    switch_assert(dmachine);
    return dmachine->target;
}

SWITCH_DECLARE(void)
switch_ivr_dmachine_set_target(switch_ivr_dmachine_t* dmachine,
                               switch_digit_action_target_t target) {
    switch_assert(dmachine);
...
SWITCH_DECLARE(void)
switch_ivr_dmachine_set_match_callback(switch_ivr_dmachine_t* dmachine,
                                       switch_ivr_dmachine_callback_t match_callback) {
    switch_assert(dmachine);
...
SWITCH_DECLARE(void)
switch_ivr_dmachine_set_nonmatch_callback(switch_ivr_dmachine_t* dmachine,
                                          switch_ivr_dmachine_callback_t nonmatch_callback) {
    switch_assert(dmachine);
...
switch_ivr_dmachine_create(switch_ivr_dmachine_t** dmachine_p, const char* name,
                           switch_memory_pool_t* pool, uint32_t digit_timeout_ms,
                           uint32_t input_timeout_ms, switch_ivr_dmachine_callback_t match_callback,
                           switch_ivr_dmachine_callback_t nonmatch_callback, void* user_data) {
...
SWITCH_DECLARE(void)
switch_ivr_dmachine_set_digit_timeout_ms(switch_ivr_dmachine_t* dmachine,
                                         uint32_t digit_timeout_ms) {
    dmachine->digit_timeout_ms = digit_timeout_ms;
}

SWITCH_DECLARE(void)
switch_ivr_dmachine_set_input_timeout_ms(switch_ivr_dmachine_t* dmachine,
                                         uint32_t input_timeout_ms) {
    dmachine->input_timeout_ms = input_timeout_ms;
}
...
SWITCH_DECLARE(switch_status_t)
switch_ivr_dmachine_set_terminators(switch_ivr_dmachine_t* dmachine, const char* terminators) {
    if (!dmachine->realm) {
...
SWITCH_DECLARE(switch_status_t)
switch_ivr_dmachine_set_realm(switch_ivr_dmachine_t* dmachine, const char* realm) {
...
SWITCH_DECLARE(switch_status_t)
switch_ivr_dmachine_clear_realm(switch_ivr_dmachine_t* dmachine, const char* realm) {
...
SWITCH_DECLARE(switch_status_t)
switch_ivr_dmachine_bind(switch_ivr_dmachine_t* dmachine, const char* realm, const char* digits,
                         switch_byte_t is_priority, int32_t key,
                         switch_ivr_dmachine_callback_t callback, void* user_data) {
    ...
    if (binding->is_regex) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
                          "%sDigit parser %s: binding %s/%s/%d callback: %p data: %p\n", msg,
...
static dm_match_t switch_ivr_dmachine_check_match(switch_ivr_dmachine_t* dmachine,
    ...
    switch_ivr_dmachine_binding_t *bp, *exact_bp = NULL, *partial_bp = NULL, *both_bp = NULL,
                                       *r_bp = NULL;
    ...
    for (bp = dmachine->realm->binding_list; bp; bp = bp->next) {
        if (bp->is_regex) {
            if (bp->rmatch) {
                if (bp->first_match || (bp->is_priority && !ematches) || is_timeout ||
                    (bp == dmachine->realm->binding_list && !bp->next)) {
                    best = DM_MATCH_EXACT;
                    exact_bp = bp;
                    break;
                }
            ...
            if (!exact_bp && pmatch &&
                (bp->first_match || !rmatches || bp->is_priority || is_timeout) &&
                !strcmp(bp->digits, dmachine->digits)) {
                best = DM_MATCH_EXACT;
                exact_bp = bp;
                if (bp->first_match || bp->is_priority ||
                    dmachine->cur_digit_len == dmachine->max_digit_len)
                    break;
            }
...
static switch_bool_t switch_ivr_dmachine_check_timeout(switch_ivr_dmachine_t* dmachine) {
    ...
    uint32_t timeout =
        dmachine->cur_digit_len ? dmachine->digit_timeout_ms : dmachine->input_timeout_ms;
...
SWITCH_DECLARE(switch_ivr_dmachine_match_t*)
...
SWITCH_DECLARE(switch_status_t)
switch_ivr_dmachine_ping(switch_ivr_dmachine_t* dmachine, switch_ivr_dmachine_match_t** match_p) {
...
SWITCH_DECLARE(switch_status_t)
switch_ivr_dmachine_feed(switch_ivr_dmachine_t* dmachine, const char* digits,
                         switch_ivr_dmachine_match_t** match) {
    const char* p;
        ...
        if (dmachine->cur_digit_len < dmachine->max_digit_len) {
            ...
            *e = '\0';
            ...
            if (status == SWITCH_STATUS_SUCCESS &&
                (istatus = switch_ivr_dmachine_ping(dmachine, match)) != SWITCH_STATUS_SUCCESS) {
                status = istatus;
            }
        } else {
            switch_mutex_unlock(dmachine->mutex);
...
// echo
SWITCH_DECLARE(switch_status_t)
switch_ivr_session_echo(switch_core_session_t* session, switch_input_args_t* args) {
    ...
    while (switch_channel_ready(channel)) {
        ...
        if (args && (args->input_callback || args->buf || args->buflen)) {
            ...
            /*
               dtmf handler function you can hook up to be executed when a digit is dialed during
               playback if you return anything but SWITCH_STATUS_SUCCESS the playback will stop.
             */
            if (switch_channel_has_dtmf(channel)) {
                ...
                if (args->input_callback) {
                    status = args->input_callback(session, (void*)&dtmf, SWITCH_INPUT_TYPE_DTMF,
            ...
            if (args->input_callback) {
                ...
                if (switch_core_session_dequeue_event(session, &event, SWITCH_FALSE) ==
...
static switch_bool_t write_displace_callback(switch_media_bug_t* bug, void* user_data,
            ...
            if (dh) {
                ...
                if (dh->mux) {
                    ...
                    if (len < rframe->samples) {
                        memset((char*)rframe->data + (len * 2 * dh->fh.channels), 0,
                               (rframe->samples - len) * 2 * dh->fh.channels);
                    }
                }
...
static switch_bool_t read_displace_callback(switch_media_bug_t* bug, void* user_data,
                                            switch_abc_type_t type) {
    displace_helper_t* dh = (displace_helper_t*)user_data;
...
SWITCH_DECLARE(switch_status_t)
switch_ivr_stop_displace_session(switch_core_session_t* session, const char* file) {
...
SWITCH_DECLARE(switch_status_t)
switch_ivr_displace_session(switch_core_session_t* session, const char* file, uint32_t limit,
                            const char* flags) {
    switch_channel_t* channel = switch_core_session_get_channel(session);
    switch_media_bug_t* bug;
    switch_status_t status;
    ...
    if (!switch_channel_media_up(channel) || !switch_core_session_get_read_codec(session)) {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
                          "Can not displace session.  Media not enabled on channel\n");
        return SWITCH_STATUS_FALSE;
    }

    if ((bug = switch_channel_get_private(channel, file))) {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
                          "Only 1 of the same file per channel please!\n");
        return SWITCH_STATUS_FALSE;
    }
    ...
    if (!strstr(file, SWITCH_URL_SEPARATOR)) {
        if (!switch_is_file_path(file)) {
            ...
            file =
                switch_core_session_sprintf(session, "%s%s%s%s%s", switch_str_nil(tfile),
                                            tfile ? "]" : "", prefix, SWITCH_PATH_SEPARATOR, file);
        }
        if ((ext = strrchr(file, '.'))) {
            ext++;
        } else {
    ...
    if (switch_core_file_open(
            &dh->fh, file, read_impl.number_of_channels, read_impl.actual_samples_per_second,
    ...
    if (flags && strchr(flags, 'r')) {
        if (strchr(flags, 'w')) {  // r&w mode, both sides can hear the same file
            ...
            switch_mutex_init(&dh->mutex, SWITCH_MUTEX_NESTED,
                              switch_core_session_get_pool(session));
            switch_buffer_create_dynamic(&dh->wbuffer, len, len, 0);
        }

        status = switch_core_media_bug_add(
            session, "displace", file, read_displace_callback, dh, to,
            bug_flags | SMBF_WRITE_REPLACE | SMBF_READ_REPLACE | SMBF_NO_PAUSE, &bug);
    } else {
        status = switch_core_media_bug_add(
            session, "displace", file, write_displace_callback, dh, to,
            bug_flags | SMBF_WRITE_REPLACE | SMBF_READ_REPLACE | SMBF_NO_PAUSE, &bug);
    }
...
struct record_helper {
    switch_media_bug_t* bug;                       // 关联的媒体 bug（音频/视频采集点）
    switch_memory_pool_t* helper_pool;             // 独立内存池（生命周期管理）
    switch_core_session_t* recording_session;      // 正在录音的会话
    switch_core_session_t* transfer_from_session;  // 来源会话（转移时）
    uint8_t transfer_complete;                     // 转移完成标志
    char* file;                                    // 录音文件路径
    switch_file_handle_t* fh;                      // 文件句柄（单文件模式）
    switch_file_handle_t in_fh;                    // 入站音频文件句柄
    switch_file_handle_t out_fh;                   // 出站音频文件句柄
    int native;
    uint32_t packet_len;
    int min_sec;
    int final_timeout_ms;        // 初始超时（等待开始说话）
    int initial_timeout_ms;      // 最终超时（说话后的静音）
    int silence_threshold;       // 静音阈值（能量值）
    int silence_timeout_ms;      // 静音超时（毫秒）
    switch_time_t silence_time;  // 静音开始时间戳
    int rready;
    int wready;
    switch_time_t last_read_time;
    switch_time_t last_write_time;
    switch_bool_t hangup_on_error;  // 错误时是否挂断
    switch_codec_implementation_t read_impl;
    switch_bool_t speech_detected;   // 是否检测到语音
    switch_buffer_t* thread_buffer;  // 线程缓冲区
    switch_thread_t* thread;         // 异步写入线程
    switch_mutex_t* buffer_mutex;    // 缓冲区互斥锁
    int thread_ready;                // 线程就绪标志
    uint8_t thread_needs_transfer;
    uint32_t writes;               // 音频帧写入计数
    uint32_t vwrites;              // 视频帧写入计数
    const char* completion_cause;  // 录音结束原因
    int start_event_sent;          // 是否已发送开始事件
    switch_event_t* variables;     // 自定义变量（标题、版权等）
    switch_mutex_t* cond_mutex;    // 条件变量互斥锁
    switch_thread_cond_t* cond;    // 条件变量（信号通知）
};
...
/**
 * Set the recording completion cause. The cause can only be set once, to minimize the logic in the
 * record_callback. [The completion_cause strings are essentially those of an MRCP Recorder
 * resource.]
 */
...
static switch_bool_t is_silence_frame(switch_frame_t* frame, int silence_threshold,
    ...
    for (channel_num = 0; channel_num < codec_impl->number_of_channels && is_silence;
         channel_num++) {
    ...
    if (rh->fh) {
        ...
        if (read_impl->actual_samples_per_second) {
            switch_channel_set_variable_printf(
                channel, "record_seconds", "%d",
                rh->fh->samples_out / read_impl->actual_samples_per_second);
            switch_channel_set_variable_printf(
                channel, "record_ms", "%d",
                rh->fh->samples_out / (read_impl->actual_samples_per_second / 1000));
        }
    }

    if (!zstr(rh->completion_cause)) {
        switch_channel_set_variable_printf(channel, "record_completion_cause", "%s",
                                           rh->completion_cause);
    }
...
static void* SWITCH_THREAD_FUNC recording_thread(switch_thread_t* thread, void* obj) {
    ...
    switch_buffer_create_dynamic(&rh->thread_buffer, SWITCH_RECOMMENDED_BUFFER_SIZE,
                                 SWITCH_RECOMMENDED_BUFFER_SIZE, 0);
    rh->thread_ready = 1;

    channels =
        switch_core_media_bug_test_flag(bug, SMBF_STEREO) ? 2 : rh->read_impl.number_of_channels;
    ...
    while (switch_test_flag(rh->fh, SWITCH_FILE_OPEN)) {
        if (rh->thread_needs_transfer) {
            ...
            bug = rh->bug;
            channels = switch_core_media_bug_test_flag(bug, SMBF_STEREO)
                           ? 2
                           : rh->read_impl.number_of_channels;
        ...
        if (switch_core_file_has_video(rh->fh, SWITCH_TRUE)) {
            switch_core_session_get_read_impl(session, &read_impl);
            if (read_impl.decoded_bytes_per_packet > 0 &&
                read_impl.decoded_bytes_per_packet <= SWITCH_RECOMMENDED_BUFFER_SIZE) {
...
static void record_helper_post_process(struct record_helper* rh, switch_core_session_t* session) {
    ...
    if (rh->variables && (post_process_exec = switch_event_get_header(
                              rh->variables, SWITCH_RECORD_POST_PROCESS_EXEC_APP_VARIABLE))) {
    ...
    if ((var =
             switch_channel_get_variable(channel, SWITCH_RECORD_POST_PROCESS_EXEC_API_VARIABLE))) {
...
static switch_bool_t record_callback(switch_media_bug_t* bug, void* user_data,
                                     switch_abc_type_t type) {
    switch_core_session_t* session = switch_core_media_bug_get_session(bug);
    switch_channel_t* channel = switch_core_session_get_channel(session);
    ...
    switch (type) {
        case SWITCH_ABC_TYPE_INIT: {
            ...
            if (rh->transfer_from_session && rh->transfer_from_session != rh->recording_session) {
                ...
                if (rh->thread_ready) {
                    ...
                    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                                      "Re-initiating recording thread for file %s\n", rh->file);
                    ...
                    if (rh->thread_needs_transfer) {
                        ...
                        rh->bug = oldbug;
                        switch_core_session_get_read_impl(rh->transfer_from_session,
                                                          &rh->read_impl);
                        rh->thread_needs_transfer = 0;
                ...
                if (rh->fh) {
                    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                                      "Record session sample rate: %d -> %d\n", rh->fh->native_rate,
                                      rh->fh->samplerate);
            ...
            if (rh->start_event_sent == 0) {
                ...
                if (switch_event_create(&event, SWITCH_EVENT_RECORD_START) ==
                    SWITCH_STATUS_SUCCESS) {
                    switch_channel_event_set_data(channel, event);
                    switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "Record-File-Path",
                                                   rh->file);
                    merge_recording_variables(rh, event);
                    switch_event_fire(&event);
                }
        ...
        case SWITCH_ABC_TYPE_CLOSE: {
            ...
            if (rh->native) {
                ...
                while (switch_core_media_bug_read(bug, &frame, SWITCH_TRUE) ==
                    ...
                    if (len && switch_core_file_write(rh->fh, mask ? null_data : data, &len) !=
                                   SWITCH_STATUS_SUCCESS) {
                        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
                                          "Error writing %s\n", rh->file);
                        /* File write failed */
                ...
                switch_core_file_get_string(rh->fh, SWITCH_AUDIO_COL_STR_FILE_TRIMMED,
                                            &file_trimmed);
                switch_core_file_get_string(rh->fh, SWITCH_AUDIO_COL_STR_FILE_TRIMMED_MS,
                                            &file_trimmed_ms);
                if (file_trimmed_ms)
                ...
                if (!rh->writes && !rh->vwrites &&
                    !switch_test_flag(rh->fh, SWITCH_FILE_WRITE_APPEND)) {
                    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                                      "Discarding empty file %s\n", rh->file);
        ...
        case SWITCH_ABC_TYPE_READ_PING:
            ...
            if (rh->fh) {
                ...
                for (;;) {
                    status = switch_core_media_bug_read(bug, &frame,
                                                        i++ == 0 ? SWITCH_FALSE : SWITCH_TRUE);
                    ...
                    if (status != SWITCH_STATUS_SUCCESS || !frame.datalen) {
                        ...
                        if (rh->thread_buffer) {
                            switch_mutex_lock(rh->buffer_mutex);
                            switch_buffer_write(rh->thread_buffer, mask ? null_data : data,
                                                frame.datalen);
                            switch_mutex_unlock(rh->buffer_mutex);
                        ...
                        } else if (switch_core_file_write(rh->fh, mask ? null_data : data, &len) !=
                                   SWITCH_STATUS_SUCCESS) {
                            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
                                              "Error writing %s\n", rh->file);
                            /* File write failed */
                        ...
                        if (rh->silence_threshold) {
                            ...
                            if (is_silence_frame(&frame, rh->silence_threshold, &read_impl)) {
                                if (!rh->silence_time) {
                                    ...
                                    /* continuing silence */
                                    int duration_ms =
                                        (int)((switch_micro_time_now() - rh->silence_time) / 1000);
                                    if (rh->silence_timeout_ms > 0 &&
                                        duration_ms >= rh->silence_timeout_ms) {
                                        switch_log_printf(
        ...
        case SWITCH_ABC_TYPE_STREAM_VIDEO_PING:
            if (rh->fh && switch_test_flag(rh->fh, SWITCH_FILE_OPEN)) {
                ...
                if (bug->video_ping_frame->img &&
                    switch_core_file_write_video(rh->fh, bug->video_ping_frame) !=
                        SWITCH_STATUS_SUCCESS &&
                    rh->hangup_on_error) {
                    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
                                      "Error writing video to %s\n", rh->file);
                    switch_channel_hangup(channel, SWITCH_CAUSE_DESTINATION_OUT_OF_ORDER);
                    switch_core_session_reset(session, SWITCH_TRUE, SWITCH_TRUE);
                    return SWITCH_FALSE;
                }
...
static switch_bool_t text_callback(switch_media_bug_t* bug, void* user_data,
                                   switch_abc_type_t type) {
    switch (type) {
...
SWITCH_DECLARE(switch_status_t)
switch_ivr_capture_text(switch_core_session_t* session, switch_bool_t on) {
    switch_media_bug_t* bug;
    switch_channel_t* channel = switch_core_session_get_channel(session);
    ...
    if (on) {
        if (bug) {
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
                              "text bug already attached\n");
            return SWITCH_STATUS_FALSE;
        }

        if (switch_core_media_bug_add(session, "capture_text",
                                      switch_core_session_get_uuid(session), text_callback, NULL, 0,
                                      SMBF_READ_TEXT_STREAM, &bug) != SWITCH_STATUS_SUCCESS) {
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
                              "Cannot attach bug\n");
            return SWITCH_STATUS_FALSE;
        }
...
SWITCH_DECLARE(switch_status_t)
switch_ivr_record_session_mask(switch_core_session_t* session, const char* file, switch_bool_t on) {
    switch_media_bug_t* bug;
    switch_channel_t* channel = switch_core_session_get_channel(session);
...
SWITCH_DECLARE(switch_status_t)
switch_ivr_record_session_pause(switch_core_session_t* session, const char* file,
                                switch_bool_t on) {
    switch_media_bug_t* bug;
    switch_channel_t* channel = switch_core_session_get_channel(session);
...
SWITCH_DECLARE(switch_status_t)
switch_ivr_stop_record_session(switch_core_session_t* session, const char* file) {
    switch_media_bug_t* bug;
    switch_channel_t* channel = switch_core_session_get_channel(session);
...
SWITCH_DECLARE(switch_status_t)
switch_ivr_transfer_recordings(switch_core_session_t* orig_session,
    ...
    if ((var = switch_channel_get_variable(orig_channel,
    ...
    switch_channel_transfer_variable_prefix(orig_channel, new_channel,
                                            SWITCH_RECORD_POST_PROCESS_EXEC_APP_VARIABLE);

    return switch_core_media_bug_transfer_callback(orig_session, new_session, record_callback,
...
//
static switch_bool_t eavesdrop_callback(switch_media_bug_t* bug, void* user_data,
                                        switch_abc_type_t type) {
    ...
    show_spy = switch_core_media_bug_test_flag(bug, SMBF_SPY_VIDEO_STREAM) ||
               switch_core_media_bug_test_flag(bug, SMBF_SPY_VIDEO_STREAM_BLEG);
    ...
    switch (type) {
        case SWITCH_ABC_TYPE_INIT:
            ...
            if (switch_core_media_bug_test_flag(bug, SMBF_READ_VIDEO_STREAM) ||
                ...
                switch_core_session_set_video_read_callback(ep->eavesdropper,
                                                            video_eavesdrop_callback, (void*)bug);
                switch_channel_set_flag_recursive(switch_core_session_get_channel(session),
        ...
        case SWITCH_ABC_TYPE_CLOSE:
            ...
            switch_channel_clear_flag_recursive(switch_core_session_get_channel(session),
                                                CF_VIDEO_DECODED_READ);
        ...
        case SWITCH_ABC_TYPE_READ_REPLACE: {
            if (switch_test_flag(ep, ED_MUX_READ)) {
                ...
                if (switch_buffer_inuse(ep->r_buffer) >= rframe->datalen) {
                    ...
                    rframe->datalen = switch_merge_sln(rframe->data, rframe->samples,
                                                       (int16_t*)ep->data, bytes / 2, channels) *
                                      2 * channels;
        ...
        case SWITCH_ABC_TYPE_STREAM_VIDEO_PING: {
            ...
            if (switch_core_session_read_lock(ep->eavesdropper) == SWITCH_STATUS_SUCCESS) {
                if (switch_core_session_write_video_frame(ep->eavesdropper, bug->video_ping_frame,
                                                          SWITCH_IO_FLAG_NONE,
    ...
    if (nframe) {
        ...
        if (switch_core_session_write_frame(ep->eavesdropper, &frame, SWITCH_IO_FLAG_NONE, 0) !=
            SWITCH_STATUS_SUCCESS) {
            return SWITCH_FALSE;
        }
    }
...
SWITCH_DECLARE(switch_status_t)
switch_ivr_eavesdrop_pop_eavesdropper(switch_core_session_t* session,
...
static void exec_cb(switch_media_bug_t* bug, void* user_data) {
    ...
    if (ep && ep->eavesdropper && ep->eavesdropper != data->caller) {
        ...
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "%s telling %s to exec %s:%s\n",
                          switch_channel_get_name(b), switch_channel_get_name(a), data->var,
                          data->val);
...
SWITCH_DECLARE(switch_status_t)
switch_ivr_eavesdrop_exec_all(switch_core_session_t* session, const char* app, const char* arg) {
...
SWITCH_DECLARE(switch_status_t)
switch_ivr_eavesdrop_update_display(switch_core_session_t* session, const char* name,
                                    const char* number) {
    struct exec_cb_data* data = NULL;
    switch_channel_t* channel = switch_core_session_get_channel(session);
    switch_status_t status = SWITCH_STATUS_FALSE;
...
SWITCH_DECLARE(switch_status_t)
switch_ivr_eavesdrop_session(switch_core_session_t* session, const char* uuid,
                             const char* require_group, switch_eavesdrop_flag_t flags) {
    ...
    if ((tsession = switch_core_session_locate(uuid))) {
        ...
        if (tsession == session) {
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
                              "Eavesdrop target invalid.\n");
            goto end;
        }

        while (switch_channel_state_change_pending(tchannel) ||
               !switch_channel_media_up(tchannel)) {
            switch_yield(10000);
            if (!--sanity) break;
        }
        ...
        if ((flags & ED_TAP_READ) || (flags & ED_TAP_WRITE)) {
            ...
            if (strcasecmp(tread_impl.iananame, read_impl.iananame)) {
                switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
                                  "Codecs do not match which is required for this mode\n");
                goto end;
            }
        }
        ...
        if (!zstr(require_group)) {
            ...
            if ((argc =
                     switch_separate_string(data, ',', argv, (sizeof(argv) / sizeof(argv[0]))))) {
                for (i = 0; i < argc; i++) {
        ...
        if (tread_impl.decoded_bytes_per_packet < read_impl.decoded_bytes_per_packet) {
            if (switch_core_codec_init(
                    &codec, "L16", NULL, NULL, read_impl.actual_samples_per_second,
            ...
            if (switch_core_codec_init(
                    &codec, "L16", NULL, NULL, tread_impl.actual_samples_per_second,
                    tread_impl.microseconds_per_packet / 1000, tread_impl.number_of_channels,
                    SWITCH_CODEC_FLAG_ENCODE | SWITCH_CODEC_FLAG_DECODE, NULL,
                    switch_core_session_get_pool(session)) != SWITCH_STATUS_SUCCESS) {
                switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
                                  "Cannot init codec\n");
        ...
        if (!(flags & ED_TAP_READ) && !(flags & ED_TAP_WRITE)) {
            switch_mutex_init(&ep->mutex, SWITCH_MUTEX_NESTED,
        ...
        if (switch_channel_test_flag(session->channel, CF_VIDEO) &&
            switch_channel_test_flag(tsession->channel, CF_VIDEO)) {
            if ((vval = switch_channel_get_variable(session->channel,
                                                    "eavesdrop_show_listener_video"))) {
                if (switch_true(vval) || !strcasecmp(vval, "aleg") || !strcasecmp(vval, "bleg") ||
                    !strcasecmp(vval, "both")) {
            ...
            if ((vval = switch_channel_get_variable(session->channel, "eavesdrop_concat_video")) &&
                switch_true(vval)) {
        ...
        if (switch_core_media_bug_add(tsession, "eavesdrop", uuid, eavesdrop_callback, ep, 0,
                                      read_flags | write_flags | SMBF_READ_PING | SMBF_THREAD_LOCK |
                                          SMBF_NO_PAUSE | stereo_flag,
                    ...
                    switch (*d) {
                        ...
                        case '4':
                            switch_media_bug_set_spy_fmt(
                                bug, switch_media_bug_parse_spy_fmt("dual-crop"));
                            break;
                        case '5':
                            switch_media_bug_set_spy_fmt(
                                bug, switch_media_bug_parse_spy_fmt("lower-right-small"));
                            break;
                        case '6':
                            switch_media_bug_set_spy_fmt(
                                bug, switch_media_bug_parse_spy_fmt("lower-right-large"));
                            break;
                        case '0':
                            switch_clear_flag(ep, ED_MUX_READ);
                            switch_clear_flag(ep, ED_MUX_WRITE);
            ...
            if (ep->buffer) {
                ...
                while (switch_buffer_inuse(ep->buffer) >= buff_min_len) {
                    ...
                    if (changed || tchanged) {
                        if (changed) {
                            switch_log_printf(
                                ...
                                read_impl.actual_samples_per_second, read_impl.number_of_channels);
                        ...
                        if (tread_impl.decoded_bytes_per_packet <
                            ...
                            buffered = 1;
                            lcm = switch_lcm(tread_impl.decoded_bytes_per_packet,
                                             read_impl.decoded_bytes_per_packet);
                        } else {
                            if (switch_core_codec_init(
                                    &codec, "L16", NULL, NULL, tread_impl.actual_samples_per_second,
                    ...
                    if (ep->tread_impl.number_of_channels != ep->read_impl.number_of_channels) {
                        ...
                        switch_mux_channels((int16_t*)write_frame.data, rlen,
                                            ep->tread_impl.number_of_channels,
                                            ep->read_impl.number_of_channels);
                    ...
                    if ((status = switch_core_session_write_frame(session, &write_frame,
                    ...
                    if (ep->tread_impl.decoded_bytes_per_packet ==
                        ep->read_impl.decoded_bytes_per_packet) {
    ...
    end_loop:
        ...
        if (codec_initialized) switch_core_codec_destroy(&codec);
...
static switch_status_t record_helper_destroy(struct record_helper** rh,
    ...
    if ((*rh)->recording_session != session) {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
                          "Destroying a record helper of another session!\n");
    }
...
SWITCH_DECLARE(switch_status_t)
switch_ivr_record_session_event(switch_core_session_t* session  // 会话指针
                                ,
                                const char* file  // 录制的文件
                                ,
                                uint32_t limit  // 录制时长限制
                                ,
                                switch_file_handle_t* fh  // 文件
                                ,
                                switch_event_t* vars  // 录制变量
) {
    switch_channel_t* channel = switch_core_session_get_channel(session);
    const char* p;
    const char* vval;
    ...
    uint8_t channels;
    switch_codec_implementation_t read_impl = {0};  // 编解码器实现
    ...
    if (recording_var_true(channel, vars, "RECORD_CHECK_BRIDGE")) {
        ...
        if (switch_core_session_get_partner(session, &other_session) == SWITCH_STATUS_SUCCESS) {
            ...
            if ((bug = switch_channel_get_private(other_channel, file))) {
                if (switch_true(
                        switch_channel_get_variable(other_channel, "RECORD_TOGGLE_ON_REPEAT"))) {
                    rstatus = switch_ivr_stop_record_session(other_session, file);
                } else {
                    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(other_session), SWITCH_LOG_WARNING,
                                      "Already recording [%s]\n", file);
                }
                exist = 1;
            }
    ...
    if (fh) {
        ...
        if ((status = switch_core_file_handle_dup(fh, &newfh, rh->helper_pool)) !=
            SWITCH_STATUS_SUCCESS) {
    ...
    if (!switch_is_file_path(file)) {
        ...
        if (*file == '[') {
            ...
            file_path =
                switch_core_sprintf(rh->helper_pool, "%s%s%s", prefix, SWITCH_PATH_SEPARATOR, file);
        }

        file = switch_core_sprintf(rh->helper_pool, "%s%s%s%s%s", switch_str_nil(tfile),
                                   tfile ? "]" : "", prefix, SWITCH_PATH_SEPARATOR, file);
    } else {
    ...
    if (file_path && !strstr(file_path, SWITCH_URL_SEPARATOR)) {
        ...
        if ((p = strrchr(path, *SWITCH_PATH_SEPARATOR))) {
            *p = '\0';
            ...
            if (switch_dir_make_recursive(path, SWITCH_DEFAULT_DIR_PERMS,
                                          switch_core_session_get_pool(session)) !=
                SWITCH_STATUS_SUCCESS) {
                switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
                                  "Error creating %s\n", path);
    ...
    if ((ext = strrchr(file, '.'))) {
        ...
        if (switch_core_file_open(fh, file, channels, read_impl.actual_samples_per_second,
        ...
        if (switch_core_file_open(&rh->in_fh, in_file, channels,
                                  read_impl.actual_samples_per_second, file_flags,
        ...
        if (switch_core_file_open(&rh->out_fh, out_file, channels,
                                  read_impl.actual_samples_per_second, file_flags,
                                  NULL) != SWITCH_STATUS_SUCCESS) {
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
                              "Error opening %s\n", out_file);
    ...
    if ((status = switch_core_media_bug_add(session, "session_record", file, record_callback, rh,
                                            to, flags, &bug)) != SWITCH_STATUS_SUCCESS) {
...
SWITCH_DECLARE(switch_status_t)
switch_ivr_record_session(switch_core_session_t* session, const char* file, uint32_t limit,
                          switch_file_handle_t* fh) {
...
static switch_bool_t preprocess_callback(switch_media_bug_t* bug, void* user_data,
    ...
    switch (type) {
        case SWITCH_ABC_TYPE_INIT: {
            switch_mutex_init(&cb->read_mutex, SWITCH_MUTEX_NESTED,
                              switch_core_session_get_pool(session));
            switch_mutex_init(&cb->write_mutex, SWITCH_MUTEX_NESTED,
                              switch_core_session_get_pool(session));
        } break;
        case SWITCH_ABC_TYPE_CLOSE: {
            if (cb->read_st) {
        ...
        case SWITCH_ABC_TYPE_READ_REPLACE: {
            ...
            if (cb->read_st) {
                if (cb->read_ec) {
                    speex_echo_cancellation(cb->read_ec, (int16_t*)frame->data,
                                            (int16_t*)cb->write_data, (int16_t*)cb->read_out);
        ...
        case SWITCH_ABC_TYPE_WRITE_REPLACE: {
            ...
            if (cb->write_st) {
                if (cb->write_ec) {
                    speex_echo_cancellation(cb->write_ec, (int16_t*)frame->data,
                                            (int16_t*)cb->read_data, (int16_t*)cb->write_out);
...
SWITCH_DECLARE(switch_status_t)
switch_ivr_preprocess_session(switch_core_session_t* session, const char* cmds) {
    switch_channel_t* channel = switch_core_session_get_channel(session);
    switch_media_bug_t* bug;
    switch_status_t status;
    time_t to = 0;
        ...
        if (var) {
            if ((val = strchr(var, '='))) {
                *val++ = '\0';
                ...
                if (rw == 'r') {
                    if (!cb->read_st) {
                        cb->read_st = speex_preprocess_state_init(read_impl.samples_per_packet,
                    ...
                    if (!cb->write_st) {
                        cb->write_st = speex_preprocess_state_init(read_impl.samples_per_packet,
                                                                   read_impl.samples_per_second);
                ...
                if (st) {
                    ...
                    if (!strcasecmp(var, "agc")) {
                        ...
                        if (db < 0) {
                            ...
                            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                                              "Setting NOISE_SUPPRESS on %c to %d [%d]\n", rw, db,
                                              r);
                        } else {
                            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
                                              "Syntax error noise_suppress should be in -db\n");
                        }
                        ...
                        if (!ec) {
                            if (rw == 'r') {
                                ec = cb->read_ec =
                                    speex_echo_state_init(read_impl.samples_per_packet, tail);
                        ...
                        if (db < 0) {
                            ...
                            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                                              "Setting ECHO_SUPPRESS on %c to %d [%d]\n", rw, db,
                                              r);
                        } else {
                            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
                                              "Syntax error echo_suppress should be in -db\n");
                        }
                    } else {
                        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING,
            ...
            if (err) {
                switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
                                  "Syntax error parsing preprocessor commands\n");
            }
    ...
    if ((status = switch_core_media_bug_add(session, "preprocess", NULL, preprocess_callback, cb,
                                            to, flags, &bug)) != SWITCH_STATUS_SUCCESS) {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
                          "Error adding media bug.\n");
        if (cb->read_st) {
            speex_preprocess_state_destroy(cb->read_st);
        }
...
static switch_bool_t session_audio_callback(switch_media_bug_t* bug, void* user_data,
                                            switch_abc_type_t type) {
    ...
    if (type == SWITCH_ABC_TYPE_READ_REPLACE || type == SWITCH_ABC_TYPE_WRITE_REPLACE) {
        if (!(pvt->read_level || pvt->write_level || pvt->read_mute || pvt->write_mute)) {
            switch_channel_set_private(switch_core_session_get_channel(pvt->session), "__audio",
                                       NULL);
            return SWITCH_FALSE;
        }
    }
    ...
    if (frame) {
        if (mute) {
            if (mute > 1) {
                switch_generate_sln_silence(frame->data, frame->datalen / 2,
                                            read_impl.number_of_channels, mute);
            } else {
...
SWITCH_DECLARE(switch_status_t)
switch_ivr_session_audio(switch_core_session_t* session, const char* cmd, const char* direction,
                         int level) {
    switch_channel_t* channel = switch_core_session_get_channel(session);
    switch_media_bug_t* bug;
    switch_status_t status;
    ...
    if (existing) {
        ...
        if ((status = switch_core_media_bug_add(session, "audio", cmd, session_audio_callback, pvt,
                                                0, flags, &bug)) != SWITCH_STATUS_SUCCESS) {
            return status;
        }
...
static switch_bool_t inband_dtmf_callback(switch_media_bug_t* bug, void* user_data,
                                          switch_abc_type_t type) {
    ...
    switch (type) {
        ...
        case SWITCH_ABC_TYPE_READ_REPLACE:
            if ((frame = switch_core_media_bug_get_read_replace_frame(bug))) {
                if (teletone_dtmf_detect(&pvt->dtmf_detect, frame->data, frame->samples) ==
                    TT_HIT_END) {
                    switch_dtmf_t dtmf = {0};
...
SWITCH_DECLARE(switch_status_t)
switch_ivr_stop_inband_dtmf_session(switch_core_session_t* session) {
    switch_media_bug_t* bug;
    switch_channel_t* channel = switch_core_session_get_channel(session);
...
SWITCH_DECLARE(switch_status_t) switch_ivr_inband_dtmf_session(switch_core_session_t* session) {
    ...
    if ((status = switch_core_media_bug_add(session, "inband_dtmf", NULL, inband_dtmf_callback, pvt,
                                            0, SMBF_READ_REPLACE | SMBF_NO_PAUSE | SMBF_ONE_ONLY,
                                            &bug)) != SWITCH_STATUS_SUCCESS) {
        return status;
    }
...
static switch_status_t generate_on_dtmf(switch_core_session_t* session, const switch_dtmf_t* dtmf,
    ...
    if (bug) {
        switch_inband_dtmf_generate_t* pvt =
            (switch_inband_dtmf_generate_t*)switch_core_media_bug_get_user_data(bug);
                ...
                *dt = *dtmf;
                ...
                if (switch_queue_trypush(pvt->digit_queue, dt) == SWITCH_STATUS_SUCCESS) {
                    ...
                    if (switch_event_create(&event, SWITCH_EVENT_DTMF) == SWITCH_STATUS_SUCCESS) {
                        switch_channel_event_set_data(channel, event);
                        switch_event_add_header(event, SWITCH_STACK_BOTTOM, "DTMF-Digit", "%c",
                                                dtmf->digit);
                        switch_event_add_header(event, SWITCH_STACK_BOTTOM, "DTMF-Duration", "%u",
                                                dtmf->duration);
                        switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "DTMF-Source",
                                                       "APP");
                        switch_event_add_header_string(event, SWITCH_STACK_BOTTOM,
                                                       "DTMF-Conversion", "native:inband");
...
static switch_bool_t inband_dtmf_generate_callback(switch_media_bug_t* bug, void* user_data,
                                                   switch_abc_type_t type) {
    ...
    switch (type) {
        case SWITCH_ABC_TYPE_INIT: {
            ...
            pvt->ts.channels = 1;
            switch_mutex_init(&pvt->mutex, SWITCH_MUTEX_NESTED,
                              switch_core_session_get_pool(pvt->session));
            if (pvt->read) {
        ...
        case SWITCH_ABC_TYPE_WRITE_REPLACE: {
                        ...
                        if (duration > (int)switch_core_max_dtmf_duration(0)) {
                            ...
                            switch_log_printf(
                                ...
                                switch_channel_get_name(
                                    switch_core_session_get_channel(pvt->session)),
                                dtmf->duration / 8, duration);
                        }
            ...
            if (switch_buffer_inuse(pvt->audio_buffer) &&
                (bytes = switch_buffer_read(pvt->audio_buffer, frame->data, frame->datalen))) {
...
SWITCH_DECLARE(switch_status_t)
switch_ivr_stop_inband_dtmf_generate_session(switch_core_session_t* session) {
    switch_channel_t* channel = switch_core_session_get_channel(session);
    switch_media_bug_t* bug = switch_channel_get_private(channel, "dtmf_generate");
...
SWITCH_DECLARE(switch_status_t)
switch_ivr_inband_dtmf_generate_session(switch_core_session_t* session, switch_bool_t read_stream) {
    switch_channel_t* channel = switch_core_session_get_channel(session);
    switch_media_bug_t* bug;
    switch_status_t status;
    switch_inband_dtmf_generate_t* pvt;
    ...
    if (!switch_channel_media_up(channel) || !switch_core_session_get_read_codec(session)) {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
                          "Can not install inband dtmf generate.  Media not enabled on channel\n");
        return status;
    }
    ...
    if ((status = switch_core_media_bug_add(
             session, "inband_dtmf_generate", NULL, inband_dtmf_generate_callback, pvt, 0,
             SMBF_NO_PAUSE | (pvt->read ? SMBF_READ_REPLACE : SMBF_WRITE_REPLACE), &bug)) !=
        SWITCH_STATUS_SUCCESS) {
        return status;
    }
...
static void tone_detect_set_total_time(switch_tone_container_t* cont, int index) {
    char* total_time =
        switch_mprintf("%d", (int)(switch_micro_time_now() - cont->list[index].start_time) / 1000);

    switch_channel_set_variable_name_printf(switch_core_session_get_channel(cont->session),
                                            total_time, "tone_detect_%s_total_time",
                                            cont->list[index].key);
    switch_safe_free(total_time);
}

static switch_status_t tone_on_dtmf(switch_core_session_t* session, const switch_dtmf_t* dtmf,
                                    switch_dtmf_direction_t direction) {
    switch_channel_t* channel = switch_core_session_get_channel(session);
    ...
    if (cont->list[i].callback) {
        ...
        switch_channel_execute_on(switch_core_session_get_channel(cont->session),
                                  SWITCH_CHANNEL_EXECUTE_ON_TONE_DETECT_VARIABLE);
        switch_channel_api_on(switch_core_session_get_channel(cont->session),
                              SWITCH_CHANNEL_API_ON_TONE_DETECT_VARIABLE);

        if (cont->list[i].app) {
            switch_core_session_execute_application_async(cont->session, cont->list[i].app,
                                                          cont->list[i].data);
        }
    }
...
static switch_bool_t tone_detect_callback(switch_media_bug_t* bug, void* user_data,
                                          switch_abc_type_t type) {
            ...
            for (i = 0; i < cont->index; i++) {
                ...
                if (teletone_multi_tone_detect(&cont->list[i].mt, frame->data, frame->samples)) {
                    ...
                    if (cont->list[i].hits >= cont->list[i].total_hits) {
                        ...
                        if (cont->list[i].callback) {
                            if ((rval = cont->list[i].callback(cont->session, cont->list[i].app,
                                                               cont->list[i].data)) ==
                                SWITCH_TRUE) {
                            ...
                            switch_channel_execute_on(
                                switch_core_session_get_channel(cont->session),
                                SWITCH_CHANNEL_EXECUTE_ON_TONE_DETECT_VARIABLE);
                            if (cont->list[i].app) {
                                switch_core_session_execute_application_async(
                                    cont->session, cont->list[i].app, cont->list[i].data);
                            }
                        }
                        ...
                        if (switch_event_create(&event, SWITCH_EVENT_DETECTED_TONE) ==
                            SWITCH_STATUS_SUCCESS) {
                            switch_event_t* dup;
                            switch_event_add_header_string(event, SWITCH_STACK_BOTTOM,
                                                           "Detected-Tone", cont->list[i].key);

                            if (switch_event_dup(&dup, event) == SWITCH_STATUS_SUCCESS) {
                                switch_event_fire(&dup);
                            }

                            if (switch_core_session_queue_event(cont->session, &event) !=
                                SWITCH_STATUS_SUCCESS) {
                                switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(
                                                      switch_core_media_bug_get_session(bug)),
...
SWITCH_DECLARE(switch_status_t)
switch_ivr_stop_tone_detect_session(switch_core_session_t* session) {
    switch_channel_t* channel = switch_core_session_get_channel(session);
    switch_tone_container_t* cont = switch_channel_get_private(channel, "_tone_detect_");
    int i = 0;
...
SWITCH_DECLARE(switch_status_t)
switch_ivr_tone_detect_session(switch_core_session_t* session, const char* key,
                               const char* tone_spec, const char* flags, time_t timeout, int hits,
    ...
    if (cont->bug_running) {
        ...
        if ((status = switch_core_media_bug_add(session, "tone_detect", key, tone_detect_callback,
                                                cont, timeout, bflags, &cont->bug)) !=
            SWITCH_STATUS_SUCCESS) {
            cont->bug_running = 0;
            return status;
        }
...
SWITCH_DECLARE(void)
switch_ivr_broadcast_in_thread(switch_core_session_t* session, const char* app, int flags) {
            ...
            *digit = dtmf->digit;
            ...
            if (ok && md->sr[direction].map[dval].app) {
                ...
                if (switch_channel_test_flag(channel, CF_PROXY_MODE)) {
                    switch_ivr_broadcast_in_thread(session, md->sr[direction].map[dval].app,
                                                   flags | SMF_REBRIDGE);
                } else {
...
SWITCH_DECLARE(switch_status_t)
switch_ivr_unbind_dtmf_meta_session(switch_core_session_t* session, uint32_t key) {
    switch_channel_t* channel = switch_core_session_get_channel(session);
...
static switch_status_t block_on_dtmf(switch_core_session_t* session, const switch_dtmf_t* dtmf,
                                     switch_dtmf_direction_t direction) {
    switch_channel_t* channel = switch_core_session_get_channel(session);
...
SWITCH_DECLARE(switch_status_t)
switch_ivr_bind_dtmf_meta_session(switch_core_session_t* session, uint32_t key,
    ...
    if (meta != '*' && meta != '#') {
        ...
        if (switch_dtmftoi(str) == (char)key) {
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
                              "Invalid key %u, same as META CHAR\n", key);
            return SWITCH_STATUS_FALSE;
        }
    }
...
static void deliver_asr_event(switch_core_session_t* session, switch_event_t* event,
                              switch_input_args_t* args) {
    if (args && args->input_callback) {
        args->input_callback(session, (void*)event, SWITCH_INPUT_TYPE_EVENT, args->buf,
                             args->buflen);
    }
}

static switch_status_t play_and_detect_input_callback(switch_core_session_t* session, void* input,
                                                      switch_input_type_t input_type, void* data,
                                                      unsigned int len) {
    ...
    if (!state->done) {
        ...
        if (input_type == SWITCH_INPUT_TYPE_EVENT) {
            ...
            const char* terminators =
                switch_channel_get_variable(channel, SWITCH_PLAYBACK_TERMINATORS_VARIABLE);
            if (terminators) {
            ...
            if (terminators && strchr(terminators, dtmf->digit)) {
                switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                                  ...
                                  dtmf->digit);
                switch_channel_set_variable_printf(channel, SWITCH_PLAYBACK_TERMINATOR_USED, "%c",
                                                   dtmf->digit);
                ...
                switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                                  "(%s) IGNORE NON-TERMINATOR DIGIT %c\n",
                                  switch_channel_get_name(channel), dtmf->digit);
            }
        }
    }
...
SWITCH_DECLARE(switch_status_t)
switch_ivr_play_and_detect_speech(switch_core_session_t* session, const char* file,
                                  const char* mod_name, const char* grammar, char** result,
                                  uint32_t input_timeout, switch_input_args_t* args) {
    switch_status_t status = SWITCH_STATUS_FALSE;
    int recognizing = 0;
    ...
    if ((status = switch_ivr_detect_speech(session, mod_name, grammar, "", NULL, NULL)) !=
        SWITCH_STATUS_SUCCESS) {
    ...
    if (args && args->dmachine &&
        switch_ivr_dmachine_last_ping(args->dmachine) != SWITCH_STATUS_SUCCESS) {
        state.done |= PLAY_AND_DETECT_DONE;
        goto done;
    }
...
static void* SWITCH_THREAD_FUNC speech_thread(switch_thread_t* thread, void* obj) {
    ...
    while (switch_channel_up_nosig(channel) && !switch_test_flag(sth->ah, SWITCH_ASR_FLAG_CLOSED)) {
        ...
        if (switch_channel_down_nosig(channel) ||
            switch_test_flag(sth->ah, SWITCH_ASR_FLAG_CLOSED)) {
            break;
        }
        ...
        if (switch_core_asr_check_results(sth->ah, &flags) == SWITCH_STATUS_SUCCESS) {
            ...
            if (status != SWITCH_STATUS_SUCCESS && status != SWITCH_STATUS_BREAK &&
                ...
                /* Try to fetch extra information for this result, the return value doesn't really
                 * matter here - it's just optional data. */
                switch_core_asr_get_result_headers(sth->ah, &headers, &flags);
            }

            if (status == SWITCH_STATUS_SUCCESS &&
                switch_true(switch_channel_get_variable(channel, "asr_intercept_dtmf"))) {
                const char* p;
            ...
            if (switch_event_create(&event, SWITCH_EVENT_DETECTED_SPEECH) ==
                SWITCH_STATUS_SUCCESS) {
                if (status == SWITCH_STATUS_SUCCESS) {
                    switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "Speech-Type",
                                                   "detected-speech");
                ...
                if (switch_core_session_queue_event(sth->session, &event) !=
                    SWITCH_STATUS_SUCCESS) {
                    switch_log_printf(SWITCH_CHANNEL_CHANNEL_LOG(channel), SWITCH_LOG_ERROR,
                                      "Event queue failed!\n");
                    switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "delivery-failure",
                                                   "true");
                    switch_event_fire(&event);
                }
            }
...
static switch_bool_t speech_callback(switch_media_bug_t* bug, void* user_data,
                                     switch_abc_type_t type) {
    ...
    switch (type) {
        ...
        case SWITCH_ABC_TYPE_READ:
            if (sth->ah) {
                if (switch_core_media_bug_read(bug, &frame, SWITCH_FALSE) != SWITCH_STATUS_FALSE) {
                    if (switch_core_asr_feed(sth->ah, frame.data, frame.datalen, &flags) !=
                        SWITCH_STATUS_SUCCESS) {
                        switch_log_printf(
                            SWITCH_CHANNEL_SESSION_LOG(switch_core_media_bug_get_session(bug)),
...
SWITCH_DECLARE(switch_status_t)
switch_ivr_detect_speech_load_grammar(switch_core_session_t* session, const char* grammar,
                                      const char* name) {
    switch_channel_t* channel = switch_core_session_get_channel(session);
    struct speech_thread_handle* sth = switch_channel_get_private(channel, SWITCH_SPEECH_KEY);
    switch_status_t status;
    ...
    if (sth) {
        if ((status = switch_core_asr_load_grammar(sth->ah, grammar, name)) !=
            SWITCH_STATUS_SUCCESS) {
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
...
SWITCH_DECLARE(switch_status_t)
switch_ivr_set_param_detect_speech(switch_core_session_t* session, const char* name,
                                   const char* val) {
    struct speech_thread_handle* sth =
        switch_channel_get_private(switch_core_session_get_channel(session), SWITCH_SPEECH_KEY);
    switch_status_t status = SWITCH_STATUS_FALSE;
...
SWITCH_DECLARE(switch_status_t)
switch_ivr_detect_speech_start_input_timers(switch_core_session_t* session) {
    switch_channel_t* channel = switch_core_session_get_channel(session);
    struct speech_thread_handle* sth = switch_channel_get_private(channel, SWITCH_SPEECH_KEY);
...
SWITCH_DECLARE(switch_status_t)
switch_ivr_detect_speech_unload_grammar(switch_core_session_t* session, const char* name) {
    switch_channel_t* channel = switch_core_session_get_channel(session);
    struct speech_thread_handle* sth = switch_channel_get_private(channel, SWITCH_SPEECH_KEY);
    switch_status_t status;
...
SWITCH_DECLARE(switch_status_t)
switch_ivr_detect_speech_enable_grammar(switch_core_session_t* session, const char* name) {
    switch_channel_t* channel = switch_core_session_get_channel(session);
    struct speech_thread_handle* sth = switch_channel_get_private(channel, SWITCH_SPEECH_KEY);
    switch_status_t status;
...
SWITCH_DECLARE(switch_status_t)
switch_ivr_detect_speech_disable_grammar(switch_core_session_t* session, const char* name) {
    switch_channel_t* channel = switch_core_session_get_channel(session);
    struct speech_thread_handle* sth = switch_channel_get_private(channel, SWITCH_SPEECH_KEY);
    switch_status_t status;
...
SWITCH_DECLARE(switch_status_t)
switch_ivr_detect_speech_disable_all_grammars(switch_core_session_t* session) {
    switch_channel_t* channel = switch_core_session_get_channel(session);
    struct speech_thread_handle* sth = switch_channel_get_private(channel, SWITCH_SPEECH_KEY);
    switch_status_t status;
...
SWITCH_DECLARE(switch_status_t)
switch_ivr_detect_speech_init(switch_core_session_t* session, const char* mod_name,
    ...
    if ((status = switch_core_asr_open(ah, mod_name, "L16", read_impl.actual_samples_per_second,
                                       dest, &flags, switch_core_session_get_pool(session))) !=
        SWITCH_STATUS_SUCCESS) {
        return status;
    }
    ...
    if ((status = switch_core_media_bug_add(session, "detect_speech", key, speech_callback, sth, 0,
                                            SMBF_READ_STREAM | SMBF_NO_PAUSE, &sth->bug)) !=
        SWITCH_STATUS_SUCCESS) {
        switch_core_asr_close(ah, &flags);
        return status;
    }

    if ((status = switch_core_event_hook_add_recv_dtmf(session, speech_on_dtmf)) !=
        SWITCH_STATUS_SUCCESS) {
        switch_ivr_stop_detect_speech(session);
        return status;
    }
...
SWITCH_DECLARE(switch_status_t)
switch_ivr_detect_speech(switch_core_session_t* session, const char* mod_name, const char* grammar,
                         const char* name, const char* dest, switch_asr_handle_t* ah) {
    switch_channel_t* channel = switch_core_session_get_channel(session);
    struct speech_thread_handle* sth = switch_channel_get_private(channel, SWITCH_SPEECH_KEY);
    const char* p;
    int resume = 0;
...
switch_ivr_schedule_hangup(time_t runtime, const char* uuid, switch_call_cause_t cause,
                           switch_bool_t bleg) {
    struct hangup_helper* helper;
    ...
    return switch_scheduler_add_task(runtime, sch_hangup_callback, (char*)__SWITCH_FUNC__, uuid, 0,
                                     helper, SSHF_FREE_ARG);
}
...
SWITCH_DECLARE(uint32_t)
switch_ivr_schedule_transfer(time_t runtime, const char* uuid, char* extension, char* dialplan,
                             char* context) {
    struct transfer_helper* helper;
    size_t len = sizeof(*helper);
    char* cur = NULL;
    ...
    return switch_scheduler_add_task(runtime, sch_transfer_callback, (char*)__SWITCH_FUNC__, uuid,
                                     0, helper, SSHF_FREE_ARG);
}
...
SWITCH_DECLARE(uint32_t)
switch_ivr_schedule_broadcast(time_t runtime, const char* uuid, const char* path,
                              switch_media_flag_t flags) {
    struct broadcast_helper* helper;
    ...
    return switch_scheduler_add_task(runtime, sch_broadcast_callback, (char*)__SWITCH_FUNC__, uuid,
                                     0, helper, SSHF_FREE_ARG);
}

SWITCH_DECLARE(switch_status_t)
switch_ivr_broadcast(const char* uuid, const char* path, switch_media_flag_t flags) {
    switch_channel_t* channel;
    switch_core_session_t* session;
    switch_event_t* event;
    ...
    if ((flags & SMF_ECHO_BLEG) && (other_uuid = switch_channel_get_partner_uuid(channel)) &&
        ...
        if ((flags & SMF_EXEC_INLINE)) {
            ...
            if (switch_event_create(&event, SWITCH_EVENT_COMMAND) == SWITCH_STATUS_SUCCESS) {
                switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "call-command",
                                               "execute");
                ...
                switch_event_add_header_string(
                    ...
                    (flags & SMF_PRIORITY) ? "event-lock-pri" : "event-lock", "true");
    ...
    if (cause) {
        if (switch_event_create(&event, SWITCH_EVENT_COMMAND) == SWITCH_STATUS_SUCCESS) {
            ...
            switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "execute-app-name",
                                           "hangup");
            switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "execute-app-arg", cause);
            switch_event_add_header_string(event, SWITCH_STACK_BOTTOM,
                                           (flags & SMF_PRIORITY) ? "event-lock-pri" : "event-lock",
                                           "true");
            switch_core_session_queue_private_event(session, &event, (flags & SMF_PRIORITY));
        }
    }
...
static switch_bool_t video_write_overlay_callback(switch_media_bug_t* bug, void* user_data,
                                                  switch_abc_type_t type) {
        ...
        case SWITCH_ABC_TYPE_WRITE_VIDEO_PING:
            if (switch_channel_test_flag(channel, CF_VIDEO_DECODED_READ)) {
                ...
                if (frame->img && oht->img) {
                    ...
                    switch_img_find_position(oht->pos, frame->img->d_w, frame->img->d_h, oimg->d_w,
                                             oimg->d_h, &x, &y);
...
SWITCH_DECLARE(switch_status_t)
switch_ivr_stop_video_write_overlay_session(switch_core_session_t* session) {
    switch_channel_t* channel = switch_core_session_get_channel(session);
...
SWITCH_DECLARE(switch_status_t)
switch_ivr_video_write_overlay_session(switch_core_session_t* session, const char* img_path,
    ...
    if (switch_channel_get_private(channel, "_video_write_overlay_bug_")) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
                          "Only one of this type of bug per channel\n");
        return SWITCH_STATUS_FALSE;
    }
    ...
    if ((status = switch_core_media_bug_add(session, "video_write_overlay", NULL,
                                            video_write_overlay_callback, oht, 0, bflags, &bug)) !=
        SWITCH_STATUS_SUCCESS) {
