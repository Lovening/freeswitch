...
static void switch_core_media_set_r_sdp_codec_string(switch_core_session_t* session,
                                                     const char* codec_string, sdp_session_t* sdp,
                                                     switch_sdp_type_t sdp_type);
static void gen_ice(switch_core_session_t* session, switch_media_type_t type, const char* ip,
                    switch_port_t port);
// #define GOOGLE_ICE
#define RTCP_MUX
...
#define type2str(type) \
    type == SWITCH_MEDIA_TYPE_VIDEO ? "video" : (type == SWITCH_MEDIA_TYPE_AUDIO ? "audio" : "text")
...
// freeswitch core media handle 包含会话的seesion 和channel
...
switch_core_media_crypto_str2type(const char* str) {
    ...
    for (i = 0; i < CRYPTO_INVALID; i++) {
        if (!strncasecmp(str, SUITES[i].name, strlen(SUITES[i].name)) ||
            (SUITES[i].alias && strlen(SUITES[i].alias) &&
             !strncasecmp(str, SUITES[i].alias, strlen(SUITES[i].alias)))) {
...
static switch_t38_options_t* switch_core_media_process_udptl(switch_core_session_t* session,
    ...
    if (m->m_connections && m->m_connections->c_address) {
        ...
        t38_options->remote_ip =
            switch_core_session_strdup(session, sdp->sdp_connection->c_address);
    }
...
switch_core_media_check_autoadj(switch_core_session_t* session) {
    ...
    if (!switch_media_handle_test_media_flag(smh, SCMF_DISABLE_RTP_AUTOADJ) &&
        !((val = switch_channel_get_variable(session->channel, "disable_rtp_auto_adjust")) &&
          switch_true(val)) &&
...
SWITCH_DECLARE(switch_status_t)
switch_core_media_get_vid_params(switch_core_session_t* session, switch_vid_params_t* vid_params) {
    switch_media_handle_t* smh;
...
switch_core_media_process_t38_passthru(switch_core_session_t* session,
    ...
    a_engine->cur_payload_map->remote_sdp_ip =
        switch_core_session_strdup(session, t38_options->remote_ip);
    a_engine->cur_payload_map->remote_sdp_port = t38_options->remote_port;

    if (remote_host && remote_port &&
        !strcmp(remote_host, a_engine->cur_payload_map->remote_sdp_ip) &&
        ...
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                          ...
                          switch_channel_get_name(session->channel), remote_host, remote_port,
                          a_engine->cur_payload_map->remote_sdp_ip,
                          a_engine->cur_payload_map->remote_sdp_port);

        switch_snprintf(tmp, sizeof(tmp), "%d", a_engine->cur_payload_map->remote_sdp_port);
        switch_channel_set_variable(session->channel, SWITCH_REMOTE_MEDIA_IP_VARIABLE,
                                    a_engine->cur_payload_map->remote_sdp_ip);
        switch_channel_set_variable(session->channel, SWITCH_REMOTE_MEDIA_PORT_VARIABLE, tmp);
        if (switch_rtp_set_remote_address(a_engine->rtp_session,
                                          a_engine->cur_payload_map->remote_sdp_ip,
                                          a_engine->cur_payload_map->remote_sdp_port, 0,
...
switch_core_session_get_payload_code(switch_core_session_t* session, switch_media_type_t type,
                                     ...
                                     switch_payload_t* ptP, switch_payload_t* recv_ptP,
                                     char** fmtpP) {
    payload_map_t* pmap;
    switch_media_handle_t* smh;
    ...
    for (pmap = engine->payload_map; pmap; pmap = pmap->next) {
        ...
        if (!strcasecmp(pmap->iananame, iananame) && !strcasecmp(fmtp_a, fmtp_in) &&
            (!rate || (rate == pmap->rate))) {
            pt = pmap->pt;
...
switch_core_media_add_payload_map(switch_core_session_t* session, switch_media_type_t type,
                                  ...
                                  switch_sdp_type_t sdp_type, uint32_t pt, uint32_t rate,
                                  uint32_t ptime, uint32_t channels, uint8_t negotiated) {
    payload_map_t* pmap;
    int exists = 0;
    switch_media_handle_t* smh;
    switch_rtp_engine_t* engine;
        ...
        if (sdp_type == SDP_ANSWER) {
            switch (type) {
                ...
                case SWITCH_MEDIA_TYPE_AUDIO:
                    exists = (type == pmap->type && !strcasecmp(name, pmap->iananame) &&
                              pmap->pt == pt && (!pmap->rate || rate == pmap->rate) &&
                              (!pmap->ptime || pmap->ptime == ptime));
                    break;
                case SWITCH_MEDIA_TYPE_VIDEO:
                    exists = (pmap->sdp_type == SDP_OFFER && type == pmap->type &&
                              !strcasecmp(name, pmap->iananame));

                    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG1,
                                      "CHECK PMAP %s:%s %d %s:%s %d ... %d\n", name, "RES", pt,
                                      pmap->iananame, pmap->sdp_type == SDP_OFFER ? "REQ" : "RES",
                                      pmap->pt, exists);
            ...
            if (type == SWITCH_MEDIA_TYPE_TEXT) {
                exists =
                    (type == pmap->type && !strcasecmp(name, pmap->iananame) && pmap->pt == pt);
            } else {
                exists =
                    (type == pmap->type && !strcasecmp(name, pmap->iananame) && pmap->pt == pt &&
                     (!pmap->rate || rate == pmap->rate) && (!pmap->ptime || pmap->ptime == ptime));
            }
...
switch_core_session_clear_crypto(switch_core_session_t* session) {
    ...
    for (i = 0; i < CRYPTO_INVALID; i++) {
        memset(&smh->engines[SWITCH_MEDIA_TYPE_AUDIO].ssec[i], 0,
               sizeof(smh->engines[SWITCH_MEDIA_TYPE_AUDIO].ssec[i]));
        memset(&smh->engines[SWITCH_MEDIA_TYPE_VIDEO].ssec[i], 0,
               sizeof(smh->engines[SWITCH_MEDIA_TYPE_VIDEO].ssec[i]));
        memset(&smh->engines[SWITCH_MEDIA_TYPE_TEXT].ssec[i], 0,
...
// #define SAME_KEY
    ...
    if (switch_channel_test_flag(channel, CF_AVPF) && type == SWITCH_MEDIA_TYPE_VIDEO) {
        if (direction == SWITCH_RTP_CRYPTO_SEND) {
            memcpy(engine->ssec[ctype].local_raw_key,
                   smh->engines[SWITCH_MEDIA_TYPE_AUDIO].ssec.local_raw_key,
                   SUITES[ctype].keysalt_len);
            key = engine->ssec[ctype].local_raw_key;
        } else {
            memcpy(engine->ssec[ctype].remote_raw_key,
                   smh->engines[SWITCH_MEDIA_TYPE_AUDIO].ssec.remote_raw_key,
                   SUITES[ctype].keysalt_len);
    ...
    if (switch_channel_var_true(channel, "rtp_secure_media_mki")) {
        engine->ssec[ctype].local_crypto_key = switch_core_session_sprintf(
            smh->session, "%d %s inline:%s|2^31|1:1", index,
            (use_alias ? SUITES[ctype].alias : SUITES[ctype].name), b64_key);
    } else {
        engine->ssec[ctype].local_crypto_key = switch_core_session_sprintf(
            smh->session, "%d %s inline:%s", index,
            (use_alias ? SUITES[ctype].alias : SUITES[ctype].name), b64_key);
    }

    switch_channel_set_variable_name_printf(smh->session->channel,
                                            engine->ssec[ctype].local_crypto_key,
                                            "rtp_last_%s_local_crypto_key", type2str(type));
    switch_channel_set_flag(smh->session->channel, CF_SECURE);

    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(smh->session), SWITCH_LOG_DEBUG,
                      "Set Local %s crypto Key [%s]\n", type2str(type),
                      engine->ssec[ctype].local_crypto_key);

    if (!(smh->mparams->ndlb & SM_NDLB_DISABLE_SRTP_AUTH) &&
        !((val = switch_channel_get_variable(channel, "NDLB_support_asterisk_missing_srtp_auth")) &&
          switch_true(val))) {
...
static uint32_t parse_lifetime_mki(const char** p, const char* end) {
    ...
    if (field_begin && (field_begin + 1 < end)) {
        ...
        if (field_end) {
            /* Closing '|' found. */
            sep = strchr(field_begin, ':'); /* The lifetime field never includes a colon, whereas
                                               the third field always does. (RFC 4568) */
...
static switch_crypto_key_material_t* switch_core_media_crypto_append_key_material(
    ...
    unsigned char raw_key[SWITCH_RTP_MAX_CRYPTO_LEN], int raw_key_len, const char* key_material,
    int key_material_len, uint64_t lifetime, unsigned int mki_id, unsigned int mki_size) {
...
/*
 * Skip all space and return pointer to the '\0' terminator of the char string candidate to be a
 * key-material or pointer to first ' ' in the candidate string.
 */
static const char* switch_core_media_crypto_find_key_material_candidate_end(const char* p) {
    ...
    if (p) {
            ...
            p,
            ' '); /* find the end of the continuous string of characters in the candidate string */
...
SWITCH_DECLARE(switch_status_t)
switch_core_media_add_crypto(switch_core_session_t* session, switch_secure_settings_t* ssec,
                             switch_rtp_crypto_direction_t direction) {
    ...
    do {
        if (*key_material_n == SWITCH_CRYPTO_MKI_MAX) {
            switch_log_printf(
                ...
                "Skipping excess of MKIs due to max number of suppoerted MKIs %d exceeded\n",
        ...
        if (opts) { /* if opts != NULL then opts points to first '|' in current key-material
                       cadidate, parse it as LIFETIME or MKI */
            ...
            for (int i = 0; i < 2 && (*opts == '|'); ++i) {
                ...
                switch ((opt_field >> 24) & 0x3) {
                    case CRYPTO_KEY_MATERIAL_LIFETIME:
                        ...
                        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                                          "LIFETIME found in %s, base %u exp %u\n", p,
                                          lifetime_base, lifetime_exp);
                        break;
            ...
            if (mki_id == 0 && lifetime == 0) {
                switch_log_printf(
                    ...
                    "Bad MKI found in %s, (parsed as: id %u size %u lifetime base %u exp %u\n", p,
                    mki_id, mki_size, lifetime_base, lifetime_exp);
                return SWITCH_STATUS_FALSE;
            } else if (mki_id == 0 || lifetime == 0) {
                if (mki_id == 0) {
                    if (key_material) goto bad_key_no_mki_index;

                    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_NOTICE,
                                      "Skipping MKI due to empty index\n");
                } else {
                    if (mki_size == 0) goto bad_key_no_mki_size;

                    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_NOTICE,
                                      "Skipping MKI due to empty lifetime\n");
                }
                continue;
            }
        }
        ...
        key_material = switch_core_media_crypto_append_key_material(
            session, key_material, method, (unsigned char*)key, SUITES[type].keysalt_len,
            (char*)key_material_begin, key_material_end - key_material_begin, lifetime, mki_id,
            mki_size);
...
bad_crypto:
    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
                      "Error! SRTP: Invalid format of crypto attribute %s\n", key_param);
    return SWITCH_STATUS_FALSE;

bad_keysalt:
    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
                      "Error! SRTP: Invalid keysalt in the crypto attribute %s\n", key_param);
    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
                      "Error! Parsing near %s\n", p);
    return SWITCH_STATUS_FALSE;
...
bad_keysalt_len:
    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
                      "Error! SRTP: Invalid keysalt length in the crypto attribute %s\n",
                      key_param);
    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
                      "Error! Parsing near %s\n", p);
    return SWITCH_STATUS_FALSE;
...
bad_key_lifetime_or_mki:
    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
                      "Error! SRTP: Invalid key param MKI or LIFETIME in the crypto attribute %s\n",
                      key_param);
    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
                      "Error! Parsing near %s\n", p);
    return SWITCH_STATUS_FALSE;
...
bad_key_no_mki_index:
    switch_log_printf(
        SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
        "Crypto invalid: multiple keys in a single crypto MUST all have MKI indices, %s\n",
        key_param);
    return SWITCH_STATUS_FALSE;

bad_key_no_mki_size:
    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
                      "Crypto invalid: MKI index with no MKI size in %s\n", key_param);
    return SWITCH_STATUS_FALSE;

bad_key_mki_size:
    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
                      "Crypto invalid: MKI sizes differ in %s\n", key_param);
    return SWITCH_STATUS_FALSE;

bad_key_param_method:
    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
                      "Error! SRTP: Invalid key param method type in %s\n", key_param);
    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
                      "Error! Parsing near %s\n", p);
    return SWITCH_STATUS_FALSE;
}
...
SWITCH_DECLARE(void)
switch_core_media_set_rtp_session(switch_core_session_t* session, switch_media_type_t type,
                                  switch_rtp_t* rtp_session) {
    switch_rtp_engine_t* engine;
...
switch_core_media_get_rtp_session(switch_core_session_t* session, switch_media_type_t type) {
    switch_assert(session);
...
static void switch_core_session_get_recovery_crypto_key(switch_core_session_t* session,
    ...
    if ((tmp = switch_channel_get_variable(session->channel, keyvar))) {
        ...
        engine->ssec[engine->crypto_type].remote_crypto_key =
            switch_core_session_strdup(session, tmp);
...
static void switch_core_session_apply_crypto(switch_core_session_t* session,
                                             switch_media_type_t type) {
    switch_rtp_engine_t* engine;
    const char* varname;
    ...
    if (engine->ssec[engine->crypto_type].remote_crypto_key &&
        switch_channel_test_flag(session->channel, CF_SECURE)) {
        if (switch_channel_var_true(session->channel, "rtp_secure_media_mki"))
            switch_core_media_add_crypto(session, &engine->ssec[engine->crypto_type],
        ...
        switch_rtp_add_crypto_key(engine->rtp_session, SWITCH_RTP_CRYPTO_SEND, 1,
                                  &engine->ssec[engine->crypto_type]);
        ...
        switch_channel_set_variable(session->channel, "rtp_secure_media_negotiated",
                                    SUITES[engine->crypto_type].name);
    }
}
    ...
    if (!strcasecmp(val, "optional")) {
        ...
        if (!switch_false(val) && strcasecmp(val, "forbidden")) {
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING,
                              "INVALID VALUE FOR %s defaulting to 'forbidden'\n", var);
        }
    }
...
SWITCH_DECLARE(int)
switch_core_session_check_incoming_crypto(switch_core_session_t* session, const char* varname,
                                          switch_media_type_t type, const char* crypto,
                                          int crypto_tag, switch_sdp_type_t sdp_type) {
    int got_crypto = 0;
    int i = 0;
    int ctype = 0;
    ...
    for (i = 0; smh->crypto_suite_order[i] != CRYPTO_INVALID; i++) {
        ...
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                          "looking for crypto suite [%s]alias=[%s] in [%s]\n", SUITES[j].name,
                          SUITES[j].alias, crypto);
    ...
    if (engine->ssec[engine->crypto_type].remote_crypto_key &&
        switch_rtp_ready(engine->rtp_session)) {
        /* Compare all the key. The tag may remain the same even if key changed */
        if (engine->crypto_type != CRYPTO_INVALID &&
            ...
            const char* a =
                switch_stristr("AE", engine->ssec[engine->crypto_type].remote_crypto_key);
            const char* b = switch_stristr("AE", crypto);

            if (sdp_type == SDP_OFFER) {
                if (!vval) {
                    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                                      "Unsupported Crypto [%s]\n", crypto);
                    goto end;
                }
                switch_channel_set_variable(session->channel, varname, vval);

                switch_core_media_build_crypto(session->media_handle, type, crypto_tag, ctype,
                                               SWITCH_RTP_CRYPTO_SEND, 1, use_alias);
                switch_rtp_add_crypto_key(engine->rtp_session, SWITCH_RTP_CRYPTO_SEND, atoi(crypto),
                                          &engine->ssec[engine->crypto_type]);
            }
            ...
            if (a && b && !strncasecmp(a, b, 23)) {
                ...
                engine->ssec[engine->crypto_type].remote_crypto_key =
                    switch_core_session_strdup(session, crypto);

                if (engine->type == SWITCH_MEDIA_TYPE_AUDIO) {
                    switch_channel_set_variable(session->channel, "srtp_remote_audio_crypto_key",
                                                crypto);
                    switch_channel_set_variable_printf(
                        session->channel, "srtp_remote_audio_crypto_tag", "%d", crypto_tag);
                    switch_channel_set_variable_printf(session->channel,
                                                       "srtp_remote_audio_crypto_type", "%s",
                                                       switch_core_media_crypto_type2str(ctype));
                } else if (engine->type == SWITCH_MEDIA_TYPE_VIDEO) {
                    switch_channel_set_variable(session->channel, "srtp_remote_video_crypto_key",
                                                crypto);
                    switch_channel_set_variable_printf(
                        session->channel, "srtp_remote_video_crypto_tag", "%d", crypto_tag);
                    switch_channel_set_variable_printf(session->channel,
                                                       "srtp_remote_video_crypto_type", "%s",
                                                       switch_core_media_crypto_type2str(ctype));
                } else if (engine->type == SWITCH_MEDIA_TYPE_TEXT) {
                    switch_channel_set_variable(session->channel, "srtp_remote_text_crypto_key",
                                                crypto);
                    switch_channel_set_variable_printf(
                        session->channel, "srtp_remote_text_crypto_tag", "%d", crypto_tag);
                    switch_channel_set_variable_printf(session->channel,
                                                       "srtp_remote_text_crypto_type", "%s",
                                                       switch_core_media_crypto_type2str(ctype));
                }
        ...
        if (engine->type == SWITCH_MEDIA_TYPE_AUDIO) {
            ...
            switch_channel_set_variable_printf(session->channel, "srtp_remote_audio_crypto_type",
                                               "%s", switch_core_media_crypto_type2str(ctype));
        } else if (engine->type == SWITCH_MEDIA_TYPE_VIDEO) {
            switch_channel_set_variable(session->channel, "srtp_remote_video_crypto_key", crypto);
            switch_channel_set_variable_printf(session->channel, "srtp_remote_video_crypto_type",
                                               "%s", switch_core_media_crypto_type2str(ctype));
        } else if (engine->type == SWITCH_MEDIA_TYPE_TEXT) {
            switch_channel_set_variable(session->channel, "srtp_remote_text_crypto_key", crypto);
            switch_channel_set_variable_printf(session->channel, "srtp_remote_text_crypto_type",
                                               "%s", switch_core_media_crypto_type2str(ctype));
        }
...
SWITCH_DECLARE(void)
switch_core_session_check_outgoing_crypto(switch_core_session_t* session) {
    ...
    for (i = 0; smh->crypto_suite_order[i] != CRYPTO_INVALID; i++) {
        switch_core_media_build_crypto(session->media_handle, SWITCH_MEDIA_TYPE_AUDIO,
                                       SWITCH_NO_CRYPTO_TAG, smh->crypto_suite_order[i],
                                       SWITCH_RTP_CRYPTO_SEND, 0, 0);

        switch_core_media_build_crypto(session->media_handle, SWITCH_MEDIA_TYPE_VIDEO,
                                       SWITCH_NO_CRYPTO_TAG, smh->crypto_suite_order[i],
                                       SWITCH_RTP_CRYPTO_SEND, 0, 0);

        switch_core_media_build_crypto(session->media_handle, SWITCH_MEDIA_TYPE_TEXT,
                                       SWITCH_NO_CRYPTO_TAG, smh->crypto_suite_order[i],
                                       SWITCH_RTP_CRYPTO_SEND, 0, 0);
    }
}
...
static void set_stats(switch_core_session_t* session, switch_media_type_t type,
                      const char* prefix) {
...
SWITCH_DECLARE(void)
switch_core_media_sync_stats(switch_core_session_t* session) {
    switch_media_handle_t* smh;
...
SWITCH_DECLARE(void)
switch_core_media_set_stats(switch_core_session_t* session) {
    if (!session->media_handle) {
        return;
    }
...
SWITCH_DECLARE(void)
switch_media_handle_destroy(switch_core_session_t* session) {
    switch_media_handle_t* smh;
    switch_rtp_engine_t *a_engine, *v_engine;  //, *t_engine;
...
SWITCH_DECLARE(switch_status_t)
switch_media_handle_create(switch_media_handle_t** smhp, switch_core_session_t* session,
    ...
    *smhp = NULL;
    ...
    if ((session->media_handle = switch_core_session_alloc(session, (sizeof(*smh))))) {
        ...
        *smhp = session->media_handle;
        ...
        session->media_handle->engines[SWITCH_MEDIA_TYPE_AUDIO].read_frame.buflen =
        ...
        session->media_handle->engines[SWITCH_MEDIA_TYPE_TEXT].read_frame.buflen =
            SWITCH_RTP_MAX_BUF_LEN;
        ...
        session->media_handle->engines[SWITCH_MEDIA_TYPE_VIDEO].read_frame.buflen =
            SWITCH_RTP_MAX_BUF_LEN;
        ...
        switch_mutex_init(&session->media_handle->mutex, SWITCH_MUTEX_NESTED,
                          switch_core_session_get_pool(session));
        switch_mutex_init(&session->media_handle->sdp_mutex, SWITCH_MUTEX_NESTED,
                          switch_core_session_get_pool(session));
        switch_mutex_init(&session->media_handle->control_mutex, SWITCH_MUTEX_NESTED,
                          switch_core_session_get_pool(session));

        session->media_handle->engines[SWITCH_MEDIA_TYPE_AUDIO].ssrc =
            (uint32_t)((intptr_t)&session->media_handle->engines[SWITCH_MEDIA_TYPE_AUDIO] +
                       (switch_time_t)time(NULL));

        session->media_handle->engines[SWITCH_MEDIA_TYPE_VIDEO].ssrc =
            (uint32_t)((intptr_t)&session->media_handle->engines[SWITCH_MEDIA_TYPE_VIDEO] +
                       (switch_time_t)time(NULL) / 2);

        session->media_handle->engines[SWITCH_MEDIA_TYPE_TEXT].ssrc =
            (uint32_t)((intptr_t)&session->media_handle->engines[SWITCH_MEDIA_TYPE_TEXT] +
                       (switch_time_t)time(NULL) / 2);

        session->media_handle->engines[SWITCH_MEDIA_TYPE_AUDIO].payload_map =
            switch_core_alloc(session->pool, sizeof(payload_map_t));
        session->media_handle->engines[SWITCH_MEDIA_TYPE_AUDIO].cur_payload_map =
            session->media_handle->engines[SWITCH_MEDIA_TYPE_AUDIO].payload_map;
        session->media_handle->engines[SWITCH_MEDIA_TYPE_AUDIO].cur_payload_map->current = 1;

        session->media_handle->engines[SWITCH_MEDIA_TYPE_VIDEO].payload_map =
            switch_core_alloc(session->pool, sizeof(payload_map_t));
        session->media_handle->engines[SWITCH_MEDIA_TYPE_VIDEO].cur_payload_map =
            session->media_handle->engines[SWITCH_MEDIA_TYPE_VIDEO].payload_map;
        session->media_handle->engines[SWITCH_MEDIA_TYPE_VIDEO].cur_payload_map->current = 1;
        session->media_handle->engines[SWITCH_MEDIA_TYPE_VIDEO]
            .codec_settings.video.try_hardware_encoder = 1;

        session->media_handle->engines[SWITCH_MEDIA_TYPE_TEXT].payload_map =
            switch_core_alloc(session->pool, sizeof(payload_map_t));
        session->media_handle->engines[SWITCH_MEDIA_TYPE_TEXT].cur_payload_map =
            session->media_handle->engines[SWITCH_MEDIA_TYPE_TEXT].payload_map;
...
SWITCH_DECLARE(void)
switch_media_handle_set_media_flag(switch_media_handle_t* smh, switch_core_media_flag_t flag) {
    switch_assert(smh);
...
SWITCH_DECLARE(void)
switch_media_handle_clear_media_flag(switch_media_handle_t* smh, switch_core_media_flag_t flag) {
    switch_assert(smh);
...
switch_media_handle_test_media_flag(switch_media_handle_t* smh, switch_core_media_flag_t flag) {
    switch_assert(smh);
    return smh->media_flags[flag];
}

SWITCH_DECLARE(switch_media_flow_t)
switch_core_session_media_flow(switch_core_session_t* session, switch_media_type_t type) {
...
SWITCH_DECLARE(switch_media_flow_t)
switch_core_session_remote_media_flow(switch_core_session_t* session, switch_media_type_t type) {
    switch_media_flow_t flow = SWITCH_MEDIA_FLOW_SENDRECV;
    switch_media_handle_t* smh;
    switch_rtp_engine_t* engine = NULL;
...
SWITCH_DECLARE(switch_status_t)
switch_core_session_media_handle_ready(switch_core_session_t* session) {
...
switch_core_session_get_media_handle(switch_core_session_t* session) {
    if (switch_core_session_media_handle_ready(session) == SWITCH_STATUS_SUCCESS) {
...
SWITCH_DECLARE(switch_status_t)
switch_core_session_clear_media_handle(switch_core_session_t* session) {
    if (!session->media_handle) {
        return SWITCH_STATUS_FALSE;
    }
...
switch_core_media_prepare_codecs(switch_core_session_t* session, switch_bool_t force) {
    ...
    if (!force && (switch_channel_test_flag(session->channel, CF_PROXY_MODE) ||
    ...
    val = switch_channel_get_variable_dup(session->channel, "media_mix_inbound_outbound_codecs",
                                          SWITCH_FALSE, -1);

    if (!switch_channel_test_flag(session->channel, CF_ANSWERED) &&
        (!val || !switch_true(val) || smh->media_flags[SCMF_DISABLE_TRANSCODING]) && ocodec) {
        codec_string = ocodec;
        goto ready;
    }
...
ready:
    ...
    smh->codec_order_last =
        switch_separate_string(tmp_codec_string, ',', smh->codec_order, SWITCH_MAX_CODECS);
    smh->mparams->num_codecs = switch_loadable_module_get_codecs_sorted(
        smh->codecs, smh->fmtp, SWITCH_MAX_CODECS, smh->codec_order, smh->codec_order_last);
    return SWITCH_STATUS_SUCCESS;
}

static void check_jb(switch_core_session_t* session, const char* input, int32_t jb_msec,
                     int32_t maxlen, switch_bool_t silent) {
    const char* val;
    switch_media_handle_t* smh;
    ...
    if (jb_msec || (val = switch_channel_get_variable(session->channel, "jitterbuffer_msec")) ||
        ...
        if (jb_msec < 0 && jb_msec > -1000) {
            jb_msec = (a_engine->read_codec.implementation->microseconds_per_packet / 1000) *
                      abs(jb_msec);
        }

        if (maxlen < 0 && maxlen > -1000) {
            maxlen =
                (a_engine->read_codec.implementation->microseconds_per_packet / 1000) * abs(maxlen);
        }
        ...
        if (jb_msec && maxlen) {
            ...
            if (switch_rtp_activate_jitter_buffer(
                    a_engine->rtp_session, qlen, maxqlen, a_engine->read_impl.samples_per_packet,
                ...
                if (!switch_false(
                        switch_channel_get_variable(session->channel, "rtp_jitter_buffer_plc"))) {
...
static void check_jb_sync(switch_core_session_t* session) {
    ...
    if ((var = switch_channel_get_variable_dup(session->channel, "jb_av_sync_msec", SWITCH_FALSE,
                                               -1))) {
        int tmp;
        char* p;
    ...
    switch_log_printf(
        SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG1,
...
//?
SWITCH_DECLARE(switch_status_t)
switch_core_media_read_lock_unlock(switch_core_session_t* session, switch_media_type_t type,
                                   switch_bool_t lock) {
    switch_rtp_engine_t* engine;
    switch_media_handle_t* smh;
    ...
    if (!switch_channel_up_nosig(session->channel) || !switch_rtp_ready(engine->rtp_session) ||
        switch_channel_test_flag(session->channel, CF_NOT_READY)) {
        return SWITCH_STATUS_FALSE;
    }

    if (lock) {
        if (smh->read_mutex[type] &&
            switch_mutex_trylock(smh->read_mutex[type]) != SWITCH_STATUS_SUCCESS) {
...
//?
SWITCH_DECLARE(switch_status_t)
switch_rtp_text_factory_create(switch_rtp_text_factory_t** tfP, switch_memory_pool_t* pool) {
    int x;
...
SWITCH_DECLARE(switch_status_t)
switch_rtp_text_factory_destroy(switch_rtp_text_factory_t** tfP) {
...
static int get_rtt_red_seq(int want_seq, void* data, switch_size_t datalen, int seq,
                           switch_payload_t* new_payload, void* new_data, uint32_t* new_datalen) {
...
static void* get_rtt_payload(void* data, switch_size_t datalen, switch_payload_t* new_payload,
                             uint32_t* new_datalen, int* red_level) {
    unsigned char* buf = data;
...
static void check_media_timeout_params(switch_core_session_t* session,
                                       switch_rtp_engine_t* engine) {
    ...
    if (switch_rtp_ready(engine->rtp_session) && engine->media_timeout) {
        ...
        if (engine->type == SWITCH_MEDIA_TYPE_AUDIO) {
            ...
            engine->max_missed_packets =
                (engine->read_impl.samples_per_second * engine->media_timeout / 1000) /
            ...
            engine->max_missed_hold_packets =
                (engine->read_impl.samples_per_second * engine->media_hold_timeout / 1000) /
                engine->read_impl.samples_per_packet;
        }
    }
}
...
SWITCH_DECLARE(switch_status_t)
switch_core_media_read_frame(switch_core_session_t* session, switch_frame_t** frame,
    ...
    if (type == SWITCH_MEDIA_TYPE_AUDIO && !switch_channel_test_flag(session->channel, CF_AUDIO)) {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG1,
                          "%s Reading audio from a non-audio session.\n",
                          switch_channel_get_name(session->channel));
        switch_yield(50000);
        return SWITCH_STATUS_INUSE;
    }
    ...
    if (type != SWITCH_MEDIA_TYPE_TEXT &&
        (!engine->read_codec.implementation || !switch_core_codec_ready(&engine->read_codec))) {
        switch_yield(50000);
        return SWITCH_STATUS_FALSE;
    }
    ...
    while (smh->media_flags[SCMF_RUNNING] && engine->read_frame.datalen == 0) {
        ...
        if (type == SWITCH_MEDIA_TYPE_VIDEO) {
            if (engine->read_frame.m) {
                ...
                if (video_globals.fps &&
                    (!video_globals.synced || ((smh->vid_frames % 300) == 0))) {
        ...
        if (type != SWITCH_MEDIA_TYPE_TEXT && engine->reset_codec > 0) {
            ...
            if (switch_rtp_ready(engine->rtp_session)) {
                ...
                if (type == SWITCH_MEDIA_TYPE_VIDEO) {
                    ...
                    if (switch_core_media_set_codec(session, 1, smh->mparams->codec_flags) !=
                ...
                if (type == SWITCH_MEDIA_TYPE_AUDIO && engine->read_impl.samples_per_second) {
                    ...
                    if ((val = switch_channel_get_variable(session->channel,
                    ...
                    if (rtp_timeout_sec) {
                        engine->max_missed_packets =
                            (engine->read_impl.samples_per_second * rtp_timeout_sec) /
                            engine->read_impl.samples_per_packet;

                        switch_rtp_set_max_missed_packets(engine->rtp_session,
                                                          engine->max_missed_packets);
                    ...
                    if (rtp_hold_timeout_sec) {
                        engine->max_missed_hold_packets =
                            (engine->read_impl.samples_per_second * rtp_hold_timeout_sec) /
                            engine->read_impl.samples_per_packet;
                    }
                }
            }
        ...
        if (switch_rtcp_zerocopy_read_frame(engine->rtp_session, &rtcp_frame) ==
            SWITCH_STATUS_SUCCESS) {
            switch_event_t* event;

            if (switch_event_create(&event, SWITCH_EVENT_RECV_RTCP_MESSAGE) ==
                SWITCH_STATUS_SUCCESS) {
                char value[30];
                char header[50];
                int i;
                ...
                switch_event_add_header_string(event, SWITCH_STACK_BOTTOM,
                                               "NTP-Most-Significant-Word", value);

                snprintf(value, sizeof(value), "%u", rtcp_frame.ntp_lsw);
                switch_event_add_header_string(event, SWITCH_STACK_BOTTOM,
                                               "NTP-Least-Significant-Word", value);
                ...
                switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "Sender-Packet-Count",
                                               value);

                snprintf(value, sizeof(value), "%u", rtcp_frame.octect_count);
                switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "Octect-Packet-Count",
                                               value);

                snprintf(value, sizeof(value), "%u", engine->read_frame.timestamp);
                switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "Last-RTP-Timestamp",
                                               value);
                ...
                for (i = 0; i < rtcp_frame.report_count; i++) {
                    ...
                    switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, header, value);
                    snprintf(header, sizeof(header), "Source%u-Highest-Sequence-Number-Received",
                             i);
                    snprintf(value, sizeof(value), "%u",
                             rtcp_frame.reports[i].highest_sequence_number_received);
                    switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, header, value);
        ...
        if (type != SWITCH_MEDIA_TYPE_TEXT && engine->read_frame.datalen > 0) {
            ...
            if (!switch_test_flag((&engine->read_frame), SFF_CNG)) {
                ...
                if (smh->media_flags[SCMF_AUTOFIX_TIMING] && type == SWITCH_MEDIA_TYPE_AUDIO &&
                    engine->read_impl.samples_per_second) {
                    char is_vbr;
                    ...
                    if ((smh->media_flags[SCMF_AUTOFIX_TIMING] &&
                         (engine->read_frame.datalen % 10) == 0) &&
                        (engine->check_frames < MAX_CODEC_CHECK_FRAMES) && !is_vbr) {
                        engine->check_frames++;

                        if (engine->last_ts && engine->read_frame.datalen !=
                                                   engine->read_impl.encoded_bytes_per_packet) {
                            uint32_t codec_ms =
                                (int)(engine->read_frame.timestamp - engine->last_ts) /
                                (engine->read_impl.samples_per_second / 1000);
                            if (engine->last_seq &&
                                (int)(engine->read_frame.seq - engine->last_seq) > 1) {
                                switch_log_printf(
                                    SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                                    "[CBR]: Correcting calculated ptime value from [%d] to [%d] to "
                                    "compensate for [%d] lost packet(s). \n",
                                    codec_ms,
                                    codec_ms / (int)(engine->read_frame.seq - engine->last_seq),
                                    (int)(engine->read_frame.seq - engine->last_seq - 1));
                                codec_ms =
                                    codec_ms / (int)(engine->read_frame.seq - engine->last_seq);
                            }
                            ...
                            if ((codec_ms % 10) != 0 ||
                                codec_ms > engine->read_impl.samples_per_packet * 10) {
                                engine->last_ts = 0;
                                engine->last_seq = 0;
                                goto skip;
                            }
                                ...
                                if (codec_ms != engine->cur_payload_map->codec_ms) {
                                    if (codec_ms > 120) { /* yeah right */
                                        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session),
                                                          SWITCH_LOG_WARNING,
                                                          "[CBR]: Your phone is trying to send "
                                                          "timestamps that suggest "
                                                          "an increment of %dms per packet\n"
                                                          "That seems hard to believe so I am "
                                                          "going to go on ahead and "
                                                          "um ignore that, mmkay?\n",
                                                          (int)codec_ms);
                                    ...
                                    if (codec_ms != engine->cur_payload_map->codec_ms) {
                                        switch_log_printf(
                                            SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING,
                                            "[CBR]: Asynchronous PTIME not supported, changing our "
                                            "end from %d to %d\n",
                                            (int)engine->cur_payload_map->codec_ms, (int)codec_ms);

                                        switch_channel_set_variable_printf(
                                            session->channel, "rtp_h_X-Broken-PTIME",
                                            "Adv=%d;Sent=%d",
                                            (int)engine->cur_payload_map->codec_ms, (int)codec_ms);
                    ...
                    } else if (smh->media_flags[SCMF_AUTOFIX_TIMING] && is_vbr &&
                               switch_rtp_get_jitter_buffer(engine->rtp_session) &&
                               type == SWITCH_MEDIA_TYPE_AUDIO && engine->read_frame.timestamp &&
                               engine->read_frame.seq && engine->read_impl.samples_per_second) {
                        uint32_t codec_ms = (int)(engine->read_frame.timestamp - engine->last_ts) /
                                            (engine->read_impl.samples_per_second / 1000);
                        ...
                        if (engine->last_seq &&
                            ...
                            switch_log_printf(
                                SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                                "[VBR]: Correcting calculated ptime value from [%d] to [%d] to "
                                "compensate for [%d] lost packet(s)\n",
                                codec_ms,
                                codec_ms / (int)(engine->read_frame.seq - engine->last_seq),
                                (int)(engine->read_frame.seq - engine->last_seq - 1));
                        ...
                        if (engine->mismatch_count > MAX_MISMATCH_FRAMES) {
                            if (codec_ms > 120) {
                                ...
                                switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session),
                                                  SWITCH_LOG_DEBUG3,
                                                  "[VBR]: Remote party is trying to send "
                                                  "timestamps that suggest an "
                                                  "increment of [%d] ms per packet, which is too "
                                                  "high. Ignoring.\n",
                                                  (int)codec_ms);
                                engine->last_ts = engine->read_frame.timestamp;
                                engine->last_seq = engine->read_frame.seq;
                            ...
                            if (codec_ms != engine->cur_payload_map->codec_ms) {
                                switch_log_printf(
                                    SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING,
                                    "[VBR]: Packet size change detected. Remote PTIME changed from "
                                    "[%d] to [%d]\n",
                                    (int)engine->cur_payload_map->codec_ms, (int)codec_ms);
                                engine->cur_payload_map->codec_ms = codec_ms;
                                engine->reset_codec = 2;
                                ...
                                if (switch_channel_test_flag(session->channel, CF_CONFERENCE)) {
                                    switch_channel_set_flag(session->channel,
                ...
                if (!engine->reset_codec && engine->codec_negotiated &&
            ...
            *(p + engine->read_frame.datalen) = '\0';
            ...
            if (switch_test_flag((&engine->read_frame), SFF_PLC)) {
                ...
                for (i = 1; i < 3; i++) {
                    ...
                    if (switch_jb_peek_frame(jb, 0, engine->read_frame.seq, i, &frame) ==
                        SWITCH_STATUS_SUCCESS) {
                        if (get_rtt_red_seq(engine->read_frame.seq, frame.data, frame.datalen,
                                            frame.seq, &engine->tf->text_frame.payload,
                ...
                if (!(engine->tf->text_frame.data = get_rtt_payload(
                          engine->read_frame.data, engine->tf->text_frame.datalen,
                          &engine->tf->text_frame.payload, &engine->tf->text_frame.datalen,
                          &engine->tf->red_level))) {
                    engine->tf->text_frame.datalen = 0;
                }
            }
...
//?
SWITCH_DECLARE(switch_status_t)
switch_core_media_write_frame(switch_core_session_t* session, switch_frame_t* frame,
                              switch_io_flag_t flags, int stream_id, switch_media_type_t type) {
    ...
    if (type == SWITCH_MEDIA_TYPE_AUDIO) {
        switch_media_flow_t audio_flow =
            switch_core_session_media_flow(session, SWITCH_MEDIA_TYPE_AUDIO);
...
switch_core_media_copy_t38_options(switch_t38_options_t* t38_options,
    ...
    local_t38_options->T38FaxRateManagement =
        switch_core_session_strdup(session, t38_options->T38FaxRateManagement);
    ...
    local_t38_options->T38VendorInfo =
        switch_core_session_strdup(session, t38_options->T38VendorInfo);
...
switch_core_media_get_offered_pt(switch_core_session_t* session,
    ...
    for (i = 0; i < smh->mparams->num_codecs; i++) {
        ...
        if (!strcasecmp(imp->iananame, mimp->iananame) &&
            imp->actual_samples_per_second == mimp->actual_samples_per_second) {
            *pt = smh->ianacodes[i];
...
static void switch_core_session_parse_codec_settings(switch_core_session_t* session,
    ...
    switch (type) {
        ...
        case SWITCH_MEDIA_TYPE_VIDEO: {
            ...
            if ((var =
                     switch_channel_get_variable(session->channel, "video_try_hardware_encoder"))) {
...
switch_core_media_set_video_codec(switch_core_session_t* session, int force) {
    ...
    if (v_engine->read_codec.implementation && switch_core_codec_ready(&v_engine->read_codec)) {
        ...
        if (strcasecmp(v_engine->read_codec.implementation->iananame,
                       v_engine->cur_payload_map->rm_encoding) ||
            v_engine->read_codec.implementation->samples_per_second !=
                v_engine->cur_payload_map->rm_rate) {
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                              "Changing Codec from %s to %s\n",
    ...
    if (switch_core_codec_init(
            &v_engine->read_codec, v_engine->cur_payload_map->rm_encoding,
            v_engine->cur_payload_map->modname, v_engine->cur_payload_map->rm_fmtp,
            v_engine->cur_payload_map->rm_rate, 0, 1,
            SWITCH_CODEC_FLAG_ENCODE | SWITCH_CODEC_FLAG_DECODE, &v_engine->codec_settings,
        ...
        if (switch_core_codec_init(
                &v_engine->write_codec, v_engine->cur_payload_map->rm_encoding,
                v_engine->cur_payload_map->modname, v_engine->cur_payload_map->rm_fmtp,
                v_engine->cur_payload_map->rm_rate, 0, 1,
                SWITCH_CODEC_FLAG_ENCODE | SWITCH_CODEC_FLAG_DECODE, &v_engine->codec_settings,
                switch_core_session_get_pool(session)) != SWITCH_STATUS_SUCCESS) {
            ...
            switch_log_printf(
                SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                "Set VIDEO Codec %s %s/%ld %d ms\n", switch_channel_get_name(session->channel),
                v_engine->cur_payload_map->rm_encoding, v_engine->cur_payload_map->rm_rate,
                v_engine->cur_payload_map->codec_ms);
            v_engine->read_frame.codec = &v_engine->read_codec;

            v_engine->write_codec.fmtp_out =
                switch_core_session_strdup(session, v_engine->write_codec.fmtp_out);
            ...
            switch_channel_set_variable_printf(session->channel, "rtp_last_video_codec_string",
                                               "%s@%dh", v_engine->cur_payload_map->rm_encoding,
                                               v_engine->cur_payload_map->rm_rate);
            ...
            if (switch_rtp_ready(v_engine->rtp_session)) {
                ...
                switch_rtp_set_default_payload(v_engine->rtp_session,
            ...
            switch_channel_set_variable(session->channel, "rtp_use_video_codec_name",
                                        v_engine->cur_payload_map->rm_encoding);
            switch_channel_set_variable(session->channel, "rtp_use_video_codec_fmtp",
                                        v_engine->cur_payload_map->rm_fmtp);
            switch_channel_set_variable_printf(session->channel, "rtp_use_video_codec_rate", "%d",
                                               v_engine->cur_payload_map->rm_rate);
            switch_channel_set_variable_printf(session->channel, "rtp_use_video_codec_ptime", "%d",
                                               0);
        }
    }
...
switch_core_media_set_codec(switch_core_session_t* session, int force, uint32_t codec_flags) {
    ...
    if (switch_core_codec_ready(&a_engine->read_codec)) {
        ...
        if (strcasecmp(a_engine->read_impl.iananame, a_engine->cur_payload_map->iananame) ||
            (uint32_t)a_engine->read_impl.microseconds_per_packet / 1000 !=
            ...
            switch_log_printf(
                ...
                a_engine->cur_payload_map->iananame, a_engine->cur_payload_map->codec_ms,
    ...
    if (switch_core_codec_init_with_bitrate(
            &a_engine->read_codec, a_engine->cur_payload_map->iananame,
            a_engine->cur_payload_map->modname, a_engine->cur_payload_map->rm_fmtp,
            a_engine->cur_payload_map->rm_rate, a_engine->cur_payload_map->codec_ms,
            a_engine->cur_payload_map->channels, a_engine->cur_payload_map->bitrate,
            SWITCH_CODEC_FLAG_ENCODE | SWITCH_CODEC_FLAG_DECODE | codec_flags,
            &a_engine->codec_settings,
            switch_core_session_get_pool(session)) != SWITCH_STATUS_SUCCESS) {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
                          "Can't load codec?\n");
        switch_channel_hangup(session->channel, SWITCH_CAUSE_INCOMPATIBLE_DESTINATION);
    ...
    if (switch_core_codec_init_with_bitrate(
            &a_engine->write_codec, a_engine->cur_payload_map->iananame,
            a_engine->cur_payload_map->modname, a_engine->cur_payload_map->rm_fmtp,
            a_engine->cur_payload_map->rm_rate, a_engine->cur_payload_map->codec_ms,
            a_engine->cur_payload_map->channels, a_engine->cur_payload_map->bitrate,
            SWITCH_CODEC_FLAG_ENCODE | SWITCH_CODEC_FLAG_DECODE | codec_flags,
    ...
    switch_channel_set_variable(session->channel, "rtp_use_codec_name",
                                a_engine->cur_payload_map->iananame);
    switch_channel_set_variable(session->channel, "rtp_use_codec_fmtp",
                                a_engine->cur_payload_map->rm_fmtp);
    switch_channel_set_variable_printf(session->channel, "rtp_use_codec_rate", "%d",
                                       a_engine->cur_payload_map->rm_rate);
    switch_channel_set_variable_printf(session->channel, "rtp_use_codec_ptime", "%d",
                                       a_engine->cur_payload_map->codec_ms);
    switch_channel_set_variable_printf(session->channel, "rtp_use_codec_channels", "%d",
                                       a_engine->cur_payload_map->channels);
    switch_channel_set_variable_printf(
        session->channel, "rtp_last_audio_codec_string", "%s@%dh@%di@%dc",
        a_engine->cur_payload_map->iananame, a_engine->cur_payload_map->rm_rate,
        a_engine->cur_payload_map->codec_ms, a_engine->cur_payload_map->channels);
    ...
    if (switch_rtp_ready(a_engine->rtp_session)) {
        ...
        if (switch_rtp_change_interval(
                a_engine->rtp_session, a_engine->read_impl.microseconds_per_packet,
    ...
    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                      "Set Codec %s %s/%ld %d ms %d samples %d bits %d channels\n",
                      switch_channel_get_name(session->channel),
                      a_engine->cur_payload_map->iananame, a_engine->cur_payload_map->rm_rate,
                      a_engine->cur_payload_map->codec_ms, a_engine->read_impl.samples_per_packet,
                      a_engine->read_impl.bits_per_second, a_engine->read_impl.number_of_channels);
    ...
    a_engine->cur_payload_map->fmtp_out =
        switch_core_session_strdup(session, a_engine->write_codec.fmtp_out);
...
//?
SWITCH_DECLARE(void)
switch_core_media_clear_ice(switch_core_session_t* session) {
    clear_ice(session, SWITCH_MEDIA_TYPE_AUDIO);
    clear_ice(session, SWITCH_MEDIA_TYPE_VIDEO);
}

SWITCH_DECLARE(void)
switch_core_media_pause(switch_core_session_t* session) {
    switch_rtp_engine_t *a_engine, *v_engine, *t_engine;
    switch_media_handle_t* smh;
...
SWITCH_DECLARE(void)
switch_core_media_resume(switch_core_session_t* session) {
    switch_rtp_engine_t *a_engine, *v_engine, *t_engine;
    switch_media_handle_t* smh;
...
//?
SWITCH_DECLARE(switch_status_t)
switch_core_media_add_ice_acl(switch_core_session_t* session, switch_media_type_t type,
                              const char* acl_name) {
    switch_media_handle_t* smh;
    switch_rtp_engine_t* engine;
...
switch_core_media_check_video_codecs(switch_core_session_t* session) {
    ...
    if (smh->mparams->num_codecs &&
        !switch_channel_test_flag(session->channel, CF_VIDEO_POSSIBLE)) {
        int i;
        smh->video_count = 0;
        for (i = 0; i < smh->mparams->num_codecs; i++) {
...
#ifdef _MSC_VER
/* remove this if the break is removed from the following for loop which causes unreachable code
 * loop */
...
//?
static switch_call_direction_t switch_ice_direction(switch_rtp_engine_t* engine,
                                                    switch_core_session_t* session) {
    ...
    if (switch_rtp_has_dtls() && dtls_ok(smh->session)) {
        r = engine->dtls_controller ? SWITCH_CALL_DIRECTION_INBOUND
...
static switch_core_media_ice_type_t switch_determine_ice_type(switch_rtp_engine_t* engine,
                                                              switch_core_session_t* session) {
...
//?
static switch_status_t check_ice(switch_media_handle_t* smh, switch_media_type_t type,
                                 sdp_session_t* sdp, sdp_media_t* m) {
    switch_rtp_engine_t* engine = &smh->engines[type];
    ...
    if (switch_true(switch_channel_get_variable_dup(smh->session->channel, "ignore_sdp_ice",
                                                    SWITCH_FALSE, -1))) {
        ...
        for (attr = attrs[attr_idx]; attr; attr = attr->a_next) {
            ...
            if (!strcasecmp(attr->a_name, "ice-ufrag")) {
                ...
                if (!strcasecmp(attr->a_value, "passive") ||
                    (!strcasecmp(attr->a_value, "actpass") &&
            ...
            } else if (switch_rtp_has_dtls() && dtls_ok(smh->session) &&
                ...
                if ((p = strchr(engine->remote_dtls_fingerprint.type, ' '))) {
                    *p++ = '\0';
                    ...
                    if (switch_channel_test_flag(smh->session->channel, CF_REINVITE) &&
                        !switch_channel_test_flag(smh->session->channel, CF_RECOVERING) &&
                ...
                //	switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(smh->session),
                // SWITCH_LOG_WARNING, "Unsupported
                ...
                for (i = 0; i < argc; i++) {
                    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(smh->session), SWITCH_LOG_DEBUG1,
                                      "CAND %d [%s]\n", i, fields[i]);
                }

                if (fields[4] && (switch_inet_pton(AF_INET, fields[4], &ip) ||
                                  switch_inet_pton(AF_INET6, fields[4], &ip))) {
                    switch_copy_string(con_addr, fields[4], sizeof(con_addr));
                    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(smh->session), SWITCH_LOG_DEBUG1,
                                      "Is an IP address: %s\n", con_addr);
                } else if (fields[4] && ice_resolve) {
                    if (switch_resolve_host(fields[4], con_addr, sizeof(con_addr)) ==
                        SWITCH_STATUS_SUCCESS) {
                        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(smh->session), SWITCH_LOG_INFO,
                                          "Resolved %s to %s\n", fields[4], con_addr);
                    } else {
                        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(smh->session), SWITCH_LOG_INFO,
                                          "Drop %s Candidate cid: %d proto: %s type: %s addr: "
                                          "%s:%s (cannot resolve)\n",
                    ...
                    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(smh->session), SWITCH_LOG_INFO,
                                      "Drop %s Candidate cid: %d proto: %s type: %s addr: %s:%s "
                                      "(not an IP address)\n",
                                      type == SWITCH_MEDIA_TYPE_VIDEO ? "video" : "audio", cid + 1,
                                      fields[2], fields[7], fields[4] ? fields[4] : "(null)",
                                      fields[5]);
                    continue;
                }
                ...
                if (!ip_possible(smh, con_addr)) {
                    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(smh->session), SWITCH_LOG_DEBUG,
                                      ...
                                      "(no network path)\n",
                                      type == SWITCH_MEDIA_TYPE_VIDEO ? "video" : "audio", cid + 1,
                                      fields[2], fields[7] ? fields[7] : "N/A", con_addr,
                                      fields[5]);
                    continue;
                } else {
                    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(smh->session), SWITCH_LOG_DEBUG,
                ...
                engine->ice_in.cands[engine->ice_in.cand_idx[cid]][cid].foundation =
                    switch_core_session_strdup(smh->session, fields[0]);
                engine->ice_in.cands[engine->ice_in.cand_idx[cid]][cid].component_id =
                    atoi(fields[1]);
                engine->ice_in.cands[engine->ice_in.cand_idx[cid]][cid].transport =
                    switch_core_session_strdup(smh->session, fields[2]);
                ...
                engine->ice_in.cands[engine->ice_in.cand_idx[cid]][cid].con_addr =
                    switch_core_session_strdup(smh->session, con_addr);
                engine->ice_in.cands[engine->ice_in.cand_idx[cid]][cid].con_port =
                    (switch_port_t)atoi(fields[5]);

                j = 6;

                while (j < argc && j <= sizeof(fields) / sizeof(char*) && fields[j + 1] &&
                       engine->ice_in.cand_idx[cid] < MAX_CAND - 1) {
                    if (!strcasecmp(fields[j], "typ")) {
                        engine->ice_in.cands[engine->ice_in.cand_idx[cid]][cid].cand_type =
                            switch_core_session_strdup(smh->session, fields[j + 1]);
                    } else if (!strcasecmp(fields[j], "raddr")) {
                        engine->ice_in.cands[engine->ice_in.cand_idx[cid]][cid].raddr =
                            switch_core_session_strdup(smh->session, fields[j + 1]);
                    } else if (!strcasecmp(fields[j], "rport")) {
                        engine->ice_in.cands[engine->ice_in.cand_idx[cid]][cid].rport =
                            (switch_port_t)atoi(fields[j + 1]);
                    } else if (!strcasecmp(fields[j], "generation")) {
                        engine->ice_in.cands[engine->ice_in.cand_idx[cid]][cid].generation =
                            switch_core_session_strdup(smh->session, fields[j + 1]);
                    }
...
relay:
    ...
    for (cid = 0; cid < MAX_CAND_IDX_COUNT; cid++) {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(smh->session), SWITCH_LOG_DEBUG,
                          "Searching for %s candidate.\n", cid ? "rtcp" : "rtp");

        for (ai = 0; ai < engine->cand_acl_count; ai++) {
            for (i = 0; i < engine->ice_in.cand_idx[cid]; i++) {
                int is_relay = engine->ice_in.cands[i][cid].cand_type &&
                               !strcmp(engine->ice_in.cands[i][cid].cand_type, "relay");

                if (relay_ok != is_relay) continue;

                if (switch_check_network_list_ip(engine->ice_in.cands[i][cid].con_addr,
                                                 engine->cand_acl[ai])) {
                    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(smh->session), SWITCH_LOG_DEBUG,
                    ...
                    if (cid == 0 && got_rtcp_mux && engine->ice_in.cand_idx[1] < MAX_CAND) {
                        switch_log_printf(
                            SWITCH_CHANNEL_SESSION_LOG(smh->session), SWITCH_LOG_DEBUG,
                            "Choose same candidate, index %d, for rtcp based on rtcp-mux attribute "
                            "%s:%d\n",
                            engine->ice_in.cand_idx[1], engine->ice_in.cands[i][cid].con_addr,
                            engine->ice_in.cands[i][cid].con_port);

                        engine->ice_in.cands[engine->ice_in.cand_idx[1]][1] =
                            engine->ice_in.cands[i][0];
...
done_choosing:
    ...
    if (!engine->ice_in.is_chosen[0]) {
        ...
        /* PUNT */
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(smh->session), SWITCH_LOG_DEBUG,
                          "%s no suitable candidates found.\n",
                          switch_channel_get_name(smh->session->channel));
        return SWITCH_STATUS_FALSE;
    }
    ...
    if (engine->ice_in.cands[engine->ice_in.chosen[0]][0].con_addr &&
        engine->ice_in.cands[engine->ice_in.chosen[0]][0].con_port) {
        char tmp[80] = "";
        const char *media_varname = NULL, *port_varname = NULL;

        engine->cur_payload_map->remote_sdp_ip = switch_core_session_strdup(
            smh->session, (char*)engine->ice_in.cands[engine->ice_in.chosen[0]][0].con_addr);
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(smh->session), SWITCH_LOG_DEBUG,
        ...
        engine->remote_rtp_ice_port =
            (switch_port_t)engine->ice_in.cands[engine->ice_in.chosen[0]][0].con_port;
        engine->remote_rtp_ice_addr = switch_core_session_strdup(
            smh->session, engine->ice_in.cands[engine->ice_in.chosen[0]][0].con_addr);

        engine->cur_payload_map->remote_sdp_ip = switch_core_session_strdup(
            smh->session, (char*)engine->ice_in.cands[engine->ice_in.chosen[0]][0].con_addr);
        engine->cur_payload_map->remote_sdp_port =
            (switch_port_t)engine->ice_in.cands[engine->ice_in.chosen[0]][0].con_port;
        ...
        switch_snprintf(tmp, sizeof(tmp), "%d",
                        engine->ice_in.cands[engine->ice_in.chosen[0]][0].con_port);
        switch_channel_set_variable(smh->session->channel, media_varname,
                                    engine->ice_in.cands[engine->ice_in.chosen[0]][0].con_addr);
    ...
    if (engine->ice_in.cands[engine->ice_in.chosen[1]][1].con_port) {
        ...
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(smh->session), SWITCH_LOG_DEBUG,
                          ...
                          engine->ice_in.cands[engine->ice_in.chosen[1]][1].con_port);
        engine->remote_rtcp_ice_port = engine->ice_in.cands[engine->ice_in.chosen[1]][1].con_port;
        engine->remote_rtcp_ice_addr = switch_core_session_strdup(
            smh->session, engine->ice_in.cands[engine->ice_in.chosen[1]][1].con_addr);
    ...
    if (engine->new_ice) {
        if (switch_rtp_ready(engine->rtp_session) &&
            ...
            switch_rtp_activate_ice(engine->rtp_session, engine->ice_in.ufrag,
                                    engine->ice_out.ufrag, engine->ice_out.pwd, engine->ice_in.pwd,
                                    IPR_RTP,
#ifdef GOOGLE_ICE
                                    ICE_GOOGLE_JINGLE, NULL
#else
                                    switch_determine_ice_type(engine, smh->session), &engine->ice_in
#endif
            );
        ...
        if (engine->rtp_session &&
            ((val = switch_channel_get_variable(smh->session->channel,
             ...
             (val = type == SWITCH_MEDIA_TYPE_VIDEO ? smh->mparams->rtcp_video_interval_msec
            ...
            if (remote_rtcp_port) {
                if (!strcasecmp(val, "passthru")) {
                    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(smh->session), SWITCH_LOG_INFO,
                                      "Activating %s RTCP PASSTHRU PORT %d\n", type2str(type),
                                      remote_rtcp_port);
                    switch_rtp_activate_rtcp(engine->rtp_session, -1, remote_rtcp_port,
                                             engine->rtcp_mux > 0);
                } else {
                    int interval = atoi(val);
                    if (interval < 100 || interval > 500000) {
                        switch_log_printf(
                            SWITCH_CHANNEL_SESSION_LOG(smh->session), SWITCH_LOG_ERROR,
                    ...
                    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(smh->session), SWITCH_LOG_INFO,
                                      "Activating %s RTCP PORT %d\n", type2str(type),
                                      remote_rtcp_port);
                    switch_rtp_activate_rtcp(engine->rtp_session, interval, remote_rtcp_port,
                                             engine->rtcp_mux > 0);
                }
            }
        }
        ...
        if (engine->rtp_session && engine->ice_in.cands[engine->ice_in.chosen[1]][1].ready) {
            if (engine->rtcp_mux > 0 &&
                !strcmp(engine->ice_in.cands[engine->ice_in.chosen[1]][1].con_addr,
                        engine->ice_in.cands[engine->ice_in.chosen[0]][0].con_addr) &&
                engine->ice_in.cands[engine->ice_in.chosen[1]][1].con_port ==
                    engine->ice_in.cands[engine->ice_in.chosen[0]][0].con_port) {
                switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(smh->session), SWITCH_LOG_INFO,
                ...
                switch_rtp_activate_ice(engine->rtp_session, engine->ice_in.ufrag,
                                        engine->ice_out.ufrag, engine->ice_out.pwd,
                                        engine->ice_in.pwd, IPR_RTCP,
#ifdef GOOGLE_ICE
                                        ICE_GOOGLE_JINGLE, NULL
#else
...
SWITCH_DECLARE(void)
switch_core_session_set_ice(switch_core_session_t* session) {
    switch_media_handle_t* smh;
...
static void greedy_sort(switch_media_handle_t* smh, struct matches* matches, int m_idx,
                        const switch_codec_implementation_t** codec_array, int total_codecs) {
    int j = 0, f = 0, g;
...
static void media_flow_get_mode(switch_media_flow_t smode, const char** mode_str,
                                switch_media_flow_t* opp_mode) {
...
static void check_stream_changes(switch_core_session_t* session, const char* r_sdp,
                                 switch_sdp_type_t sdp_type) {
    ...
    if (switch_channel_test_flag(session->channel, CF_STREAM_CHANGED)) {
        ...
        if (other_session) {
            ...
            if (sdp_type == SDP_OFFER && r_sdp) {
                const char* filter_codec_string =
                    switch_channel_get_variable(session->channel, "filter_codec_string");

                switch_channel_set_variable(session->channel, "codec_string", NULL);
                switch_core_media_merge_sdp_codec_string(session, r_sdp, sdp_type,
                                                         filter_codec_string);
            }
            ...
            msg->string_arg = switch_core_session_sprintf(
                other_session, "=%s",
                switch_channel_get_variable(session->channel, "ep_codec_string"));
            msg->from = __FILE__;
    ...
    if (other_session) {
        if (sdp_type == SDP_ANSWER &&
            switch_channel_test_flag(session->channel, CF_PROCESSING_STREAM_CHANGE)) {
...
SWITCH_DECLARE(void)
switch_core_media_set_smode(switch_core_session_t* session, switch_media_type_t type,
                            switch_media_flow_t smode, switch_sdp_type_t sdp_type) {
    switch_media_handle_t* smh;
    switch_rtp_engine_t* engine;
...
static void switch_core_media_set_rmode(switch_core_session_t* session, switch_media_type_t type,
                                        switch_media_flow_t rmode, switch_sdp_type_t sdp_type) {
    switch_media_handle_t* smh;
    switch_rtp_engine_t* engine;
    ...
    if (switch_core_session_get_partner(session, &other_session) == SWITCH_STATUS_SUCCESS) {
        if (sdp_type == SDP_ANSWER &&
            (switch_channel_test_flag(other_session->channel, CF_REINVITE) ||
...
//?
// sdp 协商
SWITCH_DECLARE(uint8_t)
switch_core_media_negotiate_sdp(switch_core_session_t* session, const char* r_sdp, uint8_t* proceed,
                                switch_sdp_type_t sdp_type) {
    ...
    int got_crypto = 0, got_video_crypto = 0, got_audio = 0, saw_audio = 0, saw_video = 0,
        got_avp = 0, got_savp = 0, got_udptl = 0, got_webrtc = 0, got_text = 0, got_text_crypto = 0,
        got_msrp = 0;
    int scrooge = 0;
    sdp_parser_t* parser = NULL;
    sdp_session_t* sdp;
    ...
    if (!r_sdp) {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING,
                          "Tried to negotiate a blank SDP?\n");
        return 0;
    }
    ...
    if (dtls_ok(session) &&
        (tmp = switch_channel_get_variable(smh->session->channel, "webrtc_enable_dtls")) &&
        switch_false(tmp)) {
        switch_channel_clear_flag(smh->session->channel, CF_DTLS_OK);
        switch_channel_clear_flag(smh->session->channel, CF_DTLS);
    }

    if (switch_true(switch_channel_get_variable_dup(session->channel, "rtp_assume_rtcp",
                                                    SWITCH_FALSE, -1))) {
        rtcp_auto_video = 1;
        rtcp_auto_audio = 1;
    }
    ...
    if ((val = switch_channel_get_variable(channel, "rtp_codec_negotiation"))) {
        if (!strcasecmp(val, "generous")) {
            ...
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                              "rtp_codec_negotiation ignored invalid value : '%s' \n", val);
        }
    }
    ...
    if ((val = switch_channel_get_variable(session->channel, "rtp_liberal_dtmf")) &&
        switch_true(val)) {
        switch_channel_set_flag(session->channel, CF_LIBERAL_DTMF);
    }

    if (switch_stristr("T38FaxFillBitRemoval:", r_sdp) ||
        switch_stristr("T38FaxTranscodingMMR:", r_sdp) ||
    ...
    if ((sdp->sdp_connection && sdp->sdp_connection->c_address &&
        ...
        switch_log_printf(
            SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING,
            "RFC2543 from March 1999 called; They want their 0.0.0.0 hold method back.....\n");
    ...
    switch_core_media_set_rmode(smh->session, SWITCH_MEDIA_TYPE_VIDEO, SWITCH_MEDIA_FLOW_INACTIVE,
                                sdp_type);
    switch_core_media_set_rmode(smh->session, SWITCH_MEDIA_TYPE_TEXT, SWITCH_MEDIA_FLOW_INACTIVE,
                                sdp_type);
    ...
    for (m = sdp->sdp_media; m; m = m->m_next) {
        ...
        if (got_msrp && m->m_type == sdp_media_message) {
            if (!smh->msrp_session) {
                smh->msrp_session = switch_msrp_session_new(switch_core_session_get_pool(session),
                                                            switch_core_session_get_uuid(session),
                                                            m->m_proto == sdp_proto_msrps);
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE,
                                  "MSRP session created %s\n", smh->msrp_session->call_id);
            }
            ...
            for (attr = m->m_attributes; attr; attr = attr->a_next) {
                ...
                if (!strcasecmp(attr->a_name, "path") && attr->a_value) {
                    smh->msrp_session->remote_path =
                        switch_core_session_strdup(session, attr->a_value);
                    switch_channel_set_variable(session->channel, "sip_msrp_remote_path",
                                                attr->a_value);
                } else if (!strcasecmp(attr->a_name, "accept-types") && attr->a_value) {
                    smh->msrp_session->remote_accept_types =
                        switch_core_session_strdup(session, attr->a_value);
                    switch_channel_set_variable(session->channel, "sip_msrp_remote_accept_types",
                                                attr->a_value);
                } else if (!strcasecmp(attr->a_name, "accept-wrapped-types") && attr->a_value) {
                    smh->msrp_session->remote_accept_wrapped_types =
                        switch_core_session_strdup(session, attr->a_value);
                    switch_channel_set_variable(
                        session->channel, "sip_msrp_remote_accept_wrapped_types", attr->a_value);
                } else if (!strcasecmp(attr->a_name, "setup") && attr->a_value) {
                    smh->msrp_session->remote_setup =
                        switch_core_session_strdup(session, attr->a_value);
                    switch_channel_set_variable(session->channel, "sip_msrp_remote_setup",
                                                attr->a_value);
                    ...
                    smh->msrp_session->remote_file_selector =
                        switch_core_session_strdup(session, attr->a_value);
                    switch_channel_set_variable(session->channel, "sip_msrp_remote_file_selector",
                                                attr->a_value);
                    ...
                    for (i = 0; i < argc; i++) {
                        ...
                        if (!strncasecmp(argv[i], "name:", 5)) {
                            ...
                            switch_channel_set_variable(session->channel, "sip_msrp_file_type",
                                                        argv[i] + 5);
                        }
                        if (!strncasecmp(argv[i], "size:", 5)) {
                            switch_channel_set_variable(session->channel, "sip_msrp_file_size",
                                                        argv[i] + 5);
                        }
                        if (!strncasecmp(argv[i], "hash:", 5)) {
                            switch_channel_set_variable(session->channel, "sip_msrp_file_hash",
                                                        argv[i] + 5);
                        }
                    }
                    switch_safe_free(tmp);
                } else if (!strcasecmp(attr->a_name, "file-transfer-id") && attr->a_value) {
                    switch_channel_set_variable(session->channel, "sip_msrp_file_transfer_id",
                                                attr->a_value);
                } else if (!strcasecmp(attr->a_name, "file-disposition") && attr->a_value) {
                    switch_channel_set_variable(session->channel, "sip_msrp_file_disposition",
                                                attr->a_value);
                } else if (!strcasecmp(attr->a_name, "file-date") && attr->a_value) {
                    switch_channel_set_variable(session->channel, "sip_msrp_file_date",
                                                attr->a_value);
                } else if (!strcasecmp(attr->a_name, "file-icon") && attr->a_value) {
                    switch_channel_set_variable(session->channel, "sip_msrp_file_icon",
                                                attr->a_value);
                } else if (!strcasecmp(attr->a_name, "file-range") && attr->a_value) {
                    switch_channel_set_variable(session->channel, "sip_msrp_file_range",
                                                attr->a_value);
                }
            }
            ...
            if (smh->msrp_session->active) {
                ...
                smh->msrp_session->local_path = switch_core_session_sprintf(
                    session, "msrp%s://%s:%d/%s;tcp", smh->msrp_session->secure ? "s" : "", ip,
                    smh->msrp_session->local_port, smh->msrp_session->call_id);
        ...
        if (got_udptl && m->m_type == sdp_media_image) {
            ...
            if (m->m_port) {
                if (switch_channel_test_app_flag_key("T38", session->channel,
                ...
                if (switch_channel_var_true(channel, "refuse_t38") ||
                    ...
                    switch_t38_options_t* t38_options =
                        switch_core_media_process_udptl(session, sdp, m);
                    ...
                    if ((pass == 2 &&
                         switch_channel_test_flag(smh->session->channel, CF_T38_PASSTHRU)) ||
                    ...
                    if (pass && switch_core_session_get_partner(session, &other_session) ==
                                    SWITCH_STATUS_SUCCESS) {
                        ...
                        switch_port_t remote_port =
                            switch_rtp_get_remote_port(a_engine->rtp_session);
                        char tmp[32] = "";

                        if (!switch_channel_test_flag(other_channel, CF_ANSWERED)) {
                            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session),
                                              SWITCH_LOG_WARNING,
                                              "%s Error Passing T.38 to unanswered channel %s\n",
                                              switch_channel_get_name(session->channel),
                        ...
                        if (switch_true(switch_channel_get_variable(session->channel,
                            ...
                            switch_channel_set_variable(other_channel, "t38_broken_boolean",
                                                        "true");
                        }
                        ...
                        if (remote_host && remote_port &&
                            ...
                            switch_snprintf(tmp, sizeof(tmp), "%d",
                                            a_engine->cur_payload_map->remote_sdp_port);
                            switch_channel_set_variable(session->channel,
                                                        SWITCH_REMOTE_MEDIA_IP_VARIABLE,
                                                        a_engine->cur_payload_map->remote_sdp_ip);
                            switch_channel_set_variable(session->channel,
                                                        SWITCH_REMOTE_MEDIA_PORT_VARIABLE, tmp);

                            if (switch_rtp_set_remote_address(
                                    a_engine->rtp_session, a_engine->cur_payload_map->remote_sdp_ip,
                                    a_engine->cur_payload_map->remote_sdp_port, 0, SWITCH_TRUE,
                ...
                if (switch_channel_wait_for_app_flag(channel, CF_APP_T38_POSSIBLE, "T38",
                                                     SWITCH_TRUE, 2000)) {
                    fmatch = 1;
                } else {
                    fmatch = 0;
            ...
            if (!sendonly && m->m_connections && m->m_connections->c_address &&
                !strcmp(m->m_connections->c_address, "0.0.0.0")) {
                sendonly = 1;
            }

            switch_core_media_set_rmode(smh->session, SWITCH_MEDIA_TYPE_AUDIO,
                                        sdp_media_flow(m->m_mode), sdp_type);
            ...
            if (sdp_type == SDP_OFFER) {
                switch (a_engine->rmode) {
                    ...
                    case SWITCH_MEDIA_FLOW_SENDONLY:
                        switch_core_media_set_smode(smh->session, SWITCH_MEDIA_TYPE_AUDIO,
                                                    SWITCH_MEDIA_FLOW_RECVONLY, sdp_type);
                        break;
                    case SWITCH_MEDIA_FLOW_INACTIVE:
                        switch_core_media_set_smode(smh->session, SWITCH_MEDIA_TYPE_AUDIO,
                                                    SWITCH_MEDIA_FLOW_INACTIVE, sdp_type);
                        break;
                    default:
                        switch_core_media_set_smode(smh->session, SWITCH_MEDIA_TYPE_AUDIO,
                                                    SWITCH_MEDIA_FLOW_SENDRECV, sdp_type);
                        break;
                }
            }
            ...
            for (attr = sdp->sdp_attributes; attr; attr = attr->a_next) {
                ...
                if (!strncasecmp(attr->a_name, "ice", 3)) {
                    ...
                    if (switch_rtp_ready(a_engine->rtp_session)) {
                        ...
                        switch_rtp_set_media_timeout(a_engine->rtp_session,
                                                     a_engine->media_timeout);
                    } else {
                        switch_channel_set_variable(session->channel, "media_timeout_audio", "0");
                        switch_channel_set_variable(session->channel, "media_hold_timeout_audio",
                                                    "0");
            ...
            if (sdp_type == SDP_ANSWER) {
                if (inactive) {
                    // When freeswitch had previously sent inactive in sip request. it should remain
                    // inactive otherwise smode should be sendrecv
                    if (a_engine->smode == SWITCH_MEDIA_FLOW_INACTIVE) {
                        switch_core_media_set_smode(smh->session, SWITCH_MEDIA_TYPE_AUDIO,
                                                    sdp_media_flow(sdp_inactive), sdp_type);
                    } else {
                        switch_core_media_set_smode(smh->session, SWITCH_MEDIA_TYPE_AUDIO,
                                                    sdp_media_flow(sdp_sendrecv), sdp_type);
                    }
                } else if (sendonly) {
                    switch_core_media_set_smode(smh->session, SWITCH_MEDIA_TYPE_AUDIO,
                                                sdp_media_flow(sdp_sendonly), sdp_type);
                } else if (recvonly) {
                    switch_core_media_set_smode(smh->session, SWITCH_MEDIA_TYPE_AUDIO,
                                                sdp_media_flow(sdp_recvonly), sdp_type);
                }
            }
            ...
            for (attr = m->m_attributes; attr; attr = attr->a_next) {
                if (!strcasecmp(attr->a_name, "rtcp") && attr->a_value && !skip_rtcp) {
                    ...
                    switch_channel_set_variable_printf(session->channel,
                                                       "rtp_remote_audio_rtcp_port", "%d",
                                                       a_engine->remote_rtcp_port);
                ...
                } else if (got_crypto < 1 && !strcasecmp(attr->a_name, "crypto") &&
                           !zstr(attr->a_value)) {
                    int crypto_tag;
                    ...
                    got_crypto = switch_core_session_check_incoming_crypto(
                        session, "rtp_has_crypto", SWITCH_MEDIA_TYPE_AUDIO, crypto, crypto_tag,
                        sdp_type);
                }
            }
            ...
            if (got_crypto == -1 && got_savp && !got_avp && !got_webrtc) {
                switch_log_printf(
                    SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING,
                    "Received invite with SAVP but secure media is administratively disabled\n");
                match = 0;
                continue;
            }
            ...
            for (map = m->m_rtpmaps; map; map = map->rm_next) {
                ...
                if (!switch_media_handle_test_media_flag(smh, SCMF_SUPPRESS_CNG) && !cng_pt &&
                    !strcasecmp(rm_encoding, "CN")) {
                ...
                if (switch_channel_get_variable(session->channel, "rtp_h_X-Broken-PTIME") &&
                    a_engine->read_impl.microseconds_per_packet) {
                    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                                      "Overwriting ptime from a known broken endpoint with the "
                                      "currently used value of %d ms\n",
                                      a_engine->read_impl.microseconds_per_packet / 1000);
                ...
                if (zstr(map->rm_fmtp)) {
                    ...
                    if ((switch_core_codec_parse_fmtp(map->rm_encoding, map->rm_fmtp, map->rm_rate,
                                                      &codec_fmtp)) == SWITCH_STATUS_SUCCESS) {
                ...
                for (i = 0; i < smh->mparams->num_codecs && i < total_codecs; i++) {
                    ...
                    switch_log_printf(
                        SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                        "Audio Codec Compare [%s:%d:%u:%d:%u:%d]/[%s:%d:%u:%d:%u:%d]\n",
                        rm_encoding, map->rm_pt, (int)remote_codec_rate, codec_ms, map_bit_rate,
                        map_channels, imp->iananame, imp->ianacode, codec_rate,
                        imp->microseconds_per_packet / 1000, bit_rate, imp->number_of_channels);
                    if ((zstr(map->rm_encoding) ||
                         (smh->mparams->ndlb & SM_NDLB_ALLOW_BAD_IANANAME)) &&
                        map->rm_pt < 96) {
                        match = (map->rm_pt == imp->ianacode) ? 1 : 0;
                    } else {
                        match = (!strcasecmp(rm_encoding, imp->iananame) &&
                                 ((map->rm_pt < 96 && imp->ianacode < 96) ||
                                  (map->rm_pt > 95 && imp->ianacode > 95)) &&
                                 (remote_codec_rate == codec_rate ||
                                  fmtp_remote_codec_rate == imp->actual_samples_per_second))
                                    ? 1
                                    : 0;
                    ...
                    if (match && bit_rate && map_bit_rate && map_bit_rate != bit_rate &&
                        strcasecmp(map->rm_encoding, "ilbc") &&
                        strcasecmp(map->rm_encoding, "isac")) {
                        /* if a bit rate is specified and doesn't match, this is not a codec match,
                         * except for ILBC */
                        match = 0;
                    }
                    ...
                    if (match && remote_codec_rate && codec_rate &&
                        remote_codec_rate != codec_rate &&
                        (!strcasecmp(map->rm_encoding, "pcma") ||
                         !strcasecmp(map->rm_encoding, "pcmu"))) {
                        /* if the sampling rate is specified and doesn't match, this is not a codec
                         * match for G.711 */
                        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                                          "sampling rates have to match for G.711\n");
                        match = 0;
                    }
                    ...
                    if (match) {
                        if (scrooge) {
                            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                                              ...
                                              imp->iananame, imp->samples_per_second,
                                              imp->microseconds_per_packet / 1000);
                        } else if ((ptime && codec_ms &&
                                    codec_ms * 1000 != imp->microseconds_per_packet) ||
                                   remote_codec_rate != codec_rate) {
                            ...
                            if (nm_idx >= MAX_MATCHES) {
                                switch_log_printf(
                                    SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                                    "Audio Codec Compare [%s:%d:%u:%u:%d:%u:%d] was not saved as a "
                                    "near-match. Too many. Ignoring.\n",
                                    imp->iananame, imp->ianacode, codec_rate,
                                    imp->actual_samples_per_second,
                                    imp->microseconds_per_packet / 1000, bit_rate,
                                    imp->number_of_channels);
                                continue;
                            }

                            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                                              "Audio Codec Compare [%s:%d:%u:%d:%u:%d] is saved as "
                                              "a near-match\n",
                                              imp->iananame, imp->ianacode, codec_rate,
                                              imp->microseconds_per_packet / 1000, bit_rate,
                                              imp->number_of_channels);
            ...
            if (!m_idx && nm_idx) {
                ...
                for (j = 0; j < nm_idx; j++) {
                    ...
                    switch_snprintf(tmp, sizeof(tmp), "%s@%uh@%ui%dc", near_match->iananame,
                                    near_rate ? near_rate : near_match->samples_per_second,
                    ...
                    if (!maxptime || timp->microseconds_per_packet / 1000 <= maxptime) {
                        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                                          "Substituting codec %s@%ui@%uh@%dc\n", timp->iananame,
                                          timp->microseconds_per_packet / 1000,
                                          timp->actual_samples_per_second,
                                          timp->number_of_channels);
                        match = 1;
            ...
            if (m_idx) {
                ...
                for (j = 0; j < m_idx && smh->num_negotiated_codecs < SWITCH_MAX_CODECS; j++) {
                    payload_map_t* pmap = switch_core_media_add_payload_map(
                        session, SWITCH_MEDIA_TYPE_AUDIO, matches[j].map->rm_encoding,
                        matches[j].imp->modname, matches[j].map->rm_fmtp, sdp_type,
                        matches[j].map->rm_pt, matches[j].imp->samples_per_second,
                        matches[j].imp->microseconds_per_packet / 1000,
                        matches[j].imp->number_of_channels, SWITCH_TRUE);
                    ...
                    pmap->rm_encoding =
                        switch_core_session_strdup(session, (char*)mmap->rm_encoding);
                    ...
                    if (!strcasecmp((char*)mmap->rm_encoding, "opus")) {
                        if (pmap->channels == 1) {
                            switch_log_printf(
                                SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING,
                                "Invalid SDP for opus.  Don't ask.. but it needs a /2\n");
                            pmap->adv_channels = 1;
                        } else {
                            pmap->adv_channels = 2; /* IKR ???*/
                        }
                        if (!zstr((char*)mmap->rm_fmtp) &&
                            switch_stristr("stereo=1", (char*)mmap->rm_fmtp)) {
                            ...
                            if (!allow_channels || allow_channels >= 2) { /*default*/
                                ...
                                switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session),
                                                  SWITCH_LOG_DEBUG,
                                                  "Opus: setting 1 audio channel via config.\n");
                                pmap->channels = 1;
                            }
                        } else {
                            pmap->channels = 1;
            ...
            if (match) {
                char tmp[50];
                // const char *mirror = switch_channel_get_variable(session->channel,
                // "rtp_mirror_remote_audio_codec_payload");

                switch_snprintf(tmp, sizeof(tmp), "%d", a_engine->cur_payload_map->remote_sdp_port);
                switch_channel_set_variable(session->channel, SWITCH_REMOTE_MEDIA_IP_VARIABLE,
                                            a_engine->cur_payload_map->remote_sdp_ip);
                switch_channel_set_variable(session->channel, SWITCH_REMOTE_MEDIA_PORT_VARIABLE,
                                            tmp);
                ...
                if (a_engine->cur_payload_map->pt == smh->mparams->te) {
                    ...
                    switch_mutex_lock(smh->sdp_mutex);
                    for (pmap = a_engine->cur_payload_map; pmap && pmap->allocated;
                         pmap = pmap->next) {
                        if (pmap->pt > pl) {
                            pl = pmap->pt;
                        }
                ...
                if (a_engine->read_impl.iananame) {
                    if (!switch_core_codec_ready(&a_engine->read_codec) ||
                        ((strcasecmp(matches[0].imp->iananame, a_engine->read_impl.iananame) ||
                          matches[0].imp->microseconds_per_packet !=
                              a_engine->read_impl.microseconds_per_packet ||
                          matches[0].imp->samples_per_second !=
                              a_engine->read_impl.samples_per_second))) {
                        a_engine->reset_codec = 1;
                    }
                } else if (switch_core_media_set_codec(session, 0, smh->mparams->codec_flags) !=
                           SWITCH_STATUS_SUCCESS) {
                    match = 0;
                }
            ...
            if (best_te) {
                ...
                if (a_engine->rtp_session) {
                    ...
                    switch_channel_set_variable_printf(session->channel, "rtp_2833_send_payload",
                                                       "%d", smh->mparams->te);
                    switch_rtp_set_telephony_recv_event(a_engine->rtp_session,
                                                        smh->mparams->recv_te);
                    switch_channel_set_variable_printf(session->channel, "rtp_2833_recv_payload",
                                                       "%d", smh->mparams->recv_te);
                }

                switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                                  "%s Set 2833 dtmf send payload to %u recv payload to %u\n",
            ...
            for (map = m->m_rtpmaps; map; map = map->rm_next) {
                ...
                pmap = switch_core_media_add_payload_map(session, SWITCH_MEDIA_TYPE_TEXT,
                                                         map->rm_encoding, NULL, NULL, SDP_OFFER,
                                                         map->rm_pt, 1000, 0, 1, SWITCH_TRUE);
            ...
            for (attr = m->m_attributes; attr; attr = attr->a_next) {
                if (!strcasecmp(attr->a_name, "rtcp") && attr->a_value) {
                    switch_channel_set_variable(session->channel, "sip_remote_text_rtcp_port",
                                                attr->a_value);
                } else if (!got_text_crypto && !strcasecmp(attr->a_name, "crypto") &&
                           !zstr(attr->a_value)) {
                    int crypto_tag;
                    ...
                    got_text_crypto = switch_core_session_check_incoming_crypto(
                        session, "rtp_has_text_crypto", SWITCH_MEDIA_TYPE_TEXT, crypto, crypto_tag,
                        sdp_type);
                }
            }
            ...
            switch_core_media_set_rmode(smh->session, SWITCH_MEDIA_TYPE_VIDEO,
                                        sdp_media_flow(m->m_mode), sdp_type);
            ...
            for (attr = m->m_attributes; attr; attr = attr->a_next) {
                if (!strcasecmp(attr->a_name, "framerate") && attr->a_value) {
                    ...
                    switch_channel_set_variable_printf(session->channel,
                                                       "rtp_remote_video_rtcp_port", "%d",
                                                       v_engine->remote_rtcp_port);
                    if (!smh->mparams->rtcp_video_interval_msec) {
                        smh->mparams->rtcp_video_interval_msec = SWITCH_RTCP_VIDEO_INTERVAL_MSEC;
                    }
                    got_video_rtcp = 1;
                } else if (!got_video_crypto && !strcasecmp(attr->a_name, "crypto") &&
                           !zstr(attr->a_value)) {
                    int crypto_tag;
                    ...
                    got_video_crypto = switch_core_session_check_incoming_crypto(
                        session, "rtp_has_video_crypto", SWITCH_MEDIA_TYPE_VIDEO, crypto,
                        crypto_tag, sdp_type);
                }
            }

            if (switch_true(switch_channel_get_variable_dup(session->channel, "inherit_codec",
                                                            SWITCH_FALSE, -1))) {
                vmatch_pt = 1;
            }
        ...
        compare:
            ...
            for (map = m->m_rtpmaps; map; map = map->rm_next) {
                ...
                for (i = 0; i < total_codecs; i++) {
                    ...
                    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                                      "Video Codec Compare [%s:%d]/[%s:%d]\n", rm_encoding,
                                      map->rm_pt, imp->iananame, imp->ianacode);
                    if ((zstr(map->rm_encoding) ||
                         (smh->mparams->ndlb & SM_NDLB_ALLOW_BAD_IANANAME)) &&
                        map->rm_pt < 96) {
                    ...
                    if (sdp_type == SDP_ANSWER && consider_video_fmtp && vmatch &&
                        !zstr(map->rm_fmtp) && !zstr(smh->fmtps[i])) {
                        almost_vmatch = 1;
                    ...
                    if (vmatch && vmatch_pt) {
                        const char* other_pt =
                            switch_channel_get_variable_partner(channel, "rtp_video_pt");
                        ...
                        if (other_pt) {
                            ...
                            if (map->rm_pt != opt) {
                                ...
                                if (switch_channel_var_true(channel, "inherit_video_fmtp")) {
                                    inherit_video_fmtp = switch_channel_get_variable_partner(
                                        channel, "rtp_video_fmtp");
                                }
                            }
                        }
                    }
                    ...
                    if (vmatch) {
                        ...
                        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                                          "Video Codec Compare [%s:%d] +++ is saved as a match\n",
            ...
            if (consider_video_fmtp && (!m_idx || almost_vmatch)) {
                switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                                  "No matches with FTMP, fallback to ignoring FMTP\n");
                almost_vmatch = 0;
                m_idx = 0;
            ...
            if (vmatch_pt && !m_idx) {
                switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                                  "No matches with inherit_codec, fallback to ignoring PT\n");
                vmatch_pt = 0;
                goto compare;
            }
            ...
            if (m_idx) {
                ...
                for (j = 0; j < m_idx && smh->num_negotiated_codecs < SWITCH_MAX_CODECS; j++) {
                    payload_map_t* pmap = switch_core_media_add_payload_map(
                        session, SWITCH_MEDIA_TYPE_VIDEO, matches[j].map->rm_encoding,
                        matches[j].imp->modname,
                        consider_video_fmtp ? matches[j].map->rm_fmtp : NULL, sdp_type,
                        matches[j].map->rm_pt, matches[j].imp->samples_per_second,
                        matches[j].imp->microseconds_per_packet / 1000,
                        matches[j].imp->number_of_channels, SWITCH_TRUE);
                    ...
                    pmap->rm_encoding =
                        switch_core_session_strdup(session, (char*)map->rm_encoding);
                    ...
                    pmap->remote_sdp_ip =
                        switch_core_session_strdup(session, (char*)connection->c_address);
                    pmap->remote_sdp_port = (switch_port_t)m->m_port;
                    pmap->rm_fmtp = switch_core_session_strdup(
                        session, (char*)inherit_video_fmtp ? inherit_video_fmtp : map->rm_fmtp);
                    smh->negotiated_codecs[smh->num_negotiated_codecs++] = mimp;
                ...
                switch_channel_set_variable(session->channel, SWITCH_REMOTE_VIDEO_IP_VARIABLE,
                                            v_engine->cur_payload_map->remote_sdp_ip);
                switch_channel_set_variable(session->channel, SWITCH_REMOTE_VIDEO_PORT_VARIABLE,
                                            tmp);
                switch_channel_set_variable(session->channel, "rtp_video_fmtp",
                                            v_engine->cur_payload_map->rm_fmtp);
                ...
                if (switch_core_codec_ready(&v_engine->read_codec) &&
                    strcasecmp(matches[0].imp->iananame,
                               v_engine->read_codec.implementation->iananame)) {
...
endsdp:
    ...
    if (rtcp_auto_audio || rtcp_auto_video) {
        if (rtcp_auto_audio && !skip_rtcp && !got_audio_rtcp && audio_port) {
            switch_channel_set_variable_printf(session->channel, "rtp_remote_audio_rtcp_port", "%d",
                                               audio_port + 1);
        ...
        if (rtcp_auto_video && !skip_video_rtcp && !got_video_rtcp && video_port) {
            switch_channel_set_variable_printf(session->channel, "rtp_remote_video_rtcp_port", "%d",
                                               video_port + 1);
    ...
    if (!saw_audio) {
        ...
        pmap = switch_core_media_add_payload_map(session, SWITCH_MEDIA_TYPE_AUDIO, "L16", NULL,
                                                 NULL, SDP_OFFER, 97, 8000, 20, 1, SWITCH_TRUE);
...
//?
SWITCH_DECLARE(void)
switch_core_media_reset_t38(switch_core_session_t* session) {
    switch_rtp_engine_t* a_engine;
    switch_media_handle_t* smh;
    switch_channel_t* channel = switch_core_session_get_channel(session);
...
SWITCH_DECLARE(int)
switch_core_media_toggle_hold(switch_core_session_t* session, int sendonly) {
    int changed = 0;
    ...
    if (sendonly && switch_channel_test_flag(session->channel, CF_ANSWERED)) {
        if (!switch_channel_test_flag(session->channel, CF_PROTO_HOLD)) {
            ...
            if ((switch_channel_test_flag(session->channel, CF_SLA_BARGE) ||
                 switch_channel_test_flag(session->channel, CF_SLA_BARGING)) &&
            ...
            if (a_engine->max_missed_hold_packets && a_engine->rtp_session) {
                switch_rtp_set_max_missed_packets(a_engine->rtp_session,
        ...
        if (switch_channel_test_flag(session->channel, CF_PROTO_HOLD)) {
            int media_on_hold_a = switch_true(switch_channel_get_variable_dup(
                session->channel, "bypass_media_resume_on_hold", SWITCH_FALSE, -1));
            int bypass_after_hold_a = 0;
            int bypass_after_hold_b = 0;

            if (media_on_hold_a) {
                bypass_after_hold_a = switch_true(switch_channel_get_variable_dup(
                    session->channel, "bypass_media_after_hold", SWITCH_FALSE, -1));
            }

            if (b_channel) {
                if (switch_true(switch_channel_get_variable_dup(
                        b_channel, "bypass_media_resume_on_hold", SWITCH_FALSE, -1))) {
                    bypass_after_hold_b = switch_true(switch_channel_get_variable_dup(
                        b_channel, "bypass_media_after_hold", SWITCH_FALSE, -1));
                }
            }
            ...
            if (b_channel &&
                (switch_channel_test_flag(session->channel, CF_BYPASS_MEDIA_AFTER_HOLD) ||
                 switch_channel_test_flag(b_channel, CF_BYPASS_MEDIA_AFTER_HOLD) ||
                 bypass_after_hold_a || bypass_after_hold_b)) {
                /* try to stay out from media stream */
                switch_ivr_bg_media(switch_core_session_get_uuid(session), SMF_REBRIDGE,
                                    SWITCH_FALSE, SWITCH_TRUE, 200);
            }
...
SWITCH_DECLARE(switch_file_handle_t*)
switch_core_media_get_video_file(switch_core_session_t* session, switch_rw_t rw) {
    switch_media_handle_t* smh;
    switch_rtp_engine_t* v_engine;
...
SWITCH_DECLARE(void)
switch_core_session_write_blank_video(switch_core_session_t* session, uint32_t ms) {
    switch_frame_t fr = {0};
    int i = 0;
...
static void* SWITCH_THREAD_FUNC video_write_thread(switch_thread_t* thread, void* obj) {
    ...
    switch_core_timer_init(&timer, "soft", fps_data.ms, fps_data.samples,
                           switch_core_session_get_pool(session));
    ...
    while (smh->video_write_thread_running > 0 && switch_channel_up_nosig(session->channel) &&
           smh->video_write_fh && switch_test_flag(smh->video_write_fh, SWITCH_FILE_OPEN)) {
        ...
        // smh->video_write_fh->mm.source_fps != fps) {
        ...
        if (smh->video_write_fh &&
            !switch_test_flag(smh->video_write_fh, SWITCH_FILE_FLAG_VIDEO_EOF)) {
            ...
            if (wstatus == SWITCH_STATUS_SUCCESS) {
                ...
                if (fr.img && smh->vid_params.d_width && smh->vid_params.d_height) {
                    switch_img_fit(&fr.img, smh->vid_params.d_width, smh->vid_params.d_height,
                                   SWITCH_FIT_SIZE);
                }
...
SWITCH_DECLARE(switch_status_t)
switch_core_media_lock_video_file(switch_core_session_t* session, switch_rw_t rw) {
    switch_media_handle_t* smh;
    switch_rtp_engine_t* v_engine;
...
SWITCH_DECLARE(switch_status_t)
switch_core_media_unlock_video_file(switch_core_session_t* session, switch_rw_t rw) {
    switch_media_handle_t* smh;
    switch_rtp_engine_t* v_engine;
...
SWITCH_DECLARE(switch_status_t)
switch_core_media_set_video_file(switch_core_session_t* session, switch_file_handle_t* fh,
                                 switch_rw_t rw) {
    switch_media_handle_t* smh;
    switch_rtp_engine_t* v_engine;
    ...
    if (!smh->video_read_fh && !smh->video_write_fh &&
        !switch_channel_test_flag(session->channel, CF_VIDEO)) {
        return SWITCH_STATUS_FALSE;
    }
    ...
    if (rw == SWITCH_RW_READ) {
        ...
        if (fh) {
            ...
            smh->video_write_thread_running = 1;
            switch_thread_create(&smh->video_write_thread, thd_attr, video_write_thread, session,
                                 switch_core_session_get_pool(session));
        }
...
SWITCH_DECLARE(void)
switch_core_autobind_cpu(void) {
    if (video_globals.cpu_count > 1) {
        switch_core_thread_set_cpu_affinity(next_cpu());
    }
}

static switch_status_t perform_write(switch_core_session_t* session, switch_frame_t* frame,
    ...
    if ((smh = session->media_handle)) {
        ...
        if (a_engine && a_engine->write_fb && !(flags & SWITCH_IO_FLAG_QUEUED)) {
            ...
            if (switch_frame_buffer_dup(a_engine->write_fb, frame, &dupframe) ==
                SWITCH_STATUS_SUCCESS) {
    ...
    if (session->bugs && !(frame->flags & SFF_NOT_AUDIO)) {
        ...
        for (bp = session->bugs; bp; bp = bp->next) {
            ...
            if (switch_core_media_bug_test_flag(bp, SMBF_PAUSE) ||
                (switch_channel_test_flag(session->channel, CF_PAUSE_BUGS) &&
                 !switch_core_media_bug_test_flag(bp, SMBF_NO_PAUSE))) {
                continue;
            }

            if (!switch_channel_test_flag(session->channel, CF_ANSWERED) &&
                switch_core_media_bug_test_flag(bp, SMBF_ANSWER_REQ)) {
                continue;
            }
    ...
    if (session->endpoint_interface->io_routines->write_frame) {
        if ((status = session->endpoint_interface->io_routines->write_frame(
                 session, frame, flags, stream_id)) == SWITCH_STATUS_SUCCESS) {
            for (ptr = session->event_hooks.write_frame; ptr; ptr = ptr->next) {
                if ((status = ptr->write_frame(session, frame, flags, stream_id)) !=
                    SWITCH_STATUS_SUCCESS) {
                    break;
                }
            }
...
SWITCH_DECLARE(switch_status_t)
switch_core_session_start_audio_write_thread(switch_core_session_t* session) {
    switch_threadattr_t* thd_attr = NULL;
    ...
    switch_thread_create(&a_engine->media_thread, thd_attr, audio_write_thread, &a_engine->mh,
                         switch_core_session_get_pool(session));
...
static void* SWITCH_THREAD_FUNC text_helper_thread(switch_thread_t* thread, void* obj) {
    ...
    while (switch_channel_up_nosig(channel)) {
        ...
        if (!switch_channel_test_flag(session->channel, CF_TEXT_PASSIVE)) {
            status =
                switch_core_session_read_text_frame(session, &read_frame, SWITCH_IO_FLAG_NONE, 0);
...
SWITCH_DECLARE(switch_status_t)
switch_core_session_start_text_thread(switch_core_session_t* session) {
    switch_threadattr_t* thd_attr = NULL;
    switch_memory_pool_t* pool = switch_core_session_get_pool(session);
    switch_rtp_engine_t* t_engine = NULL;
    switch_media_handle_t* smh;
...
static void* SWITCH_THREAD_FUNC video_helper_thread(switch_thread_t* thread, void* obj) {
    ...
    if (!blank_img) {
        switch_color_set_rgb(&bgcolor, "#2107e8ff");  // 显示的背景颜色还是画面未填充满两边的颜色
    ...
    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "%s Video thread started. Echo is %s\n",
                      switch_channel_get_name(session->channel),
                      switch_channel_test_flag(channel, CF_VIDEO_ECHO) ? "on" : "off");
    switch_core_session_request_video_refresh(session);
    ...
    fr.data = buf + 12;
    fr.buflen = buflen - 12;
    // 该函数用于生成关键帧（I帧）请求，主要应用于视频通话场景中强制编码器生成一个关键帧
    switch_core_media_gen_key_frame(session);
    ...
    while (switch_channel_up_nosig(channel)) {
        ...
        if (!switch_channel_test_flag(channel, CF_VIDEO_READY) &&
            switch_channel_test_flag(channel, CF_VIDEO_DECODED_READ) &&
            (++xloops > 10 || switch_channel_test_flag(channel, CF_VIDEO_PASSIVE))) {
        ...
        if (switch_channel_test_flag(channel, CF_VIDEO_READY) &&
            !switch_test_flag(read_frame, SFF_CNG)) {
            switch_mutex_lock(mh->file_read_mutex);
            if (smh->video_read_fh && switch_test_flag(smh->video_read_fh, SWITCH_FILE_OPEN) &&
                read_frame->img) {
...
SWITCH_DECLARE(switch_status_t)
switch_core_session_start_video_thread(switch_core_session_t* session) {
    switch_threadattr_t* thd_attr = NULL;
    switch_memory_pool_t* pool = switch_core_session_get_pool(session);
    switch_rtp_engine_t* v_engine = NULL;
    switch_media_handle_t* smh;
...
SWITCH_DECLARE(void)
switch_core_media_start_engine_function(switch_core_session_t* session, switch_media_type_t type,
                                        switch_engine_function_t engine_function, void* user_data) {
    switch_media_handle_t* smh;
    switch_rtp_engine_t* engine;
...
SWITCH_DECLARE(int)
switch_core_media_check_engine_function(switch_core_session_t* session, switch_media_type_t type) {
    switch_media_handle_t* smh;
    int r;
    switch_rtp_engine_t* engine;
...
SWITCH_DECLARE(void)
switch_core_media_end_engine_function(switch_core_session_t* session, switch_media_type_t type) {
    switch_media_handle_t* smh;
    switch_rtp_engine_t* engine;
...
SWITCH_DECLARE(switch_bool_t)
switch_core_session_in_video_thread(switch_core_session_t* session) {
    switch_rtp_engine_t* v_engine;
    switch_media_handle_t* smh;
    ...
    return switch_thread_equal(switch_thread_self(), v_engine->thread_id) ? SWITCH_TRUE
                                                                          : SWITCH_FALSE;
}

SWITCH_DECLARE(void)
switch_core_media_parse_media_flags(switch_core_session_t* session) {
    const char* var;
    switch_media_handle_t* smh;
...
SWITCH_DECLARE(switch_status_t)
switch_core_media_proxy_remote_addr(switch_core_session_t* session, const char* sdp_str) {
    const char* err;
    char rip[RA_PTR_LEN] = "";
    char rp[RA_PTR_LEN] = "";
    char rvp[RA_PTR_LEN] = "";
    ...
    if ((p = (char*)switch_stristr("c=IN IP4 ", sdp_str)) ||
        (p = (char*)switch_stristr("c=IN IP6 ", sdp_str))) {
        ip_ptr = p + 9;
    }
    ...
    if (v_engine->cur_payload_map && v_engine->cur_payload_map->remote_sdp_ip &&
        v_engine->cur_payload_map->remote_sdp_port) {
        if (!strcmp(v_engine->cur_payload_map->remote_sdp_ip, rip) &&
            atoi(rvp) == v_engine->cur_payload_map->remote_sdp_port) {
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                              "Remote video address:port [%s:%d] has not changed.\n",
                              v_engine->cur_payload_map->remote_sdp_ip,
                              v_engine->cur_payload_map->remote_sdp_port);
        } else {
            switch_channel_set_flag(session->channel, CF_VIDEO_POSSIBLE);
            ...
            if (switch_rtp_ready(v_engine->rtp_session)) {
                ...
                if (!remote_rtcp_port) {
                    if ((rport = switch_channel_get_variable(session->channel,
                ...
                if (switch_rtp_set_remote_address(
                        v_engine->rtp_session, v_engine->cur_payload_map->remote_sdp_ip,
                        v_engine->cur_payload_map->remote_sdp_port, remote_rtcp_port, SWITCH_TRUE,
                        &err) != SWITCH_STATUS_SUCCESS) {
                    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
        ...
        if (!switch_media_handle_test_media_flag(smh, SCMF_DISABLE_RTP_AUTOADJ) &&
            ...
            !((val = switch_channel_get_variable(session->channel, "disable_rtp_auto_adjust")) &&
              switch_true(val)) &&
            v_engine->rtp_session && !switch_channel_test_flag(session->channel, CF_AVPF)) {
            /* Reactivate the NAT buster flag. */
            switch_rtp_set_flag(v_engine->rtp_session, SWITCH_RTP_FLAG_AUTOADJ);
        }
    }

    if (t_engine->cur_payload_map && t_engine->cur_payload_map->remote_sdp_ip &&
        t_engine->cur_payload_map->remote_sdp_port) {
        if (!strcmp(t_engine->cur_payload_map->remote_sdp_ip, rip) &&
            atoi(rvp) == t_engine->cur_payload_map->remote_sdp_port) {
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                              "Remote text address:port [%s:%d] has not changed.\n",
            ...
            if (switch_rtp_ready(t_engine->rtp_session)) {
                ...
                if (switch_rtp_set_remote_address(
                        t_engine->rtp_session, t_engine->cur_payload_map->remote_sdp_ip,
                        t_engine->cur_payload_map->remote_sdp_port, remote_rtcp_port, SWITCH_TRUE,
                        &err) != SWITCH_STATUS_SUCCESS) {
                    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
                                      "TEXT RTP REPORTS ERROR: [%s]\n", err);
                } else {
                    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                                      "TEXT RTP CHANGING DEST TO: [%s:%d]\n",
                                      t_engine->cur_payload_map->remote_sdp_ip,
                                      t_engine->cur_payload_map->remote_sdp_port);
                    if (switch_media_handle_test_media_flag(smh, SCMF_AUTOFIX_TIMING)) {
        ...
        if (!switch_media_handle_test_media_flag(smh, SCMF_DISABLE_RTP_AUTOADJ) &&
            ...
            !((val = switch_channel_get_variable(session->channel, "disable_rtp_auto_adjust")) &&
              switch_true(val)) &&
            t_engine->rtp_session && !switch_channel_test_flag(session->channel, CF_AVPF)) {
            /* Reactivate the NAT buster flag. */
            switch_rtp_set_flag(t_engine->rtp_session, SWITCH_RTP_FLAG_AUTOADJ);
        }
    }
    ...
    if (switch_rtp_ready(a_engine->rtp_session)) {
        ...
        if (switch_rtp_set_remote_address(
                a_engine->rtp_session, a_engine->cur_payload_map->remote_sdp_ip,
                a_engine->cur_payload_map->remote_sdp_port, remote_rtcp_port, SWITCH_TRUE,
                &err) != SWITCH_STATUS_SUCCESS) {
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
                              "AUDIO RTP REPORTS ERROR: [%s]\n", err);
            ...
            if (!switch_media_handle_test_media_flag(smh, SCMF_DISABLE_RTP_AUTOADJ) &&
                !((val =
                       switch_channel_get_variable(session->channel, "disable_rtp_auto_adjust")) &&
                  switch_true(val)) &&
                !switch_channel_test_flag(session->channel, CF_AVPF)) {
                /* Reactivate the NAT buster flag. */
                switch_rtp_set_flag(a_engine->rtp_session, SWITCH_RTP_FLAG_AUTOADJ);
...
//?
SWITCH_DECLARE(int)
switch_core_media_check_nat(switch_media_handle_t* smh, const char* network_ip) {
    switch_assert(network_ip);

    return (smh->mparams->extsipip && !switch_check_network_list_ip(network_ip, "loopback.auto") &&
            !switch_check_network_list_ip(network_ip, smh->mparams->local_network));
}

//?
SWITCH_DECLARE(switch_status_t)
switch_core_media_ext_address_lookup(switch_core_session_t* session, char** ip, switch_port_t* port,
                                     const char* sourceip)

{
    ...
    if (!strncasecmp(sourceip, "host:", 5)) {
        status = (*ip = switch_stun_host_lookup(sourceip + 5, pool)) ? SWITCH_STATUS_SUCCESS
                                                                     : SWITCH_STATUS_FALSE;
        ...
        for (x = 0; x < 5; x++) {
            if ((status = switch_stun_lookup(ip, port, stun_ip, stun_port, &error, pool)) !=
                SWITCH_STATUS_SUCCESS) {
                switch_yield(100000);
            } else {
                break;
...
//?
SWITCH_DECLARE(void)
switch_core_media_reset_autofix(switch_core_session_t* session, switch_media_type_t type) {
    switch_rtp_engine_t* engine;
    switch_media_handle_t* smh;
...
//?
SWITCH_DECLARE(switch_status_t)
switch_core_media_choose_port(switch_core_session_t* session, switch_media_type_t type, int force) {
    ...
    if (zstr(smh->mparams->remote_ip)) { /* no remote_ip, we're originating */
        if (!zstr(
                smh->mparams->extrtpip)) { /* and we've got an ext-rtp-ip, eg, from verto config */
            use_ip =
                smh->mparams->extrtpip; /* let's use it for composing local sdp to send to client */
                                        /*
                                        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
                                                                "%s will use %s instead of %s in SDP, because we're originating
                                        and we have an ext-rtp-ip                             setting\n",
                                        switch_channel_get_name(smh->session->channel),                             smh->mparams->extrtpip,
                                        smh->mparams->rtpip);
                                        */
        }
    }
    engine->adv_sdp_port = sdp_port;
    engine->adv_sdp_ip = smh->mparams->adv_sdp_audio_ip = smh->mparams->extrtpip =
        switch_core_session_strdup(session, use_ip);

    if (type == SWITCH_MEDIA_TYPE_AUDIO) {
        switch_channel_set_variable(session->channel, SWITCH_LOCAL_MEDIA_IP_VARIABLE,
                                    engine->local_sdp_ip);
        switch_channel_set_variable_printf(session->channel, SWITCH_LOCAL_MEDIA_PORT_VARIABLE, "%d",
                                           sdp_port);
        switch_channel_set_variable(session->channel, SWITCH_ADVERTISED_MEDIA_IP_VARIABLE,
                                    engine->adv_sdp_ip);
    } else if (type == SWITCH_MEDIA_TYPE_VIDEO) {
        switch_channel_set_variable(session->channel, SWITCH_LOCAL_VIDEO_IP_VARIABLE,
                                    engine->adv_sdp_ip);
        switch_channel_set_variable_printf(session->channel, SWITCH_LOCAL_VIDEO_PORT_VARIABLE, "%d",
                                           sdp_port);
    } else if (type == SWITCH_MEDIA_TYPE_TEXT) {
        switch_channel_set_variable(session->channel, SWITCH_LOCAL_TEXT_IP_VARIABLE,
                                    engine->adv_sdp_ip);
        switch_channel_set_variable_printf(session->channel, SWITCH_LOCAL_TEXT_PORT_VARIABLE, "%d",
                                           sdp_port);
    }
...
SWITCH_DECLARE(switch_status_t)
switch_core_media_choose_ports(switch_core_session_t* session, switch_bool_t audio,
                               switch_bool_t video) {
    switch_status_t status = SWITCH_STATUS_SUCCESS;
    switch_media_handle_t* smh;
    ...
    if (audio && (status = switch_core_media_choose_port(session, SWITCH_MEDIA_TYPE_AUDIO, 0)) ==
                     SWITCH_STATUS_SUCCESS) {
        if (video) {
            switch_core_media_check_video_codecs(session);
...
//?
SWITCH_DECLARE(void)
switch_core_media_deactivate_rtp(switch_core_session_t* session) {
    switch_rtp_engine_t *a_engine, *v_engine, *t_engine;
    switch_media_handle_t* smh;
...
static void gen_ice(switch_core_session_t* session, switch_media_type_t type, const char* ip,
    ...
    if (!engine->ice_out.cands[0][0].component_id) {
        ...
        engine->ice_out.cands[0][0].priority =
            (1 << 24) * 126 + (1 << 8) * 65535 +
            (1 << 0) * (256 - engine->ice_out.cands[0][0].component_id);
    }
...
SWITCH_DECLARE(void)
switch_core_session_wake_video_thread(switch_core_session_t* session) {
    switch_media_handle_t* smh;
    switch_rtp_engine_t* v_engine;
...
static void check_dtls_reinvite(switch_core_session_t* session, switch_rtp_engine_t* engine) {
    if (switch_channel_test_flag(session->channel, CF_REINVITE) && engine->new_dtls) {
        if (!zstr(engine->local_dtls_fingerprint.str) && switch_rtp_has_dtls() &&
            dtls_ok(session)) {
            ...
            dtls_type_t xtype,
                dtype = engine->dtls_controller ? DTLS_TYPE_CLIENT : DTLS_TYPE_SERVER;
...

{
    ...
    if (a_engine->rtp_session || v_engine->rtp_session || t_engine->rtp_session ||
        switch_channel_test_flag(session->channel, CF_REINVITE)) {
        is_reinvite = 1;
    }
    ...
    if (!is_reinvite) {
        if (switch_rtp_ready(a_engine->rtp_session)) {
            if (switch_channel_test_flag(session->channel, CF_TEXT_POSSIBLE) &&
                !switch_rtp_ready(t_engine->rtp_session)) {
                goto text;
            }
    ...
    if ((status = switch_core_media_set_codec(session, 0, smh->mparams->codec_flags)) !=
        SWITCH_STATUS_SUCCESS) {
        goto end;
    }
    ...
    if (switch_media_handle_test_media_flag(smh, SCMF_PASS_RFC2833) ||
        ((val = switch_channel_get_variable(session->channel, "pass_rfc2833")) &&
         switch_true(val))) {
        switch_channel_set_flag(session->channel, CF_PASS_RFC2833);
    }

    if (switch_media_handle_test_media_flag(smh, SCMF_AUTOFLUSH) ||
        ((val = switch_channel_get_variable(session->channel, "rtp_autoflush")) &&
         switch_true(val))) {
    ...
    if ((flags[SWITCH_RTP_FLAG_BYTESWAP]) &&
        (val = switch_channel_get_variable(session->channel, "rtp_disable_byteswap")) &&
        switch_true(val)) {
        flags[SWITCH_RTP_FLAG_BYTESWAP] = 0;
    }

    if (a_engine->rtp_session && is_reinvite) {
        // const char *ip = switch_channel_get_variable(session->channel,
        // SWITCH_LOCAL_MEDIA_IP_VARIABLE); const char *port =
        // switch_channel_get_variable(session->channel, SWITCH_LOCAL_MEDIA_PORT_VARIABLE);
        char* remote_host = switch_rtp_get_remote_host(a_engine->rtp_session);
        switch_port_t remote_port = switch_rtp_get_remote_port(a_engine->rtp_session);
    ...
    if (!switch_channel_test_flag(session->channel, CF_PROXY_MEDIA)) {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                          "AUDIO RTP [%s] %s port %d -> %s port %d codec: %u ms: %d\n",
                          switch_channel_get_name(session->channel), a_engine->local_sdp_ip,
                          a_engine->local_sdp_port, a_engine->cur_payload_map->remote_sdp_ip,
                          a_engine->cur_payload_map->remote_sdp_port, a_engine->cur_payload_map->pt,
                          a_engine->read_impl.microseconds_per_packet / 1000);
    ...
    if (a_engine->rtp_session && is_reinvite) {
        ...
        if (!remote_rtcp_port) {
            if ((rport =
                     switch_channel_get_variable(session->channel, "rtp_remote_audio_rtcp_port"))) {
                remote_rtcp_port = (switch_port_t)atoi(rport);
            }
        }
        ...
        if (switch_rtp_set_remote_address(
            ...
            // if (switch_channel_test_flag(session->channel, CF_PROTO_HOLD) &&
            // strcmp(a_engine->cur_payload_map->remote_sdp_ip, "0.0.0.0")) {
    ...
    if (switch_channel_test_flag(session->channel, CF_PROXY_MEDIA)) {
        ...
        switch_log_printf(
            ...
            switch_channel_get_name(session->channel), a_engine->cur_payload_map->remote_sdp_ip,
            a_engine->cur_payload_map->remote_sdp_port, a_engine->cur_payload_map->remote_sdp_ip,
            a_engine->cur_payload_map->remote_sdp_port, a_engine->cur_payload_map->pt,
            a_engine->read_impl.microseconds_per_packet / 1000);
    ...
    if (switch_channel_up(session->channel)) {
        ...
        a_engine->rtp_session = switch_rtp_new(
            ...
            strcasecmp("opus", a_engine->read_impl.iananame)
                ? a_engine->read_impl.samples_per_packet
                : a_engine->read_impl.samples_per_second *
                      (a_engine->read_impl.microseconds_per_packet / 1000) / 1000,
            a_engine->cur_payload_map->codec_ms * 1000, flags, timer_name, &err,
    ...
    if (switch_rtp_ready(a_engine->rtp_session)) {
        ...
        switch_mutex_init(&smh->read_mutex[SWITCH_MEDIA_TYPE_AUDIO], SWITCH_MUTEX_NESTED,
                          switch_core_session_get_pool(session));
        switch_mutex_init(&smh->write_mutex[SWITCH_MEDIA_TYPE_AUDIO], SWITCH_MUTEX_NESTED,
                          switch_core_session_get_pool(session));

        // switch_core_media_set_rtp_session(session, SWITCH_MEDIA_TYPE_AUDIO,
        // a_engine->rtp_session);
        ...
        switch_channel_set_variable_printf(session->channel, "rtp_use_pt", "%d",
                                           a_engine->cur_payload_map->pt);

        if ((val = switch_channel_get_variable(session->channel, "rtp_enable_vad_in")) &&
            switch_true(val)) {
            vad_in = 1;
        }
        if ((val = switch_channel_get_variable(session->channel, "rtp_enable_vad_out")) &&
            switch_true(val)) {
            vad_out = 1;
        }

        if ((val = switch_channel_get_variable(session->channel, "rtp_disable_vad_in")) &&
            switch_true(val)) {
            vad_in = 0;
        }
        if ((val = switch_channel_get_variable(session->channel, "rtp_disable_vad_out")) &&
            switch_true(val)) {
            vad_out = 0;
        }
        ...
        switch_rtp_intentional_bugs(a_engine->rtp_session,
                                    a_engine->rtp_bugs | smh->mparams->manual_rtp_bugs);

        if ((vad_in && inb) || (vad_out && !inb)) {
            switch_rtp_enable_vad(a_engine->rtp_session, session, &a_engine->read_codec,
                                  SWITCH_VAD_FLAG_TALKING | SWITCH_VAD_FLAG_EVENTS_TALK |
                                      SWITCH_VAD_FLAG_EVENTS_NOTALK);

            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                              "AUDIO RTP Engage VAD for %s ( %s %s )\n",
        ...
        if (a_engine->ice_in.cands[a_engine->ice_in.chosen[0]][0].ready) {
            ...
            switch_rtp_activate_ice(a_engine->rtp_session, a_engine->ice_in.ufrag,
                                    a_engine->ice_out.ufrag, a_engine->ice_out.pwd,
                                    a_engine->ice_in.pwd, IPR_RTP,
#ifdef GOOGLE_ICE
                                    ICE_GOOGLE_JINGLE, NULL
#else
                                    switch_determine_ice_type(a_engine, session), &a_engine->ice_in
#endif
            );
        }
        ...
        if ((val = switch_channel_get_variable(session->channel, "rtcp_audio_interval_msec")) ||
            ...
            if (!strcasecmp(val, "passthru")) {
                switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO,
                                  "Activating RTCP PASSTHRU PORT %d\n", remote_rtcp_port);
            ...
            if (a_engine->ice_in.cands[a_engine->ice_in.chosen[1]][1].ready &&
                a_engine->ice_in.cands[a_engine->ice_in.chosen[0]][0].ready &&
                ...
                if (a_engine->rtcp_mux > 0 &&
                    !strcmp(a_engine->ice_in.cands[a_engine->ice_in.chosen[1]][1].con_addr,
                            a_engine->ice_in.cands[a_engine->ice_in.chosen[0]][0].con_addr) &&
                    a_engine->ice_in.cands[a_engine->ice_in.chosen[1]][1].con_port ==
                        a_engine->ice_in.cands[a_engine->ice_in.chosen[0]][0].con_port) {
                    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO,
                                      "Skipping RTCP ICE (Same as RTP)\n");
                } else {
                    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO,
                                      "Activating RTCP ICE\n");
        ...
        if (!zstr(a_engine->local_dtls_fingerprint.str) && switch_rtp_has_dtls() &&
            dtls_ok(smh->session)) {
            dtls_type_t xtype,
                dtype = a_engine->dtls_controller ? DTLS_TYPE_CLIENT : DTLS_TYPE_SERVER;
            ...
            switch_rtp_add_dtls(a_engine->rtp_session, &a_engine->local_dtls_fingerprint,
                                &a_engine->remote_dtls_fingerprint, dtype | xtype, want_DTLSv1_2);

            if (a_engine->rtcp_mux < 1 && smh->mparams->rtcp_audio_interval_msec) {
                xtype = DTLS_TYPE_RTCP;
                switch_rtp_add_dtls(a_engine->rtp_session, &a_engine->local_dtls_fingerprint,
                                    &a_engine->remote_dtls_fingerprint, dtype | xtype,
                                    want_DTLSv1_2);
            }
        }
        ...
        if (smh->mparams->rtp_timeout_sec) {
            a_engine->max_missed_packets =
                (a_engine->read_impl.samples_per_second * smh->mparams->rtp_timeout_sec) /
                a_engine->read_impl.samples_per_packet;
        ...
        if (smh->mparams->rtp_hold_timeout_sec) {
            a_engine->max_missed_hold_packets =
                (a_engine->read_impl.samples_per_second * smh->mparams->rtp_hold_timeout_sec) /
                a_engine->read_impl.samples_per_packet;
        }

        if (smh->mparams->te) {
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                              "%s Set 2833 dtmf send payload to %u\n",
                              switch_channel_get_name(session->channel), smh->mparams->te);
            switch_rtp_set_telephony_event(a_engine->rtp_session, smh->mparams->te);
            switch_channel_set_variable_printf(session->channel, "rtp_2833_send_payload", "%d",
                                               smh->mparams->te);
        }

        if (smh->mparams->recv_te) {
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                              "%s Set 2833 dtmf receive payload to %u\n",
            ...
            switch_channel_set_variable_printf(session->channel, "rtp_2833_recv_payload", "%d",
                                               smh->mparams->recv_te);
        }
        ...
        if (switch_media_handle_test_media_flag(smh, SCMF_SUPPRESS_CNG) ||
            ((val = switch_channel_get_variable(session->channel, "supress_cng")) &&
             switch_true(val)) ||
            ((val = switch_channel_get_variable(session->channel, "suppress_cng")) &&
             switch_true(val))) {
            smh->mparams->cng_pt = 0;
        }
    ...
    text:
        ...
        if (switch_channel_test_flag(session->channel, CF_TEXT_POSSIBLE) &&
            t_engine->cur_payload_map->rm_encoding && t_engine->cur_payload_map->remote_sdp_port) {
            /******************************************************************************************/
            if (t_engine->rtp_session && is_reinvite) {
                ...
                if (remote_host && remote_port &&
                    !strcmp(remote_host, t_engine->cur_payload_map->remote_sdp_ip) &&
                    remote_port == t_engine->cur_payload_map->remote_sdp_port) {
                    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                                      "Text params are unchanged for %s.\n",
                                      switch_channel_get_name(session->channel));
                    t_engine->cur_payload_map->negotiated = 1;
                    goto text_up;
                } else {
                    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                                      "Text params changed for %s from %s:%d to %s:%d\n",
                                      switch_channel_get_name(session->channel), remote_host,
                                      remote_port, t_engine->cur_payload_map->remote_sdp_ip,
                                      t_engine->cur_payload_map->remote_sdp_port);
                }
            }
            ...
            if (!switch_channel_test_flag(session->channel, CF_PROXY_MEDIA)) {
                if (switch_rtp_ready(t_engine->rtp_session)) {
                    switch_log_printf(
                        ...
                        switch_channel_get_name(session->channel), t_engine->local_sdp_ip,
                        t_engine->local_sdp_port, t_engine->cur_payload_map->remote_sdp_ip,
                        t_engine->cur_payload_map->remote_sdp_port, t_engine->cur_payload_map->pt);

                    switch_rtp_set_default_payload(t_engine->rtp_session,
            ...
            if (t_engine->rtp_session && is_reinvite) {
                ...
                if (switch_rtp_set_remote_address(
                    ...
                    if (!switch_media_handle_test_media_flag(smh, SCMF_DISABLE_RTP_AUTOADJ) &&
                        !switch_channel_test_flag(session->channel, CF_AVPF) &&
                        !((val = switch_channel_get_variable(session->channel,
            ...
            if (switch_channel_test_flag(session->channel, CF_PROXY_MEDIA)) {
                ...
                switch_log_printf(
                    ...
                    switch_channel_get_name(session->channel),
                    a_engine->cur_payload_map->remote_sdp_ip, t_engine->local_sdp_port,
                    t_engine->cur_payload_map->remote_sdp_ip,
                    t_engine->cur_payload_map->remote_sdp_port, t_engine->cur_payload_map->pt,
                    t_engine->read_impl.microseconds_per_packet / 1000);
            ...
            t_engine->rtp_session = switch_rtp_new(
                a_engine->local_sdp_ip, t_engine->local_sdp_port,
                t_engine->cur_payload_map->remote_sdp_ip,
                t_engine->cur_payload_map->remote_sdp_port, t_engine->cur_payload_map->pt,
                TEXT_TIMER_SAMPLES, TEXT_TIMER_MS * 1000, flags, NULL, &err,
                switch_core_session_get_pool(session), 0, 0);

            switch_log_printf(
                SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                "%sTEXT RTP [%s] %s:%d->%s:%d codec: %u ms: %d [%s]\n",
                switch_channel_test_flag(session->channel, CF_PROXY_MEDIA) ? "PROXY " : "",
                switch_channel_get_name(session->channel), a_engine->local_sdp_ip,
                t_engine->local_sdp_port, t_engine->cur_payload_map->remote_sdp_ip,
                t_engine->cur_payload_map->remote_sdp_port, t_engine->cur_payload_map->pt, 0,
                switch_rtp_ready(t_engine->rtp_session) ? "SUCCESS" : err);

            if (switch_rtp_ready(t_engine->rtp_session)) {
                const char* ssrc;

                if (!t_engine->tf) {
                    switch_rtp_text_factory_create(&t_engine->tf,
                                                   switch_core_session_get_pool(session));
                }
                ...
                if (t_engine->ice_in.cands[t_engine->ice_in.chosen[0]][0].ready) {
                    ...
                    switch_rtp_activate_ice(t_engine->rtp_session, t_engine->ice_in.ufrag,
                                            t_engine->ice_out.ufrag, t_engine->ice_out.pwd,
                                            t_engine->ice_in.pwd, IPR_RTP,
#ifdef GOOGLE_ICE
                                            ICE_GOOGLE_JINGLE, NULL
#else
                ...
                if ((val = switch_channel_get_variable(session->channel,
                    ...
                    (val = smh->mparams->rtcp_text_interval_msec)) {
                    const char* rport =
                        switch_channel_get_variable(session->channel, "rtp_remote_text_rtcp_port");
                    ...
                    if (!strcasecmp(val, "passthru")) {
                        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO,
                                          "Activating TEXT RTCP PASSTHRU PORT %d\n", remote_port);
                        switch_rtp_activate_rtcp(t_engine->rtp_session, -1, remote_port,
                                                 t_engine->rtcp_mux > 0);
                    } else {
                        int interval = atoi(val);
                        if (interval < 100 || interval > 500000) {
                        ...
                        switch_rtp_activate_rtcp(t_engine->rtp_session, interval, remote_port,
                                                 t_engine->rtcp_mux > 0);
                    }

                    if (t_engine->ice_in.cands[t_engine->ice_in.chosen[1]][1].ready &&
                        t_engine->ice_in.cands[t_engine->ice_in.chosen[0]][0].ready &&
                        ...
                        if (t_engine->rtcp_mux > 0 &&
                            !strcmp(
                                t_engine->ice_in.cands[t_engine->ice_in.chosen[1]][1].con_addr,
                                t_engine->ice_in.cands[t_engine->ice_in.chosen[0]][0].con_addr) &&
                            t_engine->ice_in.cands[t_engine->ice_in.chosen[1]][1].con_port ==
                                t_engine->ice_in.cands[t_engine->ice_in.chosen[0]][0].con_port) {
                            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO,
                                              "Skipping TEXT RTCP ICE (Same as TEXT RTP)\n");
                        } else {
                            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO,
                                              "Activating TEXT RTCP ICE\n");
                            switch_rtp_activate_ice(t_engine->rtp_session, t_engine->ice_in.ufrag,
                                                    t_engine->ice_out.ufrag, t_engine->ice_out.pwd,
                                                    t_engine->ice_in.pwd, IPR_RTCP,
#ifdef GOOGLE_ICE
                                                    ICE_GOOGLE_JINGLE, NULL
#else
                                                    switch_determine_ice_type(t_engine, session),
                ...
                if (!zstr(t_engine->local_dtls_fingerprint.str) && switch_rtp_has_dtls() &&
                    dtls_ok(smh->session)) {
                    dtls_type_t xtype,
                    ...
                    switch_rtp_add_dtls(t_engine->rtp_session, &t_engine->local_dtls_fingerprint,
                                        &t_engine->remote_dtls_fingerprint, dtype | xtype,
                                        want_DTLSv1_2);

                    if (t_engine->rtcp_mux < 1 && smh->mparams->rtcp_text_interval_msec) {
                        xtype = DTLS_TYPE_RTCP;
                        switch_rtp_add_dtls(
                            t_engine->rtp_session, &t_engine->local_dtls_fingerprint,
                            &t_engine->remote_dtls_fingerprint, dtype | xtype, want_DTLSv1_2);
                    }
                }
                ...
                switch_rtp_intentional_bugs(
                    t_engine->rtp_session, t_engine->rtp_bugs | smh->mparams->manual_text_rtp_bugs);

                // XX

                switch_channel_set_variable_printf(session->channel, "rtp_use_text_pt", "%d",
                                                   t_engine->cur_payload_map->pt);
                t_engine->ssrc = switch_rtp_get_ssrc(t_engine->rtp_session);
                switch_channel_set_variable_printf(session->channel, "rtp_use_text_ssrc", "%u",
                                                   t_engine->ssrc);
    ...
    video:
        ...
        if (switch_channel_test_flag(session->channel, CF_VIDEO_POSSIBLE) &&
            v_engine->cur_payload_map->rm_encoding && v_engine->cur_payload_map->remote_sdp_port) {
            /******************************************************************************************/
            if (v_engine->rtp_session && is_reinvite) {
                ...
                if (remote_host && remote_port &&
                    !strcmp(remote_host, v_engine->cur_payload_map->remote_sdp_ip) &&
                    ...
                    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                                      "Video params changed for %s from %s:%d to %s:%d\n",
                                      switch_channel_get_name(session->channel), remote_host,
                                      remote_port, v_engine->cur_payload_map->remote_sdp_ip,
                                      v_engine->cur_payload_map->remote_sdp_port);
                }
            }
            ...
            if (!switch_channel_test_flag(session->channel, CF_PROXY_MEDIA)) {
                if (switch_rtp_ready(v_engine->rtp_session)) {
                    switch_log_printf(
                        ...
                        switch_channel_get_name(session->channel), v_engine->local_sdp_ip,
                        v_engine->local_sdp_port, v_engine->cur_payload_map->remote_sdp_ip,
            ...
            if (switch_channel_test_flag(session->channel, CF_PROXY_MEDIA)) {
                ...
                switch_log_printf(
                    ...
                    switch_channel_get_name(session->channel),
                    a_engine->cur_payload_map->remote_sdp_ip, v_engine->local_sdp_port,
                    v_engine->cur_payload_map->remote_sdp_ip,
                    v_engine->cur_payload_map->remote_sdp_port, v_engine->cur_payload_map->pt,
                    v_engine->read_impl.microseconds_per_packet / 1000);
            ...
            if ((v_engine->nack) &&
                !switch_channel_var_true(session->channel, "rtp_video_nack_disable")) {
            ...
            v_engine->rtp_session = switch_rtp_new(
                a_engine->local_sdp_ip, v_engine->local_sdp_port,
                v_engine->cur_payload_map->remote_sdp_ip,
                v_engine->cur_payload_map->remote_sdp_port, v_engine->cur_payload_map->pt, 1, 90000,
                flags, NULL, &err, switch_core_session_get_pool(session), 0, 0);
            ...
            switch_log_printf(
                ...
                switch_channel_get_name(session->channel), a_engine->local_sdp_ip,
                v_engine->local_sdp_port, v_engine->cur_payload_map->remote_sdp_ip,
                v_engine->cur_payload_map->remote_sdp_port, v_engine->cur_payload_map->pt, 0,
                switch_rtp_ready(v_engine->rtp_session) ? "SUCCESS" : err);
            ...
            if (switch_rtp_ready(v_engine->rtp_session)) {
                ...
                if (v_engine->ice_in.cands[v_engine->ice_in.chosen[0]][0].ready) {
                    ...
                    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO,
                                      "Activating Video ICE\n");

                    switch_rtp_activate_ice(v_engine->rtp_session, v_engine->ice_in.ufrag,
                                            v_engine->ice_out.ufrag, v_engine->ice_out.pwd,
                                            v_engine->ice_in.pwd, IPR_RTP,
#ifdef GOOGLE_ICE
                                            ICE_GOOGLE_JINGLE, NULL
#else
                ...
                if ((val = switch_channel_get_variable(session->channel,
                    ...
                    const char* rport =
                        switch_channel_get_variable(session->channel, "rtp_remote_video_rtcp_port");
                    ...
                    if (!strcasecmp(val, "passthru")) {
                        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO,
                                          "Activating VIDEO RTCP PASSTHRU PORT %d\n", remote_port);
                    ...
                    if (v_engine->ice_in.cands[v_engine->ice_in.chosen[1]][1].ready &&
                        v_engine->ice_in.cands[v_engine->ice_in.chosen[0]][0].ready &&
                        ...
                        if (v_engine->rtcp_mux > 0 &&
                            !strcmp(
                                v_engine->ice_in.cands[v_engine->ice_in.chosen[1]][1].con_addr,
                                v_engine->ice_in.cands[v_engine->ice_in.chosen[0]][0].con_addr) &&
                            v_engine->ice_in.cands[v_engine->ice_in.chosen[1]][1].con_port ==
                                v_engine->ice_in.cands[v_engine->ice_in.chosen[0]][0].con_port) {
                            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO,
                                              "Skipping VIDEO RTCP ICE (Same as VIDEO RTP)\n");
                        } else {
                            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO,
                                              "Activating VIDEO RTCP ICE\n");
                            switch_rtp_activate_ice(v_engine->rtp_session, v_engine->ice_in.ufrag,
                                                    v_engine->ice_out.ufrag, v_engine->ice_out.pwd,
                                                    v_engine->ice_in.pwd, IPR_RTCP,
#ifdef GOOGLE_ICE
                                                    ICE_GOOGLE_JINGLE, NULL
#else
                                                    switch_determine_ice_type(v_engine, session),
                ...
                if (!zstr(v_engine->local_dtls_fingerprint.str) && switch_rtp_has_dtls() &&
                    ...
                    switch_rtp_add_dtls(v_engine->rtp_session, &v_engine->local_dtls_fingerprint,
                                        &v_engine->remote_dtls_fingerprint, dtype | xtype,
                                        want_DTLSv1_2);

                    if (v_engine->rtcp_mux < 1 && smh->mparams->rtcp_video_interval_msec) {
                        xtype = DTLS_TYPE_RTCP;
                        switch_rtp_add_dtls(
                            v_engine->rtp_session, &v_engine->local_dtls_fingerprint,
                            &v_engine->remote_dtls_fingerprint, dtype | xtype, want_DTLSv1_2);
                    }
                }
                ...
                switch_channel_set_variable_printf(session->channel, "rtp_use_video_pt", "%d",
                                                   v_engine->cur_payload_map->pt);
                v_engine->ssrc = switch_rtp_get_ssrc(v_engine->rtp_session);
                switch_channel_set_variable_printf(session->channel, "rtp_use_video_ssrc", "%u",
                                                   v_engine->ssrc);
...
static const char* get_media_profile_name(switch_core_session_t* session, int secure,
                                          switch_core_media_avp_secure_t avp) {
    switch_assert(session);
...
static char* get_setup(switch_rtp_engine_t* engine, switch_core_session_t* session,
                       switch_sdp_type_t sdp_type) {
    if (sdp_type == SDP_OFFER) {
        engine->dtls_controller = 0;
        engine->new_dtls = 1;
...
//?
static void generate_m(switch_core_session_t* session, char* buf, size_t buflen, switch_port_t port,
                       const char* family, const char* ip, int cur_ptime, const char* append_audio,
                       const char* sr, int use_cng, int cng_type, switch_event_t* map, int secure,
                       switch_sdp_type_t sdp_type) {
    int i = 0;
    int rate;
    ...
    if (secure && !switch_channel_test_flag(session->channel, CF_DTLS)) {
        ...
        for (i = 0; smh->crypto_suite_order[i] != CRYPTO_INVALID; i++) {
            ...
            if ((a_engine->crypto_type == j || a_engine->crypto_type == CRYPTO_INVALID) &&
                !zstr(a_engine->ssec[j].local_crypto_key)) {
    ...
    switch_snprintf(buf + strlen(buf), buflen - strlen(buf), "m=audio %d %s", port,
                    get_media_profile_name(
                        session, secure || a_engine->crypto_type != CRYPTO_INVALID, avp_secure));
        ...
        if (sdp_type == SDP_ANSWER) {
            ...
            for (i = 0; i < smh->num_rates; i++) {
                if (smh->dtmf_ianacodes[i] < 128 && !already_did[smh->dtmf_ianacodes[i]]) {
                    switch_snprintf(buf + strlen(buf), buflen - strlen(buf), " %d",
                                    smh->dtmf_ianacodes[i]);
                    already_did[smh->dtmf_ianacodes[i]] = 1;
                }
                if (smh->cng_ianacodes[i] < 128 && !already_did[smh->cng_ianacodes[i]] &&
                    !switch_media_handle_test_media_flag(smh, SCMF_SUPPRESS_CNG) && cng_type &&
                    use_cng) {
                    switch_snprintf(buf + strlen(buf), buflen - strlen(buf), " %d",
                                    smh->cng_ianacodes[i]);
    ...
    for (i = 0; i < smh->mparams->num_codecs; i++) {
        ...
        if (map) {
            ...
            if ((check = switch_event_get_header(map, key)) ||
                (check = switch_event_get_header(map, imp->iananame))) {
                fmtp = check;
            }
        }
    ...
    if ((smh->mparams->dtmf_type == DTMF_2833 ||
         switch_channel_test_flag(session->channel, CF_LIBERAL_DTMF)) &&
        smh->mparams->te > 95) {
        if (smh->mparams->dtmf_type == DTMF_2833 && sdp_type == SDP_ANSWER) {
            ...
            uint8_t NDLB_line_flash_16 =
                switch_channel_var_true(session->channel, "NDLB_line_flash_16");
    ...
    if (!zstr(a_engine->local_dtls_fingerprint.type) && secure) {
        switch_snprintf(buf + strlen(buf), buflen - strlen(buf),
                        "a=fingerprint:%s %s\r\na=setup:%s\r\n",
                        a_engine->local_dtls_fingerprint.type, a_engine->local_dtls_fingerprint.str,
                        get_setup(a_engine, session, sdp_type));
    }
    ...
    if (a_engine->ice_out.cands[0][0].ready) {
        ...
        switch_snprintf(buf + strlen(buf), buflen - strlen(buf),
                        ...
                        ice_out->cands[0][0].transport, c1, ice_out->cands[0][0].con_addr,
                        ice_out->cands[0][0].con_port);

        if (include_external && !zstr(smh->mparams->extsipip)) {
            switch_snprintf(buf + strlen(buf), buflen - strlen(buf),
                            "a=candidate:%s 1 %s %u %s %d typ host generation 0\r\n", tmp3,
                            ice_out->cands[0][0].transport, c1, smh->mparams->extsipip,
                            ice_out->cands[0][0].con_port);
        }
        ...
        if (!zstr(a_engine->local_sdp_ip) && !zstr(ice_out->cands[0][0].con_addr) &&
            ...
            switch_snprintf(
                ...
                ice_out->cands[0][0].transport, c2, ice_out->cands[0][0].con_addr,
                ice_out->cands[0][0].con_port, a_engine->local_sdp_ip, a_engine->local_sdp_port);
        }
        ...
        if (a_engine->rtcp_mux < 1 ||
            ...
            switch_snprintf(buf + strlen(buf), buflen - strlen(buf),
                            ...
                            ice_out->cands[0][0].transport, c1, ice_out->cands[0][0].con_addr,
                            ice_out->cands[0][0].con_port + (a_engine->rtcp_mux > 0 ? 0 : 1));
            ...
            if (!zstr(a_engine->local_sdp_ip) && !zstr(ice_out->cands[0][1].con_addr) &&
                ...
                switch_snprintf(
                    ...
                    tmp2, ice_out->cands[0][0].transport, c2, ice_out->cands[0][0].con_addr,
                    ice_out->cands[0][0].con_port + (a_engine->rtcp_mux > 0 ? 0 : 1),
                    a_engine->local_sdp_ip,
                    a_engine->local_sdp_port + (a_engine->rtcp_mux > 0 ? 0 : 1));
            }
        }
    ...
    if (append_audio) {
        switch_snprintf(buf + strlen(buf), buflen - strlen(buf), "%s%s", append_audio,
                        end_of(append_audio) == '\n' ? "" : "\r\n");
    }
...
//?
SWITCH_DECLARE(void)
switch_core_media_check_dtmf_type(switch_core_session_t* session) {
    const char* val;
    switch_media_handle_t* smh;
...
//?
switch_status_t switch_core_media_sdp_map(const char* r_sdp, switch_event_t** fmtp,
                                          switch_event_t** pt) {
    sdp_media_t* m;
    sdp_parser_t* parser = NULL;
    sdp_session_t* sdp;
        ...
        if (m->m_proto == sdp_proto_rtp) {
            ...
            for (map = m->m_rtpmaps; map; map = map->rm_next) {
                if (map->rm_encoding) {
                    ...
                    if (map->rm_fmtp) {
                        switch_event_add_header_string(*fmtp, SWITCH_STACK_BOTTOM, key,
                                                       map->rm_fmtp);
                    }
                }
            }
...
//?
SWITCH_DECLARE(void)
switch_core_media_set_local_sdp(switch_core_session_t* session, const char* sdp_str,
                                switch_bool_t dup) {
    switch_media_handle_t* smh;
    ...
    smh->mparams->local_sdp_str =
        dup ? switch_core_session_strdup(session, sdp_str) : (char*)sdp_str;
...
SWITCH_DECLARE(void)
switch_core_media_gen_local_sdp(switch_core_session_t* session, switch_sdp_type_t sdp_type,
    ...
    const char* append_audio =
        switch_channel_get_variable(session->channel, "rtp_append_audio_sdp");
    const char* append_video =
        switch_channel_get_variable(session->channel, "rtp_append_video_sdp");
    char srbuf[128] = "";
    const char* var_val;
    const char* username;
    const char* fmtp_out;
    const char* fmtp_out_var =
        switch_channel_get_variable(session->channel, "rtp_force_audio_fmtp");
    switch_event_t *map = NULL, *ptmap = NULL;
    // const char *b_sdp = NULL;
    // const char *local_audio_crypto_key = switch_core_session_local_crypto_key(session,
    ...
    if (!smh->mparams->rtcp_audio_interval_msec) {
        smh->mparams->rtcp_audio_interval_msec =
            (char*)switch_channel_get_variable(session->channel, "rtcp_audio_interval_msec");
    }

    if (!smh->mparams->rtcp_video_interval_msec) {
        smh->mparams->rtcp_video_interval_msec =
            (char*)switch_channel_get_variable(session->channel, "rtcp_video_interval_msec");
    }
    ...
    if (switch_channel_test_flag(session->channel, CF_PROXY_OFF) &&
        (tmp = switch_channel_get_variable(smh->session->channel, "uuid_media_secure_media"))) {
    ...
    if (!smh->payload_space) {
        /* it could be 98 but chrome reserves 98 and 99 for some internal stuff even though they
           should not. Everyone expects dtmf to be at 101 and Its not worth the trouble so we'll
           start at 102 */
        ...
        if (sdp_type == SDP_OFFER) {
            ...
            if (orig_session && !switch_channel_test_flag(session->channel, CF_ANSWERED)) {
                switch_core_media_set_smode(
                    smh->session, SWITCH_MEDIA_TYPE_AUDIO,
                    switch_core_session_remote_media_flow(orig_session, SWITCH_MEDIA_TYPE_AUDIO),
                    sdp_type);
                switch_core_media_set_smode(
                    smh->session, SWITCH_MEDIA_TYPE_VIDEO,
                    switch_core_session_remote_media_flow(orig_session, SWITCH_MEDIA_TYPE_VIDEO),
                    sdp_type);
            }
            ...
            for (i = 0; i < smh->mparams->num_codecs; i++) {
                ...
                if (smh->ianacodes[i] > 64) {
                    if (smh->mparams->dtmf_type == DTMF_2833 && smh->mparams->te > 95 &&
                        smh->mparams->te == smh->payload_space) {
                        smh->payload_space++;
                    }
                    if (!switch_media_handle_test_media_flag(smh, SCMF_SUPPRESS_CNG) &&
                        smh->mparams->cng_pt && use_cng &&
                        smh->mparams->cng_pt == smh->payload_space) {
                        smh->payload_space++;
                    }
                    ...
                    if (orig_session &&
                        switch_core_session_get_payload_code(
                            orig_session,
                            imp->codec_type == SWITCH_CODEC_TYPE_AUDIO ? SWITCH_MEDIA_TYPE_AUDIO
                                                                       : SWITCH_MEDIA_TYPE_VIDEO,
                            imp->iananame, imp->samples_per_second, smh->fmtp[i], &orig_pt, NULL,
                            &orig_fmtp) == SWITCH_STATUS_SUCCESS) {
                ...
                switch_core_media_add_payload_map(
                    ...
                    imp->codec_type == SWITCH_CODEC_TYPE_AUDIO ? SWITCH_MEDIA_TYPE_AUDIO
                                                               : SWITCH_MEDIA_TYPE_VIDEO,
                    imp->iananame, imp->modname, smh->fmtps[i], sdp_type, smh->ianacodes[i],
                    imp->samples_per_second, imp->microseconds_per_packet / 1000,
                    imp->number_of_channels, SWITCH_FALSE);
            }
    ...
    // if (!a_engine->cur_payload_map->rm_encoding && (b_sdp =
    // switch_channel_get_variable(session->channel, SWITCH_B_SDP_VARIABLE))) {
    ...
    for (i = 0; i < smh->mparams->num_codecs; i++) {
        ...
        if (imp->codec_type == SWITCH_CODEC_TYPE_AUDIO &&
            a_engine->smode == SWITCH_MEDIA_FLOW_DISABLED) {
            switch_core_media_set_smode(smh->session, SWITCH_MEDIA_TYPE_AUDIO,
                                        SWITCH_MEDIA_FLOW_SENDRECV, sdp_type);
            break;
        }
    ...
    if (zstr(sr)) {
        ...
        if ((var_val = switch_channel_get_variable(session->channel, "origination_audio_mode"))) {
            if (!strcasecmp(sr, "sendonly") || !strcasecmp(sr, "recvonly") ||
                !strcasecmp(sr, "sendrecv") || !strcasecmp(sr, "inactive")) {
                sr = var_val;
            }
    ...
    if (switch_true(
            switch_channel_get_variable_dup(session->channel, "drop_dtmf", SWITCH_FALSE, -1))) {
    ...
    if ((smh->mparams->ndlb & SM_NDLB_SENDRECV_IN_SESSION) ||
        ((var_val = switch_channel_get_variable(session->channel, "ndlb_sendrecv_in_session")) &&
         switch_true(var_val))) {
        if (!zstr(sr)) {
    ...
    if (switch_channel_test_flag(smh->session->channel, CF_ICE) &&
        switch_channel_var_true(session->channel, "ice_lite")) {
    ...
    if (a_engine->codec_negotiated &&
        !switch_channel_test_flag(session->channel, CF_NOSDP_REINVITE)) {
        switch_snprintf(
            buf + strlen(buf), SDPBUFLEN - strlen(buf), "m=audio %d %s", port,
            get_media_profile_name(
                session,
        ...
        switch_snprintf(buf + strlen(buf), SDPBUFLEN - strlen(buf), " %d",
                        a_engine->cur_payload_map->pt);
        ...
        if (!switch_media_handle_test_media_flag(smh, SCMF_SUPPRESS_CNG) && smh->mparams->cng_pt &&
            use_cng) {
            switch_snprintf(buf + strlen(buf), SDPBUFLEN - strlen(buf), " %d",
                            smh->mparams->cng_pt);
        }
        ...
        if (!a_engine->cur_payload_map->adv_channels) {
            a_engine->cur_payload_map->adv_channels =
                get_channels(a_engine->cur_payload_map->rm_encoding, 1);
        }

        if (a_engine->cur_payload_map->adv_channels > 1) {
            switch_snprintf(buf + strlen(buf), SDPBUFLEN - strlen(buf), "a=rtpmap:%d %s/%d/%d\r\n",
                            a_engine->cur_payload_map->pt, a_engine->cur_payload_map->rm_encoding,
                            rate, a_engine->cur_payload_map->adv_channels);
        } else {
        ...
        if ((smh->mparams->dtmf_type == DTMF_2833 ||
            ...
            if (switch_channel_test_flag(session->channel, CF_AVPF)) {
                ...
                switch_snprintf(
                    ...
                    "a=rtpmap:%d telephone-event/%d\r\na=fmtp:%d 0-%d\r\n", smh->mparams->te,
                    smh->mparams->te_rate, smh->mparams->te,
                    (switch_channel_var_true(session->channel, "NDLB_line_flash_16") ? 16 : 15));
            }
        }
        ...
        if (append_audio) {
            switch_snprintf(buf + strlen(buf), SDPBUFLEN - strlen(buf), "%s%s", append_audio,
                            end_of(append_audio) == '\n' ? "" : "\r\n");
        }
        ...
        if (!zstr(a_engine->local_dtls_fingerprint.type)) {
            switch_snprintf(
                buf + strlen(buf), SDPBUFLEN - strlen(buf), "a=fingerprint:%s %s\r\na=setup:%s\r\n",
                a_engine->local_dtls_fingerprint.type, a_engine->local_dtls_fingerprint.str,
                get_setup(a_engine, session, sdp_type));
        }
        ...
        if (a_engine->ice_out.cands[0][0].ready) {
            ...
            if (!zstr(a_engine->local_sdp_ip) && !zstr(ice_out->cands[0][0].con_addr) &&
                ...
                switch_snprintf(
                    buf + strlen(buf), SDPBUFLEN - strlen(buf),
                    "a=candidate:%s 1 %s %u %s %d typ srflx raddr %s rport %d generation 0\r\n",
                    tmp2, ice_out->cands[0][0].transport, c3, ice_out->cands[0][0].con_addr,
                    ice_out->cands[0][0].con_port, a_engine->local_sdp_ip,
            ...
            if (a_engine->rtcp_mux < 1 || is_outbound ||
                ...
                if (include_external && !zstr(smh->mparams->extsipip)) {
                    switch_snprintf(
                        buf + strlen(buf), SDPBUFLEN - strlen(buf),
                        "a=candidate:%s 2 %s %u %s %d typ host generation 0\r\n", tmp3,
                        ice_out->cands[0][0].transport, c2, smh->mparams->extsipip,
                        ice_out->cands[0][0].con_port + (a_engine->rtcp_mux > 0 ? 0 : 1));
                }
                ...
                if (!zstr(a_engine->local_sdp_ip) && !zstr(ice_out->cands[0][0].con_addr) &&
                    ...
                    switch_snprintf(
                        buf + strlen(buf), SDPBUFLEN - strlen(buf),
                        "a=candidate:%s 2 %s %u %s %d typ srflx raddr %s rport %d generation 0\r\n",
                        tmp2, ice_out->cands[0][0].transport, c4, ice_out->cands[0][0].con_addr,
                        ice_out->cands[0][0].con_port + (a_engine->rtcp_mux > 0 ? 0 : 1),
                        a_engine->local_sdp_ip,
                        a_engine->local_sdp_port + (a_engine->rtcp_mux > 0 ? 0 : 1));
                }
        ...
        if (a_engine->crypto_type != CRYPTO_INVALID &&
            ...
            !zstr(a_engine->ssec[a_engine->crypto_type].local_crypto_key) &&
            switch_channel_test_flag(session->channel, CF_SECURE)) {
        ...
        if (switch_channel_test_flag(session->channel, CF_AVPF) || (mult && switch_false(mult))) {
            char* bp = buf;
            int both = (switch_channel_test_flag(session->channel, CF_AVPF) ||
                        switch_channel_test_flag(session->channel, CF_DTLS))
                           ? 0
                           : 1;
            ...
            if ((!a_engine->no_crypto && switch_channel_test_flag(session->channel, CF_SECURE)) ||
                switch_channel_test_flag(session->channel, CF_DTLS)) {
                generate_m(session, buf, SDPBUFLEN, port, family, ip, 0, append_audio, sr, use_cng,
                           cng_type, map, 1, sdp_type);
                bp = (buf + strlen(buf));
            ...
            if (both) {
                generate_m(session, bp, SDPBUFLEN - strlen(buf), port, family, ip, 0, append_audio,
                           sr, use_cng, cng_type, map, 0, sdp_type);
            }
        } else {
            for (i = 0; i < smh->mparams->num_codecs; i++) {
                const switch_codec_implementation_t* imp = smh->codecs[i];
                ...
                if (cur_ptime != this_ptime) {
                    ...
                    if ((!a_engine->no_crypto &&
                         switch_channel_test_flag(session->channel, CF_SECURE)) ||
                        switch_channel_test_flag(session->channel, CF_DTLS)) {
                        generate_m(session, bp, SDPBUFLEN - strlen(buf), port, family, ip,
                                   cur_ptime, append_audio, sr, use_cng, cng_type, map, 1,
                                   sdp_type);
                        bp = (buf + strlen(buf));
                    ...
                    if (both) {
                        generate_m(session, bp, SDPBUFLEN - strlen(buf), port, family, ip,
                                   cur_ptime, append_audio, sr, use_cng, cng_type, map, 0,
                                   sdp_type);
                    }
                }
            }
...
video:
    ...
    if (!switch_channel_test_flag(session->channel, CF_VIDEO_POSSIBLE) &&
        switch_media_handle_test_media_flag(smh, SCMF_RECV_SDP)) {
        has_vid = 0;
    } else {
        for (i = 0; i < smh->mparams->num_codecs; i++) {
            const switch_codec_implementation_t* imp = smh->codecs[i];
    ...
    if (!has_vid) {
        if (switch_channel_test_flag(session->channel, CF_VIDEO_SDP_RECVD)) {
            ...
            switch_snprintf(
                buf + strlen(buf), SDPBUFLEN - strlen(buf), "m=video 0 %s 19\r\n",
        ...
        if ((v_port = v_engine->adv_sdp_port)) {
            ...
            for (loops = 0; loops < 2; loops++) {
                ...
                switch_snprintf(
                    buf + strlen(buf), SDPBUFLEN - strlen(buf), "m=video %d %s", v_port,
                    get_media_profile_name(
                        session,
                ...
                if (v_engine->codec_negotiated) {
                    ...
                    if (switch_media_handle_test_media_flag(smh, SCMF_MULTI_ANSWER_VIDEO)) {
                        switch_mutex_lock(smh->sdp_mutex);
                        for (pmap = v_engine->cur_payload_map; pmap && pmap->allocated;
                             pmap = pmap->next) {
                ...
                if (!(vbw = switch_channel_get_variable(smh->session->channel,
                                                        "rtp_video_max_bandwidth"))) {
                    vbw = switch_channel_get_variable(smh->session->channel,
                ...
                if (v_engine->codec_negotiated) {
                        ...
                        if (ov_fmtp) {
                            ...
                            if (!pass_fmtp ||
                                switch_true(switch_channel_get_variable_dup(
                                    session->channel, "rtp_mirror_fmtp", SWITCH_FALSE, -1))) {
                                pass_fmtp =
                                    switch_channel_get_variable(session->channel, "rtp_video_fmtp");
                            }
                        }
                    }
                    ...
                    if (append_video) {
                        switch_snprintf(buf + strlen(buf), SDPBUFLEN - strlen(buf), "%s%s",
                                        append_video, end_of(append_video) == '\n' ? "" : "\r\n");
                    }
                } else if (smh->mparams->num_codecs) {
                    int already_did[128] = {0};
                ...
                if (v_engine->smode == SWITCH_MEDIA_FLOW_SENDRECV) {
                    switch_snprintf(buf + strlen(buf), SDPBUFLEN - strlen(buf), "%s",
                                    "a=sendrecv\r\n");
                ...
                if (!zstr(v_engine->local_dtls_fingerprint.type)) {
                    switch_snprintf(buf + strlen(buf), SDPBUFLEN - strlen(buf),
                                    ...
                                    get_setup(v_engine, session, sdp_type));
                }
                ...
                if (v_engine->codec_negotiated) {
                    ...
                    if (switch_media_handle_test_media_flag(smh, SCMF_MULTI_ANSWER_VIDEO)) {
                        ...
                        for (pmap = v_engine->cur_payload_map; pmap && pmap->allocated;
                             pmap = pmap->next) {
                            if (pmap->pt != v_engine->cur_payload_map->pt && pmap->negotiated) {
                                add_fb(buf, SDPBUFLEN, pmap->pt, v_engine->fir || fir,
                                       v_engine->nack || nack, v_engine->pli || pli,
                                       v_engine->tmmbr || tmmbr);
                            }
                        }
                        switch_mutex_unlock(smh->sdp_mutex);
                    }
                    ...
                    for (i = 0; i < smh->mparams->num_codecs; i++) {
                        ...
                        add_fb(buf, SDPBUFLEN, smh->ianacodes[i], v_engine->fir || fir,
                               v_engine->nack || nack, v_engine->pli || pli, v_engine->pli || pli);
                    }
                }
                ...
                if (v_engine->ice_out.cands[0][0].ready) {
                    ...
                    switch_snprintf(buf + strlen(buf), SDPBUFLEN - strlen(buf),
                                    "a=candidate:%s 1 %s %u %s %d typ host generation 0\r\n", tmp1,
                                    ice_out->cands[0][0].transport, c1,
                    ...
                    if (!zstr(v_engine->local_sdp_ip) && !zstr(ice_out->cands[0][0].con_addr) &&
                        ...
                        switch_snprintf(buf + strlen(buf), SDPBUFLEN - strlen(buf),
                                        "a=candidate:%s 1 %s %u %s %d typ srflx raddr %s rport %d "
                                        "generation 0\r\n",
                                        tmp2, ice_out->cands[0][0].transport, c3,
                                        ice_out->cands[0][0].con_addr,
                                        ice_out->cands[0][0].con_port, v_engine->local_sdp_ip,
                    ...
                    if (v_engine->rtcp_mux < 1 || is_outbound ||
                        ...
                        switch_snprintf(
                            ...
                            "a=candidate:%s 2 %s %u %s %d typ host generation 0\r\n", tmp1,
                            ice_out->cands[0][0].transport, c2, ice_out->cands[0][0].con_addr,
                            ice_out->cands[0][0].con_port + (v_engine->rtcp_mux > 0 ? 0 : 1));
                        ...
                        if (!zstr(v_engine->local_sdp_ip) && !zstr(ice_out->cands[0][1].con_addr) &&
                            ...
                            switch_snprintf(
                                ...
                                ice_out->cands[0][0].transport, c4, ice_out->cands[0][0].con_addr,
                                ice_out->cands[0][0].con_port + (v_engine->rtcp_mux > 0 ? 0 : 1),
                                v_engine->local_sdp_ip,
                                v_engine->local_sdp_port + (v_engine->rtcp_mux > 0 ? 0 : 1));
                        }
                    }
                ...
                if (loops == 0 && switch_channel_test_flag(session->channel, CF_SECURE) &&
                    !switch_channel_test_flag(session->channel, CF_DTLS)) {
                    for (i = 0; smh->crypto_suite_order[i] != CRYPTO_INVALID; i++) {
                        switch_rtp_crypto_key_type_t j = SUITES[smh->crypto_suite_order[i]].type;

                        if ((a_engine->crypto_type == j ||
                             a_engine->crypto_type == CRYPTO_INVALID) &&
                            !zstr(a_engine->ssec[j].local_crypto_key)) {
                            switch_snprintf(buf + strlen(buf), SDPBUFLEN - strlen(buf),
    ...
    if (switch_channel_test_cap(session->channel, CC_MSRP) && !smh->msrp_session) {
        ...
        if (want_msrp || want_msrps) {
            smh->msrp_session =
                switch_msrp_session_new(switch_core_session_get_pool(session),
                                        switch_core_session_get_uuid(session), want_msrps);
    ...
    if (smh->msrp_session) {
        ...
        if (!zstr(msrp_session->remote_path)) {
            if (zstr(msrp_session->local_path)) {
                msrp_session->local_path = switch_core_session_sprintf(
                    session, "msrp%s://%s:%d/%s;tcp", msrp_session->secure ? "s" : "", ip,
            ...
            switch_snprintf(buf + strlen(buf), SDPBUFLEN - strlen(buf),
                            ...
                            "a=setup:%s\r\n",
                            msrp_session->local_port, msrp_session->secure ? "TLS/" : "",
                            msrp_session->local_path, msrp_session->local_accept_types,
                            msrp_session->local_accept_wrapped_types,
                            msrp_session->active ? "active" : "passive");
        } else {
            char* uuid = switch_core_session_get_uuid(session);
            const char* file_selector =
                switch_channel_get_variable(session->channel, "sip_msrp_local_file_selector");
            const char* msrp_offer_active =
                switch_channel_get_variable(session->channel, "sip_msrp_offer_active");
            ...
            switch_snprintf(buf + strlen(buf), SDPBUFLEN - strlen(buf),
                            ...
                            "a=setup:%s\r\n",
                            msrp_session->local_port, msrp_session->secure ? "TLS/" : "",
                            msrp_session->local_path, msrp_session->active ? "active" : "passive");
    ...
    if (sdp_type == SDP_ANSWER && !switch_channel_test_flag(session->channel, CF_RTT)) {
        if (switch_channel_test_flag(session->channel, CF_TEXT_SDP_RECVD)) {
            ...
            switch_snprintf(
                buf + strlen(buf), SDPBUFLEN - strlen(buf), "m=text 0 %s 19\r\n",
                get_media_profile_name(session,
                                       (switch_channel_test_flag(session->channel, CF_SECURE) &&
                                        switch_channel_direction(session->channel) ==
                                            SWITCH_CALL_DIRECTION_OUTBOUND) ||
    ...
    } else if ((switch_channel_test_flag(session->channel, CF_WANT_RTT) ||
                switch_channel_test_flag(session->channel, CF_RTT) ||
        ...
        if (sdp_type == SDP_OFFER) {
            ...
            switch_core_media_add_payload_map(session, SWITCH_MEDIA_TYPE_TEXT, "red", NULL, NULL,
                                              SDP_OFFER, t_engine->red_pt, 1000, 0, 1, SWITCH_TRUE);

            switch_core_media_add_payload_map(session, SWITCH_MEDIA_TYPE_TEXT, "t140", NULL, NULL,
                                              SDP_OFFER, t_engine->t140_pt, 1000, 0, 1,
                                              SWITCH_TRUE);
        ...
        if ((t_port = t_engine->adv_sdp_port)) {
            ...
            for (loops = 0; loops < 2; loops++) {
                ...
                switch_snprintf(
                    buf + strlen(buf), SDPBUFLEN - strlen(buf), "m=text %d %s", t_port,
                    get_media_profile_name(
                        session,
                        (loops == 0 && switch_channel_test_flag(session->channel, CF_SECURE) &&
                         switch_channel_direction(session->channel) ==
                ...
                if (t_engine->codec_negotiated) {
                    ...
                    switch_core_media_set_smode(smh->session, SWITCH_MEDIA_TYPE_TEXT,
                                                SWITCH_MEDIA_FLOW_SENDRECV, sdp_type);
                }
                ...
                if (t_engine->codec_negotiated) {
                    ...
                    if (t_engine->t140_pt && t_engine->red_pt) {
                        switch_snprintf(buf + strlen(buf), SDPBUFLEN - strlen(buf),
                                        "a=fmtp:%d %d/%d/%d\r\n", t_engine->red_pt,
                                        t_engine->t140_pt, t_engine->t140_pt, t_engine->t140_pt);
                    }
                ...
                if (!zstr(t_engine->local_dtls_fingerprint.type)) {
                    switch_snprintf(buf + strlen(buf), SDPBUFLEN - strlen(buf),
                                    ...
                                    t_engine->local_dtls_fingerprint.str,
                                    get_setup(t_engine, session, sdp_type));
                }
                ...
                if (t_engine->ice_out.cands[0][0].ready) {
                    ...
                    if (!zstr(t_engine->local_sdp_ip) && !zstr(ice_out->cands[0][0].con_addr) &&
                        ...
                        switch_snprintf(buf + strlen(buf), SDPBUFLEN - strlen(buf),
                                        ...
                                        tmp2, ice_out->cands[0][0].transport, c3,
                                        ice_out->cands[0][0].con_addr,
                                        ice_out->cands[0][0].con_port, t_engine->local_sdp_ip,
                                        t_engine->local_sdp_port);
                    }

                    if (t_engine->rtcp_mux < 1 || is_outbound ||
                        switch_channel_test_flag(session->channel, CF_RECOVERING)) {
                        switch_snprintf(
                            buf + strlen(buf), SDPBUFLEN - strlen(buf),
                            "a=candidate:%s 2 %s %u %s %d typ host generation 0\r\n", tmp1,
                            ice_out->cands[0][0].transport, c2, ice_out->cands[0][0].con_addr,
                            ice_out->cands[0][0].con_port + (t_engine->rtcp_mux > 0 ? 0 : 1));
                        ...
                        if (!zstr(t_engine->local_sdp_ip) && !zstr(ice_out->cands[0][1].con_addr) &&
                            ...
                            switch_snprintf(
                                ...
                                ice_out->cands[0][0].transport, c4, ice_out->cands[0][0].con_addr,
                                ice_out->cands[0][0].con_port + (t_engine->rtcp_mux > 0 ? 0 : 1),
                                t_engine->local_sdp_ip,
                                t_engine->local_sdp_port + (t_engine->rtcp_mux > 0 ? 0 : 1));
                        }
                    }
                ...
                if (loops == 0 && switch_channel_test_flag(session->channel, CF_SECURE) &&
                    ...
                    for (i = 0; smh->crypto_suite_order[i] != CRYPTO_INVALID; i++) {
                        ...
                        if ((t_engine->crypto_type == j ||
                             t_engine->crypto_type == CRYPTO_INVALID) &&
                            !zstr(t_engine->ssec[j].local_crypto_key)) {
                            switch_snprintf(buf + strlen(buf), SDPBUFLEN - strlen(buf),
...
//?
SWITCH_DECLARE(void)
switch_core_media_absorb_sdp(switch_core_session_t* session) {
    const char* sdp_str;
    switch_rtp_engine_t* a_engine;
    switch_media_handle_t* smh;
        ...
        if ((parser = sdp_parse(NULL, sdp_str, (int)strlen(sdp_str), 0))) {
            if ((sdp = sdp_session(parser))) {
                for (m = sdp->sdp_media; m; m = m->m_next) {
                    ...
                    if (connection) {
                        a_engine->proxy_sdp_ip =
                            switch_core_session_strdup(session, connection->c_address);
                    }
...
switch_core_media_set_udptl_image_sdp(switch_core_session_t* session,
    ...
    if (stream_rejected(smh, sdp_media_audio)) {
        switch_snprintf(buf + strlen(buf), sizeof(buf) - strlen(buf), "m=audio 0 RTP/AVP 0\r\n");
    }
    ...
    switch_snprintf(buf + strlen(buf), sizeof(buf) - strlen(buf),
                    ...
                    "a=T38FaxUdpEC:%s\r\n",
                    //"a=T38VendorInfo:%s\r\n",
                    port, t38_options->T38FaxVersion, t38_options->T38MaxBitRate,
                    ...
                    t38_options->T38FaxRateManagement, max_buf, max_data, t38_options->T38FaxUdpEC
...
//?
SWITCH_DECLARE(void)
switch_core_media_patch_sdp(switch_core_session_t* session) {
    switch_size_t len;
    char *p, *q, *pe, *qe;
    ...
    if (!(smh->mparams->ndlb & SM_NDLB_NEVER_PATCH_REINVITE)) {
        if (switch_channel_test_flag(session->channel, CF_ANSWERED) &&
            (switch_stristr("sendonly", smh->mparams->local_sdp_str) ||
             switch_stristr("inactive", smh->mparams->local_sdp_str) ||
             switch_stristr("0.0.0.0", smh->mparams->local_sdp_str))) {
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
    ...
    if (zstr(a_engine->local_sdp_ip) ||
        !a_engine->local_sdp_port) {  // || switch_channel_test_flag(session->channel,
                                      // CF_PROXY_MEDIA)) {
        if (switch_core_media_choose_port(session, SWITCH_MEDIA_TYPE_AUDIO, 1) !=
            SWITCH_STATUS_SUCCESS) {
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
                              "%s I/O Error\n", switch_channel_get_name(session->channel));
            return;
        ...
        pmap = switch_core_media_add_payload_map(session, SWITCH_MEDIA_TYPE_AUDIO, "PROXY", NULL,
                                                 NULL, SDP_ANSWER, 0, 8000, 20, 1, SWITCH_TRUE);
    ...
    while (p && *p) {
        ...
        if (a_engine->local_sdp_ip && !strncmp("c=IN IP", p, 7)) {
            ...
            if (oe) {
                ...
                if (!smh->owner_id) {
                    smh->owner_id =
                        (uint32_t)(switch_time_t)switch_epoch_time_now(NULL) * 31821U + 13849U;
                }
                ...
                snprintf(o_line, sizeof(o_line), "o=%s %010u %010u IN %s %s\r\n",
                         smh->mparams->sdp_username, smh->owner_id, smh->session_id, family,
                         smh->mparams->sipip);
        ...
        } else if ((!strncmp("m=audio ", p, 8) && *(p + 8) != '0') ||
                   (!strncmp("m=image ", p, 8) && *(p + 8) != '0')) {
            strncpy(q, p, 8);
            p += 8;
            ...
            if (!has_video) {
                switch_core_media_choose_port(session, SWITCH_MEDIA_TYPE_VIDEO, 1);
                clear_pmaps(v_engine);
                pmap = switch_core_media_add_payload_map(session, SWITCH_MEDIA_TYPE_VIDEO,
                                                         "PROXY-VID", NULL, NULL, SDP_ANSWER, 0,
                                                         90000, 90000, 1, SWITCH_TRUE);
                v_engine->cur_payload_map = pmap;
            ...
            if (!has_text) {
                ...
                pmap = switch_core_media_add_payload_map(session, SWITCH_MEDIA_TYPE_TEXT,
                                                         "PROXY-TXT", NULL, NULL, SDP_ANSWER, 0,
                                                         90000, 90000, 1, SWITCH_TRUE);
                t_engine->cur_payload_map = pmap;
    ...
    switch_log_printf(
        SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "%s Patched SDP\n---\n%s\n+++\n%s\n",
...
switch_core_media_start_udptl(switch_core_session_t* session, switch_t38_options_t* t38_options) {
    ...
    if (switch_rtp_ready(a_engine->rtp_session)) {
        ...
        if (remote_host && remote_port && remote_port == t38_options->remote_port &&
            !strcmp(remote_host, t38_options->remote_ip)) {
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                              "Remote address:port [%s:%d] has not changed.\n",
        ...
        if (switch_rtp_set_remote_address(a_engine->rtp_session, t38_options->remote_ip,
                                          t38_options->remote_port, 0, SWITCH_TRUE,
                                          &err) != SWITCH_STATUS_SUCCESS) {
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
...
//?
SWITCH_DECLARE(void)
switch_core_media_hard_mute(switch_core_session_t* session, switch_bool_t on) {
    switch_core_session_message_t msg = {0};
...
SWITCH_DECLARE(cJSON*)
switch_core_media_gen_json_constraint(float min, float ideal, float max) {
...
SWITCH_DECLARE(switch_status_t)
switch_core_media_media_params(switch_core_session_t* session, const char* json) {
    switch_core_session_message_t msg = {0};
    char* parse = NULL;
    char* argv[25];
    int argc = 0, i;
...
// 检查并等待 DTLS（Datagram Transport Layer Security）握手完成
SWITCH_DECLARE(switch_bool_t)
switch_core_media_check_dtls(switch_core_session_t* session, switch_media_type_t type) {
    switch_media_handle_t* smh;
    switch_rtp_engine_t* engine;
    int checking = 0;
...
SWITCH_DECLARE(uint32_t)
switch_core_media_get_orig_bitrate(switch_core_session_t* session, switch_media_type_t type) {
    switch_media_handle_t* smh;
    switch_rtp_engine_t* engine;
...
SWITCH_DECLARE(switch_status_t)
switch_core_media_set_outgoing_bitrate(switch_core_session_t* session, switch_media_type_t type,
                                       uint32_t bitrate) {
    switch_media_handle_t* smh;
    switch_rtp_engine_t* engine;
    switch_status_t status = SWITCH_STATUS_FALSE;
    ...
    if (switch_core_codec_ready(&engine->write_codec)) {
        status = switch_core_codec_control(&engine->write_codec, SCC_VIDEO_BANDWIDTH, SCCT_INT,
                                           &new_bitrate, SCCT_NONE, NULL, NULL, NULL);
    }
...
SWITCH_DECLARE(float)
switch_core_media_get_media_bw_mult(switch_core_session_t* session) {
    switch_media_handle_t* smh;
    switch_rtp_engine_t* engine;
...
SWITCH_DECLARE(void)
switch_core_media_set_media_bw_mult(switch_core_session_t* session, float mult) {
    switch_media_handle_t* smh;
    switch_rtp_engine_t* engine;
...
//?
SWITCH_DECLARE(switch_status_t)
switch_core_media_reset_jb(switch_core_session_t* session, switch_media_type_t type) {
    switch_media_handle_t* smh;
    switch_rtp_engine_t* engine;
...
//?
SWITCH_DECLARE(switch_status_t)
switch_core_media_receive_message(switch_core_session_t* session,
                                  switch_core_session_message_t* msg) {
    switch_media_handle_t* smh;
    switch_rtp_engine_t *a_engine, *v_engine, *t_engine;
    switch_status_t status = SWITCH_STATUS_SUCCESS;
    ...
    switch (msg->message_id) {
        ...
        case SWITCH_MESSAGE_INDICATE_VIDEO_REFRESH_REQ: {
            if (v_engine->rtp_session) {
                if (msg->numeric_arg ||
                    !switch_channel_test_flag(session->channel, CF_MANUAL_VID_REFRESH)) {
        ...
        case SWITCH_MESSAGE_INDICATE_CODEC_DEBUG_REQ: {
            ...
            if (engine->rtp_session) {
                switch_core_codec_control(&engine->read_codec, SCC_DEBUG, SCCT_INT, (void*)&level,
                                          SCCT_NONE, NULL, NULL, NULL);
                switch_core_codec_control(&engine->write_codec, SCC_DEBUG, SCCT_INT, (void*)&level,
                                          SCCT_NONE, NULL, NULL, NULL);
            }
        } break;
        ...
        case SWITCH_MESSAGE_INDICATE_CODEC_SPECIFIC_REQ: {
            ...
            if (engine->rtp_session) {
                if (iotype == SWITCH_IO_READ) {
                    switch_core_codec_control(&engine->read_codec, SCC_CODEC_SPECIFIC, SCCT_STRING,
                                              (void*)msg->string_array_arg[2], SCCT_STRING,
                                              (void*)msg->string_array_arg[3], &reply_type, &reply);
                } else {
                    switch_core_codec_control(&engine->write_codec, SCC_CODEC_SPECIFIC, SCCT_STRING,
                                              (void*)msg->string_array_arg[2], SCCT_STRING,
                                              (void*)msg->string_array_arg[3], &reply_type, &reply);
                }
        ...
        case SWITCH_MESSAGE_INDICATE_TRANSCODING_NECESSARY:
            if (a_engine->rtp_session &&
                switch_rtp_test_flag(a_engine->rtp_session, SWITCH_RTP_FLAG_PASS_RFC2833)) {
                switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING,
                                  "Pass 2833 mode may not work on a transcoded call.\n");
            }
            goto end;
            ...
            if (switch_rtp_ready(a_engine->rtp_session)) {
                ...
                if (!switch_channel_test_flag(session->channel, CF_VIDEO_READY) &&
                    ...
                    if (switch_channel_test_flag(session->channel, CF_JITTERBUFFER) &&
                        switch_channel_test_cap_partner(session->channel, CC_FS_RTP)) {
                        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                                          "%s PAUSE Jitterbuffer\n",
                                          switch_channel_get_name(session->channel));
                        switch_rtp_pause_jitter_buffer(a_engine->rtp_session, SWITCH_TRUE);
                ...
                if (switch_channel_test_flag(session->channel, CF_PASS_RFC2833) &&
                    switch_channel_test_flag_partner(session->channel, CF_FS_RTP)) {
		...
		if (switch_rtp_ready(v_engine->rtp_session)) {
            ...
            if (switch_rtp_ready(a_engine->rtp_session)) {
                ...
                if (switch_rtp_test_flag(a_engine->rtp_session, SWITCH_RTP_FLAG_PASS_RFC2833)) {
                    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                                      "%s deactivate passthru 2833 mode.\n",
                                      switch_channel_get_name(session->channel));
        ...
        case SWITCH_MESSAGE_INDICATE_NOMEDIA: {
            ...
            if (switch_true(switch_channel_get_variable(session->channel, "bypass_keep_codec"))) {
                switch_channel_set_variable(
                    session->channel, "absolute_codec_string",
                    switch_channel_get_variable(session->channel, "ep_codec_string"));
            }
            ...
            if ((uuid = switch_channel_get_partner_uuid(session->channel)) &&
                ...
                port =
                    switch_channel_get_variable(other_channel, SWITCH_REMOTE_MEDIA_PORT_VARIABLE);
                switch_core_session_rwunlock(other_session);
                ...
                if (ip && port) {
                    ...
                    clear_pmaps(a_engine);
                    clear_pmaps(v_engine);
                    switch_core_media_gen_local_sdp(session, SDP_OFFER, ip,
                                                    (switch_port_t)atoi(port), NULL, 1);
                }
            }
    ...
    switch (msg->message_id) {
        case SWITCH_MESSAGE_INDICATE_MEDIA_RENEG: {
            ...
            if (msg->string_arg) {
                ...
                if (*msg->string_arg == '=') {
                    ...
                    switch_channel_set_variable_printf(
                        session->channel, "codec_string", "=%s",
                        switch_channel_get_variable(session->channel, "ep_codec_string"));
                }
            ...
            if (msg->numeric_arg &&
                switch_core_session_get_partner(session, &nsession) == SWITCH_STATUS_SUCCESS) {
                msg->numeric_arg = 0;
        ...
        case SWITCH_MESSAGE_INDICATE_UDPTL_MODE: {
            switch_t38_options_t* t38_options =
                switch_channel_get_private(session->channel, "t38_options");
...
//?
SWITCH_DECLARE(void)
switch_core_media_break(switch_core_session_t* session, switch_media_type_t type) {
    switch_media_handle_t* smh;
...
//?
SWITCH_DECLARE(void)
switch_core_media_kill_socket(switch_core_session_t* session, switch_media_type_t type) {
    switch_media_handle_t* smh;
...
//?
SWITCH_DECLARE(switch_status_t)
switch_core_media_queue_rfc2833(switch_core_session_t* session, switch_media_type_t type,
...
//?
SWITCH_DECLARE(switch_status_t)
switch_core_media_queue_rfc2833_in(switch_core_session_t* session, switch_media_type_t type,
                                   const switch_dtmf_t* dtmf) {
    switch_media_handle_t* smh;
...
//?
SWITCH_DECLARE(uint8_t)
switch_core_media_ready(switch_core_session_t* session, switch_media_type_t type) {
    switch_media_handle_t* smh;
...
//?
SWITCH_DECLARE(void)
switch_core_media_set_rtp_flag(switch_core_session_t* session, switch_media_type_t type,
...
//?
SWITCH_DECLARE(void)
switch_core_media_clear_rtp_flag(switch_core_session_t* session, switch_media_type_t type,
                                 switch_rtp_flag_t flag) {
    switch_media_handle_t* smh;
...
//?
SWITCH_DECLARE(void)
switch_core_media_set_telephony_event(switch_core_session_t* session, switch_media_type_t type,
                                      switch_payload_t te) {
    switch_media_handle_t* smh;
...
//?
SWITCH_DECLARE(void)
switch_core_media_set_telephony_recv_event(switch_core_session_t* session, switch_media_type_t type,
                                           switch_payload_t te) {
    switch_media_handle_t* smh;
...
switch_core_media_get_stats(switch_core_session_t* session, switch_media_type_t type,
                            switch_memory_pool_t* pool) {
    switch_media_handle_t* smh;
...
switch_core_media_check_udptl_mode(switch_core_session_t* session, switch_media_type_t type) {
    ...
    if (switch_rtp_ready(smh->engines[type].rtp_session)) {
        return switch_rtp_test_flag(smh->engines[type].rtp_session, SWITCH_RTP_FLAG_UDPTL)
                   ? SWITCH_TRUE
                   : SWITCH_FALSE;
    }
...
//?
SWITCH_DECLARE(switch_status_t)
switch_core_media_udptl_mode(switch_core_session_t* session, switch_media_type_t type) {
    switch_media_handle_t* smh;
...
//?
SWITCH_DECLARE(switch_jb_t*)
switch_core_media_get_jb(switch_core_session_t* session, switch_media_type_t type) {
    switch_media_handle_t* smh;
...
//?
SWITCH_DECLARE(void)
switch_core_media_set_sdp_codec_string(switch_core_session_t* session, const char* r_sdp,
                                       switch_sdp_type_t sdp_type) {
    switch_core_media_merge_sdp_codec_string(session, r_sdp, sdp_type,
                                             switch_core_media_get_codec_string(session));
}

SWITCH_DECLARE(void)
switch_core_media_merge_sdp_codec_string(switch_core_session_t* session, const char* r_sdp,
...
static void add_audio_codec(sdp_rtpmap_t* map, const switch_codec_implementation_t* imp, int ptime,
                            char* buf, switch_size_t buflen) {
    int codec_ms = ptime;
    ...
    switch_snprintf(buf + strlen(buf), buflen - strlen(buf), ",%s.%s%s%s%s", imp->modname,
                    map->rm_encoding, ratestr, ptstr, bitstr);
}
...
static void switch_core_media_set_r_sdp_codec_string(switch_core_session_t* session,
    ...
    if (!zstr(codec_string)) {
        ...
        if ((tmp_codec_string = strdup(codec_string))) {
            num_codecs =
                switch_separate_string(tmp_codec_string, ',', codec_order, SWITCH_MAX_CODECS);
            num_codecs = switch_loadable_module_get_codecs_sorted(codecs, fmtp, SWITCH_MAX_CODECS,
                                                                  codec_order, num_codecs);
    ...
    for (m = sdp->sdp_media; m; m = m->m_next) {
        ...
        if ((m->m_type == sdp_media_audio || m->m_type == sdp_media_video) && m->m_port) {
            for (map = m->m_rtpmaps; map; map = map->rm_next) {
                ...
                switch_core_media_add_payload_map(
                    session,
                    m->m_type == sdp_media_audio ? SWITCH_MEDIA_TYPE_AUDIO
                                                 : SWITCH_MEDIA_TYPE_VIDEO,
                    map->rm_encoding, NULL, map->rm_fmtp, sdp_type, map->rm_pt, map->rm_rate, ptime,
                    map->rm_params ? atoi(map->rm_params) : 1, SWITCH_FALSE);
            }
        }
    }
    ...
    for (m = sdp->sdp_media; m; m = m->m_next) {
        ...
        if (m->m_type == sdp_media_image && m->m_port) {
            ...
            if (!connection) {
                switch_log_printf(SWITCH_CHANNEL_CHANNEL_LOG(channel), SWITCH_LOG_ERROR,
                                  "Cannot find a c= line in the sdp at media or session level!\n");
                break;
            }
            ...
            if (switch_channel_direction(channel) == SWITCH_CALL_DIRECTION_INBOUND || prefer_sdp) {
                ...
                for (i = 0; i < num_codecs; i++) {
                    ...
                    if (imp->codec_type != SWITCH_CODEC_TYPE_AUDIO || imp->ianacode > 127 ||
                        already_did[imp->ianacode]) {
                        continue;
                    }
            ...
            if (switch_channel_direction(channel) == SWITCH_CALL_DIRECTION_INBOUND || prefer_sdp) {
                for (map = m->m_rtpmaps; map; map = map->rm_next) {
                    ...
                    for (i = 0; i < num_codecs; i++) {
                        ...
                        if (match) {
                            if (map->rm_fmtp) {
                                switch_snprintf(buf + strlen(buf), sizeof(buf) - strlen(buf),
                                                ",%s.%s~%s", imp->modname, imp->iananame,
                                                map->rm_fmtp);
                            } else {
                                switch_snprintf(buf + strlen(buf), sizeof(buf) - strlen(buf),
                                                ",%s.%s", imp->modname, imp->iananame);
                            }
                ...
                for (i = 0; i < num_codecs; i++) {
                    ...
                    if (imp->codec_type != SWITCH_CODEC_TYPE_VIDEO || imp->ianacode > 127 ||
                        already_did[imp->ianacode]) {
                        continue;
                    }
...
//?
SWITCH_DECLARE(switch_status_t)
switch_core_media_codec_chosen(switch_core_session_t* session, switch_media_type_t type) {
    switch_rtp_engine_t* engine;
    switch_media_handle_t* smh;
...
SWITCH_DECLARE(void)
switch_core_session_stop_media(switch_core_session_t* session) {
    switch_rtp_engine_t *a_engine, *v_engine, *t_engine;
    switch_media_handle_t* smh;
    int type;
    ...
    if (switch_core_codec_ready(&v_engine->read_codec)) {
        type = 1;
        switch_core_codec_control(&v_engine->read_codec, SCC_VIDEO_RESET, SCCT_INT, (void*)&type,
                                  SCCT_NONE, NULL, NULL, NULL);
    }

    if (switch_core_codec_ready(&v_engine->write_codec)) {
        type = 2;
        switch_core_codec_control(&v_engine->write_codec, SCC_VIDEO_RESET, SCCT_INT, (void*)&type,
                                  SCCT_NONE, NULL, NULL, NULL);
    }
...
//?
SWITCH_DECLARE(void)
switch_core_media_check_outgoing_proxy(switch_core_session_t* session,
    ...
    pmap = switch_core_media_add_payload_map(session, SWITCH_MEDIA_TYPE_AUDIO, "PROXY", NULL, NULL,
                                             SDP_ANSWER, 0, 8000, 20, 1, SWITCH_TRUE);
    ...
    if (switch_stristr("m=video", r_sdp)) {
        switch_core_media_choose_port(session, SWITCH_MEDIA_TYPE_VIDEO, 1);
        pmap =
            switch_core_media_add_payload_map(session, SWITCH_MEDIA_TYPE_AUDIO, "PROXY-VID", NULL,
                                              NULL, SDP_ANSWER, 0, 90000, 90000, 1, SWITCH_TRUE);
    ...
    if (switch_stristr("m=text", r_sdp)) {
        switch_core_media_choose_port(session, SWITCH_MEDIA_TYPE_VIDEO, 1);
        pmap =
            switch_core_media_add_payload_map(session, SWITCH_MEDIA_TYPE_AUDIO, "PROXY-TXT", NULL,
                                              NULL, SDP_ANSWER, 0, 1000, 1000, 1, SWITCH_TRUE);
...
SWITCH_DECLARE(void)
switch_core_media_recover_session(switch_core_session_t* session) {
    const char* ip;
    const char* port;
    const char* a_ip;
    const char* r_ip;
    ...
    a_engine->cur_payload_map->iananame = a_engine->cur_payload_map->rm_encoding =
        (char*)switch_channel_get_variable(session->channel, "rtp_use_codec_name");
    a_engine->cur_payload_map->rm_fmtp =
        (char*)switch_channel_get_variable(session->channel, "rtp_use_codec_fmtp");
    ...
    if ((tmp = switch_channel_get_variable(session->channel, "rtp_last_audio_codec_string"))) {
        const char* vtmp =
            switch_channel_get_variable(session->channel, "rtp_last_video_codec_string");
        switch_channel_set_variable_printf(session->channel, "rtp_use_codec_string", "%s%s%s", tmp,
                                           vtmp ? "," : "", vtmp ? vtmp : "");
    }
    ...
    if (switch_channel_test_flag(session->channel, CF_VIDEO)) {
        ...
        v_engine->cur_payload_map->rm_encoding =
            (char*)switch_channel_get_variable(session->channel, "rtp_use_video_codec_name");
        v_engine->cur_payload_map->rm_fmtp =
            (char*)switch_channel_get_variable(session->channel, "rtp_use_video_codec_fmtp");
        v_engine->codec_negotiated = 1;
    ...
    if ((tmp = switch_channel_get_variable(session->channel, "rtp_last_audio_local_crypto_key")) &&
        ...
        a_engine->ssec[a_engine->crypto_type].local_crypto_key =
            switch_core_session_strdup(session, tmp);
        switch_core_media_add_crypto(session, &a_engine->ssec[a_engine->crypto_type],
                                     SWITCH_RTP_CRYPTO_SEND);
        switch_core_media_add_crypto(session, &a_engine->ssec[a_engine->crypto_type],
                                     SWITCH_RTP_CRYPTO_RECV);
        switch_channel_set_flag(smh->session->channel, CF_SECURE);
        ...
        switch_rtp_add_crypto_key(a_engine->rtp_session, SWITCH_RTP_CRYPTO_SEND, idx,
                                  &a_engine->ssec[a_engine->crypto_type]);
...
SWITCH_DECLARE(char*)
switch_core_media_filter_sdp(const char* sdp_str, const char* cmd, const char* arg) {
    char* new_sdp = NULL;
    int pt = -1, te = -1;
    switch_size_t len;
    const char* i;
...
SWITCH_DECLARE(char*)
switch_core_media_process_sdp_filter(const char* sdp, const char* cmd_buf,
                                     switch_core_session_t* session) {
    switch_channel_t* channel = switch_core_session_get_channel(session);
    ...
    for (x = 0; x < argc; x++) {
        ...
        if (zstr(command) || zstr(arg)) {
            ...
            switch_log_printf(
                SWITCH_CHANNEL_CHANNEL_LOG(channel), SWITCH_LOG_DEBUG,
                "%s Filter command %s(%s)\nFROM:\n==========\n%s\nTO:\n==========\n%s\n\n",
                switch_channel_get_name(channel), command, arg, patched_sdp ? patched_sdp : sdp,
                tmp_sdp);
...
SWITCH_DECLARE(switch_timer_t*)
switch_core_media_get_timer(switch_core_session_t* session, switch_media_type_t mtype) {
    switch_rtp_engine_t* engine = NULL;
    switch_media_handle_t* smh = NULL;
...
SWITCH_DECLARE(switch_status_t)
_switch_core_session_request_video_refresh(switch_core_session_t* session, int force,
    ...
    if (switch_channel_media_up(channel) && switch_channel_test_flag(channel, CF_VIDEO)) {
        ...
        if (!force && (smh->last_video_refresh_req &&
                       (now - smh->last_video_refresh_req) < VIDEO_REFRESH_FREQ)) {
            return SWITCH_STATUS_BREAK;
        }
        ...
        switch_log_printf(SWITCH_CHANNEL_ID_LOG, file, func, line,
                          switch_core_session_get_uuid(session), SWITCH_LOG_DEBUG1,
                          "%s Video refresh requested.\n",
                          switch_channel_get_name(session->channel));
        msg.from = __FILE__;
        msg.message_id = SWITCH_MESSAGE_INDICATE_VIDEO_REFRESH_REQ;
        switch_core_session_receive_message(session, &msg);
...
SWITCH_DECLARE(switch_status_t)
switch_core_session_send_and_request_video_refresh(switch_core_session_t* session) {
    if (switch_channel_test_flag(session->channel, CF_VIDEO)) {
        switch_core_session_request_video_refresh(session);
        switch_core_media_gen_key_frame(session);
        return SWITCH_STATUS_SUCCESS;
...
SWITCH_DECLARE(switch_status_t)
switch_core_media_codec_control(switch_core_session_t* session, switch_media_type_t mtype,
                                switch_io_type_t iotype, switch_codec_control_command_t cmd,
                                switch_codec_control_type_t ctype, void* cmd_data,
                                switch_codec_control_type_t atype, void* cmd_arg,
                                switch_codec_control_type_t* rtype, void** ret_data) {
    switch_rtp_engine_t* engine = NULL;
    switch_media_handle_t* smh = NULL;
    ...
    if (codec) {
        return switch_core_codec_control(codec, cmd, ctype, cmd_data, atype, cmd_arg, rtype,
                                         ret_data);
    }
...
SWITCH_DECLARE(switch_bool_t)
switch_core_media_codec_get_cap(switch_core_session_t* session, switch_media_type_t mtype,
...
SWITCH_DECLARE(switch_status_t)
switch_core_session_write_encoded_video_frame(switch_core_session_t* session, switch_frame_t* frame,
                                              switch_io_flag_t flags, int stream_id) {
    switch_io_event_hook_video_write_frame_t* ptr;
    switch_status_t status = SWITCH_STATUS_FALSE;

    if (switch_core_session_media_flow(session, SWITCH_MEDIA_TYPE_VIDEO) ==
            SWITCH_MEDIA_FLOW_RECVONLY ||
        switch_core_session_media_flow(session, SWITCH_MEDIA_TYPE_VIDEO) ==
            SWITCH_MEDIA_FLOW_INACTIVE) {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG3,
                          "Writing video to RECVONLY/INACTIVE session\n");
        return SWITCH_STATUS_SUCCESS;
    }

    if (session->endpoint_interface->io_routines->write_video_frame) {
        if ((status = session->endpoint_interface->io_routines->write_video_frame(
                 session, frame, flags, stream_id)) == SWITCH_STATUS_SUCCESS) {
            for (ptr = session->event_hooks.video_write_frame; ptr; ptr = ptr->next) {
                if ((status = ptr->video_write_frame(session, frame, flags, stream_id)) !=
                    SWITCH_STATUS_SUCCESS) {
                    break;
                }
            }
...
SWITCH_DECLARE(void)
switch_core_session_video_reinit(switch_core_session_t* session) {
    switch_media_handle_t* smh;
    int type;
    ...
    type = 1;
    switch_core_media_codec_control(session, SWITCH_MEDIA_TYPE_VIDEO, SWITCH_IO_READ,
                                    SCC_VIDEO_RESET, SCCT_INT, (void*)&type, SCCT_NONE, NULL, NULL,
                                    NULL);
    switch_core_session_request_video_refresh(session);
}

SWITCH_DECLARE(switch_status_t)
switch_core_session_write_video_frame(switch_core_session_t* session, switch_frame_t* frame,
                                      switch_io_flag_t flags, int stream_id) {
    switch_status_t status = SWITCH_STATUS_FALSE;
    switch_time_t now = switch_micro_time_now();
    ...
    if (!(switch_channel_test_flag(session->channel, CF_VIDEO_READY) ||
          (flags & SWITCH_IO_FLAG_FORCE))) {
        return SWITCH_STATUS_SUCCESS;
    }

    if (smh->write_mutex[SWITCH_MEDIA_TYPE_VIDEO] &&
        switch_mutex_trylock(smh->write_mutex[SWITCH_MEDIA_TYPE_VIDEO]) != SWITCH_STATUS_SUCCESS) {
        /* return CNG, another thread is already writing  */
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG1,
                          "%s is already being written to for %s\n",
                          switch_channel_get_name(session->channel),
                          type2str(SWITCH_MEDIA_TYPE_VIDEO));
        return SWITCH_STATUS_INUSE;
    }
    ...
    if (!smh->video_init && smh->mparams->video_key_first &&
        (now - smh->video_last_key_time) > smh->mparams->video_key_first) {
        switch_core_media_gen_key_frame(session);
    ...
    if (smh->mparams->video_key_freq &&
        (now - smh->video_last_key_time) > smh->mparams->video_key_freq) {
    ...
    if (switch_channel_test_flag(session->channel, CF_VIDEO_READY) &&
        ...
        if (vid_params.width && vid_params.height &&
            ((vid_params.width != img->d_w) || (vid_params.height != img->d_h))) {
    ...
    if (session->bugs) {
        ...
        for (bp = session->bugs; bp; bp = bp->next) {
            ...
            if (bp->ready && img &&
                (switch_test_flag(bp, SMBF_WRITE_VIDEO_PING) ||
                 (switch_core_media_bug_test_flag(bp, SMBF_SPY_VIDEO_STREAM) && !patched))) {
                ...
                if (bp->callback && switch_test_flag(bp, SMBF_WRITE_VIDEO_PING)) {
                    ...
                    if (bp->callback(bp, bp->user_data, SWITCH_ABC_TYPE_WRITE_VIDEO_PING) ==
                            SWITCH_FALSE ||
    ...
    if (!switch_test_flag(frame, SFF_USE_VIDEO_TIMESTAMP)) {
        if (!(timer = switch_core_media_get_timer(session, SWITCH_MEDIA_TYPE_VIDEO))) {
            if (!smh->video_timer.timer_interface) {
                switch_core_timer_init(&smh->video_timer, "soft", 1, 90,
                                       switch_core_session_get_pool(session));
            }
    ...
    do {
        ...
        if (encode_status == SWITCH_STATUS_SUCCESS || encode_status == SWITCH_STATUS_MORE_DATA) {
            ...
            status =
                switch_core_session_write_encoded_video_frame(session, frame, flags, stream_id);
        }
...
SWITCH_DECLARE(switch_status_t)
switch_core_session_wait_for_video_input_params(switch_core_session_t* session,
                                                uint32_t timeout_ms) {
    switch_media_handle_t* smh;
    ...
    while (switch_channel_ready(session->channel) && timeout_ms > 0) {
        ...
        if (video_globals.synced && switch_channel_test_flag(session->channel, CF_VIDEO_READY) &&
            smh->vid_params.width && smh->vid_params.height && smh->vid_params.fps) {
            return SWITCH_STATUS_SUCCESS;
        }
...
SWITCH_DECLARE(switch_bool_t)
switch_core_session_transcoding(switch_core_session_t* session_a, switch_core_session_t* session_b,
                                switch_media_type_t type) {
    ...
    switch (type) {
        case SWITCH_MEDIA_TYPE_AUDIO: {
            ...
            if (read_impl_a.impl_id && read_impl_b.impl_id) {
                transcoding =
                    (read_impl_a.impl_id != read_impl_b.impl_id ||
                     read_impl_a.decoded_bytes_per_packet != read_impl_b.decoded_bytes_per_packet);
            }
        } break;
        case SWITCH_MEDIA_TYPE_VIDEO:
...
SWITCH_DECLARE(void)
switch_core_session_passthru(switch_core_session_t* session, switch_media_type_t type,
                             switch_bool_t on) {
    switch_rtp_engine_t* engine;
...
SWITCH_DECLARE(switch_status_t)
switch_core_session_read_video_frame(switch_core_session_t* session, switch_frame_t** frame,
                                     switch_io_flag_t flags, int stream_id) {
    switch_status_t status = SWITCH_STATUS_FALSE;
...
top:
    ...
    if (session->endpoint_interface->io_routines->read_video_frame) {
        if ((status = session->endpoint_interface->io_routines->read_video_frame(
            ...
            for (ptr = session->event_hooks.video_read_frame; ptr; ptr = ptr->next) {
                if ((status = ptr->video_read_frame(session, frame, flags, stream_id)) !=
                    SWITCH_STATUS_SUCCESS) {
                    break;
                }
            }
    ...
    if (switch_channel_test_flag(session->channel, CF_VIDEO_DEBUG_READ)) {
        if (switch_test_flag((*frame), SFF_CNG)) {
            ...
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO,
                              "VIDEO: seq: %d ts: %u len: %4d %02x %02x %02x %02x %02x %02x %02x "
                              "%02x %02x %02x %02x mark: %d\n",
    ...
    if (switch_channel_test_flag(session->channel, CF_VIDEO_DECODED_READ) &&
        ...
        /* If any media-bugs requested raw video RTP before decode, notify them here.
           Copying is the responsibility of the bug callback if it needs the data beyond
           the callback lifetime. */
        if ((*frame) && ((*frame)->packet || (*frame)->data) &&
            ((*frame)->packetlen > 0 || (*frame)->datalen > 0)) {
            switch_media_bug_t* bp;
            switch_thread_rwlock_rdlock(session->bug_rwlock);
            for (bp = session->bugs; bp; bp = bp->next) {
                if (!bp->ready) continue;
                if (!switch_core_media_bug_test_flag(bp, SMBF_READ_VIDEO_RAW)) continue;
                if (!bp->callback) continue;
                /* Provide the raw frame pointer to the bug and invoke callback */
                // (*frame)->user_data = "nihao";
                bp->video_ping_frame = *frame;
                if (bp->callback(bp, bp->user_data, SWITCH_ABC_TYPE_READ_VIDEO_RAW) ==
                        SWITCH_FALSE ||
                    (bp->stop_time && bp->stop_time <= switch_epoch_time_now(NULL))) {
                    /* mark for prune */
                    switch_set_flag(bp, SMBF_PRUNE);
        ...
        if ((*frame)->img && switch_channel_test_flag(session->channel, CF_VIDEO_DEBUG_READ)) {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "IMAGE %dx%d %dx%d\n",
                              (*frame)->img->w, (*frame)->img->h, (*frame)->img->d_w,
                              (*frame)->img->d_h);
        }
        ...
        if ((*frame)->img && (*frame)->img->d_w && (*frame)->img->d_h) {
            ...
            if ((*frame)->img->d_w != smh->vid_params.width ||
                (*frame)->img->d_h != smh->vid_params.height) {
                new_w = (*frame)->img->d_w;
                new_h = (*frame)->img->d_h;
                ...
                if (new_w && new_h) {
                    ...
                    switch_channel_set_variable_printf(session->channel, "video_width", "%d",
                                                       new_w);
                    switch_channel_set_variable_printf(session->channel, "video_height", "%d",
                                                       new_h);
                    switch_mutex_unlock(smh->control_mutex);
                }
            }
    ...
    if (session->bugs) {
        ...
        for (bp = session->bugs; bp; bp = bp->next) {
            ...
            if (bp->ready && (*frame) && (*frame)->img &&
                (switch_test_flag(bp, SMBF_READ_VIDEO_PING) ||
                 (switch_core_media_bug_test_flag(bp, SMBF_SPY_VIDEO_STREAM) && !patched))) {
                if (bp->callback && switch_test_flag(bp, SMBF_READ_VIDEO_PING)) {
                    bp->video_ping_frame = *frame;

                    if (bp->callback(bp, bp->user_data, SWITCH_ABC_TYPE_READ_VIDEO_PING) ==
                            SWITCH_FALSE ||
                        (bp->stop_time && bp->stop_time <= switch_epoch_time_now(NULL))) {
                        ok = SWITCH_FALSE;
                    }
...
SWITCH_DECLARE(switch_status_t)
switch_core_session_set_video_read_callback(switch_core_session_t* session,
...
SWITCH_DECLARE(switch_status_t)
switch_core_session_video_read_callback(switch_core_session_t* session, switch_frame_t* frame) {
    switch_media_handle_t* smh;
...
SWITCH_DECLARE(switch_status_t)
switch_core_session_set_text_read_callback(switch_core_session_t* session,
...
SWITCH_DECLARE(switch_status_t)
switch_core_session_text_read_callback(switch_core_session_t* session, switch_frame_t* frame) {
    switch_media_handle_t* smh;
    switch_status_t status = SWITCH_STATUS_CONTINUE;
...
SWITCH_DECLARE(switch_status_t)
switch_core_session_read_text_frame(switch_core_session_t* session, switch_frame_t** frame,
                                    switch_io_flag_t flags, int stream_id) {
    switch_status_t status = SWITCH_STATUS_FALSE;
    ...
    if (read_text_frame) {
        if ((status = read_text_frame(session, frame, flags, stream_id)) == SWITCH_STATUS_SUCCESS) {
            for (ptr = session->event_hooks.text_read_frame; ptr; ptr = ptr->next) {
                if ((status = ptr->text_read_frame(session, frame, flags, stream_id)) !=
                    SWITCH_STATUS_SUCCESS) {
                    break;
                }
            }
    ...
    if ((*frame)->data && (*frame)->datalen && !((*frame)->flags & SFF_CNG)) {
        if (!session->text_buffer) {
            switch_mutex_init(&session->text_mutex, SWITCH_MUTEX_NESTED,
                              switch_core_session_get_pool(session));
            ...
            if (bp->ready && switch_test_flag(bp, SMBF_READ_TEXT_STREAM)) {
                ...
                if ((*frame)) {
                    ...
                    if ((*frame)->data && (*frame)->datalen && !((*frame)->flags & SFF_CNG)) {
                        ...
                        switch_buffer_write(bp->text_buffer, (char*)(*frame)->data,
                                            (*frame)->datalen);
                    ...
                    if (zstr(bp->text_framedata) && inuse &&
                        (switch_channel_test_flag(session->channel, CF_TEXT_IDLE) ||
                         switch_test_flag((*frame), SFF_TEXT_LINE_BREAK))) {
    ...
    if (status == SWITCH_STATUS_SUCCESS || status == SWITCH_STATUS_BREAK) {
        if ((switch_channel_test_flag(session->channel, CF_QUEUE_TEXT_EVENTS) ||
             switch_channel_test_flag(session->channel, CF_FIRE_TEXT_EVENTS)) &&
            ...
            if (!switch_channel_test_flag(session->channel, CF_TEXT_LINE_BASED)) {
                ...
                if (switch_channel_test_flag(session->channel, CF_TEXT_IDLE) ||
                    switch_test_flag((*frame), SFF_TEXT_LINE_BREAK)) {
...
SWITCH_DECLARE(switch_status_t)
switch_core_session_write_text_frame(switch_core_session_t* session, switch_frame_t* frame,
                                     switch_io_flag_t flags, int stream_id) {
    switch_status_t status = SWITCH_STATUS_FALSE;
    switch_media_handle_t* smh;
    ...
    if (switch_core_session_media_flow(session, SWITCH_MEDIA_TYPE_TEXT) ==
            SWITCH_MEDIA_FLOW_RECVONLY ||
        switch_core_session_media_flow(session, SWITCH_MEDIA_TYPE_TEXT) ==
            SWITCH_MEDIA_FLOW_INACTIVE) {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG3,
                          "Writing text to RECVONLY/INACTIVE session\n");
        return SWITCH_STATUS_SUCCESS;
    }
    ...
    if (smh->write_mutex[SWITCH_MEDIA_TYPE_TEXT] &&
        switch_mutex_trylock(smh->write_mutex[SWITCH_MEDIA_TYPE_TEXT]) != SWITCH_STATUS_SUCCESS) {
        /* return CNG, another thread is already writing  */
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG1,
                          "%s is already being written to for %s\n",
                          switch_channel_get_name(session->channel),
                          type2str(SWITCH_MEDIA_TYPE_TEXT));
        goto done;
    }
    ...
    if (!is_msrp && switch_channel_test_cap(session->channel, CC_RTP_RTT)) {
        if (!t_engine || !t_engine->tf) {
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
                              "text engine not available for processing\n");
        ...
        if (t_engine->red_pt) {
            ...
            t_engine->tf->red_buflen[t_engine->tf->red_pos] =
                switch_buffer_read(t_engine->tf->write_buffer,
                                   t_engine->tf->red_buf[t_engine->tf->red_pos], RED_PACKET_SIZE);

            *(t_engine->tf->red_buf[t_engine->tf->red_pos] +
              t_engine->tf->red_buflen[t_engine->tf->red_pos]) = '\0';

            build_red_packet(t_engine);
        } else {
            frame->datalen = switch_buffer_read(
                t_engine->tf->write_buffer, t_engine->tf->text_write_frame.data, RED_PACKET_SIZE);
    ...
    if (write_text_frame) {
        if ((status = write_text_frame(session, frame, flags, stream_id)) ==
            SWITCH_STATUS_SUCCESS) {
            for (ptr = session->event_hooks.text_write_frame; ptr; ptr = ptr->next) {
                if ((status = ptr->text_write_frame(session, frame, flags, stream_id)) !=
                    SWITCH_STATUS_SUCCESS) {
                    break;
                }
            }
...
SWITCH_DECLARE(switch_status_t)
switch_core_session_printf(switch_core_session_t* session, const char* fmt, ...) {
    char* data = NULL;
    int ret = 0;
    va_list ap;
    switch_frame_t frame = {0};
...
SWITCH_DECLARE(switch_status_t)
switch_core_session_print(switch_core_session_t* session, const char* data) {
    switch_frame_t frame = {0};
...
SWITCH_DECLARE(switch_msrp_session_t*)
switch_core_media_get_msrp_session(switch_core_session_t* session) {
    if (!session->media_handle) return NULL;
...
// 写音频帧
SWITCH_DECLARE(switch_status_t)
switch_core_session_write_frame(switch_core_session_t* session, switch_frame_t* frame,
                                switch_io_flag_t flags, int stream_id) {
    switch_status_t status = SWITCH_STATUS_FALSE;
    switch_frame_t *enc_frame = NULL, *write_frame = frame;
    unsigned int flag = 0, need_codec = 0, perfect = 0, do_bugs = 0, do_write = 0, do_resample = 0,
                 ptime_mismatch = 0, pass_cng = 0, resample = 0;
    ...
    if (!(frame->codec && frame->codec->implementation)) {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
                          "%s has received a bad frame with no codec!\n",
                          switch_channel_get_name(session->channel));
        switch_channel_hangup(session->channel, SWITCH_CAUSE_DESTINATION_OUT_OF_ORDER);
        switch_mutex_unlock(session->codec_write_mutex);
        return SWITCH_STATUS_FALSE;
    ...
    if (!(switch_core_codec_ready(session->write_codec) && switch_core_codec_ready(frame->codec)))
        goto error;
    ...
    if (frame->codec && session->write_codec->implementation != frame->codec->implementation) {
        if (session->write_impl.codec_id == frame->codec->implementation->codec_id ||
            session->write_impl.microseconds_per_packet !=
                frame->codec->implementation->microseconds_per_packet) {
            ptime_mismatch = TRUE;

            if ((switch_test_flag(frame->codec, SWITCH_CODEC_FLAG_PASSTHROUGH) ||
                 switch_test_flag(session->read_codec, SWITCH_CODEC_FLAG_PASSTHROUGH)) ||
                switch_channel_test_flag(session->channel, CF_PASSTHRU_PTIME_MISMATCH)) {
                status = perform_write(session, frame, flags, stream_id);
                goto error;
            }

            if (session->write_impl.microseconds_per_packet <
                frame->codec->implementation->microseconds_per_packet) {
    ...
    if (switch_test_flag(session, SSF_WRITE_TRANSCODE) && !need_codec &&
        switch_core_codec_ready(session->write_codec)) {
        switch_core_session_t* other_session;
        const char* uuid =
            switch_channel_get_partner_uuid(switch_core_session_get_channel(session));
    ...
    if (frame->codec) {
        ...
        status = switch_core_codec_decode(
            frame->codec, session->write_codec, frame->data, frame->datalen,
            session->write_impl.actual_samples_per_second, session->raw_write_frame.data,
            &session->raw_write_frame.datalen, &session->raw_write_frame.rate, &frame->flags);
        ...
        if (session->write_impl.number_of_channels !=
            frame->codec->implementation->number_of_channels) {
            uint32_t rlen = session->raw_write_frame.datalen / 2 /
                            frame->codec->implementation->number_of_channels;
        ...
        switch (status) {
            case SWITCH_STATUS_RESAMPLE:
                ...
                if (!session->write_resampler) {
                    ...
                    status = switch_resample_create(
                        &session->write_resampler,
            ...
            case SWITCH_STATUS_SUCCESS:
                session->raw_write_frame.samples = session->raw_write_frame.datalen /
                                                   sizeof(int16_t) /
                                                   session->write_impl.number_of_channels;
    ...
    if (session->write_resampler) {
        ...
        switch_mutex_lock(session->resample_mutex);
        if (session->write_resampler) {
            if (switch_resample_calc_buffer_size(
                    session->write_resampler->to_rate, session->write_resampler->from_rate,
                    write_frame->datalen / 2 / session->write_resampler->channels) >
                SWITCH_RECOMMENDED_BUFFER_SIZE) {
                switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_CRIT,
                                  "%s not enough buffer space for required resample operation!\n",
                                  switch_channel_get_name(session->channel));
                switch_channel_hangup(session->channel, SWITCH_CAUSE_DESTINATION_OUT_OF_ORDER);
                switch_mutex_unlock(session->resample_mutex);
                goto error;
        ...
        for (bp = session->bugs; bp; bp = bp->next) {
            ...
            if (switch_test_flag(bp, SMBF_WRITE_REPLACE)) {
                ...
                if (bp->callback) {
                    ...
                    if ((ok = bp->callback(bp, bp->user_data, SWITCH_ABC_TYPE_WRITE_REPLACE)) ==
                        SWITCH_TRUE) {
    ...
    if (session->write_codec) {
        ...
        if (perfect) {
            if (write_frame->datalen < session->write_impl.decoded_bytes_per_packet) {
                memset(write_frame->data, 255,
                       session->write_impl.decoded_bytes_per_packet - write_frame->datalen);
            ...
            status = switch_core_codec_encode(
                session->write_codec, frame->codec, enc_frame->data, enc_frame->datalen,
                session->write_impl.actual_samples_per_second, session->enc_write_frame.data,
            ...
            switch (status) {
                ...
                case SWITCH_STATUS_SUCCESS:
                    ...
                    if (frame->codec->implementation->samples_per_packet !=
                        session->write_impl.samples_per_packet) {
            ...
            if (!session->raw_write_buffer) {
                ...
                switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                                  ...
                                  session->write_impl.decoded_bytes_per_packet);
                if ((status = switch_buffer_create_dynamic(
                         &session->raw_write_buffer, bytes_per_packet * SWITCH_BUFFER_BLOCK_FRAMES,
            ...
            if (!(switch_buffer_write(session->raw_write_buffer, write_frame->data,
                                      write_frame->datalen))) {
                switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
                                  "Write Buffer %u bytes Failed!\n", write_frame->datalen);
            ...
            while (switch_buffer_inuse(session->raw_write_buffer) >=
                   session->write_impl.decoded_bytes_per_packet) {
                int rate;
                ...
                if ((session->raw_write_frame.datalen = (uint32_t)switch_buffer_read(
                         session->raw_write_buffer, session->raw_write_frame.data,
                         session->write_impl.decoded_bytes_per_packet)) == 0) {
                    goto error;
                }
                ...
                switch_assert(session->enc_write_frame.datalen <= SWITCH_RECOMMENDED_BUFFER_SIZE);
                status = switch_core_codec_encode(
                    session->write_codec, frame->codec, enc_frame->data, enc_frame->datalen, rate,
                    session->enc_write_frame.data, &session->enc_write_frame.datalen,
                ...
                if ((status = perform_write(session, write_frame, flags, stream_id)) !=
                    SWITCH_STATUS_SUCCESS) {
                    break;
                }
            }
...
SWITCH_DECLARE(switch_rtp_engine_t*)
switch_core_media_get_engine(switch_core_session_t* session, int media_type) {
    if (!session) return NULL;

    return &session->media_handle->engines[media_type];
}

SWITCH_DECLARE(switch_codec_t*)
switch_core_media_get_codec(switch_core_session_t* session, switch_media_type_t type) {
