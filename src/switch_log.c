...
static const char* LEVELS[] = {"DISABLE", "CONSOLE", "ALERT", "CRIT",  "ERR",
...
COLORS[] = {SWITCH_SEQ_DEFAULT_COLOR, SWITCH_SEQ_FRED,  SWITCH_SEQ_FRED,   SWITCH_SEQ_FRED,
                SWITCH_SEQ_FMAGEN,        SWITCH_SEQ_FCYAN, SWITCH_SEQ_FGREEN, SWITCH_SEQ_FYELLOW};

SWITCH_DECLARE(cJSON*)
switch_log_node_to_json(const switch_log_node_t* node, int log_level,
                        switch_log_json_format_t* json_format, switch_event_t* chan_vars) {
    cJSON* json = NULL;
    char* hostname;
        ...
        if (json_format->custom_field_prefix) {
            ...
            for (field = node->meta->child; field; field = field->next) {
                if (!zstr(field->string)) {
                    char* field_name =
                        switch_mprintf("%s%s", json_format->custom_field_prefix, field->string);
    ...
    if (json_format->version.name && json_format->version.value) {
        cJSON_AddItemToObject(json, json_format->version.name,
                              cJSON_CreateString(json_format->version.value));
    }
    if (json_format->host.name) {
        if (json_format->host.value) {
            cJSON_AddItemToObject(json, json_format->host.name,
                                  cJSON_CreateString(json_format->host.value));
    ...
    if (json_format->ident.name) {
        if (json_format->ident.value) {
            cJSON_AddItemToObject(json, json_format->ident.name,
                                  cJSON_CreateString(json_format->ident.value));
        } else {
    ...
    if (json_format->pid.name) {
        if (json_format->pid.value) {
            cJSON_AddItemToObject(json, json_format->pid.name,
                                  cJSON_CreateNumber(atoi(json_format->pid.value)));
        } else {
    ...
    if (!zstr(node->userdata) && chan_vars && chan_vars->headers &&
    ...
    if (strncmp(full_message, "LOG_FIELDS", 10) == 0) {
        switch_event_create_brackets(full_message + 10, '[', ']', ',', &log_fields,
                                     &parsed_full_message, SWITCH_TRUE);
    ...
    if (log_fields) {
        switch_event_header_t* hp;
        const char* prefix =
            json_format->custom_field_prefix ? json_format->custom_field_prefix : "";
        for (hp = log_fields->headers; hp; hp = hp->next) {
            if (!zstr(hp->name) && !zstr(hp->value)) {
                if (strncmp(hp->name, "@#", 2) == 0) {
                    field_name = switch_mprintf("%s%s", prefix, hp->name + 2);
                    cJSON_AddItemToObject(json, field_name,
                                          cJSON_CreateNumber(strtod(hp->value, NULL)));
                } else {
    ...
    if (json_format->full_message.name) {
        cJSON_AddItemToObject(json, json_format->full_message.name,
                              cJSON_CreateString(full_message));
    } else {
    ...
    if (json_format->short_message.name) {
        ...
        cJSON_AddItemToObject(json, json_format->short_message.name,
                              cJSON_CreateString(short_message));
    }
...
switch_log_bind_logger(switch_log_function_t function, switch_log_level_t level,
...
SWITCH_DECLARE(void)
switch_log_meta_printf(switch_text_channel_t channel, const char* file, const char* func, int line,
                       const char* userdata, switch_log_level_t level, cJSON** meta,
                       const char* fmt, ...) {
    va_list ap;
    va_start(ap, fmt);
...
SWITCH_DECLARE(void)
switch_log_printf(switch_text_channel_t channel, const char* file, const char* func, int line,
...
SWITCH_DECLARE(void)
switch_log_vprintf(switch_text_channel_t channel, const char* file, const char* func, int line,
                   const char* userdata, switch_log_level_t level, const char* fmt, va_list ap) {
    switch_log_meta_vprintf(channel, file, func, line, userdata, level, NULL, fmt, ap);
}
SWITCH_DECLARE(void)
switch_log_meta_vprintf(switch_text_channel_t channel, const char* file, const char* func, int line,
                        const char* userdata, switch_log_level_t level, cJSON** meta,
    ...
    if (channel != SWITCH_CHANNEL_ID_LOG_CLEAN) {
        ...
        switch_snprintf(date, sizeof(date), "%0.4d-%0.2d-%0.2d %0.2d:%0.2d:%0.2d.%0.6d %0.2f%%%%",
                        ...
                        tm.tm_sec, tm.tm_usec, switch_core_idle_cpu());

        // switch_strftime_nocheck(date, &retsize, sizeof(date), "%Y-%m-%d %T", &tm);

#ifdef SWITCH_FUNC_IN_LOG
        len = (uint32_t)(strlen(extra_fmt) + strlen(date) + strlen(filep) + 32 + strlen(funcp) +
                         strlen(fmt));
#else
...
#ifdef SWITCH_FUNC_IN_LOG
        switch_snprintf(new_fmt, len, extra_fmt, date, switch_log_level2str(level), filep, line,
                        funcp, 128, fmt);
#else
        switch_snprintf(new_fmt, len, extra_fmt, date, switch_log_level2str(level), filep, line,
                        128, fmt);
...
SWITCH_DECLARE(switch_status_t)
switch_log_init(switch_memory_pool_t* pool, switch_bool_t colorize) {
