...
#define database_interface_handle_callback_exec(database_interface, dih, sql, callback, pdata,  \
                                                err)                                            \
    database_interface->callback_exec_detailed(__FILE__, (char*)__SWITCH_FUNC__, __LINE__, dih, \
                                               sql, callback, pdata, err)
...
static void add_handle(switch_cache_db_handle_t* dbh, const char* db_str,
                       const char* db_callsite_str, const char* thread_str) {
...
SWITCH_DECLARE(void)
switch_cache_db_database_interface_flush_handles(switch_database_interface_t* database_interface) {
...
top:
    ...
    for (dbh_ptr = sql_manager.handle_pool; dbh_ptr; dbh_ptr = dbh_ptr->next) {
        if (switch_mutex_trylock(dbh_ptr->mutex) == SWITCH_STATUS_SUCCESS) {
            ...
            if (dbh_ptr->native_handle.database_interface_dbh->connection_options
...
static switch_cache_db_handle_t* get_handle(const char* db_str, const char* user_str,
    ...
    /* First loop allows a thread to use a handle multiple times sumiltaneously
       but only if that handle is in use by the same thread. In that case use_count will be
       incremented. This allows SQLite to read and write within a single thread, giving the same
       handle for both operations.
    */
    for (dbh_ptr = sql_manager.handle_pool; dbh_ptr; dbh_ptr = dbh_ptr->next) {
        if (dbh_ptr->thread_hash == thread_hash && dbh_ptr->hash == hash &&
            !switch_test_flag(dbh_ptr, CDF_PRUNE) &&
    ...
    if (!r) {
        ...
        */
        for (dbh_ptr = sql_manager.handle_pool; dbh_ptr; dbh_ptr = dbh_ptr->next) {
            if (dbh_ptr->hash == hash && !dbh_ptr->use_count &&
                !switch_test_flag(dbh_ptr, CDF_PRUNE) &&
                switch_mutex_trylock(dbh_ptr->mutex) == SWITCH_STATUS_SUCCESS) {
                r = dbh_ptr;
...
_switch_core_db_handle(switch_cache_db_handle_t** dbh, const char* file, const char* func,
                       int line) {
    switch_status_t r;
    char* dsn;
    ...
    if ((r = _switch_cache_db_get_db_handle_dsn_ex(dbh, dsn, SWITCH_TRUE, file, func, line)) !=
...
top:
    ...
    for (dbh = sql_manager.handle_pool; dbh; dbh = dbh->next) {
        ...
        if (prune > 0 && (dbh->use_count || switch_test_flag(dbh, CDF_NONEXPIRING) ||
                          (diff < SQL_CACHE_TIMEOUT && !switch_test_flag(dbh, CDF_PRUNE)))) {
            continue;
        }
            ...
            switch (dbh->type) {
                case SCDB_TYPE_DATABASE_INTERFACE: {
                    switch_database_interface_t* database_interface =
                        dbh->native_handle.database_interface_dbh->connection_options
                            .database_interface;
            ...
            if (!prune) {
                if (!sanity) {
                    switch_log_printf(
                        ...
                        "SANITY CHECK FAILED!  Handle %s (%s;%s) was not properly released.\n",
...
switch_cache_db_get_type(switch_cache_db_handle_t* dbh) { return dbh->type; }
    ...
    if (dbh && *dbh) {
        switch ((*dbh)->type) {
            case SCDB_TYPE_DATABASE_INTERFACE: {
                switch_database_interface_t* database_interface =
                    (*dbh)
                        ->native_handle.database_interface_dbh->connection_options
                        .database_interface;
    ...
    if (!dsn) {
        ...
        if (NULL != (colon_slashes = strstr(dsn, "://"))) {
            ...
            if (!strncasecmp(prefix, "odbc", 4)) {
                ...
                status = SWITCH_STATUS_SUCCESS;
            } else if ((database_interface =
                            switch_loadable_module_get_database_interface(prefix, NULL))) {
                status = SWITCH_STATUS_SUCCESS;
...
SWITCH_DECLARE(switch_status_t)
_switch_cache_db_get_db_handle_dsn(switch_cache_db_handle_t** dbh, const char* dsn,
                                   const char* file, const char* func, int line) {
    return _switch_cache_db_get_db_handle_dsn_ex(dbh, dsn, SWITCH_FALSE, file, func, line);
}

SWITCH_DECLARE(switch_status_t)
_switch_cache_db_get_db_handle_dsn_ex(switch_cache_db_handle_t** dbh, const char* dsn,
                                      switch_bool_t make_module_no_unloadable, const char* file,
    ...
    if (NULL != (colon_slashes = strstr(dsn, "://"))) {
        ...
        if ((database_interface = switch_loadable_module_get_database_interface(prefix, NULL))) {
            ...
            connection_options.database_interface_options.make_module_no_unloadable =
    ...
    if (!connection_options.database_interface_options.connection_string) {
        if (!strncasecmp(dsn, "sqlite://", 9)) {
            ...
            if (!strncasecmp(connection_options.core_db_options.db_path, "memory://", 9)) {
                ...
                connection_options.core_db_options.db_path =
                    (char*)(connection_options.core_db_options.db_path + 9);
            }
        } else if ((!(i = strncasecmp(dsn, "odbc://", 7))) ||
                   (strchr(dsn + 2, ':') && !colon_slashes)) {
            type = SCDB_TYPE_ODBC;
...
SWITCH_DECLARE(switch_status_t)
_switch_cache_db_get_db_handle(switch_cache_db_handle_t** dbh, switch_cache_db_handle_type_t type,
    ...
    while (runtime.max_db_handles && sql_manager.total_handles >= runtime.max_db_handles &&
    ...
    if (odbc_user || odbc_pass) {
        snprintf(db_str, sizeof(db_str) - 1, "db=\"%s\";type=\"%s\"user=\"%s\";pass=\"%s\"",
                 db_name, db_type, odbc_user, odbc_pass);
    } else {
    ...
    if ((new_dbh = get_handle(db_str, db_callsite_str, thread_str))) {
        ...
        switch (type) {
            case SCDB_TYPE_DATABASE_INTERFACE: {
                switch_database_interface_t* database_interface =
                    connection_options->database_interface_options.database_interface;

                if (SWITCH_STATUS_SUCCESS !=
                    database_interface->handle_new(connection_options->database_interface_options,
                                                   &database_interface_dbh)) {
                    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_CRIT,
                                      "Failure! Can't create new handle! Can't connect to DSN %s\n",
                                      connection_options->database_interface_options.original_dsn);
                    goto end;
                }

                if (database_interface_dbh) {
                    database_interface_dbh->connection_options =
                        connection_options->database_interface_options;

                    if (connection_options->database_interface_options.make_module_no_unloadable ==
                        SWITCH_TRUE) {
            ...
            case SCDB_TYPE_ODBC: {
                if (!switch_odbc_available()) {
                    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_CRIT,
                                      "Failure! ODBC NOT AVAILABLE! Can't connect to DSN %s\n",
        ...
        if (database_interface_dbh) {
            ...
            if (!(new_dbh->native_handle.core_db_dbh = switch_core_alloc(
                      new_dbh->pool, sizeof(*new_dbh->native_handle.core_db_dbh)))) {
                destroy_handle(&new_dbh);
                switch_core_db_close(db);
                goto end;
            }
            new_dbh->native_handle.core_db_dbh->handle = db;
            new_dbh->native_handle.core_db_dbh->in_memory =
                connection_options->core_db_options.in_memory;
        } else if (odbc_dbh) {
...
static switch_status_t switch_cache_db_execute_sql_real(switch_cache_db_handle_t* dbh,
    ...
    switch (dbh->type) {
        case SCDB_TYPE_DATABASE_INTERFACE: {
            switch_database_interface_t* database_interface =
                dbh->native_handle.database_interface_dbh->connection_options.database_interface;
            type = (char*)dbh->native_handle.database_interface_dbh->connection_options.prefix;
            status = database_interface_handle_exec(
                database_interface, dbh->native_handle.database_interface_dbh, sql, &errmsg);
        } break;
        case SCDB_TYPE_ODBC: {
            type = "ODBC";
        ...
        case SCDB_TYPE_CORE_DB: {
            int ret = switch_core_db_exec(dbh->native_handle.core_db_dbh->handle, sql, NULL, NULL,
                                          &errmsg);
            type = "NATIVE";
    ...
    if (errmsg) {
        if (!switch_stristr("already exists", errmsg) &&
            !switch_stristr("duplicate key name", errmsg)) {
...
/**
   OMFG you cruel bastards.  Who chooses 64k as a max buffer len for a sql statement, have you ever
...
SWITCH_DECLARE(switch_status_t)
switch_cache_db_execute_sql(switch_cache_db_handle_t* dbh, char* sql, char** err) {
    switch_status_t status = SWITCH_STATUS_FALSE;
    ...
    switch (dbh->type) {
        ...
        case SCDB_TYPE_DATABASE_INTERFACE: {
            switch_database_interface_t* database_interface =
                dbh->native_handle.database_interface_dbh->connection_options.database_interface;
            int affected_rows = 0;
            database_interface->affected_rows(dbh->native_handle.database_interface_dbh,
                                              &affected_rows);
...
SWITCH_DECLARE(int)
switch_cache_db_load_extension(switch_cache_db_handle_t* dbh, const char* extension) {
    switch (dbh->type) {
        case SCDB_TYPE_CORE_DB: {
        ...
        case SCDB_TYPE_ODBC: {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
                              "load extension not supported by type ODBC!\n");
        } break;
        case SCDB_TYPE_DATABASE_INTERFACE: {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
                              "load extension not supported by type DATABASE_INTERFACE!\n");
        } break;
    }
    return 0;
}
...
SWITCH_DECLARE(char*)
switch_cache_db_execute_sql2str(switch_cache_db_handle_t* dbh, char* sql, char* str, size_t len,
                                char** err) {
    switch_status_t status = SWITCH_STATUS_FALSE;
    ...
    switch (dbh->type) {
        ...
        case SCDB_TYPE_DATABASE_INTERFACE: {
            switch_database_interface_t* database_interface =
                dbh->native_handle.database_interface_dbh->connection_options.database_interface;
            status = database_interface->exec_string(dbh->native_handle.database_interface_dbh, sql,
                                                     str, len, err);
        } break;
    }
...
SWITCH_DECLARE(switch_status_t)
switch_cache_db_persistant_execute(switch_cache_db_handle_t* dbh, const char* sql,
                                   uint32_t retries) {
    char* errmsg = NULL;
    switch_status_t status = SWITCH_STATUS_FALSE;
    uint8_t forever = 0;
...
SWITCH_DECLARE(switch_status_t)
switch_cache_db_persistant_execute_trans_full(switch_cache_db_handle_t* dbh, char* sql,
                                              uint32_t retries, const char* pre_trans_execute,
...
again:
    ...
    while (begin_retries > 0) {
        ...
        switch (dbh->type) {
            ...
            case SCDB_TYPE_ODBC: {
                ...
                if ((result = switch_odbc_SQLSetAutoCommitAttr(dbh->native_handle.odbc_dbh, 0)) !=
                    SWITCH_ODBC_SUCCESS) {
                    char tmp[100];
                    switch_snprintfv(tmp, sizeof(tmp), "%q-%i", "Unable to Set AutoCommit Off",
                                     result);
                    errmsg = strdup(tmp);
                }
            } break;
            case SCDB_TYPE_DATABASE_INTERFACE: {
                ...
                if ((result = database_interface->sql_set_auto_commit_attr(
                         dbh->native_handle.database_interface_dbh, 0)) != SWITCH_STATUS_SUCCESS) {
                    char tmp[100];
                    switch_snprintfv(tmp, sizeof(tmp), "%q-%i", "Unable to Set AutoCommit Off",
                                     result);
                    errmsg = strdup(tmp);
            ...
            if (again) {
                switch (dbh->type) {
                    ...
                    case SCDB_TYPE_DATABASE_INTERFACE: {
                        ...
                        if ((result = database_interface->commit(
                                 dbh->native_handle.database_interface_dbh)) !=
                            SWITCH_STATUS_SUCCESS) {
                            char tmp[100];
                            switch_snprintfv(tmp, sizeof(tmp), "%q-%i",
    ...
    switch (dbh->type) {
        ...
        case SCDB_TYPE_DATABASE_INTERFACE: {
            ...
            if ((result = database_interface->commit(dbh->native_handle.database_interface_dbh)) !=
                SWITCH_STATUS_SUCCESS) {
                char tmp[100];
...
SWITCH_DECLARE(switch_status_t)
switch_cache_db_execute_sql_event_callback(switch_cache_db_handle_t* dbh, const char* sql,
                                           switch_core_db_event_callback_func_t callback,
                                           void* pdata, char** err) {
    switch_status_t status = SWITCH_STATUS_FALSE;
    char* errmsg = NULL;
    ...
    switch (dbh->type) {
        case SCDB_TYPE_DATABASE_INTERFACE: {
            ...
            if ((status = database_interface_handle_callback_exec(
                     database_interface, dbh->native_handle.database_interface_dbh, sql,
                     helper_callback, &h, err)) != SWITCH_STATUS_SUCCESS) {
                char tmp[100];
                switch_snprintfv(tmp, sizeof(tmp), "%q-%i", "Unable to execute_sql_event_callback",
                                 status);
            }
        } break;
        case SCDB_TYPE_ODBC: {
            status =
                switch_odbc_handle_callback_exec(dbh->native_handle.odbc_dbh, sql, helper_callback,
                                                 &h, err) == SWITCH_ODBC_SUCCESS
                    ? SWITCH_STATUS_SUCCESS
                    : SWITCH_STATUS_FALSE;
        } break;
        case SCDB_TYPE_CORE_DB: {
            int ret = switch_core_db_exec(dbh->native_handle.core_db_dbh->handle, sql,
                                          helper_callback, &h, &errmsg);
...
SWITCH_DECLARE(switch_status_t)
switch_cache_db_execute_sql_event_callback_err(switch_cache_db_handle_t* dbh, const char* sql,
                                               switch_core_db_event_callback_func_t callback,
...
SWITCH_DECLARE(switch_status_t)
switch_cache_db_execute_sql_callback(switch_cache_db_handle_t* dbh, const char* sql,
    ...
    switch (dbh->type) {
        case SCDB_TYPE_DATABASE_INTERFACE: {
            ...
            if ((status = database_interface_handle_callback_exec(
                     database_interface, dbh->native_handle.database_interface_dbh, sql, callback,
                     pdata, err)) != SWITCH_STATUS_SUCCESS) {
                char tmp[100];
                switch_snprintfv(tmp, sizeof(tmp), "%q-%i", "Unable to execute_sql_callback",
                                 status);
            }
        } break;
        case SCDB_TYPE_ODBC: {
            status = switch_odbc_handle_callback_exec(dbh->native_handle.odbc_dbh, sql, callback,
                                                      pdata, err) == SWITCH_ODBC_SUCCESS
                         ? SWITCH_STATUS_SUCCESS
                         : SWITCH_STATUS_FALSE;
        } break;
        case SCDB_TYPE_CORE_DB: {
            int ret = switch_core_db_exec(dbh->native_handle.core_db_dbh->handle, sql, callback,
                                          pdata, &errmsg);
...
SWITCH_DECLARE(switch_status_t)
switch_cache_db_execute_sql_callback_err(switch_cache_db_handle_t* dbh, const char* sql,
    ...
    switch (dbh->type) {
        case SCDB_TYPE_DATABASE_INTERFACE: {
            ...
            if ((status = database_interface_handle_callback_exec(
                     ...
                     pdata, err)) != SWITCH_STATUS_SUCCESS) {
                char tmp[100];
                switch_snprintfv(tmp, sizeof(tmp), "%q-%i", "Unable to execute_sql_callback_err",
                                 status);
            }
...
SWITCH_DECLARE(switch_status_t)
switch_cache_db_create_schema(switch_cache_db_handle_t* dbh, char* sql, char** err) {
...
switch_cache_db_test_reactive(switch_cache_db_handle_t* dbh, const char* test_sql,
                              const char* drop_sql, const char* reactive_sql) {
    return switch_cache_db_test_reactive_ex(dbh, test_sql, drop_sql, reactive_sql, NULL);
}

SWITCH_DECLARE(switch_bool_t)
switch_cache_db_test_reactive_ex(switch_cache_db_handle_t* dbh, const char* test_sql,
                                 const char* drop_sql, const char* reactive_sql,
    ...
    switch (dbh->type) {
        case SCDB_TYPE_DATABASE_INTERFACE: {
            ...
            if ((result = database_interface_handle_exec(
                     database_interface, dbh->native_handle.database_interface_dbh, test_sql,
                     NULL)) != SWITCH_STATUS_SUCCESS) {
                char tmp[100];
                switch_snprintfv(tmp, sizeof(tmp), "%q-%i", "Unable to test_reactive with test_sql",
                                 result);

                if (drop_sql) {
                    if ((result = database_interface_handle_exec(
                             database_interface, dbh->native_handle.database_interface_dbh,
                             drop_sql, NULL)) != SWITCH_STATUS_SUCCESS) {
                        char tmp[100];
                        switch_snprintfv(tmp, sizeof(tmp), "%q-%i",
                ...
                if ((result = database_interface_handle_exec(
                         database_interface, dbh->native_handle.database_interface_dbh,
                         reactive_sql, NULL)) != SWITCH_STATUS_SUCCESS) {
                    char tmp[100];
                    switch_snprintfv(tmp, sizeof(tmp), "%q-%i",
                                     "Unable to test_reactive with reactive_sql", result);

                    if (row_size_limited_reactive_sql &&
                        switch_test_flag(database_interface, SWITCH_DATABASE_FLAG_ROW_SIZE_LIMIT)) {
                        if ((result = database_interface_handle_exec(
                                 database_interface, dbh->native_handle.database_interface_dbh,
                                 row_size_limited_reactive_sql, NULL)) != SWITCH_STATUS_SUCCESS) {
                            switch_snprintfv(
                                tmp, sizeof(tmp), "%q-%i",
                                "Unable to test_reactive with row_size_limited_reactive_sql",
                                result);
                        }
                    }
                }
        ...
        case SCDB_TYPE_ODBC: {
            if (switch_odbc_handle_exec(dbh->native_handle.odbc_dbh, test_sql, NULL, NULL) !=
                SWITCH_ODBC_SUCCESS) {
                if (drop_sql) {
                    switch_odbc_handle_exec(dbh->native_handle.odbc_dbh, drop_sql, NULL, NULL);
                }
                r = switch_odbc_handle_exec(dbh->native_handle.odbc_dbh, reactive_sql, NULL,
                                            NULL) == SWITCH_ODBC_SUCCESS;
            }
        } break;
        case SCDB_TYPE_CORE_DB: {
            char* errmsg = NULL;
            switch_core_db_exec(dbh->native_handle.core_db_dbh->handle, test_sql, NULL, NULL,
                                &errmsg);
            ...
            if (errmsg) {
                ...
                errmsg = NULL;
                if (drop_sql) {
                    switch_core_db_exec(dbh->native_handle.core_db_dbh->handle, drop_sql, NULL,
                                        NULL, &errmsg);
                }
                if (errmsg) {
                    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
                ...
                switch_core_db_exec(dbh->native_handle.core_db_dbh->handle, reactive_sql, NULL,
                                    NULL, &errmsg);
                if (errmsg) {
                    r = SWITCH_FALSE;
...
static void* SWITCH_THREAD_FUNC sql_in_thread(switch_thread_t* thread, void* obj) {
    ...
    if (job->callback && !job->err_callback) {
        ...
        switch_cache_db_execute_sql_callback_err(dbh, job->sql, job->callback, job->err_callback,
                                                 job->pdata, &err);
    } else if (job->event_callback && !job->event_err_callback) {
        switch_cache_db_execute_sql_event_callback(dbh, job->sql, job->event_callback, job->pdata,
                                                   &err);
    } else if (job->event_callback && job->event_err_callback) {
        switch_cache_db_execute_sql_event_callback_err(dbh, job->sql, job->event_callback,
                                                       job->event_err_callback, job->pdata, &err);
    }
...
SWITCH_DECLARE(void)
switch_sql_queue_manager_execute_sql_callback(switch_sql_queue_manager_t* qm, const char* sql,
                                              switch_core_db_callback_func_t callback,
                                              void* pdata) {
    switch_thread_data_t* td;
...
SWITCH_DECLARE(void)
switch_sql_queue_manager_execute_sql_callback_err(switch_sql_queue_manager_t* qm, const char* sql,
                                                  switch_core_db_callback_func_t callback,
                                                  switch_core_db_err_callback_func_t err_callback,
                                                  void* pdata) {
    switch_thread_data_t* td;
...
SWITCH_DECLARE(void)
switch_sql_queue_manager_execute_sql_event_callback(switch_sql_queue_manager_t* qm, const char* sql,
                                                    switch_core_db_event_callback_func_t callback,
                                                    void* pdata) {
    switch_thread_data_t* td;
...
switch_sql_queue_manager_execute_sql_event_callback_err(
    switch_sql_queue_manager_t* qm, const char* sql, switch_core_db_event_callback_func_t callback,
...
SWITCH_DECLARE(void)
switch_sql_queue_manager_pause(switch_sql_queue_manager_t* qm, switch_bool_t flush) {
    uint32_t i;
...
SWITCH_DECLARE(switch_status_t) switch_sql_queue_manager_start(switch_sql_queue_manager_t* qm) {
    ...
    if (!qm->thread_running) {
        ...
        if (switch_thread_create(&qm->thread, thd_attr, switch_user_sql_thread, qm, qm->pool) ==
            SWITCH_STATUS_SUCCESS) {
...
SWITCH_DECLARE(switch_status_t)
switch_sql_queue_manager_push(switch_sql_queue_manager_t* qm, const char* sql, uint32_t pos,
                              switch_bool_t dup) {
    char* sqlptr = NULL;
    switch_status_t status;
    int x = 0;
...
SWITCH_DECLARE(switch_status_t)
switch_sql_queue_manager_push_confirm(switch_sql_queue_manager_t* qm, const char* sql, uint32_t pos,
                                      switch_bool_t dup) {
#define EXEC_NOW
#ifdef EXEC_NOW
    switch_cache_db_handle_t* dbh;
    ...
    while ((qm->written[pos] < want) ||
           (qm->written[pos] >= written && want < written && qm->written[pos] > want)) {
        switch_yield(5000);
...
SWITCH_DECLARE(switch_status_t)
switch_sql_queue_manager_init_name(const char* name, switch_sql_queue_manager_t** qmp,
                                   uint32_t numq, const char* dsn, uint32_t max_trans,
                                   const char* pre_trans_execute, const char* post_trans_execute,
                                   const char* inner_pre_trans_execute,
                                   const char* inner_post_trans_execute) {
    switch_memory_pool_t* pool;
    switch_sql_queue_manager_t* qm;
...
static uint32_t do_trans(switch_sql_queue_manager_t* qm) {
    ...
    switch (qm->event_db->type) {
        ...
        case SCDB_TYPE_ODBC: {
            ...
            if ((result = switch_odbc_SQLSetAutoCommitAttr(qm->event_db->native_handle.odbc_dbh,
                                                           0)) != SWITCH_ODBC_SUCCESS) {
                char tmp[100];
        ...
        case SCDB_TYPE_DATABASE_INTERFACE: {
            switch_database_interface_t* database_interface =
                qm->event_db->native_handle.database_interface_dbh->connection_options
                    .database_interface;
            switch_status_t result;

            if ((result = database_interface->sql_set_auto_commit_attr(
                     qm->event_db->native_handle.database_interface_dbh, 0)) !=
                SWITCH_STATUS_SUCCESS) {
                char tmp[100];
                switch_snprintfv(tmp, sizeof(tmp), "%q-%i", "Unable to Set AutoCommit Off", result);
                errmsg = strdup(tmp);
    ...
    while (qm->max_trans == 0 || ttl <= qm->max_trans) {
        ...
        if (pop) {
            if ((status = switch_cache_db_execute_sql(qm->event_db, (char*)pop, NULL)) ==
                SWITCH_STATUS_SUCCESS) {
                switch_mutex_lock(qm->mutex);
                qm->pre_written[i]++;
                switch_mutex_unlock(qm->mutex);
    ...
    switch (qm->event_db->type) {
        ...
        case SCDB_TYPE_DATABASE_INTERFACE: {
            ...
            if ((result = database_interface->commit(
                     qm->event_db->native_handle.database_interface_dbh)) !=
                SWITCH_STATUS_SUCCESS) {
                char tmp[100];
                switch_snprintfv(tmp, sizeof(tmp), "%q-%i", "Unable to commit transaction", result);
            }
...
static void* SWITCH_THREAD_FUNC switch_user_sql_thread(switch_thread_t* thread, void* obj) {
    ...
    while (sanity && !qm->event_db) {
        if (switch_cache_db_get_db_handle_dsn(&qm->event_db, qm->dsn) == SWITCH_STATUS_SUCCESS &&
            qm->event_db)
            break;
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
                          "%s Error getting db handle, Retrying\n", qm->name);
        switch_yield(500000);
        sanity--;
    }
    ...
    while (qm->thread_running == 1) {
        ...
        if (switch_test_flag((&runtime), SCF_DEBUG_SQL)) {
            ...
            for (i = 0; i < qm->numq; i++) {
                l = strlen(line);
                switch_snprintf(line + l, sizeof(line) - l, "%d%s",
                                switch_queue_size(qm->sql_queue[i]), i == qm->numq - 1 ? "" : "|");
            }
...
#define MAX_SQL 5
#define new_sql()                         \
    switch_assert(sql_idx + 1 < MAX_SQL); \
...
static void core_event_handler(switch_event_t* event) {
    ...
    switch (event->event_id) {
        case SWITCH_EVENT_ADD_SCHEDULE: {
            ...
            if (id) {
                new_sql() = switch_mprintf(
                    "insert into tasks (task_id, task_desc, task_group, task_runtime, "
                    "task_sql_manager, hostname) "
                    "values(%q,'%q','%q',%q,%q,'%q')",
                    id, switch_event_get_header_nil(event, "task-desc"),
                    switch_event_get_header_nil(event, "task-group"),
                    switch_event_get_header_nil(event, "task-runtime"), manager ? manager : "0",
        ...
        case SWITCH_EVENT_RE_SCHEDULE: {
            ...
            if (id) {
                new_sql() = switch_mprintf(
                    "update tasks set task_desc='%q',task_group='%q', task_runtime=%q, "
                    "task_sql_manager=%q where task_id=%q and hostname='%q'",
                    switch_event_get_header_nil(event, "task-desc"),
                    switch_event_get_header_nil(event, "task-group"),
                    switch_event_get_header_nil(event, "task-runtime"), manager ? manager : "0", id,
                    switch_core_get_hostname());
            }
        } break;
        case SWITCH_EVENT_CHANNEL_DESTROY: {
            ...
            if (uuid) {
                new_sql() = switch_mprintf("delete from channels where uuid='%q'", uuid);

                new_sql() = switch_mprintf(
                    "delete from calls where (caller_uuid='%q' or callee_uuid='%q')", uuid, uuid);
            }
        } break;
        case SWITCH_EVENT_CHANNEL_UUID: {
        ...
        case SWITCH_EVENT_CHANNEL_CREATE:
            new_sql() = switch_mprintf(
                "insert into channels (uuid,direction,created,created_epoch, "
                "name,state,callstate,dialplan,context,hostname,initial_cid_name,initial_cid_num,"
                "initial_ip_addr,initial_dest,initial_dialplan,initial_context) "
                "values('%q','%q','%q','%ld','%q','%q','%q','%q','%q','%q','%q','%q','%q','%q','%q'"
                ",'%q')",
                switch_event_get_header_nil(event, "unique-id"),
        ...
        case SWITCH_EVENT_CODEC:
            new_sql() = switch_mprintf(
                "update channels set "
                "read_codec='%q',read_rate='%q',read_bit_rate='%q',write_codec='%q',write_rate='%q'"
                ",write_bit_rate='%q' where uuid='%q'",
        ...
        case SWITCH_EVENT_CHANNEL_ORIGINATE: {
            if ((extra_cols = parse_presence_data_cols(event))) {
                new_sql() = switch_mprintf(
                    "update channels set "
                    "presence_id='%q',presence_data='%q',accountcode='%q',call_uuid='%q',%s where "
                    "uuid='%q'",
                    switch_event_get_header_nil(event, "channel-presence-id"),
                    switch_event_get_header_nil(event, "channel-presence-data"),
                    switch_event_get_header_nil(event, "variable_accountcode"),
                    switch_event_get_header_nil(event, "channel-call-uuid"), extra_cols,
                    switch_event_get_header_nil(event, "unique-id"));
                free(extra_cols);
            } else {
                new_sql() = switch_mprintf(
                    "update channels set "
                    "presence_id='%q',presence_data='%q',accountcode='%q',call_uuid='%q' where "
                    "uuid='%q'",
                    switch_event_get_header_nil(event, "channel-presence-id"),
                    switch_event_get_header_nil(event, "channel-presence-data"),
                    switch_event_get_header_nil(event, "variable_accountcode"),
        ...
        case SWITCH_EVENT_CALL_UPDATE: {
            new_sql() = switch_mprintf(
                "update channels set "
                "callee_name='%q',callee_num='%q',sent_callee_name='%q',sent_callee_num='%q',"
                "callee_direction='%q',"
                ...
                if ((extra_cols = parse_presence_data_cols(event))) {
                    new_sql() =
                        switch_mprintf("update channels set callstate='%q',%s where uuid='%q'",
                                       switch_event_get_header_nil(event, "channel-call-state"),
                                       extra_cols, switch_event_get_header_nil(event, "unique-id"));
                    free(extra_cols);
                } else {
                    new_sql() =
        ...
        case SWITCH_EVENT_CHANNEL_STATE: {
            ...
            switch (state_i) {
                ...
                case CS_HANGUP: /* marked for deprication */
                    new_sql_a() =
                        switch_mprintf("update channels set state='%q' where uuid='%q'",
                                       switch_event_get_header_nil(event, "channel-state"),
                                       switch_event_get_header_nil(event, "unique-id"));
                    break;
#endif
                case CS_EXECUTE:
                    if ((extra_cols = parse_presence_data_cols(event))) {
                        new_sql() = switch_mprintf(
                            "update channels set state='%q',%s where uuid='%q'",
                            switch_event_get_header_nil(event, "channel-state"), extra_cols,
                            switch_event_get_header_nil(event, "unique-id"));
                        free(extra_cols);
                ...
                case CS_ROUTING:
                    if ((extra_cols = parse_presence_data_cols(event))) {
                        new_sql() = switch_mprintf(
                            "update channels set "
                            "state='%q',cid_name='%q',cid_num='%q',callee_name='%q',callee_num='%q'"
                            ","
                            "sent_callee_name='%q',sent_callee_num='%q',"
                            "ip_addr='%q',dest='%q',dialplan='%q',context='%q',presence_id='%q',"
                            "presence_data='%q',accountcode='%q',%s "
                            "where uuid='%q'",
                            switch_event_get_header_nil(event, "channel-state"),
                            switch_event_get_header_nil(event, "caller-caller-id-name"),
                            switch_event_get_header_nil(event, "caller-caller-id-number"),
                            ...
                            switch_event_get_header_nil(event, "channel-presence-id"),
                            switch_event_get_header_nil(event, "channel-presence-data"),
                            switch_event_get_header_nil(event, "variable_accountcode"), extra_cols,
                            switch_event_get_header_nil(event, "unique-id"));
                        free(extra_cols);
                    } else {
                        new_sql() = switch_mprintf(
        ...
        case SWITCH_EVENT_CHANNEL_BRIDGE: {
            ...
            if (uuid && (extra_cols = parse_presence_data_cols(event))) {
                new_sql() =
                    switch_mprintf("update channels set %s where uuid='%q'", extra_cols, uuid);
                switch_safe_free(extra_cols);
            }

            new_sql() = switch_mprintf(
                "update channels set call_uuid='%q' where uuid='%q' or uuid='%q'",
            ...
            new_sql() = switch_mprintf(
                ...
                switch_event_get_header_nil(event, "channel-call-uuid"),
                switch_event_get_header_nil(event, "event-date-local"),
                (long)switch_epoch_time_now(NULL), a_uuid, b_uuid, switch_core_get_switchname());
        } break;
        case SWITCH_EVENT_CHANNEL_UNBRIDGE: {
            ...
            new_sql() = switch_mprintf(
                "delete from calls where (caller_uuid='%q' or callee_uuid='%q')", cuuid, cuuid);
            break;
        }
        case SWITCH_EVENT_SHUTDOWN:
            new_sql() = switch_mprintf(
                ...
                switch_core_get_switchname(), switch_core_get_hostname(),
        ...
        case SWITCH_EVENT_MODULE_LOAD: {
            ...
            if (!zstr(type) && !zstr(name)) {
                new_sql() = switch_mprintf(
                    "insert into interfaces (type,name,description,syntax,ikey,filename,hostname) "
                    "values('%q','%q','%q','%q','%q','%q','%q')",
                    type, name, switch_str_nil(description), switch_str_nil(syntax),
                    switch_str_nil(key), switch_str_nil(filename), switch_core_get_hostname());
            }
            break;
        }
        case SWITCH_EVENT_MODULE_UNLOAD: {
            ...
            if (!zstr(type) && !zstr(name)) {
                new_sql() = switch_mprintf(
                    "delete from interfaces where type='%q' and name='%q' and hostname='%q'", type,
                    name, switch_core_get_hostname());
            }
            break;
        }
        case SWITCH_EVENT_CALL_SECURE: {
            ...
            new_sql() = switch_mprintf("update channels set secure='%q' where uuid='%q'", type,
        ...
        case SWITCH_EVENT_NAT: {
            ...
            if (!strcmp("add", op)) {
                new_sql() = switch_mprintf(
                    ...
                    switch_event_get_header_nil(event, "proto"), sticky,
                    switch_core_get_hostname());
            } else if (!strcmp("del", op)) {
                new_sql() = switch_mprintf(
                    "delete from nat where port=%q and proto=%q and hostname='%q'",
                    switch_event_get_header_nil(event, "port"),
                ...
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
                                  "Unknown op for SWITCH_EVENT_NAT: %s\n", op);
            }
            break;
        }
        default:
    ...
    if (sql_idx) {
        ...
        for (i = 0; i < sql_idx; i++) {
            if (switch_stristr("update channels", sql[i]) ||
                switch_stristr("delete from channels", sql[i])) {
...
static char create_registrations_sql[] =
    ...
    "   realm     VARCHAR(256),\n"
    "   token     VARCHAR(256),\n"
    /* If url is modified please check for code in switch_core_sqldb_start for dependencies for
       MSSQL" */
    "   url      TEXT,\n"
    "   expires  INTEGER,\n"
...
SWITCH_DECLARE(void) switch_core_recovery_flush(const char* technology, const char* profile_name) {
    ...
    if (zstr(technology)) {
        ...
        if (zstr(profile_name)) {
            ...
            sql = switch_mprintf("delete from recovery where technology='%q' and profile_name='%q'",
...
static int recover_callback(void* pArg, int argc, char** argv, char** columnNames) {
    ...
    if (!(session = switch_core_session_request_xml(ep, NULL, xml))) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
                          "Invalid cdr data, call not recovered\n");
        goto end;
    }
    ...
    if (ep->recover_callback) {
        ...
        if (r > 0) {
            if (!switch_channel_test_flag(channel, CF_RECOVERING_BRIDGE)) {
                ...
                if ((extension = switch_caller_extension_new(session, "recovery", "recovery")) ==
                    0) {
                    abort();
                }

                if ((callflow = switch_xml_child(xml, "callflow")) &&
                    (x_extension = switch_xml_child(callflow, "extension"))) {
                    int recovery_skip_announcement_type_applications = switch_channel_var_true(
                        channel, "recovery_skip_announcement_type_applications");
                    for (param = switch_xml_child(x_extension, "application"); param;
                         param = param->next) {
                        ...
                        if (!recovery_skip_announcement_type_applications ||
                            (strcasecmp(var, "speak") && strcasecmp(var, "playback") &&
        ...
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
                          "Endpoint %s has no recovery function\n", argv[0]);
    }
...

{
    ...
    if (!sql_manager.manage) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
                          "DATABASE NOT AVAIALBLE, REVCOVERY NOT POSSIBLE\n");
        return 0;
    }
    ...
    if (zstr(technology)) {
        ...
        if (zstr(profile_name)) {
            sql =
                switch_mprintf("delete from recovery where runtime_uuid!='%q' and technology='%q' ",
                               switch_core_get_uuid(), technology);
        } else {
            sql = switch_mprintf(
                "delete from recovery where runtime_uuid!='%q' and technology='%q' and "
                "profile_name='%q'",
...
SWITCH_DECLARE(void)
switch_core_recovery_untrack(switch_core_session_t* session, switch_bool_t force) {
    char* sql = NULL;
    switch_channel_t* channel = switch_core_session_get_channel(session);
    ...
    if (switch_channel_test_flag(channel, CF_TRACKED) || force) {
        if (force) {
            sql = switch_mprintf("delete from recovery where uuid='%q'",
...
SWITCH_DECLARE(void) switch_core_recovery_track(switch_core_session_t* session) {
    ...
    profile_name =
        switch_channel_get_variable_dup(channel, "recovery_profile_name", SWITCH_FALSE, -1);
    ...
    if (xml_cdr_text) {
        if (switch_channel_test_flag(channel, CF_TRACKED)) {
            sql = switch_mprintf("update recovery set metadata='%q' where uuid='%q'", xml_cdr_text,
                                 switch_core_session_get_uuid(session));
        } else {
            sql = switch_mprintf(
                "insert into recovery (runtime_uuid, technology, profile_name, hostname, uuid, "
                "metadata) "
                "values ('%q','%q','%q','%q','%q','%q')",
                switch_core_get_uuid(), switch_str_nil(technology), switch_str_nil(profile_name),
                switch_core_get_switchname(), switch_core_session_get_uuid(session), xml_cdr_text);
        }
...
SWITCH_DECLARE(switch_status_t)
switch_core_add_registration(const char* user, const char* realm, const char* token,
                             const char* url, uint32_t expires, const char* network_ip,
                             const char* network_port, const char* network_proto,
                             const char* metadata) {
    char* sql;
    ...
    if (runtime.multiple_registrations) {
        ...
        sql = switch_mprintf(
            "delete from registrations where reg_user='%q' and realm='%q' and hostname='%q'", user,
            realm, switch_core_get_switchname());
    }
    ...
    if (!zstr(metadata)) {
        sql = switch_mprintf(
            ...
            switch_str_nil(user), switch_str_nil(realm), switch_str_nil(token), switch_str_nil(url),
            expires, switch_str_nil(network_ip), switch_str_nil(network_port),
            switch_str_nil(network_proto), switch_core_get_switchname(), metadata);
    } else {
        sql = switch_mprintf(
            "insert into registrations "
            "(reg_user,realm,token,url,expires,network_ip,network_port,network_proto,hostname) "
            ...
            switch_str_nil(user), switch_str_nil(realm), switch_str_nil(token), switch_str_nil(url),
            expires, switch_str_nil(network_ip), switch_str_nil(network_port),
            switch_str_nil(network_proto), switch_core_get_switchname());
    }
...
SWITCH_DECLARE(switch_status_t)
switch_core_del_registration(const char* user, const char* realm, const char* token) {
    char* sql;
    ...
    if (!zstr(token) && runtime.multiple_registrations) {
        sql = switch_mprintf(
            "delete from registrations where reg_user='%q' and realm='%q' and hostname='%q' and "
            "token='%q'",
            user, realm, switch_core_get_switchname(), token);
    } else {
        sql = switch_mprintf(
            "delete from registrations where reg_user='%q' and realm='%q' and hostname='%q'", user,
            realm, switch_core_get_switchname());
...
SWITCH_DECLARE(switch_status_t) switch_core_expire_registration(int force) {
    ...
    if (force) {
        sql = switch_mprintf("delete from registrations where hostname='%q'",
                             switch_core_get_switchname());
    } else {
        sql = switch_mprintf(
            "delete from registrations where expires > 0 and expires <= %ld and hostname='%q'", now,
            switch_core_get_switchname());
    }
...
top:
    ...
    if (switch_core_db_handle(&sql_manager.dbh) != SWITCH_STATUS_SUCCESS) {
        ...
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_CRIT,
                          "CORE DATABASE INITIALIZATION FAILURE! CHECK `core-db-dsn`!\n");
    ...
    switch (sql_manager.dbh->type) {
        ...
        case SCDB_TYPE_ODBC:
            if (switch_test_flag((&runtime), SCF_CLEAR_SQL)) {
                ...
                for (i = 0; tables[i]; i++) {
                    switch_snprintfv(sql, sizeof(sql), "delete from %q where hostname='%q'",
                                     tables[i], hostname);
    ...
    switch_cache_db_test_reactive(sql_manager.dbh, "select hostname from aliases",
                                  "DROP TABLE aliases", create_alias_sql);
    switch_cache_db_test_reactive(sql_manager.dbh, "select hostname from complete",
                                  "DROP TABLE complete", create_complete_sql);
    switch_cache_db_test_reactive(sql_manager.dbh, "select hostname from nat", "DROP TABLE nat",
                                  create_nat_sql);
    ...
    switch_cache_db_test_reactive(sql_manager.dbh, "select metadata from registrations", NULL,
                                  "ALTER TABLE registrations ADD COLUMN metadata VARCHAR(256)");

    switch_cache_db_test_reactive(sql_manager.dbh, "select hostname from recovery",
                                  "DROP TABLE recovery", recovery_sql);
    switch_cache_db_create_schema(sql_manager.dbh, "create index recovery1 on recovery(technology)",
                                  NULL);
    switch_cache_db_create_schema(sql_manager.dbh,
                                  "create index recovery2 on recovery(profile_name)", NULL);
    switch_cache_db_create_schema(sql_manager.dbh, "create index recovery3 on recovery(uuid)",
                                  NULL);
    switch_cache_db_create_schema(sql_manager.dbh,
                                  "create index recovery4 on recovery(runtime_uuid)", NULL);
    ...
    switch (sql_manager.dbh->type) {
        ...
        case SCDB_TYPE_ODBC: {
            ...
            switch_cache_db_test_reactive_ex(
                sql_manager.dbh,
                "select call_uuid, read_bit_rate, sent_callee_name, initial_cid_name, "
                "initial_cid_num, initial_ip_addr, initial_dest, initial_dialplan, "
                "initial_context, accountcode from channels",
                "DROP TABLE channels", create_channels_sql, create_row_size_limited_channels_sql);
            switch_cache_db_test_reactive(sql_manager.dbh, "select call_uuid from calls",
                                          "DROP TABLE calls", create_calls_sql);
            switch_cache_db_test_reactive(sql_manager.dbh,
                                          "select * from basic_calls where sent_callee_name=''",
                                          "DROP VIEW basic_calls", basic_calls_sql);
            switch_cache_db_test_reactive(sql_manager.dbh,
                                          "select * from detailed_calls where sent_callee_name=''",
                                          "DROP VIEW detailed_calls", detailed_calls_sql);
            if (runtime.odbc_dbtype == DBTYPE_DEFAULT) {
                switch_cache_db_test_reactive(sql_manager.dbh,
                                              "delete from registrations where reg_user=''",
                                              "DROP TABLE registrations", create_registrations_sql);
            } else {
                char* tmp = switch_string_replace(create_registrations_sql, "url      TEXT",
                                                  "url      VARCHAR(max)");
                switch_cache_db_test_reactive(sql_manager.dbh,
                                              "delete from registrations where reg_user=''",
            ...
            switch_cache_db_test_reactive(sql_manager.dbh, "select ikey from interfaces",
                                          "DROP TABLE interfaces", create_interfaces_sql);
            switch_cache_db_test_reactive(sql_manager.dbh,
                                          "select task_id, task_desc, task_group, task_runtime, "
                                          "task_sql_manager, hostname from tasks",
            ...
            switch (sql_manager.dbh->type) {
                ...
                case SCDB_TYPE_ODBC: {
                    ...
                    if ((result = switch_odbc_SQLSetAutoCommitAttr(
                             sql_manager.dbh->native_handle.odbc_dbh, 0)) != SWITCH_ODBC_SUCCESS) {
                        char tmp[100];
                        switch_snprintfv(tmp, sizeof(tmp), "%q-%i", "Unable to Set AutoCommit Off",
                                         result);
                        err = strdup(tmp);
                ...
                case SCDB_TYPE_DATABASE_INTERFACE: {
                    switch_database_interface_t* database_interface =
                        sql_manager.dbh->native_handle.database_interface_dbh->connection_options
                            .database_interface;
                    switch_status_t result;

                    if ((result = database_interface->sql_set_auto_commit_attr(
                             sql_manager.dbh->native_handle.database_interface_dbh, 0)) !=
                        SWITCH_STATUS_SUCCESS) {
                        char tmp[100];
                        switch_snprintfv(tmp, sizeof(tmp), "%q-%i", "Unable to Set AutoCommit Off",
                                         result);
            ...
            switch_cache_db_execute_sql(sql_manager.dbh, "delete from channels where hostname=''",
                                        &err);
            if (!err) {
                switch_cache_db_execute_sql(sql_manager.dbh,
                                            "delete from channels where hostname=''", &err);
                ...
                switch (sql_manager.dbh->type) {
                    ...
                    case SCDB_TYPE_ODBC: {
                        if (switch_odbc_SQLEndTran(sql_manager.dbh->native_handle.odbc_dbh, 1) !=
                                SWITCH_ODBC_SUCCESS ||
                            switch_odbc_SQLSetAutoCommitAttr(
                                sql_manager.dbh->native_handle.odbc_dbh, 1) !=
                                SWITCH_ODBC_SUCCESS) {
                            char tmp[100];
                            switch_snprintfv(tmp, sizeof(tmp), "%q-%i",
                                             "Unable to commit transaction.", result);
                    ...
                    case SCDB_TYPE_DATABASE_INTERFACE: {
                        switch_database_interface_t* database_interface =
                            sql_manager.dbh->native_handle.database_interface_dbh
                                ->connection_options.database_interface;
                        switch_status_t result;
    ...
    if (switch_test_flag((&runtime), SCF_CLEAR_SQL)) {
        ...
        for (i = 0; tables[i]; i++) {
            switch_snprintfv(sql, sizeof(sql), "delete from %q where sticky=0 and hostname='%q'",
                             tables[i], hostname);
            switch_cache_db_execute_sql(sql_manager.dbh, sql, NULL);
        }
    ...
    switch_cache_db_create_schema(sql_manager.dbh,
                                  "create index tasks1 on tasks (hostname,task_id)", NULL);
    switch_cache_db_create_schema(sql_manager.dbh,
                                  "create index complete1 on complete (a1,hostname)", NULL);
    switch_cache_db_create_schema(sql_manager.dbh,
                                  "create index complete2 on complete (a2,hostname)", NULL);
    switch_cache_db_create_schema(sql_manager.dbh,
                                  "create index complete3 on complete (a3,hostname)", NULL);
    switch_cache_db_create_schema(sql_manager.dbh,
                                  "create index complete4 on complete (a4,hostname)", NULL);
    switch_cache_db_create_schema(sql_manager.dbh,
                                  "create index complete5 on complete (a5,hostname)", NULL);
    switch_cache_db_create_schema(sql_manager.dbh,
                                  "create index complete6 on complete (a6,hostname)", NULL);
    switch_cache_db_create_schema(sql_manager.dbh,
                                  "create index complete7 on complete (a7,hostname)", NULL);
    switch_cache_db_create_schema(sql_manager.dbh,
                                  "create index complete8 on complete (a8,hostname)", NULL);
    switch_cache_db_create_schema(sql_manager.dbh,
                                  "create index complete9 on complete (a9,hostname)", NULL);
    switch_cache_db_create_schema(sql_manager.dbh,
                                  "create index complete10 on complete (a10,hostname)", NULL);
    switch_cache_db_create_schema(
        sql_manager.dbh,
        "create index complete11 on complete (a1,a2,a3,a4,a5,a6,a7,a8,a9,a10,hostname)", NULL);
    switch_cache_db_create_schema(
        sql_manager.dbh, "create index nat_map_port_proto on nat (port,proto,hostname)", NULL);
    switch_cache_db_create_schema(sql_manager.dbh, "create index chidx1 on channels (hostname)",
                                  NULL);
    switch_cache_db_create_schema(sql_manager.dbh,
                                  "create index uuindex on channels (uuid, hostname)", NULL);
    switch_cache_db_create_schema(sql_manager.dbh, "create index uuindex2 on channels (call_uuid)",
                                  NULL);
    switch_cache_db_create_schema(sql_manager.dbh, "create index callsidx1 on calls (hostname)",
                                  NULL);
    switch_cache_db_create_schema(sql_manager.dbh,
                                  "create index eruuindex on calls (caller_uuid, hostname)", NULL);
    switch_cache_db_create_schema(sql_manager.dbh, "create index eeuuindex on calls (callee_uuid)",
                                  NULL);
    switch_cache_db_create_schema(sql_manager.dbh, "create index eeuuindex2 on calls (call_uuid)",
                                  NULL);
    switch_cache_db_create_schema(
        sql_manager.dbh, "create index regindex1 on registrations (reg_user,realm,hostname)", NULL);
...
skip:
    ...
    if (sql_manager.manage) {
        ...
        switch_thread_create(&sql_manager.db_thread, thd_attr, switch_core_sql_db_thread, NULL,
        ...
        switch_event_bind("core_db", SWITCH_EVENT_ALL, SWITCH_EVENT_SUBCLASS_ANY,
                          core_event_handler, NULL);
#else
        switch_event_bind("core_db", SWITCH_EVENT_ADD_SCHEDULE, SWITCH_EVENT_SUBCLASS_ANY,
                          core_event_handler, NULL);
        switch_event_bind("core_db", SWITCH_EVENT_DEL_SCHEDULE, SWITCH_EVENT_SUBCLASS_ANY,
                          core_event_handler, NULL);
        switch_event_bind("core_db", SWITCH_EVENT_EXE_SCHEDULE, SWITCH_EVENT_SUBCLASS_ANY,
                          core_event_handler, NULL);
        switch_event_bind("core_db", SWITCH_EVENT_RE_SCHEDULE, SWITCH_EVENT_SUBCLASS_ANY,
                          core_event_handler, NULL);
        switch_event_bind("core_db", SWITCH_EVENT_CHANNEL_DESTROY, SWITCH_EVENT_SUBCLASS_ANY,
                          core_event_handler, NULL);
        switch_event_bind("core_db", SWITCH_EVENT_CHANNEL_UUID, SWITCH_EVENT_SUBCLASS_ANY,
                          core_event_handler, NULL);
        switch_event_bind("core_db", SWITCH_EVENT_CHANNEL_CREATE, SWITCH_EVENT_SUBCLASS_ANY,
                          core_event_handler, NULL);
        switch_event_bind("core_db", SWITCH_EVENT_CHANNEL_ANSWER, SWITCH_EVENT_SUBCLASS_ANY,
                          core_event_handler, NULL);
        switch_event_bind("core_db", SWITCH_EVENT_CHANNEL_PROGRESS_MEDIA, SWITCH_EVENT_SUBCLASS_ANY,
                          core_event_handler, NULL);
        switch_event_bind("core_db", SWITCH_EVENT_CHANNEL_HOLD, SWITCH_EVENT_SUBCLASS_ANY,
                          core_event_handler, NULL);
        switch_event_bind("core_db", SWITCH_EVENT_CHANNEL_UNHOLD, SWITCH_EVENT_SUBCLASS_ANY,
                          core_event_handler, NULL);
        switch_event_bind("core_db", SWITCH_EVENT_CHANNEL_EXECUTE, SWITCH_EVENT_SUBCLASS_ANY,
                          core_event_handler, NULL);
        switch_event_bind("core_db", SWITCH_EVENT_CHANNEL_ORIGINATE, SWITCH_EVENT_SUBCLASS_ANY,
                          core_event_handler, NULL);
        switch_event_bind("core_db", SWITCH_EVENT_CALL_UPDATE, SWITCH_EVENT_SUBCLASS_ANY,
                          core_event_handler, NULL);
        switch_event_bind("core_db", SWITCH_EVENT_CHANNEL_CALLSTATE, SWITCH_EVENT_SUBCLASS_ANY,
                          core_event_handler, NULL);
        switch_event_bind("core_db", SWITCH_EVENT_CHANNEL_STATE, SWITCH_EVENT_SUBCLASS_ANY,
                          core_event_handler, NULL);
        switch_event_bind("core_db", SWITCH_EVENT_CHANNEL_BRIDGE, SWITCH_EVENT_SUBCLASS_ANY,
                          core_event_handler, NULL);
        switch_event_bind("core_db", SWITCH_EVENT_CHANNEL_UNBRIDGE, SWITCH_EVENT_SUBCLASS_ANY,
                          core_event_handler, NULL);
        switch_event_bind("core_db", SWITCH_EVENT_SHUTDOWN, SWITCH_EVENT_SUBCLASS_ANY,
                          core_event_handler, NULL);
        switch_event_bind("core_db", SWITCH_EVENT_LOG, SWITCH_EVENT_SUBCLASS_ANY,
                          core_event_handler, NULL);
        switch_event_bind("core_db", SWITCH_EVENT_MODULE_LOAD, SWITCH_EVENT_SUBCLASS_ANY,
                          core_event_handler, NULL);
        switch_event_bind("core_db", SWITCH_EVENT_MODULE_UNLOAD, SWITCH_EVENT_SUBCLASS_ANY,
                          core_event_handler, NULL);
        switch_event_bind("core_db", SWITCH_EVENT_CALL_SECURE, SWITCH_EVENT_SUBCLASS_ANY,
                          core_event_handler, NULL);
        switch_event_bind("core_db", SWITCH_EVENT_NAT, SWITCH_EVENT_SUBCLASS_ANY,
                          core_event_handler, NULL);
        switch_event_bind("core_db", SWITCH_EVENT_CODEC, SWITCH_EVENT_SUBCLASS_ANY,
                          core_event_handler, NULL);
#endif
    }
    ...
    if (sql_manager.manage) {
        if (!sql_manager.qm) {
            ...
            switch_sql_queue_manager_init_name(
                "CORE", &sql_manager.qm, 4, dbname, SWITCH_MAX_TRANS,
                runtime.core_db_pre_trans_execute, runtime.core_db_post_trans_execute,
                runtime.core_db_inner_pre_trans_execute, runtime.core_db_inner_post_trans_execute);
        }
...
void switch_core_sqldb_stop(void) {
    ...
    char* sql = NULL;
    const char* hostname = switch_core_get_switchname();
    /* Clean up registrations for this host before shutdown
     * Only needed for external databases (ODBC/PostgreSQL), not for in-memory SQLite
     */
    if (sql_manager.qm && sql_manager.qm->event_db &&
        sql_manager.qm->event_db->type != SCDB_TYPE_CORE_DB) {
        const char* realm = switch_core_get_variable("domain");

        if (zstr(realm)) {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO,
                              "Cleaning up registrations for host [%s]\n", hostname);
            sql = switch_mprintf("delete from registrations where hostname='%q'", hostname);
        } else {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO,
                              "Cleaning up registrations for host [%s] realm [%s]\n", hostname,
                              realm);
            sql = switch_mprintf("delete from registrations where realm='%q' and hostname='%q'",
                                 realm, hostname);
        }
        ...
        // sql = switch_mprintf("delete from channels where hostname='%q'", hostname);
        // if (sql) {
        //     switch_sql_queue_manager_push(sql_manager.qm, sql, 0, SWITCH_FALSE);
        // }

        // sql = switch_mprintf("delete from calls where hostname='%q'", hostname);
        // if (sql) {
        //     switch_sql_queue_manager_push(sql_manager.qm, sql, 0, SWITCH_FALSE);
        // }

        /* Give the queue a moment to process the cleanup queries */
        switch_yield(100000);
    }
#endif
...
SWITCH_DECLARE(void) switch_cache_db_status(switch_stream_handle_t* stream) {
    ...
    for (dbh = sql_manager.handle_pool; dbh; dbh = dbh->next) {
        ...
        stream->write_function(
            ...
            "%s\n\tType: %s\n\tLast used: %d\n\tTotal used: %ld\n\tFlags: %s, %s(%d)%s\n"
            "\tCreator: %s\n\tLast User: %s\n",
            cleankey_str, switch_cache_db_type_name(dbh->type), diff, dbh->total_used_count,
            locked ? "Locked" : "Unlocked", dbh->use_count ? "Attached" : "Detached",
            dbh->use_count, switch_test_flag(dbh, CDF_NONEXPIRING) ? ", Non-expiring" : "",
            dbh->creator, dbh->last_user);
    }
...
SWITCH_DECLARE(char*) switch_sql_concat(void) {
    if (runtime.odbc_dbtype == DBTYPE_MSSQL) return "+";
