...
/*	http://tools.ietf.org/html/draft-ietf-payload-vp8-10
        ...
        The first octets after the RTP header are the VP8 payload descriptor, with the following
structure. #endif
...
static switch_status_t init_decoder(switch_codec_t* codec) {
    ...
    if (context->flags & SWITCH_CODEC_FLAG_DECODE && !context->decoder_init) {
        ...
        if ((err = vpx_codec_dec_init(&context->decoder, context->decoder_interface, &cfg,
                                      dec_flags)) != VPX_CODEC_OK) {
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(codec->session), SWITCH_LOG_ERROR,
                              "VPX decoder %s codec init error: [%d:%s:%s]\n",
                              vpx_codec_iface_name(context->decoder_interface), err,
                              vpx_codec_error(&context->decoder),
                              vpx_codec_error_detail(&context->decoder));
    ...
    if (profiles) {
        ...
        for (hp = my_cfg->codecs->headers; hp; hp = hp->next) {
            ...
            if (!strcmp(hp->name, codec_name)) {
                ...
                for (profile = switch_xml_child(profiles, "profile"); profile;
...
static switch_status_t init_encoder(switch_codec_t* codec) {
    ...
    if (context->codec_settings.video.bandwidth) {
        ...
        context->bandwidth = switch_calc_bitrate(context->codec_settings.video.width,
                                                 context->codec_settings.video.height, 1, 15);
    }
    ...
    if (context->bandwidth > vpx_globals.max_bitrate) {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(codec->session), SWITCH_LOG_WARNING,
                          "REQUESTED BITRATE TRUNCATED FROM %d TO %d\n", context->bandwidth,
                          vpx_globals.max_bitrate);
    ...
    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(codec->session), SWITCH_LOG_NOTICE,
                      ...
                      config->g_h, config->rc_target_bitrate, context->codec_settings.video.width,
                      context->codec_settings.video.height, context->bandwidth);
    ...
    if (context->encoder_init) {
        if ((err = vpx_codec_enc_config_set(&context->encoder, config)) != VPX_CODEC_OK) {
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(codec->session), SWITCH_LOG_ERROR,
                              ...
                              vpx_codec_iface_name(context->encoder_interface), err,
                              vpx_codec_error(&context->encoder),
                              vpx_codec_error_detail(&context->encoder));
            return SWITCH_STATUS_FALSE;
        }
        ...
        if (vpx_globals.debug) {
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(codec->session), SWITCH_LOG_INFO,
                              "VPX encoder %s settings:\n",
                              vpx_codec_iface_name(context->encoder_interface));
            show_config(my_cfg, config);
        }

        if ((err = vpx_codec_enc_init(&context->encoder, context->encoder_interface, config,
                                      0 & VPX_CODEC_USE_OUTPUT_PARTITION)) != VPX_CODEC_OK) {
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(codec->session), SWITCH_LOG_ERROR,
        ...
        if (context->is_vp9) {
            ...
            vpx_codec_control(&context->encoder, VP8E_SET_NOISE_SENSITIVITY,
                              my_cfg->noise_sensitivity);

            if (my_cfg->max_intra_bitrate_pct) {
                vpx_codec_control(&context->encoder, VP8E_SET_MAX_INTRA_BITRATE_PCT,
                                  my_cfg->max_intra_bitrate_pct);
            }
        }
    }
...
static switch_status_t switch_vpx_init(switch_codec_t* codec, switch_codec_flag_t flags,
    ...
    if (!(encoding || decoding) ||
        ((context = switch_core_alloc(codec->memory_pool, sizeof(*context))) == 0)) {
        return SWITCH_STATUS_FALSE;
    }
    ...
    if (!context->is_vp9) {
        context->picture_id =
            13;  // picture Id may start from random value and must be incremented on each frame
    } else {
...
static switch_status_t consume_partition(vpx_context_t* context, switch_frame_t* frame) {
    ...
    if (!context->pkt) {
        if ((context->pkt = vpx_codec_get_cx_data(&context->encoder, &context->enc_iter))) {
            start = 1;
            if (!context->pbuffer) {
                switch_buffer_create_partition(context->pool, &context->pbuffer,
                                               context->pkt->data.frame.buf,
                                               context->pkt->data.frame.sz);
            } else {
                switch_buffer_set_partition_data(context->pbuffer, context->pkt->data.frame.buf,
                                                 context->pkt->data.frame.sz);
            }
        }
    }
    ...
    *(uint8_t*)frame->data = 0;
    ...
    if (context->is_vp9) {
        ...
        hdrlen = 4; /* Send VP8 with 4 byte extended header, includes 1 byte REQUIRED header, 1 byte
                       X header and 2 bytes of I header with picture_id. */
    }
    ...
    /*
            Try to split payload to packets evenly(with largest at the end) up to
       vpx_globals.rtp_slice_size, (assume hdrlen constant across all packets of the same picture).
...
static switch_status_t switch_vpx_encode(switch_codec_t* codec, switch_frame_t* frame) {
    ...
    if (context->codec_settings.video.width != width ||
    ...
    if (context->need_key_frame > 0) {
        ...
        if (!context->last_key_frame ||
            (now - context->last_key_frame) > vpx_globals.key_frame_min_freq) {
    ...
    if ((err = vpx_codec_encode(&context->encoder, (vpx_image_t*)frame->img, pts, dur, vpx_flags,
                                VPX_DL_REALTIME)) != VPX_CODEC_OK) {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(codec->session), SWITCH_LOG_ERROR,
                          "VPX encode error [%d:%s:%s]\n", err, vpx_codec_error(&context->encoder),
                          vpx_codec_error_detail(&context->encoder));
        frame->datalen = 0;
        return SWITCH_STATUS_FALSE;
    }
...
// 					  "VIDEO VPX: seq: %d ts: %u len: %u %02x %02x %02x %02x
// %02x %02x %02x %02x %02x %02x %02x mark: %d\n",
...
// 			data++;
// 			if (M) {
// 				//switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "M BIT
// SET\n"); 				data++;
// 			}
// 		}
// 		if (X & 0x40) {
...
// 	//switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "POST PARSE: DATA LEN %d KEY %d
// KEYBYTE = %0x\n", len, key, *data);
...
// 		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG2, "wrong timestamp %u, expect
...
static switch_status_t buffer_vp8_packets(vpx_context_t* context, switch_frame_t* frame) {
    ...
    if (context->debug > 0) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
                          "buffer_vp8_packets: datalen=%d m=%d ts=%u seq=%d\n", frame->datalen,
                          frame->m, frame->timestamp, frame->seq);
    }
    ...
    /* VP8 packet loss detection - same as VP9 */
    if (context->last_received_seq && context->last_received_seq + 1 != frame->seq) {
        switch_log_printf(SWITCH_CHANNEL_LOG, VPX_SWITCH_LOG_LEVEL,
                          "VP8 Packet loss detected last=%d got=%d lost=%d\n",
                          context->last_received_seq, frame->seq,
                          frame->seq - context->last_received_seq);
        /* Drop any partially assembled frame to avoid decoding truncated data */
        switch_buffer_zero(context->vpx_packet_buffer);
        context->got_start_frame = 0;
    }
    context->last_received_seq = frame->seq;
    ...
    X = (DES & 0x80) >> 7; /* Extension bit */
    S = (DES & 0x10) >> 4; /* Start of VP8 partition */
    PID = DES & 0x07;      /* Partition Index (RFC 7741: PID uses bits 0-2, max value 7) */

    if (context->debug > 0) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
                          "VP8: DES=0x%02x X=%d S=%d PID=%d datalen=%d\n", DES, X, S, PID,
                          frame->datalen);
    }
    ...
    if (X) {
        if (frame->datalen < 2) {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG1,
                              "VP8: Incomplete extension header\n");
            return SWITCH_STATUS_MORE_DATA;
        }
        ...
        I = (ext & 0x80) >> 7; /* PictureID present */
        L = (ext & 0x40) >> 6; /* TL0PICIDX present */
        T = (ext & 0x20) >> 5; /* TID present */
        K = (ext & 0x10) >> 4; /* KEYIDX present */

        /* Skip PictureID */
        if (I) {
            if (*data & 0x80) {
                /* M bit set - 2 byte PictureID */
                data += 2;
            } else {
                /* 1 byte PictureID */
                data++;
    ...
    if (len <= 0) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG1,
                          "VP8: No payload data after header parsing\n");
        return SWITCH_STATUS_MORE_DATA;
    }

    /*
     * If buffer is empty, we must start with a valid First Partition (S=1, PID=0).
     * Any other packet (Continuation or subsequent Partition) is an orphan and should be dropped.
     * This handles cases where we cleared the buffer due to packet loss, or joined mid-stream.
     */
    if (switch_buffer_inuse(context->vpx_packet_buffer) == 0 && !(S && PID == 0)) {
        if (context->debug > 0 || (frame->m)) {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG1,
                              "VP8: Dropping orphan packet (S=%d, PID=%d, Seq=%d, Len=%d)\n", S,
                              PID, frame->seq, len);
        }
        return SWITCH_STATUS_MORE_DATA;
    }
    ...
    /*
     * FIX for VP8 Partitioning (Issue #1818):
     * Only reset buffer and check for keyframe when:
     * - S bit is set (start of partition)
     * - PID is 0 (first partition contains the uncompressed data chunk with keyframe flag)
     *
     * When PID > 0, it's a continuation partition and should be appended to buffer
     * without resetting or checking for keyframe.
     */
    if (S && PID == 0) {
        /* Start of first partition - reset buffer and check for keyframe */

        /* Check for timestamp discontinuity */
        if (context->last_received_timestamp &&
            context->last_received_timestamp != frame->timestamp) {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG2,
                              "VP8: Timestamp changed %u -> %u, resetting buffer\n",
                              context->last_received_timestamp, frame->timestamp);
        }
        ...
        /*
         * VP8 Uncompressed Data Chunk (first byte of first partition):
         * Bit 0: frame_type (0=keyframe, 1=interframe)
         *
         * So keyframe when (first_byte & 0x01) == 0
         */
        key = (*data & 0x01) == 0;
        ...
        if (key) {
            ...
            context->no_key_frame = 0;
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG1, "VP8: Keyframe detected\n");
        }

        if (context->debug > 0) {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
                              "VP8: First partition start - key=%d got_key=%d len=%d\n", key,
                              context->got_key_frame, len);
        }
    } else if (S && PID > 0) {
        /*
         * Start of a subsequent partition (PID > 0)
         * This is a continuation partition, just append data
         * Don't reset buffer or check for keyframe
         */
        if (context->debug > 0) {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
                              "VP8: Partition %d start, appending data len=%d\n", PID, len);
        }

        /* Verify timestamp matches current frame */
        if (context->last_received_timestamp != frame->timestamp) {
            switch_log_printf(
                SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG2,
                "VP8: Partition %d timestamp mismatch %u != %u, possible packet loss\n", PID,
                context->last_received_timestamp, frame->timestamp);
            switch_buffer_zero(context->vpx_packet_buffer);
            context->last_received_timestamp = frame->timestamp;
            return SWITCH_STATUS_RESTART;
        ...
        /*
         * Continuation packet (S=0) - middle of a partition
         * Verify we're still on the same frame
         */
        if (context->last_received_timestamp != frame->timestamp) {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG2,
                              "VP8: Continuation packet timestamp mismatch %u != %u\n",
                              context->last_received_timestamp, frame->timestamp);
            switch_buffer_zero(context->vpx_packet_buffer);
            return SWITCH_STATUS_RESTART;
        }
    ...
    /* Don't buffer data until we have a keyframe */
    if (!context->got_key_frame) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG1,
                          "VP8: Waiting for keyframe, got_key_frame=%d\n", context->got_key_frame);

        if (++context->no_key_frame > 50) {
            context->no_key_frame = 0;
            return SWITCH_STATUS_RESTART; /* Request keyframe */
        }
        return SWITCH_STATUS_MORE_DATA;
    }

    /* Append payload data to buffer */
    switch_buffer_write(context->vpx_packet_buffer, data, len);

    if (context->debug > 0) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
                          "VP8: Buffered %d bytes, total=%ld, marker=%d\n", len,
                          (long)switch_buffer_inuse(context->vpx_packet_buffer), frame->m);
    }

    /* Return success only when we have a complete frame (marker bit set) */
    if (frame->m) {
        return SWITCH_STATUS_SUCCESS;
    }
...
static switch_status_t buffer_vp9_packets(vpx_context_t* context, switch_frame_t* frame) {
    ...
    if (context->debug > 0) {
        switch_log_printf(SWITCH_CHANNEL_LOG, frame->m ? SWITCH_LOG_ERROR : SWITCH_LOG_INFO,
                          ...
                          *data, *(data + 1), *(data + 2), *(data + 3), frame->m, frame->datalen,
                          frame->seq, frame->timestamp, frame->ssrc, desc->have_pid,
                          desc->have_p_layer, desc->have_layer_ind, desc->is_flexible, desc->start,
                          desc->end, desc->have_ss, desc->zero);
    }
    ...
    if (desc->have_layer_ind) {
        ...
        switch_log_printf(
            ...
            "temporal_id=%d temporal_up_switch=%d spatial_id=%d inter_layer_predicted=%d\n",
            layer->temporal_id, layer->temporal_up_switch, layer->spatial_id,
            layer->inter_layer_predicted);
#endif
    ...
    if (vp9 - data >= frame->datalen) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG1,
                          "Invalid VP9 Packet %" SWITCH_SSIZE_T_FMT " > %d\n", vp9 - data,
                          frame->datalen);
        switch_buffer_zero(context->vpx_packet_buffer);
        goto end;
    }
...
#ifdef DEBUG_VP9
    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO,
                      "buffered %d bytes, buffer size: %" SWITCH_SIZE_T_FMT "\n", len,
                      switch_buffer_inuse(context->vpx_packet_buffer));
#endif
...
static switch_status_t switch_vpx_decode(switch_codec_t* codec, switch_frame_t* frame) {
    ...
    if (context->debug > 0 && context->debug < 4) {
        ...
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
                          ...
                          *(data + 1), *(data + 2), *(data + 3), frame->m, desc->start, desc->end,
    ...
    if (!is_keyframe && context->got_key_frame <= 0) {
        ...
        if (context->no_key_frame > 50) {
            if ((is_keyframe = is_start)) {
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG1,
                                  "no keyframe, treating start as key. frames=%d\n",
                                  context->no_key_frame);
            }
        }
    }
    ...
    if (!context->decoder_init) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
                          "VPX decoder is not initialized!\n");
        return SWITCH_STATUS_FALSE;
    }
    ...
    status =
        context->is_vp9 ? buffer_vp9_packets(context, frame) : buffer_vp8_packets(context, frame);
    ...
    // switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "====READ buf:%ld got_key:%d st:%d
    // m:%d\n", switch_buffer_inuse(context->vpx_packet_buffer), context->got_key_frame, status,
    // frame->m);
    ...
    if (status == SWITCH_STATUS_SUCCESS && frame->m && len) {
        ...
        if (err != VPX_CODEC_OK) {
            switch_log_printf(
                SWITCH_CHANNEL_SESSION_LOG(codec->session),
                (context->decoded_first_frame ? SWITCH_LOG_ERROR : VPX_SWITCH_LOG_LEVEL),
                "VPX error decoding %" SWITCH_SIZE_T_FMT " bytes, [%d:%s:%s]\n", len, err,
                vpx_codec_error(decoder), vpx_codec_error_detail(decoder));
...
static switch_status_t switch_vpx_control(switch_codec_t* codec, switch_codec_control_command_t cmd,
...
#define _VPX_CHECK_ERR(fmt, ...)                                                                \
    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,                                   \
                      "VPX config param \"%s\" -> \"%s\" value \"%s\" " fmt "\n", profile_name, \
...
#define _VPX_CHECK_MIN(var, val, min)                        \
    do {                                                     \
        int lval = val;                                      \
        if (lval < (min))                                    \
            _VPX_CHECK_ERRDEF(var, "is lower than %d", min); \
        else                                                 \
            var = lval;                                      \
    } while (0)
#define _VPX_CHECK_MAX(var, val, max)                         \
    do {                                                      \
        int lval = val;                                       \
        if (lval > (max))                                     \
            _VPX_CHECK_ERRDEF(var, "is larger than %d", max); \
        else                                                  \
            var = lval;                                       \
    } while (0)
#define _VPX_CHECK_MIN_MAX(var, val, min, max)                   \
    do {                                                         \
        int lval = val;                                          \
        if ((lval < (min)) || (lval > (max)))                    \
            _VPX_CHECK_ERRDEF(var, "not in [%d..%d]", min, max); \
        else                                                     \
            var = lval;                                          \
    } while (0)
    ...
    for (param = switch_xml_child(profile, "param"); param; param = param->next) {
        ...
        if (!strcmp(name, "dec-threads")) {
            ...
            if (codec_type == CODEC_TYPE_VP8) {
                ...
                _VPX_CHECK_MIN_MAX(enc_cfg->ts_number_layers, val, enc_cfg->ts_number_layers,
                                   enc_cfg->ts_number_layers);  // lock it
            } else {
                _VPX_CHECK_ERRDEF_NOTAPPL(enc_cfg->ts_number_layers);
            }
        } else if (!strcmp(name, "ts-periodicity")) {
            if (codec_type == CODEC_TYPE_VP9) {
                _VPX_CHECK_MIN_MAX(enc_cfg->ts_periodicity, val, enc_cfg->ts_periodicity,
                                   enc_cfg->ts_periodicity);  // lock it
            } else {
                _VPX_CHECK_MIN_MAX(enc_cfg->ts_periodicity, val, 0, 16);
            }
        } else if (!strcmp(name, "temporal-layering-mode")) {
            if (codec_type == CODEC_TYPE_VP9) {
                _VPX_CHECK_MIN_MAX(enc_cfg->temporal_layering_mode, val,
                                   enc_cfg->temporal_layering_mode,
                                   enc_cfg->temporal_layering_mode);  // lock it
            } else {
            ...
            _VPX_CHECK_MIN_MAX(my_cfg->token_parts, switch_parse_cpu_string(value),
                               VP8_ONE_TOKENPARTITION, VP8_EIGHT_TOKENPARTITION);
        ...
        if (settings) {
            ...
            for (param = switch_xml_child(settings, "param"); param; param = param->next) {
                ...
                if (!strcmp(name, "debug")) {
                    ...
                    _VPX_CHECK_MIN(vpx_globals.max_bitrate, switch_parse_bandwidth_string(value),
                                   1);
                    ...
                    _VPX_CHECK_MIN_MAX(vpx_globals.key_frame_min_freq, atoi(value) * 1000,
                                       10 * 1000, 3000 * 1000);
...
SWITCH_MODULE_LOAD_FUNCTION(mod_vpx_load) {
    ...
    *module_interface = switch_loadable_module_create_module_interface(pool, modname);
    ...
    switch_core_codec_add_video_implementation(
        pool, codec_interface, 99, "VP8", NULL, switch_vpx_init, switch_vpx_encode,
        switch_vpx_decode, switch_vpx_control, switch_vpx_destroy);
    SWITCH_ADD_CODEC(codec_interface, "VP9 Video");
    switch_core_codec_add_video_implementation(
        pool, codec_interface, 99, "VP9", NULL, switch_vpx_init, switch_vpx_encode,
        switch_vpx_decode, switch_vpx_control, switch_vpx_destroy);

    SWITCH_ADD_API(vpx_api_interface, "vpx", "VPX API", vpx_api_function, VPX_API_SYNTAX);
