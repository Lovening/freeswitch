...
void conference_record_launch_thread(conference_obj_t* conference, char* path, int canvas_id,
...
switch_status_t conference_record_stop(conference_obj_t* conference, switch_stream_handle_t* stream,
                                       char* path) {
    ...
    for (member = conference->members; member; member = member->next) {
        if (conference_utils_member_test_flag(member, MFLAG_NOCHANNEL) &&
            ...
            if (!conference_utils_test_flag(conference, CFLAG_CONF_RESTART_AUTO_RECORD) &&
                member->rec && member->rec->autorec) {
                stream->write_function(
                    stream, "Stopped AUTO recording file %s (Auto Recording Now Disabled)\n",
                    member->rec_path);
...
switch_status_t conference_record_action(conference_obj_t* conference, char* path,
...
void* SWITCH_THREAD_FUNC conference_record_thread_run(switch_thread_t* thread, void* obj) {
    ...
    member->flags[MFLAG_CAN_HEAR] = member->flags[MFLAG_NOCHANNEL] = member->flags[MFLAG_RUNNING] =
        1;
    ...
    if (switch_buffer_create_dynamic(&member->audio_buffer, CONF_DBLOCK_SIZE, CONF_DBUFFER_SIZE,
    ...
    if (conference_utils_test_flag(conference, CFLAG_TRANSCODE_VIDEO)) {
        ...
        if (canvas) {
            rec->path = switch_core_sprintf(
                rec->pool,
                "{video_time_audio=false,channels=%d,samplerate=%d,vw=%d,vh=%d,fps=%0.2f}%s",
                conference->channels, conference->rate, canvas->width, canvas->height,
                conference->video_fps.fps, orig_path);
        } else {
            rec->path = switch_core_sprintf(
                rec->pool,
                "{video_time_audio=false,channels=%d,samplerate=%d,vw=%d,vh=%d,fps=%0.2f}%s",
                conference->channels, conference->rate, conference->canvas_width,
                conference->canvas_height, conference->video_fps.fps, orig_path);
        }
    }
    ...
    if (switch_core_file_open(&member->rec->fh, rec->path, (uint8_t)conference->channels,
                              conference->rate, flags, rec->pool) != SWITCH_STATUS_SUCCESS) {
        ...
        if (test_eflag(conference, EFLAG_RECORD) &&
            ...
            switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "Error",
                                           "File could not be opened for recording");
    ...
    if (switch_core_timer_init(&timer, conference->timer_name, conference->interval, samples,
    ...
    switch_core_file_set_string(&member->rec->fh, SWITCH_AUDIO_COL_STR_ARTIST,
                                !zstr(conference->recording_metadata)
                                    ? conference->recording_metadata
    ...
    while (conference_utils_member_test_flag(member, MFLAG_RUNNING) &&
           conference_utils_test_flag(conference, CFLAG_RUNNING) &&
    ...
    again:
        ...
        if (!conference_utils_member_test_flag(member, MFLAG_PAUSE_RECORDING)) {
            if (!len ||
                switch_core_file_write(&member->rec->fh, data_buf, &len) != SWITCH_STATUS_SUCCESS) {
    ...
    if (switch_test_flag((&member->rec->fh), SWITCH_FILE_OPEN)) {
        ...
        switch_core_file_get_string(&member->rec->fh, SWITCH_AUDIO_COL_STR_FILE_TRIMMED,
                                    &file_trimmed);
        switch_core_file_get_string(&member->rec->fh, SWITCH_AUDIO_COL_STR_FILE_TRIMMED_MS,
                                    &file_trimmed_ms);
    ...
    if (switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM, CONF_EVENT_MAINT) ==
        ...
        switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "Path", rec->path);
        switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "Other-Recordings",
                                       conference->record_count ? "true" : "false");
        switch_event_add_header(event, SWITCH_STACK_BOTTOM, "Samples-Out", "%ld",
                                (long)member->rec->fh.samples_out);
        switch_event_add_header(event, SWITCH_STACK_BOTTOM, "Samplerate", "%ld",
                                (long)member->rec->fh.samplerate);
        switch_event_add_header(
            event, SWITCH_STACK_BOTTOM, "Milliseconds-Elapsed", "%ld",
            (long)member->rec->fh.samples_out / (member->rec->fh.samplerate / 1000));

        if (file_trimmed_ms) {
            switch_event_add_header(event, SWITCH_STACK_BOTTOM, "Milliseconds-File-Trimmed", "%s",
                                    file_trimmed_ms);
            switch_event_add_header(event, SWITCH_STACK_BOTTOM, "Record-File-Duration", "%d",
                                    atoi(file_trimmed_ms) / 1000);
        }

        if (file_size)
            switch_event_add_header(event, SWITCH_STACK_BOTTOM, "Record-File-Size", "%s",
                                    file_size);
        if (file_trimmed)
            switch_event_add_header(event, SWITCH_STACK_BOTTOM, "File-Trimmed", "%s", file_trimmed);
