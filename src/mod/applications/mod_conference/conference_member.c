...
void conference_member_do_binding(conference_member_t* member, conference_key_callback_t handler,
    ...
    switch_ivr_dmachine_bind(member->dmachine, "conf", digits, 0, 0,
                             conference_loop_dmachine_dispatcher, binding);
}
...
void conference_member_bind_controls(conference_member_t* member, const char* controls) {
    ...
    switch_event_add_header_string(params, SWITCH_STACK_BOTTOM, "Conf-Name",
                                   member->conference->name);
    switch_event_add_header_string(params, SWITCH_STACK_BOTTOM, "Conf-Profile",
                                   member->conference->profile_name);
    ...
    switch_event_add_header_string(params, SWITCH_STACK_BOTTOM, "Fetch-Call-UUID",
                                   switch_core_session_get_uuid(member->session));
    ...
    if (!(xgroups = switch_xml_child(cfg, "caller-controls"))) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
                          "Can't find caller-controls in %s\n", mod_conference_cf_name);
        goto end;
    }

    if (!(xgroups = switch_xml_find_child(xgroups, "group", "name", controls))) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
                          "Can't find group '%s' in caller-controls section of %s\n",
...
void conference_member_update_status_field(conference_member_t* member) {
    ...
    if (!member || !member->conference || !member->conference->la || !member->json ||
        !member->status_field || conference_utils_member_test_flag(member, MFLAG_SECOND_SCREEN)) {
        return;
    }
    ...
    if (conference_utils_test_flag(member->conference, CFLAG_JSON_STATUS)) {
        ...
        cJSON_AddItemToObject(
            audio, "muted",
            cJSON_CreateBool(!conference_utils_member_test_flag(member, MFLAG_CAN_SPEAK)));
        cJSON_AddItemToObject(
            audio, "deaf",
            cJSON_CreateBool(!conference_utils_member_test_flag(member, MFLAG_CAN_HEAR)));
        cJSON_AddItemToObject(audio, "onHold",
                              cJSON_CreateBool(switch_channel_test_flag(member->channel, CF_HOLD)));
        cJSON_AddItemToObject(
            audio, "talking",
            cJSON_CreateBool(conference_utils_member_test_flag(member, MFLAG_TALKING)));
        cJSON_AddItemToObject(audio, "floor",
                              cJSON_CreateBool(member->id == member->conference->floor_holder));
        ...
        if (switch_channel_test_flag(member->channel, CF_VIDEO) || member->avatar_png_img) {
            ...
            cJSON_AddItemToObject(
                video, "noRecover",
                cJSON_CreateBool(switch_channel_test_flag(member->channel, CF_NO_RECOVER)));
            if (switch_true(switch_channel_get_variable_dup(member->channel, "video_screen_share",
                                                            SWITCH_FALSE, -1))) {
                cJSON_AddItemToObject(video, "screenShare", cJSON_CreateTrue());
            }

            cJSON_AddItemToObject(video, "avatarPresented",
                                  cJSON_CreateBool(!!member->avatar_png_img));
            cJSON_AddItemToObject(
                video, "mediaFlow",
                cJSON_CreateString(
                    switch_core_session_media_flow(member->session, SWITCH_MEDIA_TYPE_VIDEO) ==
            ...
            cJSON_AddItemToObject(
                video, "muted",
                cJSON_CreateBool(!conference_utils_member_test_flag(member, MFLAG_CAN_BE_SEEN)));
            cJSON_AddItemToObject(
                video, "floor",
                cJSON_CreateBool(member->id == member->conference->video_floor_holder));
            if (member->id == member->conference->video_floor_holder &&
            ...
            cJSON_AddItemToObject(video, "reservationID",
                                  member->video_reservation_id
                                      ? cJSON_CreateString(member->video_reservation_id)
                                      : cJSON_CreateNull());

            cJSON_AddItemToObject(video, "roleID",
                                  member->video_role_id ? cJSON_CreateString(member->video_role_id)
                                                        : cJSON_CreateNull());

            cJSON_AddItemToObject(video, "videoLayerID",
                                  cJSON_CreateNumber(member->video_layer_id));
            cJSON_AddItemToObject(video, "canvasID", cJSON_CreateNumber(member->canvas_id));
            cJSON_AddItemToObject(video, "watchingCanvasID",
                                  cJSON_CreateNumber(member->watching_canvas_id));
    ...
    switch_live_array_add(member->conference->la, switch_core_session_get_uuid(member->session), -1,
                          &member->json, SWITCH_FALSE);
    switch_live_array_unlock(member->conference->la);
}

switch_status_t conference_member_add_event_data(conference_member_t* member,
    ...
    if (member->conference) {
        ...
        switch_event_add_header(
            event, SWITCH_STACK_BOTTOM, "Floor", "%s",
            (member->id == member->conference->floor_holder) ? "true" : "false");
    }
    ...
    if (member->session) {
        ...
        switch_event_add_header(
            event, SWITCH_STACK_BOTTOM, "Video", "%s",
            switch_channel_test_flag(switch_core_session_get_channel(member->session), CF_VIDEO)
                ? "true"
                : "false");
    }

    switch_event_add_header(
        event, SWITCH_STACK_BOTTOM, "Hear", "%s",
        conference_utils_member_test_flag(member, MFLAG_CAN_HEAR) ? "true" : "false");
    switch_event_add_header(
        event, SWITCH_STACK_BOTTOM, "See", "%s",
        conference_utils_member_test_flag(member, MFLAG_CAN_BE_SEEN) ? "true" : "false");
    switch_event_add_header(
        event, SWITCH_STACK_BOTTOM, "Speak", "%s",
        conference_utils_member_test_flag(member, MFLAG_CAN_SPEAK) ? "true" : "false");
    switch_event_add_header(
        event, SWITCH_STACK_BOTTOM, "Talking", "%s",
        conference_utils_member_test_flag(member, MFLAG_TALKING) ? "true" : "false");
    switch_event_add_header(
        event, SWITCH_STACK_BOTTOM, "Mute-Detect", "%s",
        conference_utils_member_test_flag(member, MFLAG_MUTE_DETECT) ? "true" : "false");
    switch_event_add_header(
        event, SWITCH_STACK_BOTTOM, "Hold", "%s",
        conference_utils_member_test_flag(member, MFLAG_HOLD) ? "true" : "false");
    switch_event_add_header(event, SWITCH_STACK_BOTTOM, "Member-ID", "%u", member->id);
    switch_event_add_header(
        event, SWITCH_STACK_BOTTOM, "Member-Type", "%s",
        conference_utils_member_test_flag(member, MFLAG_MOD) ? "moderator" : "member");
    switch_event_add_header(
        event, SWITCH_STACK_BOTTOM, "Member-Ghost", "%s",
        conference_utils_member_test_flag(member, MFLAG_GHOST) ? "true" : "false");
...
conference_relationship_t* conference_member_get_relationship(conference_member_t* member,
    ...
    for (rel = member->relationships; rel; rel = rel->next) {
        ...
        /* 0 matches everyone. (We will still test the others because a real match carries more
         * clout) */
        if (rel->id == 0) {
            global = rel;
        }
...
conference_member_t* conference_member_get(conference_obj_t* conference, uint32_t id) {
    ...
    if (member) {
        if (!conference_utils_member_test_flag(member, MFLAG_INTREE) ||
            ...
            (member->session &&
             !switch_channel_up(switch_core_session_get_channel(member->session)))) {
...
/* traverse the conference member list for the specified member with var/val  and return it's
 * pointer */
conference_member_t* conference_member_get_by_var(conference_obj_t* conference, const char* var,
                                                  const char* val) {
    conference_member_t* member = NULL;
    ...
    for (member = conference->members; member; member = member->next) {
        ...
        if ((check_var = switch_channel_get_variable_dup(member->channel, var, SWITCH_FALSE, -1)) &&
            !strcmp(check_var, val)) {
            break;
        }
    }
...
/* traverse the conference member list for the specified member id or variable and return its
 * pointer */
conference_member_t* conference_member_get_by_str(conference_obj_t* conference,
                                                  const char* id_str) {
    conference_member_t* member = NULL;
 ...
 */
conference_member_t* conference_member_get_by_role(conference_obj_t* conference,
                                                   const char* role_id) {
    conference_member_t* member = NULL;
...
void conference_member_check_channels(switch_frame_t* frame, conference_member_t* member,
                                      switch_bool_t in) {
    if (member->conference->channels != member->read_impl.number_of_channels ||
        conference_utils_member_test_flag(member, MFLAG_POSITIONAL)) {
        uint32_t rlen;
        int from, to;
        ...
        if (in && frame->rate == 48000 && ((from == 1 && to == 2) || (from == 2 && to == 2)) &&
            conference_utils_member_test_flag(member, MFLAG_POSITIONAL)) {
            if (from == 2) {
...
void conference_member_add_file_data(conference_member_t* member, int16_t* data,
                                     switch_size_t file_data_len) {
    ...
    if (member->fnode->done) {
        ...
        if (member->fnode->leadin) {
            ...
            if (member->fnode->type == NODE_TYPE_SPEECH) {
                ...
                if (switch_core_speech_read_tts(member->fnode->sh, file_frame, &speech_len,
                ...
                if (member->fnode->fh.vol) {
                    switch_change_sln_volume_granular(
                        (void*)file_frame, (uint32_t)file_sample_len * member->fnode->fh.channels,
                        member->fnode->fh.vol);
                }
            }
            ...
            if (file_sample_len <= 0) {
                ...
                if (member->volume_out_level) {
                    switch_change_sln_volume(
                        file_frame, (uint32_t)file_sample_len * member->conference->channels,
                        member->volume_out_level);
                }

                if (member->fnode->al) {
                    conference_al_process(member->fnode->al, file_frame, file_sample_len * 2,
...
conference_relationship_t* conference_member_add_relationship(conference_member_t* member,
                                                              uint32_t id) {
...
switch_status_t conference_member_add(conference_obj_t* conference, conference_member_t* member) {
    ...
    if (!conference_utils_member_test_flag(member, MFLAG_NOCHANNEL)) {
        ...
        if (switch_true(switch_channel_get_variable_dup(member->channel, "video_second_screen",
                                                        SWITCH_FALSE, -1))) {
        ...
        if ((var = switch_channel_get_variable_dup(member->channel, "video_logo_path", SWITCH_FALSE,
                                                   -1))) {
            conference_member_set_logo(member, var);
        }

        if ((var = switch_channel_get_variable_dup(member->channel, "video_codec_group",
                                                   SWITCH_FALSE, -1))) {
            member->video_codec_group = switch_core_strdup(member->pool, var);
        }

        if ((var = switch_channel_get_variable_dup(member->channel, "conference_join_volume_in",
                                                   SWITCH_FALSE, -1))) {
            int id = atoi(var);
        ...
        if ((var = switch_channel_get_variable_dup(member->channel, "conference_join_volume_out",
                                                   SWITCH_FALSE, -1))) {
            int id = atoi(var);
        ...
        if ((var = switch_channel_get_variable_dup(member->channel, "conference_join_energy_level",
                                                   SWITCH_FALSE, -1))) {
            int id = atoi(var);
        ...
        if ((var = switch_channel_get_variable_dup(member->channel, "video_initial_canvas",
                                                   SWITCH_FALSE, -1))) {
            int id = atoi(var) - 1;
        ...
        if ((var = switch_channel_get_variable_dup(member->channel, "video_initial_watching_canvas",
                                                   SWITCH_FALSE, -1))) {
            int id = atoi(var) - 1;
        ...
        if (has_video) {
            ...
            if ((var = switch_channel_get_variable_dup(member->channel, "video_mute_png",
                                                       SWITCH_FALSE, -1))) {
                member->video_mute_png = switch_core_strdup(member->pool, var);
                member->video_mute_img =
                    switch_img_read_png(member->video_mute_png, SWITCH_IMG_FMT_I420);
            }

            if ((var = switch_channel_get_variable_dup(member->channel, "video_reservation_id",
                                                       SWITCH_FALSE, -1))) {
                member->video_reservation_id = switch_core_strdup(member->pool, var);
            }

            if ((var = switch_channel_get_variable_dup(member->channel, "video_role_id",
                                                       SWITCH_FALSE, -1))) {
                member->video_role_id = switch_core_strdup(member->pool, var);
            }

            if ((var = switch_channel_get_variable(channel, "video_use_dedicated_encoder")) &&
                switch_true(var)) {
                conference_utils_member_set_flag_locked(member, MFLAG_NO_MINIMIZE_ENCODING);
            }

            if ((var =
                     switch_channel_get_variable(member->channel, "rtp_video_max_bandwidth_in"))) {
                member->max_bw_in = switch_parse_bandwidth_string(var);
            }

            if ((var =
                     switch_channel_get_variable(member->channel, "rtp_video_max_bandwidth_out"))) {
            ...
            if (bitrate) {
                switch_core_media_set_outgoing_bitrate(member->session, SWITCH_MEDIA_TYPE_VIDEO,
                                                       bitrate);
            }
        }
        ...
        switch_channel_set_variable_printf(
            channel, "conference_moderator", "%s",
            conference_utils_member_test_flag(member, MFLAG_MOD) ? "true" : "false");
        switch_channel_set_variable_printf(
            channel, "conference_ghost", "%s",
            conference_utils_member_test_flag(member, MFLAG_GHOST) ? "true" : "false");
        ...
        if (!switch_channel_get_variable(channel, "conference_call_key")) {
            char* key = switch_core_session_sprintf(
                member->session, "conference_%s_%s_%s", conference->name, conference->domain,
        ...
        if (conference->count > 1) {
            if (((conference->moh_sound || conference->tmp_moh_sound) &&
                 !conference_utils_test_flag(conference, CFLAG_WAIT_MOD)) ||
                (conference_utils_test_flag(conference, CFLAG_WAIT_MOD) &&
                 !switch_true(
                     switch_channel_get_variable(channel, "conference_permanent_wait_mod_moh")))) {
                /* stop MoH if any */
            ...
            if (!switch_channel_test_app_flag_key("conference_silent", channel, CONF_SILENT_REQ)) {
                const char* enter_sound =
                    switch_channel_get_variable(channel, "conference_enter_sound");
                if (conference_utils_test_flag(conference, CFLAG_ENTER_SOUND) &&
                    !conference_utils_member_test_flag(member, MFLAG_SILENT)) {
                    if (!zstr(enter_sound)) {
                        conference_file_play(conference, (char*)enter_sound, CONF_DEFAULT_LEADIN,
                                             switch_core_session_get_channel(member->session), 0);
                    } else {
                        conference_file_play(conference, conference->enter_sound,
                                             CONF_DEFAULT_LEADIN,
                                             switch_core_session_get_channel(member->session), 0);
                    }
                }
                ...
                if (conference->count >= conference->announce_count &&
                    ...
                    if (!conference_utils_test_flag(conference, CFLAG_BRIDGE_TO)) {
                        if (conference->alone_sound &&
                            !conference_utils_member_test_flag(member, MFLAG_GHOST)) {
                            conference_file_stop(conference, FILE_STOP_ASYNC);
                            conference_file_play(
                                conference, conference->alone_sound, CONF_DEFAULT_LEADIN,
                                switch_core_session_get_channel(member->session), 0);
                        } else {
                            switch_snprintf(
                                msg, sizeof(msg),
        ...
        if ((position = switch_channel_get_variable(channel, "conference_position"))) {
            if (conference->channels == 2) {
                if (conference_utils_member_test_flag(member, MFLAG_NO_POSITIONAL)) {
                    ...
                    if (conference_member_parse_position(member, position) !=
                        SWITCH_STATUS_SUCCESS) {
                        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
                                          "%s invalid position data\n",
                                          switch_channel_get_name(channel));
                    } else {
                ...
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
                                  "%s cannot set position data on mono conference.\n",
                                  switch_channel_get_name(channel));
            }
        }
        ...
        if (strcasecmp(controls, "none")) {
            switch_ivr_dmachine_create(&member->dmachine, "mod_conference", NULL,
                                       conference->ivr_dtmf_timeout, conference->ivr_input_timeout,
                                       NULL, NULL, NULL);
    ...
    if (conference->la && member->channel) {
        if (!conference_utils_member_test_flag(member, MFLAG_SECOND_SCREEN)) {
            ...
            cJSON_AddItemToArray(member->json, cJSON_CreateString(switch_channel_get_variable(
                                                   member->channel, "caller_id_number")));
            cJSON_AddItemToArray(member->json, cJSON_CreateString(switch_channel_get_variable(
                                                   member->channel, "caller_id_name")));

            cJSON_AddItemToArray(
                member->json,
                cJSON_CreateStringPrintf(
                    "%s@%s", switch_channel_get_variable(member->channel, "original_read_codec"),
            ...
            // switch_live_array_add_alias(conference->la,
            // switch_core_session_get_uuid(member->session), "conference");
        }

        conference_event_adv_la(conference, member, SWITCH_TRUE);

        if (!conference_utils_member_test_flag(member, MFLAG_SECOND_SCREEN)) {
            switch_live_array_add(conference->la, switch_core_session_get_uuid(member->session), -1,
                                  &member->json, SWITCH_FALSE);
        }
    }
...
switch_status_t conference_member_del(conference_obj_t* conference, conference_member_t* member) {
    ...
    if (member->session &&
        (exit_sound = switch_channel_get_variable(switch_core_session_get_channel(member->session),
    ...
    if (!conference_utils_member_test_flag(member, MFLAG_NOCHANNEL)) {
        ...
        // 停止用户录制视频
        if (member -> member_record && member->session && member->member_record_path) {
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member->session), SWITCH_LOG_INFO,
                              "Stopping member recording on leave: %s\n", member->member_record_path);
            switch_ivr_stop_record_session(member->session, member->member_record_path);
            // 触发事件
            {
                switch_event_t *rec_event;
                if (switch_event_create_subclass(&rec_event, SWITCH_EVENT_CUSTOM, 
                                                  "conference::member_record_stop") == SWITCH_STATUS_SUCCESS) {
                    switch_event_add_header_string(rec_event, SWITCH_STACK_BOTTOM, 
                                                   "Conference-Name", conference->name);
                    switch_event_add_header_string(rec_event, SWITCH_STACK_BOTTOM, 
                                                   "Conference-UUID", conference->uuid_str);
                    switch_event_add_header(rec_event, SWITCH_STACK_BOTTOM, "Member-ID", "%u", member->id);
                    switch_event_add_header_string(rec_event, SWITCH_STACK_BOTTOM, 
                                                   "Recording-Path", member->member_record_path);
                    switch_event_add_header_string(rec_event, SWITCH_STACK_BOTTOM, 
                                                   "Stop-Reason", "member-leave");
                    switch_event_fire(&rec_event);
                }
            }
            
            member->member_record = SWITCH_FALSE;
            member->member_record_path = NULL;
        }


        if ((conference->min && conference_utils_test_flag(conference, CFLAG_ENFORCE_MIN) &&
             (conference->count + conference->count_ghosts) < conference->min) ||
            (conference_utils_test_flag(conference, CFLAG_DYNAMIC) &&
             (conference->count + conference->count_ghosts == 0))) {
            ...
            if (!exit_sound && conference->exit_sound &&
                conference_utils_test_flag(conference, CFLAG_EXIT_SOUND) &&
                !conference_utils_member_test_flag(member, MFLAG_SILENT)) {
    ...
    if (conference->la && member->session) {
        ...
        // switch_live_array_clear_alias(conference->la,
        // switch_core_session_get_uuid(member->session), "conference");
...
void conference_member_send_all_dtmf(conference_member_t* member, conference_obj_t* conference,
                                     const char* dtmf) {
    ...
    for (imember = conference->members; imember; imember = imember->next) {
        ...
        if (conference_utils_member_test_flag(imember, MFLAG_SKIP_DTMF)) {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
                              "Skipping dist-dtmf to member due to skip-dtmf flag.\n");
            continue;
        }
        if (imember->session) {
            const char* p;
...
switch_status_t conference_member_play_file(conference_member_t* member, char* file,
                                            uint32_t leadin, switch_bool_t mux) {
    switch_status_t status = SWITCH_STATUS_FALSE;
    ...
    if ((expanded = switch_channel_expand_variables(
             switch_core_session_get_channel(member->session), file)) != file) {
        file = expanded;
    } else {
        expanded = NULL;
    }
    ...
    if (switch_core_new_memory_pool(&pool) != SWITCH_STATUS_SUCCESS) {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member->session), SWITCH_LOG_CRIT,
                          "Pool Failure\n");
...
retry:
    ...
    if (switch_core_file_open(&fnode->fh, file, (uint8_t)channels, member->conference->rate,
    ...
    if (fnode->fh.params) {
        ...
        if (!zstr(vol)) {
            ...
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member->session), SWITCH_LOG_DEBUG,
                              "Set playback volume for file: %d\n", fnode->fh.vol);
        }
...
switch_status_t conference_member_say(conference_member_t* member, char* text, uint32_t leadin) {
    ...
    if (member == NULL || zstr(text)) return SWITCH_STATUS_FALSE;
    ...
    if (*text == '{') {
        ...
        if (switch_event_create_brackets(fp, '{', '}', ',', &params, &new_fp, SWITCH_FALSE) !=
            SWITCH_STATUS_SUCCESS) {
            new_fp = fp;
        }
    ...
    if (!member->sh) {
        ...
        if (switch_core_speech_open(&member->lsh, conference->tts_engine, conference->tts_voice,
                                    conference->rate, conference->interval, channels, &flags,
...
void conference_member_itterator(conference_obj_t* conference, switch_stream_handle_t* stream,
                                 uint8_t non_mod, conference_api_member_cmd_t pfncallback,
                                 void* data) {
    conference_member_t* member = NULL;
...
int conference_member_get_canvas_id(conference_member_t* member, const char* val,
                                    switch_bool_t watching) {
    int index = -1;
    int cur;
    ...
    if (watching) {
        ...
        } else if ((uint32_t)index > member->conference->canvas_count ||
                   !member->conference->canvases[index]) {
            index = 0;
        }
    } else {
        if (index < 0) {
...
int conference_member_setup_media(conference_member_t* member, conference_obj_t* conference) {
    ...
    if (switch_core_codec_init(
            &member->read_codec, "L16", NULL, NULL, read_impl.actual_samples_per_second,
        ...
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member->session), SWITCH_LOG_DEBUG,
                          "Raw Codec Activation Failed L16@%uhz %d channel %dms\n",
                          read_impl.actual_samples_per_second, read_impl.number_of_channels,
                          read_impl.microseconds_per_packet / 1000);
    ...
    if (read_impl.actual_samples_per_second != conference->rate) {
        if (switch_resample_create(&member->read_resampler, read_impl.actual_samples_per_second,
                                   conference->rate, member->frame_size, SWITCH_RESAMPLE_QUALITY,
        ...
        if (!member->resample_buffer &&
            switch_buffer_create_dynamic(&member->resample_buffer, CONF_DBLOCK_SIZE,
                                         CONF_DBUFFER_SIZE,
    ...
    if (switch_core_codec_init(&member->write_codec, "L16", NULL, NULL, conference->rate,
                               read_impl.microseconds_per_packet / 1000,
        ...
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member->session), SWITCH_LOG_DEBUG,
                          "Raw Codec Activation Success L16@%uhz %d channel %dms\n",
                          conference->rate, conference->channels,
                          read_impl.microseconds_per_packet / 1000);
    } else {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member->session), SWITCH_LOG_DEBUG,
                          "Raw Codec Activation Failed L16@%uhz %d channel %dms\n",
    ...
    if (!member->audio_buffer &&
        switch_buffer_create_dynamic(&member->audio_buffer, CONF_DBLOCK_SIZE, CONF_DBUFFER_SIZE,
                                     CONF_DBUFFER_MAX) != SWITCH_STATUS_SUCCESS) {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member->session), SWITCH_LOG_CRIT,
                          "Memory Error Creating Audio Buffer!\n");
        goto codec_done1;
    ...
    if (!member->mux_buffer &&
        switch_buffer_create_dynamic(&member->mux_buffer, CONF_DBLOCK_SIZE, CONF_DBUFFER_SIZE,
                                     CONF_DBUFFER_MAX) != SWITCH_STATUS_SUCCESS) {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member->session), SWITCH_LOG_CRIT,
                          "Memory Error Creating Audio Buffer!\n");
        goto codec_done1;
