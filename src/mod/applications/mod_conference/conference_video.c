...
void conference_video_parse_layouts(conference_obj_t* conference, int WIDTH, int HEIGHT) {
    ...
    switch_xml_t cxml = NULL, cfg = NULL, x_layouts, x_layout, x_layout_settings, x_group, x_groups,
                 x_image;
    char cmd_str[256] = "";
    ...
    switch_event_add_header_string(params, SWITCH_STACK_BOTTOM, "conference_name",
                                   conference->name);
            ...
            for (x_layout = switch_xml_child(x_layouts, "layout"); x_layout;
                ...
                if (transition_in) {
                    vlayout->transition_in = switch_core_sprintf(
                        conference->pool, "{full-screen=true}%s", transition_in);
                }

                if (transition_out) {
                    vlayout->transition_out = switch_core_sprintf(
                        conference->pool, "{full-screen=true}%s", transition_out);
                }

                for (x_image = switch_xml_child(x_layout, "image"); x_image;
                     x_image = x_image->next) {
                    const char *res_id = NULL, *audio_position = NULL, *role_id = NULL;
                    int x = -1, y = -1, scale = -1, hscale = -1, floor = 0, flooronly = 0,
                        fileonly = 0, overlap = 0, zoom = 0;
                    ...
                    if (res_id) {
                        vlayout->images[vlayout->layers].res_id =
                            switch_core_strdup(conference->pool, res_id);
                    }

                    if (role_id) {
                        vlayout->images[vlayout->layers].role_id =
                            switch_core_strdup(conference->pool, role_id);
                    }
                    ...
                    if (auto_3d || audio_position) {
                        if (auto_3d || !strcasecmp(audio_position, "auto")) {
                            ...
                            vlayout->images[vlayout->layers].audio_position =
                                switch_core_sprintf(conference->pool, "%02f:0.0:%02f", xv, yv);

                        } else {
                            vlayout->images[vlayout->layers].audio_position =
                                switch_core_strdup(conference->pool, audio_position);
                        }
                    }
                ...
                if (!COMPLETE_INIT) {
                    ...
                    if (!COMPLETE_INIT) {
                        switch_snprintf(cmd_str, sizeof(cmd_str),
                                        "add conference ::conference::conference_list_conferences "
                                        "vid-layout %s",
                                        name);
                ...
                while (x_layout) {
                    ...
                    if ((vlayout = switch_core_hash_find(conference->layout_hash, nname))) {
                        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
                                          "Adding node %s to layout group %s\n", nname, name);
...
void conference_video_clear_layer(mcu_layer_t* layer) {
    if (layer->canvas && layer->canvas->img) {
        switch_img_fill(layer->canvas->img, layer->x_pos, layer->y_pos, layer->screen_w,
                        layer->screen_h, &layer->canvas->bgcolor);
    }

    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "Clearing layer %d\n",
                      layer->member_id);
...
void conference_video_reset_layer(mcu_layer_t* layer) {
    ...
    if (!layer->img && layer->screen_w && layer->screen_h) {
        layer->img =
            switch_img_alloc(NULL, SWITCH_IMG_FMT_I420, layer->screen_w, layer->screen_h, 1);
...
void conference_video_scale_and_patch(mcu_layer_t* layer, switch_image_t* ximg,
                                      switch_bool_t freeze) {
    switch_image_t *IMG, *img;
    int img_changed = 0, want_w = 0, want_h = 0, border = 0;
#ifndef HAVE_LIBAVFILTER
    static switch_bool_t filter_warning_shown = SWITCH_FALSE;
#endif
    int keep_aspect = 0;  // 默认：直接拉伸模式
    ...
    if (layer->bugged) {
        if (layer->member_id > -1 && layer->member &&
            switch_thread_rwlock_tryrdlock(layer->member->rwlock) == SWITCH_STATUS_SUCCESS) {
        ...
        if ((!layer->manual_geometry.w ||
             (layer->last_geometry.x && abs(layer->manual_geometry.x - layer->last_geometry.x) >
                                            layer->cam_opts.zoom_move_factor) ||
             (layer->last_geometry.y && abs(layer->manual_geometry.y - layer->last_geometry.y) >
                                            layer->cam_opts.zoom_move_factor) ||
             (layer->last_geometry.w && abs(layer->manual_geometry.w - layer->last_geometry.w) >
                                            layer->cam_opts.zoom_move_factor / 2))) {
            ...
            if (switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM, CONF_EVENT_MAINT) ==
                ...
                switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "action",
                                               "movement-detection");
                switch_event_add_header(event, SWITCH_STACK_BOTTOM, "member_id", "%d",
                                        layer->member_id);

                switch_event_add_header(event, SWITCH_STACK_BOTTOM, "last_x", "%d",
                                        layer->manual_geometry.x);
                switch_event_add_header(event, SWITCH_STACK_BOTTOM, "last_y", "%d",
                                        layer->manual_geometry.y);
                switch_event_add_header(event, SWITCH_STACK_BOTTOM, "last_w", "%d",
                                        layer->manual_geometry.w);
                switch_event_add_header(event, SWITCH_STACK_BOTTOM, "last_h", "%d",
                                        layer->manual_geometry.h);

                switch_event_add_header(event, SWITCH_STACK_BOTTOM, "new_x", "%d",
                                        layer->bug_frame.geometry.x);
                switch_event_add_header(event, SWITCH_STACK_BOTTOM, "new_y", "%d",
                                        layer->bug_frame.geometry.y);
                switch_event_add_header(event, SWITCH_STACK_BOTTOM, "new_w", "%d",
                                        layer->bug_frame.geometry.w);
                switch_event_add_header(event, SWITCH_STACK_BOTTOM, "new_h", "%d",
                                        layer->bug_frame.geometry.h);
    ...
    if (layer->refresh) {
        switch_img_fill(layer->canvas->img, layer->x_pos, layer->y_pos, layer->screen_w,
                        layer->screen_h, &layer->canvas->letterbox_bgcolor);
        layer->banner_patched = 0;
        layer->refresh = 0;
    }
    ...
    if (layer->geometry.scale) {
        ...
        img_h = layer->screen_h =
            (uint32_t)(IMG->d_h * layer->geometry.hscale / VIDEO_LAYOUT_SCALE);

        screen_aspect = (double)layer->screen_w / layer->screen_h;
        img_aspect = (double)img->d_w / img->d_h;

        if ((layer->geometry.zoom || layer->cam_opts.autozoom || layer->cam_opts.autopan ||
             layer->cam_opts.manual_pan || layer->cam_opts.manual_zoom)) {
            double scale = 1;
            ...
            if (layer->cam_opts.autopan) {
                can_pan = layer->bug_frame.geometry.w &&
                          (layer->geometry.zoom || layer->cam_opts.manual_zoom);
            } else {
                can_pan = layer->cam_opts.manual_pan &&
                          (layer->geometry.zoom || layer->cam_opts.manual_zoom);
            }
            ...
            if ((layer->cam_opts.autozoom || layer->cam_opts.autopan) &&
                (!layer->auto_geometry.w ||
                 (layer->last_geometry.x && abs(layer->auto_geometry.x - layer->last_geometry.x) >
                                                layer->cam_opts.zoom_move_factor) ||
                 (layer->last_geometry.y && abs(layer->auto_geometry.y - layer->last_geometry.y) >
                                                layer->cam_opts.zoom_move_factor) ||
                 (layer->last_geometry.w && abs(layer->auto_geometry.w - layer->last_geometry.w) >
                                                layer->cam_opts.zoom_move_factor / 2))) {
            ...
            // printf("BOUNDS B4 %d,%d %dx%d %dx%d\n", crop_x, crop_y, img->d_w, img->d_h, crop_w,
            // crop_h);
            set_bounds(&crop_x, &crop_y, img->w, img->h, crop_w, crop_h);
            ...
            set_pan(crop_x, &layer->crop_x, layer->cam_opts.pan_accel_speed,
                    layer->cam_opts.pan_accel_min, layer->cam_opts.pan_speed);
            set_pan(crop_y, &layer->crop_y, layer->cam_opts.pan_accel_speed,
                    layer->cam_opts.pan_accel_min, layer->cam_opts.pan_speed);

            // printf("AF %d,%d\n", layer->crop_x, layer->crop_y);

            // printf("B4 %dx%d %dx%d\n", crop_w, crop_h, layer->crop_w, layer->crop_h);
            set_pan(crop_w, &layer->crop_w, layer->cam_opts.zoom_accel_speed,
                    layer->cam_opts.zoom_accel_min, layer->cam_opts.zoom_speed);
            ...
            set_bounds(&layer->crop_x, &layer->crop_y, img->w, img->h, layer->crop_w,
                       layer->crop_h);

            assert(layer->crop_w > 0);

            // printf("RECT %d,%d %dx%d (%dx%d) [%dx%d] [%dx%d]\n", layer->crop_x, layer->crop_y,
            // layer->crop_w, layer->crop_h, layer->crop_w + layer->crop_x, layer->crop_h +
            // layer->crop_y, img->d_w, img->d_h, layer->screen_w, layer->screen_h);
        ...
        if (layer->member && layer->member->channel) {
            ...
            // 如果设置了 is_scale 变量且值为 "true"、"1"、"yes" 或 "on"，则保持比例居中
            if (scale_var && (switch_true(scale_var))) {
        ...
        if (keep_aspect) {
            // ==================== 保持比例居中模式（带黑边） ====================
            // 根据画面和画布的宽高比，计算合适的尺寸并居中显示
    
            if (screen_aspect > img_aspect) {
            // 画布更"宽"：按高度适配，宽度留黑边（pillarbox）
        ...
        // }
        /*
         * 直接拉伸到目标分辨率，填满整个区域（无黑边）
         * 不保持原始比例，避免 letterbox/pillarbox 黑边
         */
        // img_w = layer->screen_w;
        // img_h = layer->screen_h;
...
#ifdef HAVE_LIBAVFILTER
        /* 应用 FFmpeg filter（在左下角显示用户ID/名称） */
        if (layer->filter_enabled && layer->filter_graph) {
            switch_status_t filter_status = conference_video_apply_layer_filter(layer, &layer->img);
            if (filter_status != SWITCH_STATUS_SUCCESS) {
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
                    "Failed to apply video filter to layer %d\n", layer->idx);
            }
        }
#else
        /* FFmpeg filter support not compiled - show warning once */
        if (!filter_warning_shown) {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
                "FFmpeg filter NOT AVAILABLE - mod_conference was built without libavfilter support\n");
            filter_warning_shown = SWITCH_TRUE;
        }
#endif

        if (layer->logo_img) {
            // int ew = layer->screen_w - (border * 2), eh = layer->screen_h - (layer->banner_img ?
            // layer->banner_img->d_h : 0) - (border * 2);
            ...
            switch_img_find_position(layer->logo_pos, ew, eh, layer->logo_img->d_w,
                                     layer->logo_img->d_h, &ex, &ey);

            switch_img_patch(layer->img, layer->logo_img, ex + border, ey + border);
            // switch_img_patch(IMG, layer->logo_img, layer->x_pos + ex + border, layer->y_pos + ey
            // + border);
        }
        ...
        if (layer->banner_img && !layer->banner_patched) {
            ...
            switch_img_find_position(POS_LEFT_BOT, ew, eh, layer->banner_img->d_w,
                                     layer->banner_img->d_h, &ex, &ey);
        ...
        if (layer->img) {
            ...
            if (layer->overlay_img) {
                switch_img_fit(&layer->overlay_img, layer->img->d_w, layer->img->d_h,
                               SWITCH_FIT_SCALE);
                ...
                if (layer->overlay_filters & SCV_FILTER_GRAY_BG) {
                    switch_img_gray(layer->overlay_img, 0, 0, layer->overlay_img->d_w,
                                    layer->overlay_img->d_h);
                }

                if (layer->overlay_filters & SCV_FILTER_SEPIA_BG) {
                    switch_img_sepia(layer->overlay_img, 0, 0, layer->overlay_img->d_w,
                                     layer->overlay_img->d_h);
                }
            ...
            switch_img_patch_rect(IMG, x_pos + border, y_pos + border, layer->img, 0, 0, want_w,
                                  want_h);
        }
...
void conference_member_set_logo(conference_member_t* member, const char* path) {
    ...
    if (*path == '{') {
        ...
        if (switch_event_create_brackets((char*)path, '{', '}', ',', &params, &parsed,
    ...
    if (path) {
        ...
        if (member->video_logo) {
            ...
            if (params && (var = switch_event_get_header(params, "text"))) {
                ...
                if ((img = switch_img_write_text_img(member->video_logo->d_w,
                                                     member->video_logo->d_h, SWITCH_FALSE, var))) {
                    switch_img_fit(&img, member->video_logo->d_w, member->video_logo->d_h,
...
void conference_video_layer_set_banner(conference_member_t* member, mcu_layer_t* layer,const char* text) {
    ...
    const char* fg = "#ede3e3";
    const char* bg = "#4b4d4e";
    char* parsed = NULL;
    switch_event_t* params = NULL;
    ...
    if (*text == '{') {
        ...
        if (switch_event_create_brackets((char*)text, '{', '}', ',', &params, &parsed,
                                         SWITCH_FALSE) != SWITCH_STATUS_SUCCESS ||
            !parsed) {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Parse Error!\n");
        } else {
    ...
    if (zstr(text) || !strcasecmp(text, "reset")) {
        text =
            switch_channel_get_variable_dup(member->channel, "video_banner_text", SWITCH_FALSE, -1);
    }
    ...
    switch_img_txt_handle_render(layer->txthandle, layer->banner_img, font_size / 2, font_size / 2,
                                 text, NULL, fg, bg, 0, 0);
...
switch_status_t conference_video_attach_video_layer(conference_member_t* member,
                                                    mcu_canvas_t* canvas, int idx) {
    mcu_layer_t* layer = NULL;
    ...
    if ((switch_core_session_media_flow(member->session, SWITCH_MEDIA_TYPE_VIDEO) ==
             SWITCH_MEDIA_FLOW_SENDONLY ||
         switch_core_session_media_flow(member->session, SWITCH_MEDIA_TYPE_VIDEO) ==
             SWITCH_MEDIA_FLOW_INACTIVE) &&
        !member->avatar_png_img) {
        conference_utils_member_clear_flag(member, MFLAG_DED_VID_LAYER);
        return SWITCH_STATUS_FALSE;
    }
    ...
    if (!zstr(member->video_role_id) && !zstr(layer->geometry.role_id) &&
        !strcmp(layer->geometry.role_id, member->video_role_id)) {
    ...
    if (layer->geometry.res_id) {
        if (!member->video_reservation_id ||
            strcmp(layer->geometry.res_id, member->video_reservation_id)) {
            switch_goto_status(SWITCH_STATUS_FALSE, end);
        }
    }
    ...
    if (member->video_banner_text ||
        (var = switch_channel_get_variable_dup(channel, "video_banner_text", SWITCH_FALSE, -1))) {
...
#ifdef HAVE_LIBAVFILTER
    /* 为该成员初始化 FFmpeg filter（在左下角显示用户ID/名称） */
    if (layer->screen_w > 0 && layer->screen_h > 0) {
        switch_status_t filter_status = conference_video_init_layer_filter(layer, member, layer->screen_w, layer->screen_h);
        if (filter_status == SWITCH_STATUS_SUCCESS) {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO,
                "Initialized video filter for member %d\n", member->id);
        } else {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
                "Failed to initialize video filter for member %d\n", member->id);
        }
    }
#endif
    ...
    if (!canvas->disable_auto_clear) {
        switch_img_fill(canvas->img, layer->x_pos, layer->y_pos, layer->screen_w, layer->screen_h,
...
void conference_video_init_canvas_layers(conference_obj_t* conference, mcu_canvas_t* canvas,
                                         video_layout_t* vlayout, switch_bool_t force) {
    int i = 0;
    ...
    for (i = 0; i < vlayout->layers; i++) {
        ...
        layer->screen_h =
            (uint32_t)(canvas->img->d_h * layer->geometry.hscale / VIDEO_LAYOUT_SCALE);
        ...
        /* if we ever decided to reload layers config on demand the pointer assignment below  will
           lead to segs but we only load them once forever per conference so these pointers are
           valid for the life of the conference */
    ...
    if (conference->fnode &&
        (conference->fnode->canvas_id == canvas->canvas_id || conference->fnode->canvas_id == -1)) {
...
switch_status_t conference_video_set_canvas_bgimg(mcu_canvas_t* canvas, const char* img_path) {
    ...
    switch_img_find_position(POS_CENTER_MID, canvas->img->d_w, canvas->img->d_h, canvas->bgimg->d_w,
                             canvas->bgimg->d_h, &x, &y);
...
switch_status_t conference_video_set_canvas_fgimg(mcu_canvas_t* canvas, const char* img_path) {
    ...
    switch_img_find_position(POS_CENTER_MID, canvas->img->d_w, canvas->img->d_h, canvas->fgimg->d_w,
                             canvas->fgimg->d_h, &x, &y);
    switch_img_patch(canvas->img, canvas->fgimg, x, y);

    return SWITCH_STATUS_SUCCESS;
}

switch_status_t conference_video_attach_canvas(conference_obj_t* conference, mcu_canvas_t* canvas,
                                               int super) {
...
switch_status_t conference_video_init_canvas(conference_obj_t* conference, video_layout_t* vlayout,
                                             mcu_canvas_t** canvasP) {
    mcu_canvas_t* canvas;
...
int conference_video_flush_queue(switch_queue_t* q, int min) {
    ...
    while (switch_queue_size(q) > min && switch_queue_trypop(q, &pop) == SWITCH_STATUS_SUCCESS &&
           pop) {
...
void conference_video_write_canvas_image_to_codec_group(
    conference_obj_t* conference, mcu_canvas_t* canvas, codec_set_t* codec_set, int codec_index,
    uint32_t timestamp, switch_bool_t need_refresh, switch_bool_t send_keyframe,
    switch_bool_t need_reset)

{
    ...
    if (need_reset) {
        ...
        switch_core_codec_control(&codec_set->codec, SCC_VIDEO_RESET, SCCT_INT, (void*)&type,
                                  SCCT_NONE, NULL, NULL, NULL);
        need_refresh = SWITCH_TRUE;
    }

    if (send_keyframe) {
        switch_core_codec_control(&codec_set->codec, SCC_VIDEO_GEN_KEYFRAME, SCCT_NONE, NULL,
                                  SCCT_NONE, NULL, NULL, NULL);
    }

    if (scaled_img) {
        if (!send_keyframe && codec_set->fps_divisor > 1 &&
            (codec_set->frame_count++) % codec_set->fps_divisor) {
            // switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Skip one frame, total:
    ...
    do {
        ...
        if (encode_status == SWITCH_STATUS_SUCCESS || encode_status == SWITCH_STATUS_MORE_DATA) {
            ...
            for (imember = conference->members; imember; imember = imember->next) {
                ...
                if (codec_set->video_codec_group &&
                    (!imember->video_codec_group ||
                     strcmp(codec_set->video_codec_group, imember->video_codec_group))) {
                    continue;
                }
                ...
                if (switch_core_session_media_flow(imember->session, SWITCH_MEDIA_TYPE_VIDEO) ==
                    ...
                    switch_core_session_media_flow(imember->session, SWITCH_MEDIA_TYPE_VIDEO) ==
                        SWITCH_MEDIA_FLOW_INACTIVE) {
                ...
                if (switch_frame_buffer_dup(imember->fb, frame, &dupframe) ==
                    SWITCH_STATUS_SUCCESS) {
                    if (switch_frame_buffer_trypush(imember->fb, dupframe) !=
                        SWITCH_STATUS_SUCCESS) {
...
video_layout_t* conference_video_get_layout(conference_obj_t* conference,
                                            const char* video_layout_name,
                                            const char* video_layout_group) {
...
void conference_video_canvas_del_fnode_layer(conference_obj_t* conference,
...
void conference_video_canvas_set_fnode_layer(mcu_canvas_t* canvas, conference_file_node_t* fnode,
                                             int idx) {
    mcu_layer_t* layer = NULL;
    ...
    if (idx == -1) {
        ...
        if (idx < 0) {
            for (i = 0; i < canvas->total_layers; i++) {
                ...
                if (xlayer->fnode ||
                    (xlayer->geometry.res_id &&
                     (!fnode->res_id || strcmp(xlayer->geometry.res_id, fnode->res_id))) ||
                    xlayer->member_id) {
                    continue;
                }
...
void conference_video_launch_muxing_write_thread(conference_member_t* member) {
    ...
    if (!member->video_muxing_write_thread) {
        ...
        switch_thread_create(&member->video_muxing_write_thread, thd_attr,
                             conference_video_muxing_write_thread_run, member, member->pool);
    }
    switch_mutex_unlock(conference_globals.hash_mutex);
}

void conference_video_launch_muxing_thread(conference_obj_t* conference, mcu_canvas_t* canvas,
                                           int super) {
    switch_threadattr_t* thd_attr = NULL;
    ...
    if (!canvas->video_muxing_thread) {
        ...
        switch_thread_create(&canvas->video_muxing_thread, thd_attr,
            super ? conference_video_super_muxing_thread_run : conference_video_muxing_thread_run,
            canvas, conference->pool);
    }
    switch_mutex_unlock(conference_globals.hash_mutex);
}
...
void conference_video_launch_layer_thread(conference_member_t* member) {
    ...
    if (!member->video_layer_thread) {
        ...
        // switch_threadattr_priority_set(thd_attr, SWITCH_PRI_REALTIME);
        switch_threadattr_stacksize_set(thd_attr, SWITCH_THREAD_STACKSIZE);
        switch_thread_create(&member->video_layer_thread, thd_attr,
                             conference_video_layer_thread_run, member, member->pool);
    }
    switch_mutex_unlock(conference_globals.hash_mutex);
}

void* SWITCH_THREAD_FUNC conference_video_muxing_write_thread_run(switch_thread_t* thread,
                                                                  void* obj) {
    conference_member_t* member = (conference_member_t*)obj;
    void* pop;
        ...
        if (pop_status == SWITCH_STATUS_SUCCESS) {
            ...
            if ((switch_size_t)pop != 1) {
                ...
                if (switch_test_flag(frame, SFF_ENCODED)) {
                    ...
                    switch_core_session_write_video_frame(member->session, frame,
...
void conference_video_member_video_mute_banner(switch_image_t* img, conference_member_t* member) {
    ...
    if ((var = switch_channel_get_variable_dup(member->channel, "video_mute_banner", SWITCH_FALSE,
                                               -1))) {
        text = var;
    ...
    switch_snprintf(text_str, sizeof(text_str), "%s:%s:%s:%s%s:%s", fg, bg, font_face, font_scale,
                    font_scale_percentage, text);
...
void conference_video_check_recording(conference_obj_t* conference, mcu_canvas_t* canvas,
                                      switch_frame_t* frame) {
    conference_member_t* imember;
    ...
    for (imember = conference->members; imember; imember = imember->next) {
        ...
        if (!conference_utils_test_flag(conference, CFLAG_PERSONAL_CANVAS) && canvas &&
            imember->canvas_id != canvas->canvas_id) {
            continue;
        }

        if (switch_test_flag((&imember->rec->fh), SWITCH_FILE_OPEN) &&
            !switch_test_flag((&imember->rec->fh), SWITCH_FILE_PAUSE) &&
...
void conference_video_check_avatar(conference_member_t* member, switch_bool_t force) {
    ...
    if (!force && switch_channel_test_flag(member->channel, CF_VIDEO_READY) &&
        ...
        if ((var = switch_channel_get_variable_dup(member->channel, "video_no_video_avatar_png",
                                                   SWITCH_FALSE, -1))) {
            avatar = var;
        }
        novid++;
    ...
    if ((var = switch_channel_get_variable_dup(member->channel, "video_avatar_png", SWITCH_FALSE,
                                               -1))) {
        avatar = var;
    }

    if (conference_utils_test_flag(member->conference, CFLAG_VIDEO_REQUIRED_FOR_CANVAS) ||
        conference_utils_test_flag(member->conference, CFLAG_VIDEO_MUTE_EXIT_CANVAS)) {
        avatar = NULL;
        force = 0;
    }
        ...
        if (status == SWITCH_STATUS_SUCCESS) {
            if (fnode->layer_lock > -1 && layer->member_id > 0) {
                ...
                if (file_frame.img && file_frame.img->fmt != SWITCH_IMG_FMT_I420) {
                    switch_image_t* tmp = switch_img_alloc(
                        NULL, SWITCH_IMG_FMT_I420, file_frame.img->d_w, file_frame.img->d_h, 1);
...
void conference_video_fnode_check(conference_file_node_t* fnode, int canvas_id) {
    ...
    if (switch_core_file_has_video(&fnode->fh, SWITCH_TRUE) &&
        switch_core_file_read_video(&fnode->fh, NULL, SVR_CHECK) == SWITCH_STATUS_BREAK) {
        int full_screen = 0;
        char* res_id = NULL;
...
switch_status_t conference_video_find_layer(conference_obj_t* conference, mcu_canvas_t* canvas,
                                            conference_member_t* member, mcu_layer_t** layerP) {
    ...
    if ((canvas->layers_used < canvas->total_layers || (avatar_layers && !member->avatar_png_img) ||
         conference_utils_member_test_flag(member, MFLAG_MOD)) &&
        (member->avatar_png_img ||
         (switch_core_session_media_flow(member->session, SWITCH_MEDIA_TYPE_VIDEO) !=
              SWITCH_MEDIA_FLOW_SENDONLY &&
          switch_core_session_media_flow(member->session, SWITCH_MEDIA_TYPE_VIDEO) !=
        ...
        if (!layer) {
            for (i = 0; i < canvas->total_layers; i++) {
                ...
                if (xlayer->geometry.flooronly && !xlayer->fnode && !xlayer->geometry.fileonly &&
                    !xlayer->geometry.res_id) {
        ...
        if (!layer) {
            for (i = 0; i < canvas->total_layers; i++) {
                ...
                if ((!xlayer->member_id ||
                     (!member->avatar_png_img && xlayer->is_avatar && !xlayer->geometry.role_id &&
                      (conference->canvas_count > 1 ||
                       xlayer->member_id != (int)conference->video_floor_holder))) &&
                    !xlayer->fnode && !xlayer->geometry.fileonly && !xlayer->geometry.res_id &&
                    !xlayer->geometry.flooronly) {
...
void conference_video_next_canvas(conference_member_t* imember) {
    ...
    for (x = 0; x < imember->conference->canvas_count; x++) {
        ...
        if (imember->conference->canvases[y]->video_count <
            imember->conference->canvases[y]->total_layers) {
            imember->canvas_id = y;
            break;
        }
    }
...
void conference_video_pop_next_image(conference_member_t* member, switch_image_t** imgP) {
    ...
    // conference_utils_member_test_flag(member, MFLAG_ACK_VIDEO)) {
    //	switch_img_free(&member->avatar_png_img);
    // }
    ...
    if (switch_channel_test_flag(member->channel, CF_VIDEO_READY)) {
        ...
        if (conference_utils_member_test_flag(member, MFLAG_CAN_BE_SEEN) &&
            !conference_utils_member_test_flag(member, MFLAG_HOLD) && member->video_layer_id > -1 &&
            switch_core_session_media_flow(member->session, SWITCH_MEDIA_TYPE_VIDEO) !=
                SWITCH_MEDIA_FLOW_SENDONLY &&
            switch_core_session_media_flow(member->session, SWITCH_MEDIA_TYPE_VIDEO) !=
                SWITCH_MEDIA_FLOW_INACTIVE) {
            ...
            if (img) {
                ...
                if (member->blanks == (int)member->conference->video_fps.fps ||
                    (member->blanks % (int)(member->conference->video_fps.fps * 10)) == 0) {
                    switch_core_session_request_video_refresh(member->session);
                    member->good_img = 0;
                }
...
void conference_video_set_incoming_bitrate(conference_member_t* member, int kps,
    ...
    if (!force && kps < member->managed_kps && member->conference->auto_kps_debounce) {
        member->auto_kps_debounce_ticks =
            member->conference->auto_kps_debounce / member->conference->video_fps.ms;
        switch_log_printf(
            SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG1, "%s setting bitrate debounce timer to %dms\n",
    ...
    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG1,
                      "%s sending message to set bitrate to %dkps\n",
...
void conference_video_check_auto_bitrate(conference_member_t* member, mcu_layer_t* layer) {
    ...
    if (!switch_channel_test_flag(member->channel, CF_VIDEO_READY) || !vid_params.width ||
        !vid_params.height) {
        return;
    }
    ...
    if (vid_params.width != member->vid_params.width ||
        vid_params.height != member->vid_params.height) {
        switch_core_session_request_video_refresh(member->session);
        conference_video_clear_managed_kps(member);
    }
    ...
    if (member->vid_params.width && member->vid_params.height &&
        (screen_w > member->vid_params.width || screen_h > member->vid_params.height)) {
        // switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG1, "%s Layer is bigger than input
        // res, limit size to %dx%d\n", switch_channel_get_name(member->channel),
        // member->vid_params.width, member->vid_params.height);
    ...
    if ((kps_in = switch_calc_bitrate(vid_params.width, vid_params.height,
                                      member->conference->video_quality,
                                      member->conference->video_fps.fps)) < 512) {
        kps_in = 512;
    }

    if (layer) {
        kps = switch_calc_bitrate(screen_w, screen_h, member->conference->video_quality,
                                  member->conference->video_fps.fps);
    } else {
        kps = kps_in;
    }
    ...
    if (member->conference->force_bw_in || member->force_bw_in) {
        ...
        if (layer && conference_utils_member_test_flag(member, MFLAG_CAN_BE_SEEN) &&
            ...
            if (layer->screen_w != screen_w) {
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG1,
                                  "%s auto-setting bitrate to %dkps (max res %dx%d) to accommodate "
                                  "%dx%d resolution\n",
                                  switch_channel_get_name(member->channel), kps, screen_w, screen_h,
                                  layer->screen_w, layer->screen_h);
            } else {
                switch_log_printf(
                    SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG1,
                    "%s auto-setting bitrate to %dkps to accommodate %dx%d resolution\n",
                    switch_channel_get_name(member->channel), kps, screen_w, screen_h);
            }
        } else {
            kps = min;
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG1,
                              "%s auto-setting bitrate to %dkps because the user is not visible\n",
                              switch_channel_get_name(member->channel), kps);
        }
    }
...
static void wait_for_canvas(mcu_canvas_t* canvas) {
    ...
    for (;;) {
        ...
        for (i = 0; i < canvas->total_layers; i++) {
            ...
            if (layer->need_patch) {
                if (layer->member_id && layer->member &&
                    conference_utils_member_test_flag(layer->member, MFLAG_RUNNING) &&
                    layer->member->fb) {
...
static void personal_attach(mcu_layer_t* layer, conference_member_t* member) {
    ...
    if (layer->member_id != (int)member->id) {
        ...
        if (member->channel) {
            var = NULL;
            if (member->video_banner_text ||
                (var = switch_channel_get_variable_dup(member->channel, "video_banner_text",
                                                       SWITCH_FALSE, -1))) {
                conference_video_layer_set_banner(member, layer, var);
            }
...
conference_video_muxing_thread_run(switch_thread_t* thread, void* obj) {
    ...
    while (conference_globals.running && !conference_utils_test_flag(conference, CFLAG_DESTRUCT) &&
           conference_utils_test_flag(conference, CFLAG_VIDEO_MUXING)) {
        switch_bool_t need_refresh = SWITCH_FALSE, send_keyframe = SWITCH_FALSE,
                      need_reset = SWITCH_FALSE;
        switch_time_t now;
        int min_members = 0;
        int count_changed = 0;
        ...
        if (!personal) {
            if (canvas->new_vlayout &&
                switch_mutex_trylock(conference->canvas_mutex) == SWITCH_STATUS_SUCCESS) {
        ...
        if (canvas->video_timer_reset) {
            ...
            switch_core_timer_init(&canvas->timer, "soft", conference->video_fps.ms,
                                   conference->video_fps.samples, NULL);
            canvas->send_keyframe = 1;
        }
        ...
        if (conference->async_fnode &&
            switch_core_file_has_video(&conference->async_fnode->fh, SWITCH_TRUE)) {
            check_async_file = 1;
            file_count++;
            video_count++;
            if (!files_playing) {
        ...
        for (imember = conference->members; imember; imember = imember->next) {
            ...
            int no_av =
                conference_utils_test_flag(imember->conference, CFLAG_VIDEO_REQUIRED_FOR_CANVAS);
            ...
            if (imember->channel && switch_channel_ready(imember->channel) &&
                ...
                conference_utils_member_test_flag(imember, MFLAG_RUNNING) && (!no_muted || seen) &&
                (!(no_av && imember->avatar_png_img)) && imember->canvas_id == canvas->canvas_id &&
        ...
        for (imember = conference->members; imember; imember = imember->next) {
            ...
            if (imember->watching_canvas_id == canvas->canvas_id &&
                switch_channel_test_flag(imember->channel, CF_VIDEO_REFRESH_REQ)) {
            ...
            if (conference_utils_test_flag(conference, CFLAG_MINIMIZE_VIDEO_ENCODING) &&
                ...
                if (switch_channel_test_flag(imember->channel, CF_VIDEO_READY)) {
                    if (imember->video_codec_index < 0 &&
                        (check_codec =
                             switch_core_session_get_video_write_codec(imember->session))) {
                        for (i = 0; canvas->write_codecs[i] &&
                                    switch_core_codec_ready(&canvas->write_codecs[i]->codec) &&
                                    i < MAX_MUX_CODECS;
                             i++) {
                            if (check_codec->implementation->codec_id ==
                                canvas->write_codecs[i]->codec.implementation->codec_id) {
                                if ((zstr(imember->video_codec_group) &&
                                     zstr(canvas->write_codecs[i]->video_codec_group)) ||
                                    (!strcmp(switch_str_nil(imember->video_codec_group),
                                             switch_str_nil(
                                                 canvas->write_codecs[i]->video_codec_group)))) {
                        ...
                        if (imember->video_codec_index < 0) {
                            ...
                            if (switch_core_codec_copy(check_codec, &canvas->write_codecs[i]->codec,
                                ...
                                switch_log_printf(
                                    ...
                                    imember->video_codec_group ? imember->video_codec_group
                                                               : "_none_");
                                ...
                                if (imember->video_codec_group) {
                                    const char* gname = switch_core_sprintf(
                                        conference->pool, "group-%s", imember->video_codec_group);
                                    const char* val = NULL;

                                    canvas->write_codecs[i]->video_codec_group = switch_core_strdup(
                                        conference->pool, imember->video_codec_group);
                                    ...
                                    if ((val = conference_get_variable(conference, gname))) {
                                        ...
                                        switch_snprintf(cid, sizeof(cid), "%d",
                                                        canvas->canvas_id + 1);

                                        argv[2] = (char*)val;
                                        argv[3] = imember->video_codec_group;
                                        argv[4] = cid;
                                        conference_api_sub_vid_bandwidth(conference, &stream, 5,
                                                                         argv);
                                ...
                                canvas->write_codecs[i]->frame.packet =
                                    switch_core_alloc(conference->pool, buflen);
                                canvas->write_codecs[i]->frame.data =
                                    ((uint8_t*)canvas->write_codecs[i]->frame.packet) + 12;
                                canvas->write_codecs[i]->frame.packetlen = buflen;
                                canvas->write_codecs[i]->frame.buflen = buflen - 12;
                                if (conference->scale_h264_canvas_width > 0 &&
                                    conference->scale_h264_canvas_height > 0 &&
                                    !strcmp(check_codec->implementation->iananame, "H264")) {
                                    int32_t bw = -1;

                                    canvas->write_codecs[i]->fps_divisor =
                                        conference->scale_h264_canvas_fps_divisor;
                                    canvas->write_codecs[i]->scaled_img =
                                        switch_img_alloc(NULL, SWITCH_IMG_FMT_I420,
                                                         conference->scale_h264_canvas_width,
                                                         conference->scale_h264_canvas_height, 16);

                                    if (conference->scale_h264_canvas_bandwidth) {
                                        if (strcasecmp(conference->scale_h264_canvas_bandwidth,
                                                       "auto")) {
                                            bw = switch_parse_bandwidth_string(
                                                conference->scale_h264_canvas_bandwidth);
                                        }
                                    }
                                    ...
                                    if (bw == -1) {
                                        ...
                                        bw = switch_calc_bitrate(
                                            conference->scale_h264_canvas_width,
                                            conference->scale_h264_canvas_height,
                                            conference->video_quality, fps);
                                    }

                                    switch_core_codec_control(&canvas->write_codecs[i]->codec,
                                                              SCC_VIDEO_BANDWIDTH, SCCT_INT, &bw,
                                                              SCCT_NONE, NULL, NULL, NULL);
                                }
            ...
            if (conference->conference_video_mode == CONF_VIDEO_MODE_MUX &&
                conference->canvas_count == 1 && canvas->layout_floor_id > -1 &&
                imember->id == conference->video_floor_holder &&
            ...
            if (!zstr(imember->video_role_id) && canvas->role_count) {
                ...
                if (!tlayer || (zstr(tlayer->geometry.role_id) ||
                                strcmp(tlayer->geometry.role_id, imember->video_role_id))) {
                    for (i = 0; i < canvas->total_layers; i++) {
                        mcu_layer_t* xlayer = &canvas->layers[i];

                        if (!zstr(imember->video_role_id) && !zstr(xlayer->geometry.role_id) &&
                            !strcmp(xlayer->geometry.role_id, imember->video_role_id)) {
            ...
            if (imember->avatar_png_img) {
                if (layer) {
                    if (!imember->avatar_patched || !layer->cur_img) {
                        ...
                        // layer->is_avatar = 1;
                        switch_img_free(&layer->cur_img);
                        switch_img_letterbox(imember->avatar_png_img, &layer->cur_img,
                                             layer->screen_w, layer->screen_h,
                                             conference->video_letterbox_bgcolor);
            ...
            if (!layer &&
                (!conference_utils_test_flag(imember->conference,
                 ...
                 ((switch_channel_test_flag(imember->channel, CF_VIDEO_READY) &&
                   ...
                   switch_core_session_media_flow(imember->session, SWITCH_MEDIA_TYPE_VIDEO) !=
                       SWITCH_MEDIA_FLOW_INACTIVE)))) {
                switch_status_t ret =
                    conference_video_find_layer(conference, canvas, imember, &layer);
                    ...
                    if (!layer->mute_patched) {
                        ...
                        if (imember->video_mute_img || layer->mute_img) {
                            ...
                            if (!layer->mute_img) {
                                if (imember->video_mute_img) {
                                    // layer->mute_img =
                                    // switch_img_read_png(imember->video_mute_png,
                                    // SWITCH_IMG_FMT_I420);
                                    switch_img_letterbox(imember->video_mute_img, &layer->mute_img,
                                                         layer->screen_w, layer->screen_h,
                                                         conference->video_letterbox_bgcolor);
                            ...
                            if (layer->mute_img) {
                                ...
                                conference_video_scale_and_patch(layer, layer->mute_img,
                                                                 SWITCH_FALSE);
                            }
                        }
        ...
        if (personal) {
            ...
            for (imember = conference->members; imember; imember = imember->next) {
                ...
                if (!imember->canvas) {
                    if ((vlayout =
                             conference_video_get_layout(conference, conference->video_layout_name,
                                                         canvas->video_layout_group))) {
                ...
                if (conference->new_personal_vlayout) {
                    conference_video_init_canvas_layers(conference, imember->canvas,
                                                        conference->new_personal_vlayout,
                                                        SWITCH_FALSE);
                    layout_applied++;
                }
                ...
                if (count_changed) {
                    ...
                    if (conference->video_layout_group &&
                        ...
                        if ((vlayout =
                                 conference_video_find_best_layout(conference, lg, total, 0))) {
                            conference_video_init_canvas_layers(conference, imember->canvas,
                                                                vlayout, SWITCH_FALSE);
                        }
                    }

                    if (imember->channel &&
                        !switch_channel_test_flag(imember->channel, CF_VIDEO_BITRATE_UNMANAGABLE) &&
                        conference_utils_test_flag(conference,
                                                   CFLAG_MANAGE_INBOUND_VIDEO_BITRATE)) {
                        switch_core_media_get_vid_params(imember->session, &vid_params);
                        kps = switch_calc_bitrate(vid_params.width, vid_params.height,
                                                  conference->video_quality,
                                                  imember->conference->video_fps.fps);
            ...
            if (check_async_file && conference->async_fnode) {
                switch_status_t st = switch_core_file_read_video(&conference->async_fnode->fh,
                                                                 &file_frame, SVR_FLUSH);
            ...
            if (check_file && conference->fnode) {
                switch_status_t st =
                    switch_core_file_read_video(&conference->fnode->fh, &file_frame, SVR_FLUSH);
            ...
            for (imember = conference->members; imember; imember = imember->next) {
                ...
                if (!imember->rec &&
                    (!imember->session || !switch_channel_test_flag(imember->channel, CF_VIDEO) ||
                     !imember->canvas ||
                     (switch_core_session_media_flow(imember->session, SWITCH_MEDIA_TYPE_VIDEO) ==
                          SWITCH_MEDIA_FLOW_SENDONLY ||
                      switch_core_session_media_flow(imember->session, SWITCH_MEDIA_TYPE_VIDEO) ==
                ...
                if (files_playing && !file_count) {
                    ...
                    while (i < imember->canvas->total_layers) {
                        ...
                        switch_img_fill(layer->canvas->img, layer->x_pos, layer->y_pos,
                                        layer->screen_w, layer->screen_h, &layer->canvas->bgcolor);
                    }
                    i = 0;
                }

                if (!file_count && imember->canvas->layout_floor_id > -1 &&
                    imember->conference->video_floor_holder &&
                    imember->id != imember->conference->video_floor_holder) {
                    if ((omember = conference_member_get(
                             imember->conference, imember->conference->video_floor_holder))) {
                        if (conference->members_with_video + conference->members_with_avatar == 1 ||
                            imember != omember) {
                ...
                for (omember = conference->members; omember; omember = omember->next) {
                    ...
                    if (!omember->session ||
                        !switch_channel_test_flag(omember->channel, CF_VIDEO_READY) ||
                        (conference_utils_test_flag(omember->conference,
                                                    CFLAG_VIDEO_MUTE_EXIT_CANVAS) &&
                         !conference_utils_member_test_flag(omember, MFLAG_CAN_BE_SEEN)) ||
                        switch_core_session_media_flow(omember->session, SWITCH_MEDIA_TYPE_VIDEO) ==
                            SWITCH_MEDIA_FLOW_SENDONLY ||
                        switch_core_session_media_flow(omember->session, SWITCH_MEDIA_TYPE_VIDEO) ==
                            SWITCH_MEDIA_FLOW_INACTIVE) {
                        continue;
                    ...
                    if (file_count &&
                        (conference->members_with_video + conference->members_with_avatar == 1)) {
                        floor_layer = NULL;
                    }

                    if (!file_count && floor_layer &&
                        omember->id == conference->video_floor_holder) {
                        layer = floor_layer;
                    } else {
                    ...
                    if (layer) {
                        if (use_img && !omember->avatar_png_img) {
                            ...
                            if (!layer->avatar_patched) {
                                if (omember->avatar_png_img) {
                                    switch_img_letterbox(omember->avatar_png_img, &layer->cur_img,
                                                         layer->screen_w, layer->screen_h,
                                                         conference->video_letterbox_bgcolor);
                                }
                        ...
                        if (layer) {
                            if (conference_utils_member_test_flag(omember, MFLAG_CAN_BE_SEEN) &&
                                !conference_utils_member_test_flag(imember, MFLAG_HOLD)) {
                                layer->mute_patched = 0;
                            } else if (!conference_utils_test_flag(omember->conference,
                                                                   CFLAG_VIDEO_MUTE_EXIT_CANVAS)) {
                                if (!layer->mute_patched) {
                                    switch_image_t* mute_img = omember->video_mute_img
                                                                   ? omember->video_mute_img
                                                                   : omember->pcanvas_img;
                                    ...
                                    if (mute_img) {
                                        ...
                                        switch_img_fit(&tmp, layer->screen_w, layer->screen_h,
                                                       SWITCH_FIT_SIZE);
                                        // conference_video_member_video_mute_banner(imember->canvas,
                                        // layer, imember);
            ...
            for (imember = conference->members; imember; imember = imember->next) {
                ...
                if (imember->rec) {
                    if (switch_core_file_write_video(&imember->rec->fh, &write_frame) !=
                        SWITCH_STATUS_SUCCESS) {
                        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
                                          "Video Write Failed\n");
                        conference_utils_member_clear_flag_locked(imember, MFLAG_RUNNING);
                    ...
                    if (switch_frame_buffer_dup(imember->fb, &write_frame, &dupframe) ==
                        SWITCH_STATUS_SUCCESS) {
                        if (switch_frame_buffer_trypush(imember->fb, dupframe) !=
                            SWITCH_STATUS_SUCCESS) {
                            switch_frame_buffer_free(imember->fb, &dupframe);
            ...
            switch_mutex_lock(conference->file_mutex);
            if (conference->async_fnode &&
                (conference->async_fnode->canvas_id == canvas->canvas_id ||
                 conference->async_fnode->canvas_id == -1)) {
            ...
            if (!canvas->playing_video_file) {
                for (i = 0; i < canvas->total_layers; i++) {
                    ...
                    if (!layer->mute_patched && (layer->member_id > -1 || layer->fnode) &&
                        layer->cur_img && layer->tagged && !layer->geometry.overlap) {
                        if (canvas->refresh) {
                            layer->refresh = 1;
                            canvas->refresh++;
                        }
                ...
                for (i = 0; i < canvas->total_layers; i++) {
                    ...
                    if ((layer->member_id > -1 || layer->fnode) && layer->cur_img &&
                        layer->geometry.overlap) {
                        layer->mute_patched = 0;
                        layer->banner_patched = 0;
            ...
            if (conference->fnode && switch_test_flag(&conference->fnode->fh, SWITCH_FILE_OPEN)) {
                if (canvas->overlay_video_file) {
                    if (switch_core_file_read_video(&conference->fnode->fh, &write_frame,
                                                    SVR_FLUSH) == SWITCH_STATUS_SUCCESS) {
                        switch_img_free(&file_img);
                        switch_img_fit(&write_frame.img, canvas->img->d_w, canvas->img->d_h,
                                       SWITCH_FIT_SIZE);
                        ...
                        if (file_img->fmt == SWITCH_IMG_FMT_ARGB) {
                            ...
                            if (conference->fnode->filters & SCV_FILTER_GRAY_BG) {
                                switch_img_gray(overlay_img, 0, 0, overlay_img->d_w,
                                                overlay_img->d_h);
                            }

                            if (conference->fnode->filters & SCV_FILTER_SEPIA_BG) {
                                switch_img_sepia(overlay_img, 0, 0, overlay_img->d_w,
                                                 overlay_img->d_h);
                            }
                    ...
                    if (switch_core_file_read_video(&conference->fnode->fh, &write_frame,
                        ...
                        switch_img_free(&file_img);
                        switch_img_letterbox(write_frame.img, &tmp, canvas->img->d_w,
                                             canvas->img->d_h, "#000000");
                        if (tmp) {
            ...
            if (min_members &&
                conference_utils_test_flag(conference, CFLAG_MINIMIZE_VIDEO_ENCODING)) {
                for (i = 0;
                     canvas->write_codecs[i] &&
                     switch_core_codec_ready(&canvas->write_codecs[i]->codec) && i < MAX_MUX_CODECS;
                     i++) {
                    canvas->write_codecs[i]->frame.img = write_img;
                    conference_video_write_canvas_image_to_codec_group(
                        conference, canvas, canvas->write_codecs[i], i, timestamp, need_refresh,
                        send_keyframe, need_reset);
                }
            }
            ...
            for (imember = conference->members; imember; imember = imember->next) {
                ...
                switch_set_flag(&write_frame,
                                SFF_RAW_RTP | SFF_USE_VIDEO_TIMESTAMP | SFF_RAW_RTP_PARSE_FRAME);
                write_frame.img = write_img;
                write_frame.packet = packet;
                write_frame.data = ((uint8_t*)packet) + 12;
                write_frame.datalen = 0;
                ...
                // switch_core_session_write_video_frame(imember->session, &write_frame,
                // SWITCH_IO_FLAG_NONE, 0);
...
void* SWITCH_THREAD_FUNC conference_video_super_muxing_thread_run(switch_thread_t* thread,
                                                                  void* obj) {
    mcu_canvas_t* canvas = (mcu_canvas_t*)obj;
    conference_obj_t* conference = canvas->conference;
    conference_member_t* imember;
    ...
    while (conference_globals.running && !conference_utils_test_flag(conference, CFLAG_DESTRUCT) &&
        ...
        if (count_changed) {
            ...
            if ((lg = switch_core_hash_find(conference->layout_group_hash,
                                            CONFERENCE_MUX_DEFAULT_SUPER_LAYOUT))) {
        ...
        for (imember = conference->members; imember; imember = imember->next) {
            ...
            if (!imember->session ||
                (!switch_channel_test_flag(imember->channel, CF_VIDEO_READY) &&
                 !imember->avatar_png_img) ||
                conference_utils_test_flag(conference, CFLAG_PERSONAL_CANVAS) ||
                switch_core_session_read_lock(imember->session) != SWITCH_STATUS_SUCCESS) {
                continue;
            }
            ...
            if (layer_idx < canvas->total_layers) {
                ...
                if (img) {
                    if (conference->super_canvas_label_layers) {
                        ...
                        switch_snprintf(str, sizeof(str), "%sCanvas %d", format,
                                        jcanvas->canvas_id + 1);
...
void conference_video_set_floor_holder(conference_obj_t* conference, conference_member_t* member,
                                       switch_bool_t force) {
    switch_event_t* event;
    ...
    if (member && conference_utils_member_test_flag(member, MFLAG_DED_VID_LAYER)) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
                          "Setting floor not allowed on a member in a dedicated layer\n");
    }
    ...
    if (conference->video_floor_holder) {
        if (member && conference->video_floor_holder == member->id) {
            ...
            if (conference->conference_video_mode == CONF_VIDEO_MODE_MUX &&
                conference->last_video_floor_holder &&
                (imember =
                     conference_member_get(conference, conference->last_video_floor_holder))) {
                switch_core_session_request_video_refresh(imember->session);
    ...
    if (!member) {
        switch_mutex_lock(conference->member_mutex);
        for (imember = conference->members; imember; imember = imember->next) {
            if (imember->id != conference->video_floor_holder && imember->channel &&
                switch_channel_test_flag(imember->channel, CF_VIDEO_READY)) {
                member = imember;
                break;
            }
...
void conference_video_write_frame(conference_obj_t* conference, conference_member_t* floor_holder,
    ...
    if (vid_frame->img && conference->canvases[0]) {
        ...
        frame_img = switch_img_alloc(NULL, SWITCH_IMG_FMT_I420, conference->canvases[0]->width,
                                     conference->canvases[0]->height, 1);
        conference_video_reset_image(frame_img, &conference->canvases[0]->bgcolor);
        switch_img_find_position(POS_CENTER_MID, frame_img->d_w, frame_img->d_h, tmp_img->d_w,
                                 tmp_img->d_h, &x, &y);
    ...
    for (imember = conference->members; imember; imember = imember->next) {
        ...
        if (switch_channel_test_flag(imember->channel, CF_VIDEO_READY)) {
            ...
            if (conference->canvases[0] &&
                conference_utils_test_flag(imember->conference, CFLAG_VIDEO_BRIDGE_FIRST_TWO)) {
                if (switch_channel_test_flag(imember->channel, CF_VIDEO_READY) &&
                    (conference->members_with_video == 1 || imember != floor_holder)) {
                    send_frame = 1;
                }
                       ...
                       (conference_utils_test_flag(conference, CFLAG_VID_FLOOR_LOCK) ||
                        !(imember->id == imember->conference->video_floor_holder &&
                          imember->conference->last_video_floor_holder))) {
                send_frame = 1;
            }
            ...
            if (send_frame) {
                if (vid_frame->img) {
                    if (conference->canvases[0]) {
                        ...
                        if (imember->fb) {
                            if (switch_frame_buffer_dup(imember->fb, &tmp_frame, &dupframe) ==
                                SWITCH_STATUS_SUCCESS) {
                                if (switch_frame_buffer_trypush(imember->fb, dupframe) !=
                                    SWITCH_STATUS_SUCCESS) {
                                    switch_frame_buffer_free(imember->fb, &dupframe);
                            ...
                            switch_core_session_write_video_frame(imember->session, &tmp_frame,
                                                                  SWITCH_IO_FLAG_NONE, 0);
                        }
                    } else {
                        switch_core_session_write_video_frame(imember->session, vid_frame,
                                                              SWITCH_IO_FLAG_NONE, 0);
                    }
                } else {
...
switch_status_t conference_video_thread_callback(switch_core_session_t* session,
    ...
    if (member->conference->async_fnode &&
        switch_core_file_has_video(&member->conference->async_fnode->fh, SWITCH_TRUE)) {
        files_playing = 1;
    }

    if (member->conference->fnode &&
        switch_core_file_has_video(&member->conference->fnode->fh, SWITCH_TRUE)) {
        files_playing = 1;
    }
    ...
    if (conference_utils_test_flag(member->conference, CFLAG_VIDEO_BRIDGE_FIRST_TWO)) {
        if (member->conference->members_seeing_video < 3 && !files_playing &&
            member->conference->mux_paused) {
    ...
    if (conference_utils_test_flag(member->conference, CFLAG_VIDEO_MUXING)) {
        ...
        if (frame->img && (((member->video_layer_id > -1) && canvas_id > -1) || member->canvas) &&
            ...
            if (conference_utils_member_test_flag(member, MFLAG_FLIP_VIDEO) ||
                conference_utils_member_test_flag(member, MFLAG_ROTATE_VIDEO) ||
                conference_utils_member_test_flag(member, MFLAG_MIRROR_VIDEO)) {
    ...
    for (rel = member->relationships; rel; rel = rel->next) {
        ...
        if ((imember = conference_member_get(member->conference, rel->id)) &&
            conference_utils_member_test_flag(imember, MFLAG_RECEIVING_VIDEO)) {
            // switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "%s %d->%d %d\n", name,
            // member->id, imember->id, frame->datalen);
    ...
    if (member->id == member->conference->video_floor_holder) {
        conference_video_write_frame(member->conference, member, frame);
        conference_video_check_recording(member->conference, NULL, frame);
    } else if (!conference_utils_test_flag(member->conference, CFLAG_VID_FLOOR_LOCK) &&
               member->id == member->conference->last_video_floor_holder) {
        conference_member_t* fmember;

        if ((fmember = conference_member_get(member->conference,
                                             member->conference->video_floor_holder))) {
            if (!conference_utils_member_test_flag(fmember, MFLAG_RECEIVING_VIDEO))
                switch_core_session_write_video_frame(fmember->session, frame, SWITCH_IO_FLAG_NONE,
                                                      0);
