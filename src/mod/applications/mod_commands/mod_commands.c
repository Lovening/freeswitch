...
static switch_status_t select_url(const char* user, const char* domain, const char* concat,
                                  const char* exclude_contact, switch_stream_handle_t* stream) {
    struct cb_helper cb;
...
static stream_format set_format(stream_format* format, switch_stream_handle_t* stream) {
    ...
    if (stream->param_event &&
        (format->http = switch_event_get_header(stream->param_event, "HTTP-URI"))) {
...
SWITCH_STANDARD_API(say_string_function) {
    ...
    if (lbuf &&
        ...
        switch_ivr_say_string(session, argv[1], ext, tosay, argv[0], argv[2], argv[3],
                              (argc == 6) ? argv[4] : NULL, &string);
        if (string) {
...
static void dump_user(struct user_struct* us) {
    ...
    if (!strcasecmp(utype, "pointer")) {
        if (switch_xml_locate_user_in_domain(uname, x_domain_tag, &ux, NULL) ==
    ...
    if ((status = switch_api_execute("sofia_contact", apip, NULL, &apistream)) !=
        SWITCH_STATUS_SUCCESS) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
                          "sofia_contact '%s' failed. status: %d \n", apip, status);
        goto end;
    }
    stream->write_function(stream, "%s|%s|%s|%s|%s|%s|%s|%s\n",
                           switch_xml_attr_soft(x_user_tag, "id"), user_context, dname, gname,
                           apistream.data, callgroup, effective_caller_id_name,
...
SWITCH_STANDARD_API(list_users_function) {
    ...
    stream->write_function(stream,
                           "userid|context|domain|group|contact|callgroup|effective_caller_id_name|"
                           "effective_caller_id_number\n");

    if (switch_xml_locate(section, tag_name, key_name, key_value, &xml_root, &x_domains, NULL,
                          SWITCH_FALSE) == SWITCH_STATUS_SUCCESS) {
        struct user_struct us = {0};

        for (x_domain_tag = _domain ? x_domains : switch_xml_child(x_domains, "domain");
            ...
            if ((x_vars = switch_xml_child(x_domain_tag, "variables"))) {
                if ((x_var = switch_xml_find_child_multi(x_vars, "variable", "name", "user_context",
                                                         NULL))) {
                    us.context = (char*)switch_xml_attr_soft(x_var, "value");
                }
                if ((x_var = switch_xml_find_child_multi(x_vars, "variable", "name", "callgroup",
                                                         NULL))) {
                    us.callgroup = (char*)switch_xml_attr_soft(x_var, "value");
                }
                if ((x_var = switch_xml_find_child_multi(x_vars, "variable", "name",
            ...
            if ((gts = switch_xml_child(x_domain_tag, "groups"))) {
                for (gt = switch_xml_child(gts, "group"); gt; gt = gt->next) {
                    ...
                    if ((x_vars = switch_xml_child(gt, "variables"))) {
                        ...
                        if ((x_var = switch_xml_find_child_multi(
                                 x_vars, "variable", "name", "effective_caller_id_name", NULL))) {
                            us.effective_caller_id_name =
                                (char*)switch_xml_attr_soft(x_var, "value");
                        }
                        if ((x_var = switch_xml_find_child_multi(
                                 x_vars, "variable", "name", "effective_caller_id_number", NULL))) {
                            us.effective_caller_id_number =
                                (char*)switch_xml_attr_soft(x_var, "value");
                        }
                    }
...
SWITCH_STANDARD_API(version_function) {
    ...
    if (argc > 0 && switch_stristr("short", argv[0])) {
        stream->write_function(stream, "%s.%s.%s\n", switch_version_major(), switch_version_minor(),
...
SWITCH_STANDARD_API(nat_map_function) {
    ...
    if (!switch_nat_is_initialized()) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
                          "nat_map API called while NAT not initialized\n");
        goto error;
    }
    ...
    if (argv[0] && switch_stristr("add", argv[0])) {
        if (switch_nat_add_mapping((switch_port_t)atoi(argv[1]), proto, &external_port, sticky) ==
            SWITCH_STATUS_SUCCESS) {
...
usage:
    stream->write_function(stream,
                           "USAGE: nat_map [status|reinit|republish] | [add|del] <port> [tcp|udp] "
                           "[sticky] | [mapping] <enable|disable>");
...
SWITCH_STANDARD_API(timer_test_function) {
    ...
    switch_core_timer_next(&timer); /* Step timer once before testing results below, to get first
                                       timestamp as accurate as possible */
...
SWITCH_STANDARD_API(group_call_function) {
    ...
    if (!zstr(domain)) {
        ...
        if (switch_xml_locate_group(group_name, domain, &xml, &x_domain, &x_group, params) ==
            SWITCH_STATUS_SUCCESS) {
            ...
            if ((x_users = switch_xml_child(x_group, "users"))) {
                for (x_user = switch_xml_child(x_users, "user"); x_user; x_user = x_user->next) {
                    ...
                    char* d_dest = NULL;
                    switch_xml_t xml_for_pointer = NULL, x_domain_for_pointer = NULL,
                                 x_group_for_pointer = NULL, x_user_for_pointer = NULL;

                    my_x_user = x_user;

                    if (!strcmp(x_user_type, "pointer")) {
                        if (switch_xml_locate_user("id", id, domain, NULL, &xml_for_pointer,
                                                   &x_domain_for_pointer, &x_user_for_pointer,
                                                   &x_group_for_pointer,
                    ...
                    if ((x_params = switch_xml_child(x_domain, "params"))) {
                        for (x_param = switch_xml_child(x_params, "param"); x_param;
                             x_param = x_param->next) {
...
SWITCH_STANDARD_API(in_group_function) {
    ...
    if (switch_xml_locate_group(group, domain, &xml, &x_domain, &x_group, params) ==
        SWITCH_STATUS_SUCCESS) {
...
SWITCH_STANDARD_API(domain_data_function) {
    ...
    if (key && type &&
        switch_xml_locate_domain(domain, params, &xml_root, &x_domain) == SWITCH_STATUS_SUCCESS) {
        if (!strcmp(type, "attr")) {
...
SWITCH_STANDARD_API(user_data_function) {
    ...
    if (key && type &&
        switch_xml_locate_user_merged("id:number-alias", user, domain, NULL, &x_user, params) ==
            SWITCH_STATUS_SUCCESS) {
        if (!strcmp(type, "attr")) {
...
static switch_status_t _find_user(const char* cmd, switch_core_session_t* session,
                                  switch_stream_handle_t* stream, switch_bool_t tf) {
    ...
    if (switch_xml_locate_user_merged(key, user, domain, NULL, &x_user, NULL) !=
        SWITCH_STATUS_SUCCESS) {
        err = "can't find user";
        goto end;
    }
    ...
    if (session || tf) {
        ...
        if (x_user) {
            ...
            stream->write_function(stream, "%s%s%s", format.html ? "<pre>" : "", xmlstr,
                                   format.html ? "</pre>" : "");
...
SWITCH_STANDARD_API(stun_function) {
    ...
    if (zstr(stun_ip)) {
        ...
        if ((switch_stun_lookup(&ip, &port, stun_ip, stun_port, &error, pool)) ==
                SWITCH_STATUS_SUCCESS &&
            ip && port) {
...
SWITCH_STANDARD_API(expand_function) {
    ...
    if (*uuid) {
        if ((xsession = switch_core_session_locate(uuid))) {
            switch_channel_event_set_data(switch_core_session_get_channel(xsession),
                                          stream->param_event);
...
SWITCH_STANDARD_API(user_exists_function) { return _find_user(cmd, session, stream, SWITCH_TRUE); }

SWITCH_STANDARD_API(find_user_function) { return _find_user(cmd, session, stream, SWITCH_FALSE); }
...
SWITCH_STANDARD_API(xml_locate_function) {
    ...
    if (switch_xml_locate(section, tag, tag_attr_name, tag_attr_val, &xml, &obj, params,
                          SWITCH_FALSE) != SWITCH_STATUS_SUCCESS) {
...
typedef enum { O_NONE, O_EQ, O_NE, O_GT, O_GE, O_LT, O_LE } o_t;
...
SWITCH_STANDARD_API(status_function) {
    ...
    if (format.html) {
        ...
        switch_event_add_header_string(stream->param_event, SWITCH_STACK_BOTTOM, "HTTP-REFRESH",
                                       "true");
        if (format.api) {
    ...
    if (html) {
        ...
        stream->write_function(
            stream, "%s%s", switch_event_get_header(stream->param_event, "Event-Date-Local"), nl);
    }

    switch_core_measure_time(switch_core_uptime(), &duration);
    stream->write_function(stream,
                           "UP %u year%s, %u day%s, %u hour%s, %u minute%s, %u second%s, %u "
                           "millisecond%s, %u microsecond%s%s",
                           duration.yr, duration.yr == 1 ? "" : "s", duration.day,
                           duration.day == 1 ? "" : "s", duration.hr, duration.hr == 1 ? "" : "s",
                           duration.min, duration.min == 1 ? "" : "s", duration.sec,
                           duration.sec == 1 ? "" : "s", duration.ms, duration.ms == 1 ? "" : "s",
                           duration.mms, duration.mms == 1 ? "" : "s", nl);

    stream->write_function(stream, "FreeSWITCH (Version %s) is %s%s", switch_version_full_human(),
                           switch_core_ready() ? "ready" : "not ready", nl);

    stream->write_function(stream, "%" SWITCH_SIZE_T_FMT " session(s) since startup%s",
                           switch_core_session_id() - 1, nl);
    ...
    stream->write_function(stream, "%d session(s) - peak %d, last 5min %d %s",
                           switch_core_session_count(), sessions_peak, sessions_peak_fivemin, nl);
    ...
    stream->write_function(stream, "%d session(s) per Sec out of max %d, peak %d, last 5min %d %s",
                           last_sps, sps, max_sps, max_sps_fivemin, nl);
    stream->write_function(stream, "%d session(s) max%s", switch_core_session_limit(0), nl);
    stream->write_function(stream, "min idle cpu %0.2f/%0.2f%s", switch_core_min_idle_cpu(-1.0),
                           switch_core_idle_cpu(), nl);
...
#define CTL_SYNTAX                                                                                \
    "[api_expansion [on|off]|recover|send_sighup|hupall|pause [inbound|outbound]|resume "         \
    "[inbound|outbound]|shutdown [cancel|elegant|asap|now|restart]|uuid_version "                 \
    "[4|7]|sps|sps_peak_reset|sync_clock|sync_clock_when_idle|reclaim_mem|max_sessions|min_dtmf_" \
    "duration [num]|max_dtmf_duration [num]|default_dtmf_duration [num]|min_idle_cpu|loglevel "   \
    "[level]|debug_level [level]|mdns_resolve [enable|disable]]"
        ...
        if (!strcasecmp(argv[0], "hupall")) {
            ...
            for (x = 1; x < 5; x++) {
                if (argv[x]) {
                    ...
                    } else if (!strcasecmp(argv[x], "reincarnate") && (x + 1 < argc) &&
                               argv[x + 1] && !strcasecmp(argv[x + 1], "now")) {
                        ++x;
            ...
            if (arg) {
                ...
                stream->write_function(stream,
                                       "+OK clock will synchronize when there are no more calls\n");
            }
...
SWITCH_STANDARD_API(load_function) {
    ...
    if (switch_loadable_module_load_module((char*)SWITCH_GLOBAL_dirs.mod_dir, (char*)cmd,
...
SWITCH_STANDARD_API(unload_function) {
    ...
    if (switch_loadable_module_unload_module((char*)SWITCH_GLOBAL_dirs.mod_dir, (char*)cmd, force,
...
#define DUAL_TRANSFER_SYNTAX                              \
    "<uuid> <A-dest-exten>[/<A-dialplan>][/<A-context>] " \
...
SWITCH_STANDARD_API(uuid_drop_dtmf) {
    ...
    if (zstr(uuid)) {
        ...
        if ((tsession = switch_core_session_locate(uuid))) {
            ...
            file = switch_channel_get_variable_dup(channel, "drop_dtmf_masking_file", SWITCH_FALSE,
                                                   -1);
            digits = switch_channel_get_variable_dup(channel, "drop_dtmf_masking_digits",
                                                     SWITCH_FALSE, -1);

            stream->write_function(stream, "+OK %s is %s DTMF. mask_file: %s mask_digits: %s\n",
                                   uuid, is_on ? "dropping" : "not dropping", file ? file : "NONE",
                                   digits ? digits : "NONE");
...
static switch_bool_t true_enough(cJSON* json) {
    if (json &&
        (json->type == cJSON_True || json->valueint || json->valuedouble || json->valuestring)) {
        return SWITCH_TRUE;
    }
...
SWITCH_STANDARD_JSON_API(json_stats_function) {
    ...
    *json_reply = reply;
    ...
    if ((tsession = switch_core_session_locate(uuid))) {
        ...
        audio_stats = switch_core_media_get_stats(tsession, SWITCH_MEDIA_TYPE_AUDIO,
                                                  switch_core_session_get_pool(tsession));
        video_stats = switch_core_media_get_stats(tsession, SWITCH_MEDIA_TYPE_VIDEO,
                                                  switch_core_session_get_pool(tsession));
        ...
        if (true_enough(cdata) && switch_ivr_generate_json_cdr(tsession, &jevent, SWITCH_FALSE) ==
                                      SWITCH_STATUS_SUCCESS) {
...
SWITCH_STANDARD_API(uuid_ring_ready_function) {
    ...
    if (!uuid || !(xsession = switch_core_session_locate(uuid))) goto error;
    switch_channel_ring_ready_value(switch_core_session_get_channel(xsession),
                                    queued ? SWITCH_RING_READY_QUEUED : SWITCH_RING_READY_RINGING);
    switch_core_session_rwunlock(xsession);
    stream->write_function(stream, "+OK\n");
    goto done;
usage:
...
SWITCH_STANDARD_API(uuid_media_params_function) {
    ...
    if (zstr(cmd) || argc < 2 || zstr(argv[0]) || zstr(argv[1])) {
        ...
        if ((tsession = switch_core_session_locate(argv[0]))) {
            ...
            } else if ((status = switch_core_media_media_params(tsession, argv[1])) ==
                       SWITCH_STATUS_SUCCESS) {
    ...
    if (argc < 1) {
        ...
        if ((lsession = switch_core_session_locate(argv[0]))) {
            ...
            if (!zstr(cmd)) {
                ...
                stream->write_function(
                    stream, "%s video refresh now in %s mode.\n", switch_channel_get_name(channel),
...
typedef enum { BITRATE_INUSE = (1 << 0) } uuid_video_bitrate_enum_t;
    ...
    if (argc < 2) {
        ...
        if ((lsession = switch_core_session_locate(argv[0]))) {
            ...
            if (argv[1] && !strcasecmp(argv[1], "clear")) {
                if (switch_channel_test_app_flag_key("uuid_video_bitrate", channel,
                                                     BITRATE_INUSE)) {
...
SWITCH_STANDARD_API(uuid_debug_media_function) {
    ...
    if (zstr(cmd) || argc < 3 || zstr(argv[0]) || zstr(argv[1]) || zstr(argv[2])) {
        ...
        again:
            ...
            if (status == SWITCH_STATUS_SUCCESS && !strcasecmp(argv[1], "all") &&
                !strcmp(msg.string_array_arg[0], "both")) {
...
SWITCH_STANDARD_API(session_record_function) {
    ...
    if (!strcasecmp(action, "start")) {
        ...
        if (switch_ivr_record_session_event(rsession, path, limit, NULL, vars) !=
            SWITCH_STATUS_SUCCESS) {
        ...
        if (switch_ivr_record_session_pause(rsession, path, SWITCH_FALSE) !=
            SWITCH_STATUS_SUCCESS) {
...
SWITCH_STANDARD_API(session_audio_function) {
    ...
    if (strcasecmp(argv[1], "start") || argc < 5 ||
        (strcasecmp(argv[2], "read") && strcasecmp(argv[2], "write"))) {
        fail++;
        goto done;
    }
...
#define ORIGINATE_SYNTAX                                                                        \
    "<call url> <exten>|&<application_name>(<app_args>) [<dialplan>] [<context>] [<cid_name>] " \
    "[<cid_num>] [<timeout_sec>]"
SWITCH_STANDARD_API(originate_function) {
    ...
    if (session) {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_NOTICE,
                          "Originate can take 60 seconds to complete, and blocks the existing "
                          "session. Do not confuse with a lockup.\n");
    }
    ...
    if (switch_ivr_originate(NULL, &caller_session, &cause, aleg, timeout, NULL, cid_name, cid_num,
                             NULL, NULL, SOF_NONE, NULL, NULL) != SWITCH_STATUS_SUCCESS ||
        !caller_session) {
...
static void sch_api_callback(switch_scheduler_task_t* task) {
    ...
    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "Command %s(%s):\n%s\n", cmd,
                      switch_str_nil(arg), switch_str_nil((char*)stream.data));
        ...
        *group++ = '\0';
        ...
        if ((dcmd = strchr(group, ' '))) {
            *dcmd++ = '\0';
            ...
            id = switch_scheduler_add_task(when, sch_api_callback, (char*)__SWITCH_FUNC__, group, 0,
                                           api_task, flags);
...
static int show_callback(void* pArg, int argc, char** argv, char** columnNames) {
    ...
    if (holder->print_title && holder->count == 0) {
        ...
        for (x = 0; x < argc; x++) {
            ...
            if (holder->format && holder->format->html) {
                ...
                holder->stream->write_function(holder->stream, "<b>%s</b>%s", name,
                                               x == (argc - 1) ? "</td></tr>\n" : "</td><td>");
            } else {
                holder->stream->write_function(holder->stream, "%s%s", name,
                                               x == (argc - 1) ? "\n" : holder->delim);
            }
        }
    }
    ...
    if (holder->format && holder->format->html) {
        holder->stream->write_function(holder->stream, "<tr bgcolor=%s>",
                                       holder->count % 2 == 0 ? "eeeeee" : "ffffff");
    }
    ...
    for (x = 0; x < argc; x++) {
        ...
        if (holder->format && holder->format->html) {
            ...
            holder->stream->write_function(holder->stream, "<td>");
            holder->stream->write_function(holder->stream, "%s%s", aval,
                                           x == (argc - 1) ? "</td></tr>\n" : "</td><td>");
        } else {
            holder->stream->write_function(holder->stream, "%s%s", val,
                                           x == (argc - 1) ? "\n" : holder->delim);
        }
    }
...
#define SHOW_SYNTAX                                                                             \
    "codec|endpoint|application|api|dialplan|file|timer|calls [count]|channels [count|like "    \
    "<match "                                                                                   \
    "string>]|calls|detailed_calls|bridged_calls|detailed_bridged_calls|aliases|complete|chat|" \
    "management|modules|nat_map|say|interfaces|interface_types|tasks|limits|status"
SWITCH_STANDARD_API(show_function) {
    ...
    if (!command) {
    ...
    } else if (!strncasecmp(command, "codec", 5) || !strncasecmp(command, "dialplan", 8) ||
               !strncasecmp(command, "file", 4) || !strncasecmp(command, "timer", 5) ||
               !strncasecmp(command, "chat", 4) || !strncasecmp(command, "limit", 5) ||
               !strncasecmp(command, "say", 3) || !strncasecmp(command, "management", 10) ||
               !strncasecmp(command, "endpoint", 8)) {
        if (end_of(command) == 's') {
            end_of(command) = '\0';
        }
        switch_snprintfv(sql, sizeof(sql),
                         "select type, name, ikey from interfaces where hostname='%q' and type = "
                         "'%q' order by type,name",
                         switch_core_get_hostname(), command);
    } else if (!strncasecmp(command, "module", 6)) {
        if (argv[1] && strcasecmp(argv[1], "as")) {
            switch_snprintfv(sql, sizeof(sql),
                             "select distinct type, name, ikey, filename from interfaces where "
                             "hostname='%q' and ikey = '%q' order by type,name",
                             switch_core_get_hostname(), argv[1]);
        } else {
            switch_snprintfv(sql, sizeof(sql),
                             "select distinct type, name, ikey, filename from interfaces where "
                             "hostname='%q' order by type,name",
                             switch_core_get_hostname());
        }
    } else if (!strcasecmp(command, "interfaces")) {
        switch_snprintfv(
            sql, sizeof(sql),
            "select type, name, ikey from interfaces where hostname='%q' order by type,name",
            switch_core_get_hostname());
    } else if (!strcasecmp(command, "interface_types")) {
        switch_snprintfv(sql, sizeof(sql),
                         "select type,count(type) as total from interfaces where hostname='%q' "
                         "group by type order by type",
                         switch_core_get_switchname());
    } else if (!strcasecmp(command, "tasks")) {
        switch_snprintfv(sql, sizeof(sql), "select * from %q where hostname='%q'", command,
                         switch_core_get_hostname());
    } else if (!strcasecmp(command, "application") || !strcasecmp(command, "api")) {
        if (argv[1] && strcasecmp(argv[1], "as")) {
            switch_snprintfv(
                sql, sizeof(sql),
                "select name, description, syntax, ikey from interfaces where hostname='%q' and "
                "type = '%q' and description != '' and name = '%q' order by type,name",
                switch_core_get_hostname(), command, argv[1]);
        } else {
            switch_snprintfv(
                sql, sizeof(sql),
                "select name, description, syntax, ikey from interfaces where hostname='%q' and "
                "type = '%q' and description != '' order by type,name",
                switch_core_get_hostname(), command);
        }
        /* moved refreshable webpage show commands i.e. show
         * calls|registrations|channels||detailed_calls|bridged_calls|detailed_bridged_calls */
    } else if (!strcasecmp(command, "aliases")) {
        switch_snprintfv(sql, sizeof(sql),
                         "select * from aliases where hostname='%q' order by alias",
                         switch_core_get_switchname());
    } else if (!strcasecmp(command, "complete")) {
        switch_snprintfv(
            sql, sizeof(sql),
            "select * from complete where hostname='%q' order by a1,a2,a3,a4,a5,a6,a7,a8,a9,a10",
            switch_core_get_switchname());
        ...
        if ((cmdname = strchr(command, ' ')) && strcasecmp(cmdname, "as")) {
            *cmdname++ = '\0';
            switch_snprintfv(sql, sizeof(sql),
                             "select name, syntax, description, ikey from interfaces where "
                             "hostname='%q' and type = 'api' and name = '%q' order by name",
                             switch_core_get_hostname(), cmdname);
        } else {
            switch_snprintfv(sql, sizeof(sql),
                             "select name, syntax, description, ikey from interfaces where "
                             "hostname='%q' and type = 'api' order by name",
                             switch_core_get_hostname());
        }
        ...
        switch_snprintfv(sql, sizeof(sql) - 1,
                         ...
                         "  proto AS proto_num, "
                         "  sticky "
                         " FROM nat where hostname='%q' ORDER BY port, proto",
                         switch_core_get_hostname());
    } else {
        /* from here on refreshable commands:
         * calls|registrations|channels||detailed_calls|bridged_calls|detailed_bridged_calls */
        ...
        if (!strcasecmp(command, "calls")) {
            switch_snprintfv(
                sql, sizeof(sql),
                "select * from basic_calls where hostname='%q' order by call_created_epoch",
                switch_core_get_switchname());
            if (argv[1] && !strcasecmp(argv[1], "count")) {
                switch_snprintfv(sql, sizeof(sql),
                                 "select count(*) from basic_calls where hostname='%q'",
                                 switch_core_get_switchname());
                holder.justcount = 1;
            ...
            switch_snprintfv(sql, sizeof(sql), "select * from registrations where hostname='%q'",
                             switch_core_get_switchname());
            if (argv[1] && !strcasecmp(argv[1], "count")) {
                switch_snprintfv(sql, sizeof(sql),
                                 "select count(*) from registrations where hostname='%q'",
                                 switch_core_get_switchname());
                holder.justcount = 1;
                if (argv[2] && argv[3] && !strcasecmp(argv[2], "as")) {
                    as = argv[3];
            ...
            if (argv[2]) {
                ...
                if (strchr(argv[2], '%')) {
                    switch_snprintfv(
                        sql, sizeof(sql),
                        "select * from channels where hostname='%q' and uuid like '%q' or name "
                        "like '%q' or cid_name like '%q' or cid_num like '%q' or presence_data "
                        "like '%q' or accountcode like '%q' order by created_epoch",
                        switch_core_get_switchname(), argv[2], argv[2], argv[2], argv[2], argv[2],
                        argv[2]);
                } else {
                    switch_snprintfv(sql, sizeof(sql),
                                     "select * from channels where hostname='%q' and uuid like "
                                     "'%%%q%%' or name like '%%%q%%' or cid_name like '%%%q%%' or "
                                     "cid_num like '%%%q%%' or presence_data like '%%%q%%' or "
                                     "accountcode like '%%%q%%' order by created_epoch",
                                     switch_core_get_switchname(), argv[2], argv[2], argv[2],
                ...
                switch_snprintfv(
                    sql, sizeof(sql),
                    "select * from channels where hostname='%q' order by created_epoch",
                    switch_core_get_switchname());
            }
            ...
            if (argv[1] && !strcasecmp(argv[1], "count")) {
                switch_snprintfv(sql, sizeof(sql),
                                 "select count(*) from channels where hostname='%q'",
                                 switch_core_get_switchname());
                holder.justcount = 1;
                if (argv[2] && argv[3] && !strcasecmp(argv[2], "as")) {
                    as = argv[3];
            ...
            switch_snprintfv(
                sql, sizeof(sql),
                "select * from detailed_calls where hostname='%q' order by created_epoch",
                switch_core_get_switchname());
            if (argv[2] && !strcasecmp(argv[1], "as")) {
                as = argv[2];
            }
            ...
            switch_snprintfv(sql, sizeof(sql),
                             "select * from basic_calls where b_uuid is not null and hostname='%q' "
            ...
            switch_snprintfv(sql, sizeof(sql),
                             "select * from detailed_calls where b_uuid is not null and "
    ...
    if (html) {
        ...
        if (!as || strcasecmp(as, "xml")) {
            ...
            stream->write_function(stream, "<h1>FreeSWITCH %s %s</h1>\n", command,
                                   holder.justcount ? "(count)" : "");
            stream->write_function(stream, "%s%s",
                                   switch_event_get_header(stream->param_event, "Event-Date-Local"),
                                   nl);
        }
        holder.stream->write_function(holder.stream,
    ...
    if (!strcasecmp(as, "delim") || !strcasecmp(as, "csv")) {
        ...
        if (errmsg) {
            ...
            if (holder.count == 0) stream->write_function(stream, "-ERR No such command\n");
        } else {
...
SWITCH_STANDARD_API(uuid_session_heartbeat_function) {
    ...
    if (!(l_session = switch_core_session_locate(argv[0]))) {
        stream->write_function(stream,
                               "-ERR Cannot locate session. USAGE: uuid_session_heartbeat %s\n",
                               HEARTBEAT_SYNTAX);
        return SWITCH_STATUS_SUCCESS;
    }
        ...
        if ((argc == 2 || argc == 3) && !zstr(argv[0])) {
            ...
            if ((psession = switch_core_session_locate(uuid))) {
                ...
                if (zstr(var_name)) {
                    ...
                    switch_channel_add_variable_var_check(channel, var_name, var_value,
                                                          SWITCH_FALSE, SWITCH_STACK_BOTTOM);
                    stream->write_function(stream, "+OK\n");
                }
                    ...
                    if (strchr(var_name, '[')) {
                        ...
                        if ((ptr = strchr(vname, '[')) && strchr(ptr, ']')) {
                            *ptr++ = '\0';
                            idx = atoi(ptr);
                            var_value =
                                switch_channel_get_variable_dup(channel, vname, SWITCH_TRUE, idx);
                        }
...
SWITCH_STANDARD_API(uuid_send_dtmf_function) {
    ...
    if (switch_core_session_send_dtmf_string(psession, (const char*)dtmf_data) ==
        SWITCH_STATUS_SUCCESS) {
...
SWITCH_STANDARD_API(uuid_recv_dtmf_function) {
    ...
    if (switch_channel_queue_dtmf_string(switch_core_session_get_channel(psession), dtmf_data) ==
                ...
                if (switch_event_create(&event, SWITCH_EVENT_CHANNEL_DATA) ==
                    ...
                    if (!strcasecmp(format, "xml")) {
                        ...
                        switch_event_serialize(event, &buf,
                                               (switch_bool_t)strcasecmp(format, "plain"));
                    }
...
SWITCH_STANDARD_API(strftime_tz_api_function) {
    ...
    if (format && switch_strftime_tz(tz_name, format, date, sizeof(date), when * 1000000) ==
                      SWITCH_STATUS_SUCCESS) { /* The lookup of the zone may fail. */
...
SWITCH_STANDARD_API(hupall_api_function) {
    ...
    if (!vars_count) {
        ...
        stream->write_function(stream,
                               "+OK hangup all channels matching [%s]=[%s] with cause: %s\n",
...
SWITCH_STANDARD_API(getcputime_function) {
    ...
    stream->write_function(stream, "%" SWITCH_INT64_T_FMT ", %" SWITCH_INT64_T_FMT, t.userms,
                           t.kernelms);
...
SWITCH_STANDARD_API(uuid_loglevel) {
    ...
    if (zstr(uuid) || zstr(text)) {
        ...
        if (level == SWITCH_LOG_INVALID) {
            ...
            if (b &&
                switch_core_session_get_partner(tsession, &bsession) == SWITCH_STATUS_SUCCESS) {
...
#define LIMIT_HASH_USAGE_SYNTAX \
    "<realm> <id> [rate] (Using deprecated limit api, check limit_usage with backend param)"
...
SWITCH_MODULE_SHUTDOWN_FUNCTION(mod_commands_shutdown) {
    ...
    if (!x) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_CRIT,
                          "Giving up waiting for bgapi threads.\n");
    }
...
SWITCH_STANDARD_API(interface_ip_function) {
    ...
    if (!strcasecmp(argv[0], "ipv4")) {
        if (switch_find_interface_ip(addr, sizeof(addr), NULL, argv[1], AF_INET) ==
            SWITCH_STATUS_SUCCESS) {
            stream->write_function(stream, "%s", addr);
        }
    } else if (!strcasecmp(argv[0], "ipv6")) {
        if (switch_find_interface_ip(addr, sizeof(addr), NULL, argv[1], AF_INET6) ==
            SWITCH_STATUS_SUCCESS) {
            stream->write_function(stream, "%s", addr);
        }
    } else if (!strcasecmp(argv[0], "auto")) {
        if (switch_find_interface_ip(addr, sizeof(addr), NULL, argv[1], AF_UNSPEC) ==
            SWITCH_STATUS_SUCCESS) {
            stream->write_function(stream, "%s", addr);
        }
    } else {
...
SWITCH_STANDARD_JSON_API(json_channel_data_function) {
    ...
    *json_reply = reply;
    ...
    if ((tsession = switch_core_session_locate(uuid))) {
        ...
        if (switch_ivr_generate_json_cdr(tsession, &jevent, SWITCH_FALSE) ==
            SWITCH_STATUS_SUCCESS) {
            cJSON_AddItemToObject(reply, "channelData", jevent);
        }
    ...
    *json_reply = reply;
    ...
    if ((tsession = switch_core_session_locate(uuid))) {
        if (switch_true(edata)) {
            ...
            cJSON_AddItemToObject(reply, "channelName",
                                  cJSON_CreateString(switch_core_session_get_name(tsession)));
        }
...
SWITCH_STANDARD_JSON_API(json_api_function) {
    ...
    if (cmd && (status = switch_api_execute(cmd->valuestring, arg ? arg->valuestring : NULL,
                                            session, &stream)) == SWITCH_STATUS_SUCCESS) {
...
SWITCH_STANDARD_JSON_API(json_status_function) {
    ...
    cJSON_AddItemToObject(reply, "systemStatus",
                          cJSON_CreateString(switch_core_ready() ? "ready" : "not ready"));
...
SWITCH_MODULE_LOAD_FUNCTION(mod_commands_load) {
    ...
    *module_interface = switch_loadable_module_create_module_interface(pool, modname);
    ...
    if (use_system_commands) {
        SWITCH_ADD_API(commands_api_interface, "bg_system",
                       "Execute a system command in the background", bg_system_function,
                       SYSTEM_SYNTAX);
        SWITCH_ADD_API(commands_api_interface, "system", "Execute a system command",
                       system_function, SYSTEM_SYNTAX);
        SWITCH_ADD_API(commands_api_interface, "bg_spawn",
                       "Execute a spawn command in the background", bg_spawn_function,
                       SPAWN_SYNTAX);
        SWITCH_ADD_API(commands_api_interface, "spawn",
                       "Execute a spawn command without capturing it's output", spawn_function,
                       SPAWN_SYNTAX);
        SWITCH_ADD_API(commands_api_interface, "spawn_stream",
                       "Execute a spawn command and capture it's output", spawn_stream_function,
                       SPAWN_SYNTAX);
    }
    ...
    SWITCH_ADD_API(commands_api_interface, "coalesce", "Return first nonempty parameter",
                   coalesce_function, COALESCE_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "banner", "Return the system banner", banner_function,
                   "");
    SWITCH_ADD_API(commands_api_interface, "bgapi", "Execute an api command in a thread",
                   bgapi_function, "<command>[ <arg>]");
    SWITCH_ADD_API(commands_api_interface, "break", "uuid_break", break_function, BREAK_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "complete", "Complete", complete_function,
                   COMPLETE_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "cond", "Evaluate a conditional", cond_function,
                   "<expr> ? <true val> : <false val>");
    SWITCH_ADD_API(commands_api_interface, "console_complete", "", console_complete_function,
                   "<line>");
    SWITCH_ADD_API(commands_api_interface, "console_complete_xml", "",
                   console_complete_xml_function, "<line>");
    SWITCH_ADD_API(commands_api_interface, "create_uuid", "Create a uuid", uuid_function,
                   UUID_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "db_cache", "Manage db cache", db_cache_function,
                   "status");
    SWITCH_ADD_API(commands_api_interface, "domain_data", "Find domain data", domain_data_function,
                   "<domain> [var|param|attr] <name>");
    SWITCH_ADD_API(commands_api_interface, "domain_exists", "Check if a domain exists",
                   domain_exists_function, "<domain>");
    SWITCH_ADD_API(commands_api_interface, "echo", "Echo", echo_function, "<data>");
    SWITCH_ADD_API(commands_api_interface, "event_channel_broadcast", "Broadcast",
                   event_channel_broadcast_api_function, "<channel> <json>");
    SWITCH_ADD_API(commands_api_interface, "escape", "Escape a string", escape_function, "<data>");
    SWITCH_ADD_API(commands_api_interface, "eval", "eval (noop)", eval_function,
                   "[uuid:<uuid> ]<expression>");
    SWITCH_ADD_API(commands_api_interface, "expand", "Execute an api with variable expansion",
                   expand_function, "[uuid:<uuid> ]<cmd> <args>");
    SWITCH_ADD_API(commands_api_interface, "find_user_xml", "Find a user", find_user_function,
                   "<key> <user> <domain>");
    SWITCH_ADD_API(commands_api_interface, "fsctl", "FS control messages", ctl_function,
                   CTL_SYNTAX);
    ...
    SWITCH_ADD_API(commands_api_interface, "global_getvar", "Get global var",
                   global_getvar_function, GLOBAL_GETVAR_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "global_setvar", "Set global var",
                   global_setvar_function, GLOBAL_SETVAR_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "group_call", "Generate a dial string to call a group",
                   group_call_function, "<group>[@<domain>]");
    SWITCH_ADD_API(commands_api_interface, "help", "Show help for all the api commands",
                   help_function, "");
    SWITCH_ADD_API(commands_api_interface, "host_lookup", "Lookup host", host_lookup_function,
                   "<hostname>");
    SWITCH_ADD_API(commands_api_interface, "hostname", "Return the system hostname",
                   hostname_api_function, "");
    SWITCH_ADD_API(commands_api_interface, "interface_ip", "Return the primary IP of an interface",
                   interface_ip_function, INTERFACE_IP_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "switchname", "Return the switch name",
    ...
    SWITCH_ADD_API(commands_api_interface, "hupall", "hupall", hupall_api_function,
                   "<cause> [<var> <value>] [<var2> <value2>]");
    SWITCH_ADD_API(commands_api_interface, "in_group", "Determine if a user is in a group",
                   in_group_function, "<user>[@<domain>] <group_name>");
    SWITCH_ADD_API(commands_api_interface, "is_lan_addr", "See if an ip is a lan addr",
                   lan_addr_function, "<ip>");
    SWITCH_ADD_API(commands_api_interface, "limit_usage",
                   "Get the usage count of a limited resource", limit_usage_function,
                   "<backend> <realm> <id>");
    SWITCH_ADD_API(commands_api_interface, "limit_hash_usage",
                   "Deprecated: gets the usage count of a limited resource",
                   limit_hash_usage_function, "<realm> <id>");
    SWITCH_ADD_API(commands_api_interface, "limit_status", "Get the status of a limit backend",
                   limit_status_function, "<backend>");
    SWITCH_ADD_API(commands_api_interface, "limit_reset", "Reset the counters of a limit backend",
                   limit_reset_function, "<backend>");
    SWITCH_ADD_API(commands_api_interface, "limit_interval_reset",
                   "Reset the interval counter for a limited resource",
                   limit_interval_reset_function, LIMIT_INTERVAL_RESET_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "list_users", "List Users configured in Directory",
                   list_users_function, LIST_USERS_SYNTAX);
    ...
    SWITCH_ADD_API(commands_api_interface, "module_exists", "Check if module exists",
                   module_exists_function, "<module>");
    SWITCH_ADD_API(commands_api_interface, "msleep", "Sleep N milliseconds", msleep_function,
                   "<milliseconds>");
    SWITCH_ADD_API(commands_api_interface, "nat_map", "Manage NAT", nat_map_function,
                   "[status|republish|reinit] | [add|del] <port> [tcp|udp] [static]");
    SWITCH_ADD_API(commands_api_interface, "originate", "Originate a call", originate_function,
                   ORIGINATE_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "pause", "Pause media on a channel", pause_function,
                   PAUSE_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "pool_stats", "Core pool memory usage",
                   pool_stats_function, "Core pool memory usage.");
    SWITCH_ADD_API(commands_api_interface, "quote_shell_arg",
                   "Quote/escape a string for use on shell command line", quote_shell_arg_function,
                   "<data>");
    SWITCH_ADD_API(commands_api_interface, "regex", "Evaluate a regex", regex_function,
                   "<data>|<pattern>[|<subst string>][n|b]");
    SWITCH_ADD_API(commands_api_interface, "reloadacl", "Reload XML", reload_acl_function, "");
    SWITCH_ADD_API(commands_api_interface, "reload", "Reload module", reload_function,
                   UNLOAD_SYNTAX);
    ...
    SWITCH_ADD_API(commands_api_interface, "say_string", "", say_string_function,
                   SAY_STRING_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "sched_api", "Schedule an api command",
                   sched_api_function, SCHED_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "sched_broadcast",
                   "Schedule a broadcast event to a running call", sched_broadcast_function,
                   SCHED_BROADCAST_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "sched_del", "Delete a scheduled task",
                   sched_del_function, "<task_id>|<group_id>");
    SWITCH_ADD_API(commands_api_interface, "sched_hangup", "Schedule a running call to hangup",
                   sched_hangup_function, SCHED_HANGUP_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "sched_transfer",
                   "Schedule a transfer for a running call", sched_transfer_function,
                   SCHED_TRANSFER_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "show", "Show various reports", show_function,
                   SHOW_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "sql_escape", "Escape a string to prevent sql injection",
                   sql_escape, SQL_ESCAPE_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "status", "Show current status", status_function, "");
    SWITCH_ADD_API(commands_api_interface, "strftime_tz", "Display formatted time of timezone",
                   strftime_tz_api_function, "<timezone_name> [<epoch>|][format string]");
    SWITCH_ADD_API(commands_api_interface, "stun", "Execute STUN lookup", stun_function,
                   "<stun_server>[:port] [<source_ip>[:<source_port]]");
    SWITCH_ADD_API(commands_api_interface, "time_test", "Show time jitter", time_test_function,
                   "<mss> [count]");
    SWITCH_ADD_API(commands_api_interface, "timer_test", "Exercise FS timer", timer_test_function,
                   TIMER_TEST_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "tone_detect", "Start tone detection on a channel",
                   tone_detect_session_function, TONE_DETECT_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "unload", "Unload module", unload_function,
                   UNLOAD_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "unsched_api", "Unschedule an api command",
                   unsched_api_function, UNSCHED_SYNTAX);
    ...
    SWITCH_ADD_API(commands_api_interface, "url_encode", "Url encode a string", url_encode_function,
                   "<string>");
    SWITCH_ADD_API(commands_api_interface, "toupper", "Upper Case a string", toupper_function,
                   "<string>");
    SWITCH_ADD_API(commands_api_interface, "tolower", "Lower Case a string", tolower_function,
                   "<string>");
    SWITCH_ADD_API(commands_api_interface, "user_data", "Find user data", user_data_function,
                   "<user>@<domain> [var|param|attr] <name>");
    SWITCH_ADD_API(commands_api_interface, "uuid_early_ok", "stop ignoring early media",
                   uuid_early_ok_function, "<uuid>");
    SWITCH_ADD_API(commands_api_interface, "user_exists", "Find a user", user_exists_function,
                   "<key> <user> <domain>");
    SWITCH_ADD_API(commands_api_interface, "uuid_answer", "answer", uuid_answer_function, "<uuid>");
    SWITCH_ADD_API(commands_api_interface, "uuid_audio", "uuid_audio", session_audio_function,
                   AUDIO_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_break", "Break out of media sent to channel",
                   break_function, BREAK_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_bridge", "Bridge call legs", uuid_bridge_function,
                   "");
    SWITCH_ADD_API(commands_api_interface, "uuid_broadcast", "Execute dialplan application",
                   uuid_broadcast_function, BROADCAST_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_buglist", "List media bugs on a session",
                   uuid_buglist_function, BUGLIST_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_chat", "Send a chat message", uuid_chat,
                   UUID_CHAT_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_send_text", "Send text in real-time",
                   uuid_send_text, UUID_SEND_TEXT_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_capture_text", "start/stop capture_text",
                   uuid_capture_text, UUID_CAPTURE_TEXT_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_codec_debug", "Send codec a debug message",
                   uuid_codec_debug_function, CODEC_DEBUG_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_codec_param", "Send codec a param",
                   uuid_codec_param_function, CODEC_PARAM_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_debug_media", "Debug media",
                   uuid_debug_media_function, DEBUG_MEDIA_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_deflect", "Send a deflect", uuid_deflect,
                   UUID_DEFLECT_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_displace", "Displace audio",
                   session_displace_function, "<uuid> [start|stop] <path> [<limit>] [mux]");
    SWITCH_ADD_API(commands_api_interface, "uuid_display", "Update phone display",
                   uuid_display_function, DISPLAY_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_media_params", "Update remote vid params",
                   uuid_media_params_function, MEDIA_PARAMS_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_drop_dtmf",
                   "Drop all DTMF or replace it with a mask", uuid_drop_dtmf,
                   UUID_DROP_DTMF_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_dump", "Dump session vars", uuid_dump_function,
                   DUMP_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_exists", "Check if a uuid exists",
                   uuid_exists_function, EXISTS_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_fileman", "Manage session audio",
                   uuid_fileman_function, FILEMAN_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_flush_dtmf", "Flush dtmf on a given uuid",
                   uuid_flush_dtmf_function, "<uuid>");
    SWITCH_ADD_API(commands_api_interface, "uuid_getvar", "Get a variable from a channel",
                   uuid_getvar_function, GETVAR_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_hold", "Place call on hold", uuid_hold_function,
                   HOLD_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_kill", "Kill channel", kill_function, KILL_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_send_message", "Send MESSAGE to the endpoint",
                   uuid_send_message_function, SEND_MESSAGE_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_send_info", "Send info to the endpoint",
                   uuid_send_info_function, INFO_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_set_media_stats", "Set media stats",
                   uuid_set_media_stats, UUID_MEDIA_STATS_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_video_bitrate", "Send video bitrate req.",
                   uuid_video_bitrate_function, VIDEO_BITRATE_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_video_bandwidth", "Send video bandwidth",
                   uuid_video_bandwidth_function, VIDEO_BANDWIDTH_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_video_refresh", "Send video refresh.",
                   uuid_video_refresh_function, VIDEO_REFRESH_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_outgoing_answer", "Answer outgoing channel",
                   outgoing_answer_function, OUTGOING_ANSWER_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_limit", "Increase limit resource",
                   uuid_limit_function, LIMIT_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_limit_release", "Release limit resource",
                   uuid_limit_release_function, LIMIT_RELEASE_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_loglevel", "Set loglevel on session",
                   uuid_loglevel, UUID_LOGLEVEL_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_media", "Reinvite FS in or out of media path",
                   uuid_media_function, MEDIA_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_media_3p",
                   "Reinvite FS in or out of media path using 3pcc", uuid_media_3p_function,
                   MEDIA_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_media_reneg", "Media negotiation",
                   uuid_media_neg_function, MEDIA_RENEG_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_park", "Park channel", park_function, PARK_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_pause", "Pause media on a channel", pause_function,
                   PAUSE_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_phone_event", "Send an event to the phone",
                   uuid_phone_event_function, PHONE_EVENT_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_ring_ready", "Sending ringing to a channel",
                   uuid_ring_ready_function, RING_READY_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_pre_answer", "pre_answer",
                   uuid_pre_answer_function, "<uuid>");
    SWITCH_ADD_API(commands_api_interface, "uuid_preprocess", "Pre-process Channel",
                   preprocess_function, PREPROCESS_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_record", "Record session audio",
                   session_record_function, SESS_REC_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_recovery_refresh", "Send a recovery_refresh",
                   uuid_recovery_refresh, UUID_RECOVERY_REFRESH_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_recv_dtmf", "Receive dtmf digits",
                   uuid_recv_dtmf_function, UUID_RECV_DTMF_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_redirect", "Send a redirect", uuid_redirect,
                   UUID_REDIRECT_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_send_dtmf", "Send dtmf digits",
                   uuid_send_dtmf_function, UUID_SEND_DTMF_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_session_heartbeat", "uuid_session_heartbeat",
                   uuid_session_heartbeat_function, HEARTBEAT_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_setvar_multi", "Set multiple variables",
                   uuid_setvar_multi_function, SETVAR_MULTI_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_setvar", "Set a variable", uuid_setvar_function,
                   SETVAR_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_transfer", "Transfer a session", transfer_function,
                   TRANSFER_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_dual_transfer",
                   "Transfer a session and its partner", dual_transfer_function,
                   DUAL_TRANSFER_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_simplify",
                   "Try to cut out of a call path / attended xfer", uuid_simplify_function,
                   SIMPLIFY_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_jitterbuffer", "uuid_jitterbuffer",
                   uuid_jitterbuffer_function, JITTERBUFFER_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "uuid_zombie_exec",
                   "Set zombie_exec flag on the specified uuid", uuid_zombie_exec_function,
                   "<uuid>");
    SWITCH_ADD_API(commands_api_interface, "uuid_xfer_zombie",
                   "Allow A leg to hangup and continue originating", uuid_xfer_zombie,
                   XFER_ZOMBIE_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "xml_flush_cache", "Clear xml cache", xml_flush_function,
                   "<id> <key> <val>");
    SWITCH_ADD_API(commands_api_interface, "xml_locate", "Find some xml", xml_locate_function,
                   "[root | <section> <tag> <tag_attr_name> <tag_attr_val>]");
    SWITCH_ADD_API(commands_api_interface, "xml_wrap", "Wrap another api command in xml",
                   xml_wrap_api_function, "<command> <args>");
    SWITCH_ADD_API(commands_api_interface, "file_exists", "Check if a file exists on server",
                   file_exists_function, "<file>");
    SWITCH_ADD_API(commands_api_interface, "getcputime",
                   "Gets CPU time in milliseconds (user,kernel)", getcputime_function,
                   GETCPUTIME_SYNTAX);
    SWITCH_ADD_API(commands_api_interface, "json", "JSON API", json_function, "JSON");
    SWITCH_ADD_API(commands_api_interface, "memory", "Memory usage statistics", memory_function,
                   "memory");
    ...
    SWITCH_ADD_JSON_API(json_api_interface, "execute", "JSON session execute application",
                        json_execute_function, "");
    SWITCH_ADD_JSON_API(json_api_interface, "channelData", "JSON channel data application",
                        json_channel_data_function, "");
