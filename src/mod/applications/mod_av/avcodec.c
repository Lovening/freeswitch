...
/*  ff_avc_find_startcode is not exposed in the ffmpeg lib but you can use it
        ...
        The function is implemented in avc.h, guess we'll get rid of this later if we can directly
use libx264
...
static const uint8_t* fs_avc_find_startcode_internal(const uint8_t* p, const uint8_t* end) {
    const uint8_t* a = p + 4 - ((intptr_t)p & 3); //0x0011
...
#define my_dump_int(x) \
    switch_log_printf(SWITCH_CHANNEL_LOG_CLEAN, SWITCH_LOG_ERROR, STRINGIFY(x) " = %d\n", ctx->x);
#define my_dump_int64(x)                                          \
    switch_log_printf(SWITCH_CHANNEL_LOG_CLEAN, SWITCH_LOG_ERROR, \
                      STRINGIFY(x) " = %" SWITCH_INT64_T_FMT "\n", ctx->x);
#define my_dump_float(x) \
    switch_log_printf(SWITCH_CHANNEL_LOG_CLEAN, SWITCH_LOG_ERROR, STRINGIFY(x) " = %f\n", ctx->x);
#define my_dump_enum(x) \
    switch_log_printf(SWITCH_CHANNEL_LOG_CLEAN, SWITCH_LOG_ERROR, STRINGIFY(x) " = %d\n", ctx->x);
#define my_dump_uint(x) \
    switch_log_printf(SWITCH_CHANNEL_LOG_CLEAN, SWITCH_LOG_ERROR, STRINGIFY(x) " = 0x%x\n", ctx->x);
...
static switch_status_t buffer_h264_nalu(h264_codec_context_t* context, switch_frame_t* frame) {
    ...
    // switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "nalu=%02x mark=%d seq=%d ts=%d
...
static inline int is_valid_h263_dimension(int width, int height) {
    return ((width == 128 && height == 96) || (width == 176 && height == 144) ||
            (width == 352 && height == 288) || (width == 704 && height == 576) ||
...
static switch_status_t buffer_h263_packets(h264_codec_context_t* context, switch_frame_t* frame) {
    ...
    if (delta > 1) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
                          "packet loss? frame seq: %d last seq: %d, delta = %d\n", frame->seq,
                          context->last_seq, delta);
        if (delta > 2) {  // wait for key frame
            if (h->i) {
                ...
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE,
                                  "Received a H263 key frame after delta %d\n", delta);
            }
        }
        // return SWITCH_STATUS_RESTART;
    } else {
    ...
    if (0) {
        static char* h263_src[] = {"NONE", "subQCIF", "QCIF",         "CIF",
                                   "4CIF", "16CIF",   "Reserved-110", "Reserved-111"};
    ...
    if (h->sbit) {
        ...
        *((uint8_t*)old_data + inuse - 1) |=
            (((*(data + header_len) << h->sbit) & 0xFF) >> h->sbit);
        switch_buffer_write(context->nalu_buffer, data + header_len + 1,
                            frame->datalen - header_len - 1);
    } else {
...
static switch_status_t buffer_h263_rfc4629_packets(h264_codec_context_t* context,
...
void rtp_callback(struct AVCodecContext* avctx, void* data, int size, int mb_nb) {
    ...
    if (code == 0x20) {  // start
        ...
        context->nalus[context->nalu_current_index].h263_header.src =
            context->nalus[0].h263_header.src;
            ...
            if (len == SLICE_SIZE - 8) {
                ...
                if (mb_info_pos < mb_info_count) {
                    ...
                    if (pos <= end - buf_base) {
                        ...
                        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
                                          "Unable to split H263 packet! mb_info_pos=%d "
                                          "mb_info_count=%d pos=%d max=%" SWITCH_SIZE_T_FMT "\n",
                                          mb_info_pos, mb_info_count, pos,
                                          (switch_size_t)(end - buf_base));
                    }
                } else {
#if (LIBAVCODEC_VERSION_MAJOR < LIBAVCODEC_V)
                    switch_log_printf(
                        SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
                        "Should Not Happen!!! mb_info_pos=%d mb_info_count=%d mb_info_size=%d\n",
                        mb_info_pos, mb_info_count, mb_info_size);
#else
                    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
                                      "Should Not Happen!!! mb_info_pos=%d mb_info_count=%d "
                                      "mb_info_size=%" SWITCH_SIZE_T_FMT "\n",
                                      mb_info_pos, mb_info_count, mb_info_size);
#endif
                }
            }
...
static switch_status_t consume_h263_bitstream(h264_codec_context_t* context,
                                              switch_frame_t* frame) {
    our_h264_nalu_t* nalu = &context->nalus[context->nalu_current_index];
...
static switch_status_t consume_h263p_bitstream(h264_codec_context_t* context,
                                               switch_frame_t* frame) {
    our_h264_nalu_t* nalu = &context->nalus[context->nalu_current_index];
    uint8_t* data = frame->data;
...
static void set_h264_private_data(h264_codec_context_t* context, avcodec_profile_t* profile) {
    ...
    if (profile->ctx.me_cmp >= 0)
        context->encoder_ctx->me_cmp = profile->ctx.me_cmp;  // cmp=+chroma, where CHROMA = 1
    if (profile->ctx.me_range >= 0) context->encoder_ctx->me_range = profile->ctx.me_range;
    if (profile->ctx.max_b_frames >= 0)
    ...
    if (profile->ctx.i_quant_factor >= 0)
        context->encoder_ctx->i_quant_factor = profile->ctx.i_quant_factor;
    if (profile->ctx.b_quant_factor >= 0)
        context->encoder_ctx->b_quant_factor = profile->ctx.b_quant_factor;
...
static switch_status_t open_encoder(h264_codec_context_t* context, uint32_t width,
                                    uint32_t height) {
    ...
    if (!context->encoder) {
        if (context->av_codec_id == AV_CODEC_ID_H264) {
            // if (context->codec_settings.video.try_hardware_encoder && (context->encoder =
            ...
            if (context->codec_settings.video.try_hardware_encoder &&
                ...
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE,
                                  "NVENC HW CODEC ENABLED\n");
                context->hw_encoder = 1;
            } else {
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
                                  "NVENC HW CODEC NOT PRESENT\n");
            }
        }
    }
    ...
    if (context->av_codec_id == AV_CODEC_ID_H263 && (!is_valid_h263_dimension(width, height))) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
                          "You want %dx%d, but valid sizes are 128x96, 176x144, 352x288, 704x576, "
                          "and 1408x1152. Try H.263+\n",
                          width, height);
        return SWITCH_STATUS_FALSE;
    }
    ...
    if (context->codec_settings.video.bandwidth) {
        ...
        double fps =
            context->codec_settings.video.fps > 0 ? context->codec_settings.video.fps : 15.0;

        context->bandwidth = switch_calc_bitrate(context->codec_settings.video.width,
                                                 context->codec_settings.video.height, 1, fps);
    }
...
#define FF_ENABLE_DEPRECATION_WARNINGS \
    ...
    if (avcodec_open2(context->encoder_ctx, context->encoder, NULL) < 0) {
        ...
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
                          "Could not open hardware codec %s, trying software encoder\n",
                          codec_string);
...
static switch_status_t switch_h264_init(switch_codec_t* codec, switch_codec_flag_t flags,
    ...
    if (decoding) {
        ...
        if (!context->decoder && context->av_codec_id == AV_CODEC_ID_H263P) {
            switch_log_printf(
                ...
                "Cannot find AV_CODEC_ID_H263P decoder, trying AV_CODEC_ID_H263 instead\n");
    ...
    switch_buffer_create_dynamic(&(context->nalu_buffer), H264_NALU_BUFFER_SIZE,
                                 H264_NALU_BUFFER_SIZE * 8, 0);
...
static void av_unused fill_avframe(AVFrame* pict, switch_image_t* img) {
    switch_I420_copy2(img->planes, img->stride, pict->data, pict->linesize, img->d_w, img->d_h);
}
...
static switch_status_t switch_h264_encode(switch_codec_t* codec, switch_frame_t* frame) {
    ...
    if (context->av_codec_id == AV_CODEC_ID_H263 && (!is_valid_h263_dimension(width, height))) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
                          "You want %dx%d, but valid H263 sizes are 128x96, 176x144, 352x288, "
    ...
    if (context->need_key_frame &&
        (context->last_keyframe_request + avcodec_globals.key_frame_min_freq) < switch_time_now()) {
        if (avcodec_globals.debug) {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "Generate/Send AV KEYFRAME\n");
        }

        avframe->pict_type = AV_PICTURE_TYPE_I;
#if ((LIBAVCODEC_VERSION_MAJOR == LIBAVCODEC_6_V &&   \
      LIBAVCODEC_VERSION_MINOR >= LIBAVCODEC_61_V) || \
     LIBAVCODEC_VERSION_MAJOR >= LIBAVCODEC_7_V)
    ...
    if (ret == AVERROR_EOF) {
        ...
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG9,
                          "Error sending frame to encoder - BUG, should never happen\n");
        goto error;
    } else if (ret < 0) {
    ...
    while (ret >= 0) {
        ...
        if (ret == AVERROR(EAGAIN)) {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG9,
                              "No more video packets at the moment\n");
        } else if (ret == AVERROR_EOF) {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG9,
                              "No more video packets at all\n");
        } else if (ret < 0) {
    ...
    if (*got_output) {
        ...
        *got_output = 0;
        ...
        if (context->av_codec_id == AV_CODEC_ID_H263) {
            switch_log_printf(
                SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG5,
                "Encoded frame %" SWITCH_INT64_T_FMT
                " (size=%5d) [0x%02x 0x%02x 0x%02x 0x%02x] got_output: %d slices: %d\n",
                context->pts, (*pkt)->size, *((uint8_t*)(*pkt)->data),
                *((uint8_t*)((*pkt)->data + 1)), *((uint8_t*)((*pkt)->data + 2)),
...
static switch_status_t switch_h264_decode(switch_codec_t* codec, switch_frame_t* frame) {
    ...
    //	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING, "Packet Loss, skip privousely
        ...
        if (size > 0) {
            ...
            GCC_DIAG_ON(deprecated-declarations)
                ...
                if (ret == AVERROR_EOF) {
                    ...
                    /* we fully drain all the output in each decode call, so this should not ever
                     * happen */
                    ret = AVERROR_BUG;
                    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
                                      "Error sending packet to decoder BUG, should never happen\n");
                } else if (ret < 0) {
                    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
                ...
                while (ret >= 0) {
                    ...
                    if (ret == AVERROR(EAGAIN)) {
                        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG9,
                                          "No more video frames at the moment\n");
                    } else if (ret == AVERROR_EOF) {
                        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG9,
                                          "No more video frames at all\n");
                    } else if (ret < 0) {
                        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
            ...
            // switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "buffer: %d got pic: %d len:
...
#endif
                switch_I420_copy2(picture->data, picture->linesize, context->img->planes,
                                  context->img->stride, width, height);
...
void show_codecs(switch_stream_handle_t* stream) {
    ...
    for (i = 0; i < nb_codecs; i++) {
        ...
        stream->write_function(stream, " %-20s %s", desc->name,
...
static void parse_codecs(avcodec_profile_t* aprofile, switch_xml_t codecs) {
    ...
    if (!codecs) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
                          "no codecs defined in mod_av profile %s\n", aprofile->name);
        return;
    }
    ...
    for (; codec; codec = codec->next) {
        ...
        switch_event_add_header_string(aprofile->codecs, SWITCH_STACK_BOTTOM, codec_name,
                                       profile_name);
    }
}
...
static void load_config(void) {
    ...
    if (avcodec_globals.key_frame_min_freq < 10000 ||
        avcodec_globals.key_frame_min_freq > 3 * 1000000) {
...
SWITCH_MODULE_LOAD_FUNCTION(mod_avcodec_load) {
    ...
    switch_core_codec_add_video_implementation(
        pool, codec_interface, 99, "H264", NULL, switch_h264_init, switch_h264_encode,
        switch_h264_decode, switch_h264_control, switch_h264_destroy);

    SWITCH_ADD_CODEC(codec_interface, "H263 Video");
    switch_core_codec_add_video_implementation(
        pool, codec_interface, 34, "H263", NULL, switch_h264_init, switch_h264_encode,
        switch_h264_decode, switch_h264_control, switch_h264_destroy);

    SWITCH_ADD_CODEC(codec_interface, "H263+ Video");
    switch_core_codec_add_video_implementation(
        pool, codec_interface, 115, "H263-1998", NULL, switch_h264_init, switch_h264_encode,
        switch_h264_decode, switch_h264_control, switch_h264_destroy);
