...
static switch_status_t sofia_read_frame(switch_core_session_t* session, switch_frame_t** frame,
                                        switch_io_flag_t flags, int stream_id);
static switch_status_t sofia_write_frame(switch_core_session_t* session, switch_frame_t* frame,
                                         switch_io_flag_t flags, int stream_id);
static switch_status_t sofia_read_video_frame(switch_core_session_t* session,
                                              switch_frame_t** frame, switch_io_flag_t flags,
                                              int stream_id);
static switch_status_t sofia_write_video_frame(switch_core_session_t* session,
                                               switch_frame_t* frame, switch_io_flag_t flags,
                                               int stream_id);
static switch_status_t sofia_read_text_frame(switch_core_session_t* session, switch_frame_t** frame,
                                             switch_io_flag_t flags, int stream_id);
static switch_status_t sofia_write_text_frame(switch_core_session_t* session, switch_frame_t* frame,
                                              switch_io_flag_t flags, int stream_id);
...
static switch_status_t sofia_on_init(switch_core_session_t* session) {
    ...
    if ((hval = switch_channel_get_variable(channel, "sip_watch_headers"))) {
        ...
        numhdrs =
            switch_separate_string(dupvar, ',', watch_headers, switch_arraylen(watch_headers));
        if (numhdrs) {
            char** wheaders =
                switch_core_session_alloc(session, ((numhdrs + 1) * sizeof(wheaders[0])));
            for (i = 0; i < numhdrs; i++) {
    ...
    if (switch_channel_test_flag(tech_pvt->channel, CF_RECOVERING) ||
        switch_channel_test_flag(tech_pvt->channel, CF_RECOVERING_BRIDGE)) {
    ...
    if (sofia_test_pflag(tech_pvt->profile, PFLAG_AUTO_INVITE_100) &&
        ...
        if (sofia_acknowledge_call(session) != SWITCH_STATUS_SUCCESS) {
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                              "Call appears to be already acknowledged\n");
        }
    }
...
char* generate_pai_str(private_object_t* tech_pvt) {
    ...
    const char *var, *header,
        *ua = switch_channel_get_variable(tech_pvt->channel, "sip_user_agent");
    char* pai = NULL;
    ...
    if (!sofia_test_pflag(tech_pvt->profile, PFLAG_PASS_CALLEE_ID) ||
        !sofia_test_pflag(tech_pvt->profile, PFLAG_CID_IN_1XX) ||
        ((var = switch_channel_get_variable(tech_pvt->channel, "sip_cid_in_1xx")) &&
         switch_false(var))) {
        return NULL;
    }

    if (zstr((callee_name =
                  switch_channel_get_variable(tech_pvt->channel, "initial_callee_id_name"))) &&
        zstr((callee_name =
                  switch_channel_get_variable(tech_pvt->channel, "effective_callee_id_name"))) &&
        zstr(
            (callee_name = switch_channel_get_variable(tech_pvt->channel, "sip_callee_id_name")))) {
        callee_name = switch_channel_get_variable(tech_pvt->channel, "callee_id_name");
    }

    if (zstr((callee_number =
                  switch_channel_get_variable(tech_pvt->channel, "initial_callee_id_number"))) &&
        zstr((callee_number =
                  switch_channel_get_variable(tech_pvt->channel, "effective_callee_id_number"))) &&
        zstr((callee_number =
                  switch_channel_get_variable(tech_pvt->channel, "sip_callee_id_number"))) &&
        zstr(
            (callee_number = switch_channel_get_variable(tech_pvt->channel, "callee_id_number")))) {
    ...
    header = (tech_pvt->cid_type == CID_TYPE_RPID && !switch_stristr("aastra", ua))
                 ? "Remote-Party-ID"
                 : "P-Asserted-Identity";
...
switch_status_t sofia_on_destroy(switch_core_session_t* session) {
    ...
    if (tech_pvt) {
        ...
        if (!zstr(tech_pvt->call_id)) {
            switch_core_hash_delete_locked(tech_pvt->profile->chat_hash, tech_pvt->call_id,
                                           tech_pvt->profile->flag_mutex);
        }
...
switch_status_t sofia_on_hangup(switch_core_session_t* session) {
    ...
    if (!((use_my_cause = switch_channel_get_variable(channel, "sip_ignore_remote_cause")) &&
          switch_true(use_my_cause))) {
        ps_cause = switch_channel_get_variable(
            channel, "last_bridge_" SWITCH_PROTO_SPECIFIC_HANGUP_CAUSE_VARIABLE);
    }

    if (!zstr(ps_cause) &&
        (!strncasecmp(ps_cause, "sip:", 4) || !strncasecmp(ps_cause, "sips:", 5))) {
        int new_cause = atoi(sofia_glue_strip_proto(ps_cause));
        if (new_cause) {
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                              "%s Overriding SIP cause %d with %d from the other leg\n",
    ...
    if (tech_pvt->hash_key && !sofia_test_pflag(tech_pvt->profile, PFLAG_DESTROY)) {
        switch_core_hash_delete_locked(tech_pvt->profile->chat_hash, tech_pvt->hash_key,
                                       tech_pvt->profile->flag_mutex);
    }

    if (tech_pvt->profile->pres_type) {
        char* sql = switch_mprintf("delete from sip_dialogs where uuid='%q'",
    ...
    if (sofia_test_pflag(tech_pvt->profile, PFLAG_DESTROY)) {
        ...
        if (!val || switch_false(val)) {
            if ((val = switch_channel_get_variable(tech_pvt->channel, "sip_reason"))) {
                ...
                if ((switch_channel_test_flag(channel, CF_INTERCEPT) ||
                     cause == SWITCH_CAUSE_PICKED_OFF || cause == SWITCH_CAUSE_LOSE_RACE) &&
                    !switch_true(
                        switch_channel_get_variable(channel, "ignore_completed_elsewhere"))) {
                    reason = switch_core_session_sprintf(
                        session, "SIP;cause=200;text=\"Call completed elsewhere\"");
                } else if (cause > 0 && cause < 128) {
                    reason = switch_core_session_sprintf(session, "Q.850;cause=%d;text=\"%s\"",
                                                         cause, switch_channel_cause2str(cause));
                } else {
                    reason = switch_core_session_sprintf(session, "SIP;cause=%d;text=\"%s\"", cause,
                                                         switch_channel_cause2str(cause));
                }
            }
        }

        if (switch_channel_test_flag(channel, CF_INTERCEPT) || cause == SWITCH_CAUSE_PICKED_OFF ||
        ...
        if (switch_channel_test_flag(channel, CF_ANSWERED) ||
            ...
            if (switch_channel_direction(channel) == SWITCH_CALL_DIRECTION_OUTBOUND) {
                ...
                if (!sofia_test_flag(tech_pvt, TFLAG_BYE)) {
                    nua_cancel(
                        tech_pvt->nh, SIPTAG_CONTACT(SIP_NONE),
                        TAG_IF(call_info, SIPTAG_CALL_INFO_STR(call_info)),
                ...
                char* resp_headers =
                    sofia_glue_get_extra_headers(channel, SOFIA_SIP_RESPONSE_HEADER_PREFIX);
                const char* phrase;
                ...
                if (tech_pvt->respond_code) {
                    ...
                    switch (sip_cause) {
                        ...
                        case 407: {
                            const char* to_host =
                                switch_channel_get_variable(channel, "sip_challenge_realm");
                            ...
                            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                                              "Challenging call\n");
                            sofia_reg_auth_challenge(tech_pvt->profile, tech_pvt->nh, NULL,
                                                     REG_INVITE, to_host, 0, 0);
                            *reason = '\0';
                        } break;
                ...
                if (tech_pvt->respond_dest &&
                    !sofia_test_pflag(tech_pvt->profile, PFLAG_MANUAL_REDIRECT)) {
    ...
    if (cause == SWITCH_CAUSE_WRONG_CALL_STATE) {
        ...
        if (switch_event_create_subclass(&s_event, SWITCH_EVENT_CUSTOM,
                                         MY_EVENT_WRONG_CALL_STATE) == SWITCH_STATUS_SUCCESS) {
            switch_event_add_header_string(s_event, SWITCH_STACK_BOTTOM, "from_user",
                                           tech_pvt->from_user);
            switch_event_add_header_string(s_event, SWITCH_STACK_BOTTOM, "network_ip",
                                           tech_pvt->mparams.remote_ip);
            switch_event_add_header(s_event, SWITCH_STACK_BOTTOM, "network_port", "%d",
                                    tech_pvt->mparams.remote_port);
    ...
    if (tech_pvt->sofia_private) {
        /* set to NULL so that switch_core_session_locate no longer succeeds, but don't lose the
         * UUID in uuid_str so we can fire events with session UUID */
...
static switch_status_t sofia_acknowledge_call(switch_core_session_t* session) {
    ...
    if (!tech_pvt->sent_100) {
        nua_respond(tech_pvt->nh, SIP_100_TRYING,
                    TAG_IF(!zstr(session_id_header), SIPTAG_HEADER_STR(session_id_header)),
                    TAG_END());
        tech_pvt->sent_100 = 1;
        return SWITCH_STATUS_SUCCESS;
...
static switch_status_t sofia_answer_channel(switch_core_session_t* session) {
    ...
    if (sofia_acknowledge_call(session) == SWITCH_STATUS_SUCCESS) {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING,
                          "Dialplan did not acknowledge_call; sent 100 Trying");
    }
    ...
    if (switch_channel_test_flag(channel, CF_CONFERENCE) && !zstr(tech_pvt->reply_contact) &&
        ...
        tech_pvt->reply_contact =
            switch_core_session_sprintf(session, "%s;isfocus", tech_pvt->reply_contact);
    }

    // switch_core_media_set_local_sdp
    if (sofia_test_flag(tech_pvt, TFLAG_3PCC_INVITE)) {
        // SNARK: complete hack to get final ack sent when a 3pcc invite has been passed from the
        // other leg in bypass_media mode. This code handles the pass_indication sent after the 3pcc
        // ack is received by the other leg in the is_3pcc && is_proxy case below. Is there a better
        // place to hang this...?
    ...
    b_sdp = switch_channel_get_variable(channel, SWITCH_B_SDP_VARIABLE);
    is_proxy = (switch_channel_test_flag(channel, CF_PROXY_MODE) ||
                switch_channel_test_flag(channel, CF_PROXY_MEDIA));
    is_3pcc_proxy = (sofia_test_pflag(tech_pvt->profile, PFLAG_3PCC_PROXY) &&
                     sofia_test_flag(tech_pvt, TFLAG_3PCC));
    is_3pcc = (!sofia_test_pflag(tech_pvt->profile, PFLAG_3PCC_PROXY) &&
               sofia_test_flag(tech_pvt, TFLAG_3PCC));
    ...
    if (b_sdp && is_proxy && !is_3pcc_proxy) {
        ...
        if (is_3pcc || is_3pcc_proxy) {
            ...
            if (!sofia_test_flag(tech_pvt, TFLAG_BYE)) {
                ...
                if (sofia_use_soa(tech_pvt)) {
                    nua_respond(
                        tech_pvt->nh, SIP_200_OK, TAG_IF(is_proxy, NUTAG_AUTOANSWER(0)),
                        ...
                        TAG_IF(!zstr(session_id_header), SIPTAG_HEADER_STR(session_id_header)),
                        TAG_IF(
                            switch_stristr("update_display", tech_pvt->x_freeswitch_support_remote),
        ...
        if ((is_proxy && !b_sdp) || sofia_test_flag(tech_pvt, TFLAG_LATE_NEGOTIATION) ||
            switch_core_media_codec_chosen(tech_pvt->session, SWITCH_MEDIA_TYPE_AUDIO) !=
                SWITCH_STATUS_SUCCESS) {
            sofia_clear_flag_locked(tech_pvt, TFLAG_LATE_NEGOTIATION);

            if (is_proxy) {
                switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                                  "Disabling proxy mode due to call answer with no bridge\n");
            ...
            if (switch_channel_direction(tech_pvt->channel) == SWITCH_CALL_DIRECTION_INBOUND) {
                ...
                if (zstr(r_sdp) ||
                    sofia_media_tech_media(tech_pvt, r_sdp, SDP_OFFER) != SWITCH_STATUS_SUCCESS) {
        ...
        if ((status = switch_core_media_choose_port(tech_pvt->session, SWITCH_MEDIA_TYPE_AUDIO,
    ...
    if (!sofia_test_flag(tech_pvt, TFLAG_BYE)) {
        ...
        if (switch_channel_test_flag(tech_pvt->channel, CF_PROXY_MODE) &&
            tech_pvt->mparams.early_sdp && tech_pvt->mparams.local_sdp_str) {
            char *a, *b;
            ...
            if (!a || !b || strcmp(a, b)) {
                /* The SIP RFC for SOA forbids sending a 183 with one sdp then a 200 with another
                   but it won't do us much good unless we do so in this case we will abandon the SOA
                   rules and go rogue.
                */
        ...
        if ((tech_pvt->mparams.last_sdp_str && strstr(tech_pvt->mparams.last_sdp_str, "a=setup")) ||
            (tech_pvt->mparams.local_sdp_str &&
             strstr(tech_pvt->mparams.local_sdp_str, "a=setup"))) {
            session_timeout = 0;
        }
        ...
        if (sofia_use_soa(tech_pvt)) {
            nua_respond(
                tech_pvt->nh, SIP_200_OK, NUTAG_AUTOANSWER(0),
                TAG_IF(call_info, SIPTAG_CALL_INFO_STR(call_info)),
                ...
                SIPTAG_CALL_INFO_STR(switch_channel_get_variable(
                    tech_pvt->channel, SOFIA_SIP_HEADER_PREFIX "call_info")),
                SOATAG_USER_SDP_STR(tech_pvt->mparams.local_sdp_str), SOATAG_REUSE_REJECTED(1),
            ...
            nua_respond(
                tech_pvt->nh, SIP_200_OK, NUTAG_AUTOANSWER(0), NUTAG_MEDIA_ENABLE(0),
                TAG_IF(call_info, SIPTAG_CALL_INFO_STR(call_info)),
                TAG_IF(sticky, NUTAG_PROXY(tech_pvt->record_route)),
                TAG_IF(cid, SIPTAG_HEADER_STR(cid)),
                TAG_IF(!zstr(session_id_header), SIPTAG_HEADER_STR(session_id_header)),
...
static switch_status_t sofia_read_text_frame(switch_core_session_t* session, switch_frame_t** frame,
    ...
    if (switch_channel_test_flag(switch_core_session_get_channel(session), CF_MSRP)) {
        ...
        if (msrp_msg && msrp_msg->method == MSRP_METHOD_SEND && msrp_msg->payload &&
            ...
            memcpy(rframe->data, msrp_msg->payload,
                   msrp_msg->payload_bytes + 1); /* include the last NULL byte */
...
static switch_status_t sofia_write_video_frame(switch_core_session_t* session,
    ...
    if (SWITCH_STATUS_SUCCESS ==
        switch_core_media_write_frame(session, frame, flags, stream_id, SWITCH_MEDIA_TYPE_VIDEO)) {
        return SWITCH_STATUS_SUCCESS;
    }
...
static switch_status_t sofia_read_frame(switch_core_session_t* session, switch_frame_t** frame,
    ...
    status =
        switch_core_media_read_frame(session, frame, flags, stream_id, SWITCH_MEDIA_TYPE_AUDIO);
...
static switch_status_t sofia_send_dtmf(switch_core_session_t* session, const switch_dtmf_t* dtmf) {
    ...
    if (!switch_core_media_ready(tech_pvt->session, SWITCH_MEDIA_TYPE_AUDIO) ||
        !switch_channel_media_ready(tech_pvt->channel) ||
    ...
    switch (dtmf_type) {
        case DTMF_2833: {
            return switch_core_media_queue_rfc2833(tech_pvt->session, SWITCH_MEDIA_TYPE_AUDIO,
                                                   dtmf);
        }
        case DTMF_INFO: {
            if (dtmf->digit == 'w') {
                ...
                snprintf(message, sizeof(message), "Signal=%c\r\nDuration=%d\r\n", dtmf->digit,
                         dtmf->duration / 8);
                switch_mutex_lock(tech_pvt->sofia_mutex);
                nua_info(tech_pvt->nh, SIPTAG_CONTENT_TYPE_STR("application/dtmf-relay"),
...
static switch_status_t sofia_receive_message(switch_core_session_t* session,
    ...
    switch (msg->message_id) {
        ...
        case SWITCH_MESSAGE_HEARTBEAT_EVENT: {
            ...
            if (sofia_test_flag(tech_pvt, TFLAG_KEEPALIVE)) {
                if (tech_pvt->profile->keepalive == KA_MESSAGE) {
                    nua_message(
                        tech_pvt->nh, SIPTAG_CONTENT_TYPE_STR("text/plain"),
                        TAG_IF(!zstr(tech_pvt->user_via), SIPTAG_VIA_STR(tech_pvt->user_via)),
                        SIPTAG_PAYLOAD_STR(pl), TAG_END());
                } else if (tech_pvt->profile->keepalive == KA_INFO) {
                    nua_info(tech_pvt->nh, SIPTAG_CONTENT_TYPE_STR("text/plain"),
                             TAG_IF(!zstr(tech_pvt->user_via), SIPTAG_VIA_STR(tech_pvt->user_via)),
                             SIPTAG_PAYLOAD_STR(pl), TAG_END());
                }
            }
            ...
            if (event && uuid) {
                ...
                if (msg->numeric_arg) {
                    ...
                    if (other_session) {
                        ...
                        const char* invite_failure_status =
                        ...
                        if (!zstr(invite_failure_status) && !zstr(invite_failure_str)) {
                            ...
                            switch_log_printf(
                                ...
                                switch_channel_get_name(channel), invite_failure_status,
                                invite_failure_str);
                        }
        ...
        case SWITCH_MESSAGE_INDICATE_PROGRESS: {
            ...
            if ((var = switch_channel_get_variable(channel, "sip_force_nat_mode")) &&
                switch_true(var)) {
                sofia_set_flag(tech_pvt, TFLAG_NAT);
                switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                                  "Setting NAT mode based on manual variable\n");
                switch_channel_set_variable(channel, "sip_nat_detected", "true");
            }
            ...
            if (presence_id || presence_data) {
                char* sql = switch_mprintf(
                    "update sip_dialogs set presence_id='%q',presence_data='%q' "
                    "where uuid='%q';\n",
    ...
    switch (msg->message_id) {
        case SWITCH_MESSAGE_INDICATE_DEFLECT: {
            ...
            if (!strcasecmp(msg->string_arg, "sip:")) {
                const char* format =
                    strchr(tech_pvt->profile->sipip, ':') ? "sip:%s@[%s]" : "sip:%s@%s";
            ...
            nua_refer(tech_pvt->nh, SIPTAG_REFER_TO_STR(ref_to),
                      ...
                      TAG_IF(!zstr(extra_headers), SIPTAG_HEADER_STR(extra_headers)),
                      TAG_IF(!zstr(session_id_header), SIPTAG_HEADER_STR(session_id_header)),
                      SIPTAG_CONTACT_STR(tech_pvt->reply_contact), TAG_END());

            if (msg->string_array_arg[0]) {
                tech_pvt->proxy_refer_uuid = (char*)msg->string_array_arg[0];
            } else if (!switch_channel_var_true(tech_pvt->channel,
        ...
        case SWITCH_MESSAGE_INDICATE_VIDEO_REFRESH_REQ:
            if (switch_channel_media_up(channel) && !switch_channel_test_flag(channel, CF_AVPF) &&
                !switch_channel_test_flag(channel, CF_MANUAL_VID_REFRESH) &&
                ...
                if (!tech_pvt->last_vid_info || (now - tech_pvt->last_vid_info) > 500000) {
                    ...
                    nua_info(tech_pvt->nh, SIPTAG_CONTENT_TYPE_STR("application/media_control+xml"),
                             SIPTAG_PAYLOAD_STR(pl), TAG_END());
                }
            }
            break;
        case SWITCH_MESSAGE_INDICATE_BROADCAST: {
            ...
            if (ip && port) {
                switch_core_media_gen_local_sdp(session, SDP_OFFER, ip, (switch_port_t)atoi(port),
                                                msg->string_arg, 1);
            }
        ...
        case SWITCH_MESSAGE_INDICATE_MEDIA_REDIRECT: {
            ...
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                              "%s Sending media re-direct:\n%s\n", switch_channel_get_name(channel),
                              switch_str_nil(msg->string_arg));
        ...
        case SWITCH_MESSAGE_INDICATE_T38_DESCRIPTION: {
            switch_t38_options_t* t38_options =
                switch_channel_get_private(tech_pvt->channel, "t38_options");
            const char* session_id_header =
                ...
                if (sofia_use_soa(tech_pvt)) {
                    nua_respond(
                        ...
                        SIPTAG_CALL_INFO_STR(switch_channel_get_variable(
                            tech_pvt->channel, SOFIA_SIP_HEADER_PREFIX "call_info")),
                        SOATAG_USER_SDP_STR(tech_pvt->mparams.local_sdp_str),
                        SOATAG_REUSE_REJECTED(1), SOATAG_ORDERED_USER(1),
                        SOATAG_AUDIO_AUX("cn telephone-event"), NUTAG_INCLUDE_EXTRA_SDP(1),
                        TAG_IF(!zstr(extra_headers), SIPTAG_HEADER_STR(extra_headers)),
                        TAG_IF(!zstr(session_id_header), SIPTAG_HEADER_STR(session_id_header)),
                        TAG_END());
        ...
        case SWITCH_MESSAGE_INDICATE_REQUEST_IMAGE_MEDIA: {
            ...
            if (t38_options) {
                switch_core_media_set_udptl_image_sdp(tech_pvt->session, t38_options,
                                                      msg->numeric_arg);
                ...
                switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING,
                                  "%s Request to send IMAGE on channel with not t38 options.\n",
                                  switch_channel_get_name(channel));
            }
        } break;
        ...
        case SWITCH_MESSAGE_INDICATE_MEDIA: {
            ...
            switch_channel_set_variable(
                tech_pvt->channel, "absolute_codec_string",
                switch_channel_get_variable(tech_pvt->channel, "ep_codec_string"));
            switch_core_media_prepare_codecs(tech_pvt->session, SWITCH_TRUE);
        ...
        case SWITCH_MESSAGE_INDICATE_PHONE_EVENT: {
            ...
            if (!switch_channel_test_flag(channel, CF_ANSWERED) &&
                ...
                nua_notify(tech_pvt->nh, NUTAG_NEWSUB(1), NUTAG_SUBSTATE(nua_substate_active),
                           ...
                           SIPTAG_SUBSCRIPTION_STATE_STR("active"), SIPTAG_EVENT_STR(event),
                           TAG_IF(!zstr(session_id_header), SIPTAG_HEADER_STR(session_id_header)),
                           TAG_END());
            }
        } break;
        case SWITCH_MESSAGE_INDICATE_MESSAGE: {
            ...
            if (!zstr(msg->string_array_arg[3]) &&
                !strcmp(msg->string_array_arg[3], tech_pvt->caller_profile->uuid)) {
            ...
            if (ok) {
                ...
                if (!zstr(msg->string_array_arg[0]) && !zstr(msg->string_array_arg[1])) {
                    switch_snprintf(ct, sizeof(ct), "%s/%s", msg->string_array_arg[0],
                ...
                nua_message(tech_pvt->nh, SIPTAG_CONTENT_TYPE_STR(ct),
                            TAG_IF(!zstr(tech_pvt->user_via), SIPTAG_VIA_STR(tech_pvt->user_via)),
        ...
        case SWITCH_MESSAGE_INDICATE_INFO: {
            ...
            if (switch_true(switch_channel_get_variable_dup(channel, "fs_send_unsupported_info",
                                                            SWITCH_FALSE, -1))) {
                ok = 1;
            }
            ...
            if (ok) {
                ...
                nua_info(tech_pvt->nh, SIPTAG_CONTENT_TYPE_STR(ct),
        ...
        case SWITCH_MESSAGE_INDICATE_SIMPLIFY: {
            char *ref_to, *ref_by;
            const char* uuid;
            const char *call_id = NULL, *to_user = NULL, *to_host = NULL, *to_tag = NULL,
                       *from_tag = NULL, *from_user = NULL, *from_host = NULL;
            ...
            if (to_user && to_host && from_user && from_host && call_id && to_tag && from_tag) {
                ...
                switch_snprintf(in, sizeof(in), "%s;to-tag=%s;from-tag=%s", call_id, to_tag,
                                from_tag);
        ...
        case SWITCH_MESSAGE_INDICATE_DISPLAY: {
            ...
            if (!zstr(name) && strcmp(name, "_undef_")) {
                ...
                if (!sofia_test_flag(tech_pvt, TFLAG_UPDATING_DISPLAY) &&
                    switch_channel_test_flag(channel, CF_ANSWERED)) {
                    if (zstr(tech_pvt->last_sent_callee_id_name) ||
                        strcmp(tech_pvt->last_sent_callee_id_name, name) ||
                        zstr(tech_pvt->last_sent_callee_id_number) ||
                        strcmp(tech_pvt->last_sent_callee_id_number, number)) {
                        if (switch_stristr("update_display",
                                           tech_pvt->x_freeswitch_support_remote)) {
                            snprintf(message, sizeof(message),
                                     "X-FS-Display-Name: %s\nX-FS-Display-Number: %s\n", name,
                                     number);

                            if (switch_channel_test_flag(tech_pvt->channel,
                                                         CF_LAZY_ATTENDED_TRANSFER)) {
                                snprintf(message + strlen(message),
                                         sizeof(message) - strlen(message),
                                         "X-FS-Lazy-Attended-Transfer: true\n");
                                switch_channel_clear_flag(tech_pvt->channel,
                                                          CF_LAZY_ATTENDED_TRANSFER);
                            ...
                            if (switch_channel_test_flag(tech_pvt->channel, CF_ATTENDED_TRANSFER)) {
                                snprintf(message + strlen(message),
                                         sizeof(message) - strlen(message),
                                         "X-FS-Attended-Transfer: true\n");
                                switch_channel_clear_flag(tech_pvt->channel, CF_ATTENDED_TRANSFER);
                            }

                            nua_info(tech_pvt->nh,
                                     SIPTAG_CONTENT_TYPE_STR("message/update_display"),
                            ...
                            if (ver_str) {
                                ...
                                if (argv[0] && argv[1] && argv[2]) {
                                    version = (atoi(argv[0]) * 10000) + (atoi(argv[1]) * 100) +
                                              atoi(argv[2]);
                                }
                            ...
                            nua_info(tech_pvt->nh, SIPTAG_CONTENT_TYPE_STR("message/sipfrag"),
                                     TAG_IF(!zstr(tech_pvt->user_via),
                                            SIPTAG_VIA_STR(tech_pvt->user_via)),
                                     SIPTAG_PAYLOAD_STR(message), TAG_END());
                        } else if (update_allowed && ua &&
                                   (switch_channel_var_true(tech_pvt->channel,
                                                            "update_ignore_ua") ||
                                    switch_stristr("poly", ua) ||
                                    (switch_stristr("aastra", ua) &&
                                     !switch_stristr("Intelligate", ua)) ||
                            ...
                            snprintf(message, sizeof(message),
                                     "P-Asserted-Identity: \"%s\" <sip:%s@%s>", name, number,
                                     tech_pvt->profile->sipip);

                            sofia_set_flag_locked(tech_pvt, TFLAG_UPDATING_DISPLAY);
                            nua_update(tech_pvt->nh, NUTAG_SESSION_TIMER(tech_pvt->session_timeout),
                                       NUTAG_SESSION_REFRESHER(tech_pvt->session_refresher),
                                       NUTAG_UPDATE_REFRESH(tech_pvt->update_refresher),
                                       TAG_IF(!zstr(tech_pvt->privacy),
                                              SIPTAG_PRIVACY_STR(tech_pvt->privacy)),
                                       TAG_IF(call_info, SIPTAG_CALL_INFO_STR(call_info)),
                                       TAG_IF(!zstr(tech_pvt->route_uri),
                                              NUTAG_PROXY(tech_pvt->route_uri)),
                                       TAG_IF(!zstr_buf(message), SIPTAG_HEADER_STR(message)),
                                       TAG_IF(!zstr(tech_pvt->user_via),
                                              SIPTAG_VIA_STR(tech_pvt->user_via)),
                                       TAG_IF(!zstr(session_id_header),
                        ...
                        tech_pvt->last_sent_callee_id_name =
                            switch_core_session_strdup(tech_pvt->session, name);
                        tech_pvt->last_sent_callee_id_number =
                            switch_core_session_strdup(tech_pvt->session, number);

                        if (switch_event_create(&event, SWITCH_EVENT_CALL_UPDATE) ==
                            SWITCH_STATUS_SUCCESS) {
                            const char* uuid = switch_channel_get_partner_uuid(channel);
                            switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "Direction",
                                                           "SEND");

                            switch_event_add_header_string(event, SWITCH_STACK_BOTTOM,
                                                           "Sent-Callee-ID-Name", name);
                            switch_event_add_header_string(event, SWITCH_STACK_BOTTOM,
                                                           "Sent-Callee-ID-Number", number);
        ...
        case SWITCH_MESSAGE_INDICATE_HOLD: {
            if (msg->numeric_arg) {
                ...
                if (!zstr(msg->string_arg)) {
                    char message[256] = "";
                    const char* ua =
                        switch_channel_get_variable(tech_pvt->channel, "sip_user_agent");
                    const char* session_id_header =
                        sofia_glue_session_id_header(tech_pvt->session, tech_pvt->profile);
                    ...
                    if (ua && switch_stristr("snom", ua)) {
                        ...
                        nua_info(
                            tech_pvt->nh, SIPTAG_CONTENT_TYPE_STR("message/sipfrag"),
                            TAG_IF(!zstr(tech_pvt->user_via), SIPTAG_VIA_STR(tech_pvt->user_via)),
                            SIPTAG_PAYLOAD_STR(message),
                            TAG_IF(!zstr(session_id_header), SIPTAG_HEADER_STR(session_id_header)),
                            TAG_END());
                    } else if (ua && switch_stristr("poly", ua)) {
                        snprintf(message, sizeof(message), "P-Asserted-Identity: \"%s\" <%s>",
                                 msg->string_arg, tech_pvt->caller_profile->destination_number);
                        nua_update(
                            tech_pvt->nh, NUTAG_SESSION_TIMER(tech_pvt->session_timeout),
                            NUTAG_SESSION_REFRESHER(tech_pvt->session_refresher),
                            NUTAG_UPDATE_REFRESH(tech_pvt->update_refresher),
                            TAG_IF(!zstr(tech_pvt->route_uri), NUTAG_PROXY(tech_pvt->route_uri)),
                            TAG_IF(!zstr_buf(message), SIPTAG_HEADER_STR(message)),
...
#define MAX_REDIR 128
            ...
            if (!zstr(msg->string_arg)) {
                if (!switch_channel_test_flag(channel, CF_ANSWERED) &&
                    ...
                    if ((argc = switch_separate_string(dest, ' ', argv,
                        ...
                        for (i = 0; i < argc; i++) {
                            if (!strchr(argv[i], '<') && !strchr(argv[i], '>')) {
                                if (argc > 1) {
                                    if (i == argc - 1) {
                                        switch_snprintf(
                                            newdest + strlen(newdest), len - strlen(newdest),
                                            "\"unknown\" <%s>;q=%1.3f", argv[i],
                                            (double)((double)(MAX_REDIR + 1 - i)) / 1000);
                                    } else {
                                        switch_snprintf(
                                            newdest + strlen(newdest), len - strlen(newdest),
                                            "\"unknown\" <%s>;q=%1.3f,", argv[i],
                                    ...
                                    switch_snprintf(newdest + strlen(newdest),
                                                    len - strlen(newdest), "\"unknown\" <%s>",
                                                    argv[i]);
                                }
                            } else {
                                if (i == argc - 1) {
                                    switch_snprintf(newdest + strlen(newdest),
                                                    len - strlen(newdest), "%s", argv[i]);
                                } else {
                                    switch_snprintf(newdest + strlen(newdest),
                                                    len - strlen(newdest), "%s,", argv[i]);
                                }
                    ...
                    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING,
                                      "Too late for redirecting, already answered\n");
                }
            }
            break;
        case SWITCH_MESSAGE_INDICATE_RESPOND: {
            ...
            if (msg->numeric_arg || msg->string_arg) {
                ...
                /* Dialplan should really use acknowledge_call application instead of respond
                ...
                if (tech_pvt->proxy_refer_uuid) {
                    if (tech_pvt->proxy_refer_msg) {
                        nua_respond(
                            tech_pvt->nh, code, su_strdup(nua_handle_home(tech_pvt->nh), reason),
                            SIPTAG_CONTACT_STR(tech_pvt->reply_contact), SIPTAG_EXPIRES_STR("60"),
                ...
                if (code == 302 && !zstr(msg->string_arg)) {
                    ...
                    if (!sofia_test_flag(tech_pvt, TFLAG_BYE)) {
                        char* extra_headers =
                            sofia_glue_get_extra_headers(channel, SOFIA_SIP_PROGRESS_HEADER_PREFIX);
                        ...
                        if (!zstr((sdp))) {
                            if (!strcasecmp(sdp, "t38")) {
                                switch_t38_options_t* t38_options =
                                    switch_channel_get_private(tech_pvt->channel, "t38_options");
                                if (t38_options) {
                                    switch_core_media_set_udptl_image_sdp(tech_pvt->session,
                                                                          t38_options, 0);
                                    if (switch_core_media_ready(tech_pvt->session,
                                                                SWITCH_MEDIA_TYPE_AUDIO)) {
                                        switch_channel_clear_flag(tech_pvt->channel,
                                                                  CF_NOTIMER_DURING_BRIDGE);
                                        switch_core_media_udptl_mode(tech_pvt->session,
                                                                     SWITCH_MEDIA_TYPE_AUDIO);
                                    }
                                }
                            } else {
                            ...
                            if (sofia_use_soa(tech_pvt)) {
                                nua_respond(
                                    tech_pvt->nh, code,
                                    su_strdup(nua_handle_home(tech_pvt->nh), reason),
                                    SIPTAG_CONTACT_STR(tech_pvt->reply_contact),
                                    SOATAG_USER_SDP_STR(tech_pvt->mparams.local_sdp_str),
                            ...
                            if (sofia_test_pflag(tech_pvt->profile, PFLAG_3PCC_PROXY) &&
                                ...
                                sofia_clear_flag(tech_pvt, TFLAG_3PCC);
                                sofia_clear_flag(tech_pvt, TFLAG_3PCC_HAS_ACK);
                                switch_core_session_pass_indication(session,
                            ...
                            if (msg->numeric_arg &&
                                ...
                                if (code > 399) {
                                    switch_call_cause_t cause =
                                        sofia_glue_sip_cause_to_freeswitch(code);
                                    if (code == 401 || cause == 407)
                                        cause = SWITCH_CAUSE_USER_CHALLENGE;

                                    tech_pvt->respond_code = code;
                                    tech_pvt->respond_phrase =
                                        switch_core_session_strdup(tech_pvt->session, reason);
        ...
        case SWITCH_MESSAGE_INDICATE_ALERTING: {
            ...
            nua_respond(
                tech_pvt->nh, SIP_180_RINGING, SIPTAG_CONTACT_STR(tech_pvt->reply_contact),
                TAG_IF(cid, SIPTAG_HEADER_STR(cid)),
                TAG_IF(call_info, SIPTAG_CALL_INFO_STR(call_info)),
        ...
        case SWITCH_MESSAGE_INDICATE_RINGING: {
            ...
            if (!switch_channel_test_flag(channel, CF_RING_READY) &&
                ...
                const char* call_info =
                    switch_channel_get_variable(channel, "presence_call_info_full");
                char* cid = generate_pai_str(tech_pvt);
                const char* session_id_header =
                    sofia_glue_session_id_header(tech_pvt->session, tech_pvt->profile);
                ...
                if (switch_channel_direction(channel) == SWITCH_CALL_DIRECTION_INBOUND) {
                    const char* to_tag = "";
                    to_tag =
                        switch_str_nil(nta_leg_get_tag(nua_get_dialog_state_leg(tech_pvt->nh)));
                    if (to_tag) {
                ...
                switch (ring_ready_val) {
                    case SWITCH_RING_READY_QUEUED:
                        ...
                        nua_respond(
                            tech_pvt->nh, SIP_182_QUEUED,
                            SIPTAG_CONTACT_STR(tech_pvt->reply_contact),
                            TAG_IF(cid, SIPTAG_HEADER_STR(cid)),
                            TAG_IF(call_info, SIPTAG_CALL_INFO_STR(call_info)),
                            TAG_IF(!zstr(extra_header), SIPTAG_HEADER_STR(extra_header)),
                            TAG_IF(switch_stristr("update_display",
                                                  tech_pvt->x_freeswitch_support_remote),
                                   SIPTAG_HEADER_STR("X-FS-Support: " FREESWITCH_SUPPORT)),
                            TAG_IF(!zstr(session_id_header), SIPTAG_HEADER_STR(session_id_header)),
                            TAG_END());
        ...
        case SWITCH_MESSAGE_INDICATE_PROGRESS: {
            ...
            if (!sofia_test_flag(tech_pvt, TFLAG_ANS) &&
                !sofia_test_flag(tech_pvt, TFLAG_EARLY_MEDIA)) {
                ...
                if (switch_channel_test_flag(channel, CF_PROXY_MODE) ||
                    ...
                    if (sofia_test_flag(tech_pvt, TFLAG_LATE_NEGOTIATION) ||
                        switch_core_media_codec_chosen(
                            tech_pvt->session, SWITCH_MEDIA_TYPE_AUDIO) != SWITCH_STATUS_SUCCESS) {
                        sofia_clear_flag_locked(tech_pvt, TFLAG_LATE_NEGOTIATION);
                        if (switch_channel_direction(tech_pvt->channel) ==
                            ...
                            switch_core_media_prepare_codecs(tech_pvt->session, SWITCH_TRUE);
                            if (zstr(r_sdp) || sofia_media_tech_media(tech_pvt, r_sdp, SDP_OFFER) !=
                                                   SWITCH_STATUS_SUCCESS) {
                                switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session),
                                                  SWITCH_LOG_WARNING,
                ...
                if (sofia_test_flag(tech_pvt, TFLAG_NAT) ||
                    switch_channel_get_variable(channel, "sip-force-contact") ||
                    ((val = switch_channel_get_variable(channel, "sip_sticky_contact")) &&
                     switch_true(val))) {
                    sticky = tech_pvt->record_route;
                    switch_channel_set_variable(channel, "sip_nat_detected", "true");
                }
                ...
                if (!sofia_test_flag(tech_pvt, TFLAG_BYE)) {
                    ...
                    if (switch_channel_test_flag(tech_pvt->channel, CF_PROXY_MODE) &&
                        tech_pvt->mparams.early_sdp &&
                        strcmp(tech_pvt->mparams.early_sdp, tech_pvt->mparams.local_sdp_str)) {
                        /* The SIP RFC for SOA forbids sending a 183 with one sdp then a 200 with
                           another but it won't do us much good unless we do so in this case we will
                           abandon the SOA rules and go rogue.
                         */
                        sofia_clear_flag(tech_pvt, TFLAG_ENABLE_SOA);
                    }

                    tech_pvt->mparams.early_sdp = switch_core_session_strdup(
                        tech_pvt->session, tech_pvt->mparams.local_sdp_str);

                    if (sofia_use_soa(tech_pvt)) {
                        nua_respond(
                            tech_pvt->nh, send_sip_code, p_send_sip_msg, NUTAG_AUTOANSWER(0),
                            TAG_IF(sticky, NUTAG_PROXY(tech_pvt->record_route)),
                            TAG_IF(cid, SIPTAG_HEADER_STR(cid)),
                            SIPTAG_CONTACT_STR(tech_pvt->reply_contact), SOATAG_REUSE_REJECTED(1),
                            SOATAG_RTP_SELECT(1),
        ...
        case SWITCH_MESSAGE_INDICATE_SESSION_ID: {
            if (sofia_test_pflag(tech_pvt->profile, PFLAG_RFC7989_SESSION_ID) &&
                switch_channel_test_flag(channel, CF_ANSWERED)) {
                sofia_glue_do_invite(session);
            }
        } break;
...
end_lock:
    ...
    // if (msg->message_id == SWITCH_MESSAGE_INDICATE_ANSWER || msg->message_id ==
    // SWITCH_MESSAGE_INDICATE_PROGRESS) { sofia_send_callee_id(session, NULL, NULL);
    // }
...
static switch_status_t sofia_receive_event(switch_core_session_t* session, switch_event_t* event) {
    ...
    if (tech_pvt->hash_key) {
        switch_mutex_lock(tech_pvt->sofia_mutex);
        msg_nh =
            nua_handle(tech_pvt->profile->nua, NULL, SIPTAG_FROM_STR(tech_pvt->chat_from),
        ...
        nua_message(msg_nh, SIPTAG_CONTENT_TYPE_STR("text/html"), SIPTAG_PAYLOAD_STR(body),
                    TAG_END());
        switch_mutex_unlock(tech_pvt->sofia_mutex);
    }
...
const char* sofia_state_names[] = {"UNREGED",    "TRYING", "REGISTER",  "REGED",
                                   "UNREGISTER", "FAILED", "FAIL_WAIT", "EXPIRED",
                                   "NOREG",      "DOWN",   "TIMEOUT",   NULL};
...
static int show_reg_callback(void* pArg, int argc, char** argv, char** columnNames) {
    ...
    cb->stream->write_function(
        ...
        switch_str_nil(argv[3]), switch_str_nil(argv[7]), switch_str_nil(argv[4]),
        switch_str_nil(argv[5]), exp_buf, exp_secs, switch_str_nil(argv[18]),
        (float)atoll(switch_str_nil(argv[19])) / 1000, switch_str_nil(argv[11]),
        switch_str_nil(argv[12]), switch_str_nil(argv[13]), switch_str_nil(argv[14]),
...
uint32_t sofia_profile_reg_count(sofia_profile_t* profile) {
    ...
    sql = switch_mprintf("select count(*) from sip_registrations where profile_name = '%q'",
                         profile->name);
    ...
    if (argc > 0) {
        if (argc == 1) {
            ...
            stream->write_function(stream, "%25s\t%32s\t%s\t%9s\t%s\t%s\n", "Profile::Gateway-Name",
                                   "    Data    ", "State", "Ping Time", "IB Calls(F/T)",
                                   "OB Calls(F/T)");
                    ...
                    if (!strcmp(vvar, profile->name)) { /* not an alias */
                        for (gp = profile->gateways; gp; gp = gp->next) {
                            ...
                            stream->write_function(stream, "%25s\t%32s\t%s\t%6.2f\t%u/%u\t%u/%u",
                                                   pkey, gp->register_to,
                                                   sofia_state_names[gp->state], gp->ping_time,
                                                   gp->ib_failed_calls, gp->ib_calls,
                                                   gp->ob_failed_calls, gp->ob_calls);
                            free(pkey);

                            if (gp->state == REG_STATE_FAILED || gp->state == REG_STATE_FAIL_WAIT ||
                                gp->state == REG_STATE_TRYING) {
                                time_t now = switch_epoch_time_now(NULL);
                                if (gp->reg_timeout >= now) {
                                    stream->write_function(stream, " (retry: %ds)",
                                                           gp->reg_timeout - now);
                                } else {
            ...
            stream->write_function(stream, "%s\n", line);
            stream->write_function(stream,
                                   "%d gateway%s: Inbound(Failed/Total): %u/%u,"
        ...
        if (!strcasecmp(argv[0], "gateway")) {
            if ((gp = sofia_reg_find_gateway(argv[1]))) {
                ...
                stream->write_function(stream, "Uptime  \t%lds\n",
                                       gp->status == SOFIA_GATEWAY_UP
                                           ? (switch_time_now() - gp->uptime) / 1000000
                                           : 0);
            ...
            if ((argv[1]) && (profile = sofia_glue_find_profile(argv[1]))) {
                ...
                if (argv[2] && !strcasecmp(argv[2], "pres") && argv[3]) {
                    sql = switch_mprintf(
                        ...
                        "network_ip,network_port,sip_username,sip_realm,mwi_user,mwi_host,ping_"
                        "status,ping_time"
                        " from sip_registrations where profile_name='%q' and presence_hosts like "
                        "'%%%q%%'",
                        profile->name, argv[3]);
                }
    ...
    for (hi = switch_core_hash_first(mod_sofia_globals.profile_hash); hi;
        ...
        if (sofia_test_pflag(profile, PFLAG_RUNNING)) {
            if (strcmp(vvar, profile->name)) {
                ...
                if (!sofia_test_pflag(profile, PFLAG_TLS) || !profile->tls_only) {
                    stream->write_function(
                        stream, "%25s\t%s\t  %40s\t%s (%u)\n", profile->name, "profile",
                        profile->url, sofia_test_pflag(profile, PFLAG_RUNNING) ? "RUNNING" : "DOWN",
                        profile->inuse);
                }

                if (sofia_test_pflag(profile, PFLAG_TLS)) {
                    stream->write_function(
                        stream, "%25s\t%s\t  %40s\t%s (%u) (TLS)\n", profile->name, "profile",
                        profile->tls_url,
                ...
                for (gp = profile->gateways; gp; gp = gp->next) {
                    ...
                    stream->write_function(stream, "%25s\t%s\t  %40s\t%s", pkey, "gateway",
                                           gp->register_to, sofia_state_names[gp->state]);
                    free(pkey);
        ...
        if (argc == 1) {
            ...
            if ((argv[1]) && (profile = sofia_glue_find_profile(argv[1]))) {
                ...
                if (!argv[2] || (strcasecmp(argv[2], "reg") && strcasecmp(argv[2], "user"))) {
                    ...
                    stream->write_function(
                        stream, "    <auto-nat>%s</auto-nat>\n",
                        sofia_test_pflag(profile, PFLAG_AUTO_NAT) ? "true" : "false");
                    stream->write_function(
                        stream, "    <db-name>%s</db-name>\n",
                        profile->dbname ? profile->dbname : switch_str_nil(profile->odbc_dsn));
                    ...
                    stream->write_function(stream, "    <context>%s</context>\n",
                                           switch_str_nil(profile->context));
                    stream->write_function(
                        stream, "    <challenge-realm>%s</challenge-realm>\n",
                        zstr(profile->challenge_realm) ? "auto_to" : profile->challenge_realm);
                    for (x = 0; x < profile->rtpip_index; x++) {
                    ...
                    stream->write_function(stream, "    <hold-music>%s</hold-music>\n",
                                           zstr(profile->hold_music) ? "N/A" : profile->hold_music);
                    stream->write_function(
                        stream, "    <outbound-proxy>%s</outbound-proxy>\n",
                        zstr(profile->outbound_proxy) ? "N/A" : profile->outbound_proxy);
                    ...
                    stream->write_function(stream, "    <max-recv-rps>%d</max-recv-rps>\n",
                                           profile->max_recv_requests_per_second);
                    stream->write_function(
                        stream, "    <nomedia>%s</nomedia>\n",
                        sofia_test_flag(profile, TFLAG_INB_NOMEDIA) ? "true" : "false");
                    stream->write_function(
                        stream, "    <late-neg>%s</late-neg>\n",
                        sofia_test_flag(profile, TFLAG_LATE_NEGOTIATION) ? "true" : "false");
                    stream->write_function(
                        stream, "    <proxy-media>%s</proxy-media>\n",
                        sofia_test_flag(profile, TFLAG_PROXY_MEDIA) ? "true" : "false");
                    ...
                    if (profile->user_agent_filter) {
                        stream->write_function(stream,
                                               "    <user-agent-filter>%s</user-agent-filter>\n",
                                               switch_str_nil(profile->user_agent_filter));
                    }
                    if (profile->max_registrations_perext > 0) {
                        stream->write_function(stream,
                                               "    "
                                               "<max-registrations-per-extension>%d</"
            ...
            if (strcmp(vvar, profile->name)) {
                ...
                if (profile->ws_bindurl) {
                    stream->write_function(
                        ...
                        "<profile>\n<name>%s</name>\n<type>%s</type>\n<data>%s</data>\n<state>%s "
                        "(%u) (WS)</state>\n</profile>\n",
                        profile->name, "profile", profile->ws_bindurl,
                        sofia_test_pflag(profile, PFLAG_RUNNING) ? "RUNNING" : "DOWN",
                        profile->inuse);
                }
                if (profile->wss_bindurl) {
                    stream->write_function(
                        ...
                        "<profile>\n<name>%s</name>\n<type>%s</type>\n<data>%s</data>\n<state>%s "
                        "(%u) (WSS)</state>\n</profile>\n",
                        profile->name, "profile", profile->wss_bindurl,
                        sofia_test_pflag(profile, PFLAG_RUNNING) ? "RUNNING" : "DOWN",
                        profile->inuse);
                }
                ...
                for (gp = profile->gateways; gp; gp = gp->next) {
                    ...
                    stream->write_function(stream,
                                           "<gateway>\n<name>%s</name>\n<type>%s</type>\n<data>%s</"
                                           "data>\n<state>%s</state>\n</gateway>\n",
                                           gp->name, "gateway", gp->register_to,
                                           sofia_state_names[gp->state]);
                    if (gp->state == REG_STATE_FAILED || gp->state == REG_STATE_FAIL_WAIT ||
                        gp->state == REG_STATE_TRYING) {
                        time_t now = switch_epoch_time_now(NULL);
...
static switch_status_t cmd_profile(char** argv, int argc, switch_stream_handle_t* stream) {
    ...
    if (!strcasecmp(argv[1], "stop") || !strcasecmp(argv[1], "restart")) {
        ...
        if (diff < rsec) {
            stream->write_function(stream,
                                   "Profile %s must be up for at least %d seconds to "
            ...
            if (!strcasecmp(argv[1], "stop")) {
                ...
                if (argv[2] && !strcasecmp(argv[2], "wait")) {
                    int loops = 20 * 2;
                    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE,
                                      "Waiting for %s to finish SIP transactions.\n",
                                      profile->name);
                    while (!sofia_test_pflag(profile, PFLAG_SHUTDOWN)) {
                        ...
                        if (!--loops) {
                            switch_log_printf(
                                ...
                                "Timeout Waiting for %s to finish SIP transactions.\n",
                                profile->name);
                            break;
                        }
                    }
    ...
    if (!strcasecmp(argv[1], "siptrace")) {
        if (argc > 2) {
            ...
            stream->write_function(stream, "%s sip debugging on %s", value ? "Enabled" : "Disabled",
                                   profile->name);
        } else {
    ...
    if (!strcasecmp(argv[1], "capture")) {
        if (argc > 2) {
            ...
            nua_set_params(profile->nua,
                           TPTAG_CAPT(value ? mod_sofia_globals.capture_server : NULL), TAG_END());
            stream->write_function(stream, "%s sip capturing on %s", value ? "Enabled" : "Disabled",
                                   profile->name);
        } else {
...
static int contact_callback(void* pArg, int argc, char** argv, char** columnNames) {
    ...
    if (!zstr(argv[0]) && (contact = sofia_glue_get_url_from_contact(argv[0], 1))) {
        if (cb->dedup) {
            char* tmp = switch_mprintf("%ssofia/%s/sip:%s", argv[2], argv[1],
                                       sofia_glue_strip_proto(contact));
            ...
            cb->stream->write_function(cb->stream, "%ssofia/%s/sip:%s,", argv[2], argv[1],
                                       sofia_glue_strip_proto(contact));
        }
        free(contact);
    }
    ...
    if (profile_name) {
        ...
        if (profile) {
            ...
            if (zstr(user)) {
                ...
                sql = switch_mprintf(
                    "select count(*) "
                    "from sip_registrations where sip_user='%q' and (sip_host='%q' or "
                    "presence_hosts like '%%%q%%')",
                    user, domain, domain);
            }
            switch_assert(sql);
            sofia_glue_execute_sql_callback(profile, profile->dbh_mutex, sql, sql2str_callback,
                                            &cb);
            switch_safe_free(sql);
            if (!zstr(reg_count)) {
...
SWITCH_STANDARD_API(sofia_username_of_function) {
    ...
    if (profile_name) {
        ...
        if (profile) {
            ...
            sql = switch_mprintf(
                "select sip_username "
                "from sip_registrations where sip_user='%q' and (sip_host='%q' or presence_hosts "
                "like '%%%q%%')",
                user, domain, domain);
...
static void select_from_profile(sofia_profile_t* profile, const char* user, const char* domain,
                                const char* concat, const char* exclude_contact,
                                const char* match_user_agent, switch_stream_handle_t* stream,
                                switch_bool_t dedup) {
    struct cb_helper cb;
    char* sql;
    ...
    sql = switch_mprintf(
        "select contact, profile_name, '%q' "
        "from sip_registrations where profile_name='%q' "
        "and upper(sip_user)=upper('%q') "
        "and (sip_host='%q' or presence_hosts like '%%%q%%')%s%s",
        (concat != NULL) ? concat : "", profile->name, user, domain, domain,
        (sql_match_user_agent != NULL) ? sql_match_user_agent : "",
        (sql_exclude_contact != NULL) ? sql_exclude_contact : "");
    switch_assert(sql);
...
SWITCH_STANDARD_API(sofia_contact_function) {
    ...
    if (profile) {
        ...
        if (!zstr(profile->domain_name) && !zstr(profile_name) &&
            !strcmp(profile_name, profile->name)) {
            domain = profile->domain_name;
        }

        select_from_profile(profile, user, domain, concat, exclude_contact, match_user_agent,
                            &mystream, SWITCH_FALSE);
        sofia_glue_release_profile(profile);
    } else if (!zstr(domain)) {
        ...
        if (i) {
            for (j = 0; j < i; j++) {
                select_from_profile(profiles[j], user, domain, concat, exclude_contact,
                                    match_user_agent, &mystream, SWITCH_TRUE);
...
static int list_result_callback(void* pArg, int argc, char** argv, char** columnNames) {
    ...
    if (!cbt->single_col) cbt->stream->write_function(cbt->stream, "\n");
    return 0;
}

static void get_presence_data(sofia_profile_t* profile, const char* user, const char* domain,
                              const char* search, switch_stream_handle_t* stream) {
    ...
    sql = switch_mprintf(
        " select %q from sip_registrations as r left join sip_presence as p "
        " on p.sip_host = r.sip_host and p.profile_name = r.profile_name and p.hostname = "
        "r.orig_hostname "
        " and p.sip_user = r.sip_user "
        " where r.sip_realm = '%q' and r.sip_user = '%q' and r.profile_name = '%q' ",
        select, domain, user, profile->name);
...
SWITCH_STANDARD_API(sofia_presence_data_function) {
    ...
    if (!strcasecmp(search, "list")) stream->write_function(stream, "+OK\n");
...
SWITCH_STANDARD_API(sofia_gateway_data_function) {
    ...
    if (!strcasecmp(param, "ivar") && gateway->ib_vars &&
        (val = switch_event_get_header(gateway->ib_vars, varname))) {
        stream->write_function(stream, "%s", val);
    } else if (!strcasecmp(param, "ovar") && gateway->ob_vars &&
               (val = switch_event_get_header(gateway->ob_vars, varname))) {
        stream->write_function(stream, "%s", val);
...
SWITCH_STANDARD_API(sofia_function) {
    ...
    static const char usage_string[] =
        ...
        "                     capture  <on|off>\n"
        "                     watchdog <on|off>\n\n"
        "sofia <status|xmlstatus> profile <name> [reg [<contact str>]] | [pres <pres str>] | [user "
        "<user@domain>]\n"
        "sofia <status|xmlstatus> gateway <name>\n\n"
        "sofia loglevel "
        "<all|default|tport|iptsec|nea|nta|nth_client|nth_server|nua|soa|sresolv|stun> [0-9]\n"
    ...
    if (!strcasecmp(argv[0], "profile")) {
        ...
        stream->write_function(stream, "+OK tracelevel is %s",
                               switch_log_level2str(mod_sofia_globals.tracelevel));
        goto done;
        ...
        if (argc > 2 && argv[2] && switch_is_number(argv[2])) {
            ...
            if (sofia_set_loglevel(argv[1], level) == SWITCH_STATUS_SUCCESS) {
                stream->write_function(stream,
                                       "Sofia log level for component [%s] has been set to [%d]",
                                       argv[1], level);
            } else {
                stream->write_function(stream, "%s", usage_string);
            }
            ...
            if (level >= 0) {
                stream->write_function(stream, "Sofia-sip loglevel for [%s] is [%d]", argv[1],
                                       level);
            } else {
                stream->write_function(stream, "%s", usage_string);
            }
        ...
        if (ston != -1) {
            ...
            stream->write_function(stream,
                                   "-ERR Usage: siptrace <on|off>|capture <on|off>|watchdog "
                                   "<on|off>|debug <sla|presence|none");
        }
        ...
        if (argv[1] && !strcasecmp(argv[1], "flush")) {
            ...
            if (x) {
                ...
                if (switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM,
                                                 MY_EVENT_RECOVERY_RECOVERED) ==
                    SWITCH_STATUS_SUCCESS) {
                    switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "recovered_calls",
                                                   "0");
                    switch_event_fire(&event);
                }
...
static switch_status_t sofia_manage(char* relative_oid, switch_management_action_t action,
...
// 在switch_ivr_originate中调用，处理外呼请求，创建新的session，并设置相关的变量和参数
static switch_call_cause_t sofia_outgoing_channel(
    switch_core_session_t* session, switch_event_t* var_event,
    switch_caller_profile_t* outbound_profile, switch_core_session_t** new_session,
    switch_memory_pool_t** pool, switch_originate_flag_t flags, switch_call_cause_t* cancel_cause) {
    ...
    *new_session = NULL;
    ...
    if (!(nsession = switch_core_session_request_uuid(
              sofia_endpoint_interface, SWITCH_CALL_DIRECTION_OUTBOUND, flags, pool,
    ...
    if (!strncasecmp(profile_name, "gateway/", 8)) {
        ...
        *dest++ = '\0';
        ...
        if (tech_pvt->transport != gateway_ptr->register_transport) {
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
                              "You are trying to use a different transport type for this gateway "
                              "(overriding the register-transport), this is unsupported!\n");
            cause = SWITCH_CAUSE_DESTINATION_OUT_OF_ORDER;
            goto error;
        }
        ...
        if (!sofia_test_flag(gateway_ptr, REG_FLAG_CALLERID)) {
            tech_pvt->gateway_from_str =
                switch_core_session_strdup(nsession, gateway_ptr->register_from);
        }

        if (!strchr(dest, '@')) {
            tech_pvt->dest = switch_core_session_sprintf(
                nsession, "sip:%s%s@%s", gateway_ptr->destination_prefix, dest,
                sofia_glue_strip_proto(gateway_ptr->register_proxy));
        } else {
            tech_pvt->dest = switch_core_session_sprintf(nsession, "sip:%s%s",
                                                         gateway_ptr->destination_prefix, dest);
        }
        ...
        if (params) {
            tech_pvt->invite_contact = switch_core_session_sprintf(
                nsession, "%s;%s", gateway_ptr->register_contact, params);
            tech_pvt->dest = switch_core_session_sprintf(nsession, "%s;%s", tech_pvt->dest, params);
        } else {
            tech_pvt->invite_contact =
                switch_core_session_strdup(nsession, gateway_ptr->register_contact);
        }

        gateway_ptr->ob_calls++;

        if (!zstr(gateway_ptr->from_domain) &&
            !switch_channel_get_variable(nchannel, "sip_invite_domain")) {
            if (!strcasecmp(gateway_ptr->from_domain, "auto-aleg-full")) {
                ...
                switch_channel_set_variable(nchannel, "sip_invite_domain",
                                            gateway_ptr->from_domain);
            }
        }

        if (!zstr(gateway_ptr->outbound_sticky_proxy) &&
            !switch_channel_get_variable(nchannel, "sip_route_uri")) {
            switch_channel_set_variable(nchannel, "sip_route_uri",
                                        gateway_ptr->outbound_sticky_proxy);
        }
    } else {
        const char* sip_destination_prefix =
            switch_str_nil(switch_event_get_header(var_event, "sip_destination_prefix"));
        ...
        *dest++ = '\0';
        ...
        if (!strncasecmp(dest, "sip:", 4)) {
            ...
            tech_pvt->dest = switch_core_session_sprintf(nsession, "sip:%s%s",
                                                         sip_destination_prefix, tech_pvt->e_dest);
        } else if (!strncasecmp(dest, "sips:", 5)) {
            tech_pvt->e_dest = switch_core_session_strdup(nsession, dest + 5);
            tech_pvt->dest = switch_core_session_sprintf(nsession, "sips:%s%s",
                                                         sip_destination_prefix, tech_pvt->e_dest);
            ...
            *host++ = '\0';
            ...
            if (sofia_reg_find_reg_url(profile, dest, profile_name, buf, sizeof(buf))) {
                tech_pvt->dest = switch_core_session_strdup(nsession, buf);
                tech_pvt->local_url =
                    switch_core_session_sprintf(nsession, "%s@%s", dest, profile_name);
                host = profile_name;
            } else {
                switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING,
                                  "Cannot locate registered user %s@%s\n", dest, profile_name);
            ...
            if (!strchr(host, '.') ||
                switch_true(switch_event_get_header(var_event, "sip_gethostbyname"))) {
            ...
            tech_pvt->dest =
                switch_core_session_sprintf(nsession, "sip:%s%s", sip_destination_prefix, dest);
            tech_pvt->e_dest = switch_core_session_strdup(nsession, dest);
        }
    }

    switch_channel_set_variable_printf(nchannel, "sip_local_network_addr", "%s",
    ...
    if (zstr(tech_pvt->mparams.remote_ip)) {
        switch_split_user_domain(switch_core_session_strdup(nsession, tech_pvt->dest), NULL,
                                 &tech_pvt->mparams.remote_ip);
    }
    ...
    if (dest_to) {
        if (strchr(dest_to, '@')) {
            ...
            tech_pvt->dest_to = switch_core_session_sprintf(nsession, "sip:%s@%s", dest_to,
                                                            host ? host : profile->sipip);
        }
    }
    ...
    if (gateway_ptr && gateway_ptr->ob_vars) {
        ...
        for (hp = gateway_ptr->ob_vars->headers; hp; hp = hp->next) {
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(nsession), SWITCH_LOG_DEBUG,
                              "%s setting variable [%s]=[%s]\n", switch_channel_get_name(nchannel),
                              hp->name, hp->value);
    ...
    *new_session = nsession;
    ...
    if ((hval = switch_event_get_header(var_event, "sip_auto_answer")) && switch_true(hval)) {
        switch_channel_set_variable_printf(nchannel, "sip_h_Call-Info", "<sip:%s>;answer-after=0",
                                           profile->sipip);
    ...
    if (((hval = switch_event_get_header(var_event, "effective_callee_id_name")) ||
         (hval = switch_event_get_header(var_event, "sip_callee_id_name"))) &&
        !zstr(hval)) {
    ...
    if (((hval = switch_event_get_header(var_event, "effective_callee_id_number")) ||
         (hval = switch_event_get_header(var_event, "sip_callee_id_number"))) &&
        !zstr(hval)) {
    ...
    if (session) {
        ...
        if (!(vval = switch_channel_get_variable(o_channel, "sip_copy_custom_headers")) ||
            switch_true(vval)) {
            switch_ivr_transfer_variable(session, nsession, SOFIA_SIP_HEADER_PREFIX_T);
        }

        if (!(vval = switch_channel_get_variable(o_channel, "sip_copy_multipart")) ||
            switch_true(vval)) {
        ...
        if (switch_core_session_compare(session, nsession)) {
            /* It's another sofia channel! so lets cache what they use as a pt for telephone event
               so we can keep it the same
             */
            ...
            /* SNARK: lets copy this across so we can see if we're the other leg of 3PCC +
             * bypass_media... */
            if (sofia_test_flag(ctech_pvt, TFLAG_3PCC) &&
                (switch_channel_test_flag(o_channel, CF_PROXY_MODE) ||
                 switch_channel_test_flag(o_channel, CF_PROXY_MEDIA))) {
                sofia_set_flag(tech_pvt, TFLAG_3PCC_INVITE);
...
error:
    /* gateway pointer lock is really a readlock of the profile so we let the profile release below
     * free that lock if we have a profile */
...
static int notify_csta_callback(void* pArg, int argc, char** argv, char** columnNames) {
    ...
    // nh = nua_handle(profile->nua, NULL, NUTAG_URL(dst->contact), SIPTAG_FROM_STR(id),
    // SIPTAG_TO_STR(id), SIPTAG_CONTACT_STR(profile->url), TAG_END());
    nh = nua_handle(profile->nua, NULL, NUTAG_URL(dst->contact), SIPTAG_FROM_STR(full_to),
                    SIPTAG_TO_STR(full_from), SIPTAG_CONTACT_STR(profile->url), TAG_END());
    ...
    nua_notify(nh, NUTAG_NEWSUB(1), TAG_IF(dst->route_uri, NUTAG_PROXY(route_uri)),
               ...
               TAG_IF(call_id, SIPTAG_CALL_ID_STR(call_id)), SIPTAG_EVENT_STR("as-feature-event"),
               SIPTAG_CONTENT_TYPE_STR(ct),
               TAG_IF(!zstr(extra_headers), SIPTAG_HEADER_STR(extra_headers)),
               TAG_IF(!zstr(body), SIPTAG_PAYLOAD_STR(body)), SIPTAG_CSEQ(cseq), TAG_END());
...
static int notify_callback(void* pArg, int argc, char** argv, char** columnNames) {
    ...
    nua_notify(nh, NUTAG_NEWSUB(1), SIPTAG_SUBSCRIPTION_STATE_STR("terminated;reason=noresource"),
               TAG_IF(dst->route_uri, NUTAG_PROXY(route_uri)),
               TAG_IF(dst->route, SIPTAG_ROUTE_STR(dst->route)), SIPTAG_EVENT_STR(es),
               SIPTAG_CONTENT_TYPE_STR(ct), TAG_IF(!zstr(body), SIPTAG_PAYLOAD_STR(body)),
               TAG_END());
...
void general_event_handler(switch_event_t* event) {
    switch (event->event_id) {
        case SWITCH_EVENT_NOTIFY: {
            ...
            if (contact_uri) {
                ...
                if (to_uri && from_uri) {
                    ...
                    nh = nua_handle(profile->nua, NULL, NUTAG_URL(dst->contact),
                                    SIPTAG_FROM_STR(from_uri), SIPTAG_TO_STR(to_uri),
                    ...
                    nua_notify(
                        nh, NUTAG_NEWSUB(1),
                        TAG_IF(sip_sub_st, SIPTAG_SUBSCRIPTION_STATE_STR(sip_sub_st)),
                        TAG_IF(dst->route_uri, NUTAG_PROXY(dst->route_uri)),
                        TAG_IF(dst->route, SIPTAG_ROUTE_STR(dst->route)),
                        TAG_IF(call_id, SIPTAG_CALL_ID_STR(call_id)), SIPTAG_EVENT_STR(es),
                        SIPTAG_CONTENT_TYPE_STR(ct), TAG_IF(!zstr(body), SIPTAG_PAYLOAD_STR(body)),
                ...
                if (to_uri && from_uri) {
                    ...
                    nh = nua_handle(profile->nua, NULL, NUTAG_URL(to_uri),
                                    SIPTAG_FROM_STR(from_uri), SIPTAG_TO_STR(to_uri),
                                    SIPTAG_CONTACT_STR(profile->url), TAG_END());
            ...
            if (uuid && ct && es) {
                ...
                if ((session = switch_core_session_locate(uuid))) {
                    if ((tech_pvt = switch_core_session_get_private(session))) {
                        ...
                        nua_notify(
                            ...
                            SIPTAG_SUBSCRIPTION_STATE_STR("terminated;reason=noresource"),
                            SIPTAG_EVENT_STR(es), SIPTAG_CONTENT_TYPE_STR(ct),
            ...
            } else if (profile_name && ct && es && user && host &&
                       (profile = sofia_glue_find_profile(profile_name))) {
                char* sql;
                ...
                if (call_id) {
                    sql = switch_mprintf(
                        ...
                        "from sip_registrations where call_id='%q'",
                        ct, es, switch_str_nil(body), call_id);
                } else {
                    if (!strcasecmp(es, "message-summary")) {
                        sql = switch_mprintf(
                            "select sip_user,sip_host,contact,profile_name,'%q','%q','%q' "
                            "from sip_registrations where mwi_user='%q' and mwi_host='%q'",
                            ct, es, switch_str_nil(body), switch_str_nil(user),
                            switch_str_nil(host));
                    } else {
                        sql = switch_mprintf(
                            "select sip_user,sip_host,contact,profile_name,'%q','%q','%q' "
            ...
            if (profile_name && user && host && (profile = sofia_glue_find_profile(profile_name))) {
                ...
                if (csta_event) {
                    if (!strcmp(csta_event, "init")) {
                        ...
                        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
                                          "Sending multipart with DND and CFWD\n");
                        ...
                        if (switch_event_get_header(event, "forward_immediate")) {
                            ...
                            write_csta_xml_chunk(event, fwdi_stream, "ForwardingEvent",
                                                 "forwardImmediate");
                            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
                                              "[%s] is %d bytes long\n", (char*)fwdi_stream.data,
                                              (int)strlen(fwdi_stream.data));
                            stream.write_function(
                                &stream,
                                "--%s\r\nContent-Type: "
                                "application/x-as-feature-event+xml\r\nContent-Length: "
                                "%d\r\nContent-ID: <%si@%s>\r\n\r\n%s",
                                boundary_string, strlen(fwdi_stream.data), user, host,
                                fwdi_stream.data);
                        ...
                        if (switch_event_get_header(event, "forward_busy")) {
                            ...
                            write_csta_xml_chunk(event, fwdb_stream, "ForwardingEvent",
                                                 "forwardBusy");
                            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
                                              "[%s] is %d bytes long\n", (char*)fwdb_stream.data,
                                              (int)strlen(fwdb_stream.data));
                            stream.write_function(
                                &stream,
                                "--%s\r\nContent-Type: "
                                "application/x-as-feature-event+xml\r\nContent-Length: "
                                "%d\r\nContent-ID: <%sb@%s>\r\n\r\n%s",
                                boundary_string, strlen(fwdb_stream.data), user, host,
                                fwdb_stream.data);
                        ...
                        if (switch_event_get_header(event, "forward_no_answer")) {
                            ...
                            write_csta_xml_chunk(event, fwdna_stream, "ForwardingEvent",
                                                 "forwardNoAns");
                            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
                                              "[%s] is %d bytes long\n", (char*)fwdna_stream.data,
                                              (int)strlen(fwdna_stream.data));
                            stream.write_function(
                                &stream,
                                "--%s\r\nContent-Type: "
                                "application/x-as-feature-event+xml\r\nContent-Length: "
                                "%d\r\nContent-ID: <%sn@%s>\r\n\r\n%s",
                                boundary_string, strlen(fwdna_stream.data), user, host,
                                fwdna_stream.data);
                        ...
                        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
                                          "[%s] is %d bytes long\n", (char*)dnd_stream.data,
                                          (int)strlen(dnd_stream.data));
                        stream.write_function(
                            &stream,
                            "--%s\r\nContent-Type: "
                            "application/x-as-feature-event+xml\r\nContent-Length: "
                            "%d\r\nContent-ID: <%sd@%s>\r\n\r\n%s",
                            boundary_string, strlen(dnd_stream.data), user, host, dnd_stream.data);
                        ...
                        // this will need some work to handle the different types of forwarding
                        // events
                ...
                if (call_id) {
                    sql = switch_mprintf(
                        "select "
                        "sip_user,sip_host,contact,profile_name,call_id,full_from,full_to,expires,'"
                        "%q', '%q' "
                        "from sip_subscriptions where event='as-feature-event' and call_id='%q'",
                        stream.data, ct, call_id);
                } else {
                    sql = switch_mprintf(
                        "select "
                        "sip_user,sip_host,contact,profile_name,call_id,full_from,full_to,expires,'"
                        "%q', '%q' "
                        "from sip_subscriptions where event='as-feature-event' and sip_user='%q' "
                        "and sip_host='%q'",
                        stream.data, ct, switch_str_nil(user), switch_str_nil(host));
                }
        ...
        case SWITCH_EVENT_SEND_MESSAGE: {
            ...
            if (ct && user && host) {
                ...
                if (!(list = sofia_reg_find_reg_url_multi(profile, user, host))) {
                    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
                                      "Can't find registered user %s@%s\n", user, host);
                    sofia_glue_release_profile(profile);
                    return;
                }
                ...
                for (m = list->head; m; m = m->next) {
                    ...
                    nh = nua_handle(profile->nua, NULL, NUTAG_URL(contact), SIPTAG_FROM_STR(id),
                                    SIPTAG_TO_STR(id), SIPTAG_CONTACT_STR(profile->url), TAG_END());

                    nua_message(nh, NUTAG_NEWSUB(1), SIPTAG_CONTENT_TYPE_STR(ct),
                                TAG_IF(!zstr(body), SIPTAG_PAYLOAD_STR(body)),
                                TAG_IF(!zstr(subject), SIPTAG_SUBJECT_STR(subject)), TAG_END());
                }
                ...
                if ((session = switch_core_session_locate(uuid))) {
                    if ((tech_pvt = switch_core_session_get_private(session))) {
                        ...
                        nua_message(
                            tech_pvt->nh, SIPTAG_CONTENT_TYPE_STR(ct), SIPTAG_PAYLOAD_STR(body),
                            TAG_IF(!zstr(body), SIPTAG_PAYLOAD_STR(body)),
        ...
        case SWITCH_EVENT_SEND_INFO: {
            ...
            if (call_id) {
                ...
                if (local_user_full) {
                    ...
                    if (!local_host || !sofia_reg_find_reg_url(profile, local_user, local_host, buf,
                                                               sizeof(buf))) {
                        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
                                          "Can't find local user\n");
                        goto done;
                    }
                ...
                nh = nua_handle(profile->nua, NULL, NUTAG_URL(to_uri), SIPTAG_FROM_STR(from_uri),
                                SIPTAG_TO_STR(to_uri), SIPTAG_CONTACT_STR(profile->url), TAG_END());

                nua_handle_bind(nh, &mod_sofia_globals.destroy_private);
            }

            nua_info(nh, TAG_IF(ct, SIPTAG_CONTENT_TYPE_STR(ct)),
                     TAG_IF(cd, SIPTAG_CONTENT_DISPOSITION_STR(cd)),
                     TAG_IF(alert_info, SIPTAG_ALERT_INFO_STR(alert_info)),
                     TAG_IF(call_info, SIPTAG_CALL_INFO_STR(call_info)),
                     TAG_IF(!zstr(body), SIPTAG_PAYLOAD_STR(body)), TAG_END());
        ...
        case SWITCH_EVENT_TRAP: {
            ...
            if (!strcmp(cond, "network-external-address-change") &&
                mod_sofia_globals.auto_restart) {
                const char* old_ip4 =
                    switch_event_get_header_nil(event, "network-external-address-previous-v4");
                const char* new_ip4 =
                    switch_event_get_header_nil(event, "network-external-address-change-v4");
                ...
                const char* old_ip4 =
                    switch_event_get_header_nil(event, "network-address-previous-v4");
                const char* new_ip4 =
                    switch_event_get_header_nil(event, "network-address-change-v4");
                const char* old_ip6 =
                    switch_event_get_header_nil(event, "network-address-previous-v6");
                const char* new_ip6 =
                    switch_event_get_header_nil(event, "network-address-change-v6");

                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO,
                                  "EVENT_TRAP: IP change detected\n");
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO,
                                  "IP change detected [%s]->[%s] [%s]->[%s]\n", old_ip4, new_ip4,
                                  old_ip6, new_ip6);

                snprintf(mod_sofia_globals.guess_ip, sizeof(mod_sofia_globals.guess_ip), "%s",
                         new_ip4);
...
void write_csta_xml_chunk(switch_event_t* event, switch_stream_handle_t stream,
                          const char* csta_event, char* fwdtype) {
...
switch_status_t list_profiles_full(const char* line, const char* cursor,
                                   switch_console_callback_match_t** matches,
                                   switch_bool_t show_aliases) {
    sofia_profile_t* profile = NULL;
    switch_hash_index_t* hi;
    void* val;
    const void* vvar;
...
switch_status_t list_profiles(const char* line, const char* cursor,
                              switch_console_callback_match_t** matches) {
    return list_profiles_full(line, cursor, matches, SWITCH_TRUE);
}

static switch_status_t list_gateways(const char* line, const char* cursor,
                                     switch_console_callback_match_t** matches) {
    sofia_profile_t* profile = NULL;
    switch_hash_index_t* hi;
    void* val;
...
static switch_status_t list_profile_gateway(const char* line, const char* cursor,
                                            switch_console_callback_match_t** matches) {
    sofia_profile_t* profile = NULL;
    switch_console_callback_match_t* my_matches = NULL;
    switch_status_t status = SWITCH_STATUS_FALSE;
    ...
    if ((bargee_session = switch_core_session_locate((char*)data))) {
        if (bargee_session == session) {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
                              "BARGE: %s (cannot barge on myself)\n", (char*)data);
        } else {
...
static switch_status_t sofia_stir_shaken_vs_create(stir_shaken_context_t* context) {
    ...
    if (!sofia_stir_shaken_vs) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
                          "Failed to create Identity verification service!\n");
        return SWITCH_STATUS_FALSE;
    }
    if (mod_sofia_globals.stir_shaken_vs_ca_dir) {
        if (stir_shaken_vs_load_ca_dir(context, sofia_stir_shaken_vs,
                                       mod_sofia_globals.stir_shaken_vs_ca_dir) !=
            STIR_SHAKEN_STATUS_OK) {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
                              "Failed to load trusted root certificates from %s\n",
                              mod_sofia_globals.stir_shaken_vs_ca_dir);
            return SWITCH_STATUS_FALSE;
        } else {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO,
                              "Loaded trusted root certificates from %s\n",
                              mod_sofia_globals.stir_shaken_vs_ca_dir);
        }
    }
    stir_shaken_vs_set_x509_cert_path_check(context, sofia_stir_shaken_vs,
                                            mod_sofia_globals.stir_shaken_vs_cert_path_check);
...
static switch_status_t sofia_stir_shaken_as_create(stir_shaken_context_t* context) {
    if (mod_sofia_globals.stir_shaken_as_key && mod_sofia_globals.stir_shaken_as_url) {
        ...
        if (!sofia_stir_shaken_as) {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
                              "Failed to create Identity authentication service!\n");
            return SWITCH_STATUS_FALSE;
        }
        if (stir_shaken_as_load_private_key(context, sofia_stir_shaken_as,
                                            mod_sofia_globals.stir_shaken_as_key) !=
            STIR_SHAKEN_STATUS_OK) {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
                              "Failed to load key for Identity authentication service: %s",
                              mod_sofia_globals.stir_shaken_as_key);
...
static void sofia_stir_shaken_create_services(void) {
    ...
    if (stir_shaken_init(&context, STIR_SHAKEN_LOGLEVEL_NOTHING) != STIR_SHAKEN_STATUS_OK) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_CRIT,
                          "Failed to initialize stirshaken library!\n");
        return;
    }
...
static char* canonicalize_phone_number(const char* number) {
    ...
    for (i = 0; i < number_len; i++) {
        if (isdigit(canonicalized_number[i]) || canonicalized_number[i] == '#' ||
            canonicalized_number[i] == '*') {
...
static switch_status_t sofia_stir_shaken_validate_passport_claims(switch_core_session_t* session,
                                                                  long iat, const char* orig,
                                                                  int orig_is_tn, const char* dest,
                                                                  int dest_is_tn) {
    switch_channel_t* channel = switch_core_session_get_channel(session);
    ...
    if (iat > now) {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING,
                          "PASSporT iat is in the future\n");
        return SWITCH_STATUS_FALSE;
    } else if (now - iat > 60) {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING,
                          "PASSporT iat is too old\n");
        return SWITCH_STATUS_FALSE;
    }

    if (mod_sofia_globals.stir_shaken_vs_require_date ||
        switch_true(switch_channel_get_variable(channel, "sip_stir_shaken_vs_require_date"))) {
        const char* sip_epoch_time_var =
            switch_channel_get_variable(channel, "sip_date_epoch_time");
        switch_time_t sip_epoch_time;

        if (zstr(sip_epoch_time_var)) {
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING,
                              "Missing required SIP Date\n");
            return SWITCH_STATUS_FALSE;
        }
        sip_epoch_time = strtol(sip_epoch_time_var, NULL, 10);
        if (sip_epoch_time > now) {
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING,
                              "SIP Date %s is in the future\n", sip_epoch_time_var);
            return SWITCH_STATUS_FALSE;
        }
        if (now - sip_epoch_time > 60) {
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING,
                              "SIP Date %s is too old\n", sip_epoch_time_var);
            return SWITCH_STATUS_FALSE;
        }
        if ((iat > sip_epoch_time && iat - sip_epoch_time > 60) ||
            (iat < sip_epoch_time && sip_epoch_time - iat > 60)) {
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING,
                              "SIP Date %s is too far from PASSporT iat %ld\n", sip_epoch_time_var,
                              iat);
            return SWITCH_STATUS_FALSE;
        }
    ...
    if (zstr(from) || zstr(to) || zstr(orig) || zstr(dest)) {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING,
                          "Missing data to verify SIP From/To matches PASSporT claims. From=%s, "
                          "To=%s, orig=%s, dest=%s\n",
                          from, to, orig, dest);
        status = SWITCH_STATUS_FALSE;
        ...
        switch_log_printf(
            ...
            "SIP From/To does not match PASSporT claims. From=%s, To=%s, orig=%s, dest=%s\n", from,
            to, orig, dest);
    } else {
        status = SWITCH_STATUS_SUCCESS;
    }
...
SWITCH_STANDARD_APP(sofia_stir_shaken_vs_function) {
    ...
    int orig_is_tn = 0;
    switch_bool_t hangup_on_fail =
        switch_true(switch_channel_get_variable(channel, "sip_stir_shaken_vs_hangup_on_fail"));

    // TODO: compact Identity header is not supported - this will require construction of PASSporT
    // from SIP headers in order to check signature

    if (zstr(identity_header)) {
        // Nothing to do
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                          "No-TN-Validation: no SIP Identity\n");
    ...
    verify_signature_status = stir_shaken_vs_sih_verify(
        &verify_signature_context, sofia_stir_shaken_vs, identity_header, &cert, &passport);
    if (verify_signature_status != STIR_SHAKEN_STATUS_OK) {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                          "PASSporT failed signature verification: %s\n",
        ...
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                          "PASSporT passed signature verification\n");
    }
    ...
    if (passport) {
        ...
        const char* timeout_str =
            switch_channel_get_variable(channel, "sip_stir_shaken_vs_max_age");
        ...
        validate_passport_status = stir_shaken_passport_validate_iat_against_freshness(
            &validate_passport_context, passport, timeout);
        if (validate_passport_status != STIR_SHAKEN_STATUS_OK) {
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                              "PASSporT failed stale check: %s\n",
            ...
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                              "PASSporT passed stale check\n");
        }

        // validate the required PASSporT headers and grants are set
        validate_passport_status =
            stir_shaken_passport_validate_headers_and_grants(&validate_passport_context, passport);
        if (validate_passport_status != STIR_SHAKEN_STATUS_OK) {
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                              "PASSporT failed header and grant validation: %s\n",
                              stir_shaken_get_error(&validate_passport_context, &stir_error));
            if (hangup_on_fail) {
                switch_channel_hangup(channel, SWITCH_CAUSE_INVALID_IDENTITY);
                if (validate_passport_status == STIR_SHAKEN_STATUS_OK &&
                    verify_signature_status == STIR_SHAKEN_STATUS_OK) {
            ...
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                              "PASSporT passed header and grant validation\n");
        }
    }
    ...
    if (passport) {
        ...
        int dest_is_tn = 0;
        char* orig =
            stir_shaken_passport_get_identity(&validate_claims_context, passport, &orig_is_tn);
        char* dest = sofia_stir_shaken_passport_get_dest(
            passport, &dest_is_tn);  // TODO libstirshaken should provide helper for 'dest' values
        long iat = stir_shaken_passport_get_grant_int(&validate_claims_context, passport, "iat");
        claim_status = sofia_stir_shaken_validate_passport_claims(session, iat, orig, orig_is_tn,
                                                                  dest, dest_is_tn);
        if (claim_status != SWITCH_STATUS_SUCCESS) {
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                              "PASSporT claims do not match SIP request\n");
            if (hangup_on_fail) {
                switch_channel_hangup(channel, SWITCH_CAUSE_INVALID_IDENTITY);
            ...
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                              "PASSporT claims match SIP request\n");
        }
        switch_safe_free(orig);
        switch_safe_free(dest);
    }
    ...
    if (!zstr(attestation) && verify_signature_status == STIR_SHAKEN_STATUS_OK &&
        validate_passport_status == STIR_SHAKEN_STATUS_OK &&
        claim_status == SWITCH_STATUS_SUCCESS) {
        if (orig_is_tn) {
            switch_channel_set_variable_printf(channel, "sip_verstat_detailed",
                                               "TN-Validation-Passed-%s", attestation);
        } else {
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                              "No-TN-Validation: PASSporT orig is not a telephone number\n");
            switch_channel_set_variable(channel, "sip_verstat", "No-TN-Validation");
        }
        if (orig_is_tn && !strcmp(attestation, "A")) {
            ...
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                              "No-TN-Validation: PASSporT only has \"%s\" attestation\n",
                              attestation);
            switch_channel_set_variable(channel, "sip_verstat", "No-TN-Validation");
        }
    } else if (!passport || !cert || zstr(attestation) ||
               verify_signature_status == STIR_SHAKEN_STATUS_OK) {
        ...
        // bad signature
        switch_channel_set_variable_printf(channel, "sip_verstat_detailed",
                                           "TN-Validation-Failed-%s", attestation);
    ...
    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO,
                      "verstat=%s, verstat_detailed=%s\n",
                      switch_channel_get_variable(channel, "sip_verstat"),
                      switch_channel_get_variable(channel, "sip_verstat_detailed"));
}

/* This assumes TN attestation for orig and dest only */
char* sofia_stir_shaken_as_create_identity_header(switch_core_session_t* session,
                                                  const char* attest, const char* orig,
                                                  const char* dest) {
#if HAVE_STIRSHAKEN
    ...
    if (zstr(attest) || zstr(orig) || zstr(dest) || !mod_sofia_globals.stir_shaken_as_url) {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING,
                          "Missing required parameter to create PASSporT\n");
        return NULL;
    }
    ...
    passport = stir_shaken_as_authenticate_to_sih(&as_context, sofia_stir_shaken_as,
                                                  &passport_params, NULL);
...
SWITCH_MODULE_LOAD_FUNCTION(mod_sofia_load) {
    ...
    switch_find_local_ip(mod_sofia_globals.guess_ip, sizeof(mod_sofia_globals.guess_ip),
                         &mod_sofia_globals.guess_mask, AF_INET);
    ...
    switch_queue_create(&mod_sofia_globals.presence_queue, SOFIA_QUEUE_SIZE,
                        mod_sofia_globals.pool);
    switch_queue_create(&mod_sofia_globals.general_event_queue, SOFIA_QUEUE_SIZE,
                        mod_sofia_globals.pool);
    ...
    switch_queue_create(&mod_sofia_globals.msg_queue,
                        SOFIA_MSG_QUEUE_SIZE * mod_sofia_globals.max_msg_queues,
                        mod_sofia_globals.pool);
    ...
    if (sofia_init() != SWITCH_STATUS_SUCCESS) {
        ...
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
                          "Failed to initialize Sofia-SIP!\n");
        return SWITCH_STATUS_GENERR;
    }
    ...
    if (config_sofia(SOFIA_CONFIG_LOAD, NULL) != SWITCH_STATUS_SUCCESS) {
        ...
        switch_goto_status(SWITCH_STATUS_GENERR, err);
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
                          "Failed to load Sofia-SIP configuration!\n");
        return SWITCH_STATUS_GENERR;
    }
    ...
    if (switch_event_bind(modname, SWITCH_EVENT_CUSTOM, MULTICAST_EVENT, event_handler, NULL) !=
        SWITCH_STATUS_SUCCESS) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Couldn't bind!\n");
        switch_goto_status(SWITCH_STATUS_TERM, err);
    }

    if (switch_event_bind(modname, SWITCH_EVENT_CONFERENCE_DATA, SWITCH_EVENT_SUBCLASS_ANY,
                          sofia_presence_event_handler, NULL) != SWITCH_STATUS_SUCCESS) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Couldn't bind!\n");
        switch_goto_status(SWITCH_STATUS_GENERR, err);
        return SWITCH_STATUS_GENERR;
    }

    if (switch_event_bind(modname, SWITCH_EVENT_PRESENCE_IN, SWITCH_EVENT_SUBCLASS_ANY,
                          sofia_presence_event_handler, NULL) != SWITCH_STATUS_SUCCESS) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Couldn't bind!\n");
        switch_goto_status(SWITCH_STATUS_GENERR, err);
        return SWITCH_STATUS_GENERR;
    ...
    if (switch_event_bind(modname, SWITCH_EVENT_PRESENCE_OUT, SWITCH_EVENT_SUBCLASS_ANY,
                          sofia_presence_event_handler, NULL) != SWITCH_STATUS_SUCCESS) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Couldn't bind!\n");
        switch_goto_status(SWITCH_STATUS_GENERR, err);
        return SWITCH_STATUS_GENERR;
    ...
    if (switch_event_bind(modname, SWITCH_EVENT_PRESENCE_PROBE, SWITCH_EVENT_SUBCLASS_ANY,
                          sofia_presence_event_handler, NULL) != SWITCH_STATUS_SUCCESS) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Couldn't bind!\n");
        switch_goto_status(SWITCH_STATUS_GENERR, err);
        return SWITCH_STATUS_GENERR;
    ...
    if (switch_event_bind(modname, SWITCH_EVENT_ROSTER, SWITCH_EVENT_SUBCLASS_ANY,
                          sofia_presence_event_handler, NULL) != SWITCH_STATUS_SUCCESS) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Couldn't bind!\n");
        switch_goto_status(SWITCH_STATUS_GENERR, err);
        return SWITCH_STATUS_GENERR;
    ...
    if (switch_event_bind(modname, SWITCH_EVENT_MESSAGE_WAITING, SWITCH_EVENT_SUBCLASS_ANY,
                          sofia_presence_event_handler, NULL) != SWITCH_STATUS_SUCCESS) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Couldn't bind!\n");
        switch_goto_status(SWITCH_STATUS_GENERR, err);
        return SWITCH_STATUS_GENERR;
    ...
    if (switch_event_bind(modname, SWITCH_EVENT_TRAP, SWITCH_EVENT_SUBCLASS_ANY,
                          general_queue_event_handler, NULL) != SWITCH_STATUS_SUCCESS) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Couldn't bind!\n");
        switch_goto_status(SWITCH_STATUS_GENERR, err);
        return SWITCH_STATUS_GENERR;
    }

    if (switch_event_bind(modname, SWITCH_EVENT_NOTIFY, SWITCH_EVENT_SUBCLASS_ANY,
                          general_queue_event_handler, NULL) != SWITCH_STATUS_SUCCESS) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Couldn't bind!\n");
        switch_goto_status(SWITCH_STATUS_GENERR, err);
        return SWITCH_STATUS_GENERR;
    ...
    if (switch_event_bind(modname, SWITCH_EVENT_PHONE_FEATURE, SWITCH_EVENT_SUBCLASS_ANY,
                          general_queue_event_handler, NULL) != SWITCH_STATUS_SUCCESS) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Couldn't bind!\n");
        switch_goto_status(SWITCH_STATUS_GENERR, err);
        return SWITCH_STATUS_GENERR;
    ...
    if (switch_event_bind(modname, SWITCH_EVENT_SEND_MESSAGE, SWITCH_EVENT_SUBCLASS_ANY,
                          general_queue_event_handler, NULL) != SWITCH_STATUS_SUCCESS) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Couldn't bind!\n");
        switch_goto_status(SWITCH_STATUS_GENERR, err);
        return SWITCH_STATUS_GENERR;
    ...
    if (switch_event_bind(modname, SWITCH_EVENT_SEND_INFO, SWITCH_EVENT_SUBCLASS_ANY,
                          general_queue_event_handler, NULL) != SWITCH_STATUS_SUCCESS) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Couldn't bind!\n");
        switch_goto_status(SWITCH_STATUS_GENERR, err);
        return SWITCH_STATUS_GENERR;
    ...
    *module_interface = switch_loadable_module_create_module_interface(pool, modname);
    ...
    management_interface =
        switch_loadable_module_create_interface(*module_interface, SWITCH_MANAGEMENT_INTERFACE);
    ...
    SWITCH_ADD_APP(app_interface, "sofia_stir_shaken_vs",
                   "Verify SIP Identity header and store result in sip_verstat channel variable",
                   "Verify SIP Identity header and store result in sip_verstat channel variable",
                   sofia_stir_shaken_vs_function, "", SAF_SUPPORT_NOMEDIA);

    SWITCH_ADD_API(api_interface, "sofia", "Sofia Controls", sofia_function, "<cmd> <args>");
    SWITCH_ADD_API(api_interface, "sofia_gateway_data", "Get data from a sofia gateway",
                   sofia_gateway_data_function, "<gateway_name> [ivar|ovar|var] <name>");
    ...
    switch_console_set_complete(
        "add sofia global ::[siptrace::standby::capture::watchdog ::[on:off");
    ...
    switch_console_set_complete(
        "add sofia profile ::sofia::list_profiles ::[start:rescan:restart:check_sync");
    ...
    switch_console_set_complete(
        "add sofia profile ::sofia::list_profiles ::[register:unregister:killgw:startgw "
    ...
    SWITCH_ADD_API(api_interface, "sofia_username_of", "Sofia Username Lookup",
                   sofia_username_of_function, "[profile/]<user>@<domain>");
    SWITCH_ADD_API(api_interface, "sofia_contact", "Sofia Contacts", sofia_contact_function,
                   "[profile/]<user>@<domain>");
    SWITCH_ADD_API(api_interface, "sofia_count_reg", "Count Sofia registration",
                   sofia_count_reg_function, "[profile/]<user>@<domain>");
    SWITCH_ADD_API(api_interface, "sofia_dig", "SIP DIG", sip_dig_function, "<url>");
    SWITCH_ADD_API(api_interface, "sofia_presence_data", "Sofia Presence Data",
                   sofia_presence_data_function,
                   "[list|status|rpid|user_agent] [profile/]<user>@domain");
