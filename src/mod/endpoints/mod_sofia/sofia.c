...
static void parse_domain_tag(sofia_profile_t* profile, switch_xml_t x_domain_tag, const char* dname,
                             const char* parse, const char* alias);

void sofia_handle_sip_i_reinvite(switch_core_session_t* session, nua_t* nua,
                                 sofia_profile_t* profile, nua_handle_t* nh,
                                 sofia_private_t* sofia_private, sip_t const* sip,
                                 sofia_dispatch_event_t* de, tagi_t tags[]);

static void set_variable_sip_param(switch_channel_t* channel, char* header_type,
                                   sip_param_t const* params);

static void sofia_handle_sip_i_state(switch_core_session_t* session, int status, char const* phrase,
...
static void sofia_handle_sip_r_invite(switch_core_session_t* session, int status,
                                      char const* phrase, nua_t* nua, sofia_profile_t* profile,
...
static void sofia_set_accept_language_channel_variable(switch_channel_t* channel,
                                                       sip_t const* sip) {
    if (sip->sip_accept_language) {
        ...
        for (sip_accept_language = sip->sip_accept_language; sip_accept_language;
            ...
            if (count == 0) {
                switch_channel_set_variable(channel, "sip_accept_language",
                                            sip_accept_language->aa_value);
            }
...
void sofia_handle_sip_r_notify(switch_core_session_t* session, int status, char const* phrase,
                               nua_t* nua, sofia_profile_t* profile, nua_handle_t* nh,
                               sofia_private_t* sofia_private, sip_t const* sip,
                               sofia_dispatch_event_t* de, tagi_t tags[]) {
    ...
    if (tech_pvt->proxy_refer_uuid &&
        (other_session = switch_core_session_locate(tech_pvt->proxy_refer_uuid))) {
...
#define url_set_chanvars(session, url, varprefix)                                               \
    _url_set_chanvars(session, url, #varprefix "_user", #varprefix "_host", #varprefix "_port", \
                      #varprefix "_uri", #varprefix "_params")

static const char* _url_set_chanvars(switch_core_session_t* session, url_t* url,
                                     const char* user_var, const char* host_var,
                                     const char* port_var, const char* uri_var,
    ...
    if (sip) {
        ...
        if (sip->sip_record_route) {
            ...
            if (switch_channel_direction(channel) == SWITCH_CALL_DIRECTION_OUTBOUND ||
                ...
                switch_channel_set_variable(channel, "sip_invite_route_uri",
                                            (char*)reverse_stream.data);
                switch_channel_set_variable(channel, "sip_invite_record_route",
                                            (char*)forward_stream.data);
            } else {
                switch_channel_set_variable(channel, "sip_invite_route_uri",
                                            (char*)forward_stream.data);
                switch_channel_set_variable(channel, "sip_invite_record_route",
        ...
        if (sip->sip_from) {
            switch_channel_set_variable_strip_quotes(channel, "sip_from_display",
        ...
        if (sip->sip_to) {
            switch_channel_set_variable_strip_quotes(channel, "sip_to_display",
                                                     sip->sip_to->a_display);
 ...
 */
static void sofia_add_invite_header_to_chanvars(switch_channel_t* channel, nua_handle_t* nh,
                                                void* sip_header, const char* var) {
...
/**
 * Deep search into the SIP message to recreate the original headers, including multiple Diversions,
 * etc. Finally sets the "sip_invite_headers" to a string containing the 'original' SIP headers,
 * except that the order may have changed. Multiple headers will have the original internal order,
 * though.
 *
 ...
 */
static void sofia_parse_all_invite_headers(sip_t const* sip, switch_core_session_t* session,
                                           nua_handle_t* nh) {
    switch_channel_t* channel = switch_core_session_get_channel(session);
    sip_unknown_t* un;
    ...
    sofia_add_invite_header_to_chanvars(channel, nh, sip->sip_accept_encoding,
                                        "sip_i_accept_encoding");
    sofia_add_invite_header_to_chanvars(channel, nh, sip->sip_accept_language,
                                        "sip_i_accept_language");
    ...
    sofia_add_invite_header_to_chanvars(channel, nh, sip->sip_session_expires,
                                        "sip_i_session_expires");
    ...
    sofia_add_invite_header_to_chanvars(channel, nh, sip->sip_content_encoding,
                                        "sip_i_content_encoding");
    sofia_add_invite_header_to_chanvars(channel, nh, sip->sip_content_language,
                                        "sip_i_content_language");
    sofia_add_invite_header_to_chanvars(channel, nh, sip->sip_content_disposition,
                                        "sip_i_content_disposition");
    sofia_add_invite_header_to_chanvars(channel, nh, sip->sip_content_length,
                                        "sip_i_content_length");
    ...
    if (sip->sip_via) {
        ...
        for (vp = sip->sip_via; vp; vp = vp->v_next) {
            char* v = sip_header_as_string(nua_handle_get_home(nh), (void*)vp);
            switch_channel_add_variable_var_check(channel, "sip_i_via", v, SWITCH_FALSE,
                                                  SWITCH_STACK_PUSH);
            su_free(nua_handle_get_home(nh), v);
        }
    }
    ...
    if (sip->sip_record_route) {
        ...
        for (rrp = sip->sip_record_route; rrp; rrp = rrp->r_next) {
            char* rr = sip_header_as_string(nua_handle_get_home(nh), (void*)rrp);
            switch_channel_add_variable_var_check(channel, "sip_i_record_route", rr, SWITCH_FALSE,
                                                  SWITCH_STACK_PUSH);
    ...
    if (sip->sip_proxy_authorization) {
        ...
        for (vp = sip->sip_proxy_authorization; vp; vp = vp->au_next) {
            char* v = sip_header_as_string(nua_handle_get_home(nh), (void*)vp);
            switch_channel_add_variable_var_check(channel, "sip_i_proxy_authorization", v,
    ...
    if (sip->sip_call_info) {
        ...
        for (vp = sip->sip_call_info; vp; vp = vp->ci_next) {
            char* v = sip_header_as_string(nua_handle_get_home(nh), (void*)vp);
            switch_channel_add_variable_var_check(channel, "sip_i_call_info", v, SWITCH_FALSE,
                                                  SWITCH_STACK_PUSH);
            su_free(nua_handle_get_home(nh), v);
        }
    }
    ...
    if (sip->sip_accept) {
        ...
        for (vp = sip->sip_accept; vp; vp = vp->ac_next) {
            char* v = sip_header_as_string(nua_handle_get_home(nh), (void*)vp);
            switch_channel_add_variable_var_check(channel, "sip_i_accept", v, SWITCH_FALSE,
                                                  SWITCH_STACK_PUSH);
            su_free(nua_handle_get_home(nh), v);
        }
    }
    ...
    if (sip->sip_authorization) {
        ...
        for (vp = sip->sip_authorization; vp; vp = vp->au_next) {
            char* v = sip_header_as_string(nua_handle_get_home(nh), (void*)vp);
            switch_channel_add_variable_var_check(channel, "sip_i_authorization", v, SWITCH_FALSE,
                                                  SWITCH_STACK_PUSH);
            su_free(nua_handle_get_home(nh), v);
        }
    }
    ...
    if ((alert_info = sip_alert_info(sip))) {
        ...
        for (vp = alert_info; vp; vp = vp->ai_next) {
            char* v = sip_header_as_string(nua_handle_get_home(nh), (void*)vp);
            switch_channel_add_variable_var_check(channel, "sip_i_alert_info", v, SWITCH_FALSE,
                                                  SWITCH_STACK_PUSH);
            su_free(nua_handle_get_home(nh), v);
        }
    }
    ...
    if ((passerted = sip_p_asserted_identity(sip))) {
        ...
        for (vp = passerted; vp; vp = vp->paid_next) {
            char* v = sip_header_as_string(nua_handle_get_home(nh), (void*)vp);
            switch_channel_add_variable_var_check(channel, "sip_i_p_asserted_identity", v,
                                                  SWITCH_FALSE, SWITCH_STACK_PUSH);
            su_free(nua_handle_get_home(nh), v);
        }
    }
    ...
    if ((ppreferred = sip_p_preferred_identity(sip))) {
        ...
        for (vp = ppreferred; vp; vp = vp->ppid_next) {
            char* v = sip_header_as_string(nua_handle_get_home(nh), (void*)vp);
            switch_channel_add_variable_var_check(channel, "sip_i_p_preferred_identity", v,
                                                  SWITCH_FALSE, SWITCH_STACK_PUSH);
            su_free(nua_handle_get_home(nh), v);
        }
    }
    ...
    if ((rpid = sip_remote_party_id(sip))) {
        ...
        for (vp = rpid; vp; vp = vp->rpid_next) {
            char* v = sip_header_as_string(nua_handle_get_home(nh), (void*)vp);
            switch_channel_add_variable_var_check(channel, "sip_i_remote_party_id", v, SWITCH_FALSE,
                                                  SWITCH_STACK_PUSH);
            su_free(nua_handle_get_home(nh), v);
        }
    }
    ...
    if ((reply_to = sip_reply_to(sip))) {
        ...
        for (vp = reply_to; vp; vp = vp->rplyto_next) {
            char* v = sip_header_as_string(nua_handle_get_home(nh), (void*)vp);
            switch_channel_add_variable_var_check(channel, "sip_i_reply_to", v, SWITCH_FALSE,
                                                  SWITCH_STACK_PUSH);
            su_free(nua_handle_get_home(nh), v);
        }
    }
    ...
    for (un = sip->sip_unknown; un; un = un->un_next) {
        if (!zstr(un->un_name) && !zstr(un->un_value)) {
            ...
            if ((parsed_name = switch_mprintf("sip_i_%s", un->un_name))) {
                ...
                switch_channel_add_variable_var_check(channel, parsed_name, un->un_value,
                                                      SWITCH_FALSE, SWITCH_STACK_PUSH);
                free(parsed_name);
            }
        }
...
static switch_status_t sofia_pass_notify(switch_core_session_t* session, const char* uuid,
                                         const char* payload) {
    switch_core_session_t* other_session;
...
void sofia_handle_sip_i_notify(switch_core_session_t* session, int status, char const* phrase,
                               nua_t* nua, sofia_profile_t* profile, nua_handle_t* nh,
                               sofia_private_t* sofia_private, sip_t const* sip,
                               sofia_dispatch_event_t* de, tagi_t tags[]) {
    ...
    if (tech_pvt && tech_pvt->proxy_refer_uuid && sofia_test_pflag(profile, PFLAG_PROXY_REFER) &&
        sip->sip_payload && sip->sip_payload->pl_data && sip->sip_content_type &&
        sip->sip_content_type->c_type && switch_stristr("sipfrag", sip->sip_content_type->c_type)) {
    ...
    if (sip->sip_content_type && sip->sip_content_type->c_type && sip->sip_payload &&
        sip->sip_payload->pl_data && !strcasecmp(sip->sip_event->o_type, "refer")) {
        if (switch_event_create_subclass(&s_event, SWITCH_EVENT_CUSTOM, MY_EVENT_NOTIFY_REFER) ==
            SWITCH_STATUS_SUCCESS) {
            switch_event_add_header_string(s_event, SWITCH_STACK_BOTTOM, "content-type",
    ...
    if (s_event != NULL) {
        switch_event_add_header_string(s_event, SWITCH_STACK_BOTTOM, "event-package",
                                       sip->sip_event->o_type);
        switch_event_add_header_string(s_event, SWITCH_STACK_BOTTOM, "event-id",
                                       sip->sip_event->o_id);
        ...
        switch_event_add_header_string(s_event, SWITCH_STACK_BOTTOM, "from-tag",
                                       sip->sip_from->a_tag);
        ...
        if (sip->sip_call_id && sip->sip_call_id->i_id) {
            switch_event_add_header_string(s_event, SWITCH_STACK_BOTTOM, "call-id",
                                           sip->sip_call_id->i_id);
        }
        ...
        if (session && channel && tech_pvt) {
            if (sip->sip_payload && sip->sip_payload->pl_data) {
                ...
                if (!status_val || status_val >= 200) {
                    switch_channel_set_variable_printf(channel, "sip_refer_target_status_code",
                                                       "%d", status_val);
                    switch_channel_set_variable(channel, "sip_refer_reply",
                                                sip->sip_payload->pl_data);
                    if (status_val == 200 &&
                        !switch_channel_var_true(channel, "sip_refer_continue_after_reply")) {
                    ...
                    switch_channel_set_variable_printf(
                        channel, "sip_refer_target_provisional_status_code", "%d", status_val);
                }
            }
        }
        nua_respond(nh, SIP_200_OK, NUTAG_WITH_THIS_MSG(de->data->e_msg),
    ...
    if (session) {
        if (!zstr(profile->proxy_notify_events) &&
            (!strcasecmp(profile->proxy_notify_events, "all") ||
            ...
            if (switch_core_session_get_partner(session, &other_session) == SWITCH_STATUS_SUCCESS) {
                ...
                full_to = switch_str_nil(switch_channel_get_variable(
                    switch_core_session_get_channel(other_session), "sip_full_to"));
                if (sip->sip_payload && sip->sip_payload->pl_data) {
                    pl =
                        switch_core_session_strdup(other_session, (char*)sip->sip_payload->pl_data);
                }
                ...
                nua_notify(other_tech_pvt->nh, NUTAG_NEWSUB(1), NUTAG_SUBSTATE(nua_substate_active),
                           ...
                           SIPTAG_EVENT_STR(sip->sip_event->o_type),
    ...
    if (switch_event_create(&s_event, SWITCH_EVENT_NOTIFY_IN) == SWITCH_STATUS_SUCCESS) {
        switch_event_add_header_string(s_event, SWITCH_STACK_BOTTOM, "event",
                                       sip->sip_event->o_type);
        ...
        if (sip->sip_content_type != NULL)
            switch_event_add_header_string(s_event, SWITCH_STACK_BOTTOM, "sip_content_type",
                                           sip->sip_content_type->c_type);
        switch_event_add_header(s_event, SWITCH_STACK_BOTTOM, "port", "%d",
                                gateway->profile->sip_port);
        switch_event_add_header_string(s_event, SWITCH_STACK_BOTTOM, "module_name", "mod_sofia");
        switch_event_add_header_string(s_event, SWITCH_STACK_BOTTOM, "profile_name",
            ...
            if (sip->sip_to && sip->sip_to->a_url->url_user && sip->sip_to->a_url->url_host &&
                ...
                if (acl_ok) {
                    ...
                    if (mwi_status) {
                        ...
                        if (switch_event_create(&s_event, SWITCH_EVENT_MESSAGE_WAITING) ==
                            SWITCH_STATUS_SUCCESS) {
                            switch_event_add_header_string(s_event, SWITCH_STACK_BOTTOM,
                                                           "MWI-Messages-Waiting", mwi_stat);
                            switch_event_add_header(
                                s_event, SWITCH_STACK_BOTTOM, "MWI-Message-Account", "%s@%s",
                                sip->sip_to->a_url->url_user, sip->sip_to->a_url->url_host);
                            switch_event_fire(&s_event);
                        }
 ...
 */
void sofia_handle_sip_r_bye(switch_core_session_t* session, int status, char const* phrase,
                            nua_t* nua, sofia_profile_t* profile, nua_handle_t* nh,
                            sofia_private_t* sofia_private, sip_t const* sip,
                            sofia_dispatch_event_t* de, tagi_t tags[]) {
    if (profile && sofia_test_pflag(profile, PFLAG_FIRE_BYE_RESPONSE_EVENTS) && sip &&
        sip->sip_call_id && !zstr(sip->sip_call_id->i_id) && sofia_private &&
        !zstr_buf(sofia_private->uuid_str)) {
        switch_event_t* bye_response_event = NULL;
        sip_unknown_t* un;
        if (switch_event_create_subclass(&bye_response_event, SWITCH_EVENT_CUSTOM,
                                         MY_EVENT_BYE_RESPONSE) == SWITCH_STATUS_SUCCESS) {
            switch_event_add_header(bye_response_event, SWITCH_STACK_BOTTOM, "call-id", "%s",
                                    sip->sip_call_id->i_id);
            switch_event_add_header(bye_response_event, SWITCH_STACK_BOTTOM, "Unique-ID", "%s",
                                    sofia_private->uuid_str);
            for (un = sip->sip_unknown; un; un = un->un_next) {
                if (!zstr(un->un_value)) {
                    switch_event_add_header(bye_response_event, SWITCH_STACK_BOTTOM, un->un_name,
                                            "%s", un->un_value);
                }
            }
...
void sofia_handle_sip_i_bye(switch_core_session_t* session, int status, char const* phrase,
                            nua_t* nua, sofia_profile_t* profile, nua_handle_t* nh,
                            sofia_private_t* sofia_private, sip_t const* sip,
                            sofia_dispatch_event_t* de, tagi_t tags[]) {
    const char* tmp;
        ...
        if ((bargee_session = switch_core_session_locate(bargee_uuid))) {
            ...
            if (switch_core_media_bug_count(bargee_session, "eavesdrop") == 1) {
                if (switch_core_session_check_interface(bargee_session, sofia_endpoint_interface)) {
                    switch_channel_clear_flag(switch_core_session_get_channel(bargee_session),
                                              CF_SLA_BARGE);
                }
            }
    ...
    if (switch_channel_test_flag(tech_pvt->channel, CF_SLA_BARGE)) {
        ...
        if (!zstr(other_uuid) && (other_session = switch_core_session_locate(other_uuid))) {
            ...
            if (switch_ivr_eavesdrop_pop_eavesdropper(session, &new_session) ==
                SWITCH_STATUS_SUCCESS) {
                ...
                cmd =
                    switch_core_session_sprintf(session, "sleep:500,sofia_sla:%s inline", new_uuid);
    ...
    if (!(vval = switch_channel_get_variable(channel, "sip_copy_custom_headers")) ||
        switch_true(vval)) {
        ...
        if (nsession) {
            ...
            if (!(vval = switch_channel_get_variable(nchannel, "sip_copy_custom_headers")) ||
                switch_true(vval)) {
...
void sofia_handle_sip_r_message(int status, sofia_profile_t* profile, nua_handle_t* nh,
                                sip_t const* sip) {
    const char* call_id;
    int* mstatus;
...
void sofia_update_callee_id(switch_core_session_t* session, sofia_profile_t* profile,
                            sip_t const* sip, switch_bool_t send) {
    switch_channel_t* channel = switch_core_session_get_channel(session);
    ...
    if (switch_channel_inbound_display(channel)) {
        if (!strcmp(caller_profile->caller_id_name, name) &&
            !strcmp(caller_profile->caller_id_number, number)) {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG1,
                              "%s Same Caller ID \"%s\" <%s>\n", switch_channel_get_name(channel),
                              name, number);
            send = 0;
        } else {
            caller_profile->caller_id_name =
                switch_sanitize_number(switch_core_strdup(caller_profile->pool, name));
            caller_profile->caller_id_number =
                switch_sanitize_number(switch_core_strdup(caller_profile->pool, number));
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO,
                              "%s Update Caller ID to \"%s\" <%s>\n",
                              switch_channel_get_name(channel), name, number);
        }
    } else {
        if (!strcmp(caller_profile->callee_id_name, name) &&
            !strcmp(caller_profile->callee_id_number, number)) {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG1,
                              "%s Same Callee ID \"%s\" <%s>\n", switch_channel_get_name(channel),
                              name, number);
            send = 0;
        } else {
            caller_profile->callee_id_name =
                switch_sanitize_number(switch_core_strdup(caller_profile->pool, name));
            caller_profile->callee_id_number =
                switch_sanitize_number(switch_core_strdup(caller_profile->pool, number));
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO,
                              "%s Update Callee ID to \"%s\" <%s>\n",
                              switch_channel_get_name(channel), name, number);
...
static void tech_send_ack(nua_handle_t* nh, private_object_t* tech_pvt, const char* r_sdp) {
    ...
    const char* session_id_header =
        sofia_glue_session_id_header(tech_pvt->session, tech_pvt->profile);

    if (sofia_test_pflag(tech_pvt->profile, PFLAG_TRACK_CALLS)) {
        const char* invite_full_via =
            switch_channel_get_variable(tech_pvt->channel, "sip_invite_full_via");
        ...
        nua_ack(nh, TAG_IF(invite_full_from, SIPTAG_FROM_STR(invite_full_from)),
                ...
                TAG_IF((zstr(tech_pvt->user_via) && !zstr(invite_full_via)),
                       SIPTAG_VIA_STR(invite_full_via)),
...
static void notify_watched_header(switch_core_session_t* session, const char* msgline,
...
static void our_sofia_event_callback(nua_event_t event, int status, char const* phrase, nua_t* nua,
    ...
    /* sofia_private will be == &mod_sofia_globals.keep_private whenever a request is done with a
      new handle that has to be freed whenever the request is done */
    ...
    if (sofia_private && sofia_private != &mod_sofia_globals.destroy_private &&
        ...
        if (!zstr(sofia_private->gateway_name)) {
            ...
            if ((session = de->init_session)) {
            ...
            } else if ((session = de->session) ||
                       (session = switch_core_session_locate(sofia_private->uuid))) {
                tech_pvt = switch_core_session_get_private(session);
                channel = switch_core_session_get_channel(session);
                if (tech_pvt) {
                ...
                /* we can't find the session it must be hanging up or something else, its too late
    ...
    if (sofia_test_pflag(profile, PFLAG_AUTH_ALL) && tech_pvt && tech_pvt->key && sip &&
        ...
        if (authorization && sip->sip_request) {
            ...
            auth_res = sofia_reg_parse_auth(profile, authorization, sip, de,
                                            ...
                                            strlen(tech_pvt->key), network_ip, network_port, NULL,
                                            0, REG_INVITE, NULL, NULL, NULL, NULL);
        }
    ...
    if (sip && (status == 401 || status == 407)) {
        sofia_reg_handle_sip_r_challenge(status, phrase, nua, profile, nh, sofia_private, session,
                                         gateway, sip, de, tags);
        goto done;
    }
    ...
    if (sip && sip->sip_payload && sip->sip_payload->pl_data) {
        if (sip->sip_payload->pl_len != strlen(sip->sip_payload->pl_data)) {
            sip->sip_payload->pl_data = su_strndup(
                nua_handle_get_home(nh), sip->sip_payload->pl_data, sip->sip_payload->pl_len);
        }
    }
    ...
    switch (event) {
        ...
        case nua_i_cancel:
            ...
            if (sip && channel) {
                ...
                if (sip->sip_reason) {
                    char* reason_header =
                        sip_header_as_string(nua_handle_get_home(nh), (void*)sip->sip_reason);
            ...
            if (channel && sip) {
                ...
                if (sip->sip_payload && sip->sip_payload->pl_data) {
                    if (sofia_test_flag(tech_pvt, TFLAG_PASS_ACK)) {
                        ...
                        if (!zstr(tech_pvt->mparams.prev_sdp_str) &&
                            ...
                            switch_channel_set_variable(channel, "sip_reinvite_sdp",
                                                        sip->sip_payload->pl_data);
                            tech_pvt->mparams.last_sdp_str =
                                switch_core_session_strdup(session, sip->sip_payload->pl_data);
                        } else {
                ...
                if (switch_channel_test_flag(tech_pvt->channel, CF_PROXY_MEDIA) && r_sdp) {
                    if (sofia_test_pflag(tech_pvt->profile, PFLAG_3PCC_PROXY)) {
                        switch_channel_set_variable_partner(tech_pvt->channel,
                                                            SWITCH_B_SDP_VARIABLE, r_sdp);
        ...
        case nua_r_message:
            ...
            break;
        case nua_r_invite:
            sofia_handle_sip_r_invite(session, status, phrase, nua, profile, nh, sofia_private, sip,
                                      de, tags);
            break;
        case nua_r_options:
            sofia_handle_sip_r_options(session, status, phrase, nua, profile, nh, sofia_private,
                                       sip, de, tags);
            break;
        case nua_i_bye:
            sofia_handle_sip_i_bye(session, status, phrase, nua, profile, nh, sofia_private, sip,
                                   de, tags);
            break;
        case nua_r_bye:
            sofia_handle_sip_r_bye(session, status, phrase, nua, profile, nh, sofia_private, sip,
                                   de, tags);
            break;
        case nua_r_notify:
            if (session) {
                sofia_handle_sip_r_notify(session, status, phrase, nua, profile, nh, sofia_private,
                                          sip, de, tags);
            }
            break;
        case nua_i_notify:
            sofia_handle_sip_i_notify(session, status, phrase, nua, profile, nh, sofia_private, sip,
                                      de, tags);
            break;
        case nua_r_register:
            sofia_reg_handle_sip_r_register(status, phrase, nua, profile, nh, sofia_private, sip,
                                            de, tags);
            break;
        case nua_i_options:
            sofia_handle_sip_i_options(status, phrase, nua, profile, nh, sofia_private, sip, de,
                                       tags);
            break;
        case nua_i_invite:
            if (session && sofia_private) {
                if (sofia_private->is_call > 1) {
                    sofia_handle_sip_i_reinvite(session, nua, profile, nh, sofia_private, sip, de,
                                                tags);  // 重新INVITE
                } else {
                    sofia_private->is_call++;
                    sofia_handle_sip_i_invite(session, nua, profile, nh, sofia_private, sip, de,
                                              tags);  // 初始INVITE
                }
            }
            break;
        ...
        case nua_i_register:
            ...
            // NUTAG_WITH_THIS_MSG(de->data->e_msg), TAG_END()); nua_handle_destroy(nh);
            sofia_reg_handle_sip_i_register(nua, profile, nh, &sofia_private, sip, de, tags);
            break;
        case nua_i_state:
            sofia_handle_sip_i_state(session, status, phrase, nua, profile, nh, sofia_private, sip,
                                     de, tags);
            break;
        case nua_i_message: {
            ...
            if (proxy_message) {
                if (sofia_proxy_sip_i_message(nua, profile, nh, session, sip, de, tags) ==
                    SWITCH_STATUS_SUCCESS) {
                    handle_message = 0;
                }
            }
            ...
            if (handle_message) {
                sofia_presence_handle_sip_i_message(status, phrase, nua, profile, nh, session,
        ...
        case nua_i_info: {
            ...
            if (proxy_info) {
                if (sofia_proxy_sip_i_info(nua, profile, nh, session, sip, de, tags) ==
                    SWITCH_STATUS_SUCCESS) {
                    handle_info = 0;
                }
            }
        ...
        case nua_i_refer:
            if (session) {
                ...
                const char *req_user = NULL, *req_host = NULL, *action = NULL, *ref_by_user = NULL,
                           *ref_to_user = NULL, *ref_to_host = NULL;
                ...
                if (sip->sip_refer_to) {
                    ...
                    if (sip->sip_refer_to->r_url->url_params &&
                        ...
                        params =
                            su_strdup(nua_handle_home(nh), sip->sip_refer_to->r_url->url_params);
                    }

                    if ((refer_to =
                             sip_header_as_string(nua_handle_home(nh), (void*)sip->sip_refer_to))) {
                ...
                if (switch_event_create(&event, SWITCH_EVENT_CALL_SETUP_REQ) ==
                    SWITCH_STATUS_SUCCESS) {
                    switch_event_add_header_string(event, SWITCH_STACK_BOTTOM,
                                                   "Requesting-Component", "mod_sofia");
                    switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "Target-Component",
                                                   req_user);
                    switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "Target-Domain",
                                                   req_host);
                    switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "Request-Action",
                                                   action);
                    switch_event_add_header(event, SWITCH_STACK_BOTTOM, "Request-Target",
                                            "sofia/%s/%s", profile->name, refer_to);
                    switch_event_add_header(event, SWITCH_STACK_BOTTOM, "Request-Target-URI", "%s",
                                            refer_to);
                    switch_event_add_header_string(event, SWITCH_STACK_BOTTOM,
                                                   "Request-Target-Extension", ref_to_user);
                    switch_event_add_header_string(event, SWITCH_STACK_BOTTOM,
                                                   "Request-Target-Domain", ref_to_host);
                    if (switch_true(full_url)) {
                        switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "full_url",
                                                       "true");
                    }

                    if (sip->sip_call_id && sip->sip_call_id->i_id) {
                        switch_event_add_header_string(event, SWITCH_STACK_BOTTOM,
                                                       "Request-Call-ID", sip->sip_call_id->i_id);
                    }

                    if (!zstr(referred_by)) {
                        switch_event_add_header(event, SWITCH_STACK_BOTTOM, "Request-Sender",
                                                "sofia/%s/%s", profile->name, referred_by);
                    }

                    switch_event_add_header_string(event, SWITCH_STACK_BOTTOM,
                                                   "var_origination_caller_id_number", ref_by_user);
                    switch_event_add_header_string(event, SWITCH_STACK_BOTTOM,
                                                   "var_origination_caller_id_name", ref_by_user);
                    switch_event_fire(&event);
                }

                {
                    ...
                    const char *call_id, *full_from, *full_to, *full_via,
                        *from_user = NULL, *from_host = NULL, *to_user, *to_host, *full_agent;
                    char to_tag[13] = "";
                    ...
                    full_agent =
                        sip_header_as_string(nua_handle_get_home(nh), (void*)sip->sip_user_agent);
                    ...
                    while (ap) {
                        switch_snprintf(accept_header + strlen(accept_header),
                                        sizeof(accept_header) - strlen(accept_header), "%s%s ",
                                        ap->ac_type, ap->ac_next ? "," : "");
                        ap = ap->ac_next;
                    }

                    sql = switch_mprintf(
                        "insert into sip_subscriptions "
                        "(proto,sip_user,sip_host,sub_to_user,sub_to_host,presence_hosts,event,"
                        "contact,call_id,full_from,"
                        "full_via,expires,user_agent,accept,profile_name,hostname,network_port,"
                        "network_ip,version,orig_proto, full_to) "
                        "values "
                        "('%q','%q','%q','%q','%q','%q','%q','%q','%q','%q','%q',%ld,'%q','%q','%q'"
                        ",'%q','%d','%q',-1,'%q','%q;tag=%q')",
                        proto, from_user, from_host, to_user, to_host,
                        profile->presence_hosts ? profile->presence_hosts : "", event_str,
                        contact_str, call_id, full_from, full_via,
                        (long)switch_epoch_time_now(NULL) + 60, full_agent, accept_header,
                        profile->name, mod_sofia_globals.hostname, np.network_port, np.network_ip,
                        orig_proto, full_to, to_tag);

                    switch_assert(sql != NULL);

                    if (mod_sofia_globals.debug_presence > 0 || mod_sofia_globals.debug_sla > 0) {
                        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_CRIT,
                                          "%s REFER SUBSCRIBE %s@%s %s@%s\n%s\n", profile->name,
                                          from_user, from_host, to_user, to_host, sql);
                    }
                ...
                nua_respond(nh, SIP_202_ACCEPTED, SIPTAG_TO(sip->sip_to),
        ...
        case nua_r_subscribe:
            sofia_presence_handle_sip_r_subscribe(status, phrase, nua, profile, nh, sofia_private,
                                                  sip, de, tags);
            break;
        case nua_i_subscribe:
            sofia_presence_handle_sip_i_subscribe(status, phrase, nua, profile, nh, sofia_private,
                                                  sip, de, tags);
            break;
        case nua_i_media_error: {
            if (sofia_private && sofia_private->call_id && sofia_private->network_ip &&
                sofia_private->network_port) {
                char* sql;
                switch_event_t* event = NULL;

                sql = switch_mprintf(
                    "delete from sip_registrations where call_id='%q' and network_ip='%q' and "
                    "network_port='%q'",
                    sofia_private->call_id, sofia_private->network_ip, sofia_private->network_port);
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG1,
                                  "SOCKET DISCONNECT: %s %s:%s\n", sofia_private->call_id,
                                  sofia_private->network_ip, sofia_private->network_port);
                sofia_glue_execute_sql(profile, &sql, SWITCH_TRUE);

                switch_core_del_registration(sofia_private->user, sofia_private->realm,
                                             sofia_private->call_id);
                ...
                if (switch_event_create(&event, SWITCH_EVENT_PRESENCE_IN) ==
                    ...
                    switch_event_add_header_string(
                        event, SWITCH_STACK_BOTTOM, "user-agent",
                        (sip && sip->sip_user_agent) ? sip->sip_user_agent->g_string : "unknown");
                    switch_event_add_header(event, SWITCH_STACK_BOTTOM, "from", "%s@%s",
                                            sofia_private->user, sofia_private->realm);
                    switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "status",
                                                   "Unregistered");
                    switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "presence-source",
                                                   "register");
                    switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "event_type",
                                                   "presence");
                    switch_event_fire(&event);
                }

                if (switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM,
                                                 MY_EVENT_UNREGISTER) == SWITCH_STATUS_SUCCESS) {
                    switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "profile-name",
                                                   profile->name);
                    switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "from-user",
                                                   sofia_private->user);
                    switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "from-host",
                                                   sofia_private->realm);
    ...
    switch (event) {
        ...
        case nua_i_notify:
            ...
            if (sip && sip->sip_event && !strcmp(sip->sip_event->o_type, "dialog") &&
                sip->sip_event->o_params && !strcmp(sip->sip_event->o_params[0], "sla")) {
                check_destroy = 0;
            }
...
void sofia_process_dispatch_event(sofia_dispatch_event_t** dep) {
    ...
    *dep = NULL;
    ...
    our_sofia_event_callback(de->data->e_event, de->data->e_status, de->data->e_phrase, de->nua,
                             de->profile, de->nh, sofia_private, de->sip, de,
                             (tagi_t*)de->data->e_tags);
...
void sofia_queue_message(sofia_dispatch_event_t* de) {
    ...
    if ((switch_queue_size(mod_sofia_globals.msg_queue) >
         (SOFIA_MSG_QUEUE_SIZE * (unsigned int)msg_queue_threads))) {
        launch++;
    }
...
// Sofia Event Callback  sip注册消息到达
void sofia_event_callback(nua_event_t event, int status, char const* phrase, nua_t* nua,
                          sofia_profile_t* profile, nua_handle_t* nh,
                        ...
                        if (status == 403) {
                            ...
                            if (!zstr(tech_pvt->call_id)) {
                                ...
                                switch_core_hash_insert_dup_auto_free(
                                    profile->chat_hash, tech_pvt->call_id,
                                    switch_core_session_get_uuid(session));
                                switch_mutex_unlock(profile->flag_mutex);
                                nua_handle_destroy(nh);
                            } else {
            ...
            if (event == nua_i_invite) {
                if (sip->sip_session_expires && profile->minimum_session_expires) {
                    if (sip->sip_session_expires->x_delta < profile->minimum_session_expires) {
                        char buf[64] = "";
                        switch_snprintf(buf, sizeof(buf), "Min-SE: %d",
                                        profile->minimum_session_expires);
                        nua_respond(nh, SIP_422_SESSION_TIMER_TOO_SMALL, SIPTAG_HEADER_STR(buf),
                                    TAG_END());
                        goto end;
                    }
                }
    ...
    if (event == nua_i_invite && !sofia_private) {
        ...
        if (sofia_test_pflag(profile, PFLAG_CALLID_AS_UUID)) {
            session = switch_core_session_request_uuid(sofia_endpoint_interface,
                                                       SWITCH_CALL_DIRECTION_INBOUND, SOF_NONE,
                                                       NULL, sip->sip_call_id->i_id);
        } else {
...
void event_handler(switch_event_t* event) {
    ...
    if (!strcasecmp(class, "PRESENCE_IN")) {
        ...
        if (switch_event_create(&pevent, SWITCH_EVENT_PRESENCE_IN) == SWITCH_STATUS_SUCCESS) {
            switch_event_add_header_string(
                pevent, SWITCH_STACK_BOTTOM, "alt_event_type",
            ...
            switch_event_add_header_string(
                pevent, SWITCH_STACK_BOTTOM, "register-source",
            ...
            switch_event_add_header_string(
                pevent, SWITCH_STACK_BOTTOM, "variable_sip_from_user",
                switch_event_get_header_nil(event, "Orig-variable_sip_from_user"));
            switch_event_add_header_string(
                pevent, SWITCH_STACK_BOTTOM, "variable_sip_to_user",
        ...
        if (switch_event_create(&pevent, SWITCH_EVENT_MESSAGE_WAITING) == SWITCH_STATUS_SUCCESS) {
            switch_event_add_header_string(
                pevent, SWITCH_STACK_BOTTOM, "MWI-Messages-Waiting",
                switch_event_get_header_nil(event, "orig-MWI-Messages-Waiting"));
            switch_event_add_header_string(
                pevent, SWITCH_STACK_BOTTOM, "MWI-Message-Account",
                switch_event_get_header_nil(event, "orig-MWI-Message-Account"));
            switch_event_add_header_string(
                pevent, SWITCH_STACK_BOTTOM, "MWI-Voice-Message",
        ...
        if (sofia_test_pflag(profile, PFLAG_MULTIREG)) {
            ...
            sql = switch_mprintf(
                "delete from sip_registrations where sip_user='%q' and sip_host='%q'", from_user,
                from_host);
        }

        sofia_glue_execute_sql(profile, &sql, SWITCH_TRUE);
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
                          "Expired propagated registration for %s@%s->%s\n", from_user, from_host,
                          contact_str);
        ...
        if (mod_sofia_globals.rewrite_multicasted_fs_path && contact_str) {
            ...
            char *sptr, *eptr = NULL;
            /* allocate enough room for worst-case scenario, depends on rewrite_multicased_fs_path
             * setting */
            size_t len;
            ...
            switch (mod_sofia_globals.rewrite_multicasted_fs_path) {
                case 1:
                    switch_snprintf(sptr, len - (sptr - fixed_contact_str), ";fs_path=sip:%s%s",
                                    to_host, eptr ? eptr : ">");
                    break;
                case 2:
                    switch_snprintf(sptr, len - (sptr - fixed_contact_str), ";fs_path=sip:%s%s",
                                    orig_server_host, eptr ? eptr : ">");
                    break;
                case 3:
                    switch_snprintf(sptr, len - (sptr - fixed_contact_str), ";fs_path=sip:%s%s",
                                    orig_hostname, eptr ? eptr : ">");
                    break;
                default:
                    switch_snprintf(sptr, len - (sptr - fixed_contact_str), ";fs_path=sip:%s%s",
                                    to_host, eptr ? eptr : ">");
            ...
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
                              "Rewrote contact string from '%s' to '%s'\n", contact_str,
                              fixed_contact_str);
        ...
        sql = switch_mprintf(
            ...
            "user_agent, server_user, server_host, profile_name, hostname, network_ip, "
            "network_port, sip_username, sip_realm,"
        ...
        if (sql) {
            sofia_glue_execute_sql(profile, &sql, SWITCH_TRUE);
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
                              "Propagating registration for %s@%s->%s\n", from_user, from_host,
                              contact_str);
        }
        ...
        if (!profile_name || !(profile = sofia_glue_find_profile(profile_name))) {
            ...
            if (!strcmp(ping_status, "REACHABLE")) {
                sql = switch_mprintf(
                    "update sip_registrations set ping_status='%q' where sip_user='%q' and "
...
static void sofia_perform_profile_start_failure(sofia_profile_t* profile, char* profile_name,
                                                char* file, int line) {
    int arg = 0;
    ...
    if (profile) {
        if (!strcasecmp(profile->shutdown_type, "true")) {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_CRIT,
                              "Profile %s could not load! Shutting down!\n", profile->name);
            ...
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_CRIT,
                              "Profile %s could not load! Shutting down ASAP!\n", profile->name);
    ...
    if ((switch_event_create(&s_event, SWITCH_EVENT_FAILURE) == SWITCH_STATUS_SUCCESS)) {
        ...
        switch_event_add_header_string(s_event, SWITCH_STACK_BOTTOM, "failure_message",
...
#define sofia_profile_start_failure(p, xp) \
    sofia_perform_profile_start_failure(p, xp, __FILE__, __LINE__)

#define SQLLEN 1024 * 1024
void* SWITCH_THREAD_FUNC sofia_profile_worker_thread_run(switch_thread_t* thread, void* obj) {
    sofia_profile_t* profile = (sofia_profile_t*)obj;
    uint32_t ireg_loops = profile->ireg_seconds; /* Number of loop iterations done when we haven't
                                                    checked for registrations */
    uint32_t iping_loops = profile->iping_freq;  /* Number of loop iterations done when we haven't
                                                    checked for ping expires */
    uint32_t gateway_loops =
        GATEWAY_SECONDS; /* Number of loop iterations done when we haven't checked for gateways */
    void* pop;
    int tick = 0, x = 0;
    ...
    while ((mod_sofia_globals.running == 1 && sofia_test_pflag(profile, PFLAG_RUNNING))) {
        ...
        if (switch_queue_pop_timeout(mod_sofia_globals.general_event_queue, &pop, 100000) ==
            SWITCH_STATUS_SUCCESS) {
            do {
...
switch_thread_t* launch_sofia_worker_thread(sofia_profile_t* profile) {
    ...
    switch_threadattr_t* thd_attr = NULL;
    int x = 0;
    switch_xml_t cfg = NULL, xml = NULL, xprofile = NULL, xprofiles = NULL, gateways_tag = NULL,
                 domains_tag = NULL, domain_tag = NULL;
                ...
                for (domain_tag = switch_xml_child(domains_tag, "domain"); domain_tag;
                    ...
                    if (!zstr(dname)) {
                        if (!strcasecmp(dname, "all")) {
                            ...
                            if (switch_xml_locate("directory", NULL, NULL, NULL, &xml_root,
                                                  &x_domains, xml_params,
                                                  SWITCH_FALSE) == SWITCH_STATUS_SUCCESS) {
                                for (x_domain_tag = switch_xml_child(x_domains, "domain");
                                     x_domain_tag; x_domain_tag = x_domain_tag->next) {
                                    dname = switch_xml_attr_soft(x_domain_tag, "name");
                                    parse_domain_tag(profile, x_domain_tag, dname, parse, alias);
                                }
                                switch_xml_free(xml_root);
                            }
                        } else if (switch_xml_locate_domain(dname, xml_params, &droot,
                                                            &x_domain_tag) ==
                                   SWITCH_STATUS_SUCCESS) {
                            parse_domain_tag(profile, x_domain_tag, dname, parse, alias);
    ...
    switch_threadattr_stacksize_set(thd_attr, SWITCH_THREAD_STACKSIZE);
    // switch_threadattr_priority_set(thd_attr, SWITCH_PRI_REALTIME);
    switch_thread_create(&thread, thd_attr, sofia_profile_worker_thread_run, profile,
                         profile->pool);
...
void* SWITCH_THREAD_FUNC sofia_profile_thread_run(switch_thread_t* thread, void* obj) {
    ...
    supported =
        switch_core_sprintf(profile->pool, "%s%s%spath, replaces", use_100rel ? "100rel, " : "",
    ...
    if (sofia_test_pflag(profile, PFLAG_AUTO_NAT) && switch_nat_get_type()) {
        if ((!sofia_test_pflag(profile, PFLAG_TLS) || !profile->tls_only) &&
            switch_nat_add_mapping(profile->sip_port, SWITCH_NAT_UDP, NULL, SWITCH_FALSE) ==
                SWITCH_STATUS_SUCCESS) {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
                              "Created UDP nat mapping for %s port %d\n", profile->name,
                              profile->sip_port);
        }
        if (switch_nat_add_mapping(profile->sip_port, SWITCH_NAT_TCP, NULL, SWITCH_FALSE) ==
            SWITCH_STATUS_SUCCESS) {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
        ...
        if (sofia_test_pflag(profile, PFLAG_TLS) &&
            switch_nat_add_mapping(profile->tls_sip_port, SWITCH_NAT_TCP, NULL, SWITCH_FALSE) ==
                SWITCH_STATUS_SUCCESS) {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
    ...
    /* We have to init the verify_subjects here as during config stage profile->home isn't setup, it
     * should be freed when profile->home is freed */
    if ((profile->tls_verify_policy & TPTLS_VERIFY_SUBJECTS_IN) &&
        profile->tls_verify_in_subjects_str && !profile->tls_verify_in_subjects) {
    ...
    do {
        profile->nua = nua_create(
            ...
            TAG_IF(!sofia_test_pflag(profile, PFLAG_TLS) || !profile->tls_only,
            ...
            TAG_IF(profile->tls_orq_connect_timeout,
                   NTATAG_TLS_ORQ_CONNECT_TIMEOUT(
                       profile->tls_orq_connect_timeout)), /* Profile based timeout */
#endif
            ...
            TPTAG_CAPT(sofia_test_flag(profile, TFLAG_CAPTURE) ? mod_sofia_globals.capture_server
                                                               : NULL),
    ...
    nua_set_params(
        profile->nua, SIPTAG_ALLOW_STR("INVITE, ACK, BYE, CANCEL, OPTIONS, MESSAGE, INFO"),
        SIPTAG_USER_AGENT(SIP_NONE), NUTAG_AUTOANSWER(0), NUTAG_AUTOACK(0), NUTAG_AUTOALERT(0),
        NUTAG_ENABLEMESSENGER(1), NTATAG_EXTRA_100(0),
        ...
        NUTAG_ALLOW_EVENTS("hold"), NUTAG_ALLOW_EVENTS("conference"), NUTAG_APPL_METHOD("OPTIONS"),
        NUTAG_APPL_METHOD("INVITE"), NUTAG_APPL_METHOD("REFER"), NUTAG_APPL_METHOD("REGISTER"),
        NUTAG_APPL_METHOD("NOTIFY"), NUTAG_APPL_METHOD("INFO"), NUTAG_APPL_METHOD("ACK"),
    ...
    if (sofia_test_pflag(profile, PFLAG_AUTO_ASSIGN_PORT) ||
        ...
        for (via = vias; via; via = via->v_next) {
            ...
            if (sofia_test_pflag(profile, PFLAG_AUTO_ASSIGN_TLS_PORT) &&
                !strcmp(via->v_protocol, "SIP/2.0/TLS")) {
    ...
    for (node = profile->aliases; node; node = node->next) {
        node->nua =
            nua_create(profile->s_root,      /* Event loop */
                       sofia_event_callback, /* Callback for processing events */
                       profile,              /* Additional data to pass to callback */
                       NTATAG_SERVER_RPORT(profile->server_rport_level), NUTAG_URL(node->url),
                       TAG_END()); /* Last tag should always finish the sequence */

        nua_set_params(node->nua, SIPTAG_USER_AGENT(SIP_NONE), NUTAG_APPL_METHOD("OPTIONS"),
                       NUTAG_APPL_METHOD("REFER"), NUTAG_APPL_METHOD("SUBSCRIBE"),
                       NUTAG_AUTOANSWER(0), NUTAG_AUTOACK(0), NUTAG_AUTOALERT(0),
                       TAG_IF((profile->mflags & MFLAG_REGISTER), NUTAG_ALLOW("REGISTER")),
                       TAG_IF((profile->mflags & MFLAG_REFER), NUTAG_ALLOW("REFER")),
                       NUTAG_ALLOW("INFO"), TAG_IF(profile->pres_type, NUTAG_ALLOW("PUBLISH")),
                       TAG_IF(profile->pres_type, NUTAG_ENABLEMESSAGE(1)),
    ...
    switch_sql_queue_manager_init_name(
        qname, &profile->qm, 2, profile->odbc_dsn ? profile->odbc_dsn : profile->dbname,
    ...
    if (switch_event_create(&s_event, SWITCH_EVENT_PUBLISH) == SWITCH_STATUS_SUCCESS) {
        switch_event_add_header(s_event, SWITCH_STACK_BOTTOM, "service",
                                "_sip._udp,_sip._tcp,_sip._sctp%s",
        ...
        if (sofia_test_pflag(profile, PFLAG_TLS)) {
            switch_event_add_header(s_event, SWITCH_STACK_BOTTOM, "tls_port", "%d",
                                    profile->tls_sip_port);
            switch_event_add_header_string(s_event, SWITCH_STACK_BOTTOM, "profile_tls_uri",
                                           profile->tls_url);
        }
        switch_event_fire(&s_event);
    }
    ...
    switch_core_session_hupall_matching_var("sofia_profile_name", profile->name,
    ...
    sofia_sub_check_gateway(profile, switch_epoch_time_now(NULL));
    /*
     * The gateway life cycle requires a gateway to go though different states before it's
     * destroyed. Normally sofia_reg_check_gateway() is called periodically but it's not the case on
     * profile shutdown.
     *
...
nua_fail:
    if (sofia_test_pflag(profile, PFLAG_AUTO_NAT) && switch_nat_get_type()) {
        ...
        if (sofia_test_pflag(profile, PFLAG_TLS) &&
            switch_nat_del_mapping(profile->tls_sip_port, SWITCH_NAT_TCP) ==
                SWITCH_STATUS_SUCCESS) {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
...
void launch_sofia_profile_thread(sofia_profile_t* profile) {
    ...
    switch_threadattr_stacksize_set(thd_attr, SWITCH_THREAD_STACKSIZE);
    switch_threadattr_priority_set(thd_attr, SWITCH_PRI_REALTIME);
    switch_thread_create(&profile->thread, thd_attr, sofia_profile_thread_run, profile,
                         profile->pool);
}
...
static void parse_gateway_subscriptions(sofia_profile_t* profile, sofia_gateway_t* gateway,
                                        switch_xml_t gw_subs_tag) {
...
static void parse_gateways(sofia_profile_t* profile, switch_xml_t gateways_tag,
    ...
    for (gateway_tag = switch_xml_child(gateways_tag, "gateway"); gateway_tag;
        ...
        if ((gateway = switch_core_alloc(profile->pool, sizeof(*gateway)))) {
            ...
            switch_uuid_t uuid;
            uint32_t ping_freq = 0, extension_in_contact = 0, contact_in_ping = 0,
                     ping_monitoring = 0, distinct_to = 0, rfc_5626 = 0;
            int ping_max = 1, ping_min = 1;
            char *register_str = "true", *scheme = "Digest", *realm = NULL, *username = NULL,
                 *auth_username = NULL, *password = NULL, *caller_id_in_from = "false",
                 *extension = NULL, *proxy = NULL, *options_user_agent = NULL,
                 *context = profile->context, *expire_seconds = "3600", *retry_seconds = "30",
                 *fail_908_retry_seconds = NULL, *timeout_seconds = "60", *from_user = "",
                 *from_domain = NULL, *outbound_proxy = NULL, *register_proxy = NULL,
                 *contact_host = NULL, *contact_params = "", *params = NULL,
                 *register_transport = NULL, *reg_id = NULL, *str_rfc_5626 = "";
            ...
            for (; param; param = param->next) {
                ...
                if (var && val) {
                    ...
                    if (in) {
                        ...
                        switch_event_add_header_string(gateway->ib_vars, SWITCH_STACK_BOTTOM, var,
                                                       val);
                    }
                    ...
                    if (out) {
                        ...
                        switch_event_add_header_string(gateway->ob_vars, SWITCH_STACK_BOTTOM, var,
                                                       val);
                    }
                }
            }
            ...
            for (; param; param = param->next) {
                ...
                if (!strcmp(var, "register")) {
                ...
                } else if (!strcmp(var, "ping-monitoring")) {  // if true then every gw ping result
                                                               // will fire a gateway status event
                    ...
                    register_proxy = val;
                    gateway->register_proxy_host_cfg =
                        sofia_glue_get_host_from_cfg(register_proxy, gateway->pool);
                } else if (!strcmp(var, "outbound-proxy")) {
                    outbound_proxy = val;
                    gateway->outbound_proxy_host_cfg =
                        sofia_glue_get_host_from_cfg(outbound_proxy, gateway->pool);
                    ...
                    if (transport == SOFIA_TRANSPORT_UNKNOWN ||
                        (!sofia_test_pflag(profile, PFLAG_TLS) &&
            ...
            if (rfc_5626 && sofia_test_pflag(profile, PFLAG_ENABLE_RFC5626)) {
                ...
                str_rfc_5626 = switch_core_sprintf(
                    gateway->pool, ";reg-id=%s;+sip.instance=\"<urn:uuid:%s>\"", reg_id, str_guid);
            }
            ...
            if (!zstr(register_proxy)) {
                if (strncasecmp(register_proxy, "sip:", 4) &&
                    strncasecmp(register_proxy, "sips:", 5)) {
                    gateway->register_sticky_proxy =
                        switch_core_sprintf(gateway->pool, "sip:%s", register_proxy);
                } else {
                    gateway->register_sticky_proxy =
                        switch_core_strdup(gateway->pool, register_proxy);
                }
            }

            if (!zstr(outbound_proxy)) {
                if (strncasecmp(outbound_proxy, "sip:", 4) &&
                    strncasecmp(outbound_proxy, "sips:", 5)) {
                    gateway->outbound_sticky_proxy =
                        switch_core_sprintf(gateway->pool, "sip:%s", outbound_proxy);
                } else {
                    gateway->outbound_sticky_proxy =
                        switch_core_strdup(gateway->pool, outbound_proxy);
                }
            }
            ...
            if (!zstr(contact_params)) {
                if (*contact_params == ';') {
                    params = switch_core_sprintf(gateway->pool, "%s;transport=%s;gw=%s",
                                                 contact_params, register_transport, gateway->name);
                } else {
                    params = switch_core_sprintf(gateway->pool, ";%s;transport=%s;gw=%s",
                                                 contact_params, register_transport, gateway->name);
                }
            } else {
                params = switch_core_sprintf(gateway->pool, ";transport=%s;gw=%s",
            ...
            if (!zstr(register_transport) && !switch_stristr("transport=", proxy)) {
                gateway->register_url = switch_core_sprintf(gateway->pool, "sip:%s;transport=%s",
                                                            proxy, register_transport);
            } else {
            ...
            if (ping_freq) {
                if (ping_freq >= 5) {
                    ...
                    gateway->options_to_uri = switch_core_sprintf(
                        gateway->pool, "<sip:%s>", !zstr(from_domain) ? from_domain : proxy);
            ...
            if (!strncasecmp(proxy, "sip:", 4)) {
                ...
                gateway->register_to =
                    switch_core_sprintf(gateway->pool, "sip:%s@%s", username, proxy);
            }

            /* This checks to make sure we provide the right contact on register for targets behind
             * nat with us. */
            ...
            if (extension_in_contact) {
                ...
                if (rfc_5626) {
                    format =
                        strchr(sipip, ':') ? "<sip:gw+%s@[%s]:%d%s>%s" : "<sip:gw+%s@%s:%d%s>%s";
                    gateway->register_contact = switch_core_sprintf(
            ...
            if ((gateway->freq = atoi(gateway->expires_str)) < 5) {
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
                                  "Invalid register-frequency of %d on gateway %s, using the value "
                                  "of 3600 instead\n",
                                  gateway->freq, name);
                gateway->freq = 3600;
            }
...
static void parse_domain_tag(sofia_profile_t* profile, switch_xml_t x_domain_tag, const char* dname,
    ...
    if (switch_true(alias)) {
        if (sofia_glue_add_profile(switch_core_strdup(profile->pool, dname), profile) ==
            SWITCH_STATUS_SUCCESS) {
...
static void config_sofia_profile_urls(sofia_profile_t* profile) {
    if (profile->extsipip) {
        ...
        profile->public_url = switch_core_sprintf(
            profile->pool, "sip:%s@%s%s%s:%d", profile->contact_user, ipv6 ? "[" : "",
            profile->extsipip, ipv6 ? "]" : "", profile->extsipport);
    }

    if (profile->extsipip && !sofia_test_pflag(profile, PFLAG_AUTO_NAT)) {
        char* ipv6 = strchr(profile->extsipip, ':');
        profile->url = switch_core_sprintf(profile->pool, "sip:%s@%s%s%s:%d", profile->contact_user,
                                           ipv6 ? "[" : "", profile->extsipip, ipv6 ? "]" : "",
                                           profile->extsipport);
        profile->bindurl =
            switch_core_sprintf(profile->pool, "%s;maddr=%s", profile->url, profile->sipip);
    } else {
        char* ipv6 = strchr(profile->sipip, ':');
        profile->url = switch_core_sprintf(profile->pool, "sip:%s@%s%s%s:%d", profile->contact_user,
                                           ipv6 ? "[" : "", profile->sipip, ipv6 ? "]" : "",
                                           profile->sip_port);
    ...
    if (profile->public_url) {
        profile->tcp_public_contact =
            switch_core_sprintf(profile->pool, "<%s;transport=tcp>", profile->public_url);
    }
    ...
    if (profile->bind_params) {
        ...
        if (!switch_stristr("transport=", profile->bind_params)) {
            profile->bind_params =
                switch_core_sprintf(profile->pool, "%s;transport=udp,tcp", profile->bind_params);
        }
        bindurl =
            switch_core_sprintf(profile->pool, "%s;%s", profile->bindurl, profile->bind_params);
        profile->bindurl = bindurl;
    } else {
        char* bindurl =
            switch_core_sprintf(profile->pool, "%s;transport=udp,tcp", profile->bindurl);
        profile->bindurl = bindurl;
    }
    ...
    if (profile->ws_port) {
        ...
        char* ipv6 = strchr(ip, ':');
        profile->ws_bindurl =
            switch_core_sprintf(profile->pool, "sip:%s@%s%s%s:%d;transport=ws",
    ...
    if (profile->wss_port) {
        ...
        char* ipv6 = strchr(ip, ':');
        profile->wss_bindurl =
            switch_core_sprintf(profile->pool, "sips:%s@%s%s%s:%d;transport=wss",
                                profile->contact_user, ipv6 ? "[" : "", ip, ipv6 ? "]" : "", port);
    }
    ...
    if (sofia_test_pflag(profile, PFLAG_TLS)) {
        ...
        if (profile->extsipip) {
            char* ipv6 = strchr(profile->extsipip, ':');
            profile->tls_public_url = switch_core_sprintf(
                profile->pool, "sip:%s@%s%s%s:%d", profile->contact_user, ipv6 ? "[" : "",
        ...
        if (profile->extsipip && !sofia_test_pflag(profile, PFLAG_AUTO_NAT)) {
            char* ipv6 = strchr(profile->extsipip, ':');
            profile->tls_url = switch_core_sprintf(
                profile->pool, "sip:%s@%s%s%s:%d", profile->contact_user, ipv6 ? "[" : "",
                profile->extsipip, ipv6 ? "]" : "", profile->tls_sip_port);
            profile->tls_bindurl = switch_core_sprintf(
                profile->pool, "sips:%s@%s%s%s:%d;maddr=%s", profile->contact_user, ipv6 ? "[" : "",
            ...
            profile->tls_bindurl = switch_core_sprintf(
                profile->pool, "sips:%s@%s%s%s:%d", profile->contact_user, ipv6 ? "[" : "",
                profile->sipip, ipv6 ? "]" : "", profile->tls_sip_port);
        }
        ...
        if (profile->tls_bind_params) {
            ...
            profile->tls_bindurl =
                switch_core_sprintf(profile->pool, "%s;%s", tls_bindurl, profile->tls_bind_params);
        }

        profile->tls_contact =
            switch_core_sprintf(profile->pool, "<%s;transport=tls>", profile->tls_url);
        ...
        if ((xprofile = switch_xml_find_child(xprofiles, "profile", "name", profile->name))) {
            if ((gateways_tag = switch_xml_child(xprofile, "gateways"))) {
                parse_gateways(profile, gateways_tag,
                               strcmp(gateway_name, "_all_") ? gateway_name : NULL);
            }
        }
    }
...
switch_status_t config_sofia(sofia_config_t reload, char* profile_name) {
            ...
            if (!strcasecmp(var, "log-level")) {
                ...
                if (switch_true(val)) {
                    switch_log_printf(
                        ...
                        "deprecated and will be removed - let FS devs know if you think it should "
                        "stay\n");
                }
                ...
                mod_sofia_globals.stir_shaken_as_key =
                    switch_core_strdup(mod_sofia_globals.pool, val);
            } else if (!strcasecmp(var, "stir-shaken-as-url")) {
                /* The x5u URL to advertise when sip_identity_attest is set */
                mod_sofia_globals.stir_shaken_as_url =
                    switch_core_strdup(mod_sofia_globals.pool, val);
    ...
    if ((profiles = switch_xml_child(cfg, "profiles"))) {
        for (xprofile = switch_xml_child(profiles, "profile"); xprofile;
             xprofile = xprofile->next) {
            ...
            if (!(settings = switch_xml_child(xprofile, "settings"))) {
                ...
                if (!profile_already_started) {
                    ...
                    profile->user_agent = switch_core_sprintf(
                        profile->pool, "FreeSWITCH-mod_sofia/%s", switch_version_full());
                    ...
                    /* you could change profile->foo here if it was a minor change like context or
                     * dialplan ... */
                    profile->acl_count = 0;
                ...
                for (param = switch_xml_child(settings, "param"); param; param = param->next) {
                    char* var = (char*)switch_xml_attr_soft(param, "name");
                    char* val = (char*)switch_xml_attr_soft(param, "value");
                    int found = 1;  // Used to break up long if/elseif chain (MSVC2015 fails (parser
                    ...
                    if (!strcasecmp(var, "debug") && val) {
                        ...
                        if (switch_true(val)) {
                            ...
                            nua_set_params(profile->nua,
                                           TPTAG_CAPT(mod_sofia_globals.capture_server), TAG_END());
                        } else {
                    ...
                    } else if ((!strcasecmp(var, "proxy-refer-replaces") ||
                                !strcasecmp(var, "proxy-refer")) &&
                               !zstr(val)) {
                        if (switch_true(val)) {
                    ...
                    } else if (!strcasecmp(var, "inbound-no-media") ||
                               !strcasecmp(var, "inbound-bypass-media")) {
                        if (switch_true(val)) {
                        ...
                        profile->te = (switch_payload_t)atoi(val);
                               ...
                               !sofia_test_media_flag(profile, SCMF_SUPPRESS_CNG) && !zstr(val)) {
                            ...
                            if (ip) {
                                if (!strncasecmp(ip, "autonat:", 8)) {
                                    ...
                                    if (zstr(profile->extsipip)) {
                                        profile->extsipip =
                                            switch_core_strdup(profile->pool, profile->extrtpip);
                                    }
                        ...
                        if (strchr(ip, ':')) {
                            if (profile->rtpip_index6 < MAX_RTPIP) {
                                profile->rtpip6[profile->rtpip_index6++] =
                                    switch_core_strdup(profile->pool, ip);
                            } else {
                                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
                        ...
                        if (!zstr(val)) {
                            ...
                            if (!strcasecmp(val, "0.0.0.0")) {
                                ...
                                if (sofia_glue_ext_address_lookup(
                                        profile, &myip, &profile->extsipport, val, profile->pool) ==
                                    SWITCH_STATUS_SUCCESS) {
                                    ip = myip;
                                    sofia_clear_pflag(profile, PFLAG_AUTO_NAT);
                                } else {
                        ...
                        if (strncasecmp(val, "sip:", 4) && strncasecmp(val, "sips:", 5)) {
                            profile->outbound_proxy =
                                switch_core_sprintf(profile->pool, "sip:%s", val);
                        } else {
                        ...
                        if (v >= 0) {
                            ...
                            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
                                              "rtp-hold-timeout-sec deprecated use "
                                              "media_hold_timeout variable.\n");
                        }
                        ...
                        if (!strcasecmp(val, "resume-media-on-hold")) {
                            ...
                            if (profile->media_options & MEDIA_OPT_MEDIA_ON_HOLD) {
                                ...
                                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
                                                  "bypass-media-after-hold can be set only with "
                                                  "resume-media-on-hold media-option\n");
                            }
                        } else if (!strcasecmp(val, "none")) {
                        ...
                        if (switch_true(val)) {
                            ...
                            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
                                              ...
                                              "It was incomplete anyway, and we fully support the "
                                              "broadsoft SCA shared line spec.");
                    ...
                    if (!strcasecmp(var, "multiple-registrations")) {
                        ...
                        if (!strcasecmp(val, "none")) {
                            ...
                            if (!profile->extsipip && profile->sipip &&
                                ...
                                profile->nat_acl[profile->nat_acl_count++] =
                        ...
                        if (!strcasecmp(val, "none")) {
                            ...
                            profile->proxy_acl[profile->proxy_acl_count++] =
                                switch_core_strdup(profile->pool, val);
                        } else {
                            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
                                              "Max acl records of %d reached\n", SOFIA_MAX_ACL);
                        ...
                        if (!strcasecmp(val, "none")) {
                            ...
                            profile->reg_acl[profile->reg_acl_count++] =
                                switch_core_strdup(profile->pool, val);
                        } else {
                            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
                                              "Max acl records of %d reached\n", SOFIA_MAX_ACL);
                        ...
                        if (!strcasecmp(val, "none")) {
                            ...
                            profile->cand_acl[profile->cand_acl_count++] =
                                switch_core_strdup(profile->pool, val);
                        } else {
                            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
                                              "Max acl records of %d reached\n",
                        ...
                        if (dur >= switch_core_min_dtmf_duration(0) &&
                            dur <= switch_core_max_dtmf_duration(0)) {
                            ...
                            if (profile->tls_bind_params) {
                                if (!switch_stristr("transport=tls", profile->tls_bind_params)) {
                                    profile->tls_bind_params =
                                        switch_core_sprintf(profile->pool, "%s;transport=tls",
                                                            profile->tls_bind_params);
                                }
                            } else {
                                profile->tls_bind_params =
                                    switch_core_strdup(profile->pool, "transport=tls");
                            }
                        } else {
                        ...
                        if (val && switch_stristr("transport=tls", val)) {
                            ...
                            profile->tls_bind_params =
                                switch_core_sprintf(profile->pool, "%s;transport=tls", val);
                        }
                        ...
                        profile->tls_verify_in_subjects_str =
                            switch_core_strdup(profile->pool, val);
                        ...
                        while (ps && *pe) {
                            ...
                            if (n == 6 && !strncasecmp(ps, "sslv23", n))
                                profile->tls_version |=
                                    SOFIA_TLS_VERSION_SSLv2 | SOFIA_TLS_VERSION_SSLv3;
                        ...
                        if (sip_subscription_max_deviation >= 0) {
                            profile->sip_subscription_max_deviation =
                        ...
                        profile->acl_inbound_x_token_header =
                            switch_core_strdup(profile->pool, val);
                        ...
                        char* algs_arr[100] = {0};
                        uint8_t algs = switch_separate_string(
                            val, ',', algs_arr, (sizeof(algs_arr) / sizeof(algs_arr[0])));
                        if (algs && algs < SOFIA_MAX_REG_ALGS) {
                            ...
                            for (i = 0; i < profile->rfc8760_algs_count; i++) {
                                for (j = i + 1; j < profile->rfc8760_algs_count; j++) {
                                    /* when adding algs: algs must be kept in priority order in the
                                     * enum */
                ...
                if (sofia_test_flag(profile, TFLAG_INB_NOMEDIA) &&
                ...
                if (sofia_test_pflag(profile, PFLAG_SEND_DISPLAY_UPDATE) &&
                    ...
                    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
                                      "send-display-update=true is set, but we can't comply "
                                      "because allow-update=false\n");
                    sofia_clear_pflag(profile, PFLAG_SEND_DISPLAY_UPDATE);
                }
                if (sofia_test_pflag(profile, PFLAG_PROXY_HOLD)) {
                    if (profile->media_options & MEDIA_OPT_MEDIA_ON_HOLD) {
                        profile->media_options &= ~MEDIA_OPT_MEDIA_ON_HOLD;
                        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
                                          "proxy-hold=true is set, incompatible with "
                                          "media-option=resume-media-on-hold\n");
                    }
                    if (profile->media_options & MEDIA_OPT_BYPASS_AFTER_HOLD) {
                        profile->media_options &= ~MEDIA_OPT_BYPASS_AFTER_HOLD;
                        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
                                          "proxy-hold=true is set, incompatible with "
                                          "media-option=bypass-media-after-hold\n");
                    }
                }
                ...
                if (profile->nonce_ttl < 60) {
                    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO,
                                      "Setting nonce TTL to 60 seconds\n");
                ...
                if (!profile->rtpip[0] && !profile->rtpip6[0]) {
                    ...
                    profile->rtpip[profile->rtpip_index++] =
                        switch_core_strdup(profile->pool, "127.0.0.1");
                }
                ...
                if (profile->pres_type == PRES_TYPE_PNP) {
                    if (!profile->pnp_prov_url) {
                        profile->pnp_prov_url = switch_core_sprintf(
                            profile->pool, "http://%s/provision/", mod_sofia_globals.guess_ip);
                    }
                ...
                if (profile->tls_cert_dir) {
                    if (profile->wss_ip) {
                        char *key, *cert;
                        key = switch_core_sprintf(profile->pool, "%s/%s", profile->tls_cert_dir,
                                                  "wss.key");
                        if (switch_file_exists(key, profile->pool) != SWITCH_STATUS_SUCCESS)
                            key = NULL;
                        cert = switch_core_sprintf(profile->pool, "%s/%s", profile->tls_cert_dir,
                                                   "wss.crt");
                        if (switch_file_exists(cert, profile->pool) != SWITCH_STATUS_SUCCESS)
                            cert = NULL;
                        if (!key || !cert) {
                            key = switch_core_sprintf(profile->pool, "%s/%s", profile->tls_cert_dir,
                                                      "wss.pem");
                    ...
                    if (sofia_test_pflag(profile, PFLAG_TLS)) {
                        char* key = switch_core_sprintf(profile->pool, "%s/%s",
                                                        profile->tls_cert_dir, "agent.pem");
                        char* ca = switch_core_sprintf(profile->pool, "%s/%s",
                                                       profile->tls_cert_dir, "cafile.pem");
                        ;
                        if (switch_file_exists(key, profile->pool) != SWITCH_STATUS_SUCCESS)
                            key = NULL;
                        if (switch_file_exists(ca, profile->pool) != SWITCH_STATUS_SUCCESS)
                            ca = NULL;
                        if (!key || !ca) {
                            key = switch_core_sprintf(profile->pool, "%s/%s", profile->tls_cert_dir,
                                                      "tls.pem");
                ...
                if (profile_already_started) {
                                ...
                                if (!strcasecmp(dname, "all")) {
                                    ...
                                    if (switch_xml_locate("directory", NULL, NULL, NULL, &xml_root,
                                        ...
                                        for (x_domain_tag = switch_xml_child(x_domains, "domain");
                                             x_domain_tag; x_domain_tag = x_domain_tag->next) {
                                            dname = switch_xml_attr_soft(x_domain_tag, "name");
                                            parse_domain_tag(profile, x_domain_tag, dname, parse,
                                                             alias);
                                        }
                                        switch_xml_free(xml_root);
                                    }
                    ...
                    if ((aliases_tag = switch_xml_child(xprofile, "aliases"))) {
                        for (alias_tag = switch_xml_child(aliases_tag, "alias"); alias_tag;
                            ...
                            if (!zstr(aname)) {
                                if (sofia_glue_add_profile(switch_core_strdup(profile->pool, aname),
                    ...
                    if (profile->sipip) {
                        ...
                        if ((switch_event_create_subclass(&s_event, SWITCH_EVENT_CUSTOM,
...
int sofia_sip_user_status_callback(void* pArg, int argc, char** argv, char** columnNames) {
    ...
    if (argc != 3) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
                          "expected 3 arguments from query, instead got %d\n", argc);
        return 0;
    }
...
static void sofia_handle_sip_r_options(switch_core_session_t* session, int status,
                                       char const* phrase, nua_t* nua, sofia_profile_t* profile,
                                       nua_handle_t* nh, sofia_private_t* sofia_private,
                                       sip_t const* sip, sofia_dispatch_event_t* de,
                                       tagi_t tags[]) {
    sofia_gateway_t* gateway = NULL;
    ...
    if (gateway) {
        if (status >= 200 && status < 600 && status != 408 && status != 503) {
            ...
            if (gateway->ping_count < gateway->ping_max) {
                ...
                switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING,
                                  ...
                                  gateway->name, status, gateway->ping_min, gateway->ping_count,
                                  gateway->ping_max, sofia_gateway_status_name(gateway->status));
            }
        ...
        char* sip_user =
            switch_mprintf("%s@%s", sip->sip_to->a_url->url_user, sip->sip_to->a_url->url_host);
        int ping_time = 0;
        ...
        sql = switch_mprintf(
            "select ping_status, ping_count, contact from sip_registrations where sip_user='%q' "
            "and sip_host='%q' and call_id='%q'",
            sip->sip_to->a_url->url_user, sip->sip_to->a_url->url_host, call_id);
        sofia_glue_execute_sql_callback(profile, profile->ireg_mutex, sql,
                                        sofia_sip_user_status_callback, &sip_user_status);
        ...
        if (status != 200 && status != 486) {
            ...
            if (sip_user_status.count >= 0) {
                ...
                sql = switch_mprintf(
                    "update sip_registrations set ping_count=%d, ping_time=%d where sip_user='%q' "
                    "and sip_host='%q' and call_id='%q'",
            ...
            if (sip_user_status.count < sip_user_ping_min) {
                if (strcmp(sip_user_status.status, "Unreachable")) {
                    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING,
                                      ...
                                      sip->sip_to->a_url->url_user, sip->sip_to->a_url->url_host);
                    sql = switch_mprintf(
                        "update sip_registrations set ping_status='Unreachable', ping_time=%d "
                        "where sip_user='%q' and sip_host='%q' and call_id='%q'",
                        ping_time, sip->sip_to->a_url->url_user, sip->sip_to->a_url->url_host,
                        call_id);
                    sofia_glue_execute_sql(profile, &sql, SWITCH_TRUE);
                    switch_safe_free(sql);
                    ...
                    if (sofia_test_pflag(profile, PFLAG_UNREG_OPTIONS_FAIL)) {
                        ...
                        sql = switch_mprintf(
                            "update sip_registrations set expires=%ld, ping_time=%d where "
            ...
            if (sip_user_status.count >= sip_user_ping_min) {
                if (strcmp(sip_user_status.status, "Reachable")) {
                    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING,
                                      ...
                                      sip->sip_to->a_url->url_user, sip->sip_to->a_url->url_host);
                    sql = switch_mprintf(
                        "update sip_registrations set ping_status='Reachable' where sip_user='%q' "
                        "and sip_host='%q' and call_id='%q'",
                        sip->sip_to->a_url->url_user, sip->sip_to->a_url->url_host, call_id);
                    sofia_glue_execute_sql(profile, &sql, SWITCH_TRUE);
                    switch_safe_free(sql);
                    sofia_reg_fire_custom_sip_user_state_event(
                        profile, sip_user, sip_user_status.contact, sip->sip_to->a_url->url_user,
                        sip->sip_to->a_url->url_host, call_id, SOFIA_REG_UNREACHABLE, status,
                        phrase);
                }
            }
        }
...
static void sofia_handle_sip_r_invite(switch_core_session_t* session, int status,
    ...
    if (sip && session) {
        ...
        if (status == 100 && !sofia_test_flag(tech_pvt, TFLAG_100_UEPOCH_SET)) {
            ...
            switch_channel_set_variable_printf(channel, "sip_100_uepoch", "%" SWITCH_TIME_T_FMT,
                                               switch_time_now());
        }
        ...
        if (de && de->data && de->data->e_msg) {
            ...
            switch_channel_set_variable_printf(
                channel, "sip_local_network_addr", "%s",
        ...
        if ((caller_profile = switch_channel_get_caller_profile(channel)) && !zstr(network_ip) &&
            (zstr(caller_profile->network_addr) ||
             strcmp(caller_profile->network_addr, network_ip))) {
        ...
        if (sip->sip_payload && sip->sip_payload->pl_data) {
            ...
            if (!zstr(tech_pvt->mparams.prev_sdp_response) &&
                ...
                tech_pvt->mparams.last_sdp_response =
                    switch_core_session_strdup(session, sip->sip_payload->pl_data);
            }
        }

        if (status > 299 &&
            switch_channel_test_app_flag_key("T38", tech_pvt->channel, CF_APP_T38_REQ)) {
        ...
        if (sofia_test_pflag(profile, PFLAG_MANAGE_SHARED_APPEARANCE)) {
            if (channel && sip->sip_call_info) {
                ...
                if (sip->sip_from && sip->sip_from->a_url->url_user &&
                    sip->sip_from->a_url->url_host && sip->sip_to && sip->sip_to->a_url->url_user &&
                    sip->sip_to->a_url->url_host) {
                    sql = switch_mprintf(
                    ...
                    if (!zstr(buf)) {
                        sql = switch_mprintf(
                            "update sip_dialogs set call_info='%q',call_info_state='%q' "
                            "where uuid='%q'",
                        ...
                        switch_channel_set_variable_printf(channel, "presence_call_info_full",
                                                           "<sip:%s>;%s",
                                                           sip->sip_from->a_url->url_host, buf);
        ...
        if ((status == 180 || status == 183 || status > 199)) {
            ...
            if (!(vval = switch_channel_get_variable(channel, "sip_copy_custom_headers")) ||
                ...
                if (switch_core_session_get_partner(session, &other_session) ==
                    SWITCH_STATUS_SUCCESS) {
                    if (status > 199) {
                        switch_ivr_transfer_variable(session, other_session,
                                                     SOFIA_SIP_RESPONSE_HEADER_PREFIX_T);
                    } else {
                        switch_ivr_transfer_variable(session, other_session,
                                                     SOFIA_SIP_PROGRESS_HEADER_PREFIX_T);
                    }
                    switch_core_session_rwunlock(other_session);
                }
            }
        ...
        if ((status == 180 || status == 183 || status == 200)) {
            ...
            if ((x_freeswitch_support = sofia_glue_get_unknown_header(sip, "X-FS-Support"))) {
                tech_pvt->x_freeswitch_support_remote =
                    switch_core_session_strdup(session, x_freeswitch_support);
            }

            if (sip->sip_user_agent && sip->sip_user_agent->g_string) {
                switch_channel_set_variable(channel, "sip_user_agent",
        ...
        if (channel && (status == 300 || status == 301 || status == 302 || status == 305) &&
            ...
            if ((v = switch_channel_get_variable(channel, "outbound_redirect_fatal")) &&
                switch_true(v)) {
                ...
                for (p_contact = sip->sip_contact; p_contact; p_contact = p_contact->m_next) {
                    ...
                    if (p_contact->m_url->url_user) {
                        switch_snprintf(var_name, sizeof(var_name), "sip_redirect_contact_user_%d",
                                        i);
                        switch_channel_set_variable(channel, var_name, p_contact->m_url->url_user);
                    }
                    if (p_contact->m_url->url_host) {
                        switch_snprintf(var_name, sizeof(var_name), "sip_redirect_contact_host_%d",
                                        i);
                        switch_channel_set_variable(channel, var_name, p_contact->m_url->url_host);
                    }
                    if (p_contact->m_url->url_params) {
                        switch_snprintf(var_name, sizeof(var_name),
                                        "sip_redirect_contact_params_%d", i);
                        switch_channel_set_variable(channel, var_name,
                                                    p_contact->m_url->url_params);
                    }
                ...
                switch_channel_set_variable(channel, SWITCH_PROTO_SPECIFIC_HANGUP_CAUSE_VARIABLE,
                                            var_name);
            ...
            if ((br = switch_channel_get_partner_uuid(channel))) {
                ...
                if ((a_session = switch_core_session_locate(br)) &&
                    ...
                    if (!(sip_redirect_profile =
                              switch_channel_get_variable(channel, "sip_redirect_profile"))) {
                        sip_redirect_profile = profile->name;
                    }
                    if (!(sip_redirect_context =
                              switch_channel_get_variable(channel, "sip_redirect_context"))) {
                        sip_redirect_context = "redirected";
                    }
                    if (!(sip_redirect_dialplan =
                              switch_channel_get_variable(channel, "sip_redirect_dialplan"))) {
                    ...
                    for (p_contact = sip->sip_contact; p_contact; p_contact = p_contact->m_next) {
                        ...
                        if (i == 0) {
                            switch_channel_set_variable(channel, "sip_redirected_to", full_contact);
                            switch_channel_set_variable(a_channel, "sip_redirected_to",
                                                        full_contact);
                        }

                        if (p_contact->m_url->url_user) {
                            switch_snprintf(var_name, sizeof(var_name),
                                            "sip_redirect_contact_user_%d", i);
                            switch_channel_set_variable(channel, var_name,
                                                        p_contact->m_url->url_user);
                            switch_channel_set_variable(a_channel, var_name,
                                                        p_contact->m_url->url_user);
                        }
                        if (p_contact->m_url->url_host) {
                            switch_snprintf(var_name, sizeof(var_name),
                                            "sip_redirect_contact_host_%d", i);
                            switch_channel_set_variable(channel, var_name,
                                                        p_contact->m_url->url_host);
                            switch_channel_set_variable(a_channel, var_name,
                                                        p_contact->m_url->url_host);
                        }
                        if (p_contact->m_url->url_params) {
                        ...
                        switch_snprintf(var_name, sizeof(var_name), "sip_redirect_dialstring_%d",
                                        i);
                        switch_channel_set_variable_printf(channel, var_name, "sofia/%s/%s",
                                                           sip_redirect_profile, invite_contact);
                        switch_channel_set_variable_printf(a_channel, var_name, "sofia/%s/%s",
                                                           sip_redirect_profile, invite_contact);
                        stream.write_function(&stream, "%ssofia/%s/%s", i ? separator : "",
                                              sip_redirect_profile, invite_contact);
                        free(invite_contact);
                        i++;
                    }
                    ...
                    switch_channel_set_variable(channel, "sip_redirect_dialstring",
                                                redirect_dialstring);
                    switch_channel_set_variable(a_channel, "sip_redirect_dialstring",
                                                redirect_dialstring);
                    ...
                    if ((diversion_header = sofia_glue_get_unknown_header(sip, "diversion"))) {
                        ...
                        switch_channel_set_variable(a_channel, "sip_redirected_by",
                                                    diversion_header);
                    }
                    ...
                    if (sofia_test_pflag(profile, PFLAG_MANUAL_REDIRECT)) {
                        if (!switch_channel_get_variable(channel, "outbound_redirect_info")) {
                            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                                              ...
                                              p_contact->m_url->url_user, sip_redirect_dialplan,
                                              sip_redirect_context);

                            if (switch_true(switch_channel_get_variable(
                                    channel, "recording_follow_transfer"))) {
                        ...
                        switch_channel_hangup(channel, SWITCH_CAUSE_DESTINATION_OUT_OF_ORDER);
                               ...
                               (switch_xml_locate_domain(p_contact->m_url->url_host, NULL, &root,
                                                         &domain) == SWITCH_STATUS_SUCCESS)) {
                        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                        ...
                        switch_ivr_session_transfer(a_session, p_contact->m_url->url_user, NULL,
                                                    NULL);
                        switch_channel_hangup(channel, SWITCH_CAUSE_REDIRECTION_TO_NEW_DESTINATION);
        ...
        if (sip->sip_payload && sip->sip_payload->pl_data &&
            switch_stristr("m=image", sip->sip_payload->pl_data)) {
            has_t38 = 1;
        }
                ...
                if ((uuid = switch_channel_get_partner_uuid(channel)) &&
                    ...
                    if (switch_core_session_compare(session, other_session)) {
                        private_object_t* other_tech_pvt =
                            switch_core_session_get_private(other_session);
                        if (switch_core_media_ready(tech_pvt->session, SWITCH_MEDIA_TYPE_AUDIO)) {
                            switch_core_media_udptl_mode(other_tech_pvt->session,
            ...
            if (sofia_test_flag(tech_pvt, TFLAG_SENT_UPDATE)) {
                ...
                if ((uuid = switch_channel_get_partner_uuid(channel)) &&
                    ...
                    if (sip->sip_payload && sip->sip_payload->pl_data && sip->sip_content_type &&
                        ...
                        tech_pvt->mparams.remote_sdp_str = switch_core_session_strdup(
                            tech_pvt->session, sip->sip_payload->pl_data);
                    ...
                    if (status == 491 &&
                        ...
                        switch_core_media_reset_t38(other_session);
                               ...
                               switch_channel_test_flag(channel, CF_T38_PASSTHRU) && has_t38 &&
                               sip->sip_payload && sip->sip_payload->pl_data) {
                        switch_t38_options_t* t38_options = switch_core_media_extract_t38_options(
                        ...
                        if (!t38_options) {
                            ...
                            switch_core_media_process_t38_passthru(session, other_session,
                                                                   t38_options);
                        }
                    }
                    ...
                    if (status == 200 &&
                        switch_channel_test_flag(tech_pvt->channel, CF_T38_PASSTHRU) && has_t38) {
                    ...
                    if (status == 200 && switch_channel_test_flag(channel, CF_T38_PASSTHRU) &&
                        has_t38) {
        ...
        if ((status == 180 || status == 183 || status == 200)) {
            ...
            if ((!switch_channel_test_flag(channel, CF_EARLY_MEDIA) &&
                ...
                const char *from_user = "", *from_host = "", *to_user = "", *to_host = "",
                ...
                if (profile->pres_type) {
                    const char* presence_data =
                        switch_channel_get_variable(channel, "presence_data");
                    ...
                    if (sip->sip_contact) {
                        full_contact = sip_header_as_string(nua_handle_home(tech_pvt->nh),
                                                            (void*)sip->sip_contact);
                    }
                    ...
                    sql = switch_mprintf(
                        ...
                        "(call_id,uuid,sip_to_user,sip_to_host,sip_to_tag,sip_from_user,sip_from_"
                        "host,sip_from_tag,contact_user,"
                        ...
                        from_user, from_host, from_tag, contact_user, contact_host, astate,
                        "outbound", user_agent, profile->name, mod_sofia_globals.hostname,
                ...
                sql = switch_mprintf(
                    "update sip_dialogs set state='%q',presence_id='%q',presence_data='%q' "
                    "where uuid='%q';\n",
        ...
        if ((uuid = switch_channel_get_partner_uuid(channel)) &&
            ...
            if (switch_core_session_compare(session, other_session)) {
                ...
                if (switch_channel_direction(tech_pvt->channel) == SWITCH_CALL_DIRECTION_INBOUND) {
                    switch_ivr_3p_media(switch_core_session_get_uuid(other_session),
                                        SMF_REBRIDGE | SMF_REPLYONLY_B);
                } else {
...
static void launch_media_on_hold(switch_core_session_t* session) {
    ...
    switch_threadattr_detach_set(thd_attr, 1);
    switch_threadattr_stacksize_set(thd_attr, SWITCH_THREAD_STACKSIZE);
    switch_thread_create(&thread, thd_attr, media_on_hold_thread_run, session,
                         switch_core_session_get_pool(session));
}

static void mark_transfer_record(switch_core_session_t* session, const char* br_a,
                                 const char* br_b) {
    ...
    if ((br_b_session = switch_core_session_locate(br_b))) {
        ...
        cp->transfer_source = switch_core_sprintf(cp->pool, "%ld:%s:att_xfer:%s@%s/%s@%s",
                                                  (long)switch_epoch_time_now(NULL), cp->uuid_str,
        ...
        switch_channel_add_variable_var_check(br_b_channel, SWITCH_TRANSFER_HISTORY_VARIABLE,
                                              cp->transfer_source, SWITCH_FALSE, SWITCH_STACK_PUSH);
        switch_channel_set_variable(br_b_channel, SWITCH_TRANSFER_SOURCE_VARIABLE,
                                    cp->transfer_source);
    ...
    if ((br_a_session = switch_core_session_locate(br_a))) {
        ...
        switch_channel_add_variable_var_check(br_a_channel, SWITCH_TRANSFER_HISTORY_VARIABLE,
                                              cp->transfer_source, SWITCH_FALSE, SWITCH_STACK_PUSH);
        switch_channel_set_variable(br_a_channel, SWITCH_TRANSFER_SOURCE_VARIABLE,
                                    cp->transfer_source);
...
static void sofia_handle_sip_i_state(switch_core_session_t* session, int status, char const* phrase,
    ...
    tl_gets(tags, NUTAG_CALLSTATE_REF(ss_state), NUTAG_OFFER_RECV_REF(offer_recv),
            NUTAG_ANSWER_RECV_REF(answer_recv), NUTAG_OFFER_SENT_REF(offer_sent),
            NUTAG_ANSWER_SENT_REF(answer_sent), SIPTAG_REPLACES_STR_REF(replaces_str),
    ...
    if (session) {
        ...
        if (r_sdp && (switch_channel_test_flag(channel, CF_PROXY_MODE) ||
                      switch_channel_test_flag(channel, CF_PROXY_MEDIA))) {
            const char* var;
        ...
        if ((switch_channel_test_flag(channel, CF_PROXY_MODE) ||
             switch_channel_test_flag(channel, CF_PROXY_MEDIA)) ||
            (sofia_test_flag(profile, TFLAG_INB_NOMEDIA) ||
             sofia_test_flag(profile, TFLAG_PROXY_MEDIA))) {
    ...
    if (session) {
        if ((switch_channel_test_flag(channel, CF_ANSWERED) && (status == 180 || status == 183) &&
             !r_sdp) ||
        ...
        if (r_sdp) {
            ...
            if (!(profile->mndlb & SM_NDLB_ALLOW_NONDUP_SDP) ||
                (!zstr(tech_pvt->mparams.remote_sdp_str) &&
                ...
                // switch_channel_direction(channel) == SWITCH_CALL_DIRECTION_OUTBOUND)) {
                //	switch_core_media_set_sdp_codec_string(session, r_sdp, status < 200 ?
    ...
    if (channel && profile->pres_type && ss_state == nua_callstate_ready && status == 200) {
        ...
        sql = switch_mprintf(
            "update sip_dialogs set sip_to_tag='%q' "
    ...
    if (channel && (status == 180 || status == 183) &&
        ...
        if ((var = switch_channel_get_variable(channel, "sip_auto_answer")) && switch_true(var) &&
            !((var = switch_channel_get_variable(channel, "sip_auto_answer_suppress_notify")) &&
              switch_true(var))) {
            full_to = switch_str_nil(switch_channel_get_variable(channel, "sip_full_to"));

            nua_notify(
                nh, NUTAG_NEWSUB(1), NUTAG_WITH_THIS_MSG(de->data->e_msg),
    ...
    switch ((enum nua_callstate)ss_state) {
            ...
            if (r_sdp) {
                if (switch_channel_test_flag(channel, CF_PROXY_MODE)) {
                    ...
                    if ((ptr = switch_stristr("m=audio", r_sdp))) {
                        ...
                        switch_channel_set_variable(channel, SWITCH_REMOTE_MEDIA_PORT_VARIABLE,
                                                    pbuf);
                    }

                    if (switch_channel_test_flag(channel, CF_PROXY_MEDIA) &&
                        switch_channel_direction(tech_pvt->channel) ==
                    ...
                    if (switch_core_session_get_partner(session, &other_session) ==
                        ...
                        switch_core_session_queue_indication(other_session,
                    ...
                    if (sofia_test_flag(tech_pvt, TFLAG_LATE_NEGOTIATION) &&
                        ...
                        if (sofia_media_tech_media(tech_pvt, (char*)r_sdp, SDP_OFFER) !=
                            SWITCH_STATUS_SUCCESS) {
                            switch_channel_set_variable(channel,
                                                        SWITCH_ENDPOINT_DISPOSITION_VARIABLE,
        ...
        case nua_callstate_completing: {
            ...
            if (!switch_channel_test_flag(channel, CF_ANSWERED)) {
                const char* wait_for_ack =
                    switch_channel_get_variable(channel, "sip_wait_for_aleg_ack");
            ...
            if (switch_channel_test_flag(channel, CF_3P_NOMEDIA_REQUESTED)) {
                        ...
                        if (switch_core_session_compare(session, other_session)) {
                            ...
                            if (sofia_use_soa(other_tech_pvt)) {
                                nua_ack(other_tech_pvt->nh,
                                        ...
                                        SOATAG_USER_SDP_STR(r_sdp), SOATAG_REUSE_REJECTED(1),
                                        SOATAG_RTP_SELECT(1),
                                        SOATAG_AUDIO_AUX("cn telephone-event"),
                                        TAG_IF(sofia_test_pflag(other_tech_pvt->profile,
                ...
                if (!match) {
                    if (switch_channel_get_state(channel) != CS_NEW) {
                        nua_respond(
                            tech_pvt->nh, SIP_488_NOT_ACCEPTABLE,
                            TAG_IF(!zstr(session_id_header), SIPTAG_HEADER_STR(session_id_header)),
                            TAG_END());
                    }
                } else {
            ...
            if (r_sdp && sofia_test_flag(tech_pvt, TFLAG_3PCC_INVITE) &&
                !sofia_test_flag(tech_pvt, TFLAG_SDP)) {
                sofia_set_flag(tech_pvt, TFLAG_SDP);
                if (switch_core_session_get_partner(session, &other_session) ==
                    SWITCH_STATUS_SUCCESS) {
                    other_channel = switch_core_session_get_channel(other_session);
            ...
            if (!sofia_test_flag(tech_pvt, TFLAG_SDP)) {
                if (switch_core_session_get_partner(session, &other_session) ==
                    ...
                    if (r) {
                        /* Due to a race between simultaneous reinvites to both legs of a bridge,
                          ...
                          So we reject the incoming invite with a 491 and redo the failed outgoing
                          invite. */
                        ...
                        nua_respond(
                            tech_pvt->nh, SIP_491_REQUEST_PENDING,
                            TAG_IF(!zstr(session_id_header), SIPTAG_HEADER_STR(session_id_header)),
                            TAG_END());
                        sofia_glue_do_invite(session);
                        goto done;
                ...
                if (r_sdp && !sofia_test_flag(tech_pvt, TFLAG_SDP)) {
                    if (switch_channel_test_flag(channel, CF_PROXY_MODE)) {
                        ...
                        if (!match) {
                            ...
                            switch_channel_set_variable(
                                channel, SWITCH_ENDPOINT_DISPOSITION_VARIABLE, "RECEIVED");
                            sofia_set_flag_locked(tech_pvt, TFLAG_READY);
                                ...
                                if ((replaces = sip_replaces_make(home, replaces_str)) &&
                                    ...
                                    if ((b_private = nua_handle_magic(bnh))) {
                                        ...
                                        if (br_b) {
                                            ...
                                            switch_channel_set_variable_printf(
                                                channel, "transfer_to", "att:%s", br_b);
                                            ...
                                            switch_channel_hangup(
                                                channel, SWITCH_CAUSE_DESTINATION_OUT_OF_ORDER);
                                        }
                                    } else {
                                        switch_channel_set_variable(
                                            channel, SWITCH_ENDPOINT_DISPOSITION_VARIABLE,
                    ...
                    if (sofia_test_pflag(profile, PFLAG_3PCC)) {
                        if (switch_channel_test_flag(channel, CF_PROXY_MODE) ||
                            switch_channel_test_flag(channel, CF_PROXY_MEDIA)) {
                            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO,
                                              "No SDP in INVITE and 3pcc=yes cannot work with "
                                              "bypass or proxy media, hanging up.\n");
                            switch_channel_set_variable(
                                channel, SWITCH_ENDPOINT_DISPOSITION_VARIABLE, "3PCC DISABLED");
                            switch_channel_hangup(channel, SWITCH_CAUSE_MANDATORY_IE_MISSING);
                        } else {
                            switch_channel_set_variable(
                                channel, SWITCH_ENDPOINT_DISPOSITION_VARIABLE, "RECEIVED_NOSDP");
                        ...
                        //  so can be made to work with bypass media as we have time to find out
                        //  what the other end thinks codec offer should be...
                        switch_channel_set_variable(channel, SWITCH_ENDPOINT_DISPOSITION_VARIABLE,
                                                    "RECEIVED_NOSDP");
                        sofia_set_flag_locked(tech_pvt, TFLAG_3PCC);
                        // switch_core_media_choose_port(tech_pvt->session, SWITCH_MEDIA_TYPE_AUDIO,
                        // 0); switch_core_media_gen_local_sdp(session, NULL, 0, NULL, 0);
                ...
                if (switch_channel_var_true(channel, "sip_unhold_nosdp")) {
                    switch_core_media_gen_local_sdp(
                        session, SDP_ANSWER, NULL, 0, "sendrecv",
                        zstr(tech_pvt->mparams.local_sdp_str) ||
                            !switch_channel_test_flag(channel, CF_PROXY_MODE));
                } else {
                    switch_core_media_gen_local_sdp(
                        session, SDP_ANSWER, NULL, 0, NULL,
                        zstr(tech_pvt->mparams.local_sdp_str) ||
                            !switch_channel_test_flag(channel, CF_PROXY_MODE));
                }
                ...
                if (zstr(tech_pvt->mparams.local_sdp_str)) {
                    ...
                    if (sofia_use_soa(tech_pvt)) {
                        nua_respond(
                            ...
                            SOATAG_USER_SDP_STR(tech_pvt->mparams.local_sdp_str),
                            SOATAG_REUSE_REJECTED(1), SOATAG_AUDIO_AUX("cn telephone-event"),
                            TAG_IF(sofia_test_pflag(profile, PFLAG_DISABLE_100REL),
                                   NUTAG_INCLUDE_EXTRA_SDP(1)),
                            TAG_IF(!zstr(session_id_header), SIPTAG_HEADER_STR(session_id_header)),
                            TAG_END());
        ...
        case nua_callstate_early:
            if (answer_recv) {
                ...
                if (match) {
                    if (switch_core_media_choose_port(tech_pvt->session, SWITCH_MEDIA_TYPE_AUDIO,
        ...
        case nua_callstate_completed:
            if (r_sdp) {
                ...
                if ((var = switch_channel_get_variable(channel, "sip_ignore_reinvites")) &&
                    switch_true(var)) {
                    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                                      "Ignoring Re-invite\n");
                    nua_respond(
                        tech_pvt->nh, SIP_200_OK,
                        TAG_IF(!zstr(session_id_header), SIPTAG_HEADER_STR(session_id_header)),
                        TAG_END());
                    goto done;
                }
                ...
                if (switch_channel_test_flag(channel, CF_PROXY_MODE) ||
                    ...
                    if ((sofia_test_media_flag(profile, SCMF_DISABLE_HOLD) ||
                         ((var = switch_channel_get_variable(channel, "rtp_disable_hold")) &&
                          switch_true(var))) &&
                    ...
                    if (switch_core_session_get_partner(session, &other_session) ==
                        ...
                        int media_on_hold = switch_true(switch_channel_get_variable_dup(
                            channel, "bypass_media_resume_on_hold", SWITCH_FALSE, -1));
                        ...
                        if (switch_channel_test_flag(channel, CF_PROXY_MODE) && !is_t38 &&
                            ...
                            if (switch_stristr("sendonly", r_sdp) ||
                                ...
                                switch_channel_set_variable(channel, SWITCH_R_SDP_VARIABLE, r_sdp);
                                switch_channel_clear_flag(channel, CF_PROXY_MODE);
                                switch_core_media_set_local_sdp(tech_pvt->session, NULL,
                                                                SWITCH_FALSE);
                                switch_core_media_clear_rtp_flag(tech_pvt->session,
                                                                 SWITCH_MEDIA_TYPE_AUDIO,
                                                                 SWITCH_RTP_FLAG_AUTOADJ);

                                if (!switch_channel_media_ready(channel)) {
                                    // if (switch_channel_direction(tech_pvt->channel) ==
                                    // SWITCH_CALL_DIRECTION_INBOUND) { const char *r_sdp =
                                    // switch_channel_get_variable(channel, SWITCH_R_SDP_VARIABLE);

                                    switch_core_media_prepare_codecs(tech_pvt->session,
                                                                     SWITCH_TRUE);
                                    if (sofia_media_tech_media(tech_pvt, r_sdp, SDP_OFFER) !=
                                        SWITCH_STATUS_SUCCESS) {
                                        switch_channel_set_variable(
                                            channel, SWITCH_ENDPOINT_DISPOSITION_VARIABLE,
                                            "CODEC NEGOTIATION ERROR");
                                ...
                                if (!switch_core_media_ready(tech_pvt->session,
                                                             SWITCH_MEDIA_TYPE_AUDIO)) {
                                    switch_core_media_prepare_codecs(tech_pvt->session,
                                                                     SWITCH_FALSE);
                                    if (switch_core_media_choose_port(tech_pvt->session,
                                                                      SWITCH_MEDIA_TYPE_AUDIO,
                                                                      0) != SWITCH_STATUS_SUCCESS) {
                                        switch_channel_hangup(
                                            channel, SWITCH_CAUSE_DESTINATION_OUT_OF_ORDER);
                        ...
                        if (sofia_test_flag(tech_pvt, TFLAG_SIP_HOLD)) {
                            ...
                            if (sofia_test_pflag(profile, PFLAG_MANAGE_SHARED_APPEARANCE)) {
                                const char* info =
                                    switch_channel_get_variable(channel, "presence_call_info");
                                if (info) {
                        ...
                        if (switch_core_session_compare(session, other_session)) {
                            ...
                            if (sofia_test_pflag(profile, PFLAG_PROXY_HOLD)) {
                                ...
                                if (switch_stristr("inactive", r_sdp)) {
                                    ...
                                    // switch_channel_set_variable(channel, "sofia_hold_inactive",
                                    // "true");
                                } else {
                                    sofia_clear_flag_locked(other_tech_pvt,
                                ...
                                if (hold_related == 1) {
                                    ...
                                    if (sofia_test_pflag(profile, PFLAG_MANAGE_SHARED_APPEARANCE)) {
                                        const char* info = switch_channel_get_variable(
                                            channel, "presence_call_info");
                                    ...
                                    sofia_set_flag_locked(tech_pvt, TFLAG_SIP_HOLD);
                                    switch_channel_set_flag(channel, CF_LEG_HOLDING);
                                    switch_channel_presence(tech_pvt->channel, "unknown", hold_msg,
                                                            NULL);
                                } else {
                                    hold_msg = "unhold";
                    ...
                    if (switch_channel_test_app_flag_key("T38", tech_pvt->channel,
                ...
                if (is_ok) {
                    if (switch_core_session_local_crypto_key(tech_pvt->session,
                                                             SWITCH_MEDIA_TYPE_AUDIO)) {
                        switch_core_media_gen_local_sdp(session, SDP_ANSWER, NULL, 0, NULL, 0);
                    }
                    ...
                    if (switch_event_create_subclass(&s_event, SWITCH_EVENT_CUSTOM,
                                                     MY_EVENT_REINVITE) == SWITCH_STATUS_SUCCESS) {
        ...
        case nua_callstate_ready:
            if (!switch_channel_test_flag(channel, CF_PROXY_MODE) &&
                !switch_channel_test_flag(channel, CF_PROXY_MEDIA) && r_sdp &&
                (!is_dup_sdp || sofia_test_flag(tech_pvt, TFLAG_NEW_SDP)) &&
                switch_core_media_ready(tech_pvt->session, SWITCH_MEDIA_TYPE_AUDIO) &&
                !sofia_test_flag(tech_pvt, TFLAG_NOSDP_REINVITE)) {
                /* sdp changed since 18X w sdp, we're supposed to ignore it but we, of course, were
                 * pressured into supporting it */
                uint8_t match = 0;
            ...
            if (r_sdp && sofia_test_flag(tech_pvt, TFLAG_NOSDP_REINVITE)) {
                ...
                if (switch_channel_test_flag(channel, CF_PROXY_MODE) ||
                    ...
                    if (!is_ok) {
                        nua_respond(
                            nh, SIP_488_NOT_ACCEPTABLE,
                            TAG_IF(!zstr(session_id_header), SIPTAG_HEADER_STR(session_id_header)),
                            TAG_END());
                        switch_channel_hangup(tech_pvt->channel,
            ...
            if (channel) {
                if (sofia_test_flag(tech_pvt, TFLAG_EARLY_MEDIA) &&
                    !sofia_test_flag(tech_pvt, TFLAG_ANS)) {
                ...
                if (!r_sdp && !sofia_test_flag(tech_pvt, TFLAG_SDP)) {
                    r_sdp =
                        (const char*)switch_channel_get_variable(channel, SWITCH_R_SDP_VARIABLE);
                }
                    ...
                    if (switch_channel_test_flag(channel, CF_PROXY_MODE) ||
                        ...
                        if (match) {
                            if (switch_core_media_choose_port(tech_pvt->session,
                                ...
                                if (sofia_test_flag(tech_pvt, TFLAG_3PCC)) {
                                    /* Check if we are in 3PCC proxy mode, if so then set the flag
                                     * to indicate we received the ack */
        ...
        case nua_callstate_terminated:
            ...
            if (sofia_test_flag(tech_pvt, TFLAG_NOHUP)) {
                ...
                if (tech_pvt->q850_cause &&
                    !switch_channel_var_true(channel, "ignore_q850_reason")) {
                    cause = tech_pvt->q850_cause;
                } else {
                    cause = sofia_glue_sip_cause_to_freeswitch(status);
                }
                if (status) {
                    ...
                    switch_snprintf(st, sizeof(st), "sip:%d", status);
                    switch_channel_set_variable(channel,
                                                SWITCH_PROTO_SPECIFIC_HANGUP_CAUSE_VARIABLE, st);
                    if (phrase) {
                        switch_channel_set_variable_partner(channel, "sip_hangup_phrase", phrase);
                    }
                    sofia_glue_set_extra_headers(session, sip, SOFIA_SIP_BYE_HEADER_PREFIX);
...
void* SWITCH_THREAD_FUNC nightmare_xfer_thread_run(switch_thread_t* thread, void* obj) {
    ...
    if ((a_session = switch_core_session_locate(nhelper->bridge_to_uuid))) {
        ...
        if ((session = switch_core_session_locate(nhelper->reply_uuid))) {
            ...
            if ((status = switch_ivr_originate(
                     NULL, &tsession, &cause, nhelper->exten_with_params, timeout, NULL, NULL, NULL,
                     switch_channel_get_caller_profile(channel_a), nhelper->vars, SOF_NONE, NULL,
                     NULL)) == SWITCH_STATUS_SUCCESS) {
                if (switch_channel_up(channel_a)) {
                    if (switch_true(
                            switch_channel_get_variable(channel_a, "recording_follow_transfer"))) {
                        switch_ivr_transfer_recordings(session, a_session);
                    }

                    tuuid_str = switch_core_session_get_uuid(tsession);
                    switch_channel_set_variable_printf(channel_a, "transfer_to", "att:%s",
                                                       tuuid_str);
                    ...
                    switch_channel_hangup(switch_core_session_get_channel(tsession),
            ...
            nua_notify(
                tech_pvt->nh, NUTAG_NEWSUB(1), SIPTAG_CONTENT_TYPE_STR("message/sipfrag"),
...
nua_handle_t* sofia_global_nua_handle_by_replaces(sip_replaces_t* replaces) {
    ...
    if (mod_sofia_globals.profile_hash) {
        for (hi = switch_core_hash_first(mod_sofia_globals.profile_hash); hi;
...
static switch_status_t sofia_process_proxy_refer(switch_core_session_t* session,
                                                 const char* refer_to, sip_t const* sip) {
    switch_core_session_t* other_session;
    private_object_t* tech_pvt = switch_core_session_get_private(session);
...
void sofia_handle_sip_i_refer(nua_t* nua, sofia_profile_t* profile, nua_handle_t* nh,
                              switch_core_session_t* session, sip_t const* sip,
                              sofia_dispatch_event_t* de, tagi_t tags[]) {
    /* Incoming refer */
    ...
    if (refer_to) {
        ...
        if (refer_to->r_url->url_params) {
            switch_channel_set_variable(tech_pvt->channel, "sip_refer_to_params",
        ...
        if (rep) {
            ...
            if (bnh && !sofia_test_pflag(profile, PFLAG_MAKE_EVERY_TRANSFER_A_NIGHTMARE)) {
                ...
                if ((b_private = nua_handle_magic(bnh))) {
                    ...
                    if (channel_a && switch_true(switch_channel_get_variable(
                                         channel_a, "deny_refer_requests"))) {
                        deny_refer_requests = 1;
                    }

                    if (!deny_refer_requests && channel_b &&
                        switch_true(
                            switch_channel_get_variable(channel_b, "deny_refer_requests"))) {
                        deny_refer_requests = 1;
                    }
                    ...
                    if (!deny_refer_requests && br_a) {
                        ...
                        if ((a_session = switch_core_session_locate(br_a))) {
                            ...
                            if (a_channel && switch_true(switch_channel_get_variable(
                    ...
                    if (!deny_refer_requests && br_b) {
                        ...
                        if ((b_session = switch_core_session_locate(br_b))) {
                            ...
                            if (b_channel && switch_true(switch_channel_get_variable(
                    ...
                    if (deny_refer_requests) {
                        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_NOTICE,
                                          "Denying Attended Transfer, variable "
                                          "[deny_refer_requests] was set to true\n");

                        nua_notify(
                            tech_pvt->nh, NUTAG_NEWSUB(1),
                        ...
                        switch_channel_set_variable_printf(channel_b, "transfer_to", "satt:%s",
                                                           br_a);
                        ...
                        if ((a_session = switch_core_session_locate(br_a))) {
                            ...
                            switch_caller_profile_t* prof =
                                switch_channel_get_caller_profile(channel_b);
                            const char* tmp;
                            ...
                            switch_channel_set_variable(a_channel, "att_xfer_callee_id_name",
                                                        prof->callee_id_name);
                            ...
                            if (sofia_test_pflag(profile, PFLAG_FIRE_TRANFER_EVENTS)) {
                                ...
                                if (switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM,
                                    ...
                                    switch_event_add_header_string(
                                        event, SWITCH_STACK_BOTTOM, "att_xfer_replaced_call_id",
                                        switch_core_session_get_uuid(b_session));
                                    switch_event_fire(&event);
                                }
                            }
                            ...
                            if (moh) {
                                char* xdest;
                                xdest = switch_core_session_sprintf(
                                    a_session, "m:\":endless_playback:%s\"park", moh);
                            ...
                            if (switch_true(switch_channel_get_variable(
                                    channel_b, "recording_follow_transfer")) &&
                                (tmpsess = switch_core_session_locate(br_a))) {
                                switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session),
                                                  SWITCH_LOG_NOTICE,
                                                  "Early transfer detected with no media, moving "
                                                  "recording bug to other leg\n");
                            ...
                            if (b_tech_pvt && !sofia_test_flag(b_tech_pvt, TFLAG_BYE)) {
                                ...
                                val = switch_channel_get_variable(tech_pvt->channel,
                                                                  "disable_q850_reason");
                                if (!val || switch_true(val)) {
                                    q850 = switch_core_session_sprintf(
                                        a_session, "Q.850;cause=16;text=\"normal_clearing\"");
                                }
                        ...
                        if (switch_true(switch_channel_get_variable(channel_a,
                                                                    "recording_follow_transfer")) &&
                            (tmp = switch_core_session_locate(br_a))) {
                        ...
                        if (switch_true(switch_channel_get_variable(channel_b,
                                                                    "recording_follow_transfer")) &&
                        ...
                        if (sofia_test_pflag(profile, PFLAG_FIRE_TRANFER_EVENTS)) {
                            ...
                            if (switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM,
                                ...
                                switch_event_add_header_string(event, SWITCH_STACK_BOTTOM,
                                                               ...
                                                               br_b);
                                switch_event_add_header_string(
                                    event, SWITCH_STACK_BOTTOM, "att_xfer_destination_peer_uuid",
                                    switch_core_session_get_uuid(b_session));
                                switch_event_fire(&event);
                            }
                        ...
                        switch_channel_clear_flag(channel_b, CF_LEG_HOLDING);
                        sofia_clear_flag_locked(tech_pvt, TFLAG_HOLD_LOCK);
                        switch_channel_set_variable(channel_b, "park_timeout",
                                                    "2:attended_transfer");
                        ...
                        if (!br_a && !br_b) {
                            ...
                            if (t_session) {
                                switch_channel_t* t_channel =
                                    switch_core_session_get_channel(t_session);
                                const char* idest =
                                    switch_channel_get_variable(hup_channel, "inline_destination");
                                ext =
                                    switch_channel_get_variable(hup_channel, "destination_number");

                                if (!zstr(full_ref_by)) {
                                    switch_channel_set_variable(
                                        t_channel, SOFIA_SIP_HEADER_PREFIX "Referred-By",
                                        full_ref_by);
                                }
                                ...
                                if (switch_true(switch_channel_get_variable(
                                        hup_channel, "recording_follow_transfer"))) {
                                ...
                                if (sofia_test_pflag(profile, PFLAG_FIRE_TRANFER_EVENTS)) {
                                    if (switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM,
                                        ...
                                        switch_channel_event_set_data(channel_a, event);
                                        switch_event_add_header_string(
                                            event, SWITCH_STACK_BOTTOM, "att_xfer_original_call_id",
                    ...
                    if ((a_session = switch_core_session_locate(br_a))) {
                        ...
                        if (sofia_test_pflag(profile,
                            ...
                            const char *xml_url = NULL, *use_profile = profile->name,
                                       *dial_prefix = NULL, *absolute_dial_string = NULL;
                            switch_xml_t params = NULL, param = NULL;

                            switch_event_create(&xml_params, SWITCH_EVENT_REQUEST_PARAMS);
                            switch_event_add_header_string(xml_params, SWITCH_STACK_BOTTOM,
                                                           "purpose", "nightmare_xfer");
                            switch_event_add_header_string(xml_params, SWITCH_STACK_BOTTOM,
                            ...
                            switch_event_add_header_string(
                                xml_params, SWITCH_STACK_BOTTOM, "refer-to-params",
                                refer_to->r_url->url_params ? refer_to->r_url->url_params : "");
                            switch_event_add_header_string(
                                xml_params, SWITCH_STACK_BOTTOM, "refer-to-headers",
                            ...
                            switch_event_add_header_string(xml_params, SWITCH_STACK_BOTTOM,
                                                           ...
                                                           switch_core_session_get_uuid(session));
                            switch_event_add_header_string(xml_params, SWITCH_STACK_BOTTOM,
                                                           "refer-for-channel-id", br_a);

                            if (switch_xml_locate("channels", NULL, NULL, NULL, &xml_root,
                                                  &xml_channel, xml_params,
                                                  SWITCH_FALSE) == SWITCH_STATUS_SUCCESS) {
                                if ((params = switch_xml_child(xml_channel, "params"))) {
                                    for (param = switch_xml_child(params, "param"); param;
                                         param = param->next) {
                                ...
                                if (absolute_dial_string) {
                                    exten = switch_core_session_sprintf(
                                        session, "%s%s", dial_prefix, absolute_dial_string);
                                } else if (xml_url) {
                                    exten = switch_core_session_sprintf(session, "%ssofia/%s/%s",
                                                                        dial_prefix, use_profile,
                                                                        xml_url);
                                }
                        ...
                        if (zstr(exten)) {
                            exten = switch_core_session_sprintf(
                                session, "sofia/%s/sip:%s@%s%s%s", profile->name,
                                refer_to->r_url->url_user, refer_to->r_url->url_host,
                                port ? ":" : "", port ? port : "");
                        }

                        switch_core_new_memory_pool(&npool);
                        nightmare_xfer_helper =
                            switch_core_alloc(npool, sizeof(*nightmare_xfer_helper));
                        ...
                        if (refer_to->r_url->url_params || refer_to->r_url->url_headers) {
                            if (refer_to->r_url->url_headers) {
                                nightmare_xfer_helper->exten_with_params = switch_core_sprintf(
                                    npool, "{sip_invite_params=%s?%s}%s",
                                    refer_to->r_url->url_params ? refer_to->r_url->url_params : "",
                                    refer_to->r_url->url_headers, exten);
                            } else {
                                nightmare_xfer_helper->exten_with_params =
                                    switch_core_sprintf(npool, "{sip_invite_params=%s}%s",
                                                        refer_to->r_url->url_params, exten);
                            }
                        } else {
                                ...
                                if ((p = strchr(p, '='))) {
                                    *p++ = '\0';
                                    ...
                                    if (strcasecmp("Replaces", h)) {
                                        hp = switch_core_session_sprintf(
                                            session, "%s%s", SOFIA_SIP_HEADER_PREFIX, h);
                                        switch_channel_set_variable(channel, hp, v);
                                    } else {
                                        // use this one instead of rep value from above to keep all
                                        // parameters
                                        switch_channel_set_variable(channel, SOFIA_REPLACES_HEADER,
                                                                    v);
                                    }
                                    switch_log_printf(
                                        SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                        ...
                        switch_event_create(&nightmare_xfer_helper->vars,
                                            SWITCH_EVENT_CHANNEL_DATA);

                        rep_h = switch_channel_get_variable(channel, SOFIA_REPLACES_HEADER);
                        if (rep_h) {
                            switch_event_add_header_string(nightmare_xfer_helper->vars,
                                                           SWITCH_STACK_BOTTOM,
                        ...
                        if (!zstr(full_ref_by)) {
                            switch_event_add_header_string(nightmare_xfer_helper->vars,
                                                           SWITCH_STACK_BOTTOM, "Referred-By",
                                                           full_ref_by);
                        }
                        ...
                        switch_event_add_header_string(nightmare_xfer_helper->vars,
                                                       SWITCH_STACK_BOTTOM, "sip_h_X-FS-Refer-From",
                                                       switch_core_session_get_uuid(session));
                        switch_event_add_header_string(nightmare_xfer_helper->vars,
                                                       SWITCH_STACK_BOTTOM, "sip_h_X-FS-Refer-For",
                                                       br_a);
    ...
    if (exten) {
        ...
        if (!zstr(br) && (b_session = switch_core_session_locate(br))) {
            ...
            if (!zstr(full_ref_by)) {
                switch_channel_set_variable(b_channel, SOFIA_SIP_HEADER_PREFIX "Referred-By",
                                            full_ref_by);
            }
            ...
            if (((var = switch_channel_get_variable(channel, "confirm_blind_transfer")) &&
                 switch_true(var)) ||
                ...
                switch_channel_set_variable(b_channel, "blind_transfer_uuid",
                                            switch_core_session_get_uuid(session));
                switch_channel_set_variable(channel, "blind_transfer_uuid",
                                            switch_core_session_get_uuid(b_session));
            ...
            if (refer_to->r_url->url_params) {
                switch_channel_set_variable(b_channel, "sip_h_X-FS-Refer-Params",
                                            refer_to->r_url->url_params);
            }
            ...
            if (switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM, MY_EVENT_ERROR) ==
                ...
                switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "Full-Refer-To",
                                               full_ref_to);
                switch_channel_event_set_data(channel, event);
                switch_event_fire(&event);
            }
...
static switch_status_t create_info_event(sip_t const* sip, nua_handle_t* nh,
    ...
    if (sip->sip_content_type) {
        switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "SIP-Content-Type",
                                       sip->sip_content_type->c_type);
    }

    if (sip->sip_from) {
        if (sip->sip_from->a_url->url_user) {
            switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "SIP-From-User",
                                           sip->sip_from->a_url->url_user);
        }

        if (sip->sip_from->a_url->url_host) {
            switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "SIP-From-Host",
                                           sip->sip_from->a_url->url_host);
        }
    }
    ...
    if (sip->sip_to) {
        if (sip->sip_to->a_url->url_user) {
            switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "SIP-To-User",
                                           sip->sip_to->a_url->url_user);
        }

        if (sip->sip_to->a_url->url_host) {
            switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "SIP-To-Host",
                                           sip->sip_to->a_url->url_host);
        }
    }
    ...
    if (sip->sip_call_info) {
        switch_event_add_header_string(
            event, SWITCH_STACK_BOTTOM, "Call-Info",
            sip_header_as_string(nua_handle_home(nh), (void*)sip->sip_call_info));
    }

    if (alert_info) {
        switch_event_add_header_string(
            event, SWITCH_STACK_BOTTOM, "Alert-Info",
...
switch_status_t sofia_proxy_sip_i_message(nua_t* nua, sofia_profile_t* profile, nua_handle_t* nh,
    ...
    if (session &&
        ...
        if (switch_core_session_compare(session, other_session)) {
            ...
            nua_message(
                other_tech_pvt->nh,
                TAG_IF(ct, SIPTAG_CONTENT_TYPE_STR(
                               su_strdup(nua_handle_get_home(other_tech_pvt->nh), ct))),
                TAG_IF(!zstr(other_tech_pvt->user_via), SIPTAG_VIA_STR(other_tech_pvt->user_via)),
                TAG_IF(pl,
                       SIPTAG_PAYLOAD_STR(su_strdup(nua_handle_get_home(other_tech_pvt->nh), pl))),
                TAG_IF(!zstr(session_id_header), SIPTAG_HEADER_STR(session_id_header)), TAG_END());
        }
...
void sofia_handle_sip_i_info(nua_t* nua, sofia_profile_t* profile, nua_handle_t* nh,
                             switch_core_session_t* session, sip_t const* sip,
                             sofia_dispatch_event_t* de, tagi_t tags[]) {
    ...
    if (sofia_test_pflag(profile, PFLAG_EXTENDED_INFO_PARSING)) {
        if (sip && sip->sip_content_type && sip->sip_content_type->c_type &&
            sip->sip_content_type->c_subtype && sip->sip_payload && sip->sip_payload->pl_data) {
                ...
                if (!strcasecmp(sip->sip_content_type->c_subtype, "session-event")) {
                    if (session) {
                        ...
                        nua_respond(
                            nh, SIP_200_OK,
                            SIPTAG_CONTENT_TYPE_STR("freeswitch/session-event-response"),
                            SIPTAG_PAYLOAD_STR("-ERR INVALID SESSION"),
                            NUTAG_WITH_THIS_MSG(de->data->e_msg),
                            TAG_IF(!zstr(session_id_header), SIPTAG_HEADER_STR(session_id_header)),
                            TAG_END());
                    }
                    ...
                    if (switch_api_execute(cmd, arg, NULL, &stream) == SWITCH_STATUS_SUCCESS) {
                        nua_respond(
                            nh, SIP_200_OK, SIPTAG_CONTENT_TYPE_STR("freeswitch/api-response"),
                        ...
                        nua_respond(
                            nh, SIP_200_OK, SIPTAG_CONTENT_TYPE_STR("freeswitch/api-response"),
                            SIPTAG_PAYLOAD_STR("-ERR INVALID COMMAND"),
                            NUTAG_WITH_THIS_MSG(de->data->e_msg),
                            TAG_IF(!zstr(session_id_header), SIPTAG_HEADER_STR(session_id_header)),
                            TAG_END());
                    }
    ...
    if (session) {
        ...
        if (!zstr(profile->proxy_info_content_types) && sip && sip->sip_content_type &&
            ...
            if (switch_core_session_get_partner(session, &other_session) == SWITCH_STATUS_SUCCESS) {
                ...
                ct =
                    switch_core_session_strdup(other_session, (char*)sip->sip_content_type->c_type);
                if (sip->sip_payload && sip->sip_payload->pl_data) {
                    pl =
                        switch_core_session_strdup(other_session, (char*)sip->sip_payload->pl_data);
                }
        ...
        if (sip && sip->sip_content_type && sip->sip_content_type->c_subtype &&
            sip->sip_content_type->c_type &&
        ...
        if (sip && sip->sip_content_type && sip->sip_content_type->c_type &&
            ...
            if (!strncasecmp(sip->sip_content_type->c_type, "application", 11) &&
                ...
                if ((signal_ptr = switch_stristr("d=", sip->sip_payload->pl_data))) {
                    ...
                    if (*signal_ptr &&
                        (*signal_ptr == '*' || *signal_ptr == '#' || *signal_ptr == 'A' ||
            ...
            if (dtmf.digit) {
                if (tech_pvt->mparams.dtmf_type == DTMF_INFO ||
                    ...
                    if (switch_channel_test_flag(channel, CF_PROXY_MODE)) {
                        ...
                        if ((uuid = switch_channel_get_partner_uuid(channel)) &&
                            ...
                            while (switch_channel_has_dtmf(channel)) {
                                ...
                                if (switch_channel_dequeue_dtmf(channel, &idtmf) ==
                                    SWITCH_STATUS_SUCCESS) {
                    ...
                    nua_respond(
                        nh, SIP_200_OK, NUTAG_WITH_THIS_MSG(de->data->e_msg),
                        TAG_IF(!zstr(session_id_header), SIPTAG_HEADER_STR(session_id_header)),
                        TAG_END());
                } else {
                    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING,
                                      "IGNORE INFO DTMF(%c) (This channel was not configured to "
                                      "use INFO DTMF!)\n",
                                      dtmf.digit);
                }
                goto end;
        ...
        if ((rec_header = sofia_glue_get_unknown_header(sip, "record"))) {
            if (zstr(profile->record_template)) {
                ...
                if (!strcasecmp(rec_header, "on")) {
                    ...
                    if (switch_true(
                            switch_channel_get_variable(channel, "sip_disable_recording"))) {
                        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING,
                                          "Record attempted but is disabled by "
                                          "sip_disable_recording variable.\n");
                        nua_respond(
                            nh, 488, "Recording disabled for this channel",
                            NUTAG_WITH_THIS_MSG(de->data->e_msg),
                            TAG_IF(!zstr(session_id_header), SIPTAG_HEADER_STR(session_id_header)),
                            TAG_END());
                    } else {
                    ...
                    if ((file = switch_channel_get_variable(channel, "sofia_record_file"))) {
                        ...
                        nua_respond(
                            nh, 488, "Nothing to stop", NUTAG_WITH_THIS_MSG(de->data->e_msg),
                            TAG_IF(!zstr(session_id_header), SIPTAG_HEADER_STR(session_id_header)),
                            TAG_END());
                    }
                }
...
void sofia_handle_sip_i_reinvite(switch_core_session_t* session, nua_t* nua,
    ...
    if (session && profile && sip && sofia_test_pflag(profile, PFLAG_TRACK_CALLS)) {
        ...
        switch_snprintf(via_space, sizeof(via_space), "SIP/2.0/UDP %s;rport=%d;branch=%s",
                        network_ip, network_port, branch);
...
switch_status_t sofia_locate_user(char* user, switch_core_session_t* session, sip_t const* sip,
                                  switch_xml_t* x_user) {
...
void sofia_handle_sip_i_invite(switch_core_session_t* session, nua_t* nua, sofia_profile_t* profile,
    ...
    const char *dialog_from_user = "", *dialog_from_host = "", *to_user = "", *to_host = "",
               *contact_user = "", *contact_host = "";
    const char *user_agent = "", *call_id = "";
    url_t *from = NULL, *to = NULL, *contact = NULL;
    const char* to_tag = "";
    ...
    if (sip->sip_via && sip->sip_via->v_protocol &&
        switch_stristr("sip/2.0/ws", sip->sip_via->v_protocol)) {
        is_nat = "websockets";
    }
    ...
    if (!is_nat && profile->nat_acl_count) {
        ...
        if (!zstr(contact_host)) {
            /* NAT mode double check logic and examples.
               ...
               Example 1: the contact_host is 192.168.1.100 and the network_ip is also 192.168.1.100
               the end point is most likely behind nat with us so we need to veto that decision to
               turn on nat processing.

               Example 2: the contact_host is 192.168.1.100 and the network_ip is 192.0.2.100 which
               is a public internet ip the remote endpoint is likely behind a remote nat traversing
               the public internet.

               This secondary check is here to double check the conclusion of nat settigs to ensure
               we don't set net in cases where we don't really need to be doing this.

               Why would you want to do this?  Well if your FreeSWITCH is behind nat and you want to
               talk to endpoints behind remote NAT over the public internet in addition to endpoints
               behind nat with you.  This simplifies that process.
            ...
            */
            ...
            if (contact_private_ip) {
                for (x = 0; x < profile->nat_acl_count; x++) {
                    if ((network_private_ip =
                             switch_check_network_list_ip(network_ip, profile->nat_acl[x]))) {
                        break;
                    }
                }
            }
    ...
    if (profile->acl_count) {
        ...
        for (x = 0; x < profile->acl_count; x++) {
            last_acl = profile->acl[x];
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
                              "verifying acl \"%s\" for ip/port %s:%i.\n", switch_str_nil(last_acl),
                              network_ip, acl_port);
            if ((ok = switch_check_network_list_ip_port_token(network_ip, acl_port, last_acl,
                                                              &token))) {
                if (profile->acl_pass_context[x]) {
                    acl_context = profile->acl_pass_context[x];
                }
                if (!token && profile->acl_inbound_x_token_header) {
                    const char* x_auth_token =
                        sofia_glue_get_unknown_header(sip, profile->acl_inbound_x_token_header);
        ...
        if (ok) {
            ...
            if (network_ip_is_proxy) {
                ...
                if (profile->acl_proxy_x_token_header) {
                    const char* x_auth_token =
                        sofia_glue_get_unknown_header(sip, profile->acl_proxy_x_token_header);
                    if (!zstr(x_auth_token)) {
                        token = x_auth_token;
                ...
                if (!ok && (x_auth_ip = sofia_glue_get_unknown_header(sip, "X-AUTH-IP")) &&
                    !zstr(x_auth_ip)) {
                    for (x = 0; x < profile->acl_count; x++) {
                        last_acl = profile->acl[x];
                        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
                        ...
                        if ((ok = switch_check_network_list_ip_port_token(x_auth_ip, x_auth_port_i,
                                                                          last_acl, &token))) {
                            switch_copy_string(proxied_client_ip, x_auth_ip,
                                               sizeof(proxied_client_ip));
    ...
    if (!is_auth && sofia_test_pflag(profile, PFLAG_AUTH_CALLS) &&
        sofia_test_pflag(profile, PFLAG_BLIND_AUTH)) {
        char* user = NULL;
    ...
    if (!is_auth && (sofia_test_pflag(profile, PFLAG_AUTH_CALLS) ||
                     (!sofia_test_pflag(profile, PFLAG_BLIND_AUTH) &&
                      (sip->sip_proxy_authorization || sip->sip_authorization)))) {
        if (!strcmp(network_ip, profile->sipip) && network_port == profile->sip_port) {
            calling_myself++;
        } else {
            switch_event_create(&v_event, SWITCH_EVENT_REQUEST_PARAMS);

            if (sofia_reg_handle_register(nua, profile, nh, sip, de, REG_INVITE, key, sizeof(key),
                                          &v_event, NULL, NULL, &x_user)) {
                if (v_event) {
                    switch_event_destroy(&v_event);
                }
                if (x_user) {
    ...
    switch_channel_set_variable_printf(channel, "sip_network_ip", "%s", network_ip);
    switch_channel_set_variable_printf(channel, "sip_network_port", "%d", network_port);
    switch_channel_set_variable_printf(channel, "sip_invite_stamp", "%" SWITCH_TIME_T_FMT,
                                       sip_invite_time);
    ...
    if (sip->sip_via) {
        ...
        tech_pvt->record_route = switch_core_session_sprintf(
            session, "sip:%s%s%s:%d;transport=%s", ipv6 ? "[" : "", tech_pvt->mparams.remote_ip,
            ipv6 ? "]" : "", tech_pvt->mparams.remote_port, sofia_glue_transport2str(transport));
    ...
    tech_pvt->caller_profile =
        switch_caller_profile_new(switch_core_session_get_pool(session), NULL, NULL, NULL, NULL,
    ...
    if (sip->sip_from) {
        ...
        if (!zstr(from_user)) {
            if (*from_user == '+') {
                switch_channel_set_variable(channel, "sip_from_user_stripped",
                                            (const char*)(from_user + 1));
            } else {
    ...
    if ((passerted = sip_p_asserted_identity(sip))) {
        if (passerted->paid_url->url_user) {
            char* full_paid_header =
                sip_header_as_string(nua_handle_get_home(nh), (void*)passerted);
            ...
            if (!zstr(full_paid_header)) {
                if (profile->paid_type == PAID_DEFAULT || profile->paid_type == PAID_USER) {
                    ...
                    switch_channel_set_variable(
                        channel, "sip_P-Asserted-Identity",
                        switch_core_session_sprintf(session, "%s@%s", passerted->paid_url->url_user,
    ...
    if ((ppreferred = sip_p_preferred_identity(sip))) {
        if (ppreferred->ppid_url->url_user) {
            char* full_ppid_header =
                sip_header_as_string(nua_handle_get_home(nh), (void*)ppreferred);
    ...
    if (sip->sip_request->rq_url->url_user) {
        ...
        if (profile->parse_invite_tel_params) {
            if (strchr(req_user, ';')) {
                ...
                if ((argc1 = switch_separate_string(req_user, ';', argv1,
                    ...
                    for (x1 = 0; x1 < argc1; x1++) {
                        if (x1 == 0) {
                            ...
                            char* argv2[2] = {0};
                            if (switch_separate_string(argv1[x1], '=', argv2,
    ...
    if (sip->sip_to) {
        ...
        if (switch_channel_get_variable(channel, "sip_to_uri")) {
            ...
            if (sip->sip_contact->m_url->url_port) {
                ...
                port = sofia_glue_transport_has_tls(transport) ? profile->tls_sip_port
                                                               : profile->extsipport;
            }

            ipv6 = strchr(host, ':');
            tech_pvt->to_uri = switch_core_session_sprintf(
                session, "sip:%s@%s%s%s:%d;transport=%s", user, ipv6 ? "[" : "", host,
            ...
            if (url) {
                ...
                tech_pvt->reply_contact = switch_core_session_sprintf(
                    ...
                    proto ? "" : ";transport=", proto ? "" : sofia_glue_transport2str(transport),
                    brackets ? "" : ">");
            } else {
                switch_channel_hangup(tech_pvt->channel, SWITCH_CAUSE_DESTINATION_OUT_OF_ORDER);
            }
    ...
    if (sip->sip_multipart) {
        ...
        for (mp = sip->sip_multipart; mp; mp = mp->mp_next) {
            if (mp->mp_payload && mp->mp_payload->pl_data && mp->mp_content_type &&
                mp->mp_content_type->c_type) {
                char* val = switch_core_session_sprintf(
                    session, "%s:%s", mp->mp_content_type->c_type, mp->mp_payload->pl_data);
    ...
    if ((call_info = sip_call_info(sip))) {
        ...
        while (call_info) {
            call_info_str = sip_header_as_string(nua_handle_get_home(nh), (void*)call_info);
            switch_channel_add_variable_var_check(channel, "sip_call_info", call_info_str,
                                                  SWITCH_FALSE, SWITCH_STACK_PUSH);
            call_info = call_info->ci_next;
        }
        ...
        if (sip->sip_from && sip->sip_from->a_url->url_user && sip->sip_from->a_url->url_host &&
            ...
            if (!zstr(buf)) {
                ...
                switch_channel_set_variable_printf(channel, "presence_call_info_full",
                                                   "<sip:%s>;%s", sip->sip_from->a_url->url_host,
                                                   buf);
                switch_channel_set_variable(channel, "presence_call_info", buf);
                call_info_str = switch_core_session_sprintf(session, "<sip:%s>;%s",
                                                            sip->sip_from->a_url->url_host, buf);
            }
        }
    }
    ...
    if (call_info_str) {
        ...
        if (user && host && from_user && !strcmp(user, from_user)) {
            ...
            sql = switch_mprintf(
                "select call_id from sip_dialogs where (call_info='%q' or "
                "call_info='%q;appearance-state=held') and "
                "((sip_from_user='%q' and sip_from_host='%q') or presence_id='%q@%q') and call_id "
                "is not null",
                switch_str_nil(p), switch_str_nil(p), user, host, user, host);

            if ((str = sofia_glue_execute_sql2str(profile, profile->dbh_mutex, sql, cid,
                                                  sizeof(cid)))) {
    ...
    profile_dup_clean(from_user, tech_pvt->caller_profile->username,
                      tech_pvt->caller_profile->pool);
    profile_dup_clean(dialplan, tech_pvt->caller_profile->dialplan, tech_pvt->caller_profile->pool);
    profile_dup_clean(displayname, tech_pvt->caller_profile->caller_id_name,
                      tech_pvt->caller_profile->pool);
    profile_dup_clean(from_user, tech_pvt->caller_profile->caller_id_number,
                      tech_pvt->caller_profile->pool);
    profile_dup_clean(displayname, tech_pvt->caller_profile->orig_caller_id_name,
                      tech_pvt->caller_profile->pool);
    profile_dup_clean(from_user, tech_pvt->caller_profile->orig_caller_id_number,
                      tech_pvt->caller_profile->pool);
    profile_dup_clean(network_ip, tech_pvt->caller_profile->network_addr,
                      tech_pvt->caller_profile->pool);
    ...
    profile_dup_clean(destination_number, tech_pvt->caller_profile->destination_number,
                      tech_pvt->caller_profile->pool);
    ...
    if (sip->sip_replaces) {
        ...
        if (rp_common->h_class->hc_params) {
            ...
            msg_param_t const* params =
                *(msg_param_t const**)((char*)rp_common + rp_common->h_class->hc_params);
            for (i = 0; params && params[i]; i++) {
                msg_param_t param = params[i];
                if (strchr(param, '=')) {
                    n = strcspn(param, "=");
                    switch_channel_set_variable_name_printf(channel, param + n + 1,
                                                            "sip_replaces_%.*s", n, param);
                } else {
                    switch_channel_set_variable_name_printf(channel, "true", "sip_replaces_%s",
                                                            param);
                }
            }
        }
    ...
    if (bnh) {
        ...
        if ((b_private = nua_handle_magic(bnh))) {
            ...
            if ((b_session = switch_core_session_locate(b_private->uuid))) {
                switch_channel_t* b_channel = switch_core_session_get_channel(b_session);
                sofia_handle_sip_i_invite_replaces(session, channel, b_channel, b_private->uuid,
        ...
        if ((b_session = switch_core_session_locate((char*)sip->sip_replaces->rp_call_id))) {
            switch_channel_t* b_channel = switch_core_session_get_channel(b_session);
            sofia_handle_sip_i_invite_replaces(session, channel, b_channel,
                                               (char*)sip->sip_replaces->rp_call_id, tech_pvt,
                                               call_info, profile, is_nat, sip);
            switch_core_session_rwunlock(b_session);
        }
    }
    ...
    if (tech_pvt->caller_profile) {
        ...
        if (rpid) {
            if (rpid->rpid_privacy) {
                if (!strcasecmp(rpid->rpid_privacy, "yes")) {
                    switch_set_flag(tech_pvt->caller_profile,
                                    SWITCH_CPF_HIDE_NAME | SWITCH_CPF_HIDE_NUMBER);
        ...
        if (sip->sip_identity) {
            ...
            for (id = sip->sip_identity; id; id = id->id_next) {
                if (!zstr(id->id_value)) {
                    switch_channel_add_variable_var_check(channel, "sip_identity", id->id_value,
                                                          SWITCH_FALSE, SWITCH_STACK_PUSH);
                }
            }
        }
        ...
        if (sip->sip_date && sip->sip_date->d_time > 0) {
            ...
            // sofia-sip stores the Date header value in sip_date->d_time as seconds since January
            // 1, 1900 0:00:00. Unix epoch time is seconds since January 1, 1970 0:00:00, making
            // d_time larger by 2208988800. Convert to Unix epoch time and save it.
        ...
        for (un = sip->sip_unknown; un; un = un->un_next) {
            if (!strncasecmp(un->un_name, "Accept-Language", 15)) {
                ...
                if (first_history_info) {
                    /* If the header exists first time, make sure to remove old info and re-set the
                     * variable */
                    ...
                    const char* history_var =
                        switch_channel_get_variable(channel, "sip_history_info");
                    if (!zstr(history_var)) {
                        char* tmp_str;
                ...
                tech_pvt->x_freeswitch_support_remote =
                    switch_core_session_strdup(session, un->un_value);
                ...
                switch_channel_set_variable(channel, "sip_user_location", un->un_value);
                       ...
                       !strcasecmp(un->un_name, "User-to-User") ||
                       !strncasecmp(un->un_name, "On", 2) || !strncasecmp(un->un_name, "K-", 2)) {
                if (!zstr(un->un_value)) {
                    char new_name[512] = "";
                    int reps = 0;
                    for (;;) {
                        ...
                        reps++;
                        switch_snprintf(new_name, sizeof(new_name), "%s%s%s",
                                        SOFIA_SIP_HEADER_PREFIX, un->un_name, postfix);
    ...
    if (profile->pres_type) {
        ...
        sql = switch_mprintf(
            "insert into sip_dialogs "
            "(call_id,uuid,sip_to_user,sip_to_host,sip_to_tag,sip_from_user,sip_from_host,sip_from_"
            "tag,contact_user,"
            "contact_host,state,direction,user_agent,profile_name,hostname,contact,presence_id,"
            "presence_data,"
            "call_info,rcd,call_info_state) "
            "values('%q','%q','%q','%q','%q','%q','%q','%q','%q','%q','%q','%q','%q','%q','%q','%q'"
            ",'%q','%q','%q',%ld,'')",
            call_id, tech_pvt->sofia_private->uuid, to_user, to_host, to_tag, dialog_from_user,
            dialog_from_host, from_tag, contact_user, contact_host, "confirmed", "inbound",
            user_agent, profile->name, mod_sofia_globals.hostname, switch_str_nil(full_contact),
...
void sofia_handle_sip_i_invite_replaces(switch_core_session_t* session, switch_channel_t* channel,
                                        switch_channel_t* b_channel, char* uuid,
                                        private_object_t* tech_pvt, sip_call_info_t* call_info,
                                        sofia_profile_t* profile, char* is_nat, sip_t const* sip) {
    ...
    if (call_info) {
        ...
        if (switch_channel_var_true(channel, "sip_replaces_a-leg")) {
            ...
            if (sip) {
                tech_pvt->caller_profile->destination_number =
                    switch_core_sprintf(tech_pvt->caller_profile->pool, "answer,intercept:%s",
                ...
                if (sofia_test_pflag(profile, PFLAG_FIRE_TRANFER_EVENTS) && sip->sip_call_id &&
                    switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM,
                                                 MY_EVENT_INTERCEPTED) == SWITCH_STATUS_SUCCESS) {
                    switch_channel_event_set_data(b_channel, event);
            ...
            if (sip) {
                if ((nightmare_xfer_uuid = sofia_glue_get_unknown_header(sip, "X-FS-Refer-For"))) {
                    switch_channel_set_variable(b_channel, "transfer_refer_for",
                                                nightmare_xfer_uuid);
                }
                if ((nightmare_xfer_uuid = sofia_glue_get_unknown_header(sip, "X-FS-Refer-From"))) {
                    switch_channel_set_variable(b_channel, "transfer_refer_from",
                                                nightmare_xfer_uuid);
                }
            }

            if (!zstr(bridge_uuid)) {
                if (sip && sip->sip_replaces && sip->sip_replaces->rp_params &&
                    sip->sip_replaces->rp_call_id &&
                    ...
                    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                                      "call %s intercepted\n", bridge_uuid);
                    if (sofia_test_pflag(profile, PFLAG_FIRE_TRANFER_EVENTS) && sip &&
                        sip->sip_call_id &&
                        switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM,
                ...
                const char* b_app =
                    switch_channel_get_variable(b_channel, SWITCH_CURRENT_APPLICATION_VARIABLE);
                const char* b_data = switch_channel_get_variable(
                    b_channel, SWITCH_CURRENT_APPLICATION_DATA_VARIABLE);
                if (b_data && b_app) {
                    tech_pvt->caller_profile->destination_number = switch_core_sprintf(
                ...
                if (sofia_test_pflag(profile, PFLAG_FIRE_TRANFER_EVENTS) && sip &&
                    ...
                    switch_channel_event_set_data(b_channel, event);
                    switch_event_add_header_string(event, SWITCH_STACK_BOTTOM,
                                                   "att_xfer_replaced_by", sip->sip_call_id->i_id);
                    switch_event_fire(&event);
                }
...
void sofia_handle_sip_i_options(int status, char const* phrase, nua_t* nua,
                                sofia_profile_t* profile, nua_handle_t* nh,
                                sofia_private_t* sofia_private, sip_t const* sip,
                                sofia_dispatch_event_t* de, tagi_t tags[]) {
    uint32_t sess_count = switch_core_session_count();
    ...
    if (sofia_test_pflag(profile, PFLAG_OPTIONS_RESPOND_503_ON_BUSY) &&
        (sess_count >= sess_max || !sofia_test_pflag(profile, PFLAG_RUNNING) ||
         !switch_core_ready_inbound())) {
        ...
        switch_assert(sip);
        nua_respond(nh, SIP_200_OK, NUTAG_WITH_THIS_MSG(de->data->e_msg),
                    TAG_IF(sip->sip_record_route, SIPTAG_RECORD_ROUTE(sip->sip_record_route)),
                    TAG_END());
    }
}
 ...
 *
 * If no equal (=) sign is found during parsing, a channel variable name is created with the param
