...
#define SOFIA_SIP_INFO_HEADER_PREFIX "sip_info_h_"  // Added by Raymond Chandler <
...
typedef enum { SOFIA_CONFIG_LOAD = 0, SOFIA_CONFIG_RESCAN, SOFIA_CONFIG_RESPAWN } sofia_config_t;
...
#define set_anchor(t, m)  \
    if (t->Anchor) {      \
        delete t->Anchor; \
    }                     \
    t->Anchor = new SipMessage(m);
#define sofia_private_free(_pvt)    \
    if (_pvt && !_pvt->is_static) { \
        free(_pvt);                 \
    }                               \
    _pvt = NULL;
...
typedef enum { MFLAG_REFER = (1 << 0), MFLAG_REGISTER = (1 << 1) } MFLAGS;
...
typedef enum { CID_TYPE_RPID, CID_TYPE_PID, CID_TYPE_NONE } sofia_cid_type_t;
...
struct sofia_gateway {
    ...
    char* register_proxy_host_cfg; /* hold only the IP or the hostname, no port, no "sip:" or
                                      "sips:" prefix */
...
typedef enum { PAID_DEFAULT = 0, PAID_USER, PAID_USER_DOMAIN, PAID_VERBATIM } sofia_paid_type_t;

#define MAX_RTPIP 50

typedef enum { KA_MESSAGE, KA_INFO } ka_type_t;
...
#define sofia_set_pflag_locked(obj, flag) \
    assert(obj->flag_mutex != NULL);      \
    switch_mutex_lock(obj->flag_mutex);   \
    (obj)->pflags[flag] = 1;              \
    switch_mutex_unlock(obj->flag_mutex);
#define sofia_clear_pflag_locked(obj, flag) \
    switch_mutex_lock(obj->flag_mutex);     \
    (obj)->pflags[flag] = 0;                \
    switch_mutex_unlock(obj->flag_mutex);
#define sofia_clear_pflag(obj, flag) (obj)->pflags[flag] = 0

#define sofia_set_flag_locked(obj, flag) \
    assert(obj->flag_mutex != NULL);     \
    switch_mutex_lock(obj->flag_mutex);  \
    (obj)->flags[flag] = 1;              \
    switch_mutex_unlock(obj->flag_mutex);
...
#define sofia_clear_flag_locked(obj, flag) \
    switch_mutex_lock(obj->flag_mutex);    \
    (obj)->flags[flag] = 0;                \
    switch_mutex_unlock(obj->flag_mutex);
...
void sofia_glue_attach_private(switch_core_session_t* session, sofia_profile_t* profile,
                               private_object_t* tech_pvt, const char* channame);

switch_status_t sofia_glue_do_invite(switch_core_session_t* session);

uint8_t sofia_media_negotiate_sdp(switch_core_session_t* session, const char* r_sdp,
                                  switch_sdp_type_t type);
...
void sofia_glue_store_session_id(switch_core_session_t* session, sofia_profile_t* profile,
                                 sip_t const* sip, switch_bool_t is_reply);
char* sofia_glue_session_id_header(switch_core_session_t* session, sofia_profile_t* profile);

/*
 * \brief Sets the "ep_codec_string" channel variable, parsing r_sdp and taing codec_string in
 * consideration
 * \param channel Current channel
 * \param codec_string The profile's codec string or NULL if inexistant
 * \param sdp The parsed SDP content
 */
void sofia_media_set_r_sdp_codec_string(switch_core_session_t* session, const char* codec_string,
                                        sdp_session_t* sdp);
switch_status_t sofia_media_tech_media(private_object_t* tech_pvt, const char* r_sdp,
                                       switch_sdp_type_t type);
char* sofia_reg_find_reg_url(sofia_profile_t* profile, const char* user, const char* host,
                             char* val, switch_size_t len);
...
void sofia_reg_auth_challenge(sofia_profile_t* profile, nua_handle_t* nh,
                              sofia_dispatch_event_t* de, sofia_regtype_t regtype,
                              const char* realm, int stale, long exptime);
auth_res_t sofia_reg_parse_auth(sofia_profile_t* profile, sip_authorization_t const* authorization,
                                sip_t const* sip, sofia_dispatch_event_t* de, const char* regstr,
                                char* np, size_t nplen, char* ip, int network_port,
                                switch_event_t** v_event, long exptime, sofia_regtype_t regtype,
                                const char* to_user, switch_event_t** auth_params, long* reg_count,
                                switch_xml_t* user_xml);

void sofia_reg_handle_sip_r_challenge(int status, char const* phrase, nua_t* nua,
                                      sofia_profile_t* profile, nua_handle_t* nh,
                                      sofia_private_t* sofia_private,
                                      switch_core_session_t* session, sofia_gateway_t* gateway,
                                      sip_t const* sip, sofia_dispatch_event_t* de, tagi_t tags[]);
void sofia_reg_handle_sip_r_register(int status, char const* phrase, nua_t* nua,
                                     sofia_profile_t* profile, nua_handle_t* nh,
                                     sofia_private_t* sofia_private, sip_t const* sip,
                                     sofia_dispatch_event_t* de, tagi_t tags[]);
...
void sofia_glue_execute_sql(sofia_profile_t* profile, char** sqlp,
                            switch_bool_t sql_already_dynamic);
...
void sofia_glue_actually_execute_sql_trans(sofia_profile_t* profile, char* sql,
                                           switch_mutex_t* mutex);
void sofia_glue_execute_sql_now(sofia_profile_t* profile, char** sqlp,
                                switch_bool_t sql_already_dynamic);
void sofia_glue_execute_sql_soon(sofia_profile_t* profile, char** sqlp,
                                 switch_bool_t sql_already_dynamic);
...
uint8_t sofia_reg_handle_register_token(nua_t* nua, sofia_profile_t* profile, nua_handle_t* nh,
                                        sip_t const* sip, sofia_dispatch_event_t* de,
                                        sofia_regtype_t regtype, char* key, uint32_t keylen,
                                        switch_event_t** v_event, const char* is_nat,
                                        sofia_private_t** sofia_private_p, switch_xml_t* user_xml,
                                        const char* sw_acl_token);
#define sofia_reg_handle_register(_nua_, _profile_, _nh_, _sip_, _de_, _regtype_, _key_, _keylen_, \
                                  _v_event_, _is_nat_, _sofia_private_p_, _user_xml_)              \
    sofia_reg_handle_register_token(_nua_, _profile_, _nh_, _sip_, _de_, _regtype_, _key_,         \
                                    _keylen_, _v_event_, _is_nat_, _sofia_private_p_, _user_xml_,  \
                                    NULL)
...
char* sofia_glue_get_profile_url(sofia_profile_t* profile, char* remote_ip,
                                 const sofia_transport_t transport);
...
switch_bool_t sofia_glue_execute_sql_callback(sofia_profile_t* profile, switch_mutex_t* mutex,
                                              char* sql, switch_core_db_callback_func_t callback,
                                              void* pdata);
char* sofia_glue_execute_sql2str(sofia_profile_t* profile, switch_mutex_t* mutex, char* sql,
                                 char* resbuf, size_t len);
...
void sofia_glue_release_profile__(const char* file, const char* func, int line,
                                  sofia_profile_t* profile);
#define sofia_glue_release_profile(x) \
    sofia_glue_release_profile__(__FILE__, __SWITCH_FUNC__, __LINE__, x)

sofia_profile_t* sofia_glue_find_profile__(const char* file, const char* func, int line,
                                           const char* key);
#define sofia_glue_find_profile(x) sofia_glue_find_profile__(__FILE__, __SWITCH_FUNC__, __LINE__, x)

#define sofia_glue_profile_rdlock(x) \
    sofia_glue_profile_rdlock__(__FILE__, __SWITCH_FUNC__, __LINE__, x)
switch_status_t sofia_glue_profile_rdlock__(const char* file, const char* func, int line,
                                            sofia_profile_t* profile);

switch_status_t sofia_reg_add_gateway(sofia_profile_t* profile, const char* key,
                                      sofia_gateway_t* gateway);
sofia_gateway_t* sofia_reg_find_gateway__(const char* file, const char* func, int line,
                                          const char* key);
#define sofia_reg_find_gateway(x) sofia_reg_find_gateway__(__FILE__, __SWITCH_FUNC__, __LINE__, x)

sofia_gateway_t* sofia_reg_find_gateway_by_realm__(const char* file, const char* func, int line,
                                                   const char* key);
#define sofia_reg_find_gateway_by_realm(x) \
    sofia_reg_find_gateway_by_realm__(__FILE__, __SWITCH_FUNC__, __LINE__, x)

sofia_gateway_subscription_t* sofia_find_gateway_subscription(sofia_gateway_t* gateway_ptr,
                                                              const char* event);

#define sofia_reg_gateway_rdlock(x) \
    sofia_reg_gateway_rdlock__(__FILE__, __SWITCH_FUNC__, __LINE__, x)
switch_status_t sofia_reg_gateway_rdlock__(const char* file, const char* func, int line,
                                           sofia_gateway_t* gateway);

void sofia_reg_release_gateway__(const char* file, const char* func, int line,
                                 sofia_gateway_t* gateway);
#define sofia_reg_release_gateway(x) \
    sofia_reg_release_gateway__(__FILE__, __SWITCH_FUNC__, __LINE__, x);
...
#define sofia_test_extra_headers(val)                                 \
    (((!strncasecmp(val, "X-", 2) && strncasecmp(val, "X-FS-", 5)) || \
      !strncasecmp(val, "P-", 2) || !strncasecmp(val, "On", 2))       \
         ? 1                                                          \
         : 0)

#define check_decode(_var, _session)                                        \
    do {                                                                    \
        assert(_session);                                                   \
        if (!zstr(_var)) {                                                  \
            int d = 0;                                                      \
...
char* sofia_glue_find_parameter_value(switch_core_session_t* session, const char* str,
                                      const char* param);
...
switch_status_t sofia_media_build_crypto(private_object_t* tech_pvt, int index,
...
void sofia_reg_send_reboot(sofia_profile_t* profile, const char* callid, const char* user,
                           const char* host, const char* contact, const char* user_agent,
 ...
 * \note Valid components are "all", "default" (sofia's default logger), "tport", "iptsec", "nea",
 * "nta", "nth_client", "nth_server", "nua", "soa", "sresolv", "stun"
 * \return SWITCH_STATUS_SUCCESS or SWITCH_STATUS_FALSE if the component isnt valid, or the level is
 * out of range
 */
 ...
 * \note Valid components are "default" (sofia's default logger), "tport", "iptsec", "nea", "nta",
 * "nth_client", "nth_server", "nua", "soa", "sresolv", "stun"
...
switch_status_t sofia_glue_send_notify(sofia_profile_t* profile, const char* user, const char* host,
                                       const char* event, const char* contenttype, const char* body,
                                       const char* o_contact, const char* network_ip,
                                       const char* call_id);
char* sofia_glue_get_extra_headers(switch_channel_t* channel, const char* prefix);
void sofia_glue_set_extra_headers(switch_core_session_t* session, sip_t const* sip,
                                  const char* prefix);
...
switch_status_t sip_dig_function(_In_opt_z_ const char* cmd,
...
void sofia_reg_fire_custom_gateway_state_event(sofia_gateway_t* gateway, int status,
                                               const char* phrase);
const char* sofia_sip_user_status_name(sofia_sip_user_status_t status);
void sofia_reg_fire_custom_sip_user_state_event(sofia_profile_t* profile, const char* sip_user,
                                                const char* contact, const char* from_user,
                                                const char* from_host, const char* call_id,
                                                sofia_sip_user_status_t status, int options_res,
                                                const char* phrase);
uint32_t sofia_reg_reg_count(sofia_profile_t* profile, const char* user, const char* host);
char* sofia_media_get_multipart(switch_core_session_t* session, const char* prefix, const char* sdp,
                                char** mp_type);
int sofia_glue_tech_simplify(private_object_t* tech_pvt);
switch_console_callback_match_t* sofia_reg_find_reg_url_multi(sofia_profile_t* profile,
                                                              const char* user, const char* host);
switch_console_callback_match_t* sofia_reg_find_reg_url_with_positive_expires_multi(
    sofia_profile_t* profile, const char* user, const char* host, time_t reg_time,
    const char* contact_str, long exptime);
...
char* sofia_glue_get_encoded_fs_path(nua_handle_t* nh, sip_route_t* rt,
                                     switch_bool_t add_fs_path_prefix);
...
switch_status_t sofia_glue_ext_address_lookup(sofia_profile_t* profile, char** ip,
                                              switch_port_t* port, const char* sourceip,
                                              switch_memory_pool_t* pool);
...
void write_csta_xml_chunk(switch_event_t* event, switch_stream_handle_t stream,
                          const char* csta_event, char* fwd_type);
