...
static int broadsoft_sla_gather_state_callback(void* pArg, int argc, char** argv,
                                               char** columnNames);
static int broadsoft_sla_notify_callback(void* pArg, int argc, char** argv, char** columnNames);
static int sync_sla(sofia_profile_t* profile, const char* to_user, const char* to_host,
                    switch_bool_t clear, switch_bool_t unseize, const char* call_id);
static int sofia_dialog_probe_callback(void* pArg, int argc, char** argv, char** columnNames);
static int sofia_dialog_probe_notify_callback(void* pArg, int argc, char** argv,
                                              char** columnNames);
...

{
    ...
    if (!prof || !(profile = sofia_glue_find_profile(prof))) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
                          "Chat proto [%s]\nfrom [%s]\nto [%s]\n%s\nInvalid Profile %s\n", proto,
                          from, to, body ? body : "[no body]", prof ? prof : "NULL");
        goto end;
    }
    ...
    if (to_uri) {
        ...
        if ((test = sofia_glue_find_profile(host))) {
            ...
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
                              "Not sending to local box for %s@%s\n", user, host);
    ...
    if (!strcasecmp(proto, SOFIA_CHAT_PROTO)) {
        ...
        if (switch_stristr("global", proto)) {
            ...
            ffrom =
                switch_mprintf("\"%s\" <sip:%s+%s@%s>", fp, from_proto ? from_proto : proto, fp, p);
        }
    ...
    if (!list) {
        ...
        if (switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM, MY_EVENT_ERROR) ==
            ...
            switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "Error-Reason",
                                           "no recipient");
    ...
    for (m = list->head; m; m = m->next) {
        ...
        // contact, dst->route_uri, network_ip, network_port);

        // DUMP_EVENT(message_event);

        if (zstr(dst->route_uri) && !zstr(user) && !zstr(network_ip) &&
            (zstr(host) || strcmp(network_ip, host))) {
            route_uri = switch_mprintf("sip:%s@%s:%s", user, network_ip, network_port);
        }

        msg_nh = nua_handle(profile->nua, NULL, TAG_END());
        ...
        nua_message(msg_nh, TAG_IF(dst->route_uri, NUTAG_PROXY(dst->route_uri)),
                    TAG_IF(route_uri, NUTAG_PROXY(route_uri)),
                    TAG_IF(dst->route, SIPTAG_ROUTE_STR(dst->route)), SIPTAG_FROM_STR(from),
                    TAG_IF(contact, NUTAG_URL(contact)), SIPTAG_TO_STR(dup_dest),
                    SIPTAG_CALL_ID_STR(uuid_str), TAG_IF(user_via, SIPTAG_VIA_STR(user_via)),
        ...
        if (is_blocking) {
            ...
            switch_event_add_header(message_event, SWITCH_STACK_BOTTOM, "Delivery-Result-Code",
                                    "%d", mstatus);
        ...
        for (m = matches->head; m; m = m->next) {
            if ((profile = sofia_glue_find_profile(m->val))) {
                if (profile->pres_type == PRES_TYPE_FULL) {
                    ...
                    sql = switch_mprintf(
                        "select "
                        "proto,sip_user,sip_host,sub_to_user,sub_to_host,event,contact,call_id,"
                        "full_from,"
                    ...
                    r = sofia_glue_execute_sql_callback(profile, profile->dbh_mutex, sql,
                                                        sofia_presence_sub_callback, &helper);
...
static void actual_sofia_presence_mwi_event_handler(switch_event_t* event) {
    ...
    if (!(account = switch_event_get_header(event, "mwi-message-account"))) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
                          "Missing required Header 'MWI-Message-Account'\n");
        return;
    }

    if (!switch_event_get_header(event, "mwi-messages-waiting")) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
                          "Missing required Header 'MWI-Messages-Waiting'\n");
        return;
    }
    ...
    if (!profile) {
        if (!(profile = sofia_glue_find_profile(host))) {
            ...
            sql = switch_mprintf(
                "select profile_name from sip_registrations where hostname='%q' and (sip_host='%q' "
                "or mwi_host='%q')",
            ...
            if (list_profiles_full(NULL, NULL, &matches, SWITCH_FALSE) == SWITCH_STATUS_SUCCESS) {
                ...
                for (m = matches->head; m; m = m->next) {
                    if ((profile = sofia_glue_find_profile(m->val))) {
                        sofia_glue_execute_sql2str(profile, profile->dbh_mutex, sql, buf,
                                                   sizeof(buf));
                        if (!zstr(buf)) {
                            break;
                        }
    ...
    if (for_everyone) {
        sql = switch_mprintf(
            "select "
            "proto,sip_user,sip_host,sub_to_user,sub_to_host,event,contact,call_id,full_from,"
            "full_via,expires,user_agent,accept,profile_name,network_ip"
        ...
        sql = switch_mprintf(
            ...
            "and sub_to_user='%q' and (sub_to_host='%q' or presence_hosts like '%%%q%%') and "
            "call_id='%q'",
    ...
    if (for_everyone) {
        sql = switch_mprintf(
            "select sip_user,sip_host,contact,profile_name,network_ip,'%q',call_id "
    ...
    if (sql) {
        ...
        sofia_glue_execute_sql_callback(profile, profile->dbh_mutex, sql,
                                        sofia_presence_mwi_callback2, &h);
        free(sql);
        sql = NULL;
    }
    ...
    if (argc >= 4) {
        if (argc == 5 && !zstr(argv[4])) {
            if ((session = switch_core_session_locate(argv[4]))) {
                ...
                if (!switch_channel_test_flag(channel, CF_ANSWERED) &&
                    switch_true(switch_channel_get_variable_dup(channel, "presence_disable_early",
                                                                SWITCH_FALSE, -1))) {
                    done++;
                }
        ...
        if (mod_sofia_globals.debug_presence > 0) {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_CRIT,
                              "CHECK DIALOG state[%s] status[%s] rpid[%s] pres[%s] uuid[%s]\n",
...
static void do_normal_probe(switch_event_t* event) {
    ...
    if (probe_host && ((profile = sofia_glue_find_profile(probe_host)) ||
                       (profile_name && (profile = sofia_glue_find_profile(profile_name))))) {
        sql = switch_mprintf(
            ...
            "((sip_from_user='%q' and sip_from_host='%q') or presence_id='%q@%q') order by rcd "
            "desc",
            mod_sofia_globals.hostname, profile->name, probe_euser, probe_host, probe_euser,
            probe_host);

        sofia_glue_execute_sql_callback(profile, profile->dbh_mutex, sql,
                                        sofia_presence_dialog_callback, &dh);
        ...
        sql = switch_mprintf(
            ...
            "sip_dialogs.hostname = sip_registrations.hostname and sip_dialogs.profile_name = "
            "sip_registrations.profile_name and ("
            ...
            "sip_registrations.orig_server_host=sip_presence.sip_host and "
            "sip_registrations.profile_name=sip_presence.profile_name) "
            "where sip_registrations.hostname='%q' and sip_registrations.profile_name='%q' and "
            "sip_dialogs.call_info_state != 'seized' "
            ...
            "))",
            dh.status, dh.rpid, switch_str_nil(sub_call_id), switch_sql_concat(),
            switch_sql_concat(), mod_sofia_globals.hostname, profile->name, probe_euser, probe_host,
            probe_euser, probe_host, probe_host);
        ...
        if (mod_sofia_globals.debug_presence > 1) {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
                              "%s DUMP PRESENCE_PROBE_SQL:\n%s\n", profile->name, sql);
        }

        sofia_glue_execute_sql_callback(profile, profile->dbh_mutex, sql,
                                        sofia_presence_resub_callback, &h);
        switch_safe_free(sql);
        ...
        if (!h.rowcount) {
            ...
            sql = switch_mprintf(
                ...
                mod_sofia_globals.hostname, profile->name, dh.status, dh.rpid, probe_euser,
        ...
        if (!h.rowcount) {
            ...
            if (switch_event_create(&sevent, SWITCH_EVENT_PRESENCE_IN) == SWITCH_STATUS_SUCCESS) {
                switch_event_add_header_string(sevent, SWITCH_STACK_BOTTOM, "proto",
                                               SOFIA_CHAT_PROTO);
                switch_event_add_header_string(sevent, SWITCH_STACK_BOTTOM, "login", profile->name);
                switch_event_add_header_string(sevent, SWITCH_STACK_BOTTOM, "sip_profile",
                                               profile->name);
                switch_event_add_header(sevent, SWITCH_STACK_BOTTOM, "from", "%s@%s", probe_euser,
                                        probe_host);
                switch_event_add_header_string(sevent, SWITCH_STACK_BOTTOM, "status",
                                               "Unregistered");
...
static void do_dialog_probe(switch_event_t* event) {
    ...
    if (probe_host) {
        ...
        if (!profile) {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
                              "Cannot find profile for domain %s\n", probe_host);
            goto end;
        }
        ...
        sql = switch_mprintf(
            ...
            "(sip_dialogs.sip_from_user = sip_registrations.sip_user and sip_dialogs.sip_from_host "
            "= '%q' and "
            ...
            "sip_dialogs.call_info_state != 'seized' and sip_dialogs.presence_id='%q@%q' or "
            "(sip_registrations.sip_user='%q' and "
            "(sip_registrations.orig_server_host='%q' or sip_registrations.sub_host='%q' "
            "or sip_registrations.presence_hosts like '%%%q%%'))",
            probe_euser, probe_host, sub_call_id, probe_host, mod_sofia_globals.hostname,
            profile->name, probe_euser, probe_host, probe_euser, probe_host, probe_host,
            probe_host);
        switch_assert(sql);

        if (mod_sofia_globals.debug_presence > 0) {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO,
                              "%s START DIALOG_PROBE_SQL %s@%s\n", profile->name, probe_euser,
                              probe_host);
        }

        if (mod_sofia_globals.debug_presence > 1) {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
                              "%s DUMP DIALOG_PROBE_SQL:\n%s\n", profile->name, sql);
        }
        ...
        sofia_glue_execute_sql_callback(profile, profile->dbh_mutex, sql,
                                        sofia_dialog_probe_callback, h4235);
        switch_safe_free(sql);
        if (mod_sofia_globals.debug_presence > 0) {
        ...
        sql = switch_mprintf("update sip_subscriptions set version=version+1 where call_id='%q'",
                             sub_call_id);
        ...
        sql = switch_mprintf(
            ...
            "where hostname='%q' and profile_name='%q' and sub_to_user='%q' and sub_to_host='%q' "
            "and call_id='%q'",
            mod_sofia_globals.hostname, profile->name, probe_euser, probe_host, sub_call_id);

        if (mod_sofia_globals.debug_presence > 1) {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
                              "%s DUMP DIALOG_PROBE subscription sql:\n%s\n", profile->name, sql);
        }

        sofia_glue_execute_sql_callback(profile, profile->dbh_mutex, sql,
                                        sofia_dialog_probe_notify_callback, h4235);
        switch_safe_free(sql);
...
static void send_conference_data(sofia_profile_t* profile, switch_event_t* event) {
    ...
    if (switch_true(notfound)) {
        sql = switch_mprintf(
            "update sip_subscriptions set expires=%ld where "
            "hostname='%q' and profile_name='%q' and sub_to_user='%q' and sub_to_host='%q' and "
            "event='%q'",
            (long)switch_epoch_time_now(NULL), mod_sofia_globals.hostname, profile->name, from_user,
            from_host, event_str);
    ...
    if (call_id) {
        if (switch_true(final)) {
            sql = switch_mprintf(
                ...
                "event='%q' "
                "and call_id = '%q' ",
                (long)0, mod_sofia_globals.hostname, profile->name, from_user, from_host, event_str,
                call_id);
        ...
        sql = switch_mprintf(
            "select full_to, full_from, contact %q ';_;isfocus', expires, call_id, event, "
            ...
            "event='%q' "
            "and call_id = '%q' ",
            switch_sql_concat(), type, switch_str_nil(body), mod_sofia_globals.hostname,
            profile->name, from_user, from_host, event_str, call_id);
    } else {
        if (switch_true(final)) {
            sql = switch_mprintf(
                "update sip_subscriptions set expires=%ld where "
                "hostname='%q' and profile_name='%q' and sub_to_user='%q' and sub_to_host='%q' and "
                "event='%q'",
                (long)0, mod_sofia_globals.hostname, profile->name, from_user, from_host,
                event_str);
        ...
        sql = switch_mprintf(
            ...
            "event='%q'",
            switch_sql_concat(), type, switch_str_nil(body), mod_sofia_globals.hostname,
            profile->name, from_user, from_host, event_str);
    }
    ...
    if (switch_true(final)) {
        if (call_id) {
            sql = switch_mprintf(
                ...
                "event='%q' "
                "and call_id = '%q' ",
                mod_sofia_globals.hostname, profile->name, from_user, from_host, event_str,
                call_id);
...
static switch_event_t* actual_sofia_presence_event_handler(switch_event_t* event) {
    ...
    if ((status && switch_stristr("CS_HANGUP", status)) ||
        (!zstr(uuid) && !switch_ivr_uuid_exists(uuid))) {
        status = "Available";
        hup = 1;
    }
                    ...
                    if (profile->pres_type != PRES_TYPE_FULL) {
                        ...
                        if (from) {
                            sql = switch_mprintf(
                                ...
                                "hostname='%q' and profile_name='%q' and "
                                "sip_subscriptions.event='presence' and "
                                "sip_subscriptions.full_from like '%%%q%%'",
                            ...
                            sql = switch_mprintf(
                                "select "
                                "sip_subscriptions.proto,sip_subscriptions.sip_user,sip_"
                                "subscriptions.sip_host,"
                                "sip_subscriptions.sub_to_user,sip_subscriptions.sub_to_host,sip_"
                                "subscriptions.event,"
                                "sip_subscriptions.contact,sip_subscriptions.call_id,sip_"
                                "subscriptions.full_from,"
                                "sip_subscriptions.full_via,sip_subscriptions.expires,sip_"
                                "subscriptions.user_agent,"
                                "sip_subscriptions.accept,sip_subscriptions.profile_name,sip_"
                                "subscriptions.network_ip"
                                ",1,'%q','%q',sip_presence.status,sip_presence.rpid,sip_presence."
                                "open_closed,'','','','','sip',"
                                " sip_subscriptions.full_to,sip_subscriptions.network_ip,sip_"
                                "subscriptions.network_port "
                                ...
                                "where sip_subscriptions.hostname='%q' and "
                                "sip_subscriptions.profile_name='%q' and "
                                "sip_subscriptions.event='presence' and "
                                "sip_subscriptions.full_from like '%%%q%%'",
                                switch_str_nil(status), switch_str_nil(rpid),
                                mod_sofia_globals.hostname, profile->name, from);
                        } else {
                            sql = switch_mprintf(
                                "update sip_subscriptions set version=version+1 where "
        ...
        for (m = matches->head; m; m = m->next) {
            ...
            if ((profile = sofia_glue_find_profile(m->val))) {
                ...
                if (call_info) {
                    if (uuid) {
                        sql = switch_mprintf(
                            "update sip_dialogs set call_info='%q',call_info_state='%q' where "
                            "hostname='%q' and profile_name='%q' and uuid='%q'",
                            call_info, call_info_state, mod_sofia_globals.hostname, profile->name,
                            uuid);
                    } else {
                        sql = switch_mprintf(
                            "update sip_dialogs set call_info='%q', call_info_state='%q' where "
                            "hostname='%q' and profile_name='%q' and "
                            "((sip_dialogs.sip_from_user='%q' and sip_dialogs.sip_from_host='%q') "
                            "or presence_id='%q@%q') and call_info='%q'",

                            call_info, call_info_state, mod_sofia_globals.hostname, profile->name,
                            euser, host, euser, host, call_info);
                    }
                ...
                if (!strcmp(proto, "dp")) {
                    sql = switch_mprintf(
                        "update sip_presence set rpid='%q',status='%q' where hostname='%q' and "
                        "profile_name='%q' and "
                ...
                if (zstr(uuid)) {
                    sql = switch_mprintf(
                        "select state,status,rpid,presence_id,uuid from sip_dialogs "
                        "where call_info_state != 'seized' and hostname='%q' and profile_name='%q' "
                        "and "
                    ...
                    sql = switch_mprintf(
                        "select state,status,rpid,presence_id,uuid from sip_dialogs "
                        "where uuid != '%q' and call_info_state != 'seized' and hostname='%q' and "
                        "profile_name='%q' and "
                        "((sip_from_user='%q' and sip_from_host='%q') or presence_id='%q@%q') "
                        "order by rcd desc",
                ...
                if (hup && dh.hits > 0) {
                    ...
                    if (!strcasecmp(dh.state, "early")) {
                        ...
                        switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "Answer-State",
                                                       "early");
                        switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "status",
                                                       status);
                        switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "Channel-State",
                                                       status);
                        switch_event_add_header_string(event, SWITCH_STACK_BOTTOM,
                                                       "Channel-Call-State", "EARLY");
                        switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "astate",
                                                       "early");
                        switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "answer-state",
                                                       "early");
                    } else {
                ...
                if (zstr(call_id) && (dh.hits && presence_source &&
                                      (!strcasecmp(presence_source, "register") ||
                                       switch_stristr("register", status)))) {
                    switch_console_free_matches(&matches);
                    sofia_glue_release_profile(profile);
                    goto done;
                }
                ...
                if (zstr(call_id)) {
                    sql = switch_mprintf(
                        "update sip_subscriptions set version=version+1 where hostname='%q' and "
                        "profile_name='%q' and "
                        "sip_subscriptions.event != 'line-seize' "
                        "and sip_subscriptions.proto='%q' and (event='%q' or event='%q') and "
                        "sub_to_user='%q' and "
                        ...
                        mod_sofia_globals.hostname, profile->name, proto, event_type,
                        alt_event_type, euser, host, profile->sipip,
                    ...
                    sql = switch_mprintf(
                        "select distinct "
                        "sip_subscriptions.proto,sip_subscriptions.sip_user,sip_subscriptions.sip_"
                        "host,"
                        "sip_subscriptions.sub_to_user,sip_subscriptions.sub_to_host,sip_"
                        "subscriptions.event,"
                        "sip_subscriptions.contact,sip_subscriptions.call_id,sip_subscriptions."
                        "full_from,"
                        "sip_subscriptions.full_via,sip_subscriptions.expires,sip_subscriptions."
                        "user_agent,"
                        "sip_subscriptions.accept,sip_subscriptions.profile_name"
                        ",'%q','%q','%q',sip_presence.status,sip_presence.rpid,sip_presence.open_"
                        "closed,'%q','%q',"
                        ...
                        "and (sub_to_host='%q' or sub_to_host='%q' or sub_to_host='%q' or "
                        "presence_hosts like '%%%q%%') ",

                        switch_str_nil(status), switch_str_nil(rpid), host, dh.status, dh.rpid,
                        dh.presence_id, mod_sofia_globals.hostname, profile->name, proto,
                    ...
                    sql = switch_mprintf(
                        ...
                        switch_str_nil(status), switch_str_nil(rpid), host, dh.status, dh.rpid,
                        dh.presence_id, mod_sofia_globals.hostname, profile->name, call_id);
                }
                ...
                if (mod_sofia_globals.debug_presence > 0) {
                    switch_log_printf(
                        SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "%s START_PRESENCE_SQL (%s)\n",
                ...
                if (mod_sofia_globals.debug_presence > 0) {
                    switch_log_printf(
                        SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "%s END_PRESENCE_SQL (%s)\n",
                        event->event_id == SWITCH_EVENT_PRESENCE_IN ? "IN" : "OUT", profile->name);
                }
...
static void do_flush(void) {
    ...
    while (mod_sofia_globals.presence_queue &&
           switch_queue_trypop(mod_sofia_globals.presence_queue, &pop) == SWITCH_STATUS_SUCCESS &&
           pop) {
...
void sofia_presence_event_thread_start(void) {
    ...
    switch_thread_create(&mod_sofia_globals.presence_thread, thd_attr,
                         sofia_presence_event_thread_run, NULL, mod_sofia_globals.pool);
}
...
void sofia_presence_event_handler(switch_event_t* event) {
    ...
    if (switch_queue_trypush(mod_sofia_globals.presence_queue, cloned_event) !=
        SWITCH_STATUS_SUCCESS) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
                          "Presence queue overloaded.... Flushing queue\n");
        switch_mutex_lock(mod_sofia_globals.mutex);
...
static int sofia_presence_sub_reg_callback(void* pArg, int argc, char** argv, char** columnNames) {
    ...
    if (!strcasecmp(event_name, "message-summary")) {
        if (switch_event_create(&event, SWITCH_EVENT_MESSAGE_QUERY) == SWITCH_STATUS_SUCCESS) {
            switch_event_add_header(event, SWITCH_STACK_BOTTOM, "Message-Account", "sip:%s@%s",
                                    user, host);
            switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "VM-Sofia-Profile",
                                           profile->name);
            switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "VM-sub-call-id", argv[7]);

            if (mod_sofia_globals.debug_presence > 0) {
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
                                  "Create MESSAGE QUERY EVENT...\n");
                DUMP_EVENT(event);
            }
    ...
    if (switch_event_create(&event, SWITCH_EVENT_PRESENCE_PROBE) == SWITCH_STATUS_SUCCESS) {
        ...
        switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "proto-specific-event-name",
                                       event_name);
...
static int sofia_presence_resub_callback(void* pArg, int argc, char** argv, char** columnNames) {
    ...
    if (mod_sofia_globals.debug_presence > 1) {
        for (i = 0; i < argc; i++) {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
                              "sofia_presence_resub_callback: %d [%s]=[%s]\n", i, columnNames[i],
                              argv[i]);
        }
    }
    ...
    if (do_event &&
        switch_event_create(&event, SWITCH_EVENT_PRESENCE_IN) == SWITCH_STATUS_SUCCESS) {
        switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "proto",
                                       proto ? proto : SOFIA_CHAT_PROTO);
        switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "login", profile->url);
        switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "sip_profile", profile->name);
        ...
        if (h->event) {
            for (hp = h->event->headers; hp; hp = hp->next) {
                if (!strncasecmp(hp->name, "fwd-", 4)) {
                    switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, hp->name + 4,
                                                   hp->value);
                }
            }
        }
...
static int sofia_dialog_probe_callback(void* pArg, int argc, char** argv, char** columnNames) {
    ...
    if (mod_sofia_globals.debug_presence > 1) {
        for (i = 0; i < argc; i++) {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
                              "sofia_dialog_probe_callback: %d [%s]=[%s]\n", i, columnNames[i],
                              argv[i]);
        }
    }
    ...
    if (zstr(to_user) || zstr(contact_user)) {
        switch_log_printf(
            ...
            "sofia_dialog_probe_callback: not enough info to generate a dialog entry\n");
        return 0;
    }

    // Usually we report the dialogs FROM the probed user.  The exception is when the monitored
    // endpoint is internal, and its presence_id is set in the dialplan.  Reverse the direction if
    // this is not a registered entity.
    if (!strcmp(direction, "inbound") && strcmp(user, from_user)) {
        // If inbound and the entity is not the caller (i.e. internal to FS), then the direction is
        // reversed because it is not going through the B2BUA
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
                          "sofia_dialog_probe_callback: endpt is internal\n");
    ...
    tmp = switch_core_sprintf(
        ...
        " </remote>\n"
        "</dialog>\n",
        data, uuid, call_id, to_tag, from_tag, direction, state, local_user, local_user, local_host,
        switch_str_nil(local_user_param), local_user, local_host,
        !strcasecmp(event_status, "hold") ? "no" : "yes", remote_display_buf, remote_user,
        remote_host, remote_uri);
...
static switch_uint31_t check_presence_epoch(void) {
    ...
    switch_uint31_t callsequence = {.value = (uint32_t)((now - mod_sofia_globals.presence_epoch) *
    ...
    if (!mod_sofia_globals.presence_year || callsequence.value >= SOFIA_PRESENCE_ROLLOVER_YEAR) {
        ...
        if (tm.tm_year != mod_sofia_globals.presence_year) {
            mod_sofia_globals.presence_epoch = (uint32_t)now - (tm.tm_yday * 86400) -
                                               (tm.tm_hour * 60 * 60) - (tm.tm_min * 60) -
                                               tm.tm_sec;
            mod_sofia_globals.presence_year = tm.tm_year;
            callsequence.value = (uint32_t)(((uint32_t)now - mod_sofia_globals.presence_epoch) *
                                            SOFIA_PRESENCE_COLLISION_DELTA);
        }
...
#define send_presence_notify(_a, _b, _c, _d, _e, _f, _g, _h, _i, _j, _k, _l)        \
    _send_presence_notify(_a, _b, _c, _d, _e, _f, _g, _h, _i, _j, _k, _l, __FILE__, \
                          __SWITCH_FUNC__, __LINE__)

static void _send_presence_notify(sofia_profile_t* profile, const char* full_to,
                                  const char* full_from, const char* o_contact, const char* expires,
                                  const char* call_id, const char* event, const char* remote_ip,
                                  const char* remote_port, const char* ct, const char* pl,
                                  const char* call_info, const char* file, const char* func,
                                  int line) {
    char sstr[128] = "";
    nua_handle_t* nh;
    int exptime = 0;
    ...
    if (!zstr(remote_ip) && sofia_glue_check_nat(profile, remote_ip)) {
        ...
        switch (transport) {
            ...
            case SOFIA_TRANSPORT_TCP_TLS:
                contact_str = sofia_test_pflag(profile, PFLAG_TLS) ? profile->tls_public_contact
        ...
        switch (transport) {
            case SOFIA_TRANSPORT_TCP:
                ...
                break;
            case SOFIA_TRANSPORT_TCP_TLS:
                contact_str = sofia_test_pflag(profile, PFLAG_TLS) ? profile->tls_contact
                                                                   : profile->tcp_contact;
                break;
            default:
    ...
    if (mod_sofia_globals.debug_presence > 1 || mod_sofia_globals.debug_sla > 1) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
                          ...
                          "\nfrom[%s]\nurl[%s]\ncall_id[%s]\nexpires_str[%s]\n"
                          "event[%s]\nct[%s]\npl[%s]\ncall_info[%s]\nexptime[%ld]\n",
                          file, func, line, profile->name, switch_str_nil(user_via), remote_ip,
                          remote_port, route_uri, o_contact, full_to, full_from, contact, call_id,
                          expires_str, event, switch_str_nil(ct), switch_str_nil(pl),
                          switch_str_nil(call_info), (long)exptime);
    }
    ...
    nh = nua_handle(profile->nua, NULL, NUTAG_URL(contact), SIPTAG_CONTACT_STR(contact_str),
                    TAG_END());
    cseq = sip_cseq_create(nua_handle_get_home(nh), callsequence, SIP_METHOD_NOTIFY);
    nua_handle_bind(nh, &mod_sofia_globals.destroy_private);

    nua_notify(
        nh, NUTAG_NEWSUB(1), TAG_IF(route_uri, NUTAG_PROXY(route_uri)),
        TAG_IF(dst->route, SIPTAG_ROUTE_STR(dst->route)),
        TAG_IF(user_via, SIPTAG_VIA_STR(user_via)), TAG_IF(path, SIPTAG_RECORD_ROUTE_STR(path)),

        SIPTAG_FROM_STR(full_to), SIPTAG_TO_STR(full_from),

        SIPTAG_CALL_ID_STR(call_id), TAG_IF(*expires_str, SIPTAG_EXPIRES_STR(expires_str)),
        SIPTAG_SUBSCRIPTION_STATE_STR(sstr), SIPTAG_EVENT_STR(event),
        TAG_IF(!zstr(ct), SIPTAG_CONTENT_TYPE_STR(ct)), TAG_IF(!zstr(pl), SIPTAG_PAYLOAD_STR(pl)),
        TAG_IF(!zstr(call_info), SIPTAG_CALL_INFO_STR(call_info)),
        TAG_IF(!exptime, SIPTAG_EXPIRES_STR("0")), SIPTAG_CSEQ(cseq), TAG_END());
...
static int sofia_dialog_probe_notify_callback(void* pArg, int argc, char** argv,
    ...
    send_presence_notify(sh->profile, full_to, full_from, contact, expires, call_id, event,
                         remote_ip, remote_port, ct, pl, NULL);
...
static char* gen_pidf(char* user_agent, char* id, char* url, char* open, char* rpid, char* prpid,
                      char* status, const char** ct) {
    char* ret = NULL;
...
static int sofia_presence_sub_callback(void* pArg, int argc, char** argv, char** columnNames) {
    ...
    if (mod_sofia_globals.debug_presence > 0) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE,
                          "SEND PRESENCE\nTo:      \t%s@%s\nFrom:    \t%s@%s\nCall-ID:  "
                          "\t%s\nProfile:\t%s [%s]\n\n",
                          user, host, sub_to_user, sub_to_host, call_id, profile_name,
                          helper->profile->name);
    }
    ...
    if (helper->hup && helper->calls_up > 0) {
        ...
        if (helper->event) {
            ...
            force_event_status =
                switch_str_nil(switch_event_get_header(helper->event, "force-status"));
        }
    }
    ...
    if (helper->event) {
        ...
        const char* direction =
            switch_str_nil(switch_event_get_header(helper->event, "presence-call-direction"));
        // const char *force_direction = switch_str_nil(switch_event_get_header(helper->event,
        // "force-direction"));
        ...
        const char* to_user =
            switch_str_nil(switch_event_get_header(helper->event, "variable_sip_to_user"));
        const char* from_user =
            switch_str_nil(switch_event_get_header(helper->event, "variable_sip_from_user"));
        const char* disable_early = switch_str_nil(
            switch_event_get_header(helper->event, "variable_presence_disable_early"));
        const char* answer_epoch =
            switch_str_nil(switch_event_get_header(helper->event, "variable_answer_epoch"));
        int answered = 0;
        ...
        if (!strcasecmp(direction, "inbound")) {
            from_id =
                switch_str_nil(switch_event_get_header(helper->event, "Caller-Destination-Number"));

        } else {
            from_id =
                switch_str_nil(switch_event_get_header(helper->event, "Caller-Caller-ID-Number"));
            from_name = switch_event_get_header(helper->event, "Caller-Caller-ID-Name");

            if (zstr(from_id)) {
                from_id = switch_str_nil(
                    switch_event_get_header(helper->event, "Other-Leg-Caller-ID-Number"));
            }
        ...
        if (is_dialog) {
            // Usually we report the dialogs FROM the probed user.  The exception is when the
            // monitored endpoint is internal, and its presence_id is set in the dialplan.  Reverse
            // the direction if this is not a registered entity.
            const char* caller =
                switch_str_nil(switch_event_get_header(helper->event, "caller-username"));
            SWITCH_STANDARD_STREAM(stream);
            if (!strcmp(direction, "inbound") && strcmp(sub_to_user, caller)) {
                // If inbound and the entity is not the caller (i.e. internal to FS), then the
                // direction is reversed because it is not going through the B2BUA
        ...
        if (!zstr(uuid)) {
            ...
            if ((sofia_test_pflag(profile, PFLAG_PRESENCE_DISABLE_EARLY) ||
                 switch_true(disable_early)) &&
                ((!zstr(astate) &&
                  (!strcasecmp(astate, "early") || !strcasecmp(astate, "ringing") ||
                   (!strcasecmp(astate, "terminated") && !answered))))) {
                switch_safe_free(stream.data);
                goto end;
            }
            ...
            if (!strcasecmp(astate, "early") || !strcasecmp(astate, "confirmed")) {
                ...
                if (is_dialog) {
                    if (!zstr(clean_to_user) && !zstr(clean_from_user)) {
                        ...
                        if (switch_true(
                                switch_event_get_header(helper->event, "Presence-Privacy"))) {
                            stream.write_function(&stream,
                                                  "<remote>\n<identity "
                                                  "display=\"Anonymous\">sip:anonymous@anonymous."
                                                  "invalid</identity>\n");
                        } else {
                            stream.write_function(
                                &stream,
                                "<remote>\n<identity display=\"%s\">sip:%s@%s</identity>\n",
                                from_name ? from_name : clean_from_user, clean_from_user, host);
                        }
                        stream.write_function(&stream, "<target uri=\"sip:**%s@%s\"/>\n",
                                              clean_to_user, host);
                        ...
                        stream.write_function(
                            &stream,
                            "<local>\n<identity "
                            "display=\"pickup\">sip:%s@%s;proto=pickup</identity>\n",
                            !zstr(clean_to_user) ? clean_to_user : "unknown", host);
                        ...
                        stream.write_function(
                            &stream,
                            "<local>\n<identity "
                            "display=\"conference\">sip:%s@%s;proto=conference</identity>\n",
                            !zstr(clean_to_user) ? clean_to_user : "unknown", host);
                        stream.write_function(
                            &stream, "<target uri=\"sip:%s@%s;proto=conference\">\n",
                            !zstr(clean_to_user) ? clean_to_user : "unknown", host);
                        stream.write_function(&stream,
                                              "<param pname=\"+sip.rendering\" "
                                              "pvalue=\"yes\"/>\n</target>\n</local>\n");
        ...
        if (!zstr(astate) && !zstr(uuid) && helper && helper->stream.data &&
            strcmp(helper->last_uuid, uuid) && strcasecmp(astate, "terminated") &&
            strchr(uuid, '-')) {
            helper->stream.write_function(&helper->stream,
                                          "update sip_dialogs set state='%q' where hostname='%q' "
                                          "and profile_name='%q' and uuid='%q';",
        ...
        if (!is_dialog) {
            ...
            if (in) {
                ...
                if (switch_false(resub)) {
                    const char* direction =
                        switch_event_get_header(helper->event, "Caller-Direction");
                    ...
                    if (!strcmp(astate, "early")) {
                        if (!zstr(op)) {
                            ...
                            if (sofia_test_pflag(profile, PFLAG_PRESENCE_PRIVACY)) {
                                ...
                                switch_snprintf(status_line, sizeof(status_line), "%s %s", what,
                                                op);
                            }
                        }
            ...
            pl = gen_pidf(user_agent, clean_id, contact_stripped, open, rpid, prpid, status_line,
                          &ct);
    ...
    if (!is_dialog && helper->event && !switch_stristr("registered", status_line)) {
        ...
        if (!zstr(uuid) && strchr(uuid, '-') && !zstr(status_line) && !zstr(rpid) &&
            (zstr(register_source) || strcasecmp(register_source, "register"))) {
            char* sql = switch_mprintf(
                "update sip_dialogs set rpid='%q',status='%q' where hostname='%q' and "
                "profile_name='%q' and uuid='%q'",
                rpid, status_line, mod_sofia_globals.hostname, profile->name, uuid);
            sofia_glue_execute_sql(profile, &sql, SWITCH_TRUE);
        }
    }

    send_presence_notify(profile, full_to, full_from, contact, expires, call_id, event, ip, port,
                         ct, pl, NULL);
...
static int sofia_presence_mwi_callback(void* pArg, int argc, char** argv, char** columnNames) {
    ...
    send_presence_notify(profile, full_to, full_from, contact, expires, call_id, event, remote_ip,
                         remote_port, "application/simple-message-summary", body, NULL);
...
static int broadsoft_sla_notify_callback(void* pArg, int argc, char** argv, char** columnNames) {
    ...
    if (data) {
        tmp = switch_core_sprintf(sh->pool, "%s,<sip:%s>;appearance-index=*;appearance-state=idle",
                                  data, host);
    } else {
        tmp = switch_core_sprintf(sh->pool, "<sip:%s>;appearance-index=*;appearance-state=idle",
                                  host);
    }
    ...
    if (mod_sofia_globals.debug_sla > 1) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
                          "DB PRES NOTIFY: [%s]\n[%s]\n[%s]\n[%s]\n[%s]\n[%s]\n[%s]\n[%s]\n[%s]\n",
                          argv[5], argv[6], argv[7], argv[8], call_id, event, argv[9], argv[10],
                          tmp);
    }

    send_presence_notify(sh->profile, argv[5], argv[6], argv[7], argv[8], call_id, event, argv[9],
                         argv[10], NULL, NULL, tmp);
...
static int broadsoft_sla_gather_state_callback(void* pArg, int argc, char** argv,
    ...
    if (strcasecmp(state, "idle") && uuid && (session = switch_core_session_locate(uuid))) {
        ...
        if (switch_channel_test_flag(channel, CF_ORIGINATOR) ||
            switch_channel_test_flag(channel, CF_BRIDGE_ORIGINATOR) ||
            switch_channel_inbound_display(channel) ||
        ...
        if (!zstr(callee_number)) {
            callee_number =
                switch_sanitize_number(switch_core_session_strdup(session, callee_number));
        }
    ...
    if (!zstr(callee_number)) {
        ...
        if (data) {
            ...
            tmp = switch_core_sprintf(
                sh->pool,
                "<sip:%s>;%s;appearance-state=%s;appearance-uri=\"\\\"%s\\\" <sip:%s@%s>\"", host,
                info, state, callee_name, callee_number, host);
        }
    } else {
        if (data) {
            tmp = switch_core_sprintf(sh->pool, "%s,<sip:%s>;%s;appearance-state=%s", data, host,
                                      info, state);
        } else {
            tmp =
                switch_core_sprintf(sh->pool, "<sip:%s>;%s;appearance-state=%s", host, info, state);
        }
    }
...
static int sync_sla(sofia_profile_t* profile, const char* to_user, const char* to_host,
    ...
    if (clear) {
        ...
        if (call_id) {
            sql = switch_mprintf(
                "update sip_subscriptions set version=version+1,expires=%ld where "
                "call_id='%q' "
                "and event='line-seize'",
                (long)switch_epoch_time_now(NULL), call_id);
            ...
            sql = switch_mprintf(
                ...
                "and event='line-seize'",
                (long)switch_epoch_time_now(NULL), mod_sofia_globals.hostname, profile->name,
                to_user, to_host);
        ...
        sql = switch_mprintf(
            "delete from sip_dialogs where hostname='%q' and profile_name='%q' and "
            "((sip_from_user='%q' and sip_from_host='%q') or presence_id='%q@%q') "
    ...
    sql = switch_mprintf(
        "select sip_from_user,sip_from_host,call_info,call_info_state,uuid from sip_dialogs "
        "where call_info_state is not null and call_info_state != '' and call_info_state != 'idle' "
        "and hostname='%q' and profile_name='%q' "
    ...
    sofia_glue_execute_sql_callback(profile, profile->dbh_mutex, sql,
                                    broadsoft_sla_gather_state_callback, sh);
    switch_safe_free(sql);
    ...
    if (!zstr(call_id)) {
        if (unseize) {
            sql = switch_mprintf(
                "select "
                "call_id,expires,sub_to_user,sub_to_host,event,full_to,full_from,contact,expires,"
                "network_ip,network_port "
        ...
        if (unseize) {
            sql = switch_mprintf(
                ...
                "and sub_to_user='%q' and sub_to_host='%q' "
                "and (event='call-info' or event='line-seize') and (profile_name='%q' or "
                "presence_hosts like '%%%q%%')",
                mod_sofia_globals.hostname, profile->name, to_user, to_host, profile->name,
                to_host);
        } else {
            sql = switch_mprintf(
                "select "
    ...
    sh->profile = profile;
    sofia_glue_execute_sql_callback(profile, profile->dbh_mutex, sql, broadsoft_sla_notify_callback,
                                    sh);
    switch_safe_free(sql);
    total = sh->total;
...
void sofia_presence_handle_sip_i_subscribe(int status, char const* phrase, nua_t* nua,
    ...
    if ((exp_delta = sip->sip_expires ? sip->sip_expires->ex_delta : 3600)) {
        if ((profile->force_subscription_expires > 0) &&
            (profile->force_subscription_expires < (uint32_t)exp_delta)) {
    ...
    if (!strcmp("as-feature-event", event)) {
        ...
        if (authorization) {
            ...
            auth_res = sofia_reg_parse_auth(profile, authorization, sip, de,
                                            (char*)sip->sip_request->rq_method_name, key,
                                            sizeof(key), network_ip, network_port, &v_event, 0,
                                            REG_REGISTER, to_user, NULL, NULL, NULL);
            if (v_event) switch_event_destroy(&v_event);
        } else if (sofia_reg_handle_register(nua, profile, nh, sip, de, REG_REGISTER, key,
                                             sizeof(key), &v_event, NULL, NULL, NULL)) {
            if (v_event) switch_event_destroy(&v_event);
            goto end;
        }
        ...
        if (authorization) {
            ...
            int network_port;
            sofia_glue_get_addr(de->data->e_msg, network_ip, sizeof(network_ip), &network_port);
            auth_res = sofia_reg_parse_auth(
                profile, authorization, sip, de, (char*)sip->sip_request->rq_method_name, key,
                keylen, network_ip, network_port, NULL, 0, REG_INVITE, NULL, NULL, NULL, NULL);
        } else if (sofia_reg_handle_register(nua, profile, nh, sip, de, REG_INVITE, key,
                                             (uint32_t)keylen, &v_event, NULL, NULL, NULL)) {
            if (v_event) {
    ...
    if (sofia_test_pflag(profile, PFLAG_PRESENCE_MAP) && !found_proto &&
        (alt_proto = switch_ivr_check_presence_mapping(to_user, to_host))) {
        orig_proto = proto;
        proto = alt_proto;
    }
    ...
    if ((sub_state != nua_substate_terminated)) {
        sql = switch_mprintf(
            "select contact from sip_subscriptions where call_id='%q' and profile_name='%q' and "
            "hostname='%q'",
            call_id, profile->name, mod_sofia_globals.hostname);
        sofia_glue_execute_sql2str(profile, profile->dbh_mutex, sql, buf, sizeof(buf));

        if (mod_sofia_globals.debug_presence > 0 || mod_sofia_globals.debug_sla > 0) {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "check subs sql: %s [%s]\n",
                              sql, buf);
        }
    ...
    if (sub_state == nua_substate_active) {
        ...
        sql = switch_mprintf(
            ...
            "set expires=%ld, "
            "network_ip='%q',network_port='%d',sip_user='%q',sip_host='%q',full_via='%q',full_to='%"
            "q',full_from='%q',contact='%q' "
            "where call_id='%q' and profile_name='%q' and hostname='%q'",
            (long)switch_epoch_time_now(NULL) + exp_delta, np.network_ip, np.network_port,
            from_user, from_host, full_via, full_to, full_from, contact,
        ...
        if (sub_state == nua_substate_terminated) {
            sql = switch_mprintf(
                "delete from sip_subscriptions where call_id='%q' and profile_name='%q' and "
                "hostname='%q'",
                call_id, profile->name, mod_sofia_globals.hostname);

            if (mod_sofia_globals.debug_presence > 0 || mod_sofia_globals.debug_sla > 0) {
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "sub del sql: %s\n", sql);
            }
            ...
            sql = switch_mprintf(
                ...
                "version,orig_proto, full_to) "
                "values "
                "('%q','%q','%q','%q','%q','%q','%q','%q','%q','%q','%q',%ld,'%q','%q','%q','%q','%"
                "d','%q',-1,'%q','%q;tag=%q')",
                proto, from_user, from_host, to_user, to_host,
                profile->presence_hosts ? profile->presence_hosts : "", event, contact_str, call_id,
                full_from, full_via, (long)switch_epoch_time_now(NULL) + exp_delta, full_agent,
                accept_header, profile->name, mod_sofia_globals.hostname, np.network_port,
                np.network_ip, orig_proto, full_to, use_to_tag);

            switch_assert(sql != NULL);

            if (mod_sofia_globals.debug_presence > 0 || mod_sofia_globals.debug_sla > 0) {
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE,
                                  "%s SUBSCRIBE %s@%s %s@%s\n%s\n", profile->name, from_user,
                                  from_host, to_user, to_host, sql);
            }
    ...
    if (sip->sip_event && sip->sip_event->o_type &&
        !strcasecmp(sip->sip_event->o_type, "ua-profile") && contact_host) {
        char* uri = NULL;
        ...
        if (uri) {
            ...
            switch_event_add_header_string(params, SWITCH_STACK_BOTTOM, "event-string",
            ...
            switch_event_add_header_string(params, SWITCH_STACK_BOTTOM, "extra-headers",
                                           extra_headers);
    ...
    if (status < 200) {
        ...
        if (np.is_nat) {
            ...
            sticky = switch_mprintf("sip:%s@%s%s%s:%d%s", contact_user, ipv6 ? "[" : "",
                                    np.network_ip, ipv6 ? "]" : "", np.network_port, params);
        }
        ...
        if (transport == SOFIA_TRANSPORT_TCP) {
            ...
            if (np.is_auto_nat) {
                cs = sofia_test_pflag(profile, PFLAG_TLS) ? profile->tls_public_contact
                                                          : profile->tcp_public_contact;
            } else {
                cs = sofia_test_pflag(profile, PFLAG_TLS) ? profile->tls_contact
                                                          : profile->tcp_contact;
            }
        }
        ...
        if (nh) {
            /* nua_dialog_usage_set_refresh_range(nh->nh_ds->ds_usage, exp_delta + SUB_OVERLAP,
             * exp_delta + SUB_OVERLAP); */
        ...
        if (sofia_test_pflag(profile, PFLAG_SUBSCRIBE_RESPOND_200_OK)) {
            nua_respond(nh, SIP_200_OK, SIPTAG_TO(sip->sip_to),
                        TAG_IF(new_contactstr, SIPTAG_CONTACT_STR(new_contactstr)),
                        NUTAG_WITH_THIS_MSG(de->data->e_msg), SIPTAG_SUBSCRIPTION_STATE_STR(sstr),
                        SIPTAG_EXPIRES_STR(exp_delta_str), TAG_IF(sticky, NUTAG_PROXY(sticky)),
                        TAG_END());
        } else {
        ...
        if (sub_state == nua_substate_terminated) {
            if (mod_sofia_globals.debug_presence > 0) {
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
                                  "Sending NOTIFY with Expires [0] and State [%s]\n", sstr);
            }
            ...
            if (zstr(full_agent) || (*full_agent != 'z' && *full_agent != 'Z')) {
                ...
                callsequence = sofia_presence_get_cseq(profile);
                cseq = sip_cseq_create(nua_handle_get_home(nh), callsequence, SIP_METHOD_NOTIFY);
                nua_notify(nh, SIPTAG_EXPIRES_STR("0"), SIPTAG_SUBSCRIPTION_STATE_STR(sstr),
    ...
    if (sub_state == nua_substate_terminated) {
        ...
        if (sip->sip_call_info) {
            full_call_info =
                sip_header_as_string(nua_handle_get_home(nh), (void*)sip->sip_call_info);
        ...
        if (!strcasecmp(event, "line-seize")) {
            ...
            if (sip->sip_call_info) {
                ...
                callsequence = sofia_presence_get_cseq(profile);
                cseq = sip_cseq_create(nua_handle_get_home(nh), callsequence, SIP_METHOD_NOTIFY);
                nua_notify(nh, SIPTAG_FROM(sip->sip_to), SIPTAG_TO(sip->sip_from),
                           SIPTAG_EXPIRES_STR(exp_delta_str), SIPTAG_CSEQ(cseq),
                           SIPTAG_SUBSCRIPTION_STATE_STR(sstr), SIPTAG_EVENT_STR("line-seize"),
                           TAG_IF(full_call_info, SIPTAG_CALL_INFO_STR(full_call_info)), TAG_END());
                ...
                sql = switch_mprintf(
                    ...
                    "(sip_from_user,sip_from_host,call_info,call_info_state,hostname,expires,rcd,"
                    "profile_name) "
    ...
    if (!strcasecmp(event, "as-feature-event")) {
        ...
        switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "hostname",
        ...
        if (sip->sip_payload) {
            ...
            if ((xml = switch_xml_parse_str(pd_dup, strlen(pd_dup)))) {
                ...
                if ((device = switch_xml_child(xml, "device"))) {
                    switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "device",
                                                   device->txt);
                }
                ...
                if (xml->name && !strcmp(xml->name, "SetDoNotDisturb")) {
                    ...
                    switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "Feature-Action",
                                                   "SetDoNotDisturb");
                    if ((action = switch_xml_child(xml, "doNotDisturbOn"))) {
                        switch_event_add_header_string(event, SWITCH_STACK_BOTTOM,
                                                       "Feature-Enabled", action->txt);
                        switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "Action-Name",
                                                       action->name);
                    }
                }
                ...
                if (xml->name && !strcmp(xml->name, "SetForwarding")) {
                    ...
                    if ((cfwd_type = switch_xml_child(xml, "forwardingType")) &&
                        ...
                        if (!strcmp(cfwd_type->txt, "forwardImmediate")) {
                            switch_event_add_header_string(event, SWITCH_STACK_BOTTOM,
                                                           "Feature-Enabled", cfwd_enable->txt);
                            switch_event_add_header_string(event, SWITCH_STACK_BOTTOM,
                                                           "Action-Name", "forward_immediate");
                            switch_event_add_header_string(event, SWITCH_STACK_BOTTOM,
                                                           "Action-Value", cfwd_target->txt);
                        } else if (!strcmp(cfwd_type->txt, "forwardBusy")) {
                            switch_event_add_header_string(event, SWITCH_STACK_BOTTOM,
                                                           "Feature-Enabled", cfwd_enable->txt);
                            switch_event_add_header_string(event, SWITCH_STACK_BOTTOM,
                                                           "Action-Name", "forward_busy");
                            switch_event_add_header_string(event, SWITCH_STACK_BOTTOM,
                                                           "Action-Value", cfwd_target->txt);
        ...
        if ((sql = switch_mprintf(
                 "select "
                 "proto,sip_user,'%q',sub_to_user,sub_to_host,event,contact,call_id,full_from,"
                 "full_via,expires,user_agent,accept,profile_name,network_ip"
                 ...
                 to_host, mod_sofia_globals.hostname, profile->name, to_user, to_host, to_host,
                 call_id))) {
            if (mod_sofia_globals.debug_presence > 0) {
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "SUBSCRIBE MWI SQL: %s\n",
                                  sql);
            }

            sofia_glue_execute_sql_callback(profile, profile->dbh_mutex, sql,
                                            sofia_presence_sub_reg_callback, profile);
    ...
    if (strcasecmp(event, "call-info") && strcasecmp(event, "line-seize")) {
        if (to_user && (strstr(to_user, "ext+") || strstr(to_user, "user+"))) {
            ...
            if (switch_event_create(&sevent, SWITCH_EVENT_PRESENCE_IN) == SWITCH_STATUS_SUCCESS) {
                ...
                if (!zstr(orig_proto)) {
                    switch_event_add_header_string(sevent, SWITCH_STACK_BOTTOM, "orig_proto",
                                                   orig_proto);
                }
                switch_event_add_header_string(sevent, SWITCH_STACK_BOTTOM, "login", profile->name);
                switch_event_add_header_string(sevent, SWITCH_STACK_BOTTOM, "sip_profile",
                                               profile->name);
                switch_event_add_header(sevent, SWITCH_STACK_BOTTOM, "from", "%s@%s", to_user,
                                        to_host);
            ...
            if (switch_event_create(&sevent, SWITCH_EVENT_PRESENCE_PROBE) ==
                SWITCH_STATUS_SUCCESS) {
                ...
                switch_event_add_header_string(sevent, SWITCH_STACK_BOTTOM, "sip_profile",
                                               profile->name);
                switch_event_add_header(sevent, SWITCH_STACK_BOTTOM, "from", "%s@%s", from_user,
                                        from_host);
                switch_event_add_header(sevent, SWITCH_STACK_BOTTOM, "to", "%s%s%s@%s", proto, "+",
                                        to_user, to_host);
                switch_event_add_header_string(sevent, SWITCH_STACK_BOTTOM,
                                               "proto-specific-event-name", event);
                switch_event_add_header_string(sevent, SWITCH_STACK_BOTTOM, "expires",
                                               exp_delta_str);
                switch_event_add_header_string(sevent, SWITCH_STACK_BOTTOM, "event_type",
                                               "presence");
                switch_event_add_header_string(sevent, SWITCH_STACK_BOTTOM, "alt_event_type",
                                               "dialog");
                switch_event_add_header_string(sevent, SWITCH_STACK_BOTTOM, "expires",
                                               exp_delta_str);
            ...
            if (!strcasecmp(event, "dialog")) {
                if (switch_event_create(&sevent, SWITCH_EVENT_PRESENCE_PROBE) ==
                    SWITCH_STATUS_SUCCESS) {
                    switch_event_add_header_string(sevent, SWITCH_STACK_BOTTOM, "probe-type",
                                                   "dialog");
                    switch_event_add_header_string(sevent, SWITCH_STACK_BOTTOM, "proto",
                                                   SOFIA_CHAT_PROTO);
                    switch_event_add_header_string(sevent, SWITCH_STACK_BOTTOM, "login",
                                                   profile->name);
                    switch_event_add_header_string(sevent, SWITCH_STACK_BOTTOM, "sip_profile",
                                                   profile->name);
                    switch_event_add_header(sevent, SWITCH_STACK_BOTTOM, "from", "%s@%s", from_user,
                                            from_host);
                    switch_event_add_header(sevent, SWITCH_STACK_BOTTOM, "to", "%s@%s", to_user,
                                            to_host);
                    switch_event_add_header_string(sevent, SWITCH_STACK_BOTTOM,
                                                   "proto-specific-event-name", event);
                    switch_event_add_header_string(sevent, SWITCH_STACK_BOTTOM, "expires",
                    ...
                    switch_event_add_header_string(sevent, SWITCH_STACK_BOTTOM, "expires",
                                                   exp_delta_str);
                    switch_event_add_header_string(sevent, SWITCH_STACK_BOTTOM, "sub-call-id",
                                                   call_id);
                    switch_event_fire(&sevent);
                }
                ...
                if (switch_event_create(&sevent, SWITCH_EVENT_PRESENCE_PROBE) ==
                    ...
                    switch_event_add_header_string(sevent, SWITCH_STACK_BOTTOM, "sip_profile",
                                                   profile->name);
                    switch_event_add_header_string(sevent, SWITCH_STACK_BOTTOM, "presence-source",
                                                   "subscribe");
                    switch_event_add_header(sevent, SWITCH_STACK_BOTTOM, "from", "%s@%s", from_user,
                                            from_host);
                    switch_event_add_header(sevent, SWITCH_STACK_BOTTOM, "to", "%s@%s", to_user,
...
sofia_gateway_subscription_t* sofia_find_gateway_subscription(sofia_gateway_t* gateway_ptr,
                                                              const char* event) {
...
void sofia_presence_handle_sip_r_subscribe(int status, char const* phrase, nua_t* nua,
                                           sofia_profile_t* profile, nua_handle_t* nh,
                                           sofia_private_t* sofia_private, sip_t const* sip,
                                           sofia_dispatch_event_t* de, tagi_t tags[]) {
    ...
    switch (status) {
        case 200:
        case 202:
            /* TODO: in the spec it is possible for the other side to change the original expiry
             * time, this needs to be researched (eg, what sip header this information will be in)
             * and implemented. Although, since it seems the sofia stack is pretty much handling the
             * subscription expiration anyway, then maybe its not even worth bothering.
             */
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
                              "got 200 OK response, updated state to SUB_STATE_SUBSCRIBE.\n");
        ...
        default:
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
                              "status (%d) != 200, updated state to SUB_STATE_FAILED.\n", status);
...
static int sofia_presence_send_sql(void* pArg, int argc, char** argv, char** columnNames) {
    ...
    send_presence_notify(cb->profile, argv[0], argv[1], argv[2], argv[3], argv[4], argv[5], argv[6],
...
uint32_t sofia_presence_contact_count(sofia_profile_t* profile, const char* contact_str) {
    ...
    sql = switch_mprintf(
        "select count(*) from sip_subscriptions where hostname='%q' and profile_name='%q' and "
        "contact='%q'",
...
void sofia_presence_handle_sip_i_publish(nua_t* nua, sofia_profile_t* profile, nua_handle_t* nh,
                                         sofia_private_t* sofia_private, sip_t const* sip,
                                         sofia_dispatch_event_t* de, tagi_t tags[]) {
    ...
    if ((profile->force_publish_expires > 0) &&
        (profile->force_publish_expires < (uint32_t)exp_delta)) {
    ...
    if (payload) {
        ...
        if ((xml = switch_xml_parse_str(pd_dup, strlen(pd_dup)))) {
            ...
            if ((tuple = switch_xml_child(xml, "tuple")) &&
                (status = switch_xml_child(tuple, "status")) &&
            ...
            if ((person = switch_xml_child(xml, "dm:person"))) {
                ...
                if ((act = switch_xml_child(person, "rpid:activities")) && act->child &&
                    act->child->name) {
            ...
            if (!strcasecmp(open_closed, "closed")) {
                ...
                if (sofia_test_pflag(profile, PFLAG_MULTIREG)) {
                    ...
                    if (count != 1) {
                        /* Don't broadcast offline when there is more than one client or one signing
                         * off makes them all appear to sign off on some clients */
                        count = 0;
                    } else {
            ...
            if (count) {
                ...
                if (sub_count > 0 &&
                    (sql = switch_mprintf(
                         "insert into sip_presence (sip_user, sip_host, status, rpid, expires, "
                         "user_agent,"
                ...
                sql = switch_mprintf(
                    "select full_to, full_from, contact, expires, call_id, event, network_ip, "
                    "network_port, "
                    "'application/pidf+xml' as ct,'%q' as pt "
                    " from sip_subscriptions where "
                    "hostname='%q' and profile_name='%q' and sub_to_user='%q' and sub_to_host='%q' "
                    "and event='%q'"
                    "and contact = '%q' ",

                    switch_str_nil(payload->pl_data), mod_sofia_globals.hostname, profile->name,
            ...
            if (sub_count > 0) {
                if (switch_event_create(&event, SWITCH_EVENT_PRESENCE_IN) ==
                    ...
                    switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "login",
                                                   profile->url);
                    switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "sip_profile",
                                                   profile->name);
                    switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "user-agent",
                                                   full_agent);
                    switch_event_add_header(event, SWITCH_STACK_BOTTOM, "from", "%s@%s", from_user,
                                            from_host);
        ...
        char* sql = switch_mprintf(
            "update sip_presence set expires=%ld where sip_user='%q' and sip_host='%q' and "
            "profile_name='%q' and hostname='%q'",
    ...
    if (sub_count > 0) {
        nua_respond(nh, SIP_200_OK, NUTAG_WITH_THIS_MSG(de->data->e_msg), SIPTAG_ETAG_STR(etag),
                    SIPTAG_EXPIRES_STR(expstr), TAG_END());
    } else {
        nua_respond(nh, SIP_404_NOT_FOUND, NUTAG_WITH_THIS_MSG(de->data->e_msg), TAG_END());
    }
...
void sofia_presence_set_hash_key(char* hash_key, int32_t len, sip_t const* sip) {
    ...
    switch_snprintf(hash_key, len, "%s%s%s", (from && from->url_user) ? from->url_user : "",
                    (from && from->url_host) ? from->url_host : "",
                    (to && to->url_user) ? to->url_user : "");
}

void sofia_presence_handle_sip_i_message(int status, char const* phrase, nua_t* nua,
                                         sofia_profile_t* profile, nua_handle_t* nh,
                                         switch_core_session_t* session,
                                         sofia_private_t* sofia_private, sip_t const* sip,
                                         sofia_dispatch_event_t* de, tagi_t tags[]) {
    if (sip) {
        ...
        if (sofia_test_pflag(profile, PFLAG_AUTH_MESSAGES)) {
            ...
            if ((auth_res != AUTH_OK && auth_res != AUTH_RENEWED)) {
                nua_respond(nh, SIP_401_UNAUTHORIZED, NUTAG_WITH_THIS_MSG(de->data->e_msg),
                            TAG_END());
                goto end;
            }
        ...
        if ((us = sofia_glue_get_unknown_header(sip, "X-FS-Sending-Message")) &&
            !strcmp(us, switch_core_get_uuid())) {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
                              "Not sending message to ourselves!\n");
            nua_respond(nh, SIP_503_SERVICE_UNAVAILABLE, NUTAG_WITH_THIS_MSG(de->data->e_msg),
                        TAG_END());
            return;
        }
        ...
        if (nh) {
            ...
            if (switch_event_create(&event, SWITCH_EVENT_MESSAGE) == SWITCH_STATUS_SUCCESS) {
                ...
                switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "from_sip_ip",
                                               network_ip);
                switch_event_add_header(event, SWITCH_STACK_BOTTOM, "from_sip_port", "%d",
                                        network_port);
                ...
                if (sip->sip_content_type && sip->sip_content_type->c_subtype) {
                    switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "type",
                ...
                for (un = sip->sip_unknown; un; un = un->un_next) {
                    if (!strncasecmp(un->un_name, "Diversion", 9)) {
                        ...
                        if (!zstr(un->un_value)) {
                            char* tmp_name;
                            if ((tmp_name = switch_mprintf("%s%s", SOFIA_SIP_HEADER_PREFIX,
                                                           un->un_name))) {
                                switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, tmp_name,
                                                               un->un_value);
                                free(tmp_name);
                            }
                        }
                        ...
                        if (first_history_info) {
                            /* If the header exists first time, make sure to remove old info and
                             * re-set the variable */
                            switch_event_add_header_string(event, SWITCH_STACK_BOTTOM,
                                                           "sip_history_info", un->un_value);
                            first_history_info = 0;
                        } else {
                            /* Append the History-Info into one long string */
                            const char* history_var =
                                switch_event_get_header(event, "sip_history_info");
                            if (!zstr(history_var)) {
                                char* tmp_str;
                                if ((tmp_str =
                                         switch_mprintf("%s, %s", history_var, un->un_value))) {
                                    switch_event_add_header_string(event, SWITCH_STACK_BOTTOM,
                                                                   "sip_history_info", tmp_str);
                                    free(tmp_str);
                                } else {
                                    switch_event_add_header_string(event, SWITCH_STACK_BOTTOM,
                                                                   "sip_history_info",
                        ...
                        switch_event_add_header_string(event, SWITCH_STACK_BOTTOM,
                                                       "sip_geolocation", un->un_value);
                    } else if (!strcasecmp(un->un_name, "Geolocation-Error")) {
                        switch_event_add_header_string(event, SWITCH_STACK_BOTTOM,
                                                       "sip_geolocation_error", un->un_value);
                    } else if (!strncasecmp(un->un_name, "X-", 2) ||
                               !strncasecmp(un->un_name, "P-", 2) ||
                        ...
                        if (!zstr(un->un_value)) {
                            ...
                            for (;;) {
                                ...
                                switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, new_name,
                                                               un->un_value);
                                break;
                            }
                        }
                ...
                if (sofia_test_pflag(profile, PFLAG_FIRE_MESSAGE_EVENTS)) {
                    if (switch_event_dup(&event_dup, event) == SWITCH_STATUS_SUCCESS) {
                        ...
                        switch_event_add_header_string(event_dup, SWITCH_STACK_BOTTOM, "Event-Name",
                                                       switch_event_name(event_dup->event_id));
            ...
            if (sofia_test_pflag(profile, PFLAG_IN_DIALOG_CHAT) &&
                (tech_pvt = (private_object_t*)switch_core_hash_find_locked(
                     profile->chat_hash, hash_key, profile->flag_mutex))) {
...
void sofia_presence_set_chat_hash(private_object_t* tech_pvt, sip_t const* sip) {
    ...
    if (!tech_pvt || tech_pvt->hash_key || !sip || !sip->sip_from ||
        !sip->sip_from->a_url->url_user || !sip->sip_from->a_url->url_host) {
        return;
    }

    if (sofia_reg_find_reg_url(tech_pvt->profile, sip->sip_from->a_url->url_user,
                               sip->sip_from->a_url->url_host, buf, sizeof(buf))) {
...
void sofia_presence_check_subscriptions(sofia_profile_t* profile, time_t now) {
    ...
    if (now) {
        ...
        sql = switch_mprintf(
            "select full_to, full_from, contact, -1, call_id, event, network_ip, network_port, "
            "NULL as ct, NULL as pt "
            " from sip_subscriptions where ((expires > 0 and expires <= %ld)) and "
            "profile_name='%q' and hostname='%q'",
            (long)now, profile->name, mod_sofia_globals.hostname);

        sofia_glue_execute_sql_callback(profile, profile->dbh_mutex, sql, sofia_presence_send_sql,
                                        &cb);
        switch_safe_free(sql);
